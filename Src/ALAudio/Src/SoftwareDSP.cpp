...
void FLT_Design_Cheb ( int Nmax, float cutoff, float ftype, float qwidth, int *pM, int *pL, int *a, int *b )
{
	...
	epass = sqrt( pow( 10.0, Apass/10 ) - 1 );
	estop = sqrt( pow( 10.0, Astop/10 ) - 1 );
...
dly_t * DLY_AllocLP ( int D, int a, int b, int type, int M, int L, int *fa, int *fb )
{
	...
	if (!h) 
	{ 
		debugf (TEXT("Sound DSP: Out of memory."));
		FLT_Free ( pdly->pflt );
		return NULL; 
	}
	...
	if ( !w )
	{ 
		debugf (TEXT("Sound DSP: Failed to lock."));
		GlobalFree(h);
		FLT_Free ( pdly->pflt );
		return NULL; 
	}
...
{ 
	...
	if (ppset)
		memset( ppset->w, 0, sizeof (int) * (CPSET_STATES) ); 
}
...
pset_t * PSET_Alloc ( int ipsettemplate )
{
	...
	*ppset = psettemplates[ipsettemplate];
	...
	if ( !fok )
	{
		...
		debugf (TEXT("Sound DSP: preset failed to init."));
		PRC_FreeAll ( ppset->prcs, ppset->cprcs );
		return NULL;
	}
...
void SoftwareDSP::Activate( BOOL bActivate )
{
	if (bActivate)
	{
		prc_t* prm = new prc_t[ num_blocks * 2 ];
		memset( prm, 0, sizeof(prc_t) * num_blocks * 2 );

		for (INT i=0; i<num_blocks; ++i)
		{						
			prm[i*2+0].type = blocks[i].type;
			memcpy( prm[i*2+0].prm, blocks[i].params, sizeof(prm->prm) );

			prm[i*2+1].type = blocks[i].type;
			memcpy( prm[i*2+1].prm, blocks[i].params, sizeof(prm->prm) );			
		}

		handle = NULL;

		if (PRC_InitAll( prm, num_blocks * 2 ))
		{
			handle = prm;
		}
		else
	...
	{
		prc_t* prm = (prc_t*)handle;

		PRC_FreeAll( prm, num_blocks * 2 );
		delete[] prm;		
...
FMOD_RESULT F_CALLBACK SoftwareDSP::DSPCallback(FMOD_DSP_STATE *dsp_state, float *inbuffer, float *outbuffer, unsigned int length, int inchannels, int outchannels)
{	
	SoftwareDSP* dsp;	
	...
	for (UINT count = 0; count < length * inchannels; count++) 
	{
		sample[count] = inbuffer[count] * 32767;
	}

	for (INT i=0; i<dsp->num_blocks; ++i)
	{
		if (prm[i*2+0].pfnGetNextN)
			prm[i*2+0].pfnGetNextN( prm[i*2+0].pdata, (portable_samplepair_t*)sample, length, OP_LEFT );

		if (prm[i*2+1].pfnGetNextN)
			prm[i*2+1].pfnGetNextN( prm[i*2+1].pdata, (portable_samplepair_t*)sample, length, OP_RIGHT );
	}	
	...
	{		
		for (UINT count = 0; count < length * inchannels; count++) 
		{ 		
			outbuffer[count] = sample[count] * gain / 32767.0f;
		} 
	}	
	else
	{
		...
		for (UINT count = 0; count < length * inchannels; count++) 
		{ 		
			float x = sample[count] * gain / 32767.0f;
			float y = inbuffer[count];
			outbuffer[count] = (x-y) * u + y;
		} 
	}
...
FMOD_RESULT F_CALLBACK SoftwareDSP::DSPSetParameterCallback(FMOD_DSP_STATE *dsp_state, int index, float value)
{
	SoftwareDSP* dsp;	
