...
protected:
	...
	/** Whether we are currently locked.								*/
	UBOOL bIsLocked;
};

/**
 * Scoped IO manager lock helper structure..
 */
struct FIOManagerScopedLock
{
	/**
	 * Constructor, locking IO manager.
	 */
	FIOManagerScopedLock( FIOManager* InIOManager )
	...
	/**
	 * Destructor, unlocking IO manager.
	 */
	~FIOManagerScopedLock()
	{
		check(GIOManager);
		IOManager->Unlock();
	}
...
struct FIOSystem
{
	 ...
	 */
	virtual UBOOL CancelRequests( QWORD* RequestIndices, INT NumIndices ) = 0;
...
struct FAsyncIOSystemBase : public FIOSystem, FRunnable
{
	 ...
	 * @return	TRUE if all requests were still outstanding, FALSE otherwise
	 */
	virtual UBOOL CancelRequests( QWORD* RequestIndices, INT NumIndices );
...
protected:
	...
	struct FAsyncIORequest
	{
		...
		/** File start sector on media, INDEX_NONE if not support.									*/
		INT					FileStartSector;
		/** Name of file.																			*/
		FString				FileName;
		/** Offset into file.																		*/
	 ...
	 */
	virtual UBOOL PlatformRead( FAsyncIOHandle FileHandle, INT Offset, INT Size, void* Dest ) = 0;
	...
	/**
	 * Pure virtual of platform specific code to determine the next request index to be fulfilled.
	 ...
	 */
	virtual INT PlatformGetNextRequestIndex() = 0;
	 ...
	 */
	void FulfillCompressedRead( const FAsyncIORequest& IORequest );
...
private:
	...
	INT		CurrentOffset;
	/** Last sector data was read from.							*/
	INT		LastReadSector;
};
#endif
