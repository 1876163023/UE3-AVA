	...
	INT AddChildNode( FClassTree* NewChild )
	{
		...
		if ( i == INDEX_NONE )
		{
			for ( i = 0; i < Children.Num(); i++ )
			{
				...
				if ( appStrcmp(*Child->GetClass()->GetName(), *NewChild->GetClass()->GetName()) >= 0 )
				{
					break;
				}
			}
...
public:
	 ...
	 */
	void GetChildClasses( TArray<FClassTree*>& ChildClasses )
	{
		ChildClasses.Empty();
		for ( INT i = 0; i < Children.Num(); i++ )
		{
	 ...
	 * @param	ChildClasses	[out] this node's children
	 */
	void GetChildClasses( TArray<const FClassTree*>& ChildClasses ) const
	{
		ChildClasses.Empty();
		for ( INT i = 0; i < Children.Num(); i++ )
		{
...
/**
 * Common class to use for FClassTree::GenerateMaskedClassTree and FClassTree::GetChildClasses.
 */
struct FClassFlagMask
{
	/** Only classes containing these flags (or subset of these flags) will be included */
	DWORD FlagsToInclude;

	/** Any classes containing these flags (or subset of these flags) will not be included */
	DWORD FlagsToExclude;

	/** determines how the include flags are checked */
	EClassFlagMatchType IncludeFlagType;

	/** determines how the exclude flags are checked */
	EClassFlagMatchType ExcludeFlagType;

	/**
	 * Determines whether the specified class contains the correct combination
	 * of class flags
	 * 
	 * @param	Node	the FClassTree corresponding to the class.
	 *
	 * @return	TRUE if this class has any/all of the include flags, and does not
	 *			have any/all of the exclude flags
	 */
	UBOOL IsValidClass( const FClassTree* Node ) const
	{
		const UClass* Class = Node->GetClass();

		// check to see if this class passes the include flag check
		UBOOL bResult = IncludeFlagType == MATCH_Any
			? Class->HasAnyClassFlags(FlagsToInclude)
			: Class->HasAllClassFlags(FlagsToInclude);

		if ( bResult )
		{
			// then make sure that this class passes the exclude flag check
			bResult = !(ExcludeFlagType == MATCH_Any
				? Class->HasAnyClassFlags(FlagsToExclude)
				: Class->HasAllClassFlags(FlagsToExclude));
		}
	 ...
	 * Constructor
	 * 
	 * @param	IncludeFlags		the class flags to include
	 * @param	ExcludeFlags		the class flags to exclude
	 * @param	IncludeMatchType	whether classes need any or all of the flags specified
	 * @param	ExcludeMatchType	whether classes are ignored if any or all of the flags specified
	 */
	FClassFlagMask
		(	
		DWORD IncludeFlags=CLASS_AllFlags,
		DWORD ExcludeFlags=CLASS_None, 
		EClassFlagMatchType IncludeMatchType=MATCH_Any, 
		EClassFlagMatchType ExcludeMatchType=MATCH_Any
		)
		: FlagsToInclude(IncludeFlags)
		, FlagsToExclude(ExcludeFlags)
		, IncludeFlagType(IncludeMatchType)
		, ExcludeFlagType(ExcludeMatchType)
	{}
};
