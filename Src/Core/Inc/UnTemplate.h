...
public:
	...
	void RemoveSwap( INT Index, INT Count, INT ElementSize, DWORD Alignment );
protected:
	void Realloc( INT ElementSize, DWORD Alignment )
	{
		...
		if( Data || ArrayMax )
		{
			checkSlow(((QWORD)ArrayMax*(QWORD)ElementSize < (QWORD)INT_MAX));
...
public:
	...
	void BulkSerialize(FArchive& Ar)
	{
		...
		if( Ar.Ver() >= VER_EPIC_CORE_MERGE_MAY_11 )
		{
	...
	void RemoveSwap( INT Index, INT Count=1 )
	{
		...
		if( TTypeInfo<T>::NeedsDestructor )
			for( INT i=Index; i<Index+Count; i++ )
				(&(*this)(i))->~T();
		FArray::RemoveSwap( Index, Count, sizeof(T), Alignment );
	}
...
public:
	...
	{
		checkSlow(ArrayNum>=0);
		checkSlow(!ArrayNum || !(((TCHAR*)Data)[ArrayNum-1]));
		checkSlow(ArrayMax>=ArrayNum);
	}
	void Empty( INT Slack=0 )
	{
...
protected:
	...
	void Relax()
	{
		// We only want to rehash if we need to reduce the hash size
		UBOOL bRequiresRehash = FALSE; 
		while( HashCount>Pairs.Num()*3+8 )
		{
	...
	// item removal for all kinds of fast removal
	// requires that this pair has already been unlinked from the hash chain
	void RemovePairIndex( INT PairIndex )
	{
		checkSlow(PairIndex >= 0 && PairIndex < Pairs.Num());
		INT LastPair = Pairs.Num() - 1;
		Pairs.RemoveSwap(PairIndex); 
		// now we need to go fix the hash link for the item swapped into cur from the end of the array (if any)
		if (PairIndex != LastPair)
		{
			TPair &FixPtr = Pairs(PairIndex);
			INT *TargetFix = &Hash[(GetTypeHash(FixPtr.Key) & (HashCount-1))];
			while (TRUE)
			{
				INT CurFix = *TargetFix;
				check(CurFix != INDEX_NONE); // we should always find what we are looking for
				if (CurFix == LastPair) // this is the item that needs to be fixed.
				{
					*TargetFix = PairIndex; // set link to new location
					break;
				}
				TargetFix = &Pairs(CurFix).HashNext; // advance link
			}
		}
	}
	// item removal based on index
	void RemoveByIndex( INT PairIndex )
	{
		checkSlow(PairIndex >= 0 && PairIndex < Pairs.Num());
		INT *Target = &Hash[(GetTypeHash(Pairs(PairIndex).Key) & (HashCount-1))];
		INT Cur = *Target;
		while (Cur != INDEX_NONE)
		{
			...
			if( Cur == PairIndex )
			{
				*Target = Next; // overwrite link, need to do this before we possibly move target
				RemovePairIndex(Cur);
				return;
			}
			Target = &P.HashNext; // advance link
			Cur = Next;
		}
		check(0); // we should always find it
	}
	// remove one item
	// optionally copy the value somewhere
	UBOOL RemoveOne( typename TTypeInfo<TK>::ConstInitType InKey, TI* ValueCopy = 0 )
	{
		if ( Pairs.Num() > 0 )
		{
			INT *Target = &Hash[(GetTypeHash(InKey) & (HashCount-1))];
			INT Cur = *Target;
			while (Cur != INDEX_NONE)
			{
				TPair &P = Pairs(Cur);
	...
	UBOOL RemoveOneValueMatch( typename TTypeInfo<TK>::ConstInitType InKey, typename TTypeInfo<TI>::ConstInitType InValue )
	{
		if ( Pairs.Num() > 0 )
		{
			INT *Target = &Hash[(GetTypeHash(InKey) & (HashCount-1))];
			...
			while (Cur != INDEX_NONE)
			{
				TPair &P = Pairs(Cur);
				INT Next = P.HashNext;
				if( P.Key==InKey && P.Value == InValue)
				{
					*Target = Next; // overwrite link, need to do this before we possibly move target
					RemovePairIndex(Cur);
					return TRUE;
...
public:
	...
	*/
	...
	// O(1) version (actually O(num removed))
	INT Remove( typename TTypeInfo<TK>::ConstInitType InKey )
	{
		INT Count=0;
		while (RemoveOne(InKey))
		{
			Count++;
		}
		if( Count )
	...
	*/
	...
	public:
		...
		void operator++()          { ++Index; }
		void RemoveCurrent()       { Map.RemoveByIndex(Index--); Removed++; }
...
public:
	...
	*/
	...
	{
		return AddItemEx(InKey, bAllowDuplicates);
	}
	INT AddItemEx( const T& InKey, UBOOL bAllowDuplicates=FALSE )
	{
...
public:
	...
	void MultiFind( const TK& Key, TArray<TI>& Values, UBOOL bCorrectArrayOrder=FALSE ) const
	{
		if( this->Hash && this->Pairs.Num() > 0 )
		{
			for( INT i=this->Hash[(GetTypeHash(Key) & (this->HashCount-1))]; i!=INDEX_NONE; i=this->Pairs(i).HashNext )
			{
				...
				{
					if ( !bCorrectArrayOrder )
					{
						new(Values)TI(this->Pairs(i).Value);
					}
					else
					{
						Values.InsertZeroed(0);
						Values(0) = TI(this->Pairs(i).Value);
					}
				}
			}
		}
	...
	*
	* @param	InKey				the element to add.
	* @param	InsertIndex			the index to insert the new element at, within the list of values for InKey.  If not specified,
	*								the element is added to the end.
	* @param	bAllowDuplicates	specify TRUE to allow the element to be inserted even if that element
	*								already exists at a different location in the array.
	*/
	void InsertItem( typename TTypeInfo<TK>::ConstInitType InKey, typename TTypeInfo<TI>::ConstInitType InValue, INT InsertIndex=INDEX_NONE, UBOOL bAllowDuplicates=FALSE )
	{
		if ( bAllowDuplicates || this->FindPair(InKey, InValue) == NULL )
		{
			// doesn't exist (or we allow duplicates); add to the map
			this->AllocHash();

			// ensure that we have a valid insertion point
			const INT NumValues = this->Num(InKey);
			if ( InsertIndex < 0 || InsertIndex > NumValues )
			{
				InsertIndex = NumValues;
			}

			// ActualInsertIndex is the index into the Pairs array where we'll insert this item
			...
			ActualInsertIndex = Clamp(ActualInsertIndex, 0, this->Pairs.Num());

			// insert into the list
			this->Pairs.InsertZeroed(ActualInsertIndex);
			this->Pairs(ActualInsertIndex).Key = InKey;
			this->Pairs(ActualInsertIndex).Value = InValue;
	...
	{
		INT Count=0;
		while (this->RemoveOneValueMatch(InKey,InValue))
		{
			Count++;
		}
		if( Count )
	...
	*/
	INT Num( TLookupMap<TK>& out_Keys ) const
	{
		for ( INT PairIndex = 0; PairIndex < this->Pairs.Num(); PairIndex++ )
		{
			out_Keys.AddItemEx(this->Pairs(PairIndex).Key);
		}
