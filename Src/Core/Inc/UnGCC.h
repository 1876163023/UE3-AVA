...
// Variable arguments.
#define GET_VARARGS(msg,len,lastarg,fmt) {va_list ap; va_start(ap,lastarg);VSNPRINTF(msg,len,fmt,ap);}
#define GET_VARARGS_ANSI(msg,len,lastarg,fmt) {va_list ap; va_start(ap,lastarg);vsnprintf(msg,len,fmt,ap);}
#define GET_VARARGS_RESULT(msg,len,lastarg,fmt,result) {va_list ap; va_start(ap, lastarg); INT size=VSNPRINTF(msg,len+1,fmt,ap); result=size>len?-1:size; }
