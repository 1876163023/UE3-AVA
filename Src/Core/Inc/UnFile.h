...
// Unreal 버젼의 IsAlpha는 한글을 지원하지 않는다.
// 그래서 appIsChar를 appIsAlpha 대신 추가하였다.
#if !defined(PS3)

inline UBOOL appIsChar( TCHAR c )
{
#if UNICODE
	return( iswalpha(c) != 0 );
#else
...
/** String formatted print, not limited to 1024 characters */
VARARG_DECL( INT, static INT, return, appSnprintf, VARARG_NONE, const TCHAR*, VARARG_EXTRA(INT Count) VARARG_EXTRA(TCHAR* Dest), VARARG_EXTRA(Count) VARARG_EXTRA(Dest) );
