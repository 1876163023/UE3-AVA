...
#else
#define KEEP_ALLOCATION_BACKTRACE	1
#endif
...
private:
	...
	void FlushPendingCommands()
	{
		if(PendingCommandLength)
		{
			...
			if (GDebugComm)
			{
				GDebugComm->SendChannelText("MEM", PendingCommandString);
			}
	...
	void AddBufferedCommand(const ANSICHAR* Format,...)
	{
		...
		while(TRUE)
		{
			...
			{
				CommandLength = appGetVarArgsAnsi(&PendingCommandString[PendingCommandLength],RemainingBufferSpace,Format,ArgList);
			}
	...
	void SendMallocCommand( DWORD Ptr, DWORD Size )
	{
		...
		PTRINT BackTrace[MAX_BACKTRACE_TRACK_DEPTH + BASE_BACKTRACE_TRACK_DEPTH];
...
//		default: FormatString = "&Allocate%08x%08x%08x%08x-"; break;
		};
		...
		AddBufferedCommand(
			...
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 0],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 1],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 2],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 3],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 4],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 5],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 6],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 7],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 8],
			BackTrace[BASE_BACKTRACE_TRACK_DEPTH + 9]
			);
...
public:
	 ...
	 */
	UBOOL	IsInternallyThreadSafe()
	{
		// since we are now threadsafe, pass the inner one on
	...
	void PushMemoryTag(const TCHAR* TagDescription)
	{
		...
		{
			bSendingCommand = TRUE;
			AddBufferedCommand("&PushTag%08x%s",appGetCurrentThreadId(),TCHAR_TO_ANSI(TagDescription));
			bSendingCommand = FALSE;
		}
	}
	void PopMemoryTag()
	{
		...
		{
			bSendingCommand = TRUE;
			AddBufferedCommand("&PopTag%08x",appGetCurrentThreadId());
			bSendingCommand = FALSE;
		}
	}
