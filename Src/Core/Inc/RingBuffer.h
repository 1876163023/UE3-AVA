...
public:
			  ...
			  AllocationSize(Align(InAllocationSize,InRingBuffer.Alignment))
		  {
			  ...
			  while(1)
			  {
				  ...
				  if(RingBuffer.WritePointer + AllocationSize > RingBuffer.DataEnd)
				  {
					  ...
					  // Use a memory barrier to ensure that the write to EndPointer is visible to the reading thread before the write to
					  // WritePointer.
					  appMemoryBarrier();
	...
	*/
	UBOOL BeginRead(volatile void*& OutReadPointer,UINT& OutReadSize)
	{
		...
		// Use a memory barrier to ensure that we won't see a newer value for WritePointer than EndPointer or the buffer contents.
		appMemoryBarrier();
