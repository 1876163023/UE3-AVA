...
public:
	...
	FORCEINLINE void  SetSkipObjectRef( UBOOL bSkip )			{ ArShouldSkipObjectRef = bSkip;}
	FORCEINLINE UBOOL ShouldSkipObjectRef()				const	{return ArShouldSkipObjectRef;}
	//>@ ava
	...
	FORCEINLINE friend FArchive& operator<<( FArchive& Ar, CipherByte& B )
	{
		Ar.Serialize( &B, 1 );
		return Ar;
	}
...
protected:
	...
	void Reset(void)
	{
		...
		//<@ ava specific ; 207. 1. 24 changmin
		ArShouldSkipObjectRef			= FALSE;
		//>@ ava
	}
};
