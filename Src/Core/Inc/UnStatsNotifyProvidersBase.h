...
protected:
	 ...
	 */
	FStatNotifyProvider(const TCHAR* InProviderName);
...
public:
	 ...
	 * @param Value the value of the stat to write out
	 */
	virtual void WriteStat(DWORD StatId,FLOAT Value) = 0;
	 ...
	 * @param Value the value of the stat to write out
	 */
	virtual void WriteStat(DWORD StatId,DWORD Value) = 0;
...
public:
	...
	FStatNotifyManager(void) :
		FStatNotifyProvider(TEXT("StatNotifyManager"))
	{
	}
	/**
