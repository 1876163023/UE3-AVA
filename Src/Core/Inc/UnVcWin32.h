...
#define NOMINMAX
#endif
#include <winsock2.h> // 20060919 dEAthcURe
#include <windows.h>
#include <tchar.h>
#include <stdlib.h>
#include <math.h>
...
// Common method name mapping for platform independence
...
struct CipherByte {
	BYTE data;
	void encrypt(const BYTE b) {
		/*extern void onEncrypt(const BYTE valueBefore, const BYTE valueToEnc);
		onEncrypt(data, b);*/
		//data = b ^ 0xff;		
		BYTE i1 = (((b & 0x55) ^ 0x44) << 1);
		BYTE i2 = (((b & 0xaa) ^ 0x88) >> 1);	
		data = i1 | i2;
	}

	BYTE decrypt(void) const {
		//return data ^ 0xff;
		BYTE i1 = ((data >> 1) ^ 0x44) & 0x55;
		BYTE i2 = ((data << 1) ^ 0x88) & 0xaa;
		return i1 | i2;		
	}
	
	CipherByte& operator=(BYTE b) {
		encrypt(b);		
		return *this;
	}

	/*CipherByte& operator=(const int b) {		
		encrypt((BYTE)b);		
		return *this;
	}*/

	friend CipherByte operator+(const CipherByte& a, const CipherByte& b) {
		BYTE r = a.decrypt() + b.decrypt();
		CipherByte rr = r;
		return rr;
	}
 ...
 */ 
void appProgramCounterToHumanReadableString( DWORD64 ProgramCounter, ANSICHAR* HumanReadableString, EVerbosityFlags VerbosityFlags = VF_DISPLAY_ALL );
 ...
 */
void appCaptureStackBackTrace( DWORD64* BackTrace, DWORD MaxDepth, CONTEXT* Context = NULL );
 ...
 * @param	Context				Optional thread context information
 */ 
void appStackWalkAndDump( ANSICHAR* HumanReadableString, INT IgnoreCount, CONTEXT* Context = NULL );
