...
//
typedef void (UObject::*Native) _ParamList_ScriptFunctionImpl; // 20080221 dEAthcURe|CD // ( FFrame& TheStack, RESULT_DECL, UProperty* pPropertyLefthand, BYTE* pPropAddrLefthand);
...
void GInitRunaway();
FORCEINLINE void FFrame::Step _ParamList_FramStepImpl // 20080221 dEAthcURe|CD // (UObject *Context, RESULT_DECL, UProperty* pPropertyLefthand, BYTE* pPropAddrLefthand)
{
	INT B = *Code++;
	#ifdef EnableCipherData
	(Context->*GNatives[B])(*this,Result, pPropertyLefthand, pPropAddrLefthand);
	#else
