...
//#define EnableFileCache // 20080110 dEAthcURe|FC
//#define EnableFsProfile // 20080110 dEAthcURe|FP
#define EnableCipherData // 20080221 dEAthcURe|CD
#define EnableFileServiceProvider // 20080305 dEAthcURe|FSP 

#ifdef EnableFileServiceProvider
#undef EnableFileCache // FSP 활성화 상태에서는 fileCache를 죽인다.
#endif


#ifdef EnableCipherData
#define _ParamList_ScriptFunctionDef ( FFrame& Stack, RESULT_DECL, UProperty* pPropertyLefthand = 0x0, BYTE* pPropAddrLefthand = 0x0 )
#define _ParamList_ScriptFunctionImpl ( FFrame& Stack, RESULT_DECL, UProperty* pPropertyLefthand, BYTE* pPropAddrLefthand )
#define _ParamList_FramStepDef ( UObject *Context, RESULT_DECL, UProperty* pPropertyLefthand = 0x0, BYTE* pPropAddrLefthand = 0x0 )
#define _ParamList_FramStepImpl ( UObject *Context, RESULT_DECL, UProperty* pPropertyLefthand, BYTE* pPropAddrLefthand )
#else 
#define _ParamList_ScriptFunctionDef ( FFrame& Stack, RESULT_DECL )
#define _ParamList_ScriptFunctionImpl ( FFrame& Stack, RESULT_DECL )
#define _ParamList_FramStepDef ( UObject *Context, RESULT_DECL )
#define _ParamList_FramStepImpl ( UObject *Context, RESULT_DECL )
#endif
...
// {{ 20080215 dEAthcURe|CD
#ifdef EnableCipherData
static inline BYTE          CheckVA(CipherByte eb)      { return eb.decrypt();}
#endif
// }} 20080215 dEAthcURe|CD
...
public:
	...
	/**
	* Logs the current callstack for later processing - currently only works with FMallocDebugProxyWindows
	*/
	virtual void LogBacktrace( void* Ptr, DWORD Size ) {}
	/**
	* Dumps the data collected by LogBacktrace to a log file
	*/
	virtual void DumpBacktraces( void ) {}
	/** Track all mallocs from the current thread */
	virtual void BeginTrackingThread( void ) {}
	/** Stop tracking all mallocs from a given thread */
	virtual void EndTrackingThread( void ) {}
	/** Pushes a memory tag onto the top of the stack to be applied to every allocation until it's popped. */
	virtual void PushMemoryTag(const TCHAR* TagDescription) {}
	/** Pops a memory tag from the top of the stack. */
	virtual void PopMemoryTag() {}
	virtual void DumpAllocs( UBOOL bSummaryOnly=TRUE, FOutputDevice& Ar=*GLog ) {}
...
public:
	...
	*/
	 ...
	 */
	virtual void CoalesceFilesFromDisk(const TCHAR* ConfigDir, UBOOL bNeedsByteSwapping)=0;
...
extern TCHAR					GGPUIni[1024];
extern TCHAR					GNetIni[1024];
...
/** GPU Perf analyzer가 연결되어 있을 때 */
extern UBOOL					GGPUPerfAnalyzerLoaded;
#else
static const UBOOL				GEmitDrawEvents = FALSE;
static const UBOOL				GGPUPerfAnalyzerLoaded = FALSE;
#endif
...
// Normal includes.
#include "ScopedMemoryTag.h"			// Scoped memory allocation tagging.
...
public:
	 ...
	 */
	virtual void MapCheck_Add(MapCheckType InType, void* InActor, const TCHAR* InMessage, MapCheckAction bRecommendDeletion=MCACTION_NONE, const TCHAR* InUDNPage=TEXT("")) {};
