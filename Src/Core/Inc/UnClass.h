...
{
	DECLARE_ABSTRACT_CLASS(UField,UObject,CLASS_Intrinsic,Core)
	...
	// Constructors.
	UField( ENativeConstructor, UClass* InClass, const TCHAR* InName, const TCHAR* InPackageName, EObjectFlags InFlags, UField* InSuperField );
...
private:
	...
	UObject* CreateArchetype( const TCHAR* ArchetypeName, UObject* ArchetypeOuter, UObject* AlternateArchetype=NULL ) 
	{
		check(0);
		...
		return Super::CreateArchetype(ArchetypeName, ArchetypeOuter, AlternateArchetype);
	}
};
...
enum EStructFlags
{
	...
	STRUCT_Inherit			= STRUCT_HasComponents|STRUCT_Atomic,
};
...
{
	DECLARE_CLASS(UStruct,UField,CLASS_Intrinsic,Core)
...
{
	DECLARE_CLASS(UScriptStruct,UStruct,CLASS_Intrinsic,Core)
	...
	UScriptStruct( UScriptStruct* InSuperStruct );
...
{
	DECLARE_CLASS(UFunction,UStruct,CLASS_Intrinsic|CLASS_IsAUFunction,Core)
	...
	void (UObject::*Func)_ParamList_ScriptFunctionImpl; // 20080221 dEAthcURe|CD // void (UObject::*Func)( FFrame& Stack, RESULT_DECL, UProperty* pPropertyLefthand, BYTE* pPropAddrLefthand );
...
{
	DECLARE_CLASS(UState,UStruct,CLASS_Intrinsic|CLASS_IsAUState,Core)
...
{
	DECLARE_CLASS(UEnum,UField,CLASS_Intrinsic,Core)
...
public:
	...
	/**
	 * @reutrn	 The number of enum names.
	 */
	INT NumEnums() const
	{
		return Names.Num();
...
{
	DECLARE_CLASS(UClass,UState,CLASS_Intrinsic,Core)
	DECLARE_WITHIN(UPackage)

	// Variables.
	DWORD				ClassFlags;
	INT					ClassUnique;
	BYTE				ClassPlatformFlags;
	UClass*				ClassWithin;
	 ...
	 **/
	TMap<UClass*,class UProperty*> Interfaces;
	...
	/** Reference token stream used by realtime garbage collector, assembled in AssembleReferenceTokenStream */
	FGCReferenceTokenStream ReferenceTokenStream;
	/** Reference token stream for properties defined in this class */	
	FGCReferenceTokenStream ThisOnlyReferenceTokenStream;

	// Constructors.
	UClass();
	UClass( UClass* InSuperClass );
	UClass( ENativeConstructor, DWORD InSize, DWORD InClassFlags,  UClass* InBaseClass, UClass* InWithinClass, BYTE InPlatformFlags, const TCHAR* InNameStr, const TCHAR* InPackageName, const TCHAR* InClassConfigName, EObjectFlags InFlags, void(*InClassConstructor)(void*), void(UObject::*InClassStaticConstructor)(), void(UObject::*InClassStaticInitializer)() );
	UClass( EStaticConstructor, DWORD InSize, DWORD InClassFlags,                                              BYTE InPlatformFlags, const TCHAR* InNameStr, const TCHAR* InPackageName, const TCHAR* InClassConfigName, EObjectFlags InFlags, void(*InClassConstructor)(void*), void(UObject::*InClassStaticConstructor)(), void(UObject::*InClassStaticInitializer)() );
	...
	{
		static FName NAME_GPU( TEXT("GPU") );
		static FName NAME_Net( TEXT("NET") );
		...
		else if( ClassConfigName == NAME_GPU )
		{
			return GGPUIni;
		}
		else if( ClassConfigName == NAME_Net )
		{
			return GNetIni;
		}
		...
		else
		{
			...
			appCheckIniForOutdatedness( *ConfigDefaultName, *ConfigGameName, YesNoAll );
	...
	{
		return (ClassFlags & FlagsToCheck) != 0 || FlagsToCheck == CLASS_AllFlags;
	}

	/**
	 * Determines whether this native function can be bound to C++ implementation on the current platform.
	 *
	 * @return	TRUE if this is a native function that is implemented on the current platform (as determined by build defines)
	 */
	FORCEINLINE UBOOL IsImplementedOnCurrentPlatform() const
	{
		UE3::EPlatformType CurrentType = appGetPlatformType();
		return (ClassPlatformFlags&CurrentType) != 0 || ClassPlatformFlags == 0;
	}
	...
	**/
	inline UBOOL ImplementsInterface( class UClass* SomeInterface ) const
	{
		if ( SomeInterface != NULL && SomeInterface->HasAnyClassFlags(CLASS_Interface) && SomeInterface != UInterface::StaticClass() )
		{
			for ( const UClass* CurrentClass = this; CurrentClass; CurrentClass = CurrentClass->GetSuperClass() )
			{
				...
				for( TMap<UClass*,UProperty*>::TConstIterator It(CurrentClass->Interfaces); It; ++It )
				{
					UClass* InterfaceClass = It.Key();
					if( InterfaceClass->IsChildOf(SomeInterface) == TRUE )
					{
						return TRUE;
					}
				}
...
{
	DECLARE_CLASS(UConst,UField,CLASS_Intrinsic,Core)
	DECLARE_WITHIN(UStruct)
