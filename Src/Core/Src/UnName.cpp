...
FString FName::ToString() const
{
	...
	const TCHAR *TmpName = Names(Index)->Name;
	FString Out;
	Out.Empty(appStrlen(TmpName) + 6);
	Out += TmpName;
...
FArchive& operator<<( FArchive& Ar, FNameEntry& E )
{
	...
	{		
		...
		if (!Ar.IsNet() && Ar.Ver() >= VER_AVA_ALTER_PACKAGELAYOUT)
		{
			BYTE Temp[16];
			INT Length = ((Str.Len() & 0xf) ^ 0x7);

			Ar.Serialize( Temp, Length );
		}
	}
	else
	{
...
{
	INT NameEntrySize	  = sizeof(FNameEntry) - (NAME_SIZE - appStrlen(Name) - 1)*sizeof(TCHAR);
