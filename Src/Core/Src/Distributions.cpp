...
void FRawDistribution::GetValue1(FLOAT Time, FLOAT* Value, INT Extreme)
{
	switch (Op)
	{
	case RDO_None:
...
void FRawDistribution::GetValue3(FLOAT Time, FLOAT* Value, INT Extreme)
{
	switch (Op)
	{
	case RDO_None:
...
void FRawDistribution::GetValue1Extreme(FLOAT Time, FLOAT* InValue, INT Extreme)
{
...
void FRawDistribution::GetValue3Extreme(FLOAT Time, FLOAT* InValue, INT Extreme)
{
	FLOAT* RESTRICT Value = InValue;
	const LOOKUPVALUE* Entry1;
	const LOOKUPVALUE* Entry2;
...
void FRawDistribution::GetValue1Random(FLOAT Time, FLOAT* InValue)
{
	FLOAT* RESTRICT Value = InValue;
	const LOOKUPVALUE* Entry1;
	const LOOKUPVALUE* Entry2;
...
void FRawDistribution::GetValue3Random(FLOAT Time, FLOAT* InValue)
{
	FLOAT* RESTRICT Value = InValue;
	const LOOKUPVALUE* Entry1;
	const LOOKUPVALUE* Entry2;
 ...
 */
void FRawDistribution::GetValue(FLOAT Time, FLOAT* Value, INT NumCoords, INT Extreme)
{
...
void FRawDistributionFloat::Initialize()
{
	...
	// @todo: I believe this can go away once every package has been resaved, BUT TEST TO MAKE SURE
	// @todo: Make sure this works with particle prefabs (is a prefab IsTemplate? will it be ticked
	//        every frame? Need to ask people that are at E3 :))
...
FLOAT FRawDistributionFloat::GetValue(FLOAT F, UObject* Data)
{
#if !CONSOLE
	...
	FLOAT Value;
	FRawDistribution::GetValue1(F, &Value);
	return Value;
}
...
void UDistributionFloat::Serialize(FArchive& Ar)
{
	...
	// if we wnt to force rebuild the FRawDistributions, mark the UDistribution as dirty
...
void UDistributionFloat::PostEditChange(UProperty* PropertyThatChanged)
{
...
FVector FRawDistributionVector::GetValue(FLOAT F, UObject* Data, INT Extreme)
{
#if !CONSOLE
	// make sure it's up to date
	if( GIsEditor )
	...
	FVector Value;
	FRawDistribution::GetValue3(F, &Value.X);
	return Value;
}
...
void UDistributionVector::PostEditChange(UProperty* PropertyThatChanged)
{
	Super::PostEditChange(PropertyThatChanged);
	bIsDirty = TRUE;
}

FVector UDistributionVector::GetValue(FLOAT F, UObject* Data, INT Extreme)
{
