...
#include "..\\..\\Engine\\Inc\\loadingProgress.h" // 20070115 dEAthcURe
/*-----------------------------------------------------------------------------
	Globals.
-----------------------------------------------------------------------------*/
...
INT UObject::GhmId = -1; // 20070731 dEAthcURe|HM matinee에 사용할 hm id, actor의 hmIndex에도 넣을것.
...
{
	if ( HasAnyFlags(RF_ClassDefaultObject|RF_ArchetypeObject) && PropertyAboutToChange.GetActiveMemberNode() == PropertyAboutToChange.GetHead() )
	{
...
void UObject::PostEditChange( FEditPropertyChain& PropertyThatChanged )
{
	...
	if ( HasAnyFlags(RF_ClassDefaultObject|RF_ArchetypeObject) && PropertyThatChanged.GetActiveMemberNode() == PropertyThatChanged.GetHead() )
	{
...
void UObject::ConditionalPostLoadSubobjects( FObjectInstancingGraph* OuterInstanceGraph/*=NULL*/ )
{
	...
	if ( HasAnyFlags(RF_NeedPostLoadSubobjects) )
	{
		...
		FObjectInstancingGraph CurrentInstanceGraph(this);
...
void UObject::Modify( UBOOL bAlwaysMarkDirty/*=FALSE*/ )
{
	...
	if ( (GetOutermost()->PackageFlags & PKG_PlayInEditor) == 0 )
	{
		...
		&&	HasAnyFlags(RF_Transactional) && GIsEditor /** 에디터일때만 변경가능 */ )
		{
			MarkPackageDirty();
...
UBOOL UObject::StaticExec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	if( ParseCommand(&Str,TEXT("MEM")) )
	{
		...
		{
			GMalloc->DumpAllocs( bSummaryOnly, Ar );
	...
	else if( ParseCommand(&Str,TEXT("DUMPBACKTRACES")) )
	{
		GMalloc->DumpBacktraces();
		return 1;
	}
	...
	else if( ParseCommand(&Str,TEXT("OBJ")) )
	{
		...
		else if( ParseCommand(&Str,TEXT("LIST")) )
		{
			...
			&&	(InsidePackage	|| (!appStrfind(Str,TEXT("PACKAGE=")) && !appStrfind(Str,TEXT("INSIDE=")))) )
			{
...
UBOOL UObject::ResolveName( UObject*& InPackage, FString& InName, UBOOL Create, UBOOL Throw )
{
	...
	else if( appStrnicmp( *InName, TEXT("gpu-ini:"), appStrlen(TEXT("gpu-ini:")) )==0 )
	{
		IniFilename = GGPUIni;
	}
	else if( appStrnicmp( *InName, TEXT("net-ini:"), appStrlen(TEXT("net-ini:")) )==0 )
	{
		IniFilename = GNetIni;
	}
...
UPackage* UObject::LoadPackage( UPackage* InOuter, const TCHAR* Filename, DWORD LoadFlags )
{
...
#endif
	{
		...
		EndLoad();
		dLoadingProgress(lpdEndLoad); // LoadingProgressTo(TEXT("EndLoad()")); // g_loadingProgress.progress("EndLoad()"); // dEAthcURe
...
#endif
		{
			...
			while( GObjLoaded.Num() )
			{
				...
				for( INT i=0; i<ObjLoaded.Num(); i++ )
				{
					...
					dLoadingProgress(lpdPrePostLoad); // 20070510 dEAthcURe
				}
...
{
	SCOPED_MEMORY_TAG(*FString::Printf(
		TEXT("StaticConstructObject(%s)"),
		*InClass->GetName()
		));
