...
#define TRACK_BULKDATA_USE !FINAL_RELEASE


#if TRACK_BULKDATA_USE

/** Map from bulk data pointer to object it is contained by */
TDynamicMap<FUntypedBulkData*,UObject*> BulkDataToObjectMap;
...
void FUntypedBulkData::DumpBulkDataUsage( FOutputDevice& Log )
{
	...
	for( TDynamicMap<FUntypedBulkData*,UObject*>::TIterator It(BulkDataToObjectMap); It; ++It )
	{
	...
	for( INT PerObjectIndex=0; PerObjectIndex<PerObjectSizeArray.Num(); PerObjectIndex++ )
	{
		...
		Log.Logf( TEXT("  %5d KByte of bulk data for object %s"), PerObjectSize.Size / 1024, *PerObjectSize.Object->GetPathName() );
	}
	debugf(TEXT(""));
#endif
}
 ...
 */
void FUntypedBulkData::SerializeLikeLazyArray( FArchive &Ar, UObject* Owner )
{
	check( Ar.IsLoading() );
	check( Ar.IsPersistent() );

	// Initialize variables filled in by regular serialization code.
	BulkDataFlags			= BULKDATA_ForceSingleElementSerialization;
	ElementCount			= INDEX_NONE;
	BulkDataSizeOnDisk		= INDEX_NONE;
	BulkDataOffsetInFile	= INDEX_NONE;

	// Serialize EndPosition, which is later used in case lazy loading is allowed.
	INT EndPosition			= INDEX_NONE;
	Ar << EndPosition;
	check( EndPosition != INDEX_NONE );

	// Bulk data size on disk, can be different from bulk data size in the presence of compression.
	...
	if( Ar.Ver() >= VER_LAZYARRAY_SERIALIZATION_CHANGE )
	{
		...
		Ar << LazyLoaderFlags;
		// Don't support backward compatibility with bulk data stored in a separate file as this should only
		// happen for cooked data in which case it can easily be recooked.
		check( !(LazyLoaderFlags & LLF_PayloadInSeparateFile) );
		// Propagate compression flag.
		if( LazyLoaderFlags & LLF_SerializeCompressed )
		{
			BulkDataFlags |= BULKDATA_SerializeCompressedZLIB;
		}
	}
	...
	// Serialize number of elements which in combination with the size of a single element is used to
	// calculate the size of the bulk data.
	Ar << ElementCount;

	// Unknown size on disk so we can rely on the data not being compressed in which case the serialized
	// size hopefully matches the size of the bulk data. This might not be the case for data that isn't
	// compatible with bulk serialization but content like this shouldn't be brought over to use this.
	if( BulkDataSizeOnDisk == INDEX_NONE )
	{
		BulkDataSizeOnDisk = GetBulkDataSize();
	}

	// The following data is going to be the actual TArray data so the current offset is the offset in
	// file of the bulk data.
	BulkDataOffsetInFile = Ar.Tell();

	// Lazy loading is in effect so we attach to the archive and seek to the end.
	if( Ar.IsAllowingLazyLoading() )
	{
		// Attach and keep track of archive.
		Ar.AttachBulkData( Owner, this );
		AttachedAr = &Ar;	
		// Skip over the bulk data so offset in file matches offset from regular serialization.
		Ar.Seek( EndPosition );
	}
	// Serialize bulk data
	else
	{
		// Allocate memory and serialize data into.
		BulkData = appRealloc( BulkData, GetBulkDataSize() );
		SerializeBulkData( Ar, BulkData );
		check( Ar.Tell() == EndPosition );
	}
}
 ...
 * @param Owner	Object owning the bulk data
 */
void FUntypedBulkData::Serialize( FArchive& Ar, UObject* Owner )
{
	check( LockStatus == LOCKSTATUS_Unlocked );
	...
	else if( Ar.IsPersistent() && !Ar.IsObjectReferenceCollector() && !Ar.ShouldSkipBulkData() )
	{
		...
		{
#if	CONSOLE
			// Bulk data that is being serialized on consoles is single use only. This allows us to
			// free the memory as e.g. the bulk data won't be attached to an archive in the case of
			// seek free loading.
		...
		else if( Ar.IsSaving() )
		{
			...
			else
			{
				...
				INT SavedBulkDataSizeOnDiskPos = Ar.Tell();
				...
				INT SavedBulkDataOffsetInFilePos = Ar.Tell();
...
void FUntypedBulkData::StoreCompressedOnDisk( ECompressionFlags CompressionFlags )
{
	...
	else
	{
		...
		check(CompressionFlags & (COMPRESS_ZLIB | COMPRESS_LZO));
		BulkDataFlags |= (CompressionFlags & COMPRESS_ZLIB) ? BULKDATA_SerializeCompressedZLIB : BULKDATA_SerializeCompressedLZO;
	}
}
...
void FUntypedBulkData::SerializeBulkData( FArchive& Ar, void* Data )
{
	...
	if( bSerializeInBulk )
	{
		...
		{
			Ar.SerializeCompressed( Data, GetBulkDataSize(), (BulkDataFlags & BULKDATA_SerializeCompressedZLIB) ? COMPRESS_ZLIB : COMPRESS_LZO  );
		}
	...
	else
	{
		...
		if( BulkDataFlags & BULKDATA_SerializeCompressed )
		{
			...
			if( Ar.IsLoading() )
			{
				...
				Ar.SerializeCompressed( SerializedData.GetData(), SerializedData.Num(), (BulkDataFlags & BULKDATA_SerializeCompressedZLIB) ? COMPRESS_ZLIB : COMPRESS_LZO);
