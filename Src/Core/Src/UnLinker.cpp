...
FArchive& operator<<( FArchive& Ar, FPackageFileSummary& Sum )
{
	...
	if( Sum.Tag == PACKAGE_FILE_TAG || Sum.Tag == PACKAGE_FILE_TAG_SWAPPED )
	{
		...
		// We cannot use Ar.Ver() as it hasn't been set yet so we manually copy the code from UnLinker.h.
		if ((Sum.FileVersion & 0xffff) >= VER_EPIC_CORE_MERGE_MAY_11)
		{
...
FString ULinker::GetImportPathName(INT ImportIndex)
{
	...
	for (INT LinkerIndex = -ImportIndex - 1; LinkerIndex != ROOTPACKAGE_INDEX;)
	{
		...
		if ( (LinkerRoot->PackageFlags & PKG_Cooked) != 0 && !IS_IMPORT_INDEX(LinkerIndex) )
		{
			...
			if (Result.Len() > 0
				&&	Loader != NULL
				&&	(	Resource.OuterIndex == ROOTPACKAGE_INDEX
...
FString ULinker::GetExportPathName(INT ExportIndex, const TCHAR* FakeRoot,UBOOL bResolveForcedExports/*=FALSE*/)
{
	...
	{ 
		...
		bForcedExport = Export.HasAnyFlags(EF_ForcedExport);
	}
...
{
	// if package is too old, or using seek free loading, skip it
	if (Ver() < VER_EPIC_CORE_MERGE_MAY_11 || GUseSeekFreeLoading)
	{
		return TRUE;
	}
	...
	{
		TArray<INT>* Depends = new(DependsMap)TArray<INT>;
		*this << *Depends;
		DependsMapIndex++;
	}
...
void ULinkerLoad::GatherExportDependencies(INT ExportIndex, TArray<FDependencyRef>& Dependencies)
{
	...
	// @todo: remove this check after all packages have been saved up to VER_EPIC_CORE_MERGE_MAY_11
	if (DependsMap.Num() == 0)
	{
		return;
	}
	...
	if (Import.SourceLinker && Import.SourceIndex == INDEX_NONE && Import.XObject == NULL && Import.OuterIndex != 0 && Import.ObjectName != NAME_ObjectRedirector)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					// failure to load a class should be considered an error if we're not in the editor (so that the continuous integration server complains)
					if ( (!GIsEditor || GIsUCC) && FindClass == UClass::StaticClass() )
					{
						warnf(NAME_Error, TEXT("Missing Class '%s' referenced by package '%s'.  Classes should not be removed if referenced by content; mark the class 'deprecated' instead."), *GetImportFullName(i), *LinkerRoot->GetName());
					}
...
{
	//ModifyProgress(TEXT("Linker->LoadAllObjects()"), Summary.ExportCount); // g_loadingProgress.modifyStep("Linker->LoadAllObjects()", Summary.ExportCount); // 20070115 dEAthcURe
	...
	for( INT i=0; i<Summary.ExportCount; i++ )
	{
		...
		dLoadingProgress(lpdLinkerLoadAllObjects); // LoadingProgressToS(TEXT("Linker->LoadAllObjects()"), 0x0, 1); // g_loadingProgress.progress("Linker->LoadAllObjects()", 0x0, 1); // 20070115 dEAthcURe
	}
		...
		if( !GIsEditor && !GIsUCC )
		{
			...
			if( ClassPackage )
			{
				...
				if( FindClass )
				{
					...
					// Native classes need to be in-place replaced for their initial load.
					/*if( FindObject && FindClass == UClass::StaticClass() && ((UClass*)FindObject)->bNeedsPropertiesLinked )
					{
						FindObject = NULL;
					}*/
...
UObject* ULinkerLoad::IndexToObject( PACKAGE_INDEX Index )
{
	...
	if( ShouldSkipObjectRef() )
	{
		...
		if( Index > 0 )
		{
			...
			FObjectExport &Export = ExportMap( Index );
			if( Export.ObjectName.IsValid() )
			{
				debugf( NAME_Log, TEXT(" -> Skip Reference : %s"), *Export.ObjectName.ToString() );
			}
		}
		else if( Index < 0 )
		{
			...
			FObjectImport& Import = ImportMap( -Index-1 );
			if( Import.ObjectName.IsValid() )
			{
				debugf( NAME_Log, TEXT(" -> Skip Reference : %s"), *Import.ObjectName.ToString() );
			}
		}*/
		// null을 return한다~ ShouldSkipObjectRef()는 Game일때만 켜져야 한다.
		// 
		return NULL;
	}
	else
...
void ULinkerLoad::DetachExport( INT i )
{
	...
	{
		// {{ 20070920 dEAthcURe|TEST ignore linker object detachment
		debugf(TEXT("[dEAthcURe|ULinkerLoad::DetachExport] invalid linker object"));
		return;
		// }} 20070920 dEAthcURe|TEST ignore linker object detachment
