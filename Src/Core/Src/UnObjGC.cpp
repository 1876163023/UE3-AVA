...
void UObject::IncrementalPurgeGarbage( UBOOL bUseTimeLimit, FLOAT TimeLimit )
{
	...
	const FLOAT TimeLimitEnforcementGranularityForce		= 1.0f;
	...
	if( !GObjFinishDestroyHasBeenRoutedToAllObjects && !bTimeLimitReached )
	{
		...
		while( GObjCurrentPurgeObjectIndex<GObjObjects.Num() )
		{
			...
			if( !bUseTimeLimit && bPollTimeLimit && ((appSeconds() - StartTime) > TimeLimitEnforcementGranularityForce ) )
			{
				bTimeLimitReached = TRUE;
				break;
			}
...
// {{ 20070201 dEAthcURe
#ifdef UsePreloading
void UObject::clearPreloadFlag(void)
{
	for( INT ObjectIndex=UObject::GObjFirstGCIndex; ObjectIndex<UObject::GObjObjects.Num(); ObjectIndex++ ) {
		UObject* Object = UObject::GObjObjects(ObjectIndex);
		if(Object && Object->HasAnyFlags(RF_Preload)) {
			Object->ClearFlags(RF_Preload);
		}
	}
}
#endif
...
{	
	...
	while( GObjIncrementalPurgeIsInProgress || GObjPurgeIsRequired )
	{
