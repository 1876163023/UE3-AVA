 ...
 */
FStatNotifyProvider::FStatNotifyProvider(const TCHAR* InProviderName) :
	...
	CurrentFrame((DWORD)-1)
{
	...
	bIsEnabled = bIsEnabled || ParseParam(appCmdLine(),ProviderName) == TRUE;
}
 ...
 */
void FStatNotifyManager::WriteStat(DWORD StatId,DWORD ParentId,DWORD InstanceId,
	DWORD ParentInstanceId,DWORD ThreadId,DWORD Value,DWORD CallsPerFrame)
{
	...
	for (INT Index = 0; Index < Providers.Num(); Index++)
	{
		...
		if (Provider->IsEnabled())
		{
			Provider->WriteStat(StatId,ParentId,InstanceId,ParentInstanceId,
 ...
 * @param Value the value of the stat to write out
 */
void FStatNotifyManager::WriteStat(DWORD StatId,FLOAT Value)
{
	// Just multicasts the calls
	for (INT Index = 0; Index < Providers.Num(); Index++)
	{
		...
		if (Provider->IsEnabled())
		{
			Provider->WriteStat(StatId,Value);
		}
	}
}
 ...
 * @param Value the value of the stat to write out
 */
void FStatNotifyManager::WriteStat(DWORD StatId,DWORD Value)
{
	// Just multicasts the calls
	for (INT Index = 0; Index < Providers.Num(); Index++)
	{
