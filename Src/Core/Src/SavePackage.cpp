...
UBOOL UObject::SavePackage( UPackage* InOuter, UObject* Base, EObjectFlags TopLevelFlags, const TCHAR* Filename, FOutputDevice* Error, ULinkerLoad* Conform, UBOOL bForceByteSwapping, UBOOL bWarnOfLongFilename, DWORD SaveFlags )
{
	...
	struct FScopedHackCompressTexture
	{
		FScopedHackCompressTexture()			
		{
			extern UBOOL GHackCompressTexture;

			GHackCompressTexture = TRUE;
		}
	...
	// Mark package as downloadable.
	InOuter->PackageFlags  |= PKG_AllowDownload;
	...
	TempFilename[c]=0;
	appStrcat( TempFilename, *FString::Printf( TEXT("Save%04d.tmp"), GInstanceId ) );
...
#endif
	{
		...
		*Linker << Linker->Summary;
		...
		for (INT ExpIndex = 0; ExpIndex < Linker->ExportMap.Num(); ExpIndex++)
		{
			...
			TArray<INT>* DependIndices = new(Linker->DependsMap)TArray<INT>();
			check(Linker->DependsMap.Num() == ExpIndex + 1);
		...
		//@todo: nuke ExportCount and NameCount - no longer used
