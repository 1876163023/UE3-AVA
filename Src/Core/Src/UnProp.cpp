...
void UByteProperty::CopySingleValue( void* Dest, void* Src, UObject* SubobjectRoot/*=NULL*/, UObject* DestOwnerObject/*=NULL*/, FObjectInstancingGraph* InstanceGraph/*=NULL*/ ) const
{
	...
	if(PropertyFlags & CPF_Cipher) {
		*(BYTE*)Dest = ((CipherByte*)Src)->decrypt(); //*(BYTE*)Dest = *(BYTE*)Src;		
		return;
	}
	#endif
	// }} 20080219 dEAthcURe|CD
...
void UByteProperty::CopyCompleteValue( void* Dest, void* Src, UObject* SubobjectRoot/*=NULL*/, UObject* DestOwnerObject/*=NULL*/, FObjectInstancingGraph* InstanceGraph/*=NULL*/ ) const
{
	...
	if(PropertyFlags & CPF_Cipher) {
		if( ArrayDim==1 ) {
			*(BYTE*)Dest = ((CipherByte*)Src)->decrypt(); //*(BYTE*)Dest = *(BYTE*)Src;
			//debugf(TEXT("UByteProperty::CopyCompleteValue cipher %d = %d (%d)"), *(BYTE*)Dest, ((CipherByte*)Src)->decrypt(), ((CipherByte*)Src)->data);
		}
		else
			for(int lpp=0;lpp<ArrayDim;lpp++) {
				((BYTE*)Dest)[lpp] = ((CipherByte*)Src)[lpp].decrypt();
			}
		return;
	}
	#endif
...
void UByteProperty::SerializeItem( FArchive& Ar, void* Value, INT MaxReadBytes, void* Defaults ) const
{
	...
	const UBOOL bUseBinarySerialization = (Enum == NULL) || (!Ar.IsLoading() && !Ar.IsSaving()) || (Ar.Ver() < VER_EPIC_CORE_MERGE_MAY_11);
	if( bUseBinarySerialization )
	{
		...
		if(PropertyFlags & CPF_Cipher) {
			if(Ar.IsLoading()) {
				BYTE val;
				Ar << val;
				((CipherByte*)Value)->encrypt(val);
			}
			else {
				Ar << *(BYTE*)Value;				
			}
			/*if(Ar.IsLoading()) {
				Ar << *(BYTE*)Value; // debugf(TEXT("UByteProperty::SerializeItem loading CPF_Cipher %d 0x%x %s"), ((CipherByte*)Value)->decrypt(), Value, *GetFullName());
			}
			else {				
				BYTE val = ((CipherByte*)Value)->decrypt(); // debugf(TEXT("UByteProperty::SerializeItem saving CPF_Cipher %d 0x%x %s"), ((CipherByte*)Value)->decrypt(), Value, *GetFullName());
				Ar << val; 
			}*/
		}
		else {
...
FString UByteProperty::GetCPPType( FString* ExtendedTypeText/*=NULL*/, DWORD CPPExportFlags/*=0*/ ) const
{
	...
	#ifdef EnableCipherData
	if(PropertyFlags & CPF_Cipher) return TEXT("CipherByte"); 
	#endif
	// }} 20080215 dEAthcURe|CD 
	return TEXT("BYTE");
}
...
UBOOL UByteProperty::HasValue( const BYTE* Data, DWORD PortFlags/*=0*/ ) const
{
	...
	#ifdef EnableCipherData
	if(PropertyFlags & CPF_Cipher) {		
		return ((CipherByte*)Data)->decrypt() != 0;
	}
	#endif
	// }} 20080215 dEAthcURe|CD
...
UBOOL UByteProperty::GetPropertyValue( BYTE* PropertyValueAddress, UPropertyValue& out_PropertyValue ) const
{
	...
	if ( PropertyValueAddress != NULL )
	{
		...
		// {{ 20080220 dEAthcURe|CD
		#ifdef EnableCipherData
		if(PropertyFlags & CPF_Cipher) {
			BYTE byteValue = *(BYTE*)PropertyValueAddress;
			out_PropertyValue.ByteValue = ((CipherByte*)PropertyValueAddress)->decrypt();			
			//debugf(TEXT("UByteProperty::GetPropertyValue CPF_Cipher %d -> %d"), byteValue, out_PropertyValue.ByteValue);
		}
		#endif
		// }} 20080220 dEAthcURe|CD
		bResult = TRUE;
...
UBOOL UByteProperty::SetPropertyValue( BYTE* PropertyValueAddress, const UPropertyValue& PropertyValue ) const
{
	...
	if ( PropertyValueAddress != NULL )
	{
		*(BYTE*)PropertyValueAddress = PropertyValue.ByteValue;
		...
		#ifdef EnableCipherData
		if(PropertyFlags & CPF_Cipher) {
			((CipherByte*)PropertyValueAddress)->encrypt(PropertyValue.ByteValue);
		}		
		#endif
		// }} 20080220 dEAthcURe|CD
		bResult = TRUE;
