	...
	Dead( 0 )
{
	if( InFilename )
	{
		appStrncpy( Filename, InFilename, ARRAY_COUNT(Filename)-1 );
	}
	else
	{
		Filename[0]	= 0;
...
static void CreateBackupCopy(TCHAR* Filename)
{
	if (GFileManager->FileSize(Filename) > 0)
	{
		...
		FString BackupFilename = FString::Printf(TEXT("%s%s%i.%02i.%02i-%02i.%02i.%02i.%s"), *Name, BACKUP_LOG_FILENAME_POSTFIX, Year, Month, Day, Hour, Min, Sec, *Extension);
...
#define TTY_BG_BKGRND   ESCCHAR"\x2F"
			...
			{
				TCHAR Temp[1024]; //@hack: appSprintf won't use more than 1024 characters
		...
		if( GIsEpicInternal )
		{
			...
			if (AutoReportFile != NULL)
			{
				...
				if( ( GIsEpicInternal == TRUE ) && ( GIsUnattended == TRUE ) && ( FromCommandLine != 0 ) && ( ParseParam(appCmdLine(), TEXT("KillAllPopUpBlockingWindows")) == TRUE ))
				{
					...
					TCHAR CallingCommandLine[2048] = TEXT("");
...
// 					}
		...
		{
//			appMsgf( AMT_OK, TEXT("%s"), GErrorHist );
			GErrorMsgRet = appMsgf( AMT_YesNo, TEXT("%s\nwould you send error messages to developers by email?"), GErrorHist );
		}
