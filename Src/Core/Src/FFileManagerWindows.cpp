...
,   BufferCount     ( 0 )
{
	...
	extern volatile DWORD GnCreateFile;GnCreateFile++; // 20080109 dEAthcURe|RF
	//debugf(TEXT("          <---------- %d th:%d file open %s"), GnCreateFile, GetCurrentThreadId(), InFilename);
	#endif

	// {{ 20080110 dEAthcURe|FC
	#ifdef EnableFileCache
	GfileCache.onOpenFile(Handle, *Filename);
	#endif
	// }} 20080110 dEAthcURe|FC
	...
	if(GfileServiceProvider)
		GfileServiceProvider->onCreate(Handle, *Filename, false);
	#endif
	// }} 20080304 dEAthcURe|FSP
}
...
UBOOL FArchiveFileReaderWindows::InternalPrecache( INT PrecacheOffset, INT PrecacheSize )
{
	...
	if( Pos == PrecacheOffset && (!BufferBase || !BufferCount || BufferBase != Pos) )
	{
		...
		// Read data from device via Win32 ReadFile API.
		{
			...
			#ifdef EnableFsProfile
			DWORD timeBegin = timeGetTime(); // 20080109 dEAthcURe|RF
			#endif
			// {{ 20080110 dEAthcURe|FC
			#ifdef EnableFileCache
			if(!GfileCache.onReadFile(Handle, Buffer, BufferCount, (DWORD*)&Count))
			#endif
			// }} 20080110 dEAthcURe|FC

			// {{ 20080304 dEAthcURe|FSP
			#ifdef EnableFileServiceProvider
			if(0x0 == GfileServiceProvider || !GfileServiceProvider->onRead(Handle, Buffer, BufferCount, (DWORD*)&Count))
			#endif
			// }} 20080304 dEAthcURe|FSP
			...
			if(timeEnd - timeBegin > 100) {
				debugf(TEXT("1.ReadFile elapses over 100 ms %d"), timeEnd - timeBegin);
			}
			extern DWORD GtimeReadFile; GtimeReadFile += timeEnd - timeBegin; // 20080109 dEAthcURe|RF
			extern DWORD GnByteReadFile; GnByteReadFile += BufferCount; // 20080109 dEAthcURe|RF
			extern DWORD GnReadFile; GnReadFile++;			
			#endif
...
void FArchiveFileReaderWindows::Seek( INT InPos )
{
	...
	check(InPos<=Size);
	#ifdef EnableFsProfile
	DWORD timeBeginSeek = timeGetTime(); // 20080109 dEAthcURe|SF
	#endif
	// {{ 20080110 dEAthcURe|FC
	#ifdef EnableFileCache
	GfileCache.onSeek(Handle, InPos);	
	#endif
	// }} 20080110 dEAthcURe|FC

	// {{ 20080304 dEAthcURe|FSP
	#ifdef EnableFileServiceProvider
	if(0x0 == GfileServiceProvider || !GfileServiceProvider->onSeek(Handle, InPos))
	#endif
	// }} 20080304 dEAthcURe|FSP
	...
	#ifdef EnableFsProfile
	DWORD timeEndSeek = timeGetTime(); // 20080109 dEAthcURe|SF
	extern DWORD GtimeSeek; GtimeSeek += timeEndSeek - timeBeginSeek; // 20080109 dEAthcURe|SF //debugf(TEXT("          <---------- seek elapses %d ms"), timeEndSeek - timeBeginSeek);
	extern DWORD GnSeek; GnSeek++; // 20080109 dEAthcURe|SF
	#endif
	Pos         = InPos;
	BufferBase  = Pos;
	BufferCount = 0;
...
UBOOL FArchiveFileReaderWindows::Close()
{
	if( Handle )
	{
		// {{ 20080110 dEAthcURe|FC
		#ifdef EnableFileCache
		GfileCache.onCloseFile(Handle);
		#endif
		// }} 20080110 dEAthcURe|FC
		// {{ 20080304 dEAthcURe|FSP
		#ifdef EnableFileServiceProvider
		if(GfileServiceProvider) GfileServiceProvider->onClose(Handle);
		#endif
		// }} 20080304 dEAthcURe|FSP
		CloseHandle( Handle );
		#ifdef EnableFsProfile
		extern volatile DWORD GnCreateFile;GnCreateFile--; // 20080109 dEAthcURe|RF
		//debugf(TEXT("          <---------- %d th:%d file close %s"), GnCreateFile, GetCurrentThreadId(), *Filename);
		#endif
	}
	Handle = NULL;
	return !ArIsError;
		...
		if( Copy==0 )
		{
			if( Length >= ARRAY_COUNT(Buffer) )
			{
				...
				// Read data from device via Win32 ReadFile API.
				{
					...
					// {{ 20080110 dEAthcURe|FC
					#ifdef EnableFileCache					
					if(!GfileCache.onReadFile(Handle, V, Length, (DWORD*)&Count))
					#endif
					// }} 20080110 dEAthcURe|FC
					// {{ 20080304 dEAthcURe|FSP
					#ifdef EnableFileServiceProvider
					if(0x0 == GfileServiceProvider || !GfileServiceProvider->onRead(Handle, V, Length, (DWORD*)&Count))
					#endif
					// }} 20080304 dEAthcURe|FSP
					...
					DWORD timeEnd = timeGetTime();
					if(timeEnd - timeBegin > 100) {
						debugf(TEXT("2.ReadFile elapses over 100 ms %d"), timeEnd - timeBegin);
					}
					extern DWORD GtimeReadFile; GtimeReadFile += timeEnd - timeBegin; // 20080109 dEAthcURe|RF
					extern DWORD GnByteReadFile; GnByteReadFile += Length; // 20080109 dEAthcURe|RF
					extern DWORD GnReadFile; GnReadFile++;	
					#endif
...
,   BufferCount ( 0 )
{
	...
	#ifdef EnableFileServiceProvider
	if(GfileServiceProvider)
		GfileServiceProvider->onCreate(Handle, *Filename, true);
	#endif
	// }} 20080304 dEAthcURe|FSP
}
...
UBOOL FFileManagerWindows::InternalMove( const TCHAR* Dest, const TCHAR* Src, UBOOL Replace, UBOOL EvenIfReadOnly, UBOOL Attributes )
{
	...
	#ifdef EnableFileServiceProvider
	if(GfileServiceProvider) {
		GfileServiceProvider->onMove(Dest, Src);
	}
	#endif
	// }} 20080304 dEAthcURe|FSP
	return Result!=0;
