...
void FArray::RemoveSwap( INT Index, INT Count, INT ElementSize, DWORD Alignment )
{
	checkSlow(Count >= 0);
	checkSlow(Index >= 0); 
	checkSlow(Index <= ArrayNum);
	...
	INT NumToCopy = ArrayNum - (Index + Count);
	if( NumToCopy > Count )
	{
		NumToCopy = Count;
	}
	checkSlow(NumToCopy >= 0); 
	if( NumToCopy )
	{
		appMemcpy
			(
			(BYTE*)Data + (Index      ) * ElementSize,
			(BYTE*)Data + (ArrayNum-Count) * ElementSize,
			NumToCopy * ElementSize
			);
	}
	ArrayNum -= Count;
...
FString FString::Replace(const TCHAR* From, const TCHAR* To, UBOOL bIgnoreCase) const
{
	...
	while (TRUE)
	{
		...
		TCHAR* FromLocation = bIgnoreCase ? (TCHAR*)appStristr(Travel, From) : appStrstr(Travel, From);
		if (FromLocation)
		{
...
INT FString::ReplaceInline( const TCHAR* SearchText, const TCHAR* ReplacementText )
{
		...
		&&	ReplacementText != NULL && appStrcmp(SearchText, ReplacementText) != 0 )
	{
		...
		{
			TCHAR* Pos = (TCHAR*)appStristr(&(*this)(0), SearchText);
			while ( Pos != NULL )
			{
				ReplacementCount++;
				...
				{
					Pos = (TCHAR*)appStristr(Pos + NumCharsToReplace, SearchText);
				}
				else
				{
					break;
		...
		else if ( InStr(SearchText) != INDEX_NONE )
		{
			...
			// look for From in the remaining string
			TCHAR* SearchPosition = (TCHAR*)appStristr(WritePosition, SearchText);
			while ( SearchPosition != NULL )
			{
				...
				*SearchPosition = *SearchText;
				...
				SearchPosition = (TCHAR*)appStristr(WritePosition, SearchText);
			}
...
void VARARGS appSendNotificationStringf( const ANSICHAR *Format, ... )
{
	...
	GET_VARARGS_ANSI( TempStr, ARRAY_COUNT(TempStr), Format, Format );
...
void VARARGS appFailAssertFunc( const ANSICHAR* Expr, const ANSICHAR* File, INT Line, const TCHAR* Format/*=TEXT("")*/, ... )
{
	...
	appStackWalkAndDump( StackTrace, 3 );
...
//
TCHAR* appStrdup( TCHAR* String )
{
	TCHAR* NewString = (TCHAR*)appMalloc(appStrlen(String) * sizeof(TCHAR));
	appStrcpy(NewString, String); 
	return NewString;
}
...
class FAsyncStore : public FAsyncWorkBase
{	
public:
	TArray<BYTE> Buffer;
	FArchive* Ar;
	...
	*/
	FAsyncStore( FArchive* Ar, FThreadSafeCounter* Counter = NULL )
		: Ar(Ar), FAsyncWorkBase( Counter )
	{
	}
...
UBOOL appCreateBitmap( const TCHAR* Pattern, INT Width, INT Height, FColor* Data, FFileManager* FileManager )
{
	...
	if( Ar )
	{
		...
		FAsyncStore* AsyncStore = new FAsyncStore( Ar );

		AsyncStore->Buffer.AddZeroed( BytesPerLine * Height );
		...
		for( INT i=Height-1; i>=0; i-- )
		{
			...
			{
				AsyncStore->Buffer( BytesPerLine * Line + j * 3 + 0 ) = Data[i*Width+j].B;
				AsyncStore->Buffer( BytesPerLine * Line + j * 3 + 1 ) = Data[i*Width+j].G;
				AsyncStore->Buffer( BytesPerLine * Line + j * 3 + 2 ) = Data[i*Width+j].R;
			}			
...
UBOOL FMapPackageFileCache::FindPackageFile( const TCHAR* InName, const FGuid* Guid, FString& OutFileName, const TCHAR* Language, UBOOL bShouldLookForSFPackages )
{
	...
	if( !Guid && GFileManager->FileSize(InName)>=0 )
	{
	...
	for (INT SeekFreePass = bShouldLookForSFPackages ? 0 : 1; SeekFreePass < 2 && !bFoundFile; SeekFreePass++)
	{
		for (INT LocPass = 0; LocPass < 3 && !bFoundFile; LocPass++)
		{
			...
			FFilename PackageName = (SeekFreePass == 0) ? BasePackageName + TEXT("_SF") : BasePackageName;
 ...
 *
 * Outdatedness also can be affected by commandline params which allow one to nuke all .ini, have
...
void appCheckIniForOutdatedness( const TCHAR* DefaultIniFile, const TCHAR* GeneratedIniFile, UINT& YesNoToAll )
{
	...
	if( !GUseSeekFreeLoading || !CONSOLE )
	{
		...
		if( ParseParam(appCmdLine(),TEXT("NUKEINIS")) == TRUE )
		{
...
void appInit( const TCHAR* InCmdLine, FOutputDevice* InLog, FOutputDeviceConsole* InLogConsole, FOutputDeviceError* InError, FFeedbackContext* InWarn, FFileManager* InFileManager, FCallbackEventObserver* InCallbackEventDevice, FCallbackQueryDevice* InCallbackQueryDevice, FConfigCache*(*ConfigFactory)() )
{
...
#endif
	appStrncpy( GCmdLine, InCmdLine, 4096 );

	extern INT GAVAVersion, GAVABuiltFromChangeList;

	// Error history.
	appStrcpy( GErrorHist, *FString::Printf(TEXT("General protection fault!(%d.%d)\r\n\r\nHistory: "), GAVAVersion, GAVABuiltFromChangeList ) );
	...
	if( GUseSeekFreeLoading )
	{
		...
		appSprintf(GGPUIni, TEXT("%s%s%sGPU.ini"), *appGameConfigDir(), INI_PREFIX, GGameName);
		appSprintf(GNetIni, TEXT("%s%s%sNet.ini"), *appGameConfigDir(), INI_PREFIX, GGameName); 
...
#endif
	{
		...
		appCreateIniNamesAndThenCheckForOutdatedness( GGPUIni, TEXT("DEFGPUINI="), TEXT("INPUTINI="), TEXT("GPU.ini"), DEFAULT_INI_PREFIX, INI_PREFIX, YesNoToAll );
		appCreateIniNamesAndThenCheckForOutdatedness( GNetIni, TEXT("DEFGPUINI="), TEXT("INPUTINI="), TEXT("Net.ini"), DEFAULT_INI_PREFIX, INI_PREFIX, YesNoToAll );
	}
...
UBOOL USystem::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else if( ParseCommand( &Cmd, TEXT("DEBUG") ) )
	{
		...
		else if( ParseCommand( &Cmd, TEXT("BUFFEROVERRUN") ) )
		{
			...
			TCHAR Buffer[9];
			appStrcpy(Buffer, TEXT("123456789"));
			return TRUE;
		}
#if 0
