...
,	FileSize		(Package != NULL ? Package->GetFileSize() : 0)
...
//@fixme: FIXME see if able to replicate some names by index (hardcoded ones, maybe)
	...
	if( !Result && SupportsObject(Class) )
	{
		...
		{for( INT i=0; i<Class->NetFields.Num(); i++ )
		{
			...
			if( SupportsObject(Field ) )
			{
				...
				if( ItP && (ItP->RepOwner==ItP || !SupportsObject(ItP->RepOwner)) )
...
INT UPackageMap::AddPackage(UPackage* Package)
{
	...
	FPackageInfo* Info = new(List) FPackageInfo(Package);
	Compute();
	return (List.Num() - 1);
}
