//=============================================================================
//	AavaBot.cpp: UT AI implementation
...
IMPLEMENT_CLASS(AavaBot);
IMPLEMENT_CLASS(AavaAvoidMarker);
//. IMPLEMENT_CLASS(UUTTranslocatorReachSpec);
IMPLEMENT_CLASS(AavaSquadAI);
IMPLEMENT_CLASS(UavaBotDecisionComponent);
...
void AavaBot::UpdatePawnRotation()
{
	if ( Focus )
	{
		...
		if ( Focus == CurrentlyTrackedEnemy )
		{
			...
			{
				AavaPlayerController* PC = Cast<AavaPlayerController>(CurrentlyTrackedEnemy->Controller);
...
*/
DWORD AavaBot::LineOfSightTo(AActor *Other, INT bUseLOSFlag, FVector *chkLocation)
{
	if (Other == NULL)
	{
		return 0;
...
void AavaBot::UpdateEnemyInfo(APawn* AcquiredEnemy)
{
...
AActor* AavaBot::FindBestSuperPickup(FLOAT MaxDist)
{
	if ( !Pawn )
	{
		return NULL; 
...
void AavaBot::execPollWaitToSeeEnemy _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	if( !Pawn || !Enemy )
	{
...
IMPLEMENT_FUNCTION( AavaBot, BotAI_PollWaitToSeeEnemy, execPollWaitToSeeEnemy);

void AavaBot::LatentWhatToDoNext()
{
...
void AavaBot::execPollLatentWhatToDoNext _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
IMPLEMENT_FUNCTION(AavaBot, BotAI_PollLatentWhatToDoNext, execPollLatentWhatToDoNext);

/* CanMakePathTo()
// assumes valid CurrentPath, tries to see if CurrentPath can be combined with a path to N
*/
UBOOL AavaBot::CanMakePathTo(class AActor* A)
{
...
AActor* AavaBot::FindBestInventoryPath(FLOAT& Weight)
{
	if ( !Pawn )
	{
		return NULL;
...
*/
void AavaBot::JumpOverWall(FVector WallNormal)
{
	Super::JumpOverWall(WallNormal);
	bJumpOverWall = true;
	bNotifyApex = true;

	AavaPawn* MyavaPawn = Cast<AavaPawn>(Pawn);
	if ( MyavaPawn )
	{
//?		MyavaPawn->bNoJumpAdjust = true; // don't let script adjust this jump again
	}
}

void AavaBot::NotifyJumpApex()
{
	eventNotifyJumpApex();
	bJumpOverWall = false;
}

void AavaBot::PreAirSteering(float DeltaTime)
{ 
...
void AavaBot::PostAirSteering(float DeltaTime)
{
...
void AavaBot::PostPhysFalling(float DeltaTime)
{
...
/* avaBot Tick
*/
UBOOL AavaBot::Tick( FLOAT DeltaSeconds, ELevelTick TickType )
{
	...
	if ( Ticked )
	{
		if( TickType==LEVELTICK_All )
		{
			...
			if ( CurrentlyTrackedEnemy != Enemy )
			{
				...
				{
/*?
					AavaPawn *avaP = Cast<AavaPawn>(CurrentlyTrackedEnemy);
					if ( avaP )
					{
						avaP->RequestTrackingFor(this);
					}
					else
					{
						AavaVehicle *avaV = Cast<AavaVehicle>(CurrentlyTrackedEnemy);
						if ( avaV )
						{
							avaV->RequestTrackingFor(this);
						}
					}
*/
				}
...
/*
void AavaPawn::RequestTrackingFor(AavaBot *Bot)
{
...
UBOOL AavaBot::AirControlFromWall(float DeltaTime, FVector& RealAcceleration)
{
...
UReachSpec* AavaBot::PrepareForMove(ANavigationPoint *NavGoal, UReachSpec* CurrentPath)
{
...
AActor* AavaBot::FindPathToSquadRoute(UBOOL bWeightDetours/* =0 */)
{
...
void AavaBot::BuildSquadRoute(INT Iterations)
{
	...
	if (Iterations <= 0)
	{
		debugf(NAME_Warning, TEXT("Invalid Iterations of %i passed to avaBot::BuildSquadRoute()"), Iterations);
...
FLOAT AavaBot::SpecialJumpCost(FLOAT RequiredJumpZ)
{
...
FRotator AavaBot::SetRotationRate(FLOAT deltaTime)
{
...
*/
UBOOL AavaBot::ForceReached(ANavigationPoint *Nav, const FVector& TestPosition)
{
...
void AavaBot::MarkEndPoints(ANavigationPoint* EndAnchor, AActor* Goal, const FVector& GoalLocation)
{
...
void UavaBotDecisionComponent::Tick(FLOAT DeltaTime)
{
	...
	if (bTriggered)
	{
		AavaBot* Bot = Cast<AavaBot>(GetOwner());
		if (Bot != NULL)
		{
...
void ANavigationPoint::execSetBaseDistance _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(BaseNum);
	P_FINISH;
