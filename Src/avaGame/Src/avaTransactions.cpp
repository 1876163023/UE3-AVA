...
void FavaTransaction::FObjectRecord::SerializeContents( FArchive& Ar, INT InOper )
{
	if( Array )
	{
		//debugf( "Array %s %i*%i: %i",Object->GetFullName(),Index,ElementSize,InOper);
		check((SIZE_T)Array>=(SIZE_T)Object+sizeof(UObject));
		check((SIZE_T)Array+sizeof(FArray)<=(SIZE_T)Object+Object->GetClass()->GetPropertiesSize());
		check(ElementSize!=0);
		...
		if( InOper==1 )
		{
			...
			if( Ar.IsLoading() )
			{
				...
				Array->Remove( Index, Count, ElementSize, DEFAULT_ALIGNMENT );
			}
		}
		else
		{
			...
			{
				Array->Insert( Index, Count, ElementSize, DEFAULT_ALIGNMENT );
...
void FavaTransaction::FObjectRecord::Restore( FavaTransaction* Owner )
{
	if( !Restored )
	{
		Restored = 1;
		...
		FavaTransaction::FObjectRecord::FReader Reader( Owner, Data, ReferencedObjects, ReferencedNames );
		SerializeContents( Reader, Oper );
		if( Owner->Flip )
		{
			ExchangeArray( Data, FlipData );
			ExchangeArray( ReferencedObjects, FlipReferencedObjects );
			ExchangeArray( ReferencedNames, FlipReferencedNames );
			Oper *= -1;
		}
	}
}

void FavaTransaction::RemoveRecords( INT Count /* = 1  */ )
{
...
*/
void FavaTransaction::DumpObjectMap(FOutputDevice& Ar) const
{
...
FArchive& operator<<( FArchive& Ar, FavaTransaction::FObjectRecord& R )
{
	check(R.Object);
	FMemMark Mark(GMem);
	Ar << R.Object;
...
void FavaTransaction::SaveObject( UObject* Object )
{
	check(Object);
...
void FavaTransaction::SaveArray( UObject* Object, FArray* Array, INT Index, INT Count, INT Oper, INT ElementSize, STRUCT_AR Serializer, STRUCT_DTOR Destructor )
{
	check(Object);
	check(Array);
	check(ElementSize);
	...
	check((SIZE_T)Array+sizeof(FArray)<=(SIZE_T)Object+Object->GetClass()->PropertiesSize);
	check(Index>=0);
	check(Count>=0);
	check(Index+Count<=Array->Num());
...
SIZE_T FavaTransaction::DataSize() const
{
	SIZE_T Result=0;
...
*/
void FavaTransaction::GetTransactionObjects(TArray<UObject*>& Objects)
{
...
UavaTransBuffer::UavaTransBuffer( SIZE_T InMaxMemory )
...
// UObject interface.
void UavaTransBuffer::Serialize( FArchive& Ar )
{
	...
	Ar << UndoBuffer << ResetReason << UndoCount << ActiveCount;
	Ar.AllowEliminatingReferences( TRUE );

	CheckState();
}
void UavaTransBuffer::FinishDestroy()
{
...
*/
INT UavaTransBuffer::Begin(const TCHAR* SessionName)
{
	CheckState();
	...
	if( ActiveCount++==0 )
	{
		...
		GUndo = new(UndoBuffer)FavaTransaction( SessionName, 1 );
	}
	CheckState();
	return Result;
}
...
INT UavaTransBuffer::End()
{
	...
	if( --ActiveCount==0 )
	{
		...
		if ( GUndo && GLog )
		{
			...
			static_cast<FavaTransaction*>(GUndo)->DumpObjectMap( *GLog );
		}
#endif
		GUndo = NULL;
	}
...
*/
void UavaTransBuffer::Reset( const TCHAR* Reason )
{
	CheckState();
	...
	if( ActiveCount != 0 )
	{
		...
		ErrorMessage += FString::Printf(TEXT("Non zero active count in UavaTransBuffer::Reset") LINE_TERMINATOR );
...
*/
void UavaTransBuffer::Cancel( INT StartIndex /*=0*/ )
{
	CheckState();
	...
	if ( ActiveCount > 0 )
	{
		...
		{
			FavaTransaction& Transaction = UndoBuffer.Last();
...
*/
UBOOL UavaTransBuffer::CanUndo( FString* Str )
{
	CheckState();
	if( ActiveCount )
	{
...
*/
UBOOL UavaTransBuffer::CanRedo( FString* Str )
{
	CheckState();
	if( ActiveCount )
	{
...
*/
FString UavaTransBuffer::GetUndoDesc( UBOOL bCheckWhetherUndoPossible )
{
	FString Title;
	...
	const FavaTransaction* Transaction = &UndoBuffer( UndoBuffer.Num() - (UndoCount + 1) );
...
*/
FString UavaTransBuffer::GetRedoDesc()
{
	FString Title;
	if( !CanRedo( &Title ) )
	{
	...
	const FavaTransaction* Transaction = &UndoBuffer( UndoBuffer.Num() - UndoCount );
	return Transaction->GetTitle();
}
...
UBOOL UavaTransBuffer::Undo()
{
	...
	FavaTransaction& Transaction = UndoBuffer( UndoBuffer.Num() - ++UndoCount );
...
FTransactionBase* UavaTransBuffer::CreateInternalTransaction()
{
	return new FavaTransaction( TEXT("Internal"), 0 );
}

/**
* Called at the end of an Undo or Redo to notify external systems of any state changes.
*/
void UavaTransBuffer::FinishDo()
{
...
*/
SIZE_T UavaTransBuffer::GetUndoSize() const
{
	SIZE_T Result=0;
...
*/
void UavaTransBuffer::CheckState() const
{
...
void UavaTransBuffer::GetTransactionObjects(TArray<UObject*>& Objects)
{
	for( INT BufferIndex =0 ; BufferIndex < UndoBuffer.Num() ; BufferIndex++ )
	{
		FavaTransaction& Transaction = UndoBuffer( BufferIndex );
		Transaction.GetTransactionObjects( Objects );
	}
}

IMPLEMENT_CLASS(UavaTransBuffer);
IMPLEMENT_CLASS(UavaTransactor);
...
//{
//	return new UavaTransBuffer( 8*1024*1024 );
//}

#include "avaNetClient.h"

FavaScopedTransaction::FavaScopedTransaction(const TCHAR* SessionName)
{
	Index = GetAvaNetHandler()->Trans->Begin( SessionName );
	check( IsOutstanding() );
}

FavaScopedTransaction::~FavaScopedTransaction()
{
	if ( IsOutstanding() )
	{
		GetAvaNetHandler()->Trans->End();
	}
}
...
*/
void FavaScopedTransaction::Cancel()
{
	if ( IsOutstanding() )
	{
		GetAvaNetHandler()->Trans->Cancel( Index );
		Index = -1;
	}
}
...
*/
UBOOL FavaScopedTransaction::IsOutstanding() const
{
	return Index >= 0;
}
