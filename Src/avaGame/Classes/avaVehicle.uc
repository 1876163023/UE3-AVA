/*
	...
	2007/05/10	고광록
		UTVehicle에서 그대로 들고옴.(단, 무기 관련 클래스 제외)

		추가	UTVehicleBase, UTVehicleWheel, UTVehicleFactory, avaVehicleWeapon, 
				UTDestroyedVehicleHitProxy, UTSeqAct_EnterVehicle, UTSeqAct_ExitVehicle, UTSeqEvent_VehicleFactory
		...
		제외	UTWeaponPawn(이녀석 있어야 겠다. InitializeSeats에서 각 자리마다 사용한다)
				UTGameObjective, UTDefensePoint, UTCarriedObject, 
				UTSkelControl_Damage, UTSkelControl_DamageSpring, UTSkelControl_DamageHinge,
				UTConsolePlayerController, UTDrawMapPanel, 
				UTLockWarningMessage, UTVehicleCantCarryFlagMessage, UTOnslaughtMessage
				UTDmgType_VehicleExplosion, UTDmgType_ScorpionSelfDestruct

				<자원>
				UTGame.UTDmgType_RanOver, UTGame.UTDmgType_Pancake, UTGame.UTTankShellExplosionLight

	2006/06/22 by OZ
		1. ava 용 Vehicle 확장 class
*/
class avaVehicle extends avaVehicleBase
	abstract
	notplaceable
	nativereplication
...
var hmserialize int hmId; // 20070801 dEAthcURe|HM

//! avaVehicleBot(AIController)에서 Route에서 이동중인 노드의 인덱스 값을 저장한다.
var hmserialize repnotify int AIScriptedRouteIndex;

//! HM이후에 RestoreDriver()함수를 시도한 횟수.
var int	TryCount;

//! HM이후에 최대 함수 호출 횟수.
var const int MaxTryCount;

/** value for Team property that indicates the vehicle's team hasn't been set */
const AVAVEHICLE_UNSET_TEAM = 255;
...
/** If true, and bLookToSteer is enabled in the avaConsolePlayerController.  */
...
var hmserialize repnotify rotator WeaponRotation;	//! 기본 Turret의 회전값을 복원 하기 위해서 HM처리.(2007/08/07 고광록)
...
struct native VehicleSeat
{
	...
	var() class<Vehicle>			SeatPawnClass;

	// ---[ Weapon ] ------------------------

	/*! @brief 따로 붙이기 위한 메쉬가 있는 경우 사용.([0]은 Vehicle자신이 된다)
		@note
			CameraTag(Bone/Socket), GunSocket, TurretControls 등에서 고유의 Mesh를 참조함.
	*/
	var() editinline SkeletalMeshComponent Mesh;

	/** class of weapon for this seat */
	var() class<avaVehicleWeapon> GunClass;

	/** Reference to the gun */
	var() editinline  avaVehicleWeapon Gun;
	...
	var() editinline array<avaSkelControl_TurretConstrained> TurretControllers;
	...
	//! (Vehicle.Health <= 0)인 경우 Driver가 내리는가?
	var() bool bStayWhenDied;
	...
	var class<avaExplosionLight> ImpactFlashLightClass;
...
//	var VehicleMovementEffect SeatMovementEffect;
	...
	{
		SeatPawnClass = class'avaWeaponPawn'
	}
...
//! 동적배열도 replication이 되는지 몰라서 일단 정적 배열로.
`define MAX_PLAYERNAMES		2
var(Seats)	hmserialize repnotify string		PlayerNames[`MAX_PLAYERNAMES];
...
var MaterialInstance TeamMaterials[2];
...
var avaPlayerController.ViewShakeInfo ProximityShake;
...
/** Mesh to switch too when this vehicle ragdolls */
var SkeletalMesh RagdollMesh;

/** PhysicsAsset to switch to when this vehicle ragdools */
var PhysicsAsset RagdollAsset;

/** Allows for adjustment of the mesh */
var vector RagdollOffset;
var rotator RagdollRotationOffset;

/** used as collision for running over and crushing players when using a ragdoll */
//! (Vehicle의 위쪽에 위치하여 충돌되는 것처럼 사용되는 Actor이다)
var avaDestroyedVehicleHitProxy RagdollHitProxy;
/** cylinder used with RagdollHitProxy for running over and crushing players when using a ragdoll */
//! (avaDestroyedVehicleHitProxy내부에서 이 값이 있는 경우 참고해서 사용한다)
var CylinderComponent RagdollCylinder;
...
//	var avaVehicleDeathPiece DestroyedTurret;
...
var avaPlayerReplicationInfo PassengerPRI;
...
var avaPawn.TakeHitInfo LastTakeHitInfo;
...
/** material specific wheel effects, applied to all attached avaVehicleWheels with bUseMaterialSpecificEffects set to true */
...
var vector				DrawHUDOffset;
var name				DrawHUDBoneName;
var float				HealthBarLength;
...
//	`log("Spawned avaVehicle" @Self @Controller);
...
//	`log("avaVehicle.PostBeginPlay!");

	if (Role==ROLE_Authority)
	{
		if ( !bDeleteMe && avaGame(WorldInfo.Game) != None )
		{
			// WorldInfo형식의 객체에 등록함수로 등록.
			//! @comment 아직 추가하지 않음.
//			avaGame(WorldInfo.Game).RegisterVehicle(self);
		}
...
simulated function bool TryAddToHUD()
{
	...
	{
		if ( avaHUD(PC.MyHUD) != None && PC.PlayerReplicationInfo != None && ( PC.PlayerReplicationInfo.bOnlySpectator == TRUE || PC.PlayerReplicationInfo.Team != None ) )
		{
			avaHUD(PC.MyHUD).AddActorToRadar( self, 255, IconCode, , true );
			return true;
		}
	}
	return false;
...
simulated function SetInputs(float InForward, float InStrafe, float InUp)
{
	...
	//local rotator LookRot, VehicleRot;
	//local vector LookDir, VehicleDir;
	//local float VehicleHeading, LookHeading, DeltaTargetHeading;
...
simulated function PlaySpawnEffect()
{
	...
	if (SpawnTemplate != None)
	{
		...
		if (WorldInfo.NetMode != NM_DedicatedServer)
		{
			SpawnEffect = Spawn(class'avaEmitter');
...
simulated function EjectSeat(int SeatIdx)
{
	...
	{
		VB = avaVehicleBase(Seats[SeatIdx].SeatPawn);
		if(VB != none)
		{
...
function DisplayWeaponBar(Canvas canvas, avaHUD HUD);
 ...
 */
simulated function RenderMapIcon(MapInfo MP, Canvas Canvas, avaPlayerController PlayerOwner, LinearColor FinalColor)
{
//! UT와 인자가 달라서 일단 사용하지 않는다.
/*!
...
simulated function InitializeEffects()
{
	...
	{
		VehicleEffects[i].EffectRef = new(self) class'avaParticleSystemComponent';
...
{
	local int i, j;
	local avaAnimNodeSequence Player;

	// 이벤트와 같은 이름의 애니메이션 이름이 있는지 찾는다.
	...
	// 찾았다면 모든 좌석에 대한 메쉬의 애니메이션을 설정.
	if( i < VehicleAnims.Length )
	{
		for (j = 0; j < Seats.Length; j++)
		{
			if ( Seats[j].Mesh != None && Seats[j].Mesh.Animations != None )
			{
				Player = avaAnimNodeSequence(Seats[j].Mesh.Animations.FindAnimNode(VehicleAnims[i].AnimPlayerName));
				if ( Player != none )
				{
...
event RanInto(Actor Other)
{
	...
	if ( Pawn(Other) == None || 
		Vehicle(Other) != None || //&& !Other.IsA('avaVehicle_Hoverboard')) || 
		Other == Instigator || 
...
/*
			if( Pawn(Other).Health <= 0 )
			{
				...
				{
					avaPlayerController(Instigator.Controller).ClientSpawnCameraEffect(None, RanOverDamageType.default.DeathCameraEffectInstigator);
				}
			}
*/
		}
...
function DriverRadiusDamage( float DamageAmount, float DamageRadius, Controller EventInstigator,
				class<DamageType> DamageType, float Momentum, vector HitLocation, class<Weapon> DamageCauser )
{
	local int i;
...
//! @comment 아직 avaGame.VehicleList를 붙이지 않았다.
/*
	if ( avaGame(WorldInfo.Game) != None )
	{
		if ( avaGame(WorldInfo.Game).VehicleList == Self )
			avaGame(WorldInfo.Game).VehicleList = NextVehicle;
		else
		{
			Prev = avaGame(WorldInfo.Game).VehicleList;
			if ( Prev != None )
				for ( V=avaGame(WorldInfo.Game).VehicleList.NextVehicle; V!=None; V=V.NextVehicle )
				{
					if ( V == self )
					{
...
simulated function rotator GetClampedViewRotation()
{
	...
	local avaVehicleWeapon VWeap;

	VWeap = avaVehicleWeapon(Weapon);
...
simulated event rotator GetViewRotation()
{
	...
	else if ( AIController(Controller) != None && Weapon != None )
	{
		// 무기가 바라보는 방향이 곧 바라보는 방향이 된다.
...
simulated event ReplicatedEvent(name VarName)
{
	...
	else if(VarName == 'AIScriptedRouteIndex')
	{
//		`log("avaVehicle.AIScriptedRouteIndex =" @AIScriptedRouteIndex);
	}
	else if(VarName == 'PlayerNames')
	{
//		`log("avaVehicle.ReplicatedEvent - PlayerNames[0] =" @PlayerNames[0] @"PlayerNames[1] =" @PlayerNames[1]);
	}
	else
	{
		...
		{
//			`log("avaVehicle.ReplicatedEvent - " @VarString);
...
simulated function ProximityShakeTimer()
{
	...
	local avaPlayerController.ViewShakeInfo NewViewShake;
	local int i;
	...
	if (!IsZero(Velocity))
	{
		foreach LocalPlayerControllers(PC)
		{
			...
			if (PC.IsA('avaPlayerController') && PC.Pawn != None && PC.Pawn.Physics == PHYS_Walking)
			{
				...
				if (!bIsWeaponPawn)
				{
					...
					if (ShakeMag > 0.f)
					{
						...
						avaPlayerController(PC).ShakeView(NewViewShake);
					}
				}
			}
		}
...
//	local UTCarriedObject Flag;
	...
	for (i=0;i<Seats.Length;i++)
	{
		...
		{
			avaP = avaPawn(Seats[i].StoragePawn);
			if((avaP != none))// && avaP.ShieldBeltArmor > 0)	//! avaPawn.ShieldBeltArmor은 사용안함.
			{
//				avaP.ShieldBeltArmor = 0;
//				avaP.SetOverlayMaterial(none);
				EjectSeat(i);
			}
		}
	}
...
//	`log("avaVehicle.CanEnterVehicle - "@P.name);

	if ( P.bIsCrouched || (P.DrivenVehicle != None) || (P.Controller == None) || !P.Controller.bIsPlayer
	     || Health <= 0 || !avaPawn(P).CanEnterVehicle() )
	{
		`log("false : avaVehicle.CanEnterVehicle 1, " 
			@P.bIsCrouched @P.DrivenVehicle @P.Controller @P.Controller.bIsPlayer @Health);
		return false;
	}
...
//	`log("Seats.Length = " @Seats.Length);
	...
	for (i=0;i<Seats.Length;i++)
	{
		...
		{
			`log("false : avaVehicle.CanEnterVehicle - !WorldInfo.GRI.OnSameTeam(P, SeatPRI)");
			return false;
		}
	}

	`log("false : avaVehicle.CanEnterVehicle - bSeatAvailable:" @bSeatAvailable);
...
//	`log("avaVehicle.TryToDrive");

	// don't allow while playing spawn effect
	if (bPlayingSpawnEffect)
	{
		`log("false : bPlayingSpawnEffect");
		return false;
	}
	...
	if ( bIsInverted && bMustBeUpright && !bVehicleOnGround && VSize(Velocity) <= 5.0f )
	{
		...
		`log("false : bIsInverted && bMustBeUpright && !bVehicleOnGround && VSize(Velocity) <= 5.0f");
		return false;
	}
	...
	{
		// 여기서 문제
		`log("false : !CanEnterVehicle(P)");
		return false;
	}

	// Check vehicle Locking....
	if (!bIsDisabled && (Team == AVAVEHICLE_UNSET_TEAM || !bTeamLocked || !WorldInfo.Game.bTeamGame || WorldInfo.GRI.OnSameTeam(self,P) ))
	{
		if (bEnteringUnlocks)
		{
			bTeamLocked = false;
...
/*
			if (WorldInfo.GRI.OnSameTeam(self, P))
			{
				...
				for (i = 0; i < Seats.length; i++)
				{
					B = avaBot(Seats[i].SeatPawn.Controller);
			...
			if (!bFreedSeat)
			{
				...
				`log("false : !bFreedSeat");
				return false;
			}
*/
		}

//		`log("avaVehicle.DriverEnter!");
	...
	{
		`log("false : VehicleLocked - "
			@"bIsDisabled=" @bIsDisabled 
			@"Team=" @Team
			@"bTeamLocked=" @bTeamLocked
			@"WorldInfo.Game.bTeamGame=" @WorldInfo.Game.bTeamGame
			@"WorldInfo.GRI.OnSameTeam(self,P)=" @WorldInfo.GRI.OnSameTeam(self,P));
...
//		PC.ReceiveLocalizedMessage(class'avaVehicleMessage', 1);
	}
}
...
{
//! @comment UE2때의 방식이라고 한다(from 태식). 지금은 UI 툴에서 그린다고 함.
/*
	local float TextXL, XL, YL, Dist, ScaledDist, HealthX, HealthY, xscale;
...
simulated function RenderPassengerBeacons(PlayerController PC, Canvas Canvas, LinearColor TeamColor, Color TextColor, avaWeapon Weap)
{
 ...
 * @param	Canvas	The canvas to draw on
 */
simulated function PostRenderPassengerBeacon(PlayerController PC, Canvas Canvas, LinearColor TeamColor, Color TextColor, 
											 avaWeapon Weap, avaPlayerReplicationInfo InPassengerPRI, 
											 vector InPassengerTeamBeaconOffset)
{
//! @comment 위와 마찮가지로 일단 제외.
/*
 ...
 */
simulated function LockOnWarning(avaProjectile IncomingMissile)
{
	SendLockOnMessage(1);
}
...
/*
	if (PlayerController(Controller) != None)
	{
		...
		if (NumPositions > 0)
		{
			...
			{
				if (avaBot(P.Controller) != None && WorldInfo.GRI.OnSameTeam(P, self))
				{
					V = avaVehicle(P);
					if (V != None)
					{
						...
						if (avaP != None && avaP.HoverboardClass == V.Class && V.DriverLeave(false))
						{
							P = avaP;
						}
					}
					if (Vehicle(P) == None)
					{
						avaBot(P.Controller).SetTemporaryOrders('Follow', Controller);
						if (--NumPositions == 0)
						{
							break;
						}
 ...
 * AI Hint
 */
function avaVehicle GetMoveTargetFor(Pawn P)
{
	return self;
}

/** handles dealing with any flag the given driver/passenger may be holding */
function HandleEnteringFlag(avaPlayerReplicationInfo EnteringPRI)
{
//! @comment UTCarriedObject를 사용하지 않는다.(avaPlayerReplicationInfo.GetFlag())
/*
...
function bool DriverEnter(Pawn P)
{
	...
	HandleEnteringFlag(avaPlayerReplicationInfo(PlayerReplicationInfo));
...
//	Seats[0].StoragePawn = P;
	...
	if (ParentFactory != None)
	{
		ParentFactory.TriggerEventClass(class'avaSeqEvent_VehicleFactory', None, 3);
	}
 ...
 */
//! @comment 깃발은 장착하지 않는다.(UTCarriedObject 사용안함)
/*
...
function bool PassengerEnter(Pawn P, int SeatIndex)
{
	...
	{
		`log("Failed Seats[" @SeatIndex @"].SeatPawn.DriverEnter(" @p @")");
		return false;
	}

	HandleEnteringFlag(avaPlayerReplicationInfo(Seats[SeatIndex].SeatPawn.PlayerReplicationInfo));

	SetSeatStoragePawn(SeatIndex,P);

	// Passenger가 탔을 때도 이벤트를 발생한다.
	if (ParentFactory != None)
		ParentFactory.TriggerEventClass(class'avaSeqEvent_VehicleFactory', None, 3);
...
//	`log("avaVehicle.DriverLeave - Health:" @Health);
...
//		Seats[0].StoragePawn = None;
		...
		if (ParentFactory != None)
		{
			ParentFactory.TriggerEventClass(class'avaSeqEvent_VehicleFactory', None, 4);
		}
	}
...
//	Seats[SeatIndex].StoragePawn = None;
	...
	// Passenger가 내렸을 때도 이벤트를 발생한다.
	if (ParentFactory != None)
	{
//		`log("avaVehicle.PassengerLeave" @SeatIndex);
//		ret = 
		ParentFactory.TriggerEventClass(class'avaSeqEvent_VehicleFactory', None, 4);
//		`log("ParentFactory.TriggerEventClass(VehicleLeft) : " @ret);
	}
}
...
//	local Pawn P;
	...
	`log("avaVehicle.CheckReset2()" @ParentFactory);

	if ( bKeyVehicle && !Occupied() )
	{
		`log("avaVehicle.CheckReset2() - bKeyVehicle" @bKeyVehicle @"Occupied()" @Occupied());
...
/*
	if ( Occupied() )
	{
		`log("avaVehicle.CheckReset2() - Occupied()" @Occupied());
		ResetTime = WorldInfo.TimeSeconds - 1;
		return;
	}
	...
	foreach WorldInfo.AllPawns(class'Pawn', P)
	{
			...
			FastTrace(P.Location + P.GetCollisionHeight() * vect(0,0,1), Location + GetCollisionHeight() * vect(0,0,1)) )
		{
			...
			`log("avaVehicle.CheckReset2() - ResetTime =" @ResetTime);
			return;
		}
	}
*/
	...
	if ( ParentFactory != None )
	{
		`log("avaVehicle.CheckReset2() - ChildVehicle = None");
...
event CheckReset()
{
/*! 드라이버가 없다고 Reset해야 하는 경우는 없다.(2008/02/13 고광록)
	local Pawn P;
...
{
//! @comment avaBot은 준비중입니다.
/*
	local int i;

	if ( UTGame(WorldInfo.Game).JustStarted(20) && 
	     (Reservation != None) && 
	     (Reservation != P.Controller) && 
	     (Reservation.RouteGoal == Self) && 
	     (Reservation.Pawn != None) && 
...
function float BotDesireability(Actor S, int TeamIndex, Actor Objective)
{
	...
	bSameTeam = (Team == TeamIndex || Team == AVAVEHICLE_UNSET_TEAM);
	if ( bSameTeam )
	{
...
{
//! @comment avaBot(Reservation)는 준비중.
/*
	if ( Reservation == P.Controller )
		return 1.0;
	if ( avaGame(WorldInfo.Game).JustStarted(20) && (Reservation != None) && (Reservation.Pawn != None)
		&& (VSize(Reservation.Pawn.Location - Location) <= VSize(P.Location - Location)) )
	{
		return 0;
	}
...
/*
	if ( UTGame(WorldInfo.Game).JustStarted(20) && 
		 (Reservation != None) && (Reservation != P.Controller) )
	{
		if ( (Reservation.RouteGoal == Self) && 
			 (Reservation.Pawn != None) && 
			 (VSize(Reservation.Pawn.Location - Location) <= VSize(P.Location - Location)) )
		{
			return true;
...
simulated function bool StopsProjectile(Projectile P)
{
	...
	return ( (P.Instigator == None) || (P.Instigator.Base != self) ); //|| !P.Instigator.IsA('avaWeaponPawn') );
}
...
function PlayHit(float Damage, Controller InstigatedBy, vector HitLocation, class<DamageType> damageType, 
				 vector Momentum, TraceHitInfo HitInfo)
{
	local avaPlayerController Hearer;
	local class<avaDamageType> avaDamage;

	if ( InstigatedBy != None && 
		 class<avaDamageType>(DamageType) != None && 
		 class<avaDamageType>(DamageType).default.bDirectDamage )
	{
		Hearer = avaPlayerController(InstigatedBy);
		if (Hearer != None)
		{
...
simulated event PlayTakeHitEffects()
{
	...
	if (EffectIsRelevant(Location, false))
	{
		...
		if (avaDamage != None)
		{
			avaDamage.static.SpawnHitEffect(self, LastTakeHitInfo.bDamage, LastTakeHitInfo.Momentum, Mesh.GetBoneNameByIndex( LastTakeHitInfo.HitBoneIndex ), LastTakeHitInfo.HitLocation);
		}
	}

	// 현재는 안쓰이는 Code이기 때문에 주석처리함.... ClientSide Health 가 필요할까???
...
simulated event TakeDamage(int Damage, Controller EventInstigator, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo HitInfo, optional class<Weapon> DamageCauser)
{
	if (Role == ROLE_Authority)
	{
//! avaPawn.bIsInvulnerable는 선언 안됨.
//		if ( (avaPawn(Driver) != None) && avaPawn(Driver).bIsInvulnerable )
//			Damage = 0;
//! UTDmgType_ScorpionSelfDestruct는 없다.
...
//	if(DamageType == class'avaDmgType_ImpactHammer')
//	{
 ...
 */
function Actor GetHomingTarget(avaProjectile Seeker, Controller InstigatedBy)
{
	return self;
}
...
//	`log("avaVehicle.InitializeSeats()");

	for(i=0;i<Seats.Length;i++)
	{
		// 비어 있는 경우에는 기본 SkeletalMeshComponent를 넣어준다.
		if ( Seats[i].Mesh == None )
		{
//			`log("Seat[" @i @"].Mesh = Self.Mesh");
			Seats[i].Mesh = Mesh;
		}
		...
		if (i>0)
		{
	   		Seats[i].SeatPawn = Spawn(Seats[i].SeatPawnClass);

//			`log( "avaVehicle.Spawn SeatPawn" @self @Seats[i].SeatPawnClass @Seats[i].SeatPawn );


	   		Seats[i].SeatPawn.SetBase(self);
			if ( Seats[i].GunClass != None )
				Seats[i].Gun = avaVehicleWeapon(Seats[i].SeatPawn.InvManager.CreateInventory(Seats[i].GunClass));
			if ( Seats[i].Gun != None )
				Seats[i].Gun.SetBase(self);
			avaWeaponPawn(Seats[i].SeatPawn).MyVehicleWeapon = Seats[i].Gun;
			avaWeaponPawn(Seats[i].SeatPawn).MyVehicle = self;
	   		avaWeaponPawn(Seats[i].SeatPawn).MySeatIndex = i;

	   		if ( Seats[i].ViewPitchMin != 0.0f )
	   		{
				avaWeaponPawn(Seats[i].SeatPawn).ViewPitchMin = Seats[i].ViewPitchMin;
			}
			else
	   		{
				avaWeaponPawn(Seats[i].SeatPawn).ViewPitchMin = ViewPitchMin;
			}

	   		if ( Seats[i].ViewPitchMax != 0.0f )
	   		{
				avaWeaponPawn(Seats[i].SeatPawn).ViewPitchMax = Seats[i].ViewPitchMax;
			}
			else
	   		{
				avaWeaponPawn(Seats[i].SeatPawn).ViewPitchMax = ViewPitchMax;
			}
		}
		else
		{
			Seats[i].SeatPawn = self;
			Seats[i].Gun = avaVehicleWeapon(InvManager.CreateInventory(Seats[i].GunClass));
			if ( Seats[i].Gun != None )
				Seats[i].Gun.SetBase(self);
		}
		...
		if (Seats[i].Gun!=none)
		{
			...
			// Host는 여기서 OnInitialize()함수가 호출되고,
			// Client는 ReplicatedEvent함수에서 호출된다.(2007/08/01)
			Seats[i].Gun.OnInitialize();
		}
...
{
	local int Seat, i;
	local avaSkelControl_TurretConstrained Turret;
	local SkeletalMeshComponent SeatMesh;
	...
	else
	{
		...
		{
			SeatMesh = GetSeatMesh(Seat);

			for (i = 0; i < Seats[Seat].TurretControls.Length; i++)
			{
				Turret = avaSkelControl_TurretConstrained( SeatMesh.FindSkelControl(Seats[Seat].TurretControls[i]) );
				if ( Turret != none )
				{
				...
				{
					`warn("Failed to find skeletal controller named" @ Seats[Seat].TurretControls[i] @ "(Seat "$Seat$") for" @ self 
						@ "in AnimTree" @SeatMesh.AnimTreeTemplate);
				}
			}
		}
	}
...
simulated event GetBarrelLocationAndRotation(int SeatIndex, out vector SocketLocation, optional out rotator SocketRotation)
{
	...
	{
		GetSeatMesh(SeatIndex).GetSocketWorldLocationAndRotation(Seats[SeatIndex].GunSocket[GetBarrelIndex(SeatIndex)], SocketLocation, SocketRotation);
	}
	else
	{
...
simulated function Vector GetPhysicalFireStartLoc(avaWeapon ForWeapon)
{
	local avaVehicleWeapon VWeap;

	VWeap = avaVehicleWeapon(ForWeapon);
	if ( VWeap != none )
	{
 ...
 */
function rotator GetWeaponAim(avaVehicleWeapon VWeapon)
{
...
//		`log("avaVehicle.GetWeaponAim" @VWeapon @VWeapon.SeatIndex @C @C.FocalPoint );
...
//			`log("avaVehicle.GetWeaponAim #2 - DesiredAimPoint=" @DesiredAimPoint);
		}
		...
		if ( Seats[VWeapon.SeatIndex].GunSocket.Length>0 )
		{
			...
			if( VWeapon.bIgnoreSocketPitchRotation || 
				((DesiredAimPoint.Z - Location.Z)<0 && VWeapon.bIgnoreDownwardPitch) )
			{
//				`log("avaVehicle.GetWeaponAim #2-1 - Modify SocketRotation.Pitch" 
//					@SocketRotation.Pitch @Rotator(DesiredAimPoint - Location).Pitch);
				SocketRotation.Pitch = Rotator(DesiredAimPoint - Location).Pitch;
			}

//			`log("avaVehicle.GetWeaponAim #3 - SocketLocation=" @SocketLocation 
...
//		`log("avaVehicle.GetWeaponAim #4 - " @DiffAngle @">= " @VWeapon.GetMaxFinalAimAdjustment());
		...
		if ( DiffAngle >= VWeapon.GetMaxFinalAimAdjustment() )
		{
			...
			{
//				`log("avaVehicle.GetWeaponAim #5 - " @rotator(DesiredAimPoint - SocketLocation));
			...
			{
//				`log("avaVehicle.GetWeaponAim #6 - " @AdjustedAim);
...
//			`log("avaVehicle.GetWeaponAim #7 - " @Rotator( QuatRotateVector(Q,DirB)));
 ...
 */
simulated function GetWeaponViewAxes( avaWeapon WhichWeapon, out vector xaxis, out vector yaxis, out vector zaxis )
{
...
{
//	`log("avaVehicle.CauseMuzzleFlashLight" @SeatIndex);
	...
	if ( !WorldInfo.bDropDetail || 
		 (Seats[SeatIndex].SeatPawn != None && PlayerController(Seats[SeatIndex].SeatPawn.Controller) != None && Seats[SeatIndex].SeatPawn.IsLocallyControlled()) )
	{
		...
		if ( Seats[SeatIndex].MuzzleFlashLight != none )
		{
			GetSeatMesh(SeatIndex).DetachComponent(Seats[SeatIndex].MuzzleFlashLight);
			GetSeatMesh(SeatIndex).AttachComponentToSocket(Seats[SeatIndex].MuzzleFlashLight, Seats[SeatIndex].GunSocket[GetBarrelIndex(SeatIndex)]);
		}
	}
}
...
{
	// Dedi server 인 경우에 Effect 계산은 필요없다...
	if ( WorldInfo.NetMode == NM_DedicatedServer )	return;
	
	// bViaReplication 이 true 인 경우는 Replicated 된 경우이다...
	// Instigator 가 LocalController 이면 무시한다...

//	`log("avaVehicle.VehicleWeaponFired" @self @bViaReplication @HitLocation @SeatIndex @Role @Seats[SeatIndex].SeatPawn.IsLocallyControlled() );

	if ( bViaReplication == true && ( Role == ROLE_Authority || ( Seats[SeatIndex].SeatPawn.IsLocallyControlled() ) ) )	return;

	VehicleWeaponFireEffects(HitLocation, SeatIndex);
	VehicleWeaponImpactEffects(HitLocation, SeatIndex);
	if (SeatIndex == 0)
	{
		Seats[SeatIndex].Gun = avaVehicleWeapon(Weapon);
	}
	...
	if (Seats[SeatIndex].Gun != None)
	{
		...
		// 탄피출력.
		Seats[SeatIndex].Gun.PlayFiringSound2();
		Seats[SeatIndex].Gun.EjectBullet();
		PlayTrailEffect(SeatIndex, HitLocation);
		if ( EffectIsRelevant(Location,false,MaxFireEffectDistance) )
		{
			// CauseMuzzleFlash()함수가 호출 되어야 한다..
			// socket 위치도 제대로 되지 않는다.
			CauseMuzzleFlashLight(SeatIndex);
			Seats[SeatIndex].Gun.CauseMuzzleFlash();
		}
	}
}
...
simulated function VehicleWeaponImpactEffects(vector HitLocation, int SeatIndex)
{
	...
	local PlayerController PC;
	local Emitter E;
//	local avaExplosionLight ImpactLight;
	// avaWeaponAttachment.PlayImpactFX에서 가져옴.
	local Rotator					r;
//	local Rotator					BloodRotator;
...
//	`log("avaVehicle.VehicleWeaponImpactEffects" @HitLocation @SeatIndex @HitActor @HitInfo.PhysMaterial );
	...
	ImpactEffect = Seats[SeatIndex].Gun.GetImpactEffect(HitActor, HitInfo.PhysMaterial, SeatFiringMode(SeatIndex,, true));
		...
		else
		{
			if ( Seats[SeatIndex].GunClass.default.BulletWhip != None )
			{
				ForEach LocalPlayerControllers(PC)
				{
					if (!WorldInfo.GRI.OnSameTeam(self, PC))
					{
						avaPlayerController(PC).CheckBulletWhip(Seats[SeatIndex].GunClass.default.BulletWhip, Location, Normal(HitLocation - Location), HitLocation);
					}
				}
			}
	...
	{
//		`log("EffectInstigator.EffectIsRelevant - EffectInstigator" @EffectInstigator);
		...
		if ( HitActor != None && (Pawn(HitActor) == None || Vehicle(HitActor) != None) )
		{
			...
			{
				if ( ( HitNormal cross vector( EffectInstigator.Rotation ) ).z == 0.0 )
					r.Roll = avaPawn(Instigator).GetViewRotation().Yaw;
			}
			else if ( ImpactEffect.decalAlignType == DECAL_ALIGN_RANDOM )	
				r.Roll = FRand() * 65536.0f;

//			`log("ImpactEffect.ImpactDecals.Length =" @ImpactEffect.ImpactDecals.Length);
...
//			`log("ImpactEffect.ParticleTemplate =" @ImpactEffect.ParticleTemplate @class'avaOptionSettings'.Default.bDisableImpactParticle);
			...
			if ( ImpactEffect.ParticleTemplate != None && !(class'avaOptionSettings'.Default.bDisableImpactParticle) )
			{
				...
				{
					E = avaGameReplicationInfo(WorldInfo.GRI).ObjectPool.GetImpactEmitter(ImpactEffect.ParticleTemplate, HitLocation, rotator(HitNormal));			
					E.ParticleSystemComponent.ActivateSystem();
					E.SetTimer( 3, FALSE, 'HideSelf' );
				}
			}
		}
	}
...
simulated function Emitter SpawnImpactEmitter(vector HitLocation, vector HitNormal, MaterialImpactEffect ImpactEffect)
{
	...
	E = Spawn(class'avaEmitter',,, HitLocation, rotator(HitNormal));
	E.SetTemplate(ImpactEffect.ParticleTemplate, true);
	return E;
}

//! (from avaWeap_BaseGun)
simulated function PlayTrailEffect( int SeatIndex, vector HitLocation )
{
	local vector	l;
	local rotator	r;
	local float		d;

	if ( GetSeatMesh(SeatIndex).GetSocketWorldLocationAndRotation( 'GunFireSocket', l, r ) )
	{
...
{
//	`log("avaVehicle.VehicleAdjustFlashCount" @SeatIndex @FireModeNum @bClear);
...
{
//	`log("avaVehicle.VehicleAdjustFlashLocation" @SeatIndex @FireModeNum @NewLocation @bClear);
...
simulated function vector GetCameraFocus(int SeatIndex)
{
	...
	{
		GetSeatMesh(SeatIndex).GetSocketWorldLocationAndRotation(Seats[SeatIndex].CameraTag, CamStart);
	}
	else
	{
		CamStart = Location;
...
exec function FixedView(string VisibleMeshes)
{
	...
	for (SeatIdx=0; SeatIdx<Seats.length; ++SeatIdx)
	{
		P = avaPawn(Seats[SeatIdx].SeatPawn.Driver);
		if (P != None)
		{
...
simulated function VehicleCalcCamera(float DeltaTime, int SeatIndex, out vector out_CamLoc, out rotator out_CamRot, out vector CamStart, optional bool bPivotOnly)
{
	...
	if ( (avaPawn(Driver) != None) && !Driver.bHidden && Driver.Mesh.bOwnerNoSee )
		avaPawn(Driver).SetMeshVisibility(true);

	// Handle the fixed view
	P = avaPawn(Seats[SeatIndex].SeatPawn.Driver);
	...
	if (P != None)
	{
//! @comment 일단 위의 Vehicle의 Rotation값을 사용한다.(PlayerController부분까지 추가해 줘야 하더라.)
//		P.ModifyRotForDebugFreeCam(out_CamRot);
	...
	if ( !bCameraNeverHidesVehicle && bObstructed )
	{
		...
		if ( (avaPawn(Driver) != None) && !Driver.bHidden && (Driver.Mesh.bOwnerNoSee != Mesh.bOwnerNoSee) )
		{
...
simulated function bool ShouldSpawnExplosionLight(vector HitLocation, vector HitNormal)
{
	...
	ForEach LocalPlayerControllers(P)
	{
		...
		if ( (P.Pawn == Instigator) || 
			 ...
			 ((Dist < MaxExplosionLightDistance) && ((vector(P.Rotation) dot (Location - P.ViewTarget.Location)) > 0)) )
		{
			return true;
		}
	}
...
/*! @command avaEmitter를 갱신해야 사용 가능할 듯.
		if (BigExplosionTemplates.length > 0 && EffectIsRelevant(Location, false) )
		{
			Template = class'avaEmitter'.static.GetTemplateForDistance(BigExplosionTemplates, Location, WorldInfo);
			if (Template != None)
			{
				ProjExplosion = Spawn(class'avaEmitter', self);
...
/*		if (DeathExplosionShake != None)
		{
			foreach LocalPlayerControllers(class'avaPlayerController', avaPC)
			{
				Dist = VSize(Location - avaPC.ViewTarget.Location);
				if (avaPC == KillerController)
				{
					Dist *= 0.5;
				}
				if (Dist < OuterExplosionShakeRadius)
				{
					...
					avaPC.PlayCameraAnim(DeathExplosionShake, ShakeScale);
				}
			}
		}
*/
//! @comment UTDmgType_VehicleExplosion는 아직 없다.
...
//! @comment UTDmgType_VehicleCollision는 아직 없다.
/*
...
*/
	...
	*/
    simulated function VehicleCalcCamera(float fDeltaTime, int SeatIndex, out vector out_CamLoc, out rotator out_CamRot, 
										 out vector CamStart, optional bool bPivotOnly)
	{
...
{
/*! @comment WorldInfo.MyEmitterPool이 없다.
...
//		avaP.CurrentWeaponAttachment.ChangeAttachmentState( false );
		
//		avaP.SetWeaponAttachmentVisibility(false);
//		avaP.SetHandIKEnabled(false);
		if (bAttachDriver)
		{
			avaP.SetCollision( false, false);
...
simulated function SitDriver( avaPawn avaP, int SeatIndex)
{
//	`log( "avaVehicle.SitDriver" @self @avaP @SeatIndex @GetSeatMesh(SeatIndex) @Seats[SeatIndex].SeatBone );
	if (Seats[SeatIndex].SeatBone != '')
	{
		avaP.SetBase( Self, , GetSeatMesh(SeatIndex), Seats[SeatIndex].SeatBone);
	}
	else
	{
		avaP.SetBase( Self );
	}
	SetMovementEffect(SeatIndex,true, avaP);

	if ( Seats[SeatIndex].bSeatVisible )
	{
		if ( (avaP.Mesh != None) && (Mesh != None) )
		{
			avaP.Mesh.SetShadowParent(Mesh);
			avaP.Mesh.SetLightEnvironment(LightEnvironment);
		}
		avaP.SetMeshVisibility(true);
		avaP.SetRelativeLocation( Seats[SeatIndex].SeatOffset );
		avaP.SetRelativeRotation( Seats[SeatIndex].SeatRotation );
//		avaP.Mesh.SetCullDistance(5000);
		avaP.SetHidden(false);
	}
	else
	{
		avaP.SetHidden(true);
...
simulated function DetachDriver(Pawn P)
{
	...
	if (avaP != None)
	{
		//avaP.CurrentWeaponAttachment.ChangeAttachmentState( true );
		//avaP.SetWeaponAttachmentVisibility(true);
//		avaP.SetHandIKEnabled(true);
//		SetMovementEffect(0,false);
	}
}

/** Allows a vehicle to do specific physics setup on a driver when physics asset changes. */
simulated function OnDriverPhysicsAssetChanged(avaPawn avaP);
					...
					const out CollisionImpactData RigidCollisionData, int ContactIndex )
{
	...
	if (LastCollisionDamageTime != WorldInfo.TimeSeconds)
	{
		...
		if (OtherComponent != None && avaPawn(OtherComponent.Owner) != None && OtherComponent.Owner.Physics == PHYS_RigidBody)
		{
		...
		{
			// take impact damage
			Damage = int(VSizeSq(Mesh.GetRootBodyInstance().PreviousVelocity) * GetCollisionDamageModifier(RigidCollisionData.ContactInfos));
			if (Damage > 1)
			{
//				TakeDamage(Damage, GetCollisionDamageInstigator(), ContactInfos[0].ContactPosition, vect(0,0,0), class'avaDmgType_VehicleCollision');
...
{
//! @comment UTVehicleSimCar는 아직 추가하지 않았다.
/*
	local int i;
...
function OnExitVehicle(avaSeqAct_ExitVehicle Action)
{
	...
	{
		// 해당 자리의 Driver만 내리게 한다.
		if ( Action.SeatIndex < Seats.Length )
		{
			i = Action.SeatIndex;

			// 해당 Seat의 Driver만 내리게 한다.
			if (Seats[i].SeatPawn != None && Seats[i].SeatPawn.Driver != None)
				Seats[i].SeatPawn.DriverLeave(true);
		}
	}
...
function SetSeatStoragePawn(int SeatIndex, Pawn PawnToSit)
{
	...
	// HM처리 후 탑승자를 복원하기 위해서 저장.
	if ( PawnToSit != None && PlayerController(Seats[SeatIndex].SeatPawn.Controller) != None )
	{
		if ( avaPlayerReplicationInfo(Seats[SeatIndex].SeatPawn.PlayerReplicationInfo) != None )
			bHost = avaPlayerReplicationInfo(Seats[SeatIndex].SeatPawn.PlayerReplicationInfo).bHost;

		// 자기자신이 호스트가 아닌 경우에만 (복원하기 위해) 저장하면 된다.
		// (호스트가 나갈 경우에는 복원할 필요가 없다)
		if ( !bHost )
			PlayerNames[SeatIndex] = Seats[SeatIndex].SeatPawn.PlayerReplicationInfo.PlayerName;
		else
			PlayerNames[SeatIndex] = "";
	}
	else
		PlayerNames[SeatIndex] = "";

//	`log("PlayerNames[" @SeatIndex @"] =" @PlayerNames[SeatIndex]);
...
simulated function SetMovementEffect(int SeatIndex, bool bSetActive,optional avaPawn avaP)
{
/*
	local bool bIsLocal;
	if(bSetActive && ReferenceMovementMesh != none)
	{
		bIsLocal = avaP==none? IsLocallyControlled():avaP.IsLocallyControlled();
...
simulated function DisplayHud(avaHud Hud, Canvas Canvas, vector2D HudPOS, optional int SeatIndex)
{
/*
	local vector2D POS;
	local float W,H;
...
simulated function DisplayExtraHud(avaHud Hud, Canvas Canvas, vector2D POS, float Width, float Height, int SIndex);

simulated function DisplaySeats(avaHud Hud, Canvas Canvas, vector2D HudPOS, float Width, float Height, int SIndex)
{
/*
	local int i;
	local float X,Y;
...
simulated function SkeletalMeshComponent GetSeatMesh(int SeatIndex)
{
	if ( SeatIndex < Seats.Length )
	{
		// 해당 자리에 대한 메쉬가 설정된 경우에만 리턴해준다.
		if ( Seats[SeatIndex].Mesh != None )
			return Seats[SeatIndex].Mesh;
	}

	// GunMesh가 None이거나 Seat[SeatIndex].Mesh가 None이면......... 안되는건데...
//	`log("### VehicleHMProblem Seats.Length=" @Seats.Length 
//		@"Seats[" @SeatIndex @"].Mesh=" @Seats[SeatIndex].Mesh);

	// 그렇지 않은 경우에는 기본 메쉬를 리턴해 준다.
	return Mesh;
}
...
{
//	`log("[dEAthcURe|avaVehicle::OnHmRestore] hmID=" @ hmId 
//		@"Loc" @ Location @"Rot" @ Rotation @"Phys" @Physics 
//		@"AIScriptedRouteIndex" @AIScriptedRouteIndex);

//	`log("ForceWeaponRotation(0, " @WeaponRotation @")");

	// 복구된 WeaponRotation을 강제로 적용해 준다.
	ForceWeaponRotation(0, WeaponRotation);

	// 만약 탑승중인 Pawn이 있었다면 다시 탑승시켜야 한다.
	// (호출에 실패하면 타이머 함수로 계속 호출되도록 한다)
	if ( !RestoreDriver() )
		SetTimer(1, true, 'RestoreDriver');
...
function TriggerSpawnedEvent()
{
	if ( ParentFactory != None )
	{
//		`log("avaVehicle.TriggerSpawnedEvent()" @ParentFactory);
		ParentFactory.TriggerSpawnedEventEx(0.1);
	}
}

//! HM 후 탑승했던 드라이버를 다시 태워준다.
function bool RestoreDriver()
{
	local PlayerController	PC;
	local int				i;
	local int				UsedCount;
	local int				ProcessedCount;

	TryCount++;

//	`log("avaVehicle.RestoreDriver - TryCount =" @TryCount);

	// 만약 defaultproperties를 통과하지 못한 경우에는
	// 다시 실행하도록 false를 리턴한다.
	if ( Seats.Length == 0 )
		return false;
	...
	if ( Driver == None )
	{
		`log("avaVehicle.RestoreDriver - (Driver == None)");
		return false;
	}

	// 최대 횟수 이상 호출되었다면 제거한다.
	if ( TryCount > MaxTryCount )
	{
		ClearTimer('RestoreDriver');
		return false;
	}
	...
	for ( i = 0; i < `MAX_PLAYERNAMES; i++ )
	{
		if ( PlayerNames[i] == "" )
		{
			`log("RestoreDriver - PlayerNames[" @i @"].Length == 0");
			continue;
		}

//		`log("avaVehicle.RestoreDriver - UsedCount =" @UsedCount @PlayerNames[i]);

		UsedCount++;

		// 탑승했던 드라이버와 같은 이름이 있는지 찾는다.
		// (Host에서 다른 Client의 PlayerController를 얻어와야 한다)
		foreach WorldInfo.DynamicActors(class'PlayerController', PC)
		{
			// 이미 탑승 처리가 되었다면.
			if ( Vehicle(PC.Pawn) != None )
			{
//				`log("RestoreDriver - already enter vehicle" @PC.Pawn);
				ProcessedCount++;
				continue;
			}
			else if ( PlayerNames[i] == PC.PlayerReplicationInfo.PlayerName )
			{
				// 해당 자리에 누군가 앉아있다면 강제로 내리게 한다.
				//if ( Seats[i].SeatPawn != None && Seats[i].SeatPawn.Driver != None )
				//	Seats[i].SeatPawn.DriverLeave(true);
				...
				{
					DriverEnter(PC.Pawn);
//					`log("RestoreDriver - DrivenEnter" @PC.Pawn);
				}
				else
				{
					// 무조건 기존 탑승자가 있다면 내려!!
					...
					PassengerEnter(PC.Pawn, i);
//					`log("RestoreDriver - PassengerEnter" @i @PC.Pawn);
				}
			...
			{
//				`log("RestoreDriver - cannot found PlayerName" @PlayerNames[i] @PC.Pawn);
			}
		}
	}
	...
	{
		`log("RestoreDriver - Complete " @ProcessedCount);
		ClearTimer('RestoreDriver');
		return true;
	}
...
//	BigExplosionTemplates[0]=(Template=ParticleSystem'FX_VehicleExplosions.Effects.P_FX_VehicleDeathExplosion')
	...
	Team=AVAVEHICLE_UNSET_TEAM // impossible value so that we know if it hasn't replicated yet
