 ...
 */
class avaDefensePoint extends NavigationPoint
	placeable
	native;

var Controller CurrentUser;
var avaDefensePoint NextDefensePoint;	// list of defensepoints for same objective
var() avaGameObjective DefendedObjective;
...
function PreBeginPlay()
{
	...
	else if ( bFirstScript )
	{
		...
		ForEach AllActors(class'avaDefensePoint',S)
 ...
 */
function bool HigherPriorityThan(avaDefensePoint S, avaBot B, bool bAutoPointsInUse, bool bPrioritizeSameGroup, out int NumChecked)
{
	if (bBlocked || (bOnlyOnFoot && Vehicle(B.Pawn) != None) || (!bAutoPointsInUse && DefensePriority < DEFPRI_High))
	{
		return false;
	}
	if (CurrentUser != None && !CurrentUser.bDeleteMe && CurrentUser != B && WorldInfo.GRI.OnSameTeam(CurrentUser, B))
	{
		if (avaBot(CurrentUser) != None && avaBot(CurrentUser).DefensePoint != self)
		{
			avaBot(CurrentUser).DefensePoint = None;
		}
		else
		{
			return false;
