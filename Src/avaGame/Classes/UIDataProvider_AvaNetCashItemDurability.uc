/*
	유료 아이템상점에서 내구도를 변경시 보여준다.
*/
class UIDataProvider_AvaNetCashItemDurability extends avaUIParamDataProvider
	native(inherit)
...
var input int					SelectedShopItemListIndex;		//!< 선택된 상점 아이템의 리스트 인덱스.

/*! @brief 선택된 내구도 리스트 인덱스.
	@note
		Shop에서 스크롤 시에도 선택된 값이 값이 유지되도록 하기 위해서저장한다.
*/
...
defaultproperties
{
	...
	SelectedShopItemListIndex = INDEX_NONE
	DurabilityListIndex = 0
}