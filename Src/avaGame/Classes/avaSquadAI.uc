...
class avaSquadAI extends ReplicationInfo
	native;

var avaTeamInfo Team;
var Controller SquadLeader;
var avaPlayerReplicationInfo LeaderPRI;
var avaSquadAI NextSquad;	// list of squads on a team
var avaGameObjective SquadObjective;
var int Size;
var avaBot SquadMembers;
...
var avaBot PendingSquadRouteMaker;
...
function bool ShouldSuppressEnemy(avaBot B)
{
	return ( (FRand() < 0.7) && (VSize(B.Enemy.Location - B.FocalPoint) < 350)
			&& (WorldInfo.TimeSeconds - B.LastSeenTime < 4) );
}

function bool AllowDetourTo(avaBot B,NavigationPoint N)
{
	return true;
}
...
function bool AllowTranslocationBy(avaBot B)
{
	return true;
}

function bool AllowImpactJumpBy(avaBot B)
{
	return true;
}

function actor SetFacingActor(avaBot B)
{
	return None;
}

function avaVehicle GetLinkVehicle(avaBot B)
{
	local avaVehicle V;

	if (avaVehicleBase(SquadLeader.Pawn) == None)
	{
		return None;
	}
	else
	{
		V = avaVehicle(SquadLeader.Pawn.GetVehicleBase());
...
*/
function bool LostEnemy(avaBot B)
{
	local pawn Lost;
	local bool bFound;
	local avaBot M;
...
function bool SetEnemy( avaBot B, Pawn NewEnemy )
{
	local avaBot M;
	local bool bResult;
...
function byte PriorityObjective(avaBot B)
{
	return 0;
}
...
function NotifyKilled(Controller Killer, Controller Killed, pawn KilledPawn)
{
	...
	if (Killed == SquadLeader)
	{
		...
		{
			PendingSquadRouteMaker = avaBot(SquadLeader);
		}
	}
...
function bool FindNewEnemyFor(avaBot B, bool bSeeEnemy)
{
	local int i;
...
*/
function float ModifyThreat(float current, Pawn NewThreat, bool bThreatVisible, avaBot B)
{
	return current;
}

function bool UnderFire(Pawn NewThreat, avaBot Ignored)
{
	local avaBot B;
...
function float AssessThreat( avaBot B, Pawn NewThreat, bool bThreatVisible )
{
...
{
	if ( avaBot(P.Controller).NeedWeapon() || (PlayerController(SquadLeader) == None) || (SquadLeader.Pawn == None) )
		return false;
	return CloseToLeader(P);
}

function bool WanderNearLeader(avaBot B)
{
...
function bool CloseToLeader(Pawn P)
{
	...
	if ( Vehicle(P) == None )
	{
		if ( (avaVehicle(SquadLeader.Pawn) != None)
			&& avaVehicle(SquadLeader.Pawn).OpenPositionFor(P) )
			return false;
	}
...
function MergeWith(avaSquadAI S)
{
...
function Initialize(avaTeamInfo T, avaGameObjective O, Controller C)
{
	Team = T;
	SetLeader(C);
...
function SetAlternatePathTo(NavigationPoint NewRouteObjective, avaBot RouteMaker)
{
	local avaBot M;
...
function bool TryToIntercept(avaBot B, Pawn P, Actor RouteGoal)
{
...
function bool CloseEnoughToObjective(avaBot B, Actor O)
{
	return ( B.Pawn.Location.Z - O.Location.Z < 500.0 &&
		((VSize(B.Pawn.Location - O.Location) < avaVehicle(B.Pawn).ObjectiveGetOutDist && B.LineOfSightTo(O)) || B.ActorReachable(O)) );
}
 ...
 */
function bool LeaveVehicleToReachObjective(avaBot B, Actor O)
{
...
/*?
	...
	if (avaCarriedObject(O) != None)
	{
		return true;
	}
	else
	{
		Objective = avaGameObjective(O);
...
function LeaveVehicleAtParkingSpot(avaBot B, Actor O)
{
//	local avaVehicle_Deployable DeployableVehicle;

	if (!B.Pawn.bStationary && avaVehicle(B.Pawn) != None && avaVehicle(B.Pawn).bKeyVehicle)
	{
		if ( Team.Size == avaVehicle(B.Pawn).NumPassengers() )
		{
			B.NoVehicleGoal = O;
			B.RouteGoal = O;
			avaVehicle(B.Pawn).bKeyVehicle = false;
			B.LeaveVehicle(true);
		}
		else
		{
/*! avaVehicle_Deployable NOT implemented.
			DeployableVehicle = avaVehicle_Deployable(B.Pawn);
...
*/
function bool FindPathToObjective(avaBot B, Actor O)
{
	local Vehicle V;
	local avaGameObjective Objective;
	local int i;
	...
	{
		if (avaVehicle(B.Pawn) != None && avaVehicle(B.Pawn).bKeyVehicle && avaVehicle(B.Pawn).NumPassengers() < Team.Size )
		{
			if (CloseEnoughToObjective(B, O))
			{
				return false;
...
event SetLeader(Controller C)
{
	...
	{
		LeaderPRI = avaPlayerReplicationInfo(C.PlayerReplicationInfo);
		NetUpdateTime = WorldInfo.Timeseconds - 1;
	}
	if ( avaBot(C) != None )
		AddBot(avaBot(C));
...
{
	local avaGameObjective NewObjective;
...
function AddBot(avaBot B)
{
	...
	SquadMembers = B;
	B.Squad = self;
	if ( avaPlayerReplicationInfo(B.PlayerReplicationInfo) != None )
	{
//?		avaPlayerReplicationInfo(B.PlayerReplicationInfo).Squad = self;
	}
}

function SetDefenseScriptFor(avaBot B)
{
	local avaDefensePoint OldPoint, S;
	local bool bPrioritizeSameGroup, bAutoPointsInUse, bAllDefenseGroupsCovered;
	local int NumChecked;
	local avaBot OtherBot;
	...
	foreach WorldInfo.AllControllers(class'avaBot', OtherBot)
	{
...
function SetObjective(avaGameObjective O, bool bForceUpdate)
{
	local avaBot M;
	...
	else
	{
		...
		if ( SquadObjective != None )
		{
			if ( (SquadObjective.DefenderTeamIndex == Team.TeamIndex) && (SquadObjective.DefenseSquad == None) )
					SquadObjective.DefenseSquad = self;
			RouteObjective = None;
			if ( avaBot(SquadLeader) != None )
				SetAlternatePathTo(SquadObjective, avaBot(SquadLeader));
		}
	}
	for	( M=SquadMembers; M!=None; M=M.NextSquadMember )
		if ( M.Pawn != None )
...
simulated function String GetOrderStringFor(avaPlayerReplicationInfo PRI)
{
...
simulated function String GetShortOrderStringFor(avaPlayerReplicationInfo PRI)
{
	if ( (LeaderPRI != None) && !LeaderPRI.bBot )
	{
		// FIXME - holding replication
...
function PickNewLeader()
{
	...
	if ( SquadLeader != B )
	{
		...
		if ( SquadLeader == None )
			...
			LeaderPRI = avaPlayerReplicationInfo(SquadLeader.PlayerReplicationInfo);
		NetUpdateTime = WorldInfo.Timeseconds - 1;
	}
}

function bool TellBotToFollow(avaBot B, Controller C)
{
	local Pawn Leader;
	...
	if ( CloseToLeader(B.Pawn) )
	{
		...
		else if ( (avaWeapon(B.Pawn.Weapon) != None) && avaWeapon(B.Pawn.Weapon).FocusOnLeader(false) )
		{
			B.FightEnemy(false,0);
			return true;
		}
...
function bool AllowTaunt(avaBot B)
{
	return ( FRand() < 0.5 );
}

function AddTransientCosts(avaBot B, float f)
{
	local avaBot S;
...
function bool AssignSquadResponsibility(avaBot B)
{
	...
	else if ( (B.DefensePoint != None) /*? && (avaHoldSpot(B.DefensePoint) == None)*/ )
		B.FreePoint();
...
{
	return (avaVehicle(P) != None && avaVehicle(P).bKeyVehicle);
}

function BotEnteredVehicle(avaBot B)
{
	if ( (PlayerController(SquadLeader) != None) )
	{
...
function bool GetOnHoverboard(avaBot B)
{
	local avaPawn P;
...
function bool EnterAndExitVehicle(avaBot B)
{
	local avaVehicle V;

	V = avaVehicle(B.RouteGoal);
...
function bool GotoVehicle(avaVehicle SquadVehicle, avaBot B)
{
...
*/
function bool CheckVehicle(avaBot B)
{
	local avaVehicle V, SquadVehicle;
...
//	local avaPawn P;
	...
	if (B.NoVehicleGoal != None)
	{
		...
		if (B.NoVehicleGoal.IsA('avaCarriedObject'))
		{
//?			B.NoVehicleGoal = avaCarriedObject(B.NoVehicleGoal).LastAnchor;
		}
	...
	if (Vehicle(B.Pawn) == None && V != None)
	{
		...
		if (!bSkip)
		{
			...
			if (V.Health < V.Default.Health && (B.Enemy == None || !B.LineOfSightTo(B.Enemy)) && B.CanAttack(V))
			{
				...
				{
/*?
					if (avaWeapon(B.Pawn.Weapon) != None && avaWeapon(B.Pawn.Weapon).CanHeal(V))
					{
						B.GoalString = "Heal "$V;
					...
					else
					{
						...
						if (avaWeapon(B.Pawn.InvManager.PendingWeapon) != None && avaWeapon(B.Pawn.InvManager.PendingWeapon).CanHeal(V))
						{
							B.GoalString = "Heal "$V;
							B.DoRangedAttackOn(V);
							return true;
...
*/
	...
	if (BotVehicle != None)
	{
		if (!NeverBail(BotVehicle))
		{
			...
			{
				// vehicle is stuck
				if (BotVehicle.IsA('avaVehicle'))
				{
					avaVehicle(BotVehicle).VehicleLostTime = WorldInfo.TimeSeconds + 20;
				}
				B.LeaveVehicle(true);
				return true;
			}
			else if ((BotVehicle.Health < BotVehicle.Default.Health * 0.125) && !BotVehicle.bStationary && (B.Skill + B.Tactics > 4.0 + 7.0 * FRand()))
			{
				...
				if (BotVehicle.IsA('avaVehicle'))
				{
					avaVehicle(BotVehicle).VehicleLostTime = WorldInfo.TimeSeconds + 10;
				}
				B.LeaveVehicle(true);
				return true;
			}
			...
			{
				V = avaVehicle(BotVehicle);
				if (V == None)
				{
					V = avaVehicle(BotVehicle.GetVehicleBase());
				}
...
/* //@todo steve
	if (SpecialVehicleObjective(B.RouteGoal) != None && CheckSpecialVehicleObjectives(B))
		return true;
*/
	if ( (avaVehicle(SquadLeader.Pawn) != None) && (VSize(SquadLeader.Pawn.Location - B.Pawn.Location) < 4000) )
	{
		SquadVehicle = avaVehicle(SquadLeader.Pawn).OpenPositionFor(B.Pawn) ? avaVehicle(SquadLeader.Pawn) : None;
	}
	...
	if ( SquadVehicle == None )
	{
		for ( S=SquadMembers; S!=None; S=S.NextSquadMember )
		{
			if ( (avaVehicle(S.Pawn) != None) && (VSize(S.Pawn.Location - B.Pawn.Location) < BestDist) && avaVehicle(S.Pawn).OpenPositionFor(B.Pawn) )
			{
				SquadVehicle = avaVehicle(S.Pawn);
				break;
			}
		}
	}

	if ( SquadVehicle == None && avaVehicle(SquadLeader.RouteGoal) != None && !avaVehicle(SquadLeader.RouteGoal).Occupied()
	     && VSize(SquadLeader.RouteGoal.Location - B.Pawn.Location) < BestDist && avaVehicle(SquadLeader.RouteGoal).OpenPositionFor(B.Pawn) )
	{
		SquadVehicle = avaVehicle(SquadLeader.RouteGoal);
	}

	if ( SquadVehicle == None && avaVehicle(B.RouteGoal) != None && !avaVehicle(B.RouteGoal).Occupied() &&
		VSize(B.RouteGoal.Location - B.Pawn.Location) < BestDist )
	{
		SquadVehicle = avaVehicle(B.RouteGoal);
	}
	...
	if ( SquadVehicle == None )
	{
		...
		GetOrders();
/*? avaGame.VehicleList NOT implemented.
		for ( V=avaGame(WorldInfo.Game).VehicleList; V!=None; V=V.NextVehicle )
		{
...
function float VehicleDesireability(avaVehicle V, avaBot B)
{
	local float result;
...
/* //@todo steve
function bool CheckSpecialVehicleObjectives(avaBot B)
{
	local avaGame G;
	local SpecialVehicleObjective O, Best;

	G = avaGame(WorldInfo.Game);
	if (G == None)
		return false;
...
*/
function bool OverrideFollowPlayer(avaBot B)
{
	local avaGameObjective PickedObjective;
...
function bool CheckSquadObjectives(avaBot B)
{
	local Actor DesiredPosition;
	local bool bInPosition, bMovingToSuperPickup;
	local float SuperDist;

	if ( /*? (avaHoldSpot(B.DefensePoint) == None) &&*/ CheckVehicle(B) )
		return true;
...
/*?
		if ( avaHoldSpot(B.DefensePoint) != None )
		{
			if ( avaHoldSpot(B.DefensePoint).HoldVehicle != B.Pawn )
			{
				B.LeaveVehicle(true);
				return true;
			}
...
*/
		if (avaVehicle(B.Pawn) != None && avaVehicle(B.Pawn).bKeyVehicle)
		{
			if ( B.DefensePoint != None )
				B.FreePoint();
			return false;
	...
	if ( B.Pawn.bCanPickupInventory && (B.Skill > 1) /*? && (avaHoldSpot(B.DefensePoint) == None)*/ )
	{
...
function bool ShouldDestroyTranslocator(avaBot B)
{
	local avaGame G;
//	local avaProj_TransDisc T;
...
/*! avaProj_TransDisc NOT implemented.
...
function float BotSuitability(avaBot B)
{
...
pick a bot to lose
*/
function avaBot PickBotToReassign()
{
	local avaBot B,Best;
...
function bool PickRetreatDestination(avaBot B)
{
...
function bool IsDefending(avaBot B)
{
	if ( GetOrders() == 'Defend' )
		return true;
...
function NavigationPoint FindDefensivePositionFor(avaBot B)
{
...
function float RateDefensivePosition(NavigationPoint N, avaBot CurrentBot, Actor Center)
{
	local float Rating;
	local float Dist;
	local avaBot B;
...
function bool AcceptableDefensivePosition(NavigationPoint N, avaBot B)
{
	local Actor Center;
...
function avaGameObjective GetStartObjective(avaBot B)
{
 ...
 */
function bool HandlePathObstruction(avaBot B, Actor BlockedBy)
{
	local int i;
