...
***/
class avaNetEntryGameRe extends avaNetEntryGame;
...
event PostLogin( PlayerController NewPlayer )
{
	...
	GlobalScene = LoadScene( class'GameInfo'.static.GetFullUIPath(GlobalSceneName) );
	if( GlobalScene == None )
...
`devexec function GotoScene( string InScene, optional bool bDirect, optional name InNextUIEventName, optional bool bPreload )
{
	...
	NextSceneName = class'GameInfo'.static.GetFullUIPath(NextSceneName);
...
{
	DynamicLoadObject( class'GameInfo'.static.GetFullUIPath(default.TitleSceneName), class'Object' );	
}
...
defaultproperties
{
	...
	LogoSceneName			=	"Logo.NeowizLogo"
	RedduckLogoSceneName	=	"Logo.Logo"
	AvaLogoSceneName		=	"Logo.avaBI"
	TitleSceneName			=	"Title.Title"
	NewCharacterSceneName	=	"CreateCharacter.CreateCharacter"
	ChannelListSceneName	=	"Channel.ChannelList"
	LobbySceneName			=	"Lobby.Lobby"
	ClanLobbySceneName		=	"ClanLobby.ClanLobby"
	RoomSceneName			=	"ReadyRoom.ReadyRoom"
	ResultSceneName			=	"Result.Result"
	ResultMDSceneName		=	"Result.Result_MilitaryDrill"
	
	ExitSceneName	= "_Exit.Exit"

//  PopUpMessage를 Kismet으로 넘기지 않고 원래의 팝업을 사용하였음
	PopUpMsgSceneName = "Misc.PopUpWindow"
	WidgetMessageLabel = "UILabel_PopUpMessage"

	GlobalSceneName			=	"Global.GlobalScene"

	DefaultUIPathPrefix		=	"avaUISceneRe"
}
