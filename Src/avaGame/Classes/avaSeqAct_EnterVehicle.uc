/*
	...
	2007/05/11	고광록
		UTSeqAct_EnterVehicle에서 그대로 들고옴.
*/
class avaSeqAct_EnterVehicle extends SequenceAction;
...
event Activated()
{
	...
	local Pawn Target;
	local avaVehicle TheVehicle;
	local avaVehicleFactory Factory;

	foreach LinkedVariables(class'SeqVar_Object', ObjVar, "Vehicle/Vehicle Factory")
	{
		TheVehicle = avaVehicle(ObjVar.GetObjectValue());
		if (TheVehicle == None)
		{
			Factory = avaVehicleFactory(ObjVar.GetObjectValue());
			if (Factory != None)
			{
	...
	else
	{
		...
		{
			Target = Pawn(ObjVar.GetObjectValue());
			if (Target != None)
			{
