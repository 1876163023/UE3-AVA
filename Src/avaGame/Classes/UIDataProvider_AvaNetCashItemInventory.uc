 ...
 * @ingroup _AVAGame
 *
 * 유료 아이템중 자신이 가진 아이템 정보를 제공하는 데이터 프로바이더
 *
 * @date 2007-11-12
 *
 * @author YTS
 *
 */
class UIDataProvider_AvaNetCashItemInventory extends avaUIParamDataProvider
	native(inherit)
	implements(UIListElementCellProvider, UIListElementProvider)
	transient;

enum CashItemEffectType
{
	CASHITEMEFFECT_All,
	CASHITEMEFFECT_HotAndNew,
	CASHITEMEFFECT_Xp,
	CASHITEMEFFECT_Supply,
	CASHITEMEFFECT_Money,
};
...
var input CashItemEffectType	CurrentEffectType;
var input int					SelectedItemListIndex;

defaultproperties
{
	WriteAccessType=ACCESS_WriteAll

	CurrentEffectType=CASHITEMEFFECT_All
