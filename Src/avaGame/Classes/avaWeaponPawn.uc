/*
	...
	@note
		Seat[0].SeatPawn = Vehicle계열.
		Seat[1..n].SeatPawn = WeaponPawn계열.
*/

class avaWeaponPawn extends avaVehicleBase
	native
	nativereplication
	notplaceable;

/** MyVehicleWeapon points to the weapon assoicated with this WeaponPawn and is replcated */
var repnotify avaVehicleWeapon MyVehicleWeapon;
...
{
	`log( "avaWeaponPawn.StartFire" @FireModeNum @InvManager @Weapon @Weapon.GetStateName() );
...
simulated event ReplicatedEvent(name VarName)
{
	...
	{
		//`log("avaWeaponPawn.ReplicatedEvent " @VarName);
		...
		if (MySeatIndex > 0 && MyVehicle != None && MySeatIndex < MyVehicle.Seats.length)
		{
			...
			if ( VarName == 'MyVehicleWeapon' )
				`log("avaWeaponPawn.ReplicatedEvent2 - " @self @MyVehicleWeapon.InvManager @InvManager);

			// 어디서 Pawn.Weapon의 설정을 하는 걸까요...?
			//Weapon = MyVehicleWeapon;

			// 카메라 반경을 설정.
			if ( MyVehicle.Seats[MySeatIndex].ViewPitchMin != 0.0 )
				ViewPitchMin = MyVehicle.Seats[MySeatIndex].ViewPitchMin;
			if ( MyVehicle.Seats[MySeatIndex].ViewPitchMax != 0.0 )
				ViewPitchMax = MyVehicle.Seats[MySeatIndex].ViewPitchMax;
		}
	}
	else 
	{
		if ( VarName == 'InvManager' )
		{
			...
			{
				MyVehicleWeapon.InvManager = InvManager;
			}
			`log("avaWeaponPawn.ReplicatedEvent InvManager" @self @MyVehicleWeapon );
		}
...
{
//! @comment WorldInfo.bUseConsoleInput는 사용하지 않는다.(PC니까 해당사항이 없기도 하다)
/*
...
/**
 * An accessor for the location of the current target.  WeaponPawn's forward this to the controlling
 * vehicle.
 *
 * @param	RequiredBy	What actor is requesting this information
 * @Returns the current Target location.
 */

simulated function vector GetTargetLocation(optional Actor RequestedBy)
{
	if ( MyVehicle != None )
		return MyVehicle.GetTargetLocation();
...
//	`log("avaWeaponPawn.AttachDriver - Pawn:" @P @MySeatIndex);
...
//! @comment UTCarriedObject를 사용하지 않아서 제외.
/*
...
{
//! @comment avaBot은 아직 붙이지 않았다.
//	local avaBot B;
//	B = avaBot(Controller);

	if (Super.DriverLeave(bForceLeave))
	{
		// bot might have gotten in because of temporary orders through avaVehicle::PlayHorn(), so clear that now
//		if (B != None)
//		{
...
function bool Died(Controller Killer, class<DamageType> DamageType, vector HitLocation, optional bool bScoreKill = true, optional class<Weapon> weaponBy)
{
	...
	if (Super.Died(Killer, DamageType, HitLocation))
	{
		...
		if (OldPC != None && MyVehicle != None)
		{
			...
			OldPC.ClientSetViewTarget(MyVehicle);
		}
		Destroy();
		return true;
	}
 ...
 */
function DisplayHud(avaHud Hud, Canvas Canvas, vector2D HudPOS, optional int SIndex)
{
	if ( MyVehicle != none )
	{
...
defaultproperties
{
	//@note: even though avaWeaponPawns don't usually have visible components, they must have bHidden=false so AI can see them
