...
class avaBot extends AIController
	native;
...
var avaSquadAI		Squad;			// 분대처리 제외.

/** linked list of members of this squad */
var avaBot			NextSquadMember;
...
var avaDefensePoint DefensePoint;	// DefensePoint to which bot is assigned (assigned by TeamAI)
var NavigationPoint DefensivePosition;

var avaVehicle		FormerVehicle;
...
var		avaAvoidMarker	FearSpots[2];	// avoid these spots when moving - used for very short term stationary hazards like bio goo or sticky grenades
...
var avaBotDecisionComponent DecisionComponent;
...
*
* Returns shortest path to squad route (avaSquadAI.ObjectiveRouteCache), or next node along route
...
//	if ( avaGame(WorldInfo.Game).bSoaking )
//		bSoaking = true;
}
...
function AdjustView( float DeltaTime )
{
	...
	if ( avaPawn(Pawn).bUpdateEyeHeight )
	{
		...
		if (!bViewTarget)
		{
			...
			avaPawn(Pawn).bUpdateEyeHeight = false;
		}
	}
...
function weapon HasSuperWeapon()
{
	...
	ForEach CheckPawn.InvManager.InventoryActors(class'avaWeapon',W)
	{
...
function NotifyAddInventory(inventory NewItem)
{
	...
	bHasSuperWeapon = (avaWeapon(NewItem) != None && avaWeapon(NewItem).bSuperWeapon);
}
...
event SetupSpecialPathAbilities()
{
	...
	if ( Pawn.bCanJump )
	{
		...
		if ( (avaPawn(Pawn) != None) )
		{
			...
			MultiJumpZ = (Pawn.JumpZ * 0.3) /*? + avaPawn(Pawn).MultiJumpBoost*/;
...
function GetOutOfVehicle()
{
	if (Vehicle(Pawn) != None)
	{
		...
		{
			avaVehicle(Pawn).PlayerStartTime = WorldInfo.TimeSeconds + 20;
		}
...
function FearThisSpot(avaAvoidMarker aSpot)
{
	local int i;
...
//	if( T == avaGame(WorldInfo.Game).LastTaunt[0] || T == avaGame(WorldInfo.Game).LastTaunt[1] )
//		return true;

	LastTaunt = T;

//	avaGame(WorldInfo.Game).LastTaunt[1] = avaGame(WorldInfo.Game).LastTaunt[0];
//	avaGame(WorldInfo.Game).LastTaunt[0] = T;
	return false;
}
...
function ImpactJump()
{
	...
	if (!Pawn.SuggestJumpVelocity(Pawn.Velocity, ImpactTarget.Location, Pawn.Location) )
	{
		// need a double jump
		bNotifyApex = true;
		bPendingDoubleJump = true;
...
function OnAIStartFireAt(avaSeqAct_AIStartFireAt FireAction)
{
...
function OnAIStopFire(avaSeqAct_AIStopFire FireAction)
{
	ScriptedTarget = None;
	StopFiring();
...
simulated function float RateWeapon(Weapon w)
{
	...
	Rating = avaWeapon(W).GetAIRating() + FRand() * 0.05 + WeaponPreference(W);
...
{
//?	if ( avaHoldSpot(DefensePoint) != None )
//?		return 'Hold';
...
//?	avaPlayerReplicationInfo(PlayerReplicationInfo).SendVoiceMessage(Recipient, MessageType, MessageID, BroadcastType);
}
...
function SetOrders(name NewOrders, Controller OrderGiver)
{
	...
	bInstantAck = false;
	avaTeamInfo(PlayerReplicationInfo.Team).AI.SetOrders(self,NewOrders,OrderGiver);
	WhatToDoNext();
}
...
function ClearTemporaryOrders()
{
	if (OldOrders != 'None')
	{
		...
		avaTeamInfo(PlayerReplicationInfo.Team).AI.SetOrders(self,OldOrders,OldOrderGiver);
...
function bool ClearShot(Vector TargetLoc, bool bImmediateFire)
{
	...
	if ( !bSeeTarget || !FastTrace(TargetLoc , Enemy.Location + Enemy.BaseEyeHeight * vect(0,0,1)) );
		return false;
	if ( (/*? avaWeapon(Pawn.Weapon).bSplashDamage &&*/ (VSize(Pawn.Location - TargetLoc) < avaWeapon(Pawn.Weapon).GetDamageRadius()))
...
function bool IsSniping()
{
	...
	return ( (DefensePoint != None) && DefensePoint.bSniping && avaWeapon(Pawn.Weapon) != None /*? && avaWeapon(Pawn.Weapon).bSniping*/
			&& Pawn.ReachedDestination(DefensePoint.GetMoveTarget()) );
}
...
function float RelativeStrength(Pawn Other)
{
	...
	CompareValue = 0.01 * float(adjustedOther - Pawn.health);
	CompareValue = CompareValue - Pawn.AdjustedStrength() + Other.AdjustedStrength();

	if ( avaWeapon(Pawn.Weapon) != None )
	{
		CompareValue -= 0.5 * Pawn.GetDamageScaling() /*? * avaWeapon(Pawn.Weapon).CurrentRating*/;
	...
	if ( Other.Weapon != None )
		CompareValue += 0.5 * Other.GetDamageScaling() * Other.Weapon.AIRating;
...
/*?
	...
	{
		if ( (X Dot Dir) > 0 )
			avaPawn(Pawn).CurrentDir = DCLICK_Forward;
		else
			avaPawn(Pawn).CurrentDir = DCLICK_Back;
	}
	else if ( (Y Dot Dir) < 0 )
		avaPawn(Pawn).CurrentDir = DCLICK_Left;
	else
		avaPawn(Pawn).CurrentDir = DCLICK_Right;
*/

	bPlannedJump = true;
//?	avaPawn(Pawn).PerformDodge(avaPawn(Pawn).CurrentDir, Normal(Dir), vect(0,0,0));
	...
	if ( Dir.Z < -1 * Pawn.GetCollisionHeight() )
	{
		Pawn.Velocity.Z = 0;
		RealJumpZ = Pawn.JumpZ;
//?		DodgeV = avaPawn(Pawn).BotDodge(vect(1,0,0));
		Pawn.JumpZ = DodgeV.Z;
...
/*?
	...
	{
		Pawn.bWantsToCrouch = false;
		if ( avaPawn(Pawn).CanDoubleJump() )
			avaPawn(Pawn).DoDoubleJump(false);
...
event TranslocateTo(Actor DestinationActor)
{
	...
	if ( Pawn.Weapon.IsA('avaWeap_Translocator') )
	{
...
{
/*! CharacterInfo대신에 class<Pawn> PawnClass 같은 AIPawn을 넘겨 받아서 되도록 하는게 낳지 않을까?
...
function ResetSkill()
{
	...
	Aggressiveness = BaseAggressiveness;
//?	if ( avaPawn(Pawn) != None )
//?		avaPawn(Pawn).bCanDoubleJump = ( ((Skill >= 3) ||(WorldInfo.WorldGravityZ > WorldInfo.DefaultGravityZ) ) && avaPawn(Pawn).CanMultiJump() );
	bLeadTarget = ( Skill >= 4 );
	SetCombatTimer();
	SetPeripheralVision();
...
function SetMaxDesiredSpeed()
{
	...
	{
/*?
		if ( (avaGame(WorldInfo.Game) != None) && avaGame(WorldInfo.Game).bDemoMode )
		{
...
event WhatToDoNext()
{
	...
	{
		if (!bSpawnedByKismet /*? && avaGame(WorldInfo.Game).TooManyBots(self)*/)
		{
			if ( Pawn != None )
			{
...
protected event ExecuteWhatToDoNext()
{
	...
	if (Squad == None && PlayerReplicationInfo != None && avaTeamInfo(PlayerReplicationInfo.Team) != None)
	{
		avaTeamInfo(PlayerReplicationInfo.Team).SetBotOrders(self);
	}
	SwitchToBestWeapon();
...
{
	if (Pawn.bStationary || (avaVehicle(Pawn) != None && avaVehicle(Pawn).bKeyVehicle))
	{
	...
	if ((Enemy == FailedHuntEnemy && WorldInfo.TimeSeconds == FailedHuntTime) || (avaVehicle(Pawn) != None && avaVehicle(Pawn).bKeyVehicle))
	{
...
function FightEnemy(bool bCanCharge, float EnemyStrength)
{
	...
	AdjustedCombatStyle = CombatStyle + avaWeapon(Pawn.Weapon).SuggestAttackStyle();
	...
	if ( avaWeapon(Enemy.Weapon) != None )
		Aggression += 2 * avaWeapon(Enemy.Weapon).SuggestDefenseStyle();
	...
	if ( avaWeapon(Pawn.Weapon).RecommendRangedAttack() || IsSniping() || ((FRand() > 0.17 * (skill + Tactics - 1)) && !DefendMelee(enemyDist)) )
	{
	...
	GoalString = "Do tactical move";
	if ( /*? !avaWeapon(Pawn.Weapon).bRecommendSplashDamage &&*/ (FRand() < 0.7) && (3*Jumpiness + FRand()*Skill > 3) )
	{
		GetAxes(Pawn.Rotation,X,Y,Z);
...
/*?
		if ( avaWeapon(Pawn.Weapon).CurrentRating > 0.5 )
...
/*?
		if ( Pawn.Weapon.AIRating < 0.5 )
		{
			if ( EnemyStrength > 0.3 )
				...
				WeaponRating = avaWeapon(Pawn.Weapon).CurrentRating/2000;
		}
		else if ( EnemyStrength > 0.3 )
			WeaponRating = avaWeapon(Pawn.Weapon).CurrentRating/2000;
		else
			WeaponRating = avaWeapon(Pawn.Weapon).CurrentRating/1000;
*/
...
function bool FindSuperPickup(float MaxDist)
{
	...
	local avaPickupFactory P;

	if ( (LastSearchTime == WorldInfo.TimeSeconds) || !Pawn.bCanPickupInventory || (Vehicle(Pawn) != None) )
		return false;

	if ( (avaDeathMatch(WorldInfo.Game) != None) /*? && !avaDeathMatch(WorldInfo.Game).WantsPickups(self)*/ )
		return false;
...
event SoakStop(string problem)
{
	...
	ForEach DynamicActors(class'avaPlayerController',PC)
	{
//?		PC.SoakPause(Pawn);
		break;
	}
...
//?	if ( (avaPawn(Pawn) != None) && avaPawn(Pawn).bNoJumpAdjust )
//?	{
//?		avaPawn(Pawn).bNoJumpAdjust = false;
//?		return;
//?	}
//?	else
//?	{
		...
		Pawn.SuggestJumpVelocity(Pawn.Velocity, Destination, Pawn.Location);
/*?
		if ( avaPawn(Pawn).bRequiresDoubleJump )
		{
			bNotifyApex = true;
			bPendingDoubleJump = true;
		}
...
function float AdjustAimError(avaWeapon W, float TargetDist, bool bDefendMelee, bool bInstantProj, bool bLeadTargetNow )
{
/*?
	...
	if ( Stopped() && (WorldInfo.TimeSeconds > StopStartTime) )
	{
		...
		if ( avaWeapon(Pawn.Weapon) != None && avaWeapon(Pawn.Weapon).bSniping )
...
function Rotator GetAdjustedAimFor( Weapon InWeapon, vector projstart )
{
	...
	local class<avaProjectile> ProjectileClass;
	local avaWeapon W;

	W = avaWeapon(InWeapon);
	if ( W == None )
		return Rotation;

	ProjectileClass = class<avaProjectile>(W.GetProjectileClass());
	...
	else
	{
		...
		if ( !bClean && (Focus == Enemy) && bEnemyInfoValid )
		{
			...
			if ( HitActor != None )
			{
				...
				if ( avaWeapon(Pawn.Weapon) != None /*! && avaWeapon(Pawn.Weapon).bSplashDamage*/ && (Skill >= 4) )
				{
			 		HitActor = Trace(HitLocation, HitNormal, FireSpot, ProjStart, false);
					if ( HitActor != None )
						FireSpot += 2 * TargetHeight * HitNormal;
...
event DelayedWarning()
{
	...
	if ( dist > 1.2 * Pawn.GetCollisionHeight() )
	{
		...
		if ( (WarningProjectile.Physics == PHYS_Projectile) && (dist > Pawn.GetCollisionHeight() + 100) && !WarningProjectile.IsA('avaProj_Shockball') )
		{
...
/* ReceiveRunOverWarning()
 AI controlled creatures may duck
*/
event ReceiveRunOverWarning(avaVehicle V, float projSpeed, vector VehicleDir)
{
...
event MissedDodge()
{
	...
	if ( /*? avaPawn(Pawn).CanDoubleJump() &&*/ (Abs(Pawn.Velocity.Z) < 100) )
	{
//?		avaPawn(Pawn).DoDoubleJump(false);
		bPendingDoubleJump = false;
	}
...
*/
 	...
 	bPlannedJump = true;
//?	avaPawn(Pawn).PerformDodge(avaPawn(Pawn).CurrentDir, Dir,Normal(Dir Cross vect(0,0,1)));
	return true;
}
...
//?	if ( avaVehicle(Pawn) != None )
//?		return avaVehicle(Pawn).Dodge(DCLICK_None);
	if ( Pawn.bStationary )
		return false;
	if ( Stopped() )
		GotoState('TacticalMove');
...
//?	avaPawn(Pawn).Dodge(avaPawn(Pawn).CurrentDir);
	return true;
}
...
function bool ShouldStrafeTo(Actor WayPoint)
{
	...
	if ( (avaVehicle(Pawn) != None) && !avaVehicle(Pawn).bFollowLookDir )
		return true;
...
function Actor AlternateTranslocDest()
{
	...
	if ( (RouteCache.Length < 2)
		|| ((PathNode(RouteCache[1]) == None) && (PickupFactory(RouteCache[1]) == None) && (avaGameObjective(RouteCache[1]) == None)) )
	{
		if ( (FRand() < 0.5) /*! && (avaCarriedObject(RouteGoal) != None) */
	...
	if ( (FRand() < 0.3)
		&& (avaGameObjective(RouteCache[1]) == None)
		&& (RouteCache.Length > 2)
		&& ((PathNode(RouteCache[2]) != None) || (PickupFactory(RouteCache[2]) != None) || (avaGameObjective(RouteCache[2]) != None))
...
function Actor FaceActor(float StrafingModifier)
{
	...
	if ( (avaGameObjective(Focus) != None) && (Focus == Squad.SquadObjective) && (Squad.GetOrders() == 'ATTACK') && Pawn.IsFiring() && avaGameObjective(Focus).Shootable() )
		return Focus;
	bTranslocatorHop = false;
	...
	if ( CanUseTranslocator() )
	{
		...
		bCatchup = ((Pawn(RouteGoal) != None) && !WorldInfo.GRI.OnSameTeam(self,Pawn(RouteGoal).Controller)) /*|| (avaCarriedObject(RouteGoal) != None)*/;
		if ( bEnemyNotEngaged )
		{
			if ( bCatchup )
		...
		if (  bTranslocTactics && (TranslocUse > FRand()) && (Vehicle(Pawn) == None)
			...
			&& ((NavigationPoint(Movetarget) != None) /*! || (avaCarriedObject(MoveTarget) != None)*/)
			&& (LiftCenter(MoveTarget) == None)
			&& (bEnemyNotEngaged || bRecommendFastMove /*! || (avaCarriedObject(MoveTarget) != None)*/ || (VSize(Enemy.Location - Pawn.Location) > ENEMYLOCATIONFUZZ * (1 + FRand()))
				|| (bCatchup && (FRand() < 0.65) && (!LineOfSightTo(RouteGoal) /*! || (avaCarriedObject(RouteGoal) != None)*/))) )
		{
			...
			if ( /*! (avaCarriedObject(RouteGoal) != None) &&*/ (VSize(Pawn.Location - RouteGoal.Location) < 1000 + 1200 * FRand()) && LineOfSightTo(RouteGoal) )
			{
			...
			if ( (Dist < MinDist) || ((Dist < MinDist + 150) && !Pawn.Weapon.IsA('avaWeap_Translocator')) )
			{
	...
	bRecommendFastMove = false;
	if ( (!Pawn.bCanStrafe && (avaVehicle(Pawn) == None || !avaVehicle(Pawn).bSeparateTurretFocus))
	...
	if ( WorldInfo.TimeSeconds - LastSeenTime > 4 - StrafingModifier)
		return FaceMoveTarget();
	if ( (Skill > 2.5) /*! && (avaCarriedObject(MoveTarget) != None)*/ )
		return Enemy;
	...
	if ( RelativeDir > 0.85 )
		return Enemy;
	if ( (RelativeDir > 0.3) && (avaBot(Enemy.Controller) != None) && (MoveTarget == Enemy.Controller.MoveTarget) )
		return Enemy;
...
function bool NeedWeapon()
{
	...
	ForEach Pawn.InvManager.InventoryActors(class'avaWeapon',W)
...
{
/*!
	if (PickupHolder.IsA('avaWeaponLocker'))
	{
		return avaWeaponLocker(PickupHolder).BotDesireability(Pawn);
	}
*/
...
{
	if ( !SuperPickupNotSpokenFor(avaPickupFactory(P)) )
		return 0;
	return P.InventoryType.static.BotDesireability(P, Pawn);
}

function bool SuperPickupNotSpokenFor(avaPickupFactory P)
{
	local avaBot CurrentOwner;
	...
	CurrentOwner = avaBot(P.TeamOwner[PlayerReplicationInfo.Team.TeamIndex]);
...
function OnAIFreeze(avaSeqAct_AIFreeze FreezeAction)
{
...
state Defending
{
	...
	function SetRouteGoal()
	{
		...
		if ( (DefensePoint == None) || (avaGameObjective(Squad.FormationCenter()) == None) )
		{
		...
		if ( RouteGoal != None )
		{
			...
			{
				if ( (DefensePoint != None) /*! && (avaHoldSpot(DefensePoint) == None) */ )
					FreePoint();
				DefensivePosition = Squad.FindDefensivePositionFor(self);
				if ( DefensivePosition == None )
				{
...
Begin:
	...
	if ( Pawn.bStationary )
		Goto('Pausing');
	if ( (avaVehicle(Pawn) != None) && (!avaVehicle(Pawn).bTurnInPlace || (Pawn.FindAnchorFailedTime != WorldInfo.TimeSeconds)) )
	{
...
Pausing:
	if (avaVehicle(Pawn) != None && avaVehicle(Pawn).bShouldLeaveForCombat)
	{
		avaVehicle(Pawn).DriverLeave(false);
	}
	StopMovement();
	Pawn.bWantsToCrouch = IsSniping();
	if ( DefensePoint != None )
	{
	...
	SwitchToBestWeapon();
	Sleep(0.5);
	if (avaWeapon(Pawn.Weapon) != None /*&& avaWeapon(Pawn.Weapon).ShouldFireWithoutTarget()*/)
		Pawn.BotFire(false);
...
function Celebrate()
{
	if (avaPawn(Pawn) != None)
	{
		avaPawn(Pawn).PlayVictoryAnimation();
	}
}
...
state MoveToGoal
{
	function bool CheckPathToGoalAround(Pawn P)
	{
		if ( (MoveTarget == None) || (avaBot(P.Controller) == None) || !WorldInfo.GRI.OnSameTeam(self,P.Controller) )
			return false;

		if ( (avaBot(P.Controller).Squad != None) && avaBot(P.Controller).Squad.ClearPathFor(self) )
			return true;
		return false;
	}
...
Begin:
	SwitchToBestWeapon();
	WaitForLanding();
	if ( Pawn.bCanPickupInventory && (avaPickupFactory(MoveTarget) != None) && (Squad.PriorityObjective(self) == 0) && (Vehicle(Pawn) == None)
		/*&& avaPickupFactory(MoveTarget).ShouldCamp(self, 5)*/ )
	{
...
state Fallback extends MoveToGoalWithEnemy
{
	function bool FireWeaponAt(Actor A)
	{
		if ( (A == Enemy) && (Pawn.Weapon != None) && (Pawn.Weapon.AIRating < 0.5)
			&& (WorldInfo.TimeSeconds - Pawn.SpawnTime < avaDeathMatch(WorldInfo.Game).SpawnProtectionTime)
	...
	*/
	...
	function bool EngageDirection(vector StrafeDir, bool bForced)
	{
		...
		if ( !bForced )
		{
			...
			bWantJump = (Vehicle(Pawn) == None) && (Pawn.Physics != PHYS_Falling) && ((FRand() < 0.05 * Skill + 0.6 * Jumpiness) || (/*avaWeapon(Pawn.Weapon).SplashJump() &&*/ ProficientWithWeapon()))
			...
			if ( bWantJump )
			{
				if ( (avaWeapon(Pawn.Weapon) != None) /*&& avaWeapon(Pawn.Weapon).SplashJump()*/ )
				{
					StopFiring();
...
RecoverEnemy:
	...
	{
		Pawn.Acceleration = vect(0,0,0);
		if ( (avaWeapon(Pawn.Weapon) != None) /*&& avaWeapon(Pawn.Weapon).bSplashDamage*/ )
		{
			StopFiring();
			Sleep(0.05);
		}
...
{
	return ( (avaWeapon(Pawn.Weapon) != None) && avaWeapon(Pawn.Weapon).FocusOnLeader(bLeaderFiring) );
}
...
ignores SeePlayer, HearNoise, Bump;
	...
	function bool FindStrafeDest()
	{
		...
		avaWeap = avaWeapon(Pawn.Weapon);
		MaxRange = Pawn.Weapon.MaxRange();	//! (avaWeap != None ? avaWeap.GetOptimalRangeFor(Focus) : Pawn.Weapon.MaxRange());
		...
		else if (Pawn.Anchor.PathList.length > 0)
		{
			...
			{
				bAllowBackwards = (CombatStyle + (avaWeap != None ? avaWeap.SuggestAttackStyle() : 0.0) <= 0.0);
			}
...
Begin:
	...
	if ( Focus == Enemy )
		...
		FireWeaponAt(Focus);
	Sleep(0.1);
	if ( ((Pawn.Weapon != None) && Pawn.Weapon.bMeleeWeapon) || (Focus == None) || ((Focus != Enemy) && (avaGameObjective(Focus) == None) && (Enemy != None) && LineOfSightTo(Enemy)) )
		LatentWhatToDoNext();
	if ( Enemy != None )
		CheckIfShouldCrouch(Pawn.Location,Enemy.Location, 1);
	if (FindStrafeDest())
...
ignores SeePlayer, EnemyNotVisible, HearNoise, ReceiveWarning, NotifyLanded, NotifyPhysicsVolumeChange,
	...
	function BeginState(Name PreviousStateName)
	{
		if (bSpawnedByKismet /*|| avaGame(WorldInfo.Game).TooManyBots(self)*/)	//? TooManyBots NOT implemented.
		{
			Destroy();
			return;
		}
		if ( (DefensePoint != None) /*&& (avaHoldSpot(DefensePoint) == None)*/ )	//? avaHoldSpot NOT implemented.
			FreePoint();
		if ( NavigationPoint(MoveTarget) != None )
		{
...
TryAgain:
	...
	{
		Sleep(0.25 /*! + avaGame(WorldInfo.Game).SpawnWait(self) */);
...
ignores SeePlayer, HearNoise, KilledBy, NotifyBump, HitWall, NotifyPhysicsVolumeChange, NotifyHeadVolumeChange,
	...
	function Timer()
	{
		if ( avaGame(WorldInfo.Game) != None )
		{
			if ( (avaGame(WorldInfo.Game).EndGameFocus == Pawn) && (Pawn != None) )
		{
			avaPawn(Pawn).PlayVictoryAnimation();
		}
		/* //@todo steve
	...
	function BeginState(Name PreviousStateName)
	{
		...
		if ( (avaGame(WorldInfo.Game) != None) && (avaGame(WorldInfo.Game).EndGameFocus == Pawn) && (Pawn != None) )
			SetTimer(3.0, false);
	}
}
...
state LeavingVehicle
{
	...
	{
		local Vehicle V;
//?		local avaAirVehicle AirVehicle;
 ...
 */
delegate bool CustomActionFunc(avaBot B)
{
	return true;
}
...
{
	local avaVehicle V, MyVehicle;
	local Pawn P;
	local Weapon Weap;

	MyVehicle = avaVehicle(Pawn);
...
state ScriptedMove
{
	...
	event bool NotifyHitWall(vector HitNormal, Actor Wall)
	{
		// if we're moving to a avaVehicleFactory, we hit the vehicle it spawned and that vehicle is still on top of the factory,
		// then consider the factory as reached
		if ( Vehicle(Wall) != None && ScriptedMoveTarget.IsA('avaVehicleFactory') &&
			Wall == avaVehicleFactory(ScriptedMoveTarget).ChildVehicle && Wall.ContainsPoint(ScriptedMoveTarget.Location) )
		{
			PopState();
			return false;
		}
...
defaultproperties
{
	...
	bCanDoSpecial=true
	bIsPlayer=true					// PlayerReplicationInfo를 생성하려면 true여야 한다.
	bLeadTarget=true
	...
	Begin Object Class=avaBotDecisionComponent Name=TheDecider
	End Object
