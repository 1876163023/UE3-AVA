...
CUtlVector<byte> dlightdata2;
CUtlVector<byte> bouncedlightdata;
CUtlVector<byte> dshadowdata;

//!{ 2006-05-12	허 창 민
int					num_blackmesh_vertexes;
CUtlVector<Vector>	blackmesh_vertexes;
CUtlVector<Vector>	blackmesh_tangent_x;
CUtlVector<Vector>	blackmesh_tangent_y;
CUtlVector<Vector>	blackmesh_tangent_z;

//temporary
CUtlVector<Vector>	blackmesh_vertexlight_float_vector_data;
CUtlVector<Vector>	blackmesh_realtime_vertexlighting;
CUtlVector<Vector>	blackmesh_bouncing_data;
CUtlVector<float>	blackmesh_vertexlight_weight_data;

// output of vertex lightmap without sun
CUtlVector<byte>	blackmesh_vertexlightdata;		// size = rgbe(=4) * numbumpvect(=4) * numvertices

// temporary for world ambient sampling and output of vertex lightmap
CUtlVector<byte>	blackmesh_worldvertexlightdata;		// size = rgbe(=4) * numbumpvect(=4) * numvertices
//!} 2006-05-12	허 창 민
...
CUtlVector<dplane_t>	dplanes;

int			numvertexes;
//dvertex_t	dvertexes[MAX_MAP_VERTS];
CUtlVector<dvertex_t>	dvertexes;
...
CUtlVector<int>	g_vertnormalindices;

int				g_numvertnormals;	
//Vector			g_vertnormals[MAX_MAP_VERTNORMALS];
CUtlVector<Vector>	g_vertnormals;
...
CUtlVector<texinfo_t> texinfo;

int			numtexdata;
//dtexdata_t	dtexdata[MAX_MAP_TEXDATA];
CUtlVector<dtexdata_t>	dtexdata;
...
CUtlVector<dface_t>	dfaces;

int			numedges;
//dedge_t		dedges[MAX_MAP_EDGES];
CUtlVector<dedge_t>	dedges;
...
{
	// 우린 이거 안써요.. 무한 루프 조심.. false return 할 것..
	return false;
...
void CalcFaceExtents(dface_t *s, int lightmapTextureMinsInLuxels[2], int lightmapTextureSizeInLuxels[2])
{
	...
	for (i=0 ; i<s->numedges ; i++)
	{
		...
		{
			//!{ 2006-04-19	허 창 민
			// 컴파일러가 최적화를 수행하면서 연산 순서를 바꾸고,
			// 그래서 Floating Point Value 연사에서 오차가 발생한다.
			// 컴파일 옵션에서 SSE 옵션을 켜서 UnrealEngine3와 값을 맞추었다.
			...
			val = v->point[0] * tex->lightmapVecsLuxelsPerWorldUnits[j][0] + 
				  ...
				  1.0f * tex->lightmapVecsLuxelsPerWorldUnits[j][3];
	...
	{	
		//!{ 2006-04-27	허 창 민
		//maxs[i] -= 0.1f / 16.0f;	// winding clip epsilon 을 고려
		//!} 2006-04-27	허 창 민
		mins[i] = ( float )floor( mins[i] );
		maxs[i] = ( float )ceil( maxs[i] );

		//!{ 2006-06-09	허 창 민
		// 우리는 export할때, min이 0 이 되도록 조정해서 export 합니다.
		// 이것은 우리 export와 작동할 때만의 특별한 경우입니다.
		if( mins[i] < 0.0f )
			mins[i] = 0.0f;
		//!} 2006-06-09	허 창 민
		...
		//<@ ava specific ; 2006/8/28 changmin
		if( lightmapTextureSizeInLuxels[i] < 0 )
		{
			...
			Error( "Bad surface extents - surface is negative to have a lightmap\n\tmaterial %s around point (%.1f %.1f %.1f)\n\t(dimension: %d, %d>%d)\n", 
