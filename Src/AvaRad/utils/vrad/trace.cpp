...
{
	//<@ ava specific ; 2007. 2. 7 changmin
	// static mesh 를 vertex lighting 할 때, self occlusion 현상이 생기지 않도록 바꿈.
	// vertex lighting을 할때에만, skip_id가 유효하고, 다를 때는 모두 -1이다.
	// 이는 모두 vertex lighting logic이 제어한다.
	if( skip_id == -1 )
	{
		return TestLineWithOpcode( start, stop );
	}
	else
	{
		return TestLineWithSkipByOpcode( start, stop );
	}
	//>@ ava
