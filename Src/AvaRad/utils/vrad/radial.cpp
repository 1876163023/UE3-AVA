...
radial_t *AllocateRadial( int facenum )
{
	...
	if( !rad )
	{
		Msg("null rad in AllocateRadial\n");
	}
...
radial_t *BuildLuxelRadial( int facenum, int style, bool bRemoveSecondaryLightSource = false, bool bInRemoveRealtimeLighting = false )
{
	int		        j, k;
	facelight_t	    *fl;
	faceneighbor_t  *fn;
	...
	for (k=0 ; k<fl->numsamples ; k++)
	{
		if( needsBumpmap )
		{
			for( bumpSample = 0; bumpSample < NUM_BUMP_VECTS + 1; bumpSample++ )
			{
				if( bSeoncdaryLightSourceFace && bRemoveSecondaryLightSource && fl->light[style][0][k].y > secondarylight_threshold)
				{
					light[bumpSample].x = 0.0f;
					light[bumpSample].y = 0.0f;
					light[bumpSample].z = 0.0f;
				}
				else
				{
					VectorCopy( fl->light[style][bumpSample][k], light[bumpSample] );
					//<@ Realtime Light Style : 2007. 10. 23
					if( bRemoveRealtimeLighting )
					{
						VectorSubtract( light[bumpSample], fl->light[1][bumpSample][k], light[bumpSample]);
					}
					//>@
				}
			}
		...
		else
		{
			...
			else
			{
				...
				if( bRemoveRealtimeLighting )
				{
					VectorSubtract( light[0], fl->light[1][0][k], light[0]);
				}
				//>@
			}
		}
...
void FinalLightFace( int iThread, int facenum )
{
	...
	for (k=0 ; k < lightstyles; k++ )
	{
		...
		//!{ 2006-08-18	허 창 민
		// shadowmap 지원 제거방법이 항상 shadow_count = 0으로 만드는 것이다.
		// 이는 나중에 되살리기 쉽게 하기 위해, 전체적인 framework은 살려 놓고,
		// 제거하기 위함이다.
		// 아래 코드는 dshadowdata에 shadowcount를 적는 로직이므로 지우지 말아야 한다.
		// save shadow info 
		const int shadow_count = fl->shadows.Count();
		unsigned char* pShadowData = &dshadowdata[f->lightshadowofs];
		// total shadow count
		pShadowData[0] = (unsigned char) shadow_count;
		const int shadowOffset = 1 + fl->numluxels;
		for( int shadowIndex = 0; shadowIndex < shadow_count; ++shadowIndex )
		{
			pShadowData[1 + shadowIndex * shadowOffset] = (unsigned char) (fl->shadows[shadowIndex].light_id);
		}
		//!} 2006-08-18	허 창 민
...
=============
2007. 5. 1.
build face light map
face light sample을 lightmap으로 만든다.
=============
*/
void AVA_MakeFaceLightmapFromDiffuseReflectionSamples( int iThread, int facenum )
{
	dface_t	        *f;
	int		        i, j, k;
	facelight_t	    *fl;
	...
	Vector		    lb[NUM_BUMP_VECTS + 1], v[NUM_BUMP_VECTS + 1];
	unsigned char   *pdata[NUM_BUMP_VECTS + 1];
	unsigned char   *lightmapdata[NUM_BUMP_VECTS + 1];
	unsigned char   *lightmapdata2[NUM_BUMP_VECTS + 1];
	int				bumpSample;
	radial_t	    *rad = NULL;
	...
	for (k=0 ; k < lightstyles; k++ )
	{
		...
		for( bumpSample = 0; bumpSample < bumpSampleCount; ++bumpSample )
		{
			pdata[bumpSample] = &bouncedlightdata[f->lightofs + (k * bumpSampleCount + bumpSample) * fl->numluxels*4]; 
			lightmapdata[bumpSample] = &dlightdata[f->lightofs + (k * bumpSampleCount + bumpSample) * fl->numluxels*4]; 
			lightmapdata2[bumpSample] = &dlightdata2[f->lightofs + (k * bumpSampleCount + bumpSample) * fl->numluxels*4]; 
		}
		...
		for (j=0 ; j<fl->numluxels; j++)
		{
			...
			for( bumpSample = 0; bumpSample < bumpSampleCount; bumpSample++ )
			{
				...
				// accumulate light
				{
					colorRGBExp32* facelight = (colorRGBExp32*)lightmapdata[bumpSample];
					Vector fl;
					fl.x = TexLightToLinear(facelight->r, facelight->exponent);
					fl.y = TexLightToLinear(facelight->g, facelight->exponent);
					fl.z = TexLightToLinear(facelight->b, facelight->exponent);
					VectorAdd( fl, lb[bumpSample], fl );
					Vec3toColorRGBExp32( fl, (colorRGBExp32*)lightmapdata[bumpSample] );
					lightmapdata[bumpSample] += 4;
				}

				{
					colorRGBExp32* facelight2 = (colorRGBExp32*)lightmapdata2[bumpSample];
					Vector fl;
					fl.x = TexLightToLinear(facelight2->r, facelight2->exponent);
					fl.y = TexLightToLinear(facelight2->g, facelight2->exponent);
					fl.z = TexLightToLinear(facelight2->b, facelight2->exponent);
					VectorAdd( fl, lb[bumpSample], fl );
					Vec3toColorRGBExp32( fl, (colorRGBExp32*)lightmapdata2[bumpSample] );
					lightmapdata2[bumpSample] += 4;
				}
		...
		// Remember, the data goes *before* the specified light_ofs, in *reverse order*
		//colorRGBExp32 *pAvgColor = (colorRGBExp32*)&dlightdata[f->lightofs - (k+1) * 4];
		colorRGBExp32 *pAvgColor = (colorRGBExp32*)&bouncedlightdata[f->lightofs - (k+1) * 4];
		if (avgCount == 0)
		{
			Vector median( 0, 0, 0 );
			Vec3toColorRGBExp32( median, pAvgColor );
...
2007. 8. 20.
build face light map
direct lighting sample을 lightmap으로 만든다.
=============
*/
void AVA_MakeFaceLightmapFromDirectLightingSamples( int iThread, int facenum )
{
	dface_t	        *f;
	int		        i, j, k;
	facelight_t	    *fl;
	...
	int			    lightstyles;
	Vector		    lb[NUM_BUMP_VECTS + 1], v[NUM_BUMP_VECTS + 1];
	Vector		    secondarylb[NUM_BUMP_VECTS + 1];
	Vector		    realtimelb[NUM_BUMP_VECTS + 1];
	unsigned char   *pdata[NUM_BUMP_VECTS + 1];
	unsigned char   *lightmapdata[NUM_BUMP_VECTS + 1];
	unsigned char   *lightmapdata2[NUM_BUMP_VECTS + 1];
	int				bumpSample;
	...
	radial_t	    *rad = NULL;
	radial_t	    *realtimerad = NULL;
	radial_t		*secondaryrad = NULL;	// Secondary Light Source : 2007. 8. 19 
	...
	for (k=0 ; k < lightstyles; k++ )
	{
		...
		if (!do_fast)
		{
			if( !bDisp )
			{
				// BuildLuxelRadial( facenum, lightstyle, removesecondarylight, removerealtimelight )
				rad = BuildLuxelRadial( facenum, k, false, false );		// radial for normal lightmap
				realtimerad = BuildLuxelRadial( facenum, k, false, true );		// radial for special lightmap(without sun)
				secondaryrad = BuildLuxelRadial( facenum, k, true );	// Secondary Light Source : 2007. 8. 19
			}
			else
			{
				rad = StaticDispMgr()->BuildLuxelRadial( facenum, k, needsBumpmap );
				// Displacement Secondary Light Source 처리할 필요가 있을까~~~ 2007. 8. 19
			}
		}
		...
		for (j=0 ; j<fl->numluxels; j++)
		{
			...
			if (!do_fast)
			{
				if( !bDisp )
				{
					baseSampleOk = SampleRadial( rad, fl->luxel[j], lb, bumpSampleCount );		// lightmap
					SampleRadial(realtimerad, fl->luxel[j], realtimelb, bumpSampleCount );				// lightmap for realtime sun lighting
					SampleRadial( secondaryrad, fl->luxel[j], secondarylb, bumpSampleCount );	// Secondary Light Source
				}
				else
				{
					baseSampleOk = StaticDispMgr()->SampleRadial( facenum, rad, fl->luxel[j], j, lb, bumpSampleCount, false );
			...
			for( bumpSample = 0; bumpSample < bumpSampleCount; bumpSample++ )
			{
				...
				}
				{
					...
					fl.y = TexLightToLinear(facelight2->g, facelight2->exponent);
					fl.z = TexLightToLinear(facelight2->b, facelight2->exponent);
					VectorAdd( fl, realtimelb[bumpSample], fl );
					Vec3toColorRGBExp32( fl, (colorRGBExp32*)lightmapdata2[bumpSample] );
					lightmapdata2[bumpSample] += 4;
				}
				// convert to a 4 byte r,g,b,signed exponent format
				Vec3toColorRGBExp32( secondarylb[bumpSample], ( colorRGBExp32 *)pdata[bumpSample] );
				pdata[bumpSample] += 4;
			}
		}
		FreeRadial( rad );
		FreeRadial( realtimerad );
		FreeRadial( secondaryrad );	// Secondary Light Source
