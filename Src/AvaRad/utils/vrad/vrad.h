...
//<@ ava specific ; 2007. 2. 9 changmin
#include "ivraddll.h"
extern IVRadDLL2* s_pVRadDll2;
#include "ava_trace.h"
#pragma warning(disable : 4996)	// crt_secure
//>@ ava
...
typedef struct directlight_s
{
	...
	unsigned short	m_IncrementalID;	// 없애려구요......

	//!{ 2006-06-29	허 창 민
	int		light_id;	// for mapping to unreal lights
	//!} 2006-06-29	허 창 민
	//<@ Secondary Light Source 2007. 8. 20 
	int		secondaryfacenum;
...
typedef struct patch_s
{
	...
	int			faceNumber;
//	int			clusterNumber;	//<@ ava specific ; 2007. 2. 21 changmin 용량 줄이기 위해 삭제...
...
//	int			ndxNextClusterChild;		// next terminal child index in cluster	//<@ ava specific ; 2007. 2. 21 chanmin 우리는 이거 필요 없어요.. 용량 감소..
...
extern CUtlVector<entity_t*>	face_entity;
extern CUtlVector<Vector>		face_offset;
extern CUtlVector<Vector>		face_centroids;
//!} 2006-05-03	허 창 민
...
extern bool	g_bInterrupt;	// Wsed with background lighting in WC. Tells VRAD
							// to stop lighting.
//extern IIncremental *g_pIncremental; // null if not doing incremental lighting	없애려구요..
...
//void AddDispsToClusterTable( void );	// 쓰이는 데가 없네... 2007. 2. 21 changmin..
...
//!{ 2006-05-12	허 창 민
extern	int		vertex_chunk_size;	// BuildVertexLights 나 AmbientSample에서 계산할 vertex 개수
extern	int		ambientcube_chunk_size;

extern	int		sampleskylevel;	// 0 = none, 1 = 162 sphere sampling, 2 = 1024 cosine weighted hemisphere sampling
extern  qboolean g_bUseRaytrace;	// default = false
//!} 2006-05-12	허 창 민
...
//!{ 2006-05-12
void AVA_BuildVertexLights( int threadnum, int meshid );
...
__forceinline void GatherAmbientCubeWorker( UINT threadnum, UINT ambientcube_chunk_number, void* )
{
	if ((int)threadnum<0) return;
	GatherAmbientCube( threadnum, ambientcube_chunk_number );
}
//!} 2006-05-12
...
//!{ 2007-05-01
void AddLightFaceToTonemapImage( int threadnum, int facenum );
void TonemapLightFace( int threadnum, int facenum );
...
void NormalizeLightFace( int threadnum, int facenum );

void AVA_MakeFaceLightmapFromDirectLightingSamples( int threadnum, int facenum );
void AVA_MakeFaceLightmapFromDiffuseReflectionSamples( int threadnum, int facenum );
...
__forceinline void GatherAmbientCubeGIWorker( UINT threadnum, UINT ambientcube_chunk_number, void* )
{
	if ((int)threadnum<0) return;
	GatherAmbientCubeGI( threadnum, ambientcube_chunk_number );
}
//!} 2006-05-01
...
void AVA_GatherVertexAmbientLights( int threadnum, int facenum );
//>@ 2007. 5. 2
//<@ 2006. 8. 16
...
//{{ 이전 작업중
//!{ 2006-03-20	허 창 민
int AVA_VRAD_Main(int argc, char** argv, IVRadDLL2* );
//!} 2006-03-20	허 창 민
//}}
...
float GatherSampleLight( directlight_t *dl, int facenum, 
						Vector const& pos, NVector const& normal, Vector& delta, 
						float *scale, int iThread, const NVector* extra_normals = NULL, float* extra_dots = NULL, Vector* extra_skys = NULL,
...
typedef long long SampleHandle_t;					// the upper 32 bits = facelight index
													// the lower 32 bits = sample index inside of facelight
//!} 2006-08-18	허 창 민
