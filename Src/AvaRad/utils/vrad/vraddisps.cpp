//========= Copyright1996-2001, Valve LLC, All rights reserved. ============
//
// Purpose: 
//
// $NoKeywords: $
...
void CVRadDispMgr::DispBuilderInit( CCoreDispInfo *pBuilderDisp, dface_t *pFace, int ndxFace )
{
	...
	//!{ 2006-08-10	허 창 민
	// CDispTri를 사용하지 않도록 조정했다.
	CDispTri *pTris = NULL;
	if( pDisp->m_iDispTriStart != -1 )
	{
		pTris = &g_DispTris[pDisp->m_iDispTriStart];
	}
	//!} 2006-08-10	허 창 민
...
void CVRadDispMgr::UnserializeDisps( void ) 
{
	...
	for( int ndxDisp = 0; ndxDisp < g_dispinfo.Count(); ndxDisp++ )
	{
	...
	//!{ 2006-08-10	허 창 민
	// add disp surface to  opcode model for trace..
	// Warining : 이 이후에 createopcodemodel이 불리도록 해야 합니다...
	int curOpcodeVertexBase = 0;
	int ndxDisp;
	for( ndxDisp = 0; ndxDisp < g_dispinfo.Count(); ndxDisp++ )
	{
		const int vertexCount = pBuilderDisps[ndxDisp].GetSize();
		for( int vertexIndex = 0; vertexIndex < vertexCount; ++vertexIndex )
		{
			AddDispVertexToOpcodeModel( pBuilderDisps[ndxDisp].GetDispVert(vertexIndex)->m_Vert );
		}
		const int triCount = pBuilderDisps[ndxDisp].GetTriCount();
		for( int triIndex = 0; triIndex < triCount; ++triIndex )
		{
			unsigned short v1, v2, v3;
			pBuilderDisps[ndxDisp].GetTriIndices( triIndex, v1, v2, v3 );
			AddDispTriangleToOpcodeModel( v1 + curOpcodeVertexBase, v2 + curOpcodeVertexBase, v3 + curOpcodeVertexBase, pBuilderDisps[ndxDisp].GetSurface()->GetHandle() );
		}

		curOpcodeVertexBase += vertexCount;
	}
	//!} 2006-08-10	허 창 민
	...
	for( ndxDisp = 0; ndxDisp < g_dispinfo.Count(); ndxDisp++ )
	{
		...
		//!{ 2006-08-11	허 창 민
		// collision을 위해서 bsptree에 추가하는 코드이다.
		// 우리는 opcode collision을 사용하기 때문에, 이를 제거한다.
			...
			for( int ndxX = voxelMin[0]; ndxX < voxelMax[0] + 1; ndxX++ )
			{
				...
				if( handle != g_SampleHashTable.InvalidHandle() )
				{
					...
					for( int ndx = 0; ndx < count; ndx++ )
					{
						...
						int ndxSample = ( sampleHandle & 0xffffffff );
						int ndxFaceLight = ( ( sampleHandle >> 32 ) & 0xffffffff );
						//!} 2006-08-18	허 창 민
	...
	for( int ndxFace = 0; ndxFace < numfaces; ndxFace++ )
	{
		...
		for( int ndxSample = 0; ndxSample < pFaceLight->numsamples; ndxSample++ )
		{
			...
			if( pSample )
			{
				...
				sampleHandle |= ( ((long long) ndxFace) << 32 );
				//!} 2006-08-18	허 창 민
