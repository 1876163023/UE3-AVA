...
void CVRADDispColl::InitPatch( int ndxPatch, int ndxParentPatch, bool bFirst )
{
	...
	if( pPatch )
	{
		...
		//pPatch->ndxNextClusterChild = patches.InvalidIndex();	//<@ ava specific ; 2007. 2. 21 changmin 우리는 이거 안씁니다. //>@ ava
...
bool CVRADDispColl::MakeChildPatch( int ndxPatch )
{
	...
	if( pPatch )
	{
		...
		// 2006 / 8 / 17 changmin
		//patch의 planeDist field를 만들어 주어야 합니다.
		pPatch->planeDist = pPatch->normal.Dot( pPatch->origin );
		//!} 2006 / 8 / 17 changmin
