...
public:
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,char Ch)
	{
		checkSlow(Ar.NumBytes + 1 <= Size);
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,BYTE& B)
	{
		checkSlow(Ar.NumBytes + 1 <= Size);
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,INT& I)
	{
		return Ar << *(DWORD*)&I;
	}
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,DWORD& D)
	{
		checkSlow(Ar.NumBytes + 4 <= Size);
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,QWORD& Q)
	{
		checkSlow(Ar.NumBytes + 8 <= Size);
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,FLOAT& F)
	{
		DWORD Value = *(DWORD*)&F;
		return Ar << Value;
	}
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,DOUBLE& Dbl)
	{
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,FString& String)
	{
		// We send strings length prefixed
		INT Len = String.Len();
		Ar << Len;
		checkSlow(Ar.NumBytes + Len <= Size);
	 ...
	 */
	friend inline TNboSerializeBuffer<Size>& operator<<(TNboSerializeBuffer<Size>& Ar,const TCHAR* String)
	{
		// We send strings length prefixed
...
typedef TNboSerializeBuffer<MAX_STAT_PACKET_SIZE> FNboSerializeBuffer;
...
public:
	...
	FStatNotifyProvider_UDP(void) :
		FStatNotifyProvider(TEXT("StatsNotifyProvider_UDP")),
		ListenerThread(NULL),
