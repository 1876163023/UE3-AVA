...
public:
	UBOOL Connect();
	void SetIPAddress(DWORD RemoteIPAddress, UBOOL bRemoteIsIntelByteOrder);
...
public:
	...
	void OnReceivedData(BYTE* Data, INT Count);

protected:
	void OnPropertyChange(struct FNetworkPropertyChange* PropertyChange);
	void OnActorMove(struct FNetworkActorMove* ActorMove);
	void OnActorCreate(struct FNetworkActorCreate* ActorCreate);
	void OnActorDelete(struct FNetworkActorDelete* ActorDelete);
	void OnObjectRename(struct FNetworkObjectRename* ObjectRename);
	void OnRemoteConsoleCommand(struct FNetworkRemoteConsoleCommand* RemoteConsoleCommand);
