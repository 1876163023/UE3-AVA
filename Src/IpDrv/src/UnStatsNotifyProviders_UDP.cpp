	...
	BYTE* Data,INT Count)
{
	...
	{
		debugf(NAME_DevStats,TEXT("Sending server announce to %s"),
...
// @todo joeg - add a "requires password" option
	...
	{
		debugf(NAME_DevStats,TEXT("Received client connect from %s"),
			*SrcAddr.GetString(TRUE));
	...
	{
		debugf(NAME_DevStats,TEXT("Received client disconnect from %s"),
			*SrcAddr.GetString(TRUE));
	...
	else
	{
		...
		debugf(NAME_DevStats,TEXT("Unknown request of size %d from %s"),Count,
			*SrcAddr.GetString(TRUE));
	}
}
 ...
 */
void FStatNotifyProvider_UDP::WriteStat(DWORD StatId,DWORD ParentId,
 ...
 * @param Value the value of the stat to write out
 */
void FStatNotifyProvider_UDP::WriteStat(DWORD StatId,FLOAT Value)
{
	// Don't bother building a packet if there are no clients connected
	if (ClientList->GetConnectionCount() > 0)
	{
 ...
 * @param Value the value of the stat to write out
 */
void FStatNotifyProvider_UDP::WriteStat(DWORD StatId,DWORD Value)
{
	// Don't bother building a packet if there are no clients connected
	if (ClientList->GetConnectionCount() > 0)
	{
