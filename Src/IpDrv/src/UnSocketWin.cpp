...
#if !XBOX // PC socket subsystem creation/destruction support
...
UBOOL FSocketSubsystemWindows::Initialize(FString& Error)
{
	if (bTriedToInit == FALSE)
	{
		...
		if (Code == 0)
		{
			...
			HostByNameSynch = GSynchronizeFactory->CreateCriticalSection();
			if (HostByNameSynch != NULL)
			{
...
void FSocketSubsystemWindows::Destroy(void)
{
	if (HostByNameSynch != NULL)
	{
		GSynchronizeFactory->Destroy(HostByNameSynch);
	}
	WSACleanup();
}
 ...
 */
FSocket* FSocketSubsystemWindows::CreateDGramSocket(UBOOL)
{
	SOCKET Socket = socket(AF_INET,SOCK_DGRAM,IPPROTO_UDP);
	return Socket != INVALID_SOCKET ? new FSocketWin(Socket) : NULL;
}
 ...
 * @return the new socket or NULL if failed
 */
FSocket* FSocketSubsystemWindows::CreateStreamSocket(void)
{
...
#if !XBOX // Xbox has it's own version of this
/**
