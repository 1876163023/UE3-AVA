...
void FDebugCommunication::FDebugReceiver::ProcessPacket( FIpAddr SrcAddr, BYTE* Data, INT Count )
{
	...
	{
		debugfSuppressed(NAME_DevStats,TEXT("Sending server announce to %s"),*SrcAddr.GetString(TRUE));
	...
	{
		debugfSuppressed(NAME_DevStats,TEXT("Received client connect from %s"),*SrcAddr.GetString(TRUE));
		// Add this client
	...
	{
		debugfSuppressed(NAME_DevStats,TEXT("Received client disconnect from %s"),*SrcAddr.GetString(TRUE));
		// Remove this client
		SrcAddr.Port = TrafficPort;
	...
	else
	{
		...
		debugfSuppressed(NAME_DevStats,TEXT("Unknown request of size %d from %s"),Count,*SrcAddr.GetString(TRUE));
	}
#endif
}
