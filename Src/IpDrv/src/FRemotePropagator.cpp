...
struct FListenHelper : FUdpLink
{
	...
	{
		if (Propagator)
			Propagator->OnReceivedData(Data, Count);
	}
};
...
UBOOL FRemotePropagator::Connect()
{
 ...
 // An IPAddress of 0 means don't send
void FRemotePropagator::SetIPAddress(DWORD RemoteIPAddress, UBOOL bRemoteIsIntelByteOrder)
{
	IPAddress = RemoteIPAddress;
	bIsIntelByteOrder = bRemoteIsIntelByteOrder;
...
void FRemotePropagator::SendChange(class FNetworkPropagatorBase* Change)
{
	...
	Change->SendTo(SendHelper, IpAddr, bIsIntelByteOrder);
}
...
UBOOL FListenPropagator::Connect()
{
	...
	{
		appMsgf(AMT_OK, TEXT("Failed to bind to port %d for listening for propagation messages."), PROPAGATION_PORT);
		return false;
	}
...
void FListenPropagator::OnReceivedData(BYTE* Data, INT Count)
{
...
void FListenPropagator::OnPropertyChange(struct FNetworkPropertyChange* PropertyChange)
{
...
void FListenPropagator::OnActorMove(struct FNetworkActorMove* ActorMove)
{
...
void FListenPropagator::OnActorCreate(struct FNetworkActorCreate* ActorCreate)
{
	// find the referenced actor
...
void FListenPropagator::OnActorDelete(struct FNetworkActorDelete* ActorDelete)
{
	// find the referenced actor
...
void FListenPropagator::OnObjectRename(struct FNetworkObjectRename* ObjectRename)
{
...
void FListenPropagator::OnRemoteConsoleCommand(struct FNetworkRemoteConsoleCommand* RemoteConsoleCommand)
{
