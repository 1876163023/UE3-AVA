...
#define EnableP2pConn // [!enable] 20070123 dEAthcURe|HP

IP2PHandler *UTcpNetDriver::P2PHandler = NULL;
...
void UTcpipConnection::LowLevelSend( void* Data, INT Count )
{
	...
	if(Driver->pSecureUdpLayer) {
		int lengthCipherText = 0;
		unsigned char* pPacket = Driver->pSecureUdpLayer->onSendPacket(lengthCipherText, (unsigned char*)Data, Count, RemoteAddr.Addr);
		if(pPacket) {
			Socket->SendTo((BYTE*)pPacket,lengthCipherText,BytesSent,RemoteAddr);			
		}
		else {
			debugf(TEXT("[UTcpipConnection::LowLevelSend] packet encryption failed %d bytes"), Count);
		}
	}
	else {
		Socket->SendTo((BYTE*)Data,Count,BytesSent,RemoteAddr);
		debugf(TEXT("[UTcpipConnection::LowLevelSend] plain text %d bytes sent due to invalid object:secureUdpLayer_t"), BytesSent);
	}
	#else
	Socket->SendTo((BYTE*)Data,Count,BytesSent,RemoteAddr);
	#endif
	...
	extern int G_tt_bytesSend;
	G_tt_bytesSend += Count;

	extern int G_tt_minPacketSize, G_tt_maxPacketSize;
	if(G_tt_minPacketSize > Count) G_tt_minPacketSize = Count;
	if(G_tt_maxPacketSize < Count) G_tt_maxPacketSize = Count;
	#endif
	// }} 20071127 dEAthcURe|ON dd
}
...
/*-----------------------------------------------------------------------------
	UTcpNetDriver.
-----------------------------------------------------------------------------*/
// {{ 20070117 dEAthcURe|HP
// host의 hole punching이 끝날때까지 client가 대기하지 않도록 
// InitBase 이전에 초기화되어 LoadMap 이전에 host를 hole punching할 목적으로 사용한다.
// 룸에 입장할때 init하고 나갈때 deinit한다.
// (optional) 게임종료시 reinit
// (optional:룸에서 호스트를 다시 선출할수 있다면) host/client구분없이 모두 _ahead_socket을 사용한다.
//---------------------------------------------------------------------------
FSocketWin* _ahead_socket = 0x0;
int _ahead_port = 7777;
//---------------------------------------------------------------------------
int _ahead_setDefaultPort(int port)
{
	int old = _ahead_port;
	_ahead_port = port;
	return old;
...
//---------------------------------------------------------------------------
bool _ahead_initSocket(int port)
{
	if(port == 0) port = _ahead_port;
	UBOOL Connect = FALSE; // listening
	FInternetIpAddr LocalAddr;

	// Create UDP socket and enable broadcasting.
	_ahead_socket = (FSocketWin*)GSocketSubsystem->CreateDGramSocket();	 
	if( _ahead_socket == NULL ) 
	{
		_ahead_socket = 0;
	...
	_ahead_socket->SetReceiveBufferSize(RecvSize,RecvSize);
	_ahead_socket->SetSendBufferSize(SendSize,SendSize);
	debugf( NAME_Init, TEXT("%s: _ahead_socket queue %i / %i"), SOCKET_API, RecvSize, SendSize );

	// Bind socket to our port.
	LocalAddr.SetIp(getlocalbindaddr( *GLog ));
	LocalAddr.SetPort(0);
	UBOOL HardcodedPort     = FALSE;
	if( !Connect )
	{
		// Init as a server.
		HardcodedPort = port!=0;
		LocalAddr.SetPort(port);
	}
	INT AttemptPort = LocalAddr.GetPort();
	INT boundport   = bindnextport( _ahead_socket, LocalAddr, HardcodedPort ? 1 : 20, 1 );
	if( boundport==0 )
	{
		// Error = FString::Printf( TEXT("%s: binding to port %i failed (%i)"), SOCKET_API, AttemptPort, GSocketSubsystem->GetLastErrorCode() );
		return false;
	}
	if( _ahead_socket->SetNonBlocking() == FALSE )
	{
		// Error = FString::Printf( TEXT("%s: SetNonBlocking failed (%i)"), SOCKET_API, GSocketSubsystem->GetLastErrorCode());
		return false;
	}

	// Success.
	if(_ahead_socket) debugf(TEXT("_ahead_initSocket sd=%d, boundPort=%d"), _ahead_socket->Socket, boundport);
	return true;
}
//---------------------------------------------------------------------------
void _ahead_deinitSocket(void)
{
	if(_ahead_socket) debugf(TEXT("_ahead_deinitSocket sd=%d"), _ahead_socket->Socket);
	if(_ahead_socket && _ahead_socket->Close() ) {
		debugf( NAME_Exit, TEXT("[_ahead_deinitSocket] closesocket error (%i)"), GSocketSubsystem->GetLastErrorCode() );
	}
	_ahead_socket = 0x0;
}
//---------------------------------------------------------------------------
...
UBOOL UTcpNetDriver::InitConnect( FNetworkNotify* InNotify, FURL& ConnectURL, FString& Error )
{
	...
	#ifdef EnableP2pConn // 20060926 dEAthcURe|HP establish p2p connection	
	
	if (P2PHandler)
		P2PHandler->OnInitConnect(Socket, ConnectURL);
	
	#endif // 20060926 dEAthcURe|HP establish p2p connection
	...
	debugf( NAME_DevNet, TEXT("Game client on port %i, rate %i"), LocalAddr.GetPort(), ServerConnection->CurrentNetSpeed );
...
UBOOL UTcpNetDriver::InitListen( FNetworkNotify* InNotify, FURL& LocalURL, FString& Error )
{
	...
	if (P2PHandler) // [+] 20070224
		P2PHandler->OnInitListen(Socket); // [+] 20070224
	#endif // 20060926 dEAthcURe|HP establish p2p connection
...
void UTcpNetDriver::TickDispatch( FLOAT DeltaTime )
{
	...
	BYTE Data[4096]; // BYTE Data[NETWORK_MAX_PACKET]; // [!] 20070527 dEAthcURe
	...
	for( ; ; )
	{
		...
		// {{ [-] 20070221 dEAthcURe netdriver가 안올라와있어도 p2pConn::tick은 되어야한다. UGameEngine::Tick으로 이사
		/*
		#ifdef EnableP2pConn
		if (P2PHandler)
			P2PHandler->OnTick();
		...
		*/
		...
		check(BytesRead<=NETWORK_MAX_PACKET); // [+] 20070527 dEAthcURe|P2p

		#ifdef EnableP2pConn // 20060919 dEAthcURe|HP establish p2p connection	
		if (P2PHandler)
			if(P2PHandler->OnRecvFrom((char*)Data, BytesRead, FromAddr)) {
				return;
			}
		#endif
		...
		// {{ 20080213 dEAthcURe|SUL
		#ifdef EnableSecureUdpLayer		
		if(BytesRead>0 && pSecureUdpLayer) {
			int lengthPlainText = 0;
			unsigned char* pPacket = pSecureUdpLayer->onRecvPacket(lengthPlainText, (unsigned char*)Data, BytesRead, FromAddr.Addr);
			if(pPacket) {
				BytesRead = lengthPlainText;
				if(pPacket != Data) {					
					memcpy(Data, pPacket, lengthPlainText);
				}			
			}
			else {				
				BytesRead = 0;
...
{
	// {{ 20070118 dEAthcURe|HP
	if(_ahead_socket == Socket) {
		Socket = 0x0;
		debugf(TEXT("[UTcpNetDriver::LowLevelDestroy] Socket cleared because of _ahead_socket == Socket"));
		return;
	}
	// }} 20070118 dEAthcURe|HP

	// {{ 20070501 dEAthcURe|HP
	if(_ahead_socket) {		
		Socket = NULL;
		debugf(TEXT("[UTcpNetDriver::LowLevelDestroy] Socket cleared because of _ahead_socket"));
		return;
	}
	// }} 20070501 dEAthcURe|HP
...
{
	// {{ 20070118 dEAthcURe|HP
	if(_ahead_socket) { // 20070413 dEAthcURe|HP nonblock p2p client // if(!Connect && _ahead_socket) {
		Socket = _ahead_socket;
		return TRUE;
	}
	// }} 20070118 dEAthcURe|HP
