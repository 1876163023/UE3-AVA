...
class UWorld : public UObject, public FNetworkNotify
{
	...
	UBOOL										bDrawEmissiveDynamicPrimitives;
	//>@ ava
	...
	*/
	...
	/**
	 * WorldInfo를 얻기위한 선조건을 판별(PersistentLevel은 있나. PersistentLevel에 Actor(0)가 있나 )
	 */
	UBOOL IsValidWorldInfo() const;
	...
	/**
	 * Finds the lights relevant to a primitive for a static lighting rebuild.
	 * @param Primitive - The primitive to find relevant lights for.
	 * @param bShadowingLightsOnly - Only return lights which cast shadows from this primitive.
	 * @param OutRelevantLights - Upon return, contains the lights which are relevant to the primitive.
	 */
	void FindRelevantLights(UPrimitiveComponent* Primitive,UBOOL bShadowingLightsOnly,TArray<ULightComponent*>& OutRelevantLights);
	...
	UBOOL SingleLineCheck( FCheckResult& Hit, AActor* SourceActor, const FVector& End, const FVector& Start, DWORD TraceFlags, const FVector& Extent=FVector(0,0,0), const ULightComponent* SourceLight = NULL );
	FCheckResult* MultiPointCheck( FMemStack& Mem, const FVector& Location, const FVector& Extent, DWORD TraceFlags );
	FCheckResult* MultiLineCheck( FMemStack& Mem, const FVector& End, const FVector& Start, const FVector& Size, DWORD TraceFlags, AActor* SourceActor, const ULightComponent* SourceLight = NULL );
	...
	void HmBeginPlay(const FURL& InURL, UBOOL bResetTime = TRUE); // 20070103 dEAthcURe|HM
	#endif
