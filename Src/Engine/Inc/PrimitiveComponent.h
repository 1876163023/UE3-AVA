...
struct FSmartCullDistance
{
	FLOAT Value[8];

	UBOOL operator == ( const FSmartCullDistance& Other ) const
	{
		return appMemcmp(this, &Other, sizeof(Value) ) == 0;
	}
	...
	friend FArchive& operator<<(FArchive& Ar, FSmartCullDistance& C )
	{
		Ar << C.Value[0] << C.Value[1] << C.Value[2] << C.Value[3] << C.Value[4] << C.Value[5] << C.Value[6] << C.Value[7];		
		return Ar;
	}

	static INT FindIndex( FLOAT DeltaX, FLOAT DeltaY )
	{
		static const FLOAT A = 0.414213f;
		static const FLOAT B = 2.414213f;
...
struct FPrimitiveSceneProxyCacheInfo
{
	FPrimitiveSceneProxyCacheInfo(const UPrimitiveComponent* Component);
 ...
 */
class FPrimitiveSceneProxy : public FPrimitiveSceneProxyCacheInfo
{
public:
	...
	virtual void Touch( const FPrimitiveSceneProxyCacheInfo& PRI );
	...
	* @param bFirstView - This is the 'first' viewport processed in InitViews.
	*/
	virtual void PreRenderView(const FSceneView* View, UBOOL bFirstView) {}
	...
	virtual EMemoryStats GetMemoryStatType( void ) const PURE_VIRTUAL( FPrimitiveSceneProxy::GetMemoryStatType, return( STAT_GameToRendererMallocOther ); );
	virtual DWORD GetMemoryFootprint( void ) const PURE_VIRTUAL( FPrimitiveSceneProxy::GetMemoryFootprint, return( 0 ); );
...
public:
	...
	BITFIELD bCastSunShadow : 1;	//<@ ava specific ; 2007. 11. 12 changmin ; add cascaded shadow
...
public:
	...
	UBOOL IsSeenThrough(const FSceneView* View) const;
...
public:
	...
	UPrimitiveComponent* OcclusionGroup;
	...
	/** If true, primitive does not cast dynamic shadows. */
	...
	*/
	...
	virtual UBOOL HasUnlitDistortion() const { return FALSE; }	
	/** Returns true if the prim is using a material with unlit translucency */
	virtual UBOOL HasUnlitTranslucency() const { return FALSE; }
	...
	*/
	virtual UBOOL UsesSceneColor() const { return FALSE; }
	...
	*/
	...
	DECLARE_FUNCTION(execSetOcclusionGroup);
	DECLARE_FUNCTION(execSetSeeThroughGroupIndex);
...
public:
	...
	virtual void SetOcclusionGroup(UPrimitiveComponent* NewOcclusionGroup);
	virtual void SetSeeThroughGroupIndex(INT NewSeenThroughGroupIndex);
...
public:
	...
	*/
	...
	void SetMaterial(INT SkinIndex,UMaterialInstance* Material);
...
class UBrushComponent : public UPrimitiveComponent
{
	...
	/**
	 * Adds the convex volumes for this brush to the passed in array.
	 *
	 * @param ConvexVolumes			Array of convex volumes to add to.
	 * @param bConvertToWorldSpace	Whether to convert the brush to world space before convexifying it
	 * 
	 * @return the number of convex volumes this brush consists off
	 */
	INT AddConvexVolumes( TArray<FConvexVolume>& ConvexVolumes, UBOOL bConvertToWorldSpace );
