...
public:
	DECLARE_CLASS(USelection,UObject,CLASS_Transient|CLASS_Intrinsic,Engine)

	USelection();

	typedef TIndexedContainerIterator< ObjectArray >		TObjectIterator;
	typedef TIndexedContainerConstIterator< ObjectArray >	TObjectConstIterator;

	typedef TIndexedContainerIterator< ClassArray >			TClassIterator;
	typedef TIndexedContainerConstIterator< ClassArray >	TClassConstIterator;
	...
	*/
	 ...
	 */
	template< class T > INT CountSelections( UBOOL bIgnorePendingKill=FALSE )
	{
		INT Count = 0;
		for( INT i=0; i<SelectedObjects.Num(); ++i )
		{
