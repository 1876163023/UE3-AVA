...
UBOOL FindPhysicalMaterialOnStaticMeshActor( const HIT_DATA& HitData, OUT_STRUCT& OutHitInfo )
{
		...
		)
	{
		...
		{
			UMaterialInstance* MaterialInstance = TheStaticMesh->StaticMeshComponent->GetMaterial(ElementIndex);

			if( MaterialInstance != NULL)
			{
				UPhysicalMaterial* PhysMaterial = MaterialInstance->GetPhysicalMaterial();
