...
/** TRUE if Low end HW used, ava extension */
...
*/
extern UBOOL GSupportsFPBlending;
...
// add cascaded shadow
extern UBOOL GAvaShadowDepthBufferSize;
//>@ ava
...
// GEnableVsync ¸¦ USystemSettings.bEnableVSync·Î ¿Å±è
...
extern void* RHILockIndexBuffer(FIndexBufferRHIParamRef IndexBuffer,UINT Offset,UINT Size,UBOOL bAppend=FALSE);
...
extern void* RHILockVertexBuffer(FVertexBufferRHIParamRef VertexBuffer,UINT Offset,UINT Size,UBOOL bAppend=FALSE);
...
enum ETextureCreateFlags
{
	...
	// Texture will be dynamic :) (deif added)
	TexCreate_Dynamic				= 1<<5,
};
...
extern FTexture3DRHIRef RHICreateTexture3D(UINT SizeX,UINT SizeY,UINT SizeZ, BYTE Format, UINT NumMips, DWORD Flags);
extern void* RHILockTexture3D(FTexture3DRHIParamRef Texture,UINT MipIndex,UBOOL bIsDataBeingWrittenTo, UINT& DestRowPitch, UINT& DestSlidePitch);
extern void RHIUnlockTexture3D(FTexture3DRHIParamRef Texture,UINT MipIndex);
...
struct FCopyTextureRegion2D
{
	...
	INT SizeY;	
	/** constructor */
	FCopyTextureRegion2D(FTexture2DRHIParamRef InSrcTexture,INT InOffsetX=-1,INT InOffsetY=-1,INT InSizeX=-1,INT InSizeY=-1)
...
extern void* RHILockTextureCubeFace(FTextureCubeRHIParamRef Texture,UINT FaceIndex,UINT MipIndex,UBOOL bIsDataBeingWrittenTo,UINT& DestStride);
extern void RHIUnlockTextureCubeFace(FTextureCubeRHIParamRef Texture,UINT FaceIndex,UINT MipIndex);
...
//  [2006/11/20 YTS, temporarily inserted] similar above
extern FSurfaceRHIRef RHICreateTargetable3DSurface(
	UINT SizeX,
	UINT SizeY,
	UINT SizeZ,
	BYTE Format,
...
extern void RHIReadHdrSurfaceData(FSurfaceRHIRef &Surface,UINT MinX,UINT MinY,UINT MaxX,UINT MaxY,TArray<FLOAT>& OutData);
...
// Vertex state.
extern void RHISetStreamSource(FCommandContextRHI* Context,UINT StreamIndex,FVertexBufferRHIParamRef VertexBuffer,UINT Stride);
...
extern void RHISetDepthBoundsTest(FCommandContextRHI* Context,UBOOL bEnable,const FVector4 &ClipSpaceNearPos,const FVector4 &ClipSpaceFarPos);
...
extern void RHISetPixelShaderParameter(FCommandContextRHI* Context,FPixelShaderRHIParamRef PixelShader,UINT BaseRegisterIndex,UINT NumVectors,const UBOOL* NewValue);
...
extern void RHISetSRGBWriteEnable(FCommandContextRHI* Context,UBOOL bEnable);
...
// AA function
extern UBOOL RHIGetUserFriendlyAntialiasingName( INT Mode, FString& OutName );
extern INT RHISetAntialiasMode( INT Mode );
