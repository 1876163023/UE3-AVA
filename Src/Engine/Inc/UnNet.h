...
static inline UBOOL NEQ(CipherByte A,CipherByte B,UPackageMap* Map,UActorChannel* Channel) {return A!=B;} 
#endif
// }} 20080215 dEAthcURe|CD
...
{
	return ((A.Position - B.Position).SizeSquared() > 0.4f) || ((A.Quaternion - B.Quaternion).SizeSquared() > 0.001f);
}

static inline UBOOL NEQ(FUniqueNetId& A,FUniqueNetId& B,UPackageMap* Map,UActorChannel* Channel)
{
	return appMemcmp(A.Uid,B.Uid,sizeof(BYTE) * 8);
}
static inline UBOOL NEQ(FVector2D& A,FVector2D& B,UPackageMap* Map,UActorChannel* Channel) {return A.X!=B.X || A.Y!=B.Y;}

static inline UBOOL NEQ(FReplicatedHitImpulse& A,FReplicatedHitImpulse& B,UPackageMap* Map,UActorChannel* Channel) {return A.HitLocation != B.HitLocation;}
