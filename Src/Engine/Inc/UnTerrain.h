...
class UTerrainComponent : public UPrimitiveComponent
{
	 ...
	 */
	TkDOPTree<class FTerrainCollisionDataProvider> BoxCheckTree;
	/**
...
protected:
	 ...
	 */
	FMaterialInstance* GetContextSpecificMaterial( FMaterialInstance* Material, const FSceneView* View );
...
public:
	...
	//<@ ava specific ; 2006. 12. 15 changmin
	virtual void CacheRadiosityLighting(const FLightingBuildOptions& BuildOptions, AvaRadiosityAdapter& RadiosityAdapter);
	INT ExportMaterial( const INT TerrainPatchX, const INT TerrainPatchY, AvaRadiosityAdapter* RadiosityAdapter );
	virtual void ExportSurfaces( AvaRadiosityAdapter* RadiosityAdapter, const INT ExportVertexBufferBase, TMap<INT,INT>& PatchToDispInfo );
	INT PatchToQuad( INT PatchIndex );
	INT QuadToPatch( INT QuadIndex );
	//>@ ava specific
...
//		TID_	= 0x8000,
	...
	// Getters
	inline UBOOL IsVisible()
	{
		return ((Data & TID_Visibility_Off) == 0);
	}

	inline UBOOL IsOrientationFlipped()
	{
		return ((Data & TID_OrientationFlip) != 0);
	}

	inline UBOOL IsUnreachable()
	{
		return ((Data & TID_Unreachable) != 0);
	}

	inline UBOOL IsLocked()
	{
...
class FTerrainMaterialResource: /***public UMaterial, ***//***private***/public FMaterial, public FMaterialInstance
{
public:
//    DECLARE_CLASS(FTerrainMaterialResource,UMaterial,0,Engine)
	...
	virtual UBOOL NeedsBumpedLightmap() const { return TRUE; }
	//>@ ava
	/**
...
private:
	...
	TArray<const FMaterialInstance*> MaterialDependencies;
