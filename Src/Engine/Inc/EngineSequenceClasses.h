...
public:
    ...
    UBOOL HasLinkedOps(UBOOL bConsiderInputLinks=0) const;
    void GetLinkedObjects(TArray<class USequenceObject*>& out_Objects,class UClass* ObjectType=NULL,UBOOL bRecurse=0);
    virtual void PopulateLinkedVariableValues();
    virtual void PublishLinkedVariableValues();
    DECLARE_FUNCTION(execHasLinkedOps)
    {
        P_GET_UBOOL_OPTX(bConsiderInputLinks,0);
        P_FINISH;
...
class USeqAct_AudioStream : public USequenceAction
{
public:
    //## BEGIN PROPS SeqAct_AudioStream
    BYTE Operation;
    INT Stream;
    //## END PROPS SeqAct_AudioStream

    DECLARE_CLASS(USeqAct_AudioStream,USequenceAction,0,Engine)
	void Activated();
};
...
public:
    ...
    FLOAT ForceDuration;
    TArrayNoInit<class UMaterialInstance*> ForceMaterials;
    FLOAT RemainingTime;
...
public:
    ...
    BITFIELD bForceReversePos:1;
    BITFIELD bClientSideOnly:1;
    BITFIELD bIsSkippable:1;
    BITFIELD bHostMigration:1;
    FLOAT SkipPosition;
...
class USeqAct_Nuke : public USequenceAction
{
public:
    //## BEGIN PROPS SeqAct_Nuke
    //## END PROPS SeqAct_Nuke

    DECLARE_CLASS(USeqAct_Nuke,USequenceAction,0,Engine)
	virtual void Activated();
};
...
public:
    ...
    class UMaterialInstance* NewMaterial;
    INT MaterialIndex;
...
class USeqCond_SwitchClass : public USequenceCondition
{
public:
    ...
    DECLARE_CLASS(USeqCond_SwitchClass,USequenceCondition,0,Engine)
	void Activated();
	virtual void	UpdateDynamicLinks();
	virtual FColor	GetConnectionColor( INT ConnType, INT ConnIndex, INT MouseOverConnType, INT MouseOverConnIndex );
};
...
public:
    ...
    BITFIELD bAimToInteract:1 GCC_BITFIELD_MAGIC;
...
DECLARE_NATIVE_TYPE(Engine,USeqAct_AudioStream);
...
DECLARE_NATIVE_TYPE(Engine,USeqAct_Nuke);
...
#define AUTO_INITIALIZE_REGISTRANTS_ENGINE_SEQUENCE \
	...
	USeqAct_AudioStream::StaticClass(); \
...
VERIFY_CLASS_OFFSET_NODIE(U,SeqAct_AudioStream,Operation)
VERIFY_CLASS_OFFSET_NODIE(U,SeqAct_AudioStream,Stream)
VERIFY_CLASS_SIZE_NODIE(USeqAct_AudioStream)
...
VERIFY_CLASS_OFFSET_NODIE(U,SeqAct_Interp,prevPosition)
...
VERIFY_CLASS_SIZE_NODIE(USeqAct_Nuke)
...
VERIFY_CLASS_OFFSET_NODIE(U,SequenceEvent,MaxWidth)
