...
public:
	...
	friend FArchive& operator<< (FArchive &Ar, FVert &Vert)
	{
		...
		if(Ar.Ver() >= VER_BACKFACESHADOWTEXCOORD)
		{
...
//
struct FBspNode // 58 bytes + ( ava specific : 4 bytes for cluster number )
{
	//<@ ava specific; 2006/8/25 changmin
	// vertices °¡ ¸¹À¸¸é, geometric error occurrance °¡ ³ô¾ÆÁø´Ù. radÀÇ 64·Î ¸ÂÃã
	//enum {MAX_NODE_VERTICES=255};	// Max vertices in a Bsp node.
	enum { MAX_NODE_VERTICES=64 };
	//>@ ava
...
//<@ ava specific ; 2006. 11. 16 changmin
struct FLegacyBspNode // 58 bytes
{
	// Persistent information.
	FPlane		Plane;			// 16 Plane the node falls into (X, Y, Z, W).
	FZoneSet	ZoneMask;		// 8  Bit mask for all zones at or below this node (up to 64).
	...
	friend FArchive& operator<<( FArchive& Ar, FLegacyBspNode& N );
};
//>@ ava
...
enum EPolyFlags
{
	...
	PF_Hint					= 0x00001000,	// Hint brush	// 2006. 11. 27
	PF_SecondaryLightSource	= 0x00002000,	// 2007. 8. 13 This Polygon has Secondary Light Source ( LD set this flags for good lighting )
	PF_ForceCastSunShadow	= 0x00004000,	// 2007. 12. 26	
	//>@ ava
...
public:
	/** model vertex data */
	TResourceArray<FModelVertex,FALSE,VERTEXBUFFER_ALIGNMENT> Vertices;
	...
	virtual FString GetFriendlyName() { return TEXT("BSP vertices"); }
...
public:
	...
	// FRenderResource interface.
	virtual void InitRHI();
	virtual FString GetFriendlyName() { return TEXT("BSP shadow vertices"); }
...
//!{ 2006-05-08	Çã Ã¢ ¹Î
struct FBspNodeLightMap
{
	INT				SizeX;
	INT				SizeY;
	TArray<FVector>	Luxels;
	FMatrix			WorldToLuxel;

	FVector GetSample(const FVector& HitLocation) const;

	friend FArchive& operator<<( FArchive& Ar, FBspNodeLightMap& Surf );
};
//!} 2006-05-08	Çã Ã¢ ¹Î
...
class FAvaPortal : public FPoly
{
public:
	INT	iFrontLeaf, iBackLeaf, iNode;

	FAvaPortal( FPoly& Poly, INT InFrontLeaf, INT InBackLeaf, INT InNode )
		: FPoly( Poly ), iFrontLeaf( InFrontLeaf ), iBackLeaf( iBackLeaf ), iNode( InNode )
	{}
...
{
public:
	INT		iFirstPortal, NumPortals;
	FBox	PortalBounds;
	friend FArchive& operator<<( FArchive& Ar, AvaLeafInfo& LeafInfo )
	{
		Ar << LeafInfo.iFirstPortal << LeafInfo.NumPortals << LeafInfo.PortalBounds;
		return Ar;
	}
};
...
public:
	...
	friend FArchive& operator<<( FArchive& Ar, AvaPortal& Portal )
	{
		Ar << Portal.Normal << Portal.Vertices;
		return Ar;
	}
};
//>@ ava
...
class UModel : public UObject
{
	...
	// avarad export context
	INT							ExportedVertexBase;
	TArray<INT>					NodeToExportedFace;
	// pvs system °ü·Ã Ãß°¡
	// persistent
	INT							NumClusters;
	INT							LeafBytes;
	TArray<BYTE>				VisBytes;
	TArray<FColor>				LeafColors;
	TArray<INT>					StaticMeshLeaves;
	...
	//<@ ava specific ; 2008. 2. 25 changmin
	TArray<AvaPortal>				Portals;
	TArray<AvaLeafInfo>			LeafInfos;

	//>@ ava

	/** An index buffer for each material used by the model, containing all the nodes with that material applied. */
	TDynamicMap<UMaterialInstance*,FRawIndexBuffer32> MaterialIndexBuffers;

	//<@ ava specific ; 2006. 9. 27 changmin
	/** An index buffer for each component for depth drawing. */
	TDynamicMap<WORD, FRawIndexBuffer32> ComponentIndexBuffers;
	//>@ ava
	...
	*/
	...
	// to understand bsp tree
	void DumpBspTree();
	// to view model data info for optimizing performance
	void DumpBspInfo();
	//>@ ava

	//<@ ava specific ; 2006. 11. 17 changmin
	INT GetCurrentLeaf( const FVector& Position );
	//>@ ava

	//<@ ava specific ; 2008. 2.26 changmin
	UBOOL IsReadyToRender() { return !InvalidSurfaces; }
	//>@ ava
...
public:
	...
	mutable const FModelElement *Next;
	mutable const FModelElement *Sibling;
	//>@ ava
	...
	/** Node render data */
	TArray<FBspNodeRenderData> NodeRenderData;
	TArray<FBspNodeRenderData> SunLitNodeRenderData;

	/**
	 * Minimal initialization constructor.
	 */
	FModelElement(UModelComponent* InComponent,UMaterialInstance* InMaterial);
	//<@ ava specific ; 2006. 12. 01
	FModelElement() : Next(NULL), Sibling(NULL) {}
	//FModelElement() {}
	//>@ ava
...
class UModelComponent : public UPrimitiveComponent
{
	 ...
	 * Minimal initialization constructor.
	 */
	UModelComponent( UModel* InModel, INT InZoneIndex, WORD InComponentIndex, DWORD MaskedSurfaceFlags, const TArray<WORD>& InNodes );
	UModelComponent() {}	

	//<@ ava specific ; 2006. 11. 17 changmin
	UModelComponent( UModel* InModel, INT InZoneIndex, WORD InComponentIndex, DWORD MaskedSurfaceFlags, const TArray<WORD>& InNodes, INT ClusterNumber );
	//>@ ava
	...
	virtual UBOOL HasTranslucency() const;

	//<@ ava specific ; 2006. 10. 9 changmin
	void CreateDepthMeshElement();
	void UpdateLightmapTextureCoordinate();
	void ResetLightmapTextureCoordinateScaleAndBias();
	//>@ ava
	...
	const TIndirectArray<FModelElement>& GetDepthElements() const { return DepthElements; }
	//>@ ava
...
private:
	...
	//<@ ava specific ; 2006. 9. 28 changmin
	TIndirectArray<FModelElement> DepthElements;	// The elements used to render the component for depth.
	INT ClusterNumber;	

	friend struct FAvaModelComponentStaticLighting;
	//>@ ava
};
