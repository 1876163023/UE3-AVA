...
public:
	...
	FTextureLayout(const FTextureLayout& Other):
		SizeX(Other.SizeX),
		SizeY(Other.SizeY),
		PowerOfTwoSize(Other.PowerOfTwoSize)
	{
		for (INT NodeIndex=0; NodeIndex<Other.Nodes.Num(); ++NodeIndex)
		{	
			new (Nodes) FTextureLayoutNode( Other.Nodes(NodeIndex) );
		}
	}
	 ...
	 */
	UBOOL AddElement(UINT* OutBaseX,UINT* OutBaseY,UINT ElementSizeX,UINT ElementSizeY,const UBOOL* ZeroTable = NULL, INT ZeroPitch = 0)
	{
...
#if !FINAL_RELEASE
			if (ZeroTable != NULL && ZeroPitch > 1)
			{
				GatherReusableArea(NodeIndex,ZeroTable,ZeroPitch);
			}
#endif
...
private:
...
#if !FINAL_RELEASE
	void GatherReusableArea( INT NodeIndex, const UBOOL* ZeroTable, INT ZeroPitch );
#endif
	...
	struct FTextureLayoutNode
	{
		...
		FTextureLayoutNode(const FTextureLayoutNode& Other):
			ChildA(Other.ChildA),
			ChildB(Other.ChildB),
			MinX(Other.MinX),
			MinY(Other.MinY),
