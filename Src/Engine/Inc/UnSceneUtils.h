...
/**
 * Platform specific function for adding a draw event that can be viewed in
 * PIX
 */
void appBeginDrawEvent(const FColor& Color,const TCHAR* szText);
/**
 * Platform specific function for closing a draw event that can be viewed in
 * PIX
 */
...
#if defined(XBOX) || defined(_WINDOWS_) || PS3
#define PLATFORM_SUPPORTS_DRAW_MESH_EVENTS 1
#define PLATFORM_SUPPORTS_RENDERING_FROM_LOCKED_TEXTURES 1
#else
...
#define PLATFORM_SUPPORTS_SCREENSHOT_FROM_SCREEN 1
	...
	struct FDrawEvent
	{
		...
		void CDECL operator()(const FColor& Color,const TCHAR* Fmt, ...) const
		{
			...
			appGetVarArgs(TempStr,ARRAY_COUNT(TempStr),Fmt,ptr);
			va_end(ptr);
	...
	#define BEGIN_DRAW_EVENT(x) { FDrawEvent Event_##x; if( GEmitDrawEvents ) Event_##x
	#define END_DRAW_EVENT }
	...
	#define BEGIN_DRAW_EVENT(x) __noop
	#define END_DRAW_EVENT
	...
	// Macros to allow for scoping of draw events
	#define BEGIN_DRAW_EVENT(x) { FDrawEvent Event_##x; Event_##x
	#define END_DRAW_EVENT }
	#define SCOPED_DRAW_EVENT(x) FDrawEvent Event_##x; Event_##x
