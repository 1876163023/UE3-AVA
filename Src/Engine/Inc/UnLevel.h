...
public:
	...
	virtual FCheckResult* ActorLineCheck( FMemStack& Mem, const FVector& End, const FVector& Start, const FVector& Extent, DWORD TraceFlags, AActor *SourceActor, const ULightComponent* SourceLight )=0;
...
class ULineBatchComponent : public UPrimitiveComponent, public FPrimitiveDrawInterface
{
	...
	virtual void DrawTriangle(
		...
		INT V1,
		INT V2,
		const FMaterialInstance* Material,
		BYTE DepthPriorityGroup
		) {}
...
enum ETraceFlags
{
	...
	TRACE_Tesselation			= 0x04000, // Check collision against highest tesselation level (not valid for box checks)
...
class ULevel : public ULevelBase
{
	...
	TMap<UTexture2D*,TArray<FStreamableTextureInstance> >	TextureToInstancesMapPerPlatform[SP_MaxPlatforms]; 
	...
	TMap<UTexture2D*,UBOOL>									ForceStreamTexturesPerPlatform[SP_MaxPlatforms];
	...
	void HmRouteBeginPlay(UBOOL bForDynamicActorsOnly=FALSE); // 20070103 dEAthcURe|HM
	#endif
	...
	AActor* findActorByClassName(const FString& name, int& idxStart);
	AActor* findActor(UClass* pBase);
	#endif
	// }} 20061026 dEAthcURe|HM
};
