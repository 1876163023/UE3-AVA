...
struct FkDOPCollisionTriangle
{
	...
	// Set up indices
	FkDOPCollisionTriangle(DWORD Index1,DWORD Index2,DWORD Index3) :
	 ...
	 */
	FkDOPCollisionTriangle(DWORD Index1,DWORD Index2,DWORD Index3,DWORD Material) :
...
struct FkDOPBuildCollisionTriangle : public FkDOPCollisionTriangle
{
	 ...
	 */
	FkDOPBuildCollisionTriangle(DWORD Index1,DWORD Index2,DWORD Index3,
		DWORD MaterialIndex,
		const FVector& vert0,const FVector& vert1,const FVector& vert2) :
		FkDOPCollisionTriangle(Index1,Index2,Index3,MaterialIndex),
...
template <typename COLL_DATA_PROVIDER> struct TkDOP;
template <typename COLL_DATA_PROVIDER> struct TkDOPNode;
template <typename COLL_DATA_PROVIDER> struct TkDOPTree;
template <typename COLL_DATA_PROVIDER> struct TkDOPLineCollisionCheck;
template <typename COLL_DATA_PROVIDER> struct TkDOPBoxCollisionCheck;
template <typename COLL_DATA_PROVIDER> struct TkDOPPointCollisionCheck;
template <typename COLL_DATA_PROVIDER> struct TkDOPSphereQuery;
template <typename COLL_DATA_PROVIDER> struct TkDOPFrustumQuery;
	...
	public FkDOPPlanes
{
	...
	/**
	 * Intializes the planes to invalid states so that adding any point
	 ...
	 */
	FORCEINLINE TkDOP(const TkDOP<COLL_DATA_PROVIDER>& kDOP,const FVector& Extent)
	{
	 ...
	 */
	void AddTriangles(INT StartIndex,INT NumTris,
		TArray<FkDOPBuildCollisionTriangle>& BuildTriangles)
	{
		// Reset the min/max planes
		Init();
		// Go through the list and add each of the triangle verts to our volume
		for (INT Triangle = StartIndex; Triangle < StartIndex + NumTris; Triangle++)
		{
	 ...
	 */
	UBOOL LineCheck(TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>& Check,FLOAT& HitTime) const
	{
	 ...
	 */
	UBOOL PointCheck(TkDOPPointCollisionCheck<COLL_DATA_PROVIDER>& Check) const
	{
	...
	// Serialization
	friend FArchive& operator<<(FArchive& Ar,TkDOP<COLL_DATA_PROVIDER>& kDOP)
	{
...
template <typename COLL_DATA_PROVIDER> struct TkDOPNode
{
	...
	typedef TkDOPNode<DataProviderType>	NodeType;

	/** Exposes kDOP type to clients. */
	typedef TkDOP<DataProviderType>		kDOPType;
	 ...
	 */
	inline UBOOL LineCheck(TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>& Check) const
	{
		UBOOL bHit = 0;
	 ...
	 * @param Check -- The aggregated line check data
	 */
	FORCEINLINE UBOOL LineCheckTriangles(TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>& Check) const
	{
		// Assume a miss
		UBOOL bHit = FALSE;
		// Check for an early out
		...
		for (DWORD CollTriIndex = t.StartIndex;
			CollTriIndex < t.StartIndex + t.NumTriangles &&
			(bHit == FALSE || bStopAtAnyHit == FALSE);
			CollTriIndex++)
		{
			// Get the collision triangle that we are checking against
			const FkDOPCollisionTriangle& CollTri =	Check.CollisionTriangles(CollTriIndex);
	 ...
	 */
	UBOOL LineCheckTriangle(TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>& Check,
	 ...
	 */
	UBOOL BoxCheck(TkDOPBoxCollisionCheck<COLL_DATA_PROVIDER>& Check) const
	{
		UBOOL bHit = FALSE;
		// If this is a node, check the two child nodes and pick the closest one
		// to recursively check against and only check the second one if there is
	 ...
	 * @param Check -- The aggregated box check data
	 */
	FORCEINLINE UBOOL BoxCheckTriangles(TkDOPBoxCollisionCheck<COLL_DATA_PROVIDER>& Check) const
	{
		// Assume a miss
		UBOOL bHit = 0;
	 ...
	 * @param MaterialIndex -- The material for this triangle if it is hit
	 */
	FORCEINLINE UBOOL BoxCheckTriangle(TkDOPBoxCollisionCheck<COLL_DATA_PROVIDER>& Check,
		const FVector& v1,const FVector& v2,const FVector& v3,INT MaterialIndex) const
	{
		FLOAT HitTime = 1.f;
	 ...
	 * @param Check -- The aggregated point check data
	 */
	FORCEINLINE UBOOL PointCheckTriangles(TkDOPPointCollisionCheck<COLL_DATA_PROVIDER>& Check) const
	{
		// Assume a miss
		UBOOL bHit = FALSE;
		// Loop through all of our triangles. We need to check them all in case
	 ...
	 * @param MaterialIndex -- The material for this triangle if it is hit
	 */
	FORCEINLINE UBOOL PointCheckTriangle(TkDOPPointCollisionCheck<COLL_DATA_PROVIDER>& Check,
		const FVector& v1,const FVector& v2,const FVector& v3,INT MaterialIndex) const
	{
		// Use the separating axis thereom to see if we hit
		FSeparatingAxisPointCheck PointCheck(v1,v2,v3,Check.LocalStart,Check.Extent,
	 ...
	 */
	FORCEINLINE void SphereQuery(TkDOPSphereQuery<COLL_DATA_PROVIDER>& Query) const
	{
		...
		{
			for (DWORD TriIndex = t.StartIndex;
	 ...
	 */
	UBOOL FrustumQuery( TkDOPFrustumQuery<COLL_DATA_PROVIDER>& Query ) const
	{
...
template<typename COLL_DATA_PROVIDER> struct TkDOPTree
{
	 ...
	 * @param BuildTriangles -- The list of triangles to use for the build process
	 */
	void Build(TArray<FkDOPBuildCollisionTriangle>& BuildTriangles)
	{
	 ...
	 * @param Check -- The aggregated line check data
	 */
	UBOOL LineCheck(TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>& Check) const
	{
		UBOOL bHit = FALSE;
		FLOAT HitTime;
	 ...
	 */
	void SphereQuery(TkDOPSphereQuery<COLL_DATA_PROVIDER>& Query) const
	{
	...
	// Serialization
	friend FArchive& operator<<(FArchive& Ar,TkDOPTree<COLL_DATA_PROVIDER>& Tree)
	{
 ...
 */
template <typename COLL_DATA_PROVIDER> struct TkDOPCollisionCheck
{
	/** Exposes data provider type to clients. */
	typedef COLL_DATA_PROVIDER DataProviderType;
	...
	typedef TkDOPTree<DataProviderType> TreeType;
 ...
 */
template <typename COLL_DATA_PROVIDER> struct TkDOPLineCollisionCheck :
	public TkDOPCollisionCheck<COLL_DATA_PROVIDER>
{
	/**
	...
	TkDOPLineCollisionCheck(const FVector& InStart,const FVector& InEnd,
		...
		TkDOPCollisionCheck<COLL_DATA_PROVIDER>(InCollDataProvider),
		Result(InResult), Start(InStart), End(InEnd), TraceFlags(InTraceFlags)
	{
		const FMatrix& WorldToLocal = TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetWorldToLocal();
	...
	FORCEINLINE FVector GetHitNormal(void)
	{
		...
		FVector Normal = TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetLocalToWorldTransposeAdjoint().TransformNormal(LocalHitNormal).SafeNormal();
		// Flip the normal if the triangle is inverted
		if (TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetDeterminant() < 0.f)
		{
			Normal = -Normal;
		}
		return Normal;
 ...
 */
template <typename COLL_DATA_PROVIDER> struct TkDOPBoxCollisionCheck :
	public TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>
{
	// Constant input vars
	...
	TkDOPBoxCollisionCheck(const FVector& InStart,const FVector& InEnd,
		...
		TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>(InStart,InEnd,InTraceFlags,InCollDataProvider,InResult),
		Extent(InExtent)
	{
		// Move extent to local space
		LocalExtent = FBox(-Extent,Extent).TransformBy(TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetWorldToLocal()).GetExtent();
		// Transform the PlaneNormals into local space.
		LocalBoxX = TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetWorldToLocal().TransformNormal(FkDOPPlanes::PlaneNormals[0]);
		LocalBoxY = TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetWorldToLocal().TransformNormal(FkDOPPlanes::PlaneNormals[1]);
		LocalBoxZ = TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetWorldToLocal().TransformNormal(FkDOPPlanes::PlaneNormals[2]);
	}
};
 ...
 */
template <typename COLL_DATA_PROVIDER> struct TkDOPPointCollisionCheck :
	public TkDOPBoxCollisionCheck<COLL_DATA_PROVIDER>
{
	...
	TkDOPPointCollisionCheck(const FVector& InLocation,const FVector& InExtent,
		const COLL_DATA_PROVIDER& InCollDataProvider,FCheckResult* InResult) :
		TkDOPBoxCollisionCheck<COLL_DATA_PROVIDER>(InLocation,InLocation,InExtent,0,InCollDataProvider,InResult),
	...
	FORCEINLINE FVector GetHitLocation(void)
	{
		...
		return TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetLocalToWorld().TransformFVector(
			TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>::LocalStart + 
			TkDOPLineCollisionCheck<COLL_DATA_PROVIDER>::LocalHitNormal * BestDistance);
	}
};
	...
	public TkDOPCollisionCheck<COLL_DATA_PROVIDER>
{
		...
		ReturnTriangles(OutTriangles)
	{
		...
		FPlane radiusVec(InSphere.W, InSphere.W, InSphere.W, 0);
		FBox WorldBox(InSphere - radiusVec, InSphere + radiusVec);
		LocalBox = WorldBox.TransformBy(TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetWorldToLocal());
	}
};
	...
	public TkDOPCollisionCheck<COLL_DATA_PROVIDER>
{
	...
	TkDOPFrustumQuery(FPlane* InFrustumPlanes,INT InNumFrustumPlanes,
		TArray<DWORD>& OutLeaves,const COLL_DATA_PROVIDER& InCollDataProvider) :
		TkDOPCollisionCheck<COLL_DATA_PROVIDER>(InCollDataProvider),
		ReturnLeaves( OutLeaves ),
		ReturnNumTris(0)
	{
		// matrices needed to go from world to local space
		const FMatrix& ToLocalMat = TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetWorldToLocal();
		const FMatrix& ToLocalMatTA = ToLocalMat.TransposeAdjoint();
		FLOAT Determinant = TkDOPCollisionCheck<COLL_DATA_PROVIDER>::CollDataProvider.GetDeterminant();
	...
	FORCEINLINE void AddLeafIndex( DWORD LeafIndex )
	{
		checkSlow( TkDOPCollisionCheck<COLL_DATA_PROVIDER>::Nodes(LeafIndex).bIsLeaf );
		ReturnLeaves.AddItem( LeafIndex );
		ReturnNumTris += TkDOPCollisionCheck<COLL_DATA_PROVIDER>::Nodes(LeafIndex).t.NumTriangles;
	}
};
#endif
