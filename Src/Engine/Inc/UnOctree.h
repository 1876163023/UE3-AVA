...
public:
	 ...
	 * @param Check the precalculated info for this check
	 */
	void ActorOverlapCheck(FPrimitiveOctree* Octree,const FOctreeNodeBounds& Bounds,FRadiusOverlapCheck& Check);
...
public:
	...
	DWORD			ChkTraceFlags;
	AActor*			ChkActor;
	const ULightComponent* ChkLight;
	FLOAT			ChkRadiusSqr;
	FBox			ChkBox;
	...
	virtual FCheckResult* ActorLineCheck(FMemStack& Mem, 
		...
		DWORD TraceFlags, 
		AActor *SourceActor,
		const class ULightComponent* SourceLight);
