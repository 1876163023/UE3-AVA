...
// UIRoot::FInputCompositionStringData
struct FInputCompositionStringData;
struct FInputCandidateStringData;
struct FInputReadingStringData;
...
public:
	 ...
	 */
	virtual UBOOL InputKey(FViewport* Viewport,INT ControllerId,FName Key,EInputEvent Event,FLOAT AmountDepressed = 1.f,UBOOL bGamepad=FALSE) { InputKey(Viewport,Key,Event,AmountDepressed,bGamepad); return 0; }
	 ...
	 */
	virtual UBOOL InputChar(FViewport* Viewport,INT ControllerId, const FInputCompositionStringData& CompStrData ) { InputChar(Viewport, CompStrData); return 0; }
	
	/**
	 * IME CandidateWindow Notification
	 * CandidateWindow는 동북아 지역의 언어(중국어, 한국어, 일본어)에 쓰임.
	 * 예를 들어 한국어에서는 'ㅇ'을 'ⓐ'로 바꾸거나 '대'를 '大'로 변환하는데 쓰임
	 *
	 * @param	Viewport - The viewport which the axis movement is from.
	 * @param	ControllerId - The controller which the axis movement is from.
	 ...
	 * @return	True to consume the character, false to pass it on.	 
	 */
	virtual UBOOL InputCandidate(FViewport* Viewport, UINT ControllerID, const FInputCandidateStringData& CandStrData)
	{
		InputCandidate(Viewport, CandStrData);
		return FALSE;
	}
	 ...
	 * @return	True to consume the character, false to pass it on.	 
	 */
	virtual UBOOL InputReadingString(FViewport* Viewport, UINT ControllerID, const FInputReadingStringData& ReadStrData)
	{
		InputReadingString(Viewport, ReadStrData);
		return FALSE;
	}
	...
	virtual void InputAxis(FViewport* Viewport,FName Key,FLOAT Delta,FLOAT DeltaTime) {}
	
	/* IME Support */
	virtual void InputChar(FViewport* Viewport, const FInputCompositionStringData& CompStrData ) {}
	virtual void InputCandidate(FViewport* Viewport, const FInputCandidateStringData& CandStrData ) {}
	virtual void InputReadingString(FViewport *Viewport, const FInputReadingStringData& ReadStrData ) {}
	//@}
	...
	*/
	virtual UBOOL ReguiresKeyboardInput() const { return TRUE; }
};
...
public:
	 ...
	 */
	virtual FName GetVirtualKeyName( INT KeyCode ) const PURE_VIRTUAL(UClient::GetVirtualKeyName,return NAME_None;);
	...
	*/
	virtual BYTE GetKeyCode( FName VirtualKeyName ) const PURE_VIRTUAL(UClient::GetKeyCode,return NAME_None;);
};
