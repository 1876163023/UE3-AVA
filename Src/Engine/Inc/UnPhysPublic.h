...
enum EPhysicsStats
{
	...
	STAT_TotalHWDynamicBodies,
	STAT_AwakeHWDynamicBodies,
	STAT_SWSolverBodies,
	STAT_SWNumPairs,
	STAT_SWNumContacts,
	STAT_SWNumJoints,
	...
	STAT_EffectDynamicsTime,
	STAT_TotalEffectBodies
};
...
struct FRigidBodyContactInfo
{
	...
	FRigidBodyContactInfo( const FVector& InContactPosition, const FVector& InContactNormal, const FVector& InContactVelocity0, const FVector& InContactVelocity1,
							UPhysicalMaterial* InPhysMat0, UPhysicalMaterial* InPhysMat1 )
	: ContactPosition(InContactPosition), ContactNormal(InContactNormal)
	{
...
public:
	...
	void	DrawElemSolid(class FPrimitiveDrawInterface* PDI, const FMatrix& ElemTM, FLOAT Scale, const FMaterialInstance* MaterialInstance);
...
public:
	...
	void InitConvexVertexFactory(const FConvexCollisionVertexBuffer* VertexBuffer)
	{
		if(IsInRenderingThread())
		{
			...
			NewData.TangentBasisComponents[1] = STRUCTMEMBER_VERTEXSTREAMCOMPONENT(VertexBuffer,FDynamicMeshVertex,TangentY,VET_PackedNormal);
			NewData.TangentBasisComponents[2] = STRUCTMEMBER_VERTEXSTREAMCOMPONENT(VertexBuffer,FDynamicMeshVertex,TangentZ,VET_PackedNormal);
			SetData(NewData);
		}
		else
		{
...
public:
	...
	void	DrawAggGeom(class FPrimitiveDrawInterface* PDI, const FMatrix& ParentTM, const FVector& Scale3D, const FColor Color, const FMaterialInstance* MatInst, UBOOL bPerHullColor, UBOOL bDrawSolid);
