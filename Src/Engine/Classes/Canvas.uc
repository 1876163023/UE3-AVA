...
function FillColor(float XL, float YL)
{
	DrawTile(DefaultTexture, XL, YL, 0, 0, 1, 1);
}

/**	2006/12/11 YTS, 돌려찍기위한 DrawTile
 *
 *	@param	Tex - 찍으려고 하는 Texture2D
 ...
 */
native final function DrawTileRotate( Texture2D Tex, float AxisX, float AxisY, float DegAngle, float X, float Y, float XL, float YL, float U, float V, float UL, float VL );
