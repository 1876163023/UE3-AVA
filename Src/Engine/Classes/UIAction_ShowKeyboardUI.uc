...
event ReadKeyboardInput()
{
	...
	if (OnlineSub != None)
	{
		...
		if (PlayerInt != None)
		{
			...
			// Show the UI
			if (PlayerInt.ShowKeyboardUI(class'UIInteraction'.static.GetPlayerControllerId(PlayerIndex),TitleText,DescriptionText,bShouldValidate,DefaultText) == false)
			{
...
function OnKeyboardInputDone(bool bWasSuccessful)
{
	...
	if (OnlineSub != None)
	{
		...
		if (PlayerInt != None)
		{
			...
			{
				StringReturnValue = PlayerInt.GetKeyboardInputResults();
			}
			else
			{
