...
struct native ConsoleMessage
{
	...
	/** ava 채팅 메세지에 색깔, 플레이어 위치, 메세지를 보낸사람의 팀정보등을 저장*/
	var string	LeaderLabel;
	var Name	TypeName;
	var string	PlayerName;
	var int		TeamIndex;				//	(TeamSay / Say) , (BlueWaypointTeam / YellowWaypointTeam)
...
function DrawEngineHUD()
{
	...
	myText = "Copyright (C) 1998-2006, Epic Games, Inc.";
	Canvas.Font = class'Engine'.Static.GetTinyFont();
	Canvas.StrLen(myText, XL, YL);
...
)
{
	...
	else if ( InMessageClass.default.bIsPartiallyUnique )
	{
		for( i = 0; i < LocalMessagesArrayCount; i++ )
		{
		    if( ( LocalMessages[i].Message == InMessageClass ) && ( LocalMessages[i].Switch == Switch ) )
				break;
		}
	}
	...
	LocalMessages[i].Message		= InMessageClass;
	LocalMessages[i].Switch			= Switch;
	LocalMessages[i].EndOfLife		= LifeTime + WorldInfo.TimeSeconds;
