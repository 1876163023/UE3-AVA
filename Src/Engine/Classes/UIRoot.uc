...
enum ETextClipMode
{
	...
	/** scale the text to fit the bounds */
	CLIP_Scaled<DisplayName=Scaled>,

	/** proportionally scale the text to fit best */
	CLIP_ScaledBestFit<DisplayName=ScaledBestFit>,
...
enum EUIAlignment
{
	...
	UIALIGN_Default<DisplayName=Inherit>,
};
...
const DEFAULT_SIZE_X = 800;
const DEFAULT_SIZE_Y = 600;
...
{
	var() interp float					Value[EUIOrientation.UIORIENT_MAX];
...
struct native UIScreenValue_Bounds
{
	 ...
	 */
	var()	interp editconst public{private}	float					Value[EUIWidgetFace.UIFACE_MAX];
...
struct native UIScreenValue_AutoSizeRegion
{
	...
	var()		EUIAutoSizeEvalType		EvalType[EUIAutoSizeConstraintType.UIAUTOSIZEREGION_MAX];
 ...
 * Rather than representing an X,Y coordinate, this struct's Value represents the
 * amount of padding to apply to each end of the range, where Value[0] is the padding
 * to apply to the left/top side of the auto-size region, and Value[1] is the padding
 * to apply to the right/bottom side of the auto-size region.
 * @todo - currently unused
 */
struct native AutoSizePadding extends UIScreenValue_Position
{
structcpptext
{
	/**
...
struct native AutoSizeData
{
	...
	// [2006/11/09 YTS, Insert], there's an indipendency between 'AutoSize' and 'Padding'
	var()		bool					bUsePadding;
...
struct native DefaultEventSpecification
{
	...
	var	duplicatetransient UIEvent				EventTemplate;
...
struct native InputKeyAction
{
	...
	/** The old action to execute variable. */
	var()	deprecated UIAction 	ActionToExecute;
...
struct native UIDockingSet
{
	 ...
	 */
	var()	const	interp				float					DockPadding[EUIWidgetFace.UIFACE_MAX];
...
structcpptext
{
	...
	/**
	 * 현재 UIDataStoreBinding의 MarkupString에 해당하는 Provider와 DataStore를 반환
	 * ex) MarkupString이 avaNet.CashItems.Popup.ItemName이면 DataStore는 avaNet, Provider는 CashItemPopup을 반환
	 * 
	 * 위의 ResolveMarkup 과 달리 RegisterSubscribeCallback, UnregisterSubscribeCallback을 사용하지 않고 해당하는 Provider만 반환
	 *
	 ...
	 * @param	out_FieldName			MarkupString에서 Provider를 찾았을때 사용할 Field이름
...
struct native UIStringCaretParameters
{
	...
	/**
	 * For carets that use parametized materials, the MaterialInstance that was created for this caret
	 */
	var	transient	MaterialInstance	CaretMaterial;
...
/**
 * Container for data used in text styles.
 * @note: not yet used by anything
 */
struct native UITextStyleOverride
{
	/** Color to use for rendering the string or image */
	var()				LinearColor			DrawColor<DisplayName=Draw Color|EditCondition=bOverrideDrawColor>;

	/** Allow us to override the opacity of the object regardless to color */
	...
	var()				EUIAlignment		TextAlignment[EUIOrientation.UIORIENT_MAX]<EditCondition=bOverrideAlignment>;
 ...
 */
struct native UIImageStyleOverride
{
	/** Color to use for rendering the string or image */
	var()			LinearColor				DrawColor<DisplayName=Draw Color|EditCondition=bOverrideDrawColor>;
...
struct native transient UIStringNodeModifier
{
	...
	struct native transient ModifierData
	{
		...
		/* 텍스트 컬러, 이미지 컬러용 스택 (UIDataStore_Color 참조) */
		var const	transient	array<Color>	InlineTextColorStack;
		var const	transient	array<Color>	InlineImageColorStack;
	};
...
struct native transient UIStringNode_Image extends UIStringNode
{
	...
	/**
	 * 강제로 DrawCoords를 정해준다. 
	 * UIList ImageNode에서 U,V, UL, VL, 값을 강제로 적용하기 위해 쓴다.
	 */
	var()	TextureCoordinates		ForcedDrawCoords;
...
struct native export UIInputActionAlias
{
	...
	/** the input key names (e.g. KEY_Left, KEY_Right) which will use this alias */
	var deprecated array<name>		InputKeyNames;
