...
cpptext
{
	...
	virtual void hmSerialize(FArchive& Ar); // 20070807 dEAthcURe|HM
	#endif
...
/** @name Sounds													*/
//@{
//! 기획쪽에서 테스트 가능하도록 임시로 const를 주석처리함.
...
/*! @brief
		Actor.Location이 복원되어도 PHYS_RigidBody상태의 Position이 갱신되지 않으면
		소용없기 때문에 HM처리 추가.(2007/08/07 고광록)

	@note 2007/10/30
		repnotify추가해서 Client에서 마지막으로 받은 VState.RBState.Position을 저장.
*/
var hmserialize repnotify native const VehicleState	VState;
//! HM 후에 복구될 위치.
var hmserialize Vector		LastLocation;
//! HM 후에 복구될 회전값.
var hmserialize Rotator		LastRotation;
...
/**
 * SVehicle implementation of AddVelocity() uses the damagetype's KDamageImpulse as the impulse magnitude, rather than the passed in (Unreal physics) NewVelocity magnitude
 */
...
simulated function GetSVehicleDebug( out Array<String> DebugInfo )
{
	...
	DebugInfo[DebugInfo.Length] = "Gear: "$OutputGear;
