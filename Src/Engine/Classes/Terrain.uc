...
// NOTE: The size of this array must be the same as TERRAIN_CACHED_MATERIALS in UnTerrain.h
var	native const int			CachedMaterialCount;
var native const pointer		CachedMaterials{FTerrainMaterialResource*};
...
var()		int					EditorTessellationLevel;

/** Viewing collision tessellation level					*/
var			bool				bShowingCollision;

//!{ 2006-08-10	Çã Ã¢ ¹Î
var private native transient const int ExportDispInfoOffset;
//!} 2006-08-10	Çã Ã¢ ¹Î
...
public:
	...
	inline UBOOL IsTerrainQuadVisible(INT X, INT Y)
	{
		FTerrainInfoData* InfoData = GetInfoData(X, Y);
		check(InfoData);
		return InfoData->IsVisible();
	}
	...
	FMaterialInstance* GetCachedMaterial(const FTerrainMaterialMask& Mask, UBOOL& bIsTerrainResource);
	 ...
	 */
	void UpdateMaterialInstance(UMaterialInstance* InMatInst);
	 ...
	 */
	UBOOL GetClosestVertex(const FVector& InLocation, FVector& OutVertex);
