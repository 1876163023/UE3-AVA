...
{
	if (Role == ROLE_Authority && !WorldInfo.Game.bAllowVehicles && !bDeleteMe)
	{
		Destroy();
	}
	else
...
*/
function DriverRadiusDamage(float DamageAmount, float DamageRadius, Controller EventInstigator, class<DamageType> DamageType, float Momentum, vector HitLocation, class<Weapon> DamageCauser)
{
...
{
	return ( AnySeatAvailable() && !P.bIsCrouched && P.DrivenVehicle == None && P.Controller != None &&
...
function bool FindAutoExit(Pawn ExitingDriver)
{
	...
	return (	TryExitPos(ExitingDriver, GetTargetLocation() + ExitOffset + PlaceDist * CrossProduct)
			||	TryExitPos(ExitingDriver, GetTargetLocation() + ExitOffset - PlaceDist * CrossProduct)
			||	TryExitPos(ExitingDriver, GetTargetLocation() + ExitOffset - PlaceDist * FacingDir)
			||	TryExitPos(ExitingDriver, GetTargetLocation() + ExitOffset + PlaceDist * FacingDir) );
}
...
*/
function bool TryExitPos(Pawn ExitingDriver, vector ExitPos)
{
	local vector Slice, HitLocation, HitNormal, StartLocation;
	...
	HitActor = Trace(HitLocation, HitNormal, ExitPos - ExitingDriver.GetCollisionHeight()*vect(0,0,3), ExitPos, true, Slice);
	...
	return ExitingDriver.SetLocation(HitLocation + (ExitingDriver.GetCollisionHeight()+ExitingDriver.MaxStepHeight)*vect(0,0,1));
}
...
{
	if ( Super.Died(Killer, DamageType, HitLocation,bScoreKill,weaponBy) )
	{	
		SetDriving(false);
		return true;
	}
...
event bool EncroachingOn(Actor Other)
{
	...
	if ( Other == Instigator || (Vehicle(Other) != None) || Other.Role != ROLE_Authority || (!Other.bCollideActors && !Other.bBlockActors)
	     || VSize(Velocity) < MinCrushSpeed )
	{
