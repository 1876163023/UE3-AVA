...
struct native AmbientSH
{
	var	float										SHValue[12];
	var	byte										Irradiance[4];

	structcpptext
	{
		operator const FSHVectorRGB& () const
		{
			return (FSHVectorRGB&)(*this->SHValue);
		}
	}	
};
...
var(Movement) hmserialize const enum EPhysics // [!] 20070323 dEAthcURe|HM 'hmserialize'
{
	PHYS_None,
	PHYS_Walking,
	PHYS_Falling,
...
var hmserialize		bool	bTearOff; // [!] 20070323 dEAthcURe|HM 'hmserialize'	// if true, this actor is no longer replicated to new clients, and
...
var()				bool	bConsiderAllStaticMeshComponentsForStreaming;

//debug
var					bool	 bDebug;	// Used to toggle debug logging
...
var(Movement) hmserialize const vector			Location;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Actor's location; use Move to set.
var(Movement) hmserialize const rotator			Rotation;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Rotation.
var	hmserialize				vector			Velocity;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Velocity.
var bool bVelocityOptimized; // 20071218 dEAthcURe|ON velocity 최적화
var hmserialize BYTE optVelocityAngle; // [+] 20071212 dEAthcURe|ON velocity 최적화
var hmserialize float optVelocityLength; // [+] 20071212 dEAthcURe|ON velocity 최적화
var	hmserialize				vector			Acceleration;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Acceleration.
...
var hmserialize const vector		RelativeLocation;	// [!] 20070323 dEAthcURe|HM 'hmserialize' // location relative to base/bone (valid if base exists)
var hmserialize const rotator		RelativeRotation;	// [!] 20070323 dEAthcURe|HM 'hmserialize' // rotation relative to base/bone (valid if base exists)

var(Attachment) hmserialize const bool bHardAttach; // [!] 20070323 dEAthcURe|HM 'hmserialize'		// Uses 'hard' attachment code. bBlockActor must also be false.
...
var(Display) hmserialize const interp	float	DrawScale;	// [!] 20070323 dEAthcURe|HM // Scaling factor, 1.0=normal size.
...
var hmserialize const	bool		bCollideActors;		// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Collides with other actors.
var hmserialize			bool		bCollideWorld;		// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Collides with the world.
var(Collision)			bool		bCollideComplex;	// Ignore Simple Collision on Static Meshes, and collide per Poly.
var hmserialize			bool		bBlockActors;		// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Blocks other nonplayer actors.
var(Collision)			bool		bBlockOnlyProj;		// Projectile 만 Block 한다. (bBlockActors 가 treu이어야만 의미가 있다... 
var hmserialize			bool		bProjTarget;		// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Projectiles should potentially target this actor.
...
var int hmIndex; // 20061204 dEAthcURe|HM
...
/// static이면 static한 ambient cube를 바로 적용. 아니면 이것을 이용 :)
var private transient AmbientSH CurrentAmbientCube;

var private float LastAmbientCubeUpdatedTime;
...
struct native NavReference
{
	...
	structcpptext
	{
		...
		ANavigationPoint* operator~();
		FNavReference* operator=(ANavigationPoint *TargetNav)
		{
			Nav = TargetNav;
			return this;
		}
		UBOOL operator==(const FNavReference &Ref) const
		{
		...
		UBOOL operator==(ANavigationPoint *TestNav) const
		{
			return (Nav == TestNav);
		}
		UBOOL operator!=(ANavigationPoint *TestNav) const
...
cpptext
{
	...
	virtual UBOOL IsPlayerPawn() {return false;}
	...
	UBOOL IsOverlapping( AActor *Other, FCheckResult* Hit=NULL, UPrimitiveComponent* OtherPrimitiveComponent=NULL );
	 ...
	 */
	virtual UBOOL HasStaticShadowing() const { return bStatic ||  (bNoDelete && !bMovable); }
	...
	*/
	 ...
	 */
	void PlaySound( class USoundCue* InSoundCue, UBOOL bNotReplicated=FALSE, UBOOL bNoRepToOwner=FALSE, UBOOL bStopWhenOwnerDestroyed=FALSE, FVector* SoundLocation=NULL, UBOOL bNoRepToRelevant = FALSE, EAudioChannel Channel=(EAudioChannel)0 );	
	...
	/** Called each fram by Matinee to update the desired sequence by name and position within it. */
	virtual void PreviewSetAnimPosition(FName SlotName, INT ChannelIndex, FName InAnimSeqName, FLOAT InPosition) {}

	/** Called each fram by Matinee to update the desired animation channel weights for this Actor. */
...
public:
	 ...
	 */
	UBOOL IsInLevel(ULevel *TestLevel) const;
	 ...
	 */
	UBOOL IsReferencedByKismet() const;
	...
	virtual const FSHVectorRGB& GetAmbientCube();	
...
replication
{
	...
	if( (!bSkipActorPropertyReplication || bNetInitial) && bReplicateMovement
		...
		//Velocity, Physics, Acceleration; // 20071212 dEAthcURe|ON velocity 최적화 Velocity 대신 optVelocityLength, optVelocityAngle 추가
		bVelocityOptimized, Velocity, optVelocityLength, optVelocityAngle, Physics, Acceleration; // 20071212 dEAthcURe|ON velocity 최적화 Velocity 대신 optVelocityLength, optVelocityAngle 추가
 ...
 */
native final function AudioComponent CreateAudioComponent(SoundCue InSoundCue, optional bool bPlay, optional bool bStopWhenOwnerDestroyed, optional bool bUseLocation, optional vector SourceLocation, optional bool bAttachToSelf = true, optional EAudioChannel Channel);
 ...
 */
native noexport final function PlaySound(SoundCue InSoundCue, optional bool bNotReplicated, optional bool bNoRepToOwner, optional bool bStopWhenOwnerDestroyed, optional vector SoundLocation, optional bool bNoRepToRelevant, optional EAudioChannel Channel);
...
bUserOverlapCheck uses a sphere vs. box check instead of checking to see if the center of an object lies within a sphere
*/
...
event function HmReinit()
{
	/*
	`log("[dEAthcURe] Actor::HmReinit" @ self);
	bScriptInitialized = false;
	if(bDisabledForHostMigration) {
		`log("[dEAthcURe] Actor::HmReinit waking up " @ self);
	}
	bDisabledForHostMigration = false;
	*/
}
// }} 20070105 dEAthcURe|HM
...
{
	// hmserialize variable이 백업되기 전에 호출된다	
}

event function OnHmRestore()
{
	// hmserialize variable이 복원된 다음에 호출된다.	
}
// }} 20070326 dEAthcURe|HM
...
)
{
	...
	foreach VisibleCollidingActors( class'Actor', Victim, DamageRadius, HurtOrigin )
	{
		...
		{
			Victim.TakeRadiusDamage(InstigatedByController, BaseDamage, DamageRadius, DamageType, Momentum, HurtOrigin, bDoFullDamage );
...
(
	...
	vector				HurtOrigin,
	bool				bFullDamage,
	optional class<Weapon>		DamageCauser
)
{
...
simulated function TakeRadiusDamageEx
(
	Controller			InstigatedBy,
	float				BaseDamage,
	float				DamageRadius,
...
)
{
	...
	if ( bFullDamage || dist <= FullDamageMinRadius )
	{
		damageScale = 1;
	}
	else
	{
		dist = FClamp(dist - ColRadius, 0.f, DamageRadius);
		damageScale = 1 - (dist-FullDamageMinRadius)/DamageRadius;
	}
...
event SetAnimPosition(name SlotName, int ChannelIndex, name InAnimSeqName, float InPosition, bool bFireNotifies);
 ...
 */
native final simulated function bool IsPlayerOwned();
 ...
 * @param Thickness (optional) decal thickness; if unspecified or not greater than 0, uses the default in DecalComponent
 * @param bNoClip (optional) if true, use the bNoClip code path for decal generation (requires DecalMaterial to have clamped texture coordinates)
 * @param LifeSpan (optional) if specified and non-zero, the decal will be destroyed after this many seconds
 ...
 */
simulated function DecalComponent AddDecal( MaterialInstance DecalMaterial, vector DecalLocation, rotator DecalOrientation,
						...
						optional float DepthBias = -0.0001
						)
{
 ...
 * @return the created DecalComponent
 */
simulated function DecalComponent AVA_AddDecal( MaterialInstance DecalMaterial, vector DecalLocation, rotator DecalOrientation,
						float DecalRotation, float Width, float Height, float NearPlane, float FarPlane,
						bool bNoClip,
						PrimitiveComponent HitComponent, DecalLifetimeData LifetimeData,
						bool bProjectOnTerrain, bool bProjectOnSkeletalMeshes, name HitBone,
						int HitNodeIndex, int HitLevelIndex,
						...
						)
{
	...
	if (WorldInfo.NetMode != NM_DedicatedServer && !bDeleteMe)
	{
		...
		Decal.Width = Width;
		Decal.Height = Height;
		Decal.Thickness = FarPlane - NearPlane;
		Decal.FarPlane = FarPlane;
		Decal.NearPlane = NearPlane;
		Decal.bNoClip = bNoClip;
		Decal.HitComponent = HitComponent;
		Decal.HitBone = HitBone;
		Decal.HitNodeIndex = HitNodeIndex;
 ...
 * @param ContactInfos information on the contact points
...
defaultproperties
{
	...
	hmIndex = -1; // 20061204 dEAthcURe|HM
}
