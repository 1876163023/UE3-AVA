...
class UIComboBox extends UIObject
	native(UIPrivate)
	placeable
`if(`isdefined(requires_unique_list_datasource))
...
/*
	...
	General todo:
		- [DONE] need to override UpdateDockingSet and ResolveFacePosition for UIComboBox; needs to ensure that the top & bottom faces of the button are added before
			its undocked side so that it can be constrained to a square.

		- [N/A] pretty sure that since the UIList has its own data binding, I won't need a separate property to store the data binding for the list
			but this may break assumptions in the UI editor, so need to validate that.  If so, uncomment the `define requires_unique_list_datasource line above
			to enable all the necessary code.
		...
		- [DONE] need to intercept and override the list's default processing for when an item is selected.
			-- close the list
			-- set the editbox's text to the selected item's text
		...
		- [DONE] need to sync the editbox's text with the list's selected item

		- [DONE] LoseFocus(), if Sender == list, hidelist
*/
cpptext
{
...
/**
 * The data source that this combo box's list will link to
 */
`if(`isdefined(requires_unique_list_datasource))
var(Data)	editconst	const	private		UIDataStoreBinding	ListDataSource;
`endif
...
/**
 * 콤보 리스트를 선택했을때 선택된 콤보리스트의 값을 바로 콤보 에디트박스에 넣는것이 아니라
 * 콤보 리스트의 선택 아이템 인덱스를 가지고 SetFieldValue를 한 뒤에 콤보 에디트 박스를 RefreshBinding Value 한다.
 * 옵션 > 비디오 항목에서 위와 같은 동작을 자주 사용한다. 
 * 예를들어 블룸을 바꾸면 바뀐 블룸값을 적용(SetListIndex)하고 현재의 블룸상태를 반영(RefreshBindingValue)해야한다.
 */
var()										bool				bSetFieldValueOnSubmit;

/**
 * 콤보 박스를 리프레쉬 할때마다 콤보에디트 박스에 있는 내역을 기준으로 콤보리스트의 항목을 선택한다
 */
var()										bool				bSelectListWhenRefresh;

/**
 * 
 */
var()										array<UIComboBox>	NextDependencies;
...
/** bSetFieldValueOnSubmit 의 기능 구현 */
native function bool SetFieldValueOnSubmit(  );
...
function SetEditboxText( string NewText, int PlayerIndex, optional bool bListItemsOnly, optional bool bSkipNotification )
{
	...
	if ( ComboEditbox != None )
	{
		if ( (bListItemsOnly || ComboEditbox.bReadOnly) && ComboList != None )
		{
...
function EditboxPressed( UIScreenObject EventObject, int PlayerIndex )
{
	...
	if ( ComboList != None && ComboList.IsHidden() && ComboEditbox.bReadOnly )
	{
...
function SelectedItemChanged( UIObject Sender, int PlayerIndex )
{
	...
	if ( ComboEditbox != None && Sender == ComboList && ComboList != None )
	{
		...
		SetEditboxText(SelectedItemText, PlayerIndex, true);
	}
}
...
{
	Local UIComboBox NextComboBox;

	`log(`location@`showobj(Sender));
	if ( ComboEditbox != None )
	{
		if( bSetFieldValueOnSubmit )
		{
			if( SetFieldValueOnSubmit() )
			{
				ComboEditBox.RefreshSubscriberValue();
	...
	/** 다음차례로 연결된 ComboBox가 있다면 열어준다 */
	foreach NextDependencies( NextComboBox )
	{
		if( NextComboBox != none )
		{
			`Log("NextComboBox = "$NextComboBox);
			NextComboBox.RefreshSubscriberValue();
			NextComboBox.ComboEditBox.RefreshSubscriberValue();
			NextComboBox.ComboList.RefreshSubscriberValue();
			NextComboBox.ShowList();
		}		
	}
}
...
DefaultProperties
{
	...
	// Data
`if(`isdefined(requires_unique_list_datasource))
	ListDataSource=(RequiredFieldType=DATATYPE_Collection)
`endif
	CaptionDataSource=(RequiredFieldType=DATATYPE_Property)
//
