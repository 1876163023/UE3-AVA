...
// [2006/09/28 , YTS] 
var(Text)									bool					bShowIMECandidateWindow;

var(Text)									bool					bDisableCopyAndPaste;
...
var(Text)									bool					bLimitTextInRenderBound;

/**
 * Label for displaying an IME Candidate String
 */
var()	editinline	const	noclear		UILabel					CandidateLabel;

/**
 * Label for displaying an IME Reading String ( CHS, CHT only )
 */
var()	editinline	const	noclear		UILabel					ReadingLabel;

/**
 * Label for indicating the current IME Language.
 */
var()	editinline	const	noclear		UILabel					IndicatorLabel;

/**
 * Button for Page Up
 */
var()	editinline	const	noclear		UINumericEditboxButton	PageUpButton;

/**
 * Button for PageDown
 */
var()	editinline	const	noclear		UINumericEditboxButton	PageDownButton;
...
public:
	...
	/**	@brief - Notify the style changed
	 *
	 *	Epic 150030에서 삭제된것을 다시 끌어옴.
	 *	이게 없으면 Caret의 StyleData가 초기화가 안되며 StyleData가 초기화 안되면 Caret Rendering을 안하고 지나감
	 *	OnStyleResolved > ApplyCaretStyle > SetImageStyle > InitializeStyleDataContainer(이렇게 StyleData가 초기화됨)
	 *
...
public:
	...
	// Storing an IME Candidate Info to prepare rendering the Candidate window.
	virtual UBOOL ProcessInputCandidate( INT PlayerIndex, const FInputCandidateStringData& CandStrData );

	// ReadingString for CHT, CHS
...
DefaultProperties
{
	...
	DataSource=(MarkupString="Editbox Text",RequiredFieldType=DATATYPE_Property)
	//bSupportsPrimaryStyle=false
	bSupportsPrimaryStyle=true		// StyleData을 사용해야만 CaretRendering이 가능
	...
	Begin Object Class=UIComp_DrawStringEditbox Name=EditboxStringRenderer
		bIgnoreMarkup=true
		StringCaret=(bDisplayCaret=false)
	End Object
	...
	// [2006/09/28, YTS] Show an IME Candidate Window by default
	bShowIMECandidateWindow=true
}
