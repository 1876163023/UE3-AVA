...
public:
	 ...
	 *
	 * @param	ContainerDataStore	the root datastore serve as a initial position
	...
	/**
	 * 마크업 스트링을 쪼개서 데이터 프로바이더를 제외한 필드 이름을 얻어낸다. 밑의 예에서 ActiveItems에 해당
	 *
	 * @param MarkupString		데이터 필드를 뽑아낼 마크업 스트링. 예) <avaNet:CashItems.Inventory.ActiveItems>
	 *
	 * @return	NAME_None		데이터 필드를 얻어내는데 실패
	 */
	FName ParseDataField( const FString& MarkupString ) const;
	/**
...
protected:
	/**
	 * InternalProvider들을 초기화(생성)
	 */
	void InitializeInternalDataProviders();

	/**
	 * UIEditor의 F8시 나오는 "DataStore Browser"에 InternalProvider가 지원하는 Field들을 등록
	 */
	void GetInternalDataProviderSupportedDataFields(TArray<FUIDataProviderField>& out_Fields);
}
...
/** 내부 데이터 프로바이더를 자동으로 생성해 에디터에서 보여주기 위한 정보 */
struct native InternalProviderInfo
{
	var class<UIDataProvider>	ProviderClass;
	var UIDataProvider			ProviderInstance;
	var name					ProviderTag;
};

/** 내부 데이터 프로바이더 정보묶음( 프로바이더 클래스, 인스턴트, 프로바이더 추가 ) */
var array<InternalProviderInfo>	InternalProviderData;
