...
simulated function DumpWeaponDebugToLog()
{
	...
	for (i=0;i<DebugInfo.Length;i++)
	{
		`log("Weapondebuginfo "$ DebugInfo[i] );
	}
}
...
{
	`log( "[" $ WorldInfo.TimeSeconds $"]" @ Msg @ "(" $ FuncStr $ ")" );
}
...
simulated function float MaxRange()
{
	...
	for (i = 0; i < WeaponProjectiles.length; i++)
	{
		...
		{
			Range = FMax(Range, WeaponProjectiles[i].static.GetRange());
		}
	}
...
simulated function WeaponPlaySound( SoundCue Sound, optional float NoiseLoudness, optional bool bNotReplicated, optional bool bRepToOwner, optional bool bNoDup )
{
	...
	if (bNoDup)
		Instigator.PlaySound(Sound, bNotReplicated, true, ,,, AudioChannel_Weapon);
	else
		Instigator.PlaySound(Sound, bNotReplicated, true, ,,, AudioChannel_Auto);
}
...
//{{ Foreground FOV
//!{ 2006. 3. 6		허 창 민
/** Foreground Fov 조정. 이 함수는 subclass 에서 override 되어야 한다. */
simulated function float AdjustForegroundFOVAngle(float FOVAngle)
{
	return FOVAngle;	// 기본은 아무것도 하지 않는다.
}
//!} 2006. 3. 6		허 창 민
//}}
...
simulated function ImpactInfo CalcWeaponFire(vector StartTrace, vector EndTrace, optional out array<ImpactInfo> ImpactList)
{
	...
	if( HitActor != None )
	{
		if( HitActor.IsA('Trigger') || HitActor.IsA('TriggerVolume') )
		{
