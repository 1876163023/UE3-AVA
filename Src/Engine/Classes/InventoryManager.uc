...
var class<weapon> pendingWeaponClass; // [+] 20070213 dEAthcURe|HM
 ...
 * Network: Server only
 */
simulated function SetupFor(Pawn P)
{
	Instigator = P;
	SetOwner(P);
}
...
simulated function Inventory CreateInventory(class<Inventory> NewInventoryItemClass, optional bool bDoNotActivate)
{
	...
	//`log("[dEAthcURe|CreateInventory]" @ NewInventoryItemClass @ bDoNotActivate);
...
simulated function bool AddInventory(Inventory NewItem, optional bool bDoNotActivate)
{
	...
	//`log("[dEAthcURe|AddInventory] " @ NewItem @ bDoNotActivate);	
	...
	if( (NewItem != None) && !NewItem.bDeleteMe )
	{
		...
		NewItem.SetInventoryManager( Self );
		NewItem.GivenTo( Instigator, bDoNotActivate);
		
		//`log("[dEAthcURe|AddInventory] NewItem.GivenTo " @ Instigator @ bDoNotActivate);
...
simulated function RemoveFromInventory(Inventory ItemToRemove)
{
	...
	if( ItemToRemove != None )
	{
		...
		//{
		//	Log( "avaInventoryManager.RemoveFromInventory Call Controller.ClientSwitchToBestWeapon" );
 ...
 */
function int ModifyDamage( int Damage, Controller instigatedBy, class<DamageType> DamageType )
{
	return Damage;
}
...
simulated function ChangedWeapon()
{
	...
	if( PendingWeapon != None )
	{
		...
		{
			Instigator.MakeNoise(0.1 * WorldInfo.Game.GameDifficulty);
		}
...
reliable client function ClientSetWeapon( class<Weapon> WeaponClass )
{
    ...
    //`log("[dEAthcURe|InventoryManager::ClientSetWeapon] WeaponClass=" @ WeaponClass);		

	Inv = FindInventoryType( WeaponClass, true);
	if ( Weapon(Inv) != None ) {
		//`log("[dEAthcURe|InventoryManager::ClientSetWeapon] -- before SetCurrentWeapon" @ Weapon(Inv) @ Instigator.Weapon);		
		//Instigator.Weapon = None; // [!] 기본무기를 안들고나온다 // test
		SetCurrentWeapon( Weapon(Inv), true);
	}
	else {
		pendingWeaponClass = WeaponClass;
		//`log("[dEAthcURe|InventoryManager::ClientSetWeapon] -- No proper inventory found" @ WeaponClass);
	}
}
// }} 20070213 dEAthcURe|HM

// {{ 20070309 dEAthcURe|HM
event function HmSwitchToBestWeapon()
{
	SwitchToBestWeapon();
}
// }} 20070309 dEAthcURe|HM
...
defaultproperties
{
	...
	pendingWeaponClass = None; // 20070213 dEAthcURe|HM
}
