...
//! 해상도에 따라서 폰트 크기를 일정하게 유지시켜 주기 위한 변수 추가(2007/01/10 고광록).
var(Font) bool										bFontRatioScaling;	//!< 폰트의 해상도 비율 스케일링 유무.
var(Font) float										FontScreenWidth;	//!< 폰트의 기준이 되는 해상도 가로.
var(Font) float										FontScreenHeight;	//!< 폰트의 기준이 되는 해상도 세로.
 ...
 */
var(Presentation) editconst	interp	UIDockingSet				DockTargets;
...
var(Presentation) interp	UIRotation							Rotation;

/** Screenspace offset to apply to the widget's rendering. */
var(Presentation) interp	vector								RenderOffset;

/** 확대/축소의 기준축 */
var(Presentation) interp	vector								ScaleAxis;

/** 확대/축소의 값 1.f일때 원래와 같은 크기 */
var(Presentation) Interp	vector								ScaleValue;
...
/** 오버라이드 할 툴팁 (씬내에 미리 배치해놓고 에디터에서 할당해준다) */
var(Data)	const				UIToolTip						ToolTipToOverride;
...
/** Animations for a specified event */
var(Animation)		name							ShownAnim;
var(Animation)		name							HiddenAnim;
var(Animation)		name							DisabledAnim;
var(Animation)		name							EnabledAnim;
 ...
 * @param	PlayerIndex	Player that performed the action that issued the event.
 */
delegate bool OnDoubleClick( UIScreenObject EventObject, int PlayerIndex );
 ...
 */
native function bool SetDockTarget( EUIWidgetFace SourceFace, UIScreenObject Target, EUIWidgetFace TargetFace, optional float Padding );
 ...
 **/
native final function GetUIComponents( out array<UIComponent> out_Components, bool bRecursive = false);
...
/** private RenderBounds의 GetMethod */
final function float GetRenderBoundValue( EUIWidgetFace Face )
{
	return RenderBounds[Face];
}
...
native function AnimSetScale(Vector NewScale);
						...
						optional float PlaybackRate=1.0,optional bool bLoop, optional float InitialPosition=0.0)
{
	...
	local int Idx;
	Local float SeqDuration;

	SceneClient = GetScene().SceneClient;
	if ( SceneClient == none )
	{
		return;
	}
	...
	if ( AnimName != '' )
	{
		for (Idx=0;Idx<AnimStack.Length;Idx++)
		{
			if ( AnimStack[Idx].SeqRef.SeqName == AnimName )
			{
				...
				SeqDuration = AnimStack[Idx].SeqRef.InTimeMax - AnimStack[Idx].SeqRef.InTimeMin;
				AnimStack[Idx].AnimTime = SeqDuration * InitialPosition;
...
*/
				SceneClient.AnimSubscribe(self);
				return;
			}
		}
	}
	...
	{
		TargetAnimSeq = SceneClient.AnimLookupSequence(AnimName);
	}

	`log("TargetAnimSeq = "$TargetAnimSeq$", Track.Length = "$TargetAnimSeq.Tracks.Length);
				...
				(AnimSeq != none && AnimStack[Idx].SeqRef == AnimSeq) )
		{
			if (bFinalize)
			{
				...
				for (i=0; i<Seq.Tracks.Length; i++)
				{
					...
					Seq.ApplyAnimation(TargetWidget, i, 1.0, AnimStack[Idx]);
				}
			}
			UIAnimEnd(Idx);
		}
...
event UIAnimEnd(int SeqIndex)
{
	...
	{
		// Unsubscribe
		GetScene().SceneClient.AnimUnSubscribe( self );
	}
}
...
/**
 * DoubleClick을 입력받거나 입력받지 않도록 Delegate를 설정
 */
event SetDblClickEnabled( bool bEnable )
{
	if( bEnable )
	{
		OnDoubleClick = none;	// @TODO : restore the previous doubleclick-delegate
	}
	else
	{
		OnDoubleClick = DoubleClick_Disable;
	}
}

function bool DoubleClick_Disable( UIScreenObject EventObject, int EvtParmPlayerIdx )
{
	return true;
}
...
//	BoundsAdjustment=(XPlane=(X=1,Y=0,Z=0,W=0),YPlane=(X=0,Y=1,Z=0,W=0),ZPlane=(X=0,Y=0,Z=1,W=0),WPlane=(X=0,Y=0,Z=0,W=1))
	...
	//! 해상도에 따라 폰트 스케일링 변수 초기화(2007/01/11 고광록).
	bFontRatioScaling	= false;	// 일단 적용 안되게 막았다.
	FontScreenWidth		= 1024.0;
	FontScreenHeight	= 768.0;

	ScaleAxis=(X=0.5,Y=0.5)
	ScaleValue=(X=1.0,Y=1.0,Z=1.0)
}