...
cpptext
{
	 ...
	 */
	virtual INT InsertElement( INT InsertIndex, INT ElementToInsert );
	...
	/**
	 * 셀바운드를 다시 계산할 필요가 있음 ( TopIndex가 바뀌었거나 셀크기가 변했거나 등등 리스트의 크기는 변하지 않았는데 보여주는 셀은 변했을때 )
	 */
	virtual void RecalcCellBounds();
	 ...
	 *
	 * @param	ColumnIndex		Alignment를 변경할 Column 순서
	 * @param	Orientation		Horizontal / Vertical
	 * @param	Alignment		Left / Center / Right | Top / Center / Bottom
	 *
	 */
	void SetColumnAlignment( INT ColumnIndex, EUIOrientation Orientation , EUIAlignment Alignment);

	/**
	 * 스키마를 포함한 열데이터의 모든 ClipMode 값을 변경한다.
	 *
	 * @param	ColumnIndex		ClipMode를 변경할 Column 순서
	 * @param	ClipMode		CLIP_None / Clip_Ellipse / Clip_Wrapped
	 */
	void SetColumnClipMode( INT ColumnIndex, ETextClipMode ClipMode ); 
...
protected:
	 ...
	 */
	virtual void UpdateCellBounds( struct FRenderParameters& Parameters );

	/**
	 * Creating & Positioning the internal-widget's cell-bounds according to the current list setting.
	 */
	virtual void UpdateInternalUIObject( struct FRenderParameters& Parameters );

	/**
	 *
	 * @param Cell				Bounds를 계산해 저장해둘 Cell
	 * @param ElementIndex		INDEX_NONE : bCellTemplate, another : index of ListItems
	 * @param Parameters		Bounds가 넘을 수 없는 최소 범위를 지정
	 *
	 * @return					드로잉 영역을 넘어섰는지의 여부 (TRUE 넘지 않았음 / FALSE  넘었음)
	 */
	virtual UBOOL CalculateCellBounds( INT ElementIndex, INT CellIndex, FRenderParameters& Parameters );
	...
	*/
	virtual void Render_Cells( FCanvas *Canvas, const TArray<struct FUIListElementCell*> Cells, UBOOL bCellTemplate = FALSE, UBOOL bForceDraw = FALSE);
...
struct native UIListElementCell
{
	...
	var(Presentation)	editconst	transient EUIListElementState	CellState;

	/** Text Alignment 리스트 안에서 셀마다 정렬을 다르게 할 수 있도록 */
	var(Presentation)		EUIAlignment				Alignment[EUIOrientation.UIORIENT_MAX];

	/** Clip Mode 변경할 수 있도록 추가 ( ClipMode는 CLIP_None과 CLIP_Normal둘중에 하나를 쓸것으로 보임 ) */
	var(Presentation)		ETextClipMode				ClipMode;
	...
	/** 셀의 경계 정보 */
	var		transient		float						Bounds[EUIWidgetFace.UIFACE_MAX];
...
structcpptext
{
	 ...
	 */
	void SetCellState( EUIListElementState NewCellState );
...
structdefaultproperties
{
	...
	CellStyle(ELEMENT_Disabled)=(RequiredStyleClass=class'Engine.UIStyle_Combo')
...
struct native UIListElementCellTemplate extends UIListElementCell
{
	...
	/**
	 * CellTemplate(Header)에 해당하는 배경은 따로 있어야한다. 
	 * ( 항목을 누를때 각기 달리 반응해야하기 때문에 , ex) '방이름'을 누름 ->  방이름 항목만 배경이 바뀌고 다른 항목은 안바뀜")
	 */
	var		transient				UITexture			Background;

	/**
	 * 리스트에 항목을 그릴 우선순위. 높은 값을 가지는 것을 먼저 그린다
	 */
	var()					const	int					DrawingPriority;
...
structcpptext
{
	 ...
	 *
	 * @param	ResolvedStyle			the style resolved by the style reference
	 * @param	DesiredState			지정된 스테이트로 헤더를 할당하도록 한다. NULL일시 원래대로 현재 스테이트에 해당하는 스타일을 적용
	 */
	void ApplyHeaderStyleData( UUIStyle* ResolvedStyle , UClass* DesiredState = NULL);
}
};
...
struct native UIListItem
{
	...
	/** LINKED_Column에서는 행하나, LINKED_Rows에서는 열 하나에 해당하는 경계 정보 */
	var		transient				UIListElementCell						ElementCell;
...
struct native UIElementCellSchema
{
	...
	/** bDrawVisibleItems 일 때 리스트 빈 항목을 그려야하게되면 사용할 셀 정보 */
	var()	transient init	array<UIListElementCell>			EmptyCells;
 ...
 */
// const 주석처리 : ElementSchema내의 ColumnHeaderText::FStringNoInit을 초기화하기위해서는 스크립트내에서 접근이 가능해야한다.
 ...
 */
var()	editconst	editinline	transient	init array<UIListItem>	ListItems;
...
var(Image)	deprecated instanced	editinlineuse	UITexture			ColumnHeaderBackground;
...
/** 각 셀들의 바운드를 새로 계산해야하는지 여부 ( TopIndex가 바뀌었거나, 셀의 크기가 변경되었거나 ) */
var			transient					bool				bRecalcCellBounds;


struct native InternalUIObjectItem
{
	var()	editinline					UIObject			Archetype;
	var()	transient					array<UIObject>		Instances;
	var()	transient					bool				bRefresh;
	var()	transient					bool				bRemove;
};

/** Internal Buttons(archetype)*/
var()									array<InternalUIObjectItem>	InternalUIObjects;
...
/**
  ...
  */
native final function RefreshInternalUIObjectSubscriberValue( INT BindingIndex = INDEX_NONE );

/**
 * UIListelementCellTemplate의 맴버인 ColumnHeaderText::FStringNoInit을 초기화해주기 위함.
 */
event ConstructElementSchemaCell( int InsertIndex )
{
	if( InsertIndex >= ElementSchema.Cells.Length )
	{
		ElementSchema.Cells.Add( InsertIndex - (ElementSchema.Cells.Length - 1) );
		ElementSchema.Cells[InsertIndex].ColumnHeaderText = "";
	}
	else
	{
		ElementSchema.Cells.Insert(InsertIndex, 1);
		ElementSchema.Cells[InsertIndex].ColumnHeaderText = "";
	}
}
...
DefaultProperties
{
	...
	End Object
	Begin Object Class=UITexture Name=DisabledOverlayTemplate
	End Object
	...
	ListItemOverlay(ELEMENT_Disabled)=DisabledOverlayTemplate

	Begin Object Class=UITexture Name=ColumnHeaderBackgroundTemplate
	End Object
	ColumnHeaderBackground=ColumnHeaderBackgroundTemplate
}
