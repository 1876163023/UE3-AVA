...
// IUIListElementProvider interface
	 ...
	 *
	 * @param FieldName	the tag of the list needed
	 *
 ...
 */
event bool RefreshStats(byte ConrollerIndex)
{
	...
	switch (CurrentReadType)
	{
		case SFT_Player:
			...
			PlayerInterface.GetUniquePlayerId(ConrollerIndex,PlayerId);
			Players[0] = PlayerId;
			if (StatsInterface.ReadOnlineStats(Players,StatsRead) == false)
			{
				// Clear the delegate that tells the UI to refresh
				StatsInterface.ClearReadOnlineStatsCompleteDelegate(OnReadComplete);
				return false;
			}
			return true;

		case SFT_CenteredOnPlayer:
			if (StatsInterface.ReadOnlineStatsByRankAroundPlayer(ConrollerIndex,StatsRead,98) == false)
			{
				// Clear the delegate that tells the UI to refresh
				StatsInterface.ClearReadOnlineStatsCompleteDelegate(OnReadComplete);
				return false;
		...
		case SFT_Friends:
			if (StatsInterface.ReadOnlineStatsForFriends(ConrollerIndex,StatsRead) == false)
			{
				// Clear the delegate that tells the UI to refresh
				StatsInterface.ClearReadOnlineStatsCompleteDelegate(OnReadComplete);
				return false;
