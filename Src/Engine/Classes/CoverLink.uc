...
cpptext
{
	...
	void EditorAutoSetup(FVector Direction);
};
...
final event UnClaim( Controller OldClaim, int SlotIdx, bool bUnclaimAll )
{
...
/** Removes claim to specific slot */
final event UnClaimSlot( int SlotIdx )
{
`if (`notdefined(FINAL_RELEASE))
	if( bDebug )
	{
		//debug
		`log( self@"UnClaimSlot"@SlotIdx@Slots[SlotIdx].SlotOwner );
