...
/** Scene이 열릴때 페이드 효과를 넣기 위함 */
var UIScene		latestOpendScene;
var float		OpendSceneTime;
var float		OpendSceneDefaultOpacity;

// GameUISceneClient에서 여기로 이전. 에디터(EditorUISceneClient)에서도 AnimSubscribers를 사용한다
...
/** AnimSequenceName for DynamicLoadObject. ex) avaUIAnimSeq.BlinkThreeTimes */
var config array<string> AnimSequenceNameStrings;
 ...
 * @note - noexport to handle the optional out parameter correctly
 */
native final noexport function bool OpenScene( UIScene Scene, optional LocalPlayer SceneOwner, optional out UIScene OpenedScene, optional byte Priority = 1 /* EScenePrior_UIScene_Normal */);
...
event InitializeSceneClient( UISkin InitialSkin )
{
	RefreshAnimSequencePool();
}

function RefreshAnimSequencePool()
{
	Local string AnimSeqName;
	Local UIAnimationSeq AnimSeq;

	foreach AnimSequenceNameStrings(AnimSeqName)
	{
		AnimSeq = UIAnimationSeq(DynamicLoadObject(AnimSeqName, class'UIAnimationSeq'));
		if( AnimSeq != none )
		{
			AnimSequencePool.AddItem(AnimSeq);
		}
	}
