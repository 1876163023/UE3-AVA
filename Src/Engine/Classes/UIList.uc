...
cpptext
{
	 ...
	 * @param	out_bOverride	메뉴스테이트가 원래 위젯의 것인지 (FALSE), 데이터 프로바이더에서 덮어씌운것인지(TRUE) 알려준다
	 *
	 ...
	 */
	UUIState* GetElementMenuState( INT ElementIndex, UBOOL& out_bOverride );
	...
	// */
	//virtual UBOOL CalculateCellFromPosition( const FIntPoint& HitLocation, struct FCellHitDetectionInfo& out_HitInfo );
...
public:
	 ...
	 * @param	PlayerIndex		the index of the player associated with this index change.
	 */
	void NotifyActiveIndexChanged( INT PreviousIndex, INT PlayerIndex );

	/**
	 * Called when the list's top item has changed
	 *
	 * @param	PreviousTopIndex	the list's TopIndex before it was changed
	 ...
	 */
	// migrate to the native function has same name.
...
public:
	...
	UBOOL SetActiveElement( INT NewIndex, UBOOL bClampValue );
 ...
 */
var(List)					UIScreenValue				RowHeight;

/**
 *	헤더(항목표시줄)의 크기를 지정.
 *	값이 0이면 GetRowHeight()값을 반환한다.
 */
var(List)					UIScreenValue				RowHeaderHeight;
 ...
 */
var(List)					UIScreenValue				ColumnWidth;

/**
 *	헤더(항목표시줄)의 크기를 지정.
 *	값이 0이면 GetColumnWidth()값을 반환한다.
 */
var(List)					UIScreenValue				ColumnHeaderWidth;

/**
 * Number of pixels between each cell in the list.
 */
var(List)					UIScreenValue				CellSpacing;

/**
 * Number of pixels to pad the inside of each cell.
 */
var(List)					UIScreenValue				CellPadding;
 ...
 */
var(List) editconst	transient int						MaxVisibleItems;
...
/** 리스트내에 없는 아이템들을 보여줄지말지 결정. 
 *	예를들어 10칸을 보여주는 리스트에서  5칸의 내용물이 들어가 있고 5칸이 비어있다면
 *		bDrawVisibleItems == true일때는 10칸 모두 화면에 출력하나
 *		bDrawVisibleItems == false 일때는 5칸만 화면에 출력하고 나머지는 그리지 않는다.
 */
var(List)	bool										bDrawVisibleItems;

/**
 * 셀 단위로 배경을 그릴지 결정한다. 
 * 셀단위로 드로잉하지 않으면 한 ListElement단위로 그린다. (기존의 드로잉은 ListElement 드로잉)
 */
var(List)	bool										bCellUnitDrawing;
 ...
 */
var(Data)					ECellLinkType				CellLinkType;
 ...
 */
var(List)					EListWrapBehavior			WrapType;
...
var(List)					bool						bNoSelectWhenRefresh;
...
/** 선택된 항목을 배경외에 추가로 표시 ( UIComp_DrawComponents의 페이드, 스케일 부분추가 ) */
var	editinline	const	noclear	UIComp_DrawImage		SelectionOverlayComponent;
 ...
 */
var(Data)	transient		bool						bDisplayDataBindings;
...
/**
 * The zero-based indexes of all currently selected items in the list.  These values are indexes into the UIList's Items array.
 *
 * @todo - not yet implemented
 */
var	transient 	public{private}	array<int>				SelectedIndices;
...
/** 이소리는 각 리스트 엘리먼트가 Active상태로 변할때 납니다*/
var(Sound)						name					ActiveIndexChangedCue;
/** 이 소리는 리스트의 해당 엘리먼트에 클릭했을때 납니다 */
var(Sound)						name					ClickCue;
/** 이 소리는 리스트의 해당 엘리먼트에 더블클릭했을때 납니다 */
var(Sound)						name					DoubleClickCue;

// ===============================================
// DoubleClick Event 2006/12/14 윤태식
// 
// InGame중에는 IE_DoubleClick이 들어오지 않는다.
// WinClient에서 억지로 WM_*BUTTONDBLCLK이 들어오도록하면 게임이 망가진다. (총이 연사되었다 안되었다)
// 따라서 List에서 더블클릭 이벤트를 받으려면 자체적인 DoubleClick 검사 방법이 필요하다.
// ===============================================
var(InputEvent)					float					DoubleClickDuration;
var								transient float			MousePressUpdateTime;
var								transient bool			bWaitSecondClick;
var								int						PrvSelectedIndex;	// 이전 클릭에서 선택한 List Index

//! Enable/Disable처리를 입력처리 유무로 처리하려고 추가함.(2007/02/15 고광록)
var(InputEvent)					bool					bProcessInput;

var(List)						bool					bNotifyAlways;		// Index 가 바뀌지 않더라도 Notify를 받는다.

var								transient bool			bAlwaysSkipNotification;	//! 내부적으로 사용하는 변수.
 ...
 */
native function float GetRowHeight( optional int RowIndex=INDEX_NONE ) const;

/**
 * 헤더의 크기 (LinkedType_Row일때는 헤더 높이, LinkedType_Column일때는 헤더 너비)를 반환
 */
native function float GetHeaderSize() const;
 ...
 * @return	TRUE if HitLocation was located inside this list.
 */
native final virtual function bool CalculateCellFromPosition( IntPoint HitLocation, out CellHitDetectionInfo out_HitInfo ) const;
 ...
 * the OnSubmitSelection delegate.
 */
native final virtual function NotifySubmitSelection( INT PlayerIndex );
...
protected final function OnSetListIndex( UIAction_SetListIndex Action )
{
	...
	if ( Action != None )
	{
		...
		if ( Action.TopIndex != -1 )
			SetTopIndex( Action.TopIndex );

		if ( Action.bSubmitSelection )
		{
			NotifySubmitSelection(GetBestPlayerIndex());
		}
...
/** List를 ListItem(CorrectionIndex)을 통해서 선택하고자함*/
protected final function OnSetListIndexByItem( UIAction_SetListIndexByItem Action )
{
	local int OutputLinkIndex;
	...
	if ( Action != None )
	{
		...
		if ( ListIndex == INDEX_NONE || !SetIndex( ListIndex, Action.bClampInvalidValues, !Action.bActivateListChangeEvent) )
		{
			// 1 is the index of the "Failed" output link
			OutputLinkIndex = 1;
		}
...
/** MarkupString을 UI Kismet에서 변경하기위함 ( avaUISceneEx_WeapInv 에 쓰임) */
function OnSetLabelText( UIAction_SetLabelText Action )
{
	if( DataSource.MarkUpString != Action.NewText )
	{
		DataSource.MarkUpString = Action.NewText;
		RefreshSubscriberValue();
	}
}

/*! @brief Kismet Action Handler.
	@note
		UICheckbox.uc참고.
*/
function OnSetBoolValue( UIAction_SetBoolValue Action )
{
	`log("UIList::OnSetBoolValue(" @Action.bNewValue @")" );
	bProcessInput = Action.bNewValue;
}
...
/*! @brief Kismet Action Handler.
	@note
		UICheckbox.uc참고.
*/
function OnGetBoolValue( UIAction_GetBoolValue Action )
{
	Action.bNewValue = bProcessInput;
}

/** 마우스 좌표로부터 CellIndex(Row, Column)를 얻어옴 */
function OnGetCellIndexFromPos( UIAction_GetCellIndexFromPos Action )
{
	Local CellHitDetectionInfo HitInfo;
	Local UIScene Scene;
	Local UISceneClient SceneClient;
	Local bool bFindResult;
	Local int ListIndex;

	Action.RowIndex = INDEX_NONE;
	Action.ColIndex = INDEX_NONE;
	Action.CollectionIndex = INDEX_NONE;

	Scene = GetScene();
	if( Scene != None)
		SceneClient = Scene.SceneClient;

	if( SceneClient != None)
	{
		bFindResult = CalculateCellFromPosition( SceneClient.MousePosition, HitInfo );
		
		if( bFindResult )
		{
			Action.RowIndex = HitInfo.HitRow == INDEX_NONE ? HitInfo.ResizeRow : HitInfo.HitRow ;
			Action.ColIndex = HitInfo.HitColumn == INDEX_NONE ? HitInfo.ResizeColumn : HitInfo.HitColumn;

			if( Action.bAbsoluteIndex )
			{
				switch( CellLinkType )
				{
				case LINKED_Rows:		ListIndex = TopIndex + Hitinfo.HitColumn;
										Action.ColIndex += TopIndex;	break;
				case LINKED_Columns:	ListIndex = TopIndex + HitInfo.HitRow;
										Action.RowIndex += TopIndex;	break;
				case LINKED_None:		ListIndex = HitInfo.HitColumn + (HitInfo.HitRow * GetMaxNumVisibleColumns());
										Action.ColIndex += TopIndex;	break;
				}
			}

			if( ListIndex < Items.Length )
				Action.CollectionIndex = Items[ListIndex];
		}
...
/** CellIndex(Row, Column)으로부터 Position(Left,Right,Top,Bottom)을 읽어옴 */
function OnGetCellPosFromIndex( UIAction_GetCellPosFromIndex Action )
{
	Local float Width, Height;
	Local int RowNum, ColNum, i;
	
	Height = GetRowHeight(Action.RowIndex);
	Width = GetColumnWidth(Action.ColIndex);
	RowNum = GetMaxNumVisibleRows();
	ColNum = GetMaxNumVisibleColumns();

	Action.ViewportLeft = GetRenderBoundValue(UIFACE_Left);
	Action.ViewportTop = GetRenderBoundValue(UIFACE_Top);

	for( i = 0 ; i < Min(Action.RowIndex, RowNum) ; i++ )
		Action.ViewportTop += GetRowHeight(i);

	for( i = 0 ; i < Min(Action.ColIndex, ColNum) ; i++ )
		Action.ViewportLeft += GetColumnWidth(i);

	Action.ViewportRight = Width + Action.ViewportLeft;
	Action.ViewportBottom = Height + Action.ViewportTop;
}
...
DefaultProperties
{
	...
	GlobalCellStyle(ELEMENT_Disabled)=(DefaultStyleTag="DefaultCellStyleDisabled",RequiredStyleClass=class'Engine.UIStyle_Combo')
	...
	ItemOverlayStyle(ELEMENT_Disabled)=(DefaultStyleTag="ListItemBackgroundDisabledStyle",RequiredStyleClass=class'Engine.UIStyle_Image')
	...
	RowCount=4
	ColumnCount=1
	// 기존의 패키지에 있던 리스트들이 모두 CELLAUTOSIZE_None이다.
	...
	ActiveIndexChangedCue=ListActiveIndex
	ClickCue=ListClick
	DoubleClickCue=ListDoubleClick
	...
	bAlwaysSkipNotification = false;

	Begin Object class=UIComp_DrawImage Name=SelectionOverlayTemplate
		ImageStyle=(DefaultStyleTag="DefaultImageStyle",RequiredStyleClass=class'Engine.UIStyle_Image')
		StyleResolverTag="Selection Overlay Style"
	End Object
	SelectionOverlayComponent=SelectionOverlayTemplate
}
