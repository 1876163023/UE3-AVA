...
/** If set, this component won't never be drawn */
var bool bForceNotToDraw;

var(Rendering) bool bForceNotToUseCPUSkinInGame;
...
var(Rendering) int LODBias;

/*! @brief 최근에 렌더링이 되었는지 판단할 때 사용되는 시간.(2007/07/13 고광록)
	@note 
		GSkelVisibleTime변수 사용하던 부분을 스크립트로 옮김.
*/
var transient float		SkelVisibleTime;
...
native final function name	GetBoneNameByIndex( int Index );
 ...
 */
function AnimNodeSequence PlayAnim(name AnimName, optional float Duration, optional bool bLoop, optional bool bRestartIfAlreadyPlaying = true)
{
...
//	bDisplayBones=true
	...
	// 기본값은 0.1이다.(필요한 곳에서 고쳐서 사용하면 될 듯 싶다)
	SkelVisibleTime = 0.1
}
