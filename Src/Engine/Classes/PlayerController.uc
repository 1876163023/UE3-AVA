...
//var float RemainingMoveDelta;
var float NetClientAdjustTime; // 20071204 dEAthcURe
var bool bHmForceSpectating; // [+] 20070306 dEAthcURe
var	bool bHmRestoring; // [+] 20070301 dEAthcURe
...
/** used by automatic tonemapping */
var	float			ExposureCenterRegionX, ExposureCenterRegionY;
...
var globalconfig int ReconfigureNetspeedThreshold;
var globalconfig int StableNetspeed;
...
//	bEnableFreeCam 이 True 인 경우에만 FreeCamera 가 가능하다.
var bool bUseFreeCam;

/** 테스트용 Ragdoll flag */
var bool				bDisableRagdoll;
...
cpptext
{
	...
	virtual void HearSound(USoundCue* InSoundCue, AActor* SoundPlayer, const FVector& SoundLocation, UBOOL bStopWhenOwnerDestroyed, EAudioChannel Channel);
	...
	virtual UBOOL CanApplyCullDistance() { return TRUE; }
...
reliable server function ServerShortTimeout()
{
	...
	ClientSetRemainingTime(WorldInfo.GRI, WorldInfo.GRI.RemainingTime);
}

reliable client function ClientSetRemainingTime(GameReplicationInfo GRI, int RemainingTime)
{
	GRI.RemainingTime = RemainingTime;
}
...
function Reset()
{
	...
	// {{ 20070306 dEAthcURe|HM
	bHmForceSpectating = false;
	`log("[dEAthcURe|PlayerController::Reset] HM returns to normal state");
	// }} 20070306 dEAthcURe|HM
}
...
simulated event ReplicatedEvent(name VarName)
{
	...
	if (VarName == 'PlayerReplicationInfo')
	{
		...
		if (ZeroUniqueId == PlayerReplicationInfo.UniqueId)
		{
			InitUniquePlayerId();
		}
	}
...
reliable client function GivePawn(Pawn NewPawn)
{
	if ( NewPawn == None )
		return;
	Pawn = NewPawn;
	//Pawn.bOptimizeVelocityReplication = true; // 20071217 dEAthcURe|ON 
...
simulated function AudioComponent GetPooledAudioComponent(SoundCue ASound, Actor SourceActor, bool bStopWhenOwnerDestroyed, optional bool bUseLocation, optional vector SourceLocation, optional EAudioChannel Channel)
{
	local int i;
	local AudioComponent Result;

	// try to grab one from the pool
	while (HearSoundPoolComponents.length > 0)
	{
		i = HearSoundPoolComponents.length - 1;
	...
	if (Result == None)
	{
		...
		i = 0;
		while (i < HearSoundActiveComponents.length)
		{
			if (HearSoundActiveComponents[i] != None && !HearSoundActiveComponents[i].IsPendingKill())
			{
				i++;
			}
			else
		...
		if (MaxConcurrentHearSounds > 0 && HearSoundActiveComponents.length >= MaxConcurrentHearSounds)
		{
			if (bLogHearSoundOverflow)
			{
				`Warn("Exceeded max concurrent active HearSound() sounds! Sound list:");
				for (i = 0; i < MaxConcurrentHearSounds; i++)
				{
					`Log(PathName(HearSoundActiveComponents[i].SoundCue));
				}
			}
			// overwrite oldest sound
			Result = HearSoundActiveComponents[0];
			Result.OnAudioFinished = None; // so HearSoundFinished() doesn't get called and mess with the arrays
			Result.ResetToDefaults();
			HearSoundActiveComponents.Remove(0, 1);
		}
		else
		{
			Result = CreateAudioComponent(ASound, false,,,, false);
			if (Result == None)
			{
				// sound is disabled
				return None;
	...
	if (SourceActor != None && !SourceActor.IsPendingKill())
	{
		Result.bUseOwnerLocation = !bUseLocation;
		Result.Location = SourceLocation;
		SourceActor.ModifyHearSoundComponent(Result);
	...
	Result.OnAudioFinished = HearSoundFinished;
	return Result;
}
...
reliable client event Kismet_ClientPlaySound(SoundCue ASound, Actor SourceActor, float VolumeMultiplier, float PitchMultiplier, float FadeInTime, bool bSuppressSubtitles, bool bSuppressSpatialization, optional EAudioChannel Channel)
{
	local AudioComponent AC;
	...
	if (SourceActor != None)
	{
		...
		{
			AC = SourceActor.CreateAudioComponent(ASound, false, true,,,, Channel);
			if ( AC != None )
			{
...
unreliable client function ClientFOV(float F)
{
	if( PlayerCamera != None )
	{
		PlayerCamera.BlendTimeToGo	=	0.3f;
		PlayerCamera.BlendLength	=	0.3f;
...
unreliable server function ServerSay( string Msg )
{
	...
	Msg = WorldInfo.Game.StripColor(Msg);
...
)
{
	...
	if (Player.CurrentNetSpeed > ReconfigureNetspeedThreshold)
	{
		`log( "NETSPEED RECONFIGURED "@Player.CurrentNetSpeed @StableNetSpeed );
		SetNetSpeed(StableNetspeed);
		ClientCapBandwidth(StableNetspeed);
	}
...
)
{
	...
	//---- alcor
	//RemainingMoveDelta += DeltaTime;
	//if (RemainingMoveDelta >= 0.03)
	//{
	...
	//			OldMove );
	//	RemainingMoveDelta -= 0.03;
	//}
	//else
	//{
	//	`log("Skipping ServerMove");
	//}
	//---- alcor
...
{
	ClientSetProgressMessage(Index, S, C);
}

reliable client event ClientSetProgressMessage( int Index, string S, color C )
{
...
//{{ Foreground fov angle
//!{ 2006. 3. 6		허 창 민
/** returns foreground FOV angle */
event float GetForegroundFOVAngle()
{
	// 기본은 다른 FOV와 같은 FOV 사용
	return GetFOVAngle();
}
//!} 2006. 3. 6		허 창 민
//}}
...
/**
 * This will turn the subtitles on or off depending on the valud of bValue
 *
...
reliable server function ServerSendMusicInfo()
{
	...
	{
		ClientCrossFadeMusicTrack(WorldInfo.LastMusicAction, WorldInfo.LastMusicTrack);
	}
}

/** called to crossfade to a Kismet played music track on the client */
reliable client event ClientCrossFadeMusicTrack(SeqAct_CrossFadeMusicTracks MusicAction, MusicTrackStruct MusicTrack)
{
	if (MusicAction != None && MusicTrack.TheSoundCue != None)
	{
		MusicAction.CrossFadeTrack(MusicTrack);
		...
		WorldInfo.LastMusicAction = MusicAction;
		WorldInfo.LastMusicTrack = MusicTrack;
	}
...
final simulated function DrawDebugTextList(Canvas Canvas, float RenderDelta)
{
	...
	if (DebugTextList.Length > 0)
	{
		...
		for (Idx = 0; Idx < DebugTextList.Length; Idx++)
		{
			...
			ScreenLoc = Canvas.Project(DebugTextList[Idx].SrcActor.Location + (DebugTextList[Idx].SrcActorOffset >> CameraRot));
 ...
 */
final reliable client event AddDebugText(string DebugText, optional Actor SrcActor, optional float Duration = -1.f, optional vector Offset)
{
	local int Idx;
	if (SrcActor == None)
	{
...
event NotifyTakeScreenShot( optional string ScreenShotPath );

defaultproperties
{
	//RemainingMoveDelta=0.0
	...
	ExposureCenterRegionX=0.75
	ExposureCenterRegionY=0.80
}
