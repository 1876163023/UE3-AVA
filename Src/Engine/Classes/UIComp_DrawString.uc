...
var(Presentation) 	private	ETextClipMode				WrapMode<ToolTip=Controls what happens when the text doesn't fit into the bounding region of this string>;

/** Determines how the nodes of this string are ordered when the string is being clipped */
var(Presentation)	private	EUIAlignment				ClipAlignment<ToolTip=Determines which part of the string should be clipped when it doesn't fit into the bounding region (only relevant is wrap mode is Clipped)>;
...
var deprecated private{private} EUIAlignment TextAlignmentHorizontal;
var deprecated private{private} EUIAlignment TextAlignmentVertical;

///** 문자를 타이핑하는 효과를 넣기 위해 한문자와 그 문자가 조합문자인지를 저장하였음 */
//struct native TypeCharInfo
//{
//	var byte		UnicodeChar;
//	var bool		bComposing;
//};
//
//var(TypeChar) transient array<TypeCharInfo> DrawTypeString;
//
//var() bool bTypingEnabled<ToolTip=triggered by a new text>;
//var() int TypingCPM<ToolTip=Character Per Minute>;
//var transient float TypingUpdateTime;
...
cpptext
{
	 ...
	 *
	 * @param	NewComboStyle	the new style to use for rendering the string
	 */
...
protected:
	 ...
	 * @param	Parameters	the region of RenderBounds
	 */
...
/* === Unrealscript === */
/**
 * Changes the wrapping mode for this string component.
 *
 * @param	NewClipMode	the new wrapping mode for this string.
 */
final function SetWrapMode( ETextClipMode NewClipMode )
{
	bReapplyFormatting = bReapplyFormatting || NewClipMode != WrapMode;
	WrapMode = NewClipMode;
 ...
 * @param	NewClipMode	the new wrapping mode for this string.
 */
final event SetClipAlignment( EUIAlignment NewClipAlignment )
{
	bReapplyFormatting = bReapplyFormatting || NewClipAlignment != ClipAlignment;
...
final event SetAutoSizePadding( EUIOrientation Orientation, float NearValue, float FarValue, EPositionEvalType NearScaleType, EPositionEvalType FarScaleType )
{
	local bool bNeedsReformatting;
	...
	bNeedsReformatting = AutoSizeParameters[Orientation].Padding.Value[0] != NearValue
		...
		|| AutoSizeParameters[Orientation].Padding.ScaleType[0] != NearScaleType
		|| AutoSizeParameters[Orientation].Padding.ScaleType[1] != FarScaleType;
...
DefaultProperties
{
	...
	// must assign values to these deprecated variables since UIALIGN_Default does not have a value of 0
	TextAlignmentHorizontal=UIALIGN_Default
	TextAlignmentVertical=UIALIGN_Default
}
