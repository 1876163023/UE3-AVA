...
var hmserialize bool	bIsWalking;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// currently walking (can't jump, affects animations)
...
var hmserialize const	bool	bTryToUncrouch;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// when auto-crouch during movement, continually try to uncrouch
...
var hmserialize const bool	bSimulateGravity;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// simulate gravity for this pawn on network clients when predicting position (true if pawn is walking or falling)
...
var hmserialize float   GroundSpeed;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// The maximum ground speed.
var hmserialize float   WaterSpeed;		// [!] 20070323 dEAthcURe|HM 'hmserialize'	// The maximum swimming speed.
var hmserialize float   AirSpeed;		// [!] 20070323 dEAthcURe|HM 'hmserialize'	// The maximum flying speed.
var float	LadderSpeed;	// Ladder climbing speed
var hmserialize float	AccelRate;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// max acceleration rate
var hmserialize float	JumpZ;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// vertical acceleration w/ jump
...
var hmserialize float   AirControl;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// amount of AirControl available to the pawn
...
var(Eyes) float  	BaseEyeHeight; 	// Base eye height above collision center.
var(Eyes) float   	EyeHeight;     	// Current eye height, adjusted for bobbing and stairs.
...
var() hmserialize int Health;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// Health: default.Health controls normal maximum
...
var hmserialize vector	TakeHitLocation;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// location of last hit (for playing hit/death anims)
var class<DamageType>	HitDamageType;			// damage type of last hit (for playing hit/death anims)
var hmserialize vector	TearOffMomentum;	// [!] 20070323 dEAthcURe|HM 'hmserialize'	// momentum to apply when torn off (bTearOff == true)
...
var	hmserialize const	byte	RemoteViewPitch;	// [!] 20070323 dEAthcURe|HM 'hmserialize'
...
var hmserialize repnotify	vector	FlashLocation;	// [!] 20070323 dEAthcURe|HM 'hmserialize'
/** increased when weapon fires. 0 = not firing. 1 - 255 = firing */
var hmserialize repnotify	byte	FlashCount;	// [!] 20070323 dEAthcURe|HM 'hmserialize'
/** firing mode used when firing */
var	hmserialize repnotify	byte	FiringMode;	// [!] 20070323 dEAthcURe|HM 'hmserialize'
...
var const PrimitiveComponent PreRagdollCollisionComponent;
...
var	Vector				LastLoc;						// 보간을 위한 Properties...
var Rotator				LastRot;
var	float				MeshInterpSpeedT;
var	float				MeshInterpSpeedR;
...
cpptext
{
	...
	void NotifyBump(AActor *Other, UPrimitiveComponent* OtherComp, const FVector &HitNormal);
	...
	virtual UBOOL UseInstigatorBlockingFor(const AActor* Other) const;
...
replication
{
	...
	if( bNetDirty && (Role==ROLE_Authority) )
		InvManager, FlashLocation, bSimulateGravity, bIsWalking, PlayerReplicationInfo, HitDamageType,
...
simulated event ReplicatedEvent( name VarName )
{
	...
	if( VarName == 'FlashCount' && Controller == None)	// FlashCount and FlashLocation are changed when a weapon is fired.
	{
		FlashCountUpdated( true );
	}
	else if( VarName == 'FlashLocation' && Controller == None) // FlashCount and FlashLocation are changed when a weapon is fired.
	{
...
simulated native function SetTempLocation( vector NewLocation, rotator NewRotation );
 ...
 */
native function bool IsValidTargetFor(Controller C);
...
*/
simulated native function bool IsPlayerPawn();
...
event TakeDamage(int Damage, Controller InstigatedBy, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo HitInfo, optional class<Weapon> DamageCauser)
{
 ...
 * @Param	bScoreKill		true 이면 Kill Score 를 올려준다. Medic 때문에 들어갔다... 2006/6/14 by OZ
 *
...
//	local Vector	TossVel;
	...
	if ( bDeleteMe || WorldInfo.Game == None || WorldInfo.Game.bLevelChange )
		return false; // already destroyed, or level is being cleaned up
  ...
  //  }
	...
	if ( bScoreKill == true )
	{
		if ( Controller != None )
		{
			WorldInfo.Game.Killed(Killer, Controller, self, damageType, weaponBy);
		}
		else
			WorldInfo.Game.Killed(Killer, Controller(Owner), self, damageType, weaponBy);
	}
...
ignores Bump, HitWall, HeadVolumeChange, PhysicsVolumeChange, Falling, BreathTimer, FellOutOfWorld;
	...
	function PlayFiring(float Rate, name FiringModeName) {}
	...
	event TakeDamage(int Damage, Controller EventInstigator, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo HitInfo, optional class<Weapon> DamageCauser)
	{
		SetPhysics(PHYS_Falling);
...
simulated function PlayFiring(float Rate, name FiringModeName);
...
*/
event PlayFootStepSound(int FootDown,optional bool bManual);
...
simulated event StopDriving(Vehicle V)
{
	if ( Mesh != None )
	{
		Mesh.CullDistanceEx = Default.Mesh.CullDistanceEx;
...
function TossOtherWeapon(Weapon Weap)
{
	local vector	POVLoc, TossVel;
	local rotator	POVRot;
	local Vector	X,Y,Z;
 ...
 */
simulated event float GetCollisionRadius()
{
...
function DoKismetAttachment(Actor Attachment, SeqAct_AttachToActor Action)
{
	...
	// if attaching to bone...
	if( Mesh != None && Action.BoneName != '' )
	{
		if( Action.bUseRelativeOffset || Action.bUseRelativeRotation )
		{
			Rot = QuatToRotator(Mesh.GetBoneQuaternion(Action.BoneName));
			Loc = Mesh.GetBoneLocation(Action.BoneName);
			...
			{
				Attachment.SetRotation(Rot + Action.RelativeRotation);
			}

			// if we're using the offset, place attachment relatively to the target
			if( Action.bUseRelativeOffset )
			{
				Attachment.SetLocation(Loc + Vector(Rot) * Action.RelativeOffset);
			}
...
// {{ 20061103 dEAthcURe|HM
//사용여부확인?
event Weapon HmAddWeapon(String WeaponClassStr, bool bActive)
{
	Local Weapon		Weap;
	local class<Weapon> WeaponClass;

	WeaponClass = class<Weapon>(DynamicLoadObject(WeaponClassStr, class'Class'));
	Weap		= Weapon(self.FindInventoryType(WeaponClass));
	if( Weap != None )
	{
		return Weap;
	}
	return Weapon(self.CreateInventory( WeaponClass, !bActive));
}
// }} 20061103 dEAthcURe|HM

// {{ 20070301 dEAthcURe
event Weapon HmAddWeaponBegin(String WeaponClassStr)
{
	local Weapon		Weap;
	local class<Weapon> WeaponClass;
	
	WeaponClass = class<Weapon>(DynamicLoadObject(WeaponClassStr, class'Class'));
	Weap = Spawn( WeaponClass );
	`log("[dEAthcURe|HmAddWeaponBegin] " @ Weap);
	return Weap;
}

event HmAddWeaponEnd(Weapon weap, bool bActive)
{	
	InvManager.AddInventory( weap, !bActive );
	`log("[dEAthcURe|HmAddWeaponEnd] " @ Weap @ bActive);
}
// }} 20070301 dEAthcURe
