...
var private const DSPPreset LastDSPPreset;
var private const SoundCue LastAmbientSound;
...
cpptext
{
	 ...
	 * @return	TRUE to consume the key event, FALSE to pass it on.
	 */
	virtual UBOOL InputChar(FViewport* Viewport,INT ControllerId, const FInputCompositionStringData& CompStrData );
...
//delegate bool HandleInputKey( int ControllerId, name Key, EInputEvent EventType, float AmountDepressed );
 ...
 */
//@todo ronp - input processing optimization
//delegate bool HandleInputAxis( int ControllerId, name Key, float Delta, float DeltaTime);
...
event bool InputKey(int ControllerId,name Key,EInputEvent Event, float AmountDepressed, bool bGamepad )
{
	...
	{
		if(GlobalInteractions[InteractionIndex].InputKey(ControllerId,Key,Event,AmountDepressed, bGamepad))
		{
			`log("InputKey processed - " $ `showvar(Key) @ `showvar(GetEnum(enum'EInputEvent',Event),Event) @ `showobj(GlobalInteractions[InteractionIndex],Handler),,'Input');
			return true;
		}
	}
	return false;
...
event bool InputAxis(int ControllerId,name Key,float Delta,float DeltaTime)
{
	...
	for(InteractionIndex = 0;InteractionIndex < GlobalInteractions.Length;InteractionIndex++)
	{
		if(GlobalInteractions[InteractionIndex].InputAxis(ControllerId,Key,Delta,DeltaTime))
		{
			return true;
		}
	}
...
event bool InputChar(int ControllerId, InputCompositionStringData CompStrData )
{
	...
	for(InteractionIndex = 0;InteractionIndex < GlobalInteractions.Length;InteractionIndex++)
	{
		if(GlobalInteractions[InteractionIndex].InputChar(ControllerId, CompStrData))
		{
//			`log("InputChar processed - " $ `showvar(ControllerId) @ `showvar(Unicode,Character) @ `showobj(GlobalInteractions[InteractionIndex],Handler),,'Input');
			return true;
		}
	}
	return false;
...
event bool InputCandidate(int ControllerID, InputCandidateStringData CandStrData)
{
	...
	for(InteractionIndex = 0;InteractionIndex < GlobalInteractions.Length;InteractionIndex++)
	{
		if(GlobalInteractions[InteractionIndex].InputCandidate(ControllerID, CandStrData))
		{
			return true;
		}
	}
 ...
 * @return	True to consume the character, false to pass it on.	 
 */
event bool InputReadingString(int ControllerID, InputReadingStringData ReadStrData)
{
	local int InteractionIndex;
	// Pass the input on to the target player's interactions.
	for(InteractionIndex = 0;InteractionIndex < GlobalInteractions.Length;InteractionIndex++)
	{
		if(GlobalInteractions[InteractionIndex].InputReadingString(ControllerID, ReadStrData))
		{
			return true;
		}
	}
...
event HmReinit()
{
	...
	`log("[dEAthcURe] GameViewportClient:HmReinit");	
