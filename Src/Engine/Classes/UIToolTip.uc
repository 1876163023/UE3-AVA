...
native final function bool IsActiveToolTip() const;
...
event Initialized()
{
	...
	Local byte ButtonMaskDisable;
	
	Super.Initialized();

	ButtonMask = PRIVATE_NotDockable | PRIVATE_NotRotatable;
	ButtonMaskDisable = PRIVATE_TreeHiddenRecursive | PRIVATE_Protected;

	SetPrivateBehavior( ButtonMask, true );
	SetPrivateBehavior( ButtonMaskDisable, false );
}
...
DefaultProperties
{
	...
	Begin Object Name=LabelStringRenderer
		...
		AutoSizeParameters(UIORIENT_Horizontal)=(bAutoSizeEnabled=true,Extent=(Value[UIAUTOSIZEREGION_Maximum]=0.3,EvalType[UIAUTOSIZEREGION_Maximum]=UIAUTOSIZEEVAL_PercentScene))
