...
class UITabControl extends UIObject
	native(UIPrivate)
	`devplaceable;	// notplaceable for now, until it's fully functional.

`include(Core/Globals.uci)

/*
	- override NavigateFocus to override left/right/up/down for switching tab pages when the tab control itself has focus.

	- sequence actions
		add, remove, replace tab (by ref, caption, or index)
		enable, disable tab (by ref, caption, or index)
		get active tab

	- sequence events
		tab activated, enabled, disabled
		tab added, removed
 ...
 */
var(Presentation)	UIScreenValue		TabButtonSize;

/**
 * The amount of padding to apply to each button's text.  The specified value will be evenly distributed to the left and
 * right sides of the button's caption.
 */
var(Presentation)	UIScreenValue		TabButtonPadding;
 ...
 */
native function UITabPage CreateTabPage( class<UITabPage> TabPageClass );
...
event bool InsertPage( UITabPage PageToInsert, int PlayerIndex, int InsertIndex=INDEX_NONE, optional bool bActivateImmediately=true )
{
	...
	`log(">>>" @ `location @ `showobj(PageToInsert) @ `showvar(bActivateImmediately),,'TabControl' );
	...
	`log("<<<" @ `location @ `showobj(PageToInsert) @ `showvar(bActivateImmediately) @ `showvar(bResult),,'TabControl');
	return bResult;
}
...
event bool RemovePage( UITabPage PageToRemove, int PlayerIndex )
{
	...
	`log(">>>" @ `location @ `showobj(PageToRemove),,'TabControl' );
	...
	`log("<<<" @ `location @ `showobj(PageToRemove) @ `showvar(bResult),,'TabControl');
	return bResult;
}
...
event bool ReplacePage( UITabPage ExistingPage, UITabPage NewPage, int PlayerIndex, optional bool bFocusPage=true )
{
	...
	`log(">>>" @ `location @ `showobj(ExistingPage) @ `showobj(NewPage) @ `showvar(bFocusPage),,'TabControl' );
	...
	`log("<<<" @ `location @ `showobj(ExistingPage) @ `showobj(NewPage) @ `showvar(bResult),,'TabControl');
	return bResult;
}
...
event bool ActivatePage( UITabPage PageToActivate, int PlayerIndex, optional bool bFocusPage=true )
{
	...
	`log(">>>" @ `location @ `showobj(PageToActivate) @ `showvar(bFocusPage),,'TabControl' );
	...
	`log("<<<" @ `location @ `showobj(PageToActivate) @ `showvar(bResult),,'TabControl');
	return bResult;
}
...
event bool ActivateNextPage( int PlayerIndex, optional bool bFocusPage=true, optional bool bAllowWrapping=true )
{
	...
	`log("<<<" @ `location @ `showvar(bResult),,'TabControl');
	return bResult;
}
 ...
 */
function int FindPageIndexByCaption( string PageCaption )
{
	local int PageIndex;
	...
	if ( Len(PageCaption) > 0 )
	{
		for ( PageIndex = Pages.Length - 1; PageIndex >= 0; PageIndex-- )
		{
			if ( Pages[PageIndex] != None )
			{
				...
				if ( btn != None && btn.GetCaption() ~= PageCaption )
				{
					break;
				}
			}
...
DefaultProperties
{
	...
	TabButtonSize=(Value=0.02,ScaleType=EVALPOS_PercentageOwner)
	TabButtonPadding=(Value=10,ScaleType=EVALPOS_PixelOwner)
	bUpdateLayout=true
