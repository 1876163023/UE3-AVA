...
struct native PerMapPackagesInfo
{
	/** When this map is loaded, the packages in the following array will be loaded and added to root, then removed from root when map is unloaded */
	var name MapName;
...
/** A list of map/array pairs that is used at LoadMap time to fully load packages that may be needed for the map with DLC, but we can't use DynamicLoadObject to load from the packages */
var array<PerMapPackagesInfo> PerMapPackagesToLoad;

/** ViewportResizer를 UnGame.cpp에서 가지고옴. 옵션창에서 UGameEngine에 접근해 ViewportResizer값을 수정해야함 */
struct native ViewportResizer
{
	var INT SizeX, SizeY;
	...
	structcpptext
	{
		...
		void Resize(INT NewSizeX, INT NewSizeY, UBOOL bIsDefaultLocalMap, UBOOL& bImmediately);
	}
};

var ViewportResizer GViewportResizer;
...
cpptext
{
	...
	// {{ 20071123 dEAthcURe|PL 클라이언트의 로딩을 둘로 쪼개서 호스트와 동시로딩
	#ifdef EnableClientPreloading
	UBOOL preloadMapClient(const FURL& URL, FString& Error );
	UBOOL postloadMapClient(const FURL& URL, UPendingLevel* Pending, FString& Error);
	#endif
	// }} 20071123 dEAthcURe|PL 클라이언트의 로딩을 둘로 쪼개서 호스트와 동시로딩
	 ...
	 */
	void AddPerMapPackagesToLoad(FName MapName, TArray<FName> Packages, UBOOL bLoadPackagesForCurrentMap);

	/**
	 * Empties the PerMapPackages array, and removes any currently loaded packages from the Root
	 */
	void CleanupAllPerMapPackagesToLoad();
	 ...
	 */
	void LoadPerMapPackages(FName MapName);
	 ...
	 */
	void CleanupPerMapPackages(FName MapName);
}
