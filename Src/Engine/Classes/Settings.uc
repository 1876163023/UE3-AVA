...
struct native SettingsData
{
	 ...
	 */
	var const native pointer Value2{INT};
...
struct native SettingsPropertyPropertyMetaData
{
	...
	var const string MinVal;
	/** The max value for this property */
	var const string MaxVal;
};
...
cpptext
{
	...
	FORCEINLINE FSettingsProperty* FindProperty(INT PropertyId)
	{
		...
		{
			FSettingsProperty& Property = Properties(Index);
	...
	FORCEINLINE FLocalizedStringSetting* FindStringSetting(INT SettingId)
	{
		...
		{
			FLocalizedStringSetting& Setting = LocalizedSettings(Index);
 ...
 * @return true if found, false otherwise
 */
native function bool GetStringSettingValueNames(int StringSettingId,out array<name> Values);
