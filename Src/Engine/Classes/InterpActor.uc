...
var()	editconst int		hmId; // 20071011 dEAthcURe|HM
...
event Detach(Actor Other)
{
	...
	// InterpActor의 속도를 Detach되는 Pawn의 속도에 더해준다.
	if ( Pawn(Other) != None )
	{
		`log("InnterpActor.Detach" @Other @Velocity);

		// 80%의 속도로 캐릭터에게 적용해 준다.
		Pawn(Other).Velocity.X += Velocity.X * 0.8;
		Pawn(Other).Velocity.Y += Velocity.Y * 0.8;
	}
}
