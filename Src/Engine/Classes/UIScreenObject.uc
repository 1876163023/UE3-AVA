...
var(Presentation) interp	UIScreenValue_Bounds		Position;
 ...
 * @note: noexport because non-const optional arrays aren't exported correctly by the script compiler.
 */
native final noexport function ActivateEventByClass( int ControllerIndex, class<UIEvent> EventClassToActivate, optional Object InEventActivator, optional bool bActivateImmediately, optional array<int> IndicesToActivate, optional out array<UIEvent> out_ActivatedEvents );
 ...
 */
native final function bool EmulatePlayerInput( InputEventParameters EventParms );
 ...
 */
native final function float GetPosition(EUIWidgetFace Face, EPositionEvalType OutputType=EVALPOS_None, bool bZeroOrigin=FALSE) const;

/**
 * @Returns a given bounds for this widget
 * @Dimension	Which bounds to return
* @param	OutputType		indicates the format of the returnedvalue
 *							EVALPOS_None:
 *								NewValue will be considered to be in whichever format is configured as the ScaleType for the specified face
 *							EVALPOS_PercentageOwner:
...
native final function float GetBounds(EUIOrientation Dimension, EPositionEvalType OutputType=EVALPOS_None) const;
...
/**
 * Changes this widget's visibility.  Should never be called directly unless you explicitely want to bypass overrides
 * of this method.
 *
 * @param	bVisible	specify FALSE to hide the widget.
 */
final event PrivateSetVisibility( bool bVisible )
{
	if ( bHidden == bVisible )
	{
		bHidden = !bVisible;
...
{
	Local UIObject ThisObj;
	Local name AnimSeqName;

	PrivateSetVisibility(bIsVisible);
	ActivateEventByClass(GetBestPlayerIndex(), bIsVisible ? class'UIEvent_Show' : class'UIEvent_Hidden',Self);

	ThisObj = UIObject(Self);
	if( ThisObj != none )
	{
		AnimSeqName = bIsVisible ? ThisObj.ShownAnim : ThisObj.HiddenAnim;
		if( AnimSeqName != '' )
		{
			ThisObj.PlayUIAnimation(AnimSeqName);
		}
	}
}
...
{
	if( Action.bCtrlOpacity )
		Opacity = Action.bVisible ? 1.0 : 0.0;
...
// 키설정을 변경하기위해 추가
function OnUnsetUserKey( UIAction_UnsetUserKey Action )
{
	local LocalPlayer PlayerOwner;

//	Local string ConsoleMessage;
//	ConsoleMessage = "UnsetUserKey"@Action.bFactoryDefault;

	PlayerOwner = GetPlayerOwner();
	if ( PlayerOwner != None && PlayerOwner.Actor != None && PlayerOwner.Actor.PlayerInput != None)
	{
		PlayerOwner.Actor.PlayerInput.UnSetUserKey( Action.bFactoryDefault); 
	}
	else
	{
		`log(`location@"Couldn't execute UnsetUserKey '" $ Action.bFactoryDefault $ "':" @ `showobj(PlayerOwner) @ (PlayerOwner != None ? `showobj(PlayerOwner.Actor) : ""));
	}
}
...
function SetChildGroupVisibility( int GroupID, bool bIsVisible )
{
	local int	i;
	for ( i = 0; i < Children.Length; i++ )
	{
		if ( Children[i].GroupID == GroupID )
		{
			Children[i].SetVisibility( bIsVisible );
		}
		else
		{
			Children[i].SetChildGroupVisibility( GroupID, bIsVisible );
		}
	}
}
