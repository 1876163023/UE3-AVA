...
//! 안보이는 켜서를 넣기 위해 추가.(2007/05/22 고광록)
var()								string					DisplayCursorName<ToolTip=custom cursor(default="Arror")>;
...
var()								float					FadeInTimeWhenOpen;

var()								bool					bPreventChildSceneMouseHover;

enum EScenePriorityType
{
	EScenePrior_Script_Low,
	EScenePrior_UIScene_Normal,
	EScenePrior_PopUpScene_High,
};

/**
 * Scene의 우선순위를 가리킴
 * 일반적으로 Script에서 생성한 Scene은 바닥에 깔려야함
 * 채널에서 생성한 Scene은 원래와 같이 동작
 * 팝업은 위의 두 씬 ( 일반 채널씬, 스크립트 씬 )보다 위에서 동작
 */
var	transient						EScenePriorityType		Priority;

var()		public{private}			bool					bUseEscapeEvent<ToolTip=whether using 'Escape Event' or Not(close this scene automatically)>;
var()								class<UIEvent>			EscapeEvent<EditCondition=bUseEscapeEvent>;
var()								UIScreenObject			EscapeEmulationTarget<EditCondition=bUseEscapeEvent>;
var()								name					EscapeEmulationKey<EditCondition=bUseEscapeEvent>;
 ...
 */
native final function bool IsSceneActive() const;
 ...
 */
native final function bool IsSceneReachable() const;
