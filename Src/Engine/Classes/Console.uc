...
/** Forced command **/
var string ForcedConsoleCommandPrefix;
var string MiniPrefix;
...
/* Language Support for far east countries ( Korea, China, Japan, ... ) */
var bool bComposing;
var bool bBlockFirstChar;

/* IME Support (Candidate Window)*/
var string CandidateStr;
...
{
	MiniPrefix = "(>";
	ForcedConsoleCommandPrefix = "";	
	GotoState('Typing');
	SetInputText(Text);
	SetCursorPos(Len(Text));
}

function StartTyping_Forced(string prefix, coerce string Text)
{
	MiniPrefix = prefix;
	ForcedConsoleCommandPrefix = Text;
	Text = "";
	GotoState('Typing');
	SetInputText(Text);
	SetCursorPos(Len(Text));
...
// [ 2006/09/28, YTS ]
// Draw a Candidate Window for Windows IME
function DrawCandidateWindow( Canvas Canvas )
{
	Local float XL, YL, Left, Top;
	Local float CellPadding; 
	...
	Canvas.Strlen( CandidateStr, XL, YL );
	Left = Canvas.ClipX - XL - CellPadding * 2;
	Top = Canvas.ClipY - YL - CellPadding * 2;

	Canvas.SetPos( Left, Top );
	Canvas.SetDrawColor(0,0,0);
	Canvas.DrawRect(XL + CellPadding * 2, YL + CellPadding * 2);
	
	Canvas.SetPos( Left, Top );
	Canvas.SetDrawColor(0,255,0);
	Canvas.DrawBox(XL + CellPadding * 2, YL + CellPadding * 2);
	Canvas.SetPos( Left + CellPadding , Top + CellPadding);
	Canvas.DrawText(CandidateStr);
}
...
`endif
	...
	//@todo ronp - input processing optimization (return false)
...
// [2006/09/28 , YTS] 
// InputCandidate for drawing IME Candidate Window.
event bool InputCandidate( int ControllerId, InputCandidateStringData CandStrData )
{
	Local int i, PageEnd;
	Local bool bPrev, bNext;

	if( !IsUIConsoleOpen() && !IsUIMiniConsoleOpen() )
		return false;

	PageEnd = Min(CandStrData.PageStart + CandStrData.PageSize, CandStrData.Count);
	bPrev = CandStrData.PageStart > 0;
	bNext = (CandStrData.PageStart + CandStrData.PageSize) < CandStrData.Count;

	if( bPrev )
		CandidateStr = "<";

	for( i = CandStrData.PageStart ; i < PageEnd ; i++ )
		CandidateStr = CandidateStr @ (i - CandStrData.PageStart + 1 ) $ "." $ CandStrData.StrList[i];
...
state Typing
{
	...
	{
		local int ResultChar, CompChar, CompLen, ResultLen;
		Local bool bAppend;

		Local string ResultStr, CompStr;

		if( CompStrData.CompType == IMECOMPOSITION_Composition )
			CompStr = CompStrData.CompStr;
		else if( CompStrData.CompType == IMECOMPOSITION_Result || CompStrData.CompType == IMECOMPOSITION_None )
			ResultStr = CompStrData.CompStr;
		...
		// Assume that the User-Input is 'BackSpace' removing a composition character.
		// ex) '¤¡' --(UserInput BackSpace)--> ''
		if( ResultLen == 0 && CompLen == 0 )
		{
			SetInputText(Left(TypedStr, TypedStrPos) $ Right(TypedStr, Len(TypedStr) - (TypedStrPos + 1)));
		}
		
		ResultChar = Asc(Left(ResultStr,1));
		if( ( ResultLen > 0 && ResultChar >= 0x20) &&					// Check its Character Validity
			( ResultChar != Asc("~") && ResultChar != Asc("`") ) &&		// '~', '`' Pre-defined Character
			!( bBlockFirstChar && ResultChar > 0xff ) )					//	Block Character when Talk, TeamTalk called
		{
			bAppend = !bComposing;
			SetInputText(Left(TypedStr, TypedStrPos) $ ResultStr $ Right(TypedStr , Len(TypedStr) - ( TypedStrPos + bAppend ? 0 : 1 )));
			SetCursorPos(TypedStrPos + Len(ResultStr));
		}
		
		CompChar = Asc(Left(CompStr,1));	
		if( CompLen > 0 && CompChar >= 0x20 && CompChar != Asc("~") && CompChar != Asc("`"))
		{
			bAppend = !bComposing;
			SetInputText(Left(TypedStr, TypedStrPos) $ CompStr $ Right(TypedStr , Len(TypedStr) - ( TypedStrPos + bAppend ? 0 : 1) ) );
		}
		
		bComposing =  (CompLen > 0) ? true : false;
		bBlockFirstChar = false;

		// Original Code From Epic. ( No Support for far east languages )
		//
		//while(Len(ResultStr) > 0)
		//{
		//	Character = Asc(Left(ResultStr,1));
		//	ResultStr = Mid(ResultStr,1);
...
`endif
		else if( Key=='Enter' && Event == IE_Released )
		{
			...
			if( TypedStr!="" )
			{
				...
				else
				{
					...
					Temp=ForcedConsoleCommandPrefix $ TypedStr;
				}
		...
		else if( Super.InputKey( ControllerId, Key, Event, AmountDepressed, bGamepad ) )
		{
			return true;
		}
	...
	{
		local float xl,yl, CursorX, CursorY;
		local string OutStr, OutCurStr;
		local float ClipX;
		local float ClipY;
		local float LeftPos;
		...
		if ( !IsUIMiniConsoleOpen() )
		{
			...
			OutStr = MiniPrefix@TypedStr;
			OutCurStr = MiniPrefix@Left(TypedStr,TypedStrPos);
			...
			Canvas.StrLen(OutCurStr,xl,yl);

			CursorX = LeftPos + xl;
			CursorY = ClipY - yl;
			// move the pen to that position
			Canvas.SetPos(CursorX, CursorY - 1);
			...
			{
				// Draw a cursor for composing
				Canvas.StrLen(Mid(TypedStr, TypedStrPos ,1), xl, yl);
				Canvas.DrawRect(xl, yl);
				// Draw an Inverted Character
				Canvas.SetPos(CursorX, CursorY - 3);
				Canvas.SetDrawColor(0,0,0);
				Canvas.DrawText(Mid(TypedStr, TypedStrPos, 1));
			}

			DrawCandidateWindow(Canvas);
		}
	}
...
state Open
{
	...
	event bool InputChar( int ControllerId, InputCompositionStringData CompStrData )
	{
		...
		ResultChar = Asc(Left(ResultStr,1));
		if( (ResultLen > 0 && ResultChar >= 0x20) &&				// Check its character validity
			(ResultChar != Asc("~") && ResultChar != Asc("`")) &&	// '~' '`' Predefined character
			!(bBlockFirstChar && ResultChar > 0xff))				// Block First Character when Talk, TeamTalk called  
		{
			bAppend = !bComposing;
			SetInputText(Left(TypedStr, TypedStrPos) $ ResultStr $ Right(TypedStr , Len(TypedStr) - ( TypedStrPos + bAppend ? 0 : 1 )));
			SetCursorPos(TypedStrPos + Len(ResultStr));
...
`endif
	...
	event PostRender_Console(Canvas Canvas)
	{
		...
		local float Height;
		local float xl,yl,y, CursorX, CursorY;
		local string OutStr, OutCurStr;
		local int idx;
		...
		if ( !IsUIConsoleOpen() )
		{
			...
			OutStr = "(>"@TypedStr;
			OutCurStr = "(>"@Left(TypedStr,TypedStrPos);
			...
			else
			{
				...
				Canvas.DrawRect(xl, yl);
				// Draw an Inverted Character
				Canvas.SetPos(CursorX, CursorY - 2);
				Canvas.SetDrawColor(0,0,0);
				Canvas.DrawText(Mid(TypedStr, TypedStrPos, 1));
			}
