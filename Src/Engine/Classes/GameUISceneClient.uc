...
cpptext
{
	 ...
	 *
	 * @param	ControllerId - The controller which the axis movement is from.
	 * @param	ResultStr - 조합이 마쳐진 결과 문자
	 * @param	CompStr - 조합중인 미완성 문자
	 *
	...
	/**
	 * IME Candidate 들을 보여줘야할 때 들어옴 (한국어 자판에서 ㄱ->한자, 가->한자 키를 누를 때)
	 * 
	 * @param ControllerId - The controller which the axis movement is from.
	 ...
	 *
	 * @param	NewMouseX	the X position to move the mouse cursor to (in pixels)
	 * @param	NewMouseY	the Y position to move the mouse cursor to (in pixels)
	 */
	 ...
	 */
	virtual UBOOL IsUIActive( DWORD Flags=SCENEFILTER_IgnoreTransient ) const;
...
protected:
	 ...
	 * @param	SceneToActivate		the scene to activate
 	 * @param	Priority		씬의 우선순위 결정 (우선순위가 높은것이 위로 오도록), 언젠가 Weight란 이름으로 에픽엔진에 추가되면 변경바람.
	 */
	virtual void ActivateScene( UUIScene* SceneToActivate , BYTE Priority);
...
/** 플레이어 입력을 초기화 (눌린 상태의 버튼을 풀어줌)
 * cpptext의 void FlushPlayerInput에서 여기로 이전
 */
native function FlushPlayerInput();
