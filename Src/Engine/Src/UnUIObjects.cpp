...
// EmulatePlayerInput으로 처리할 때 Mouse버튼이 IE_Release되는 경우에 mouse cursor의 inside여부 판별에서 씹힌다. 이것을 우회하기 위해 hack flag하나 추가요
static LONG GHackEmulatePlayerInputDoNotCheckMouseCursor = 0;
...
// exec wrappers
void UUIRoot::execGetCurrentUIController _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
	*(UUIInteraction**)Result=GetCurrentUIController();
}
void UUIRoot::execGetSceneClient _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
	*(UGameUISceneClient**)Result=GetSceneClient();
}
void UUIRoot::execGetPrimitiveTransform _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIRoot::execGetCursorPosition _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIRoot::execGetCursorSize _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIRoot::execSetMouseCaptureOverride _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
/* == Unrealscript Stubs == */
void UUIScreenObject::execIsInitialized _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
	*(UBOOL*)Result=IsInitialized();
}
void UUIScreenObject::execRequestSceneUpdate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIScreenObject::execRequestFormattingUpdate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
	RequestFormattingUpdate();
}
void UUIScreenObject::execRequestPrimitiveReview _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIScreenObject::execGetWidgetPathName _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
	*(FString*)Result=GetWidgetPathName();
}
void UUIScreenObject::execGetActivePlayerCount _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
UBOOL UUIScreenObject::GetViewportSize( FVector2D& out_ViewportSize ) const
{
	...
	if ( OwnerScene != NULL && OwnerScene->SceneClient != NULL )
	{
		...
		UBOOL SceneIsInitialized = OwnerScene->SceneClient->IsSceneInitialized(OwnerScene);
		...
		{
			bResult = OwnerScene->SceneClient->GetViewportSize(NULL, out_ViewportSize);
		}
	}
...
void UUIScreenObject::execRemoveChild _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIScreenObject::execGetChildren _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
/**
 * Generates a list of any children of this widget which are of a class that has been obsolete, recursively.
 */
void UUIScreenObject::FindObsoleteWidgets( TArray<UUIScreenObject*>& out_ObsoleteWidgets )
{
	if( HasAnyFlags(RF_Obsolete) )
	{
		out_ObsoleteWidgets.AddUniqueItem(this);
	}

	for( INT ChildIndex = 0 ; ChildIndex < Children.Num() ; ChildIndex++ )
	{
		Children(ChildIndex)->FindObsoleteWidgets(out_ObsoleteWidgets);
	}
}
 ...
 */
FLOAT UUIScreenObject::GetPosition( BYTE Face, BYTE OutputType/*=EVALPOS_None*/, UBOOL bZeroOrigin/*=FALSE*/ ) const
{
	...
	if ( Position.IsPositionCurrent(Face) && IsA(UUIObject::StaticClass()) )
	{
		Result = ((UUIObject*)this)->RenderBounds[Face];
	}
	else
	{
		...
		if ( !bZeroOrigin )
		{
			if ( Face == UIFACE_Left || Face == UIFACE_Right )
			{
				BaseValue += ViewportOrigin.X;
			}
			else
			{
				BaseValue += ViewportOrigin.Y;
			}
		}
...
FLOAT UUIScreenObject::GetBounds(BYTE Dimension, BYTE OutputType) const
{
	return Position.GetBoundsExtent(this, EUIOrientation(Dimension), EPositionEvalType(OutputType) );
}
...
UBOOL UUIScreenObject::ContainsPoint(const FVector2D& Point) const
{
	...
	// UUIObject::ContainsPoint와 같이 Canvas좌표를 가져야한다
	FVector CanvasPoint = PixelToCanvas(Point);
	if(CanvasPoint.X < RenderBounds[UIFACE_Left] ||
		CanvasPoint.X > RenderBounds[UIFACE_Right] ||
		CanvasPoint.Y < RenderBounds[UIFACE_Top] ||
		CanvasPoint.Y > RenderBounds[UIFACE_Bottom])
	{
		return FALSE;
	}
	else
...
void UUIScreenObject::ActivateInitialState( INT PlayerIndex )
{
	if ( !HasActiveStateOfClass(InitialState,PlayerIndex) )
	{
		...
		if ( InitialState != UUIState_Enabled::StaticClass() && InitialState != UUIState_Disabled::StaticClass() )
		{
			...
			bSuccess = SetEnabled( TRUE, PlayerIndex );
		}
...
void UUIScreenObject::execHasActiveStateOfClass _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
/**
 * Inactive state에 해당 State가 있는지 검사 ( Inactive state = active가 될수 있는 state )
 *
 * @param	StateClass	the class to search for
 ...
 * @return	TRUE if there is at least one active state of the class specified
 */
UBOOL UUIScreenObject::HasInactiveStateOfClass( UClass* StateClass, INT* OutStateIndex /*=NULL*/ ) const
{
	UBOOL bResult = FALSE;
	for( INT StateIndex = 0 ; StateIndex < InactiveStates.Num() ; StateIndex++ )
	{
		UUIState* State = InactiveStates(StateIndex);
...
UBOOL UUIScreenObject::ActivateState( UUIState* StateToActivate, INT PlayerIndex )
{
	...
	if ( StateToActivate != NULL && InactiveStates.FindItemIndex(StateToActivate) != INDEX_NONE )
	{
		...
		debugf(NAME_DevUIStates, TEXT("%-24s %s %s %i Success:%s"), TEXT(">> ActivateState"), *GetWidgetPathName(), *StateToActivate->GetName(), PlayerIndex, bResult ? GTrue : GFalse);
	}
...
void UUIScreenObject::execActivateStateByClass _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIScreenObject::execDeactivateStateByClass _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIScreenObject::execActivateEventByClass _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIScreenObject::Render_Children( FCanvas* Canvas )
{
	...
	// 엔진 새 코드 ( 통합 2007.03.29 )
	// 포커스를 가진 Control(Widget)을 다른것보다 상위에 위치하도록 한다.
	...
	// 엔진 이전 코드
	// 포커스된것이 먼저 입력을 받을 때까지 이전 코드를 쓰겠음.
	...
	for ( INT i = 0; i < Children.Num(); i++ )
	{
		...
		if ( Child->IsVisible() )
		{
			...
			//@todo ronp - in the old version of this code (prior to making widgets responsible for rendering their children), the EditorUISceneClient used this slightly
			// different logic for determining the correct transform. (Notice how similar this is to the code in GetPositionExtents....)
			// need to figure out which way is the right way:
			/*
			// Push the transform
			if( Widget->Owner )
				Canvas->PushRelativeTransform(Widget->GetWidgetTransformLocal());
			else
				Canvas->PushRelativeTransform(Widget->GetWidgetTransformGlobal());
			*/
			...
			if ( Canvas->IsHitTesting() )
			{
				...
				Child->GetPositionExtents(X1, X2, Y1, Y2);
...
void UUIScreenObject::execPlayUISound _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
UBOOL UUIScreenObject::ProcessInputKey( const FSubscribedInputEventParameters& EventParms )
{
	...
	{
		// Tab 키를 이용해서 Control 의 Focus 를 옮기지 못하도록 막는다.
 ...
 */
UBOOL UUIScreenObject::ProcessInputChar( INT PlayerIndex, const FInputCompositionStringData& CompStrData )
{
	UBOOL bResult = FALSE;
...
UBOOL UUIScreenObject::ProcessInputCandidate( INT PlayerIndex, const FInputCandidateStringData& CandStrData)
{
	return FALSE;
}

UBOOL UUIScreenObject::ProcessInputReadingString( INT PlayerIndex, const FInputReadingStringData& ReadStrData)
{
	return FALSE;
}
 ...
 */
UBOOL UUIScreenObject::TranslateKey( const struct FInputEventParameters& EventParms, FName& out_UIKeyName, UBOOL bUseDefaultStates /* = FALSE  */)
{
	...
	if(WidgetKeyMapping != NULL)
	{
		...
		// bUseDefaultState일때는 this UUIScreenObject가 가지고 있는 모든 DefaultState를 검사한다
		TArray<UClass*> StateClasses;
		if( bUseDefaultStates )
		{
			StateClasses = DefaultStates;
		}
		else
		{
			for(INT StateIdx = 0; StateIdx < StateStack.Num(); StateIdx++)
				if( StateStack(StateIdx) )
					StateClasses.AddUniqueItem( StateStack(StateIdx)->GetClass() );
		}

		for(INT ClassIdx = StateClasses.Num() - 1; !bTranslatedKey && ClassIdx >= 0; ClassIdx--)
		{
			//const UUIState* State = StateStack(StateIdx);
			FUIInputAliasMap* InputMap = WidgetKeyMapping->StateLookupTable.Find(StateClasses(ClassIdx));
			if(InputMap != NULL)
			{
...
UBOOL UUIScreenObject::SetEnabled( UBOOL bEnabled, INT PlayerIndex/*=0*/ )
{
	...
	UUIObject* Obj = Cast<UUIObject>(this);
	if( bResult && Obj )
	{
		FName AnimSeqName = bEnabled ? Obj->EnabledAnim : Obj->DisabledAnim;
		if( AnimSeqName != NAME_None )
		{
			Obj->eventPlayUIAnimation(AnimSeqName);
		}
	}
 ...
 *						has the class specified
 */
UBOOL UUIScreenObject::IsDisabled( INT PlayerIndex/*=0*/, INT* StateIndex/*=NULL*/ ) const
{
	return HasActiveStateOfClass(UUIState_Disabled::StaticClass(),PlayerIndex, StateIndex);
}
/**
...
void UUIScreenObject::execIsEnabled _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIScreenObject::execIsFocused _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT_OPTX(PlayerIndex,GetBestPlayerIndex());
	P_GET_INT_OPTX_REF(StateIndex,0);
	P_FINISH;
...
void UUIScreenObject::execIsActive _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT_OPTX(PlayerIndex,GetBestPlayerIndex());
	P_GET_INT_OPTX_REF(StateIndex,0);
	P_FINISH;
...
void UUIScreenObject::execIsPressed _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT_OPTX(PlayerIndex,GetBestPlayerIndex());
	P_GET_INT_OPTX_REF(StateIndex,0);
	P_FINISH;
...
// 	}
	...
	if ( bResult )
	{
		ActivateEventByClass( PlayerIndex, UUIEvent_OnGainFocus::StaticClass(), this );
	}
...
UBOOL UUIScreenObject::LoseFocus( UUIObject* FocusedChild, INT PlayerIndex )
{
	...
	if( bResult )
	{
		ActivateEventByClass( PlayerIndex, UUIEvent_OnLoseFocus::StaticClass(), this );
	}
...
UBOOL UUIScreenObject::SetFocus(UUIScreenObject* Sender,INT PlayerIndex/* =0 */)
{
	...
	if ( IsVisible() )
	{
		...
		if ( bSetFocusToChild )
		{
			...
			UUIObject* LastFocused = FocusControls(PlayerIndex).GetLastFocusedControl();
		...
		else
		{
			...
			if ( WidgetSender != NULL && ContainsChild(WidgetSender,FALSE) )
			{
...
UBOOL UUIScreenObject::SetFocusToChild( UUIObject* ChildToFocus/*=NULL*/, INT PlayerIndex/*=0*/ )
{
	...
	if ( ChildToFocus != NULL )
	{
		...
		if ( ContainsChild(ChildToFocus,FALSE) )
		{
	...
	if ( !bResult )
	{
		...
		if ( ChildToFocus == NULL )
		{
			...
			if ( FocusTarget != NULL )
			{
				...
				if ( ContainsChild(FocusTarget,FALSE) )
				{
...
UBOOL UUIScreenObject::KillFocus( UUIScreenObject* Sender, INT PlayerIndex )
{
	...
	if ( bKillFocusToChild )
	{
		...
		UUIObject* Focused = FocusControls(PlayerIndex).GetFocusedControl();
		if ( Focused != NULL )
		{
			// this widget might have changed parents since it was set as the last focused control, so double-check that here
			//if ( CanPropagateFocusFor(Focused) )
			if ( ContainsChild(Focused,FALSE) )
			{
 ...
 * 사용자가 키를 누른것처럼 흉내낸다.
 * 
 * @param	KeyNameToEmulate		사용자가 눌렀다고 흉내낼 키 (LeftMouseButtom, Space 등등)
 *
 * @return	이 입력이 현재 ScreenObject에 의해 삼켜지면 TRUE. 아니면 FALSE
 */
UBOOL UUIScreenObject::EmulatePlayerInput( struct FInputEventParameters InputEventParms )
{
	FName UserKeyAlias;
	UBOOL bResult = FALSE;
	if( TranslateKey( InputEventParms, UserKeyAlias, TRUE ) )
	{
		if( IsDisabled() )
			bResult = FALSE;
		else
		{
			GHackEmulatePlayerInputDoNotCheckMouseCursor++;

			FSubscribedInputEventParameters SubcribedInputEventParms( InputEventParms, UserKeyAlias );
			SubcribedInputEventParms.EventType = IE_Pressed;
			ProcessInputKey(SubcribedInputEventParms);
			SubcribedInputEventParms.EventType = IE_Released;
			ProcessInputKey(SubcribedInputEventParms);

			GHackEmulatePlayerInputDoNotCheckMouseCursor--;
			if (GHackEmulatePlayerInputDoNotCheckMouseCursor<0)
				GHackEmulatePlayerInputDoNotCheckMouseCursor = 0;
 ...
 */
UBOOL UUIScene::IsSceneActive() const
{
	UBOOL bResult = FALSE;
...
/** 씬이 상호작용을 할 수 있는 위치에 있는가 ? 
 *
 * 예를들어 InputMode_Locked인 씬B가 씬 A위에 있으면
 * A는 Active씬이지만 도달 불가능하다
 */
UBOOL UUIScene::IsSceneReachable() const
{
	UBOOL bResult = FALSE;
	...
	if ( GameSceneClient != NULL )
	{
		for( INT SceneIndex = GameSceneClient->ActiveScenes.Num() - 1 ; SceneIndex >= 0 ; SceneIndex-- )
		{
			UUIScene* Scene = GameSceneClient->ActiveScenes(SceneIndex);
			if( Scene == NULL )
				continue;
			...
			if( Scene->SceneInputMode == INPUTMODE_Locked )
			{
				bResult = FALSE;
				break;
			}
...
{
	// @ REMOVE : 씬이 열릴때 이상이 없으면 삭제
	// register this scene to receive notifications when the viewport is resized
...
void UUIScene::PreRenderCallback()
{
	...
	// Viewport가 없어서 실패하는 경우가 있다. 왜일까 ?
	if(SceneClient->GetViewportSize(this, CurrentViewportSize))
	{
...
void UUIScene::InvalidatePositionDependencies( BYTE Face )
{
	...
	// if this is the left face 
		...
		if ( Position.IsPositionCurrent(UIFACE_Right)
		...
		for ( INT ChildIndex = 0; ChildIndex < Children.Num(); ChildIndex++ )
		{
			...
			if ( Child != NULL )
			{
				...
				if ( ChildPosition.IsPositionCurrent(UIFACE_Left) )
				{
				...
				if ( ChildPosition.IsPositionCurrent(UIFACE_Right)
	...
	// if this is the top face
		...
		if ( Position.IsPositionCurrent(UIFACE_Bottom)
		...
		for ( INT ChildIndex = 0; ChildIndex < Children.Num(); ChildIndex++ )
		{
			...
			if ( Child != NULL )
			{
				...
				if ( ChildPosition.IsPositionCurrent(UIFACE_Top) )
				{
				...
				if ( ChildPosition.IsPositionCurrent(UIFACE_Bottom)
	...
	for ( INT ChildIndex = 0; ChildIndex < SceneChildren.Num(); ChildIndex++ )
	{
		...
		for ( INT FaceIndex = 0; FaceIndex < UIFACE_MAX; FaceIndex++ )
		{
			...
			if ( Child->IsDockedTo(OldChild, FaceIndex) )
			{
				...
				Child->Modify();
				Child->SetDockTarget(FaceIndex, NULL, UIFACE_MAX, 0.f);
			}
			...
			if ( OldChild->IsDockedTo(Child, FaceIndex) )
			{
				...
				// SetDockTarget calls Modify, so no need to do that here
				OldChild->Modify();
				OldChild->SetDockTarget(FaceIndex, NULL, UIFACE_MAX, 0.f);
			}
...
UBOOL UUIScene::InputKey( INT ControllerId, FName Key, EInputEvent Event, FLOAT AmountDepressed/*=1.f*/, UBOOL bGamepad/*=FALSE*/ )
{
	...
	if ( GetInputEventSubscribers(Key, PlayerIndex, &SubscribersList) )
	{
		if ( SubscribersList != NULL )
		{
			...
			//debugf(TEXT("--- Subscribers of Key %s (Scene : %s) ---"), Key.GetName(),*this->GetName());
			//for( INT HandlerIndex = 0 ; HandlerIndex < SubscribersList->Subscribers.Num() ; HandlerIndex++ )
			//	debugf(TEXT("%s"), *SubscribersList->Subscribers(HandlerIndex)->GetName());
			//debugf(TEXT("--------------------------------------------------------------------"));
			for ( INT HandlerIndex = 0; HandlerIndex < SubscribersList->Subscribers.Num(); HandlerIndex++ )
			{
				...
				if ( Handler->HandleInputKeyEvent(EventParms) )
				{
					...
					//debugf(TEXT("### %s swallowed the input key %s ###"), *Handler->GetName(), Key.GetName());
					bResult = TRUE;
					break;
				}
			}
 ...
 * @return	TRUE to consume the axis movement, FALSE to pass it on.
 */
UBOOL UUIScene::InputAxis( INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime )
{
 ...
 * @return	TRUE to consume the character, false to pass it on.
 */
UBOOL UUIScene::InputChar( INT ControllerId, const FInputCompositionStringData& CompStrData )
{
	UBOOL bResult = FALSE;
	...
	if ( GetInputEventSubscribers(KEY_Character, PlayerIndex, &SubscribersList) && SubscribersList != NULL )
	{
		if ( SubscribersList != NULL )
		{
			for ( INT HandlerIndex = 0; HandlerIndex < SubscribersList->Subscribers.Num(); HandlerIndex++ )
			{
				...
				if ( Handler->ProcessInputChar(PlayerIndex, CompStrData) )
				{
					// swallowed the input event
					bResult = TRUE;
					break;
...
//  Cast an IME Candidate Info to ProcessInputCandidate()
UBOOL UUIScene::InputCandidate(INT ControllerId, const FInputCandidateStringData& CandStrData)
{
	UBOOL bResult = FALSE;
	EScreenInputMode InputMode;
	...
	if ( GetInputEventSubscribers(KEY_Character, PlayerIndex, &SubscribersList) && SubscribersList != NULL )
	{
		if ( SubscribersList != NULL )
		{
			for ( INT HandlerIndex = 0; HandlerIndex < SubscribersList->Subscribers.Num(); HandlerIndex++ )
			{
				...
				// todo: should this go through HandleInputEvent as well?
				if ( Handler->ProcessInputCandidate(PlayerIndex, CandStrData) )
				{
					// swallowed the input event
					bResult = TRUE;
					break;
...
//  Cast an IME ReadingString to ProcessInputReadingString()
UBOOL UUIScene::InputReadingString(INT ControllerId, const FInputReadingStringData& ReadStrData )
{
	UBOOL bResult = FALSE;
	EScreenInputMode InputMode;
	...
	if ( GetInputEventSubscribers(KEY_Character, PlayerIndex, &SubscribersList) && SubscribersList != NULL )
	{
		if ( SubscribersList != NULL )
		{
			for ( INT HandlerIndex = 0; HandlerIndex < SubscribersList->Subscribers.Num(); HandlerIndex++ )
			{
				...
				// todo: should this go through HandleInputEvent as well?
				if ( Handler->ProcessInputReadingString(PlayerIndex, ReadStrData) )
				{
					// swallowed the input event
					bResult = TRUE;
					break;
...
UBOOL UUIScene::ProcessInputKey( const FSubscribedInputEventParameters& EventParms )
{
	...
	if ( EventParms.EventType == IE_Released && EventParms.InputAliasName == UIKEY_CloseScene )
	{
		...
		{	
			if( EscapeEmulationTarget != NULL && EscapeEmulationKey != NAME_None )
			{
				FInputEventParameters NewInputEventParms = EventParms;
				NewInputEventParms.InputKeyName = EscapeEmulationKey;
				bResult = EscapeEmulationTarget->EmulatePlayerInput( NewInputEventParms );
			}
			if( !bResult && EscapeEvent )
			{
				// UIScene의 직속 Sequence들만 이벤트를 발생하기로 한다
				ActivateEventByClass( EventParms.PlayerIndex, EscapeEvent, this );
				bResult = TRUE;
			}

			// 위의 두 경우가 아니면 입력을 다음으로 넘긴다 // bResult = FALSE;
		}
	}
 ...
 */
void UUIScene::Send(ECallbackEventType InType, FViewport* InViewport, UINT InMessage)
{
	if ( InType == CALLBACK_ViewportResized )
	{
		if ( SceneClient != NULL && SceneClient->RenderViewport == InViewport )
		{
			const FVector2D PreviousViewportSize = CurrentViewportSize;
			SceneClient->GetViewportSize(this, CurrentViewportSize);
			...
			NotifyResolutionChanged(PreviousViewportSize, NewViewportSize);
			if ( DELEGATE_IS_SET(NotifyResolutionChanged) )
			{
				delegateNotifyResolutionChanged(PreviousViewportSize, NewViewportSize);
			}
		}
	}
}
...
void UUIScene::PreSave()
{
	...
	FindObsoleteWidgets( DeprecatedObjects );
...
// @ REMOVE : Rotated Widget을 위한 옛날 코드
///** 
// * @return Returns the full transformation for the widget.
// */
//FMatrix UUIObject::GetWidgetTransform() const
//{
//	const FLOAT LeftPos = Position.GetPositionValue(this, UIFACE_Left, EVALPOS_PixelViewport);
//	const FLOAT TopPos = Position.GetPositionValue(this, UIFACE_Top, EVALPOS_PixelViewport);
//
//	//@fixme - hmmm, this doesn't seem to be taking the anchor position into account, but then
//	// the anchor position is apparently already built into the Rotation.TransformMatrix, so maybe it doesn't need to...
//	// verify this!
//	FVector WidgetPos(LeftPos, TopPos, 0.0f);
//	FMatrix FullTransform = FTranslationMatrix(-WidgetPos) * Rotation.TransformMatrix * FTranslationMatrix(WidgetPos);
//
//	return FullTransform;
//}
//
///**
// * @param Transformation	The full transformation for the widget NOT including inherited tranforms from its parents.
// */
//FMatrix UUIObject::GetWidgetTransformLocal() const
//{
//	return Rotation.TransformMatrix;
//}
//
///**
// * @param Transformation	The full transformation for the widget including inherited tranforms from its parents.
// */
//FMatrix UUIObject::GetWidgetTransformGlobal() const
//{
//	// we'll need to build the rotation matrix starting with the top-most owner widget
//	// so first, build a list of owners that we can iterate easily.
//	TArray<UUIObject*> OwnerChain;
//	for ( UUIObject* CurrentParent = GetOwner(); CurrentParent; CurrentParent = CurrentParent->GetOwner() )
//	{
//		OwnerChain.Push(CurrentParent);
//	}
//
//	// now the OwnerChain's first element is our immediate parent, the last element is the top-most parent
//	// iterate from the outer-most to the inner-most
//	FMatrix FinalTransform = FMatrix::Identity;
//	for ( INT OwnerIndex = OwnerChain.Num() - 1; OwnerIndex >= 0; OwnerIndex-- )
//	{
//		UUIObject* CurrentParent = OwnerChain(OwnerIndex);
//		FinalTransform *= CurrentParent->GetWidgetTransform();
//	}
//
//	// finally, apply our own transform to the result
//	FinalTransform *= GetWidgetTransform();
...
FMatrix UUIObject::GenerateTransformMatrix( UBOOL bIncludeParentTransforms/*=TRUE*/ ) const
{
	...
	FMatrix ScaleMatrix = FScaleMatrix(1.f);
	if( ScaleValue != FVector(1.f) )
	{
		FVector ExtentVect(GetBounds(UIORIENT_Horizontal, EVALPOS_PixelViewport)*ScaleAxis.X,GetBounds(UIORIENT_Vertical, EVALPOS_PixelViewport)*ScaleAxis.Y,0.f*ScaleAxis.Z);	
		ScaleMatrix = FTranslationMatrix(-ExtentVect) * FScaleMatrix(FVector(ScaleValue.X, ScaleValue.Y, ScaleValue.Z)) * FTranslationMatrix(ExtentVect) ;
	}
	FMatrix Result = FTranslationMatrix(RenderOffset) * FTranslationMatrix(FVector(-PositionVect.X, -PositionVect.Y, ZDepth)) * ScaleMatrix * Rotation.TransformMatrix * FTranslationMatrix(PositionVect);
...
{
	// @ CHECK : 중복해서 InvalidatePosition 하지는 않는지 ?
	// invalidate any faces which are set to be 
	for ( INT FaceIndex = 0; FaceIndex < UIFACE_MAX; FaceIndex++ )
	{
		EPositionEvalType ScaleType = Position.GetScaleType((EUIWidgetFace)FaceIndex);
	...
	if ( DockTargets.IsWidthLocked() )
	{
		if ( FaceScaleType >= EVALPOS_PercentageViewport && FaceScaleType < EVALPOS_MAX )
		{
			if ( DockTargets.IsDocked(UIFACE_Right) && !DockTargets.IsDocked(UIFACE_Left) )
			{
				...
				const FLOAT PreviousWidth = Position.GetPositionValue(this, UIFACE_Right, EVALPOS_PixelOwner);
	...
	if ( DockTargets.IsHeightLocked() )
	{
		...
		if ( FaceScaleType >= EVALPOS_PercentageViewport && FaceScaleType < EVALPOS_MAX )
		{
			if ( DockTargets.IsDocked(UIFACE_Bottom) && !DockTargets.IsDocked(UIFACE_Top) )
			{
				...
				const FLOAT PreviousHeight = Position.GetPositionValue(this, UIFACE_Bottom, EVALPOS_PixelOwner);
...
void UUIObject::NotifySceneActivated()
{
	for ( INT i = 0; i < Children.Num(); i++ )
	{
		UUIObject* Child = Children(i);
		Child->NotifySceneActivated();
	}
}

void UUIObject::NotifySceneDeactivated()
{
	for ( INT i = 0; i < Children.Num(); i++ )
	{
		UUIObject* Child = Children(i);
		Child->NotifySceneDeactivated();
	}
}

/** Widget의 활성화/비활성화 보임/안보임 등등을 데이터프로바이더에서 조작 */
void UUIObject::UpdateWidgetWithProvider( FUIDataStoreBinding& DataBinding )
{
	UUIDataProvider* Provider = NULL;
	FString OutFieldName;
	if( DataBinding.ResolveMarkup( this, Provider, OutFieldName) && Provider )
	{
		Provider->UpdateWidget( OutFieldName, this );
	}
}
 ...
 */
UBOOL UUIObject::SetDockTarget( BYTE SourceFace, UUIScreenObject* Target, BYTE TargetFace, FLOAT Padding/*=0*/ )
{
	UBOOL bResult = FALSE;
	...
	if( IsTransformable() )
	{
		...
		else 
		{
			...
			else
			{
				...
				{
					DockTargets.DockPadding[SourceFace] = Padding;
...
void UUIObject::InvalidatePositionDependencies( BYTE Face )
{
	...
	// if this is the left face 
		...
		for ( INT ChildIndex = 0; ChildIndex < Children.Num(); ChildIndex++ )
		{
			...
			if ( Child != NULL )
			{
				...
				if ( ChildPosition.IsPositionCurrent(UIFACE_Right) && ChildPosition.GetScaleType(UIFACE_Right) == EVALPOS_PercentageOwner )
				{
					Child->InvalidatePosition(UIFACE_Right);
				}
			}
	...
	case UIFACE_Right:
		if ( DockTargets.IsWidthLocked() && Position.IsPositionCurrent(UIFACE_Left) )
		{
	...
	// if this is the right face
		for ( INT ChildIndex = 0; ChildIndex < Children.Num(); ChildIndex++ )
		{
			...
			if ( Child != NULL )
			{
				FUIScreenValue_Bounds& ChildPosition = Child->Position;
				// for any children which have a left/right scale type of PercentageOwner, invalidate
				if ( ChildPosition.IsPositionCurrent(UIFACE_Left) && ChildPosition.GetScaleType(UIFACE_Left) == EVALPOS_PercentageOwner )
				{
					Child->InvalidatePosition(UIFACE_Left);
				}
				if ( ChildPosition.IsPositionCurrent(UIFACE_Right) && ChildPosition.GetScaleType(UIFACE_Right) == EVALPOS_PercentageOwner )
	...
	// if this is the top face
		...
		for ( INT ChildIndex = 0; ChildIndex < Children.Num(); ChildIndex++ )
		{
			...
			if ( Child != NULL )
			{
				...
				// for any children which have a bottom scale type of PercentageOwner, invalidate
				if ( ChildPosition.IsPositionCurrent(UIFACE_Bottom) && ChildPosition.GetScaleType(UIFACE_Bottom) == EVALPOS_PercentageOwner )
				{
					Child->InvalidatePosition(UIFACE_Bottom);
				}
			}
	...
	// if this is the bottom face
		...
		if ( DockTargets.IsHeightLocked() && Position.IsPositionCurrent(UIFACE_Top) )
		{
		...
		for ( INT ChildIndex = 0; ChildIndex < Children.Num(); ChildIndex++ )
		{
			...
			if ( Child != NULL )
			{
				FUIScreenValue_Bounds& ChildPosition = Child->Position;
				// for any children which have a top/bottom scale type of PercentageOwner, invalidate
				if ( ChildPosition.IsPositionCurrent(UIFACE_Top) && ChildPosition.GetScaleType(UIFACE_Top) == EVALPOS_PercentageOwner )
				{
					Child->InvalidatePosition(UIFACE_Top);
				}
				if ( ChildPosition.IsPositionCurrent(UIFACE_Bottom) && ChildPosition.GetScaleType(UIFACE_Bottom) == EVALPOS_PercentageOwner )
...
/** StyleSubscriber에 속해있는 UIComponent들을 반환 
 *
 * @param	bRecursive		Children에 대해서도 Component를 찾음.
 **/
void UUIObject::GetUIComponents(TArray<class UUIComponent*>& out_Components,UBOOL bRecursive/* =FALSE */)
{
	TArray<UUIObject*> ObjectsToFind;

	ObjectsToFind.AddItem(this);
	if( bRecursive )
	{
		TArray<UUIObject*> ChildObjects = GetChildren(TRUE, &ObjectsToFind);
		ObjectsToFind.Append(ChildObjects);
	}

	for( INT ObjIndex = 0 ; ObjIndex < ObjectsToFind.Num() ; ObjIndex++ )
	{
		UUIObject* Obj = ObjectsToFind(ObjIndex);
		if( Obj == NULL )
			continue;

		for( INT StyleSubscriberIndex = 0 ; StyleSubscriberIndex < Obj->StyleSubscribers.Num() ; StyleSubscriberIndex++ )
		{
			UObject* NewObj = Obj->StyleSubscribers(StyleSubscriberIndex)->GetUObjectInterfaceUIStyleResolver();
			UUIComponent* Comp;
			if( (Comp = Cast<UUIComponent>(NewObj)) != NULL )
			{
				out_Components.AddUniqueItem(Comp);
			}
		}
	}
}
...
UBOOL UUIObject::SetWidgetStyle( UUIStyle* NewStyle, FStyleReferenceId StyleProperty/*=FStyleReferenceId()*/, INT ArrayIndex/*=INDEX_NONE*/ )
{
	...
	for ( INT ObjIndex = 0; ObjIndex < StyleOwners.Num(); ObjIndex++ )
	{
		...
		{
//			위에서 StyleReferences.MultiFind();할때 References가 우리가 생각하는것과 반대의 순서로 들어온다
//			예를 들면 UList에서	Style						예상 Index		실제 Index
//								ItemOverlayStyle[Normal] ,		0				2
//								ItemOverlayStyle[Active] ,		1				1
//								ItemOverlayStyle[Selected ,		2				0
//
//			우선 임시로 바꿔놓은 것이므로 다시 순서가 뒤바뀌면 원래대로 돌려놓으시길 바람.
//			if ( ArrayIndex == INDEX_NONE || Index == ArrayIndex )
			if ( ArrayIndex == INDEX_NONE || (References.Num() - Index - 1) == ArrayIndex )
			{
...
void UUIObject::GetStyleReferences( TMultiMap<FStyleReferenceId,FUIStyleReference*>& out_StyleReferences, FStyleReferenceId TargetStyleRef/*=FStyleReferenceId()*/, UObject* SearchObject/*=NULL*/ )
{
	...
	for ( INT ObjIndex = 0; ObjIndex < StyleReferenceOwners.Num(); ObjIndex++ )
	{
		...
		for ( INT RefIndex = 0; RefIndex < StyleReferences.Num(); RefIndex++ )
		{
			...
			||	(TargetStyleRef.StyleProperty == StyleRef.StyleProperty && TargetStyleRef.StyleReferenceTag == StyleRef.StyleReferenceTag) )
			{
 ...
 */
FUIDataStoreBinding* UUIObject::GetDataBinding(const FName& PropertyName)
{
	FUIDataStoreBinding* DataBinding = NULL;

	//@fixme ronp - this doesn't account for arrays of data store bindings (either static or dynamic)
	UStructProperty* StructProperty = FindFieldWithFlag<UStructProperty, CLASS_IsAUStructProperty>(GetClass(), PropertyName);
	if(StructProperty != NULL)
	{
		DataBinding = (FUIDataStoreBinding*)((BYTE*)this + StructProperty->Offset);
	}
	...
	if ( !IsA(UUIPrefab::StaticClass()) && !IsInUIPrefab() )
	{
		...
		if ( SubscriberInterface != NULL )
		{
			...
			if ( BindingIndex == UCONST_TOOLTIP_BINDING_INDEX )
			{
				...
				UUIScene* OwnerScene = GetScene();
				UUIToolTip* ToolTipObj = OwnerScene ? OwnerScene->GetActiveToolTip() : NULL;

				// @TODO 툴팁에 대해 각각의 Owner관계를 만들어 this == ToolTipObj->Owner 체크
				// ( 현재 구현에선 Tooltip은 Scene당 하나씩 생성, Owner는 없다 
				// 어느 Object에 의해 Tooltip이 열렸는지 알 수 없다)

				// Resolve 한 툴팁값을 화면에 반영
				if( ToolTipObj )
					ToolTipObj->LinkBinding( &ToolTip );
			}
			else if ( BindingIndex == UCONST_CONTEXTMENU_BINDING_INDEX )
			{
...
UUIToolTip* UUIObject::ActivateToolTip()
{
	...
	if ( !DELEGATE_IS_SET(OnQueryToolTip) || delegateOnQueryToolTip(this, NewToolTip) )
	{
		...
		if ( ToolTip )
		{
			...
			if ( NewToolTip == NULL )
			{
				if( ToolTipToOverride != NULL )
				{
					NewToolTip = ToolTipToOverride;
				}
				else
				{
					UUIScene* Scene = GetScene();
...
UBOOL UUIObject::TickSequence(FLOAT DeltaTime, FUIAnimSeqRef& AnimSeqRef)
{
	...
	FLOAT SeqDuration = Seq->InTimeMax - Seq->InTimeMin;

	FLOAT AnimTime = AnimSeqRef.AnimTime + (DeltaTime * AnimSeqRef.PlaybackRate);
	FLOAT Position = AnimTime / SeqDuration;
	...
	for (INT TrackIdx = 0 ; TrackIdx < Seq->Tracks.Num(); TrackIdx++)
	{
		...
		if ( Position >= 1.0f )
		{
			...
			if ( !AnimSeqRef.bIsLooping )
			{
				...
				Seq->ApplyAnimation( TargetObj? TargetObj : this, TrackIdx, 1.0, AnimSeqRef );
		...
		if(bSkipNormalUpdate==false)
		{
			...
			Seq->ApplyAnimation(TargetObj ? TargetObj : this, TrackIdx, Position, AnimSeqRef);
		}
	}

	// Step 5. Store the animation time and get on with it

	AnimSeqRef.AnimTime = appFmod(AnimTime , SeqDuration);
...
void UUIObject::AnimSetScale(FVector NewScale)
{
	ScaleValue = FVector(NewScale.X, NewScale.Y, 1.f);
}
...
void UUILabel::Render_Widget(FCanvas* Canvas)
{
	...
	if ( StringRenderComponent != NULL )
	{
		//! 폰트를 화면 비율에 맞춰서 크기를 조정하도록 처리(2007/01/11 고광록).
		FVector2D vpSize;
		OwnerScene->GetViewportSize(vpSize);
		Canvas->SetFontRatioScaling(bFontRatioScaling, vpSize.X/FontScreenWidth, vpSize.Y/FontScreenHeight);
		StringRenderComponent->Render_String(Canvas);
		Canvas->SetFontRatioScaling(false);
	}
}
...
UBOOL UUILabel::RefreshSubscriberValue( INT BindingIndex/*=INDEX_NONE*/ )
{
	...
	else if ( StringRenderComponent != NULL && IsInitialized() )
	{
		...
		/** 활성화/비활성화 , 보임/안보임 처리를 데이터 프로바이더에서 받아온다 */
	...
	if ( PropertyThatChanged.Num() > 0 )
	{
		...
		if ( MemberProperty != NULL )
		{
			...
			if ( PropertyName == TEXT("DataSource") )
			{
				...
				{
					if ( StringRenderComponent->AutoSizeParameters[UIORIENT_Horizontal].bAutoSizeEnabled || StringRenderComponent->AutoSizeParameters[UIORIENT_Vertical].bAutoSizeEnabled || 
...
UBOOL UUIImage::RefreshSubscriberValue( INT BindingIndex/*=INDEX_NONE*/ )
{
	...
	FString OriginalMarkupString = ImageDataSource.MarkupString;

	// 마크업 스트링안에 이미지 마크업이 있을 수 있으므로 이미지값이 있을때까지 찾아본다
	while ( ImageDataSource.ResolveMarkup( this ) )
	{
		FUIProviderFieldValue ResolvedValue(EC_EventParm);
		if ( ImageDataSource.GetBindingValue( ResolvedValue ) )
		{
			if( ! ResolvedValue.HasValue() )
				break;

			if( ResolvedValue.ImageValue != NULL )
			{
				SetValue(ResolvedValue.ImageValue);
				FUIStringNode_Image* ImageNode = NULL;
				if( ImageComponent != NULL &&
					ResolvedValue.CustomStringNode != NULL && ResolvedValue.CustomStringNode->IsImageNode() &&
					!(ImageNode = (FUIStringNode_Image*)ResolvedValue.CustomStringNode)->ForcedDrawCoords.IsZero())
				{
					ImageComponent->SetCoordinates( ImageNode->ForcedDrawCoords );
				}
			...
			{
				ImageDataSource.MarkupString = ResolvedValue.StringValue;				
			}
			else
			{
				//이미지 값도 아니고 문자열도 아니면 더이상 이미지마크업을 찾을 수 없다.
				break;
			}
		}
	}

	// MarkupString과 ResolvedDataStore가 꼭 같을 필요는 없다.
	// ResolvedDataStore만이 ResolveSubscriberNotify 에서 Subscriber(this)를 뺄때 사용된다.
	ImageDataSource.MarkupString = OriginalMarkupString;

	//// 원래 코드
	//FUIProviderFieldValue ResolvedValue(EC_EventParm);
	//if( ImageDataSource.ResolveMarkup( this ) )
	//{
	...
	if ( EventParms.InputAliasName == UIKEY_Clicked )
	{
		...
		else if ( EventParms.EventType == IE_Released )
		{
			...
			if ( IsPressed(EventParms.PlayerIndex) )
			{
				...
				if ( !IsCursorInputKey(EventParms.InputKeyName) || !GetCursorPosition(MousePos, GetScene()) || ContainsPoint(MousePos) || GHackEmulatePlayerInputDoNotCheckMouseCursor )
				{
					// Play the ClickedCue
...
UBOOL UUILabelButton::RefreshSubscriberValue( INT BindingIndex/*=INDEX_NONE*/ )
{
	...
	else if ( StringRenderComponent != NULL && IsInitialized() )
	{
		...
		UpdateWidgetWithProvider(CaptionDataSource);
	}
...
void UUICheckbox::SetValue( UBOOL bShouldBeChecked, INT PlayerIndex/*=INDEX_NONE*/ )
{
	if ( bIsChecked != bShouldBeChecked )
	{
		...
		if( ValueDataSource )
		{
			...
			this->SetDataStoreFieldValue( ValueDataSource.MarkupString, FieldValue, GetScene() );
		}

		// activate the OnCheckValue changed event to notify anyone who cares that this checkbox's value has been changed
		ActivateEventByClass(PlayerIndex,UUIEvent_CheckValueChanged::StaticClass(), this, FALSE, &OutputLinkIndexes);
	}
}
...
UBOOL UUICheckbox::RefreshSubscriberValue( INT BindingIndex/*=INDEX_NONE*/ )
{
	...
	else if ( ValueDataSource.ResolveMarkup( this ) )
	{
		...
		UpdateWidgetWithProvider(ValueDataSource);
	}
...
void UUIEditBox::Initialize( UUIScene* inOwnerScene, UUIObject* inOwner/*=NULL*/ )
{
	...
	TArray<UUILabel*> IMELabels;

	if( CandidateLabel == NULL )
	{
		CandidateLabel = CastChecked<UUILabel>(CreateWidget( this, UUILabel::StaticClass(), GetArchetype<UUIEditBox>()->CandidateLabel, TEXT("CandidateLabel")) );
		IMELabels.AddItem(CandidateLabel);
	}

	if( ReadingLabel == NULL )
	{
		ReadingLabel = CastChecked<UUILabel>(CreateWidget( this, UUILabel::StaticClass(), GetArchetype<UUIEditBox>()->ReadingLabel, TEXT("ReadingLabel")) );
		IMELabels.AddItem(ReadingLabel);
	}

	if( IndicatorLabel == NULL )
	{
		IndicatorLabel = CastChecked<UUILabel>(CreateWidget( this, UUILabel::StaticClass(), GetArchetype<UUIEditBox>()->IndicatorLabel, TEXT("IndicatorLabel")) );
		IMELabels.AddItem(IndicatorLabel);
	}

	TArray<UUIButton*> PageButtons;
	if( PageUpButton == NULL )
	{
		PageUpButton = CastChecked<UUINumericEditBoxButton>(CreateWidget( this, UUINumericEditBoxButton::StaticClass(), GetArchetype<UUIEditBox>()->PageUpButton, TEXT("PageUpButton") ));
		PageButtons.AddItem(PageUpButton);
	}

	if( PageDownButton == NULL )
	{
		PageDownButton = CastChecked<UUINumericEditBoxButton>(CreateWidget( this, UUINumericEditBoxButton::StaticClass(), GetArchetype<UUIEditBox>()->PageDownButton, TEXT("PageDownButton") ));
		PageButtons.AddItem(PageDownButton);
	}

	const INT ControlFlags=UCONST_PRIVATE_NotFocusable|UCONST_PRIVATE_NotDockable|UCONST_PRIVATE_Protected;
	for( INT LabelIndex = 0 ; LabelIndex < IMELabels.Num() ; LabelIndex++ )
	{
		UUILabel* EachLabel = IMELabels(LabelIndex);
		if( EachLabel != NULL )
		{
			if( EachLabel->StringRenderComponent != NULL )
			{
				for( INT Orient = 0 ; Orient < UIORIENT_MAX ; Orient++ )
					EachLabel->StringRenderComponent->AutoSizeParameters[Orient].bAutoSizeEnabled = TRUE;
				EachLabel->StringRenderComponent->WrapMode = CLIP_Wrap;
			}
			EachLabel->eventSetVisibility(FALSE);
			EachLabel->SetPrivateBehavior(ControlFlags, TRUE);
			InsertChild(EachLabel);
		}
	}

	for( INT ButtonIndex = 0 ; ButtonIndex < PageButtons.Num() ; ButtonIndex++ )
	{
		UUIButton* EachButton = PageButtons(ButtonIndex);
		if( EachButton )
		{
			EachButton->eventSetVisibility(FALSE);
			EachButton->SetPrivateBehavior(ControlFlags, TRUE);
			CandidateLabel->InsertChild(EachButton);
		}
	}
...
void UUIEditBox::Render_Widget( FCanvas* Canvas )
{
  ...
  * @param	bInvalidateStyleData	if TRUE, the resolved style is different than the style that was previously resolved by this style reference.
  */
 void UUIEditBox::OnStyleResolved( UUIStyle* ResolvedStyle, const struct FStyleReferenceId& StyleProperty, INT ArrayIndex, UBOOL bInvalidateStyleData )
 {
 	Super::OnStyleResolved(ResolvedStyle,StyleProperty,ArrayIndex,bInvalidateStyleData);

	const TCHAR* szStyleProperty = *StyleProperty.GetStyleReferenceName();
 	if ( StyleProperty.GetStyleReferenceName() == TEXT("PrimaryStyle") )
 	{
 		if ( StringRenderComponent != NULL )
 		{
			UUIStyle_Combo* ComboStyle = Cast<UUIStyle_Combo>(PrimaryStyle.GetStyleData(GetCurrentState()));
			check(ComboStyle);
 			StringRenderComponent->SetStringStyle(ComboStyle);
...
UBOOL UUIEditBox::DeleteCharacter( INT PlayerIndex/*=INDEX_NONE*/ )
{
	...
	if ( StringRenderComponent != NULL )
	{
		FString& UserText = StringRenderComponent->UserText;
...
//	debugf(NAME_Input, TEXT("%s::ProcessInputKey	Key:%s (%s)"), *GetWidgetPathName(), *EventParms.InputKeyName.ToString(), *UUIRoot::GetInputEventText(EventParms.EventType));
	if ( EventParms.EventType == IE_Pressed 
	...
	else if( EventParms.EventType == IE_Released )
	{
		...
		{
			if ( UIKey == UIKEY_SubmitText )
			{
				FInputCompositionStringData CompStrData(IMECOMPOSITION_Result,TEXT("\n"));
				if( StringRenderComponent != NULL && StringRenderComponent->WrapMode == CLIP_Wrap )
				{
					bResult = ProcessInputChar( EventParms.PlayerIndex, FInputCompositionStringData( CompStrData ) );
				}
				else
				{
					NotifySubmitText(EventParms.PlayerIndex);
					// Submit 후 Clear해야할때만 CaretPosition을 원점으로 잡아준다
					// 따라서 SetCaretPosition은 NotifySubmitText안의 bClearValue 시에만 사용된다
/*! Caret위치를 처음으로 이동되게 하는 부분 수정.(2008/01/31)
					StringRenderComponent->SetCaretPosition(0);
					StringRenderComponent->CertainEndPos = 0;
*/
					bResult = TRUE;
				}
			}
			else if ( UIKey == UIKEY_Clicked )
				bResult = ProcessControlChar(EventParms);
		}
	}
...
UBOOL UUIEditBox::ProcessControlChar( const FSubscribedInputEventParameters& EventParms )
{
	...
	if ( StringRenderComponent != NULL &&
		(!StringRenderComponent->bComposing || (UIMEComposition::StaticGetImeProperty() & UCONST_IMEPROP_AT_CARET)) )
	{
		if ( EventParms.InputAliasName == UIKEY_Clicked )
		{
			...
			case IE_Pressed:
				...
				// 아바에서 클릭했을때 포커스를 잡고
				// 문자열 중간을 클릭했을때 캐럿 위치를 잡기위해 추가한 코드
				{
					// when not in focus, take the focus ( conventionally, generated by 'LeftMouseClick' )
					...
					// and then set a new position of the caret near the position of mouse
					if( StringRenderComponent != NULL && GetSceneClient() != NULL)
					{
						FRenderParameters CaretParameters;
						FLOAT PaddingLeft = StringRenderComponent->AutoSizeParameters[UIORIENT_Horizontal].GetPaddingValue( UIORIENT_Horizontal, EVALPOS_PixelOwner, this);
						FLOAT PaddingTop = StringRenderComponent->AutoSizeParameters[UIORIENT_Vertical].GetPaddingValue( UIORIENT_Vertical, EVALPOS_PixelOwner, this );
						CaretParameters.DrawX = RenderBounds[UIFACE_Left] + PaddingLeft;
						CaretParameters.DrawY = RenderBounds[UIFACE_Top] + PaddingTop;
						CaretParameters.DrawFont = StringRenderComponent->ValueString->StringStyleData.DrawFont;

						UUIString::StringSize(CaretParameters, *StringRenderComponent->GetValue());

						if( (CaretParameters.DrawX <= MousePosition.X && MousePosition.X <= CaretParameters.DrawX + CaretParameters.DrawXL) &&
							(RenderBounds[UIFACE_Top] <= MousePosition.Y && MousePosition.Y <= RenderBounds[UIFACE_Bottom]) )
						{
							// @TODO : OPTIMIZE. String이 길어질수록 o(n^2)의 시간이 소모됨. o(n)으로 고치시길 바람
							FString RenderStr = StringRenderComponent->GetValue();
							for(INT i = 0 ; i < RenderStr.Len() ; i++)
							{
								UUIString::StringSize( CaretParameters , *RenderStr.Mid(0, i + 1) );
								if( CaretParameters.DrawX + CaretParameters.DrawXL >= MousePosition.X)
								{
									StringRenderComponent->SetCaretPosition(i);
									break;
								}
							}
						}
		...
		// IE_Released is only handled for click input
		else if ( EventParms.EventType != IE_Released )
		{
			FString& UserText = StringRenderComponent->UserText;
			...
			else if ( EventParms.InputAliasName == UIKEY_Backspace )
			{
				...
				{
					INT DecrCharCount = 1;
					StringRenderComponent->SetCaretPosition(StringRenderComponent->StringCaret.CaretPosition - DecrCharCount);
					StringRenderComponent->InsertPos = Max(0, StringRenderComponent->InsertPos - DecrCharCount);
					StringRenderComponent->CertainEndPos = Max(0, StringRenderComponent->CertainEndPos - DecrCharCount);

					// @TODO : Delete a Combined Character.
			...
			{
				INT CaretPosition = StringRenderComponent->StringCaret.CaretPosition;
				UBOOL bNoMoreChars = (MaxCharacters != 0 && UserText.Len() == MaxCharacters);
				if ( StringRenderComponent->StringCaret.CaretPosition < UserText.Len() )
				{
					INT NewCaretPosition = CaretPosition + 1;
			...
			{
				StringRenderComponent->SetCaretPosition( UserText.Len() );
				StringRenderComponent->SetValue(UserText);
				bResult = TRUE;
			}
			// 엔터가 릴리즈될때 SubmitText해야 SubmitText한후에 다른 Widget들이 Release에 반응하지 않는다.
			//else if ( UIKey == UIKEY_SubmitText )
			//{
			//	bResult = TRUE;
			//	NotifySubmitText(EventParms.PlayerIndex);
			//}
...
UBOOL UUIEditBox::ProcessInputCandidate( INT PlayerIndex, const FInputCandidateStringData& CandStrData )
{
	FString CandidateString;

	INT PageEnd = Min(CandStrData.PageStart + CandStrData.PageSize, CandStrData.Count);
	UBOOL bPrev = CandStrData.PageStart > 0;
	UBOOL bNext = (CandStrData.PageStart + CandStrData.PageSize) < CandStrData.Count;
	PageUpButton->SetEnabled( bPrev, GetBestPlayerIndex() );
	PageDownButton->SetEnabled( bNext, GetBestPlayerIndex() );

	//static TCHAR *PrevMarkTrue = TEXT("◀");
	//static TCHAR *PrevMarkFalse = TEXT("◁");
	//static TCHAR *NextMarkTrue = TEXT("▶");
	//static TCHAR *NextMarkFalse = TEXT("▷");

	UBOOL bVisible = CandStrData.StrList.Num();

	BYTE KBDLang = UIMEComposition::StaticGetImeLanguage();
	if( KBDLang == KBDINPUTLANG_Korean )
	{
		for( INT i = CandStrData.PageStart ; i < PageEnd ; i++)
			CandidateString += (appItoa(i - CandStrData.PageStart + 1) + FString(TEXT(".")) + CandStrData.StrList(i) + TEXT(" "));
		PageUpButton->SetDockTarget( UIFACE_Right, CandidateLabel, UIFACE_Left );
		PageUpButton->SetDockTarget( UIFACE_Top, CandidateLabel, UIFACE_Top );
		PageUpButton->SetDockTarget( UIFACE_Bottom, CandidateLabel, UIFACE_Bottom );
		PageDownButton->SetDockTarget( UIFACE_Left, CandidateLabel, UIFACE_Right );
		PageDownButton->SetDockTarget( UIFACE_Top, CandidateLabel, UIFACE_Top );
		PageDownButton->SetDockTarget( UIFACE_Bottom, CandidateLabel, UIFACE_Bottom );
	...
	else
	{
		for( INT i = CandStrData.PageStart ; i < PageEnd ; i++ )
		{
			CandidateString += (appItoa(i - CandStrData.PageStart + 1) + FString(TEXT(".")) + CandStrData.StrList(i) + (i == CandStrData.Selection ? TEXT("*") : TEXT("")) + TEXT("\\n"));
		}

		PageDownButton->SetDockTarget( UIFACE_Right, CandidateLabel, UIFACE_Right );
		PageDownButton->SetDockTarget( UIFACE_Bottom, CandidateLabel, UIFACE_Bottom);
		PageUpButton->SetDockTarget( UIFACE_Right, PageDownButton, UIFACE_Left );
		PageUpButton->SetDockTarget( UIFACE_Top, PageDownButton, UIFACE_Top );
		PageUpButton->SetDockTarget( UIFACE_Bottom, PageDownButton, UIFACE_Bottom );
	}

	if( CandidateLabel != NULL && CandidateLabel->StringRenderComponent != NULL && CandidateLabel->StringRenderComponent->ValueString != NULL)
	{
		check(StringRenderComponent && StringRenderComponent->ValueString);
		CandidateLabel->SetValue( CandidateString);

		FString FrontString = StringRenderComponent->UserText.Left(StringRenderComponent->StringCaret.CaretPosition);
		INT StrXL, StrYL, DefaultXL, DefaultYL;
		StringSize( StringRenderComponent->ValueString->StringStyleData.DrawFont, StrXL, StrYL, *FrontString );
		StringSize( StringRenderComponent->ValueString->StringStyleData.DrawFont, DefaultXL, DefaultYL, TEXT("A") );

		FLOAT Left = GetPosition(UIFACE_Left, EVALPOS_PixelViewport) + StrXL;
		FLOAT Top = GetPosition(UIFACE_Top, EVALPOS_PixelViewport);
		// adjust position when out-bounded

		//CandidateLabel->SetPosition( Left, Top, Left + GetBounds(UIORIENT_Horizontal,EVALPOS_PixelViewport), Top + GetBounds(UIORIENT_Vertical, EVALPOS_PixelViewport), EVALPOS_PixelViewport );
		CandidateLabel->SetDockTarget( UIFACE_Bottom, this, UIFACE_Top, 0 );
		CandidateLabel->SetDockTarget( UIFACE_Left, this, UIFACE_Left, StrXL );
		CandidateLabel->eventSetVisibility( bVisible );
	}

	return TRUE;
}

UBOOL UUIEditBox::ProcessInputReadingString( INT PlayerIndex, const FInputReadingStringData& ReadStrData )
{
	if( ReadingLabel != NULL && ReadingLabel->StringRenderComponent != NULL )
	{
		check(StringRenderComponent && StringRenderComponent->ValueString);
		...
		FLOAT Left = GetPosition(UIFACE_Left, EVALPOS_PixelViewport) + StrXL;
		FLOAT Top = GetPosition(UIFACE_Top, EVALPOS_PixelViewport) - DefaultYL;
		ReadingLabel->SetValue(ReadStrData.ReadingString);
		ReadingLabel->SetDockTarget(UIFACE_Left, this, UIFACE_Left, StrXL);
		ReadingLabel->SetDockTarget(UIFACE_Top,this, UIFACE_Bottom, 0);
		//ReadingLabel->SetPosition( Left, Top, Left + GetBounds(UIORIENT_Horizontal,EVALPOS_PixelViewport), Top + GetBounds(UIORIENT_Vertical, EVALPOS_PixelViewport), EVALPOS_PixelViewport );
		ReadingLabel->eventSetVisibility( ReadStrData.ReadingString.Len() > 0 );
	}
	return TRUE;
}
 ...
 * @return	TRUE to consume the character, false to pass it on.
 */
UBOOL UUIEditBox::ProcessInputChar( INT PlayerIndex, const FInputCompositionStringData& CompStrData )
{
	UBOOL bResult = FALSE;
	UBOOL bActOnCharEvent = FALSE;

	if ( StringRenderComponent != NULL )
	{
		if( CompStrData.CompType == IMECOMPOSITION_InputLangChanged )
		{
			// Clear LabelText
			SetValue( StringRenderComponent->BackupUserText = FString(), PlayerIndex, TRUE );

			if( UIMEComposition::StaticGetImeLanguage() == KBDINPUTLANG_Korean )
			{
				PageUpButton->SetDockTarget( UIFACE_Right, CandidateLabel, UIFACE_Left );
				PageUpButton->SetDockTarget( UIFACE_Top, CandidateLabel, UIFACE_Top );
				PageUpButton->SetDockTarget( UIFACE_Bottom, CandidateLabel, UIFACE_Bottom );
		...
		{
			// @TODO : Notify to stop the composing process

			// send a notification that the user attempted to change the value of a read-only editbox
			//@todo - play a sound?  fire an event?
		}
		else if ( CompStrData.CompType == IMECOMPOSITION_Start )
		{
			StringRenderComponent->bComposing = TRUE;
			if( UIMEComposition::StaticGetImeProperty() & UCONST_IMEPROP_AT_CARET )
				StringRenderComponent->BackupUserText = StringRenderComponent->UserText;
			bResult = TRUE;
		}
		else if ( CompStrData.CompType == IMECOMPOSITION_End )
		{
			StringRenderComponent->bComposing = FALSE;
			bResult = TRUE;
		}
		else if ( CompStrData.CompType == IMECOMPOSITION_Result ||
			CompStrData.CompType == IMECOMPOSITION_Composition ||
			CompStrData.CompType == IMECOMPOSITION_None )
		{
			bResult = StringRenderComponent->OnImeComposition( CompStrData );
		}
	}

	if( bResult == TRUE && bActOnCharEvent )
		ActivateEventByClass( PlayerIndex, UUIEvent_OnChar::StaticClass(), this);
...
UBOOL UUIEditBox::IsValidCharacter( TCHAR Character ) const
{
	...
	switch(CharacterSet)
	{
		...
		case CHARSET_All:
		{
			...
			UBOOL bAcceptLineFeed = StringRenderComponent && StringRenderComponent->WrapMode == CLIP_Wrap;
			bResult = (Character >= TCHAR(' ') || (bAcceptLineFeed && appIsLinebreak(Character)) )
					&& Character != 127;
			break;
		}
...
void UUIEditBox::NotifyValueChanged( INT PlayerIndex/*=INDEX_NONE*/, INT NotifyFlags/*=0*/ )
{
	...
	ActivateEventByClass(PlayerIndex,UUIEvent_TextValueChanged::StaticClass(),this);
}
...
void UUIEditBox::NotifySubmitText( INT PlayerIndex/*=INDEX_NONE*/ )
{
	...
	FindEventsOfClass(UUIEvent_SubmitTextData::StaticClass(), SubmitTextEvents);
	for ( INT EventIndex = 0; EventIndex < SubmitTextEvents.Num(); EventIndex++ )
	{
		...
		TextEvent->Value = StringRenderComponent->UserText;

		// determine which widget owns this event
		UUIScreenObject* EventOwner = NULL;
		UUISequence* EventOwnerSequence = Cast<UUISequence>(TextEvent->ParentSequence);
		...
		if ( TextEvent->ConditionalActivateUIEvent(PlayerIndex,EventOwner, this) )
		{
			// allow the event to indicate that it wants to clear the existing value of the editbox
			bClearValue = bClearValue || TextEvent->bClearValue;
		}
	}
	...
	if ( bClearValue )
	{
		...
		// 제출 후 값을 비워야할때만 커서를 앞으로 이동시켜줘여한다.
		// 비워야할 할때 : 로비/대기방 등의 채널 채팅창
		// 비우지 않는경우 : 게임중 채팅창
...
void UUIEditBox::SetValue( const FString& NewText, INT PlayerIndex/*=INDEX_NONE*/, UBOOL bSkipNotification/*=FALSE*/ )
{
	...
	if ( StringRenderComponent != NULL )
	{
		FString PreviousValue = StringRenderComponent->GetValue(TRUE);
		StringRenderComponent->SetValue(NewText);

		if ( !bSkipNotification && StringRenderComponent->GetValue(TRUE) != PreviousValue )
		{
 ...
 */
FString UUIEditBox::GetValue( UBOOL bReturnProcessedText/*=TRUE*/ )
{
	if ( StringRenderComponent != NULL )
	{
		return StringRenderComponent->GetValue(bReturnProcessedText);
	}
...
UBOOL UUIEditBox::RefreshSubscriberValue( INT BindingIndex/*=INDEX_NONE*/ )
{
	...
	else if ( StringRenderComponent != NULL && IsInitialized() )
	{
		...
		FUIProviderFieldValue FieldValue(EC_EventParm);
		if( DataSource.ResolveMarkup(this) && DataSource.GetBindingValue(FieldValue) )
		{
		...
		else if ( DataSource.MarkupString.Len() > 0 )
		{
			StringRenderComponent->SetValue(DataSource.MarkupString);
		}
		else
...
{
	UBOOL bRet = Super::GainFocus( FocusedChild, PlayerIndex );	
	
	StringRenderComponent->StringCaret.bDisplayCaret = TRUE;
	//포커스 되었을때 캐럿을 문장 맨 뒤에 놓는다.
	StringRenderComponent->SetCaretPosition( StringRenderComponent->UserText.Len() );

	return bRet;	
}

UBOOL UUIEditBox::LoseFocus( UUIObject* FocusedChild, INT PlayerIndex )
{
	check(StringRenderComponent);
	StringRenderComponent->StringCaret.bDisplayCaret = FALSE;
	if( StringRenderComponent->bComposing && UIMEComposition::StaticGetImeLanguage() != KBDINPUTLANG_Korean )
	{
		if( UIMEComposition::StaticNotifyAction( IMENOTIFY_CompositionStr, IMENOTIFYCOMPSTR_Complete, 0 ) )
		{
			ProcessInputChar( PlayerIndex, FInputCompositionStringData ( IMECOMPOSITION_Result, StringRenderComponent->CurrCompStr ) );
			ProcessInputChar( PlayerIndex, FInputCompositionStringData ( IMECOMPOSITION_End ) );
		}	
	}

	CandidateLabel->eventSetVisibility(FALSE);
	ReadingLabel->eventSetVisibility(FALSE);
	IndicatorLabel->eventSetVisibility(FALSE);

	return Super::LoseFocus( FocusedChild, PlayerIndex );	
}
///}}ENDAVA
...
/* ==========================================================================================================
	UConsoleEntry
========================================================================================================== */
static INT CursorPromptOffset = INDEX_NONE;
/**
...
{
	UUIComp_DrawString* StringRenderComponent = NULL;
	UUIObject* InputChild = NULL;
	if ( InputText != NULL  )
	{
		InputChild = InputText;
		StringRenderComponent = InputText->StringRenderComponent;
	}
 	else if ( InputBox != NULL )
 	{
 		InputChild = InputBox;
 		StringRenderComponent = InputBox->StringRenderComponent;
 	}

	if ( InputChild != NULL && bRenderCursor )
	{
		//@todo - move this to Initialize()
		if ( CursorPromptOffset == INDEX_NONE )
		{
			CursorPromptOffset = FString(UCONST_ConsolePromptText).Len();
		}
		...
		INT WrappedCursorPosition = CursorPosition + CursorPromptOffset;
		...
		DrawString(Canvas,InputChild->RenderBounds[UIFACE_Left] + Parameters.DrawXL, InputChild->RenderBounds[UIFACE_Top] + CursorYOffset + InputChild->DockTargets.DockPadding[UIFACE_Bottom],
			TEXT("_"), Parameters.DrawFont, StringRenderComponent->ValueString->StringStyleData.TextColor);
	}
}
...
FLOAT UUIScrollbar::GetScrollZoneExtent( FLOAT* ScrollZoneStart/*=NULL*/ ) const
{
	...
	{
		*ScrollZoneStart = ScrollZoneBegin;
	}
	return Max(0.f, ScrollZoneEnd - ScrollZoneBegin);
}
void UUIScrollbar::execGetScrollZoneExtent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
{
	return BarWidth.GetValue(this,EVALPOS_PixelViewport);
}
...
void UUIScrollbar::Initialize( UUIScene* inOwnerScene, UUIObject* inOwner /*=NULL*/ )
{
	...
	{
		IncrementButton = Cast<UUINumericEditBoxButton>(CreateWidget(this, UUINumericEditBoxButton::StaticClass(), NULL, TEXT("IncrementButton")));
		check(IncrementButton);
	}
	
	if(DecrementButton == NULL)
	{
		DecrementButton = Cast<UUINumericEditBoxButton>(CreateWidget(this, UUINumericEditBoxButton::StaticClass(), NULL, TEXT("DecrementButton")));
...
void UUIScrollbar::OnStyleResolved( UUIStyle* ResolvedStyle, const FStyleReferenceId& StylePropertyId, INT ArrayIndex, UBOOL bInvalidateStyleData )
{
	...
	{
		// assign the incrementstyle to the increment button
		IncrementButton->SetWidgetStyle(ResolvedStyle);
	}
	else if( StylePropertyName == TEXT("DecrementStyle"))
	{
		// assign the decrementstyle to the decrement button
		DecrementButton->SetWidgetStyle(ResolvedStyle);
	}
	else if( StylePropertyName == TEXT("MarkerStyle"))
	{
		// assign the markerstyle to the marker button
		MarkerButton->SetWidgetStyle(ResolvedStyle);
	}
}
...
void UUIScrollbar::UpdateMarkerPosition( FLOAT PositionChange )	// MoveGripBy
{
	...
	if(ScrollbarOrientation == UIORIENT_Horizontal)
	{
		...
		MarkerButton->SetPosition( MarkerButton->GetPosition(UIFACE_Left,EVALPOS_PixelViewport) + ValidPositionChange, UIFACE_Left, EVALPOS_PixelViewport );
	...
	{
		//Change the marker's position
		MarkerButton->SetPosition( MarkerButton->GetPosition(UIFACE_Top,EVALPOS_PixelViewport) + ValidPositionChange, UIFACE_Top, EVALPOS_PixelViewport );
	...
	UUIList* ListOwner = Cast<UUIList>(GetOwner());
	MousePositionDelta += ListOwner ? PositionChange : ValidPositionChange;
	...
	{	
		...
		{
			delegateOnScrollActivity( Nudges, bPositionMaxedOut );
		}

		MousePositionDelta = 0.f;
	}

	//! if문을 주석처리하면 스크롤 때 2번 클릭에 1번씩 증감되지 않는다.(2007/01/09 고광록)
	...
	// 현재의 스크롤바 구현으로는 맨끝단에 도달했는지 안했는지 정확하게 확인하기가 어렵다.
	// 따라서 끝단에 도달한것처럼 보이나 마지막 아이템이 안보이는 경우가 있으며
	// 끝단에 도달하더라도 MousePositionDelta를 0으로 만들지 말고 
	// 해당 MousePositionDelta를 보내서 리스트가 완전히 끝에 도달했는지 확인해야한다

	if( !ListOwner && Abs(ValidPositionChange - PositionChange) > KINDA_SMALL_NUMBER )
	{
...
//debugf(TEXT("%s::SetMarkerSize: %f"), *GetName(), PercentageSize);
...
//debugf(TEXT("%s::SetMarkerPosition: %f"), *GetName(), PositionPercentage);
...
void UUIScrollbar::SetNudgeSizePixels( FLOAT NudgePixels )
{
	...
	NudgePercent = ZoneSize > 0.f ? (NudgeValue / ZoneSize) : NudgeValue;
}
...
{
	FLOAT ScrollZoneStart;
	const FLOAT AvailableScrollZoneSize = GetScrollZoneExtent(&ScrollZoneStart) - MarkerButton->Position.GetBoundsExtent(MarkerButton, (EUIOrientation)ScrollbarOrientation, EVALPOS_PixelViewport);
	const FLOAT NewMarkerPosition = ScrollZoneStart + AvailableScrollZoneSize * MarkerPosPercent;

	// Calculate final position of the marker in pixels
	//@fixme ronp - prevent this from triggering another scene update
	MarkerButton->SetPosition( NewMarkerPosition, ScrollbarOrientation == UIORIENT_Vertical ? UIFACE_Top : UIFACE_Left, EVALPOS_PixelViewport );
	return;
#if 0
	if(ScrollbarOrientation == UIORIENT_Vertical)
	{
		// Vertical Orientation
		MarkerButton->SetPosition( NewMarkerPosition, UIFACE_Top, EVALPOS_PixelViewport );
	}
	else  
	{
		// Horizontal Orientation

		// get the total width of the scroll-marker
		const FLOAT NewMarkerPosition = AvailableScrollZoneSize * Marker
		const FLOAT MarkerSize = MarkerButton->GetPosition(UIFACE_Right, EVALPOS_PixelViewport, TRUE) -  MarkerButton->GetPosition(UIFACE_Left, EVALPOS_PixelViewport, TRUE);
		const FLOAT FinalPosition = (MarkerPosPercent * ScrollZoneSize) + DecButtonRight;
		MarkerButton->SetPosition( FinalPosition , UIFACE_Left, EVALPOS_PixelViewport, TRUE);
		MarkerButton->SetPosition( FinalPosition + MarkerSize, UIFACE_Right, EVALPOS_PixelViewport, TRUE);
	}
#endif
}
...
void UUIScrollbar::ResolveMarkerSize()
{
	if ( !MarkerButton->IsPressed(GetBestPlayerIndex()) )
	{
		...
		const FLOAT MarkerSize = Max<FLOAT>(GetScrollZoneExtent() * MarkerSizePercent, MinimumMarkerSize.GetValue(this, EVALPOS_PixelViewport));
		if(ScrollbarOrientation == UIORIENT_Vertical)
		{
/*
			// Update the marker button vertical size
			const FLOAT MarkerTop = MarkerButton->GetPosition(UIFACE_Top, EVALPOS_PixelViewport);
*/

			//@fixme ronp - prevent this from triggering another scene update
			MarkerButton->SetPosition( MarkerSize, UIFACE_Bottom, EVALPOS_PixelOwner );
		}
		else  
		{
/*
			// Update the marker button horizontal size
			const FLOAT MarkerLeft = MarkerButton->GetPosition(UIFACE_Left, EVALPOS_PixelViewport, TRUE);
*/

			//@fixme ronp - prevent this from triggering another scene update
			MarkerButton->SetPosition( MarkerSize, UIFACE_Right, EVALPOS_PixelOwner );
		}
	}
}
...
void UUIScrollbar::ResolveNudgeSize()
{
	...
	NudgeValue = NudgeSize != 0.f ? NudgeSize : 1.f;
}
...
void UUIScrollbar::ScrollIncrement( UUIScreenObject* Sender, INT PlayerIndex )
{
	...
	UBOOL bUpdatePosition = FALSE;
	FLOAT CurrentTime = GWorld->GetRealTimeSeconds();

	switch( ArrowButtonState )
	{
	case ARROWBUTTONSTATE_Clear:
		ArrowButtonState = ARROWBUTTONSTATE_ClickedUp;
		ArrowButtonClickTime = CurrentTime;
		bUpdatePosition = TRUE;
		break;
	case ARROWBUTTONSTATE_ClickedUp:
		if( (CurrentTime - ArrowButtonClickTime) > UCONST_PRIVATE_ArrowClickDelay )
		{
			ArrowButtonState = ARROWBUTTONSTATE_HeldUp;
			ArrowButtonClickTime = CurrentTime;
			bUpdatePosition = TRUE;
		}
		break;
	case ARROWBUTTONSTATE_HeldUp:
		if( (CurrentTime - ArrowButtonClickTime) > UCONST_PRIVATE_ArrowClickRepeat )
		{
			ArrowButtonClickTime = CurrentTime;
			bUpdatePosition = TRUE;
		}
		break;
	case ARROWBUTTONSTATE_ClickedDown:
	case ARROWBUTTONSTATE_HeldDown:
	default:
		/** nothing to do */
		break;
	}
...
void UUIScrollbar::ScrollDecrement( UUIScreenObject* Sender, INT PlayerIndex )
{
	...
	{
	case ARROWBUTTONSTATE_Clear:
		ArrowButtonState = ARROWBUTTONSTATE_ClickedDown;
		ArrowButtonClickTime = CurrentTime;
		bUpdatePosition = TRUE;
		break;
	case ARROWBUTTONSTATE_ClickedDown:
		if( (CurrentTime - ArrowButtonClickTime) > UCONST_PRIVATE_ArrowClickDelay )
		{
			ArrowButtonState = ARROWBUTTONSTATE_HeldDown;
			ArrowButtonClickTime = CurrentTime;
			bUpdatePosition = TRUE;
		}
		break;
...
void UUIScrollbar::SetupDocLinks( UBOOL bResetLinks /*= FALSE*/ )
{
	...
	if(OwnerWidget)
	{
		...
		const FLOAT ButtonWidth = ButtonsExtent.GetValue(this,EVALPOS_PixelViewport);
		...
		if( ScrollbarOrientation == UIORIENT_Vertical )
		{
			...
			{
				SetDockTarget(UIFACE_Left, OwnerWidget, UIFACE_Right, -Width);
			}
			if( bResetLinks || !DockTargets.IsDocked(UIFACE_Top))
			{
				SetDockTarget(UIFACE_Top, OwnerWidget, UIFACE_Top); 
			}
			if( bResetLinks || !DockTargets.IsDocked(UIFACE_Right))
			{
				SetDockTarget(UIFACE_Right, OwnerWidget, UIFACE_Right);
			}
			if( bResetLinks || !DockTargets.IsDocked(UIFACE_Bottom) || DockTargets.GetDockPadding(UIFACE_Bottom) != -CornerPadding)
			{
				SetDockTarget(UIFACE_Bottom, OwnerWidget, UIFACE_Bottom, -CornerPadding);
			}
			...
			{
				DecrementButton->SetDockTarget(UIFACE_Right, this, UIFACE_Right);
			}
			if( bResetLinks || !DecrementButton->DockTargets.IsDocked(UIFACE_Bottom))
			{
				DecrementButton->SetDockTarget(UIFACE_Bottom, this, UIFACE_Top, ButtonWidth);
			}
			...
			{
				IncrementButton->SetDockTarget(UIFACE_Left, this, UIFACE_Left);
			}
			if( bResetLinks || !IncrementButton->DockTargets.IsDocked(UIFACE_Top))
			{
				IncrementButton->SetDockTarget(UIFACE_Top, this, UIFACE_Bottom, -ButtonWidth);
			}
		...
		else
		{
			...
			if( bResetLinks || !DockTargets.IsDocked(UIFACE_Left))
			{
				SetDockTarget(UIFACE_Left, OwnerWidget, UIFACE_Left);
			}
			if( bResetLinks || !DockTargets.IsDocked(UIFACE_Top))
			{
				SetDockTarget(UIFACE_Top, OwnerWidget, UIFACE_Bottom, -Width); 
			}
			if( bResetLinks || !DockTargets.IsDocked(UIFACE_Right) || DockTargets.GetDockPadding(UIFACE_Right) != -CornerPadding)
			{
				SetDockTarget(UIFACE_Right, OwnerWidget, UIFACE_Right, -CornerPadding);
			}
			if( bResetLinks || !DockTargets.IsDocked(UIFACE_Bottom))
			{
				SetDockTarget(UIFACE_Bottom, OwnerWidget, UIFACE_Bottom);
			}
			...
			if( bResetLinks || !DecrementButton->DockTargets.IsDocked(UIFACE_Right))
			{
				DecrementButton->SetDockTarget(UIFACE_Right, this, UIFACE_Left, ButtonWidth);
			}
			if( bResetLinks || !DecrementButton->DockTargets.IsDocked(UIFACE_Bottom))
			{
			...
			if( bResetLinks || !IncrementButton->DockTargets.IsDocked(UIFACE_Left))
			{
				IncrementButton->SetDockTarget(UIFACE_Left, this, UIFACE_Right, -ButtonWidth);
			}
			if( bResetLinks || !IncrementButton->DockTargets.IsDocked(UIFACE_Top))
			{
...
FLOAT UUISlider::GetMarkerPosition()
{
	...
	if ( Marker != NULL )
	{
		FLOAT CurrentMarkerWidth = MarkerWidth.GetValue(this,EVALPOS_PixelViewport);
		FLOAT CurrentMarkerHeight = MarkerHeight.GetValue(this,EVALPOS_PixelViewport);
	...
	FLOAT RelativeLocation = (SliderWidth - MarkerWidth.GetValue(this,EVALPOS_PixelViewport)) * Percentage;
...
UBOOL UUISlider::GetCursorPosition( FLOAT& out_Percentage )
{
	...
	if ( SceneClient != NULL )
	{
		FIntPoint& MousePosition = SceneClient->MousePosition;
		...
		if ( bResult )
		{
			...
			FLOAT MarkerSize = MarkerWidth.GetValue(this,EVALPOS_PixelViewport);
			FLOAT SliderWidth = RenderBounds[UIFACE_Right] - RenderBounds[UIFACE_Left];
...
void UUISlider::NotifyValueChanged( INT PlayerIndex/*=INDEX_NONE*/, INT NotifyFlags/*=0*/ )
{
	...
	{
		FUIProviderFieldValue FieldValue(EC_EventParm);
		FieldValue.RangeValue.SetCurrentValue( FieldValue.RangeValue.MinValue = FieldValue.RangeValue.MaxValue = SliderValue.GetCurrentValue() );
		if( SetDataStoreFieldValue( DataSource.MarkupString, FieldValue, GetScene() ) )
			RefreshSubscriberValue();
	}
...
void UUISlider::Render_Widget( FCanvas* Canvas )
{
	...
	if ( SliderBarImageComponent != NULL )
	{
		...
		if ( SliderOrientation == UIORIENT_Horizontal )
		{
			BarRenderParameters.ImageExtent.Y = BarSize.GetValue(this, EVALPOS_PixelViewport);
		}
		else
		{
			BarRenderParameters.ImageExtent.X = BarSize.GetValue(this, EVALPOS_PixelViewport);
		}
	...
	if ( MarkerImageComponent != NULL )
	{
		...
		if ( SliderOrientation == UIORIENT_Horizontal )
		{
			...
			MarkerRenderParameters.DrawXL = MarkerWidth.GetValue(this,EVALPOS_PixelViewport);
			MarkerRenderParameters.DrawYL = RenderBounds[UIFACE_Bottom] - RenderBounds[UIFACE_Top];

			MarkerRenderParameters.ImageExtent.Y = MarkerHeight.GetValue(this,EVALPOS_PixelViewport);
		}
		else
		{
			FLOAT ActualMarkerHeight = MarkerHeight.GetValue(this,EVALPOS_PixelViewport);
			...
			MarkerRenderParameters.ImageExtent.X = MarkerWidth.GetValue(this,EVALPOS_PixelViewport);
		}
...
void UUINumericEditBox::Initialize( UUIScene* inOwnerScene, UUIObject* inOwner/*=NULL*/ )
{
	if ( IncrementButton == NULL )
	{
		IncrementButton = ConstructObject<UUINumericEditBoxButton>( UUINumericEditBoxButton::StaticClass(), this, *Localize(TEXT("UINumericEditBox"), TEXT("IncrementButtonName"), TEXT("Engine"), NULL, TRUE), GetMaskedFlags(RF_PropagateToSubObjects));
	}

	if ( DecrementButton == NULL )
	{
		DecrementButton = ConstructObject<UUINumericEditBoxButton>( UUINumericEditBoxButton::StaticClass(), this, *Localize(TEXT("UINumericEditBox"), TEXT("DecrementButtonName"), TEXT("Engine"), NULL, TRUE), GetMaskedFlags(RF_PropagateToSubObjects));
	}
...
void UUINumericEditBox::Render_Widget( FCanvas* Canvas )
{
	if ( EditBoxBackground != NULL )
	{
		FRenderParameters Parameters(
			...
			Parameters.DrawCoords = Coordinates;
			EditBoxBackground->Render_Texture(Canvas, Parameters);
	}
...
UBOOL UUINumericEditBox::ProcessInputKey( const FSubscribedInputEventParameters& EventParms )
{
	...
	if ( EventParms.InputAliasName == UIKEY_IncrementNumericValue )
	{
		if ( EventParms.EventType == IE_Pressed )
		{
			...
			IncrementValue(IncrementButton, EventParms.PlayerIndex);
		...
		{
			// activate the on click event
			IncrementButton->ActivateEventByClass(EventParms.PlayerIndex,UUIEvent_OnClick::StaticClass(), this);

			// deactivate the pressed state
			IncrementButton->DeactivateStateByClass(UUIState_Pressed::StaticClass(),EventParms.PlayerIndex);

			SetFocus( IncrementButton );
			bResult = TRUE;
		}
	}
	else if ( EventParms.InputAliasName == UIKEY_DecrementNumericValue )
	{
		if ( EventParms.EventType == IE_Pressed )
		{
			// activate the pressed state
			...
			DecrementValue(DecrementButton, EventParms.PlayerIndex);
		...
		{
			// activate the on click event
			DecrementButton->ActivateEventByClass(EventParms.PlayerIndex,UUIEvent_OnClick::StaticClass(), this);

			// deactivate the pressed state
			DecrementButton->DeactivateStateByClass(UUIState_Pressed::StaticClass(),EventParms.PlayerIndex);

			SetFocus( DecrementButton );
			bResult = TRUE;
		}
	}
...
{	
	...
	if( ValidateNumericInputString( ) )
	{
		...
		for ( INT EventIndex = 0; EventIndex < SubmitTextEvents.Num(); EventIndex++ )
		{
			...
			// set the event's string Value to the current value of the editbox.  this value will be copied into the
			// event's variable links via InitializeLinkedVariableValues().
			TextEvent->Value = StringRenderComponent->ValueString->GetValue( );
			...
			// now activate the event
			TextEvent->ConditionalActivateUIEvent(PlayerIndex,EventOwner, this);
		}
	}
...
{
	FString ValueString = StringRenderComponent->ValueString->GetValue( );
	UBOOL Result = ValueString.IsNumeric();

	// If the user has entered valid data then store it otherwise revert back to the previous valid entry.
	if( Result )
	{
		FLOAT NewValue = appAtof( &ValueString[0] );
 ...
 */
UBOOL UUINumericEditBox::SetNumericValue( FLOAT NewValue )
{
	UBOOL Result = FALSE;
	...
	if( NumericValue.SetCurrentValue( NewValue ) )
	{
		...
		FString Output = TEXT("%."); 
		Output += FString::Printf( TEXT("%i"), DecimalPlaces );
		Output += TEXT("f\0");

		FString ValueString = FString::Printf( &Output[0], NumericValue.GetCurrentValue( ) );

		StringRenderComponent->ValueString->SetValue( ValueString, TRUE );
		StringRenderComponent->ReapplyFormatting();
 ...
 * @param	RI	the FRenderInterface to use for rendering this widget
 */
void UUINumericEditBoxButton::Render_Widget( FCanvas* Canvas )
{
	Super::Render_Widget(Canvas);

#if !FINAL_RELEASE
	if ( GIsGame && GetDefaultUIController()->GetDefaultSceneClient()->bRenderDebugInfo )
	{
		FVector2D OutlineStart( RenderBounds[UIFACE_Left], RenderBounds[UIFACE_Top] );
		FVector2D OutlineEnd( RenderBounds[UIFACE_Right], RenderBounds[UIFACE_Bottom] );

		DrawBox2D( Canvas, OutlineStart, OutlineEnd, FLinearColor(0.f,1.f,0.5f,1.f) );
	}
#endif
}
	...
	if ( !IsTemplate(RF_ClassDefaultObject) )
	{
		...
		if ( ComboList == NULL )
		{
			...
			if ( ComboList == NULL )
			{
				...
				ComboList->CellLinkType = LINKED_Columns;
...
void UUIComboBox::NotifyValueChanged( INT PlayerIndex/*=INDEX_NONE*/, INT NotifyFlags/*=0*/ )
{
	...
	if ( GIsGame )
	{
		TArray<UUIEvent_ComboboxValueChanged*> ValueChangedEvents;
		FindEventsOfClass(UUIEvent_ComboboxValueChanged::StaticClass(),  *((TArray<UUIEvent*>*)&ValueChangedEvents));

		for ( INT EventIndex = 0; EventIndex < ValueChangedEvents.Num(); EventIndex++ )
		{
			UUIEvent_ComboboxValueChanged* Event = ValueChangedEvents(EventIndex);
			if ( (NotifyFlags&UCONST_TEXT_CHANGED_NOTIFY_MASK) != 0 )
			{
				Event->OutputLinks(0).ActivateOutputLink();
			}
			else if ( (NotifyFlags&UCONST_INDEX_CHANGED_NOTIFY_MASK) != 0 )
			{
				Event->OutputLinks(1).ActivateOutputLink();
			}
...
{
//	debugf(TEXT(">> ComboBox::GainFocus  FocusedChild: %s"), FocusedChild != NULL ? *FocusedChild->GetTag().ToString() : TEXT("NULL"));
...
{
//	debugf(TEXT(">> ComboBox::LoseFocus  FocusedChild: %s"), FocusedChild != NULL ? *FocusedChild->GetTag().ToString() : TEXT("NULL"));
...
UBOOL UUIComboBox::RefreshSubscriberValue( INT BindingIndex/*=INDEX_NONE*/ )
{
	...
	else if ( BindingIndex == INDEX_NONE || BindingIndex == CaptionDataSource.BindingIndex )
	{
		...
		if( ComboList && ComboList->IsInitialized() )
			ComboList->RefreshSubscriberValue(BindingIndex);
		if( ComboEditbox && ComboEditbox->IsInitialized() )
			ComboEditbox->RefreshSubscriberValue(BindingIndex);
		if( ComboButton && ComboButton->IsInitialized() )
			ComboButton->RefreshSubscriberValue(BindingIndex);


		// 현재의 콤보에디트의 값을 반영해 콤보리스트를 선택한다.
		if( bSelectListWhenRefresh && 
			ComboEditbox && ComboEditbox->StringRenderComponent && 
			ComboList && ComboList->CellDataComponent )
		{
			const FString EditBoxValue = ComboEditbox->GetValue();
			for( INT ListIndex = 0 ; ListIndex < ComboList->CellDataComponent->ListItems.Num() ; ListIndex++ )
			{
				const FUIListItem& ListItem = ComboList->CellDataComponent->ListItems(ListIndex);
				for( INT CellIndex = 0 ; CellIndex < ListItem.Cells.Num() ; CellIndex++ )
				{
					UUIListString* ValueString = ListItem.Cells(CellIndex).ValueString;
					if( ValueString != NULL && EditBoxValue == ValueString->GetValue() )
					{
						ComboList->SetIndex(ListIndex);
						break;
					}
				}
			}
...
UBOOL UUIComboBox::SetFieldValueOnSubmit()
{
	UBOOL bResult = FALSE;
	...
	FString FieldTag;
	INT ArrayIndex = ComboList->GetCurrentItem();
	FUIProviderScriptFieldValue FieldValue(EC_EventParm);

	FieldValue.RangeValue.bIntRange = TRUE;
	FieldValue.RangeValue.MaxValue = FieldValue.RangeValue.MinValue = ComboList->GetCurrentItem();
	FieldValue.RangeValue.SetCurrentValue( FieldValue.RangeValue.MaxValue, FALSE );
	FieldValue.StringValue = ComboList->GetElementValue( ComboList->Index );
	bResult = ComboEditbox->SetDataStoreFieldValue( ComboEditbox->DataSource.MarkupString, FieldValue, GetScene() );
...
FLOAT UUIToolTip::GetIdealTop() const
{
	...
	if ( GetCursorPosition(MouseX, MouseY, GetScene()) )
	{
		...
		FVector2D ViewportRange, ViewportOrigin;
		if( GetViewportSize( ViewportRange ) &&
			GetViewportOrigin( ViewportOrigin ))
		{
			FLOAT Height = GetBounds( UIORIENT_Vertical, EVALPOS_PixelViewport );
			Result += Max<FLOAT>( ViewportOrigin.Y - MouseY, 0);					// 커서가 화면위를 넘어간 경우
			Result -= Max<FLOAT>( (MouseY + Height) - (ViewportOrigin.Y + ViewportRange.Y), 0);	// 커서가 화면아래를 넘어간 경우	
		}

		// 원래의 문제의 코드 (Above, Below 의 무한루프에 빠지는 경우가 있음)
...
void UUIToolTip::RequestSceneUpdate( UBOOL bDockingStackChanged, UBOOL bPositionsChanged, UBOOL bNavLinksOutdated/*=FALSE*/, UBOOL bWidgetStylesChanged/*=FALSE*/ )
{
	...
	{
		Super::RequestSceneUpdate(bDockingStackChanged, bPositionsChanged, bNavLinksOutdated, bWidgetStylesChanged);
	}
	else
	{
...
/** 해당 툴팁이 동작중인지(화면에 표시되고 있는지) 알려줌 */
UBOOL UUIToolTip::IsActiveToolTip() const
{
	const UUIScene* OwnerScene = GetScene();
	return OwnerScene && (OwnerScene->GetActiveToolTip() == this);
}


void UUIPIP::Initialize( UUIScene* inOwnerScene, UUIObject* inOwner )
{
	Super::Initialize( inOwnerScene, inOwner );
}

void UUIPIP::PostUpdateMatinee()
{
	Super::PostUpdateMatinee();

	FRotationTranslationMatrix NewLightTM( LightDirection, DirectionalLightComponent->LightToWorld.GetOrigin() );
	DirectionalLightComponent->ConditionalUpdateTransform(NewLightTM);
}

void UUIPIP::NotifySceneActivated()
{
	Super::NotifySceneActivated();

	Scene = NULL;

	ViewState = NULL;

	ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER( DeleteViewState, FSceneViewStateInterface**, ViewStatePtr, &ViewState,
	{
		*ViewStatePtr = AllocateViewState();
	}
	)
...
void UUIPIP::NotifySceneDeactivated()
{
	Super::NotifySceneDeactivated();

	ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER( DeleteViewState, FSceneViewStateInterface*, ViewState, ViewState,
	{
		ViewState->Destroy();
		ViewState = NULL;
	}
...
void UUIPIP::FinishDestroy()
{
	Super::FinishDestroy();
}

void UUIPIP::DeleteScene()
{
	if (Scene == NULL) return;

	for (INT i=0; i<Components.Num(); ++i)
	{
		Components(i)->DetachFromAny();		
	}	
...
void UUIPIP::SetupScene()
{
	...
	for (INT i=0; i<Components.Num(); ++i)
	{
		Components(i)->ConditionalAttach(Scene,NULL,FMatrix::Identity);		
	}	
...
void UUIPIP::PostEditChange( FEditPropertyChain& PropertyThatChanged )
{
	if ( PropertyThatChanged.Num() > 0 )
	{
		eventRefresh();			
...
void UUIPIP::PostEditChange( UProperty* PropertyThatChanged )
{
	Super::PostEditChange( PropertyThatChanged );
}

FSceneView* UUIPIP::CalcSceneView( FCanvas* Canvas, FSceneViewFamily* ViewFamily )
{
	...
	FMatrix Transform = Canvas->GetTransform();	

	FVector4 Result = Transform.TransformFVector4( FVector4( RenderBounds[UIFACE_Left], RenderBounds[UIFACE_Top]) );
	INT X = (INT)(Result.X / Result.W);
	INT Y = (INT)(Result.Y / Result.W);

	Result = Transform.TransformFVector4( FVector4( RenderBounds[UIFACE_Right], RenderBounds[UIFACE_Bottom]) );
	UINT SizeX = (INT)(Result.X / Result.W) - X;
	UINT SizeY = (INT)(Result.Y / Result.W) - Y;	

	// only use post process chain for the perspective view
	LevelViewPostProcess = PostProcess;	

	if (bUsingLookAt)
	{
		ViewMatrix =
			FTranslationMatrix( -LookAt ) *
			FRotationMatrix( FRotator(0,Yaw * 65536.0f / 360.0f,0) ) * 
			FRotationMatrix( FRotator(0, 0, Pitch * 65536.0f / 360.0f)) *
			FTranslationMatrix( FVector( 0, Zoom, 0 ) ) *
			FTranslationMatrix( FVector(0,256,0) ) *
			FInverseRotationMatrix( FRotator(0,16384,0) );

		FMatrix Mat =  ViewMatrix.Inverse();
		ViewRotation = Mat.Rotator();
		ViewLocation = Mat.GetOrigin();
	}
	else
	{
	...
	FSceneView* View = new FSceneView(
		...
		ViewMatrix,
		ProjectionMatrix,
		/* Editor viewport doesn't require Foreground DPG */
		ProjectionMatrix, 
		90,
		/* Editor viewport doesn't require Foreground DPG */
		BackgroundColor,
...
static UBOOL UpdateSkelComponent( UComponent* Component, FLOAT DeltaTime, UBOOL bNeedsAnimations = FALSE )
{
	USkeletalMeshComponent* SkelComponent = Cast<USkeletalMeshComponent>( Component );

	if (!SkelComponent) return FALSE;		

	if (bNeedsAnimations && !SkelComponent->Animations && SkelComponent->AnimTreeTemplate)
	{
		// make sure anim tree is instanced from template.
		SkelComponent->SetAnimTreeTemplate( SkelComponent->AnimTreeTemplate );
	}

	if (SkelComponent->Animations != NULL) 
	{
		// Tick the PreviewSkelComp to move animation forwards, then Update to update bone locations.
		SkelComponent->TickAnimNodes(DeltaTime);
		SkelComponent->TickSkelControls(DeltaTime);
	}
	
	SkelComponent->UpdateSkelPose();				

	for (INT i=0; i<SkelComponent->Attachments.Num(); ++i)
	{
		const FAttachment& Attachment = SkelComponent->Attachments(i);

		UpdateSkelComponent( Attachment.Component, DeltaTime );
	}

	return TRUE;
}

void UUIPIP::Render_Widget( FCanvas* Canvas )
{
	/// 선택할 것 없으삼~~~
	if (Canvas->GetHitProxyConsumer() || ViewState == NULL) return;
	...
	UBOOL bNeedsToBeUpdated = FALSE;

	for (INT i=0; i<Components.Num(); ++i)
	{
		if (UpdateSkelComponent( Components(i), DeltaTime, TRUE ))
		{		
			bNeedsToBeUpdated = TRUE;
		}
	}

	if (bNeedsToBeUpdated)
	{
		for (INT i=0; i<Components.Num(); ++i)
		{
			Components(i)->ConditionalUpdateTransform();		
		}
	}

	FViewport* Viewport = static_cast<FViewport*>( Canvas->GetRenderTarget() );

	FSceneViewFamilyContext ViewFamily(Viewport,Scene,SHOW_DefaultGame,CurrentTime,CurrentTime,NULL,FALSE,TRUE);
	FSceneView* View = CalcSceneView( Canvas, &ViewFamily );
...
UBOOL UUIPIP::ProcessInputKey( const struct FInputEventParameters& EventParms )
{
	return FALSE;
}
