...
FString AActor::ConsoleCommand(const FString& Cmd, UBOOL bWriteToLog)
{
	...
	TCHAR CommandBuffer[1001];
	const TCHAR* Command = CommandBuffer;
	// copy the command into a modifiable buffer
	appStrcpy(CommandBuffer, *Cmd.Left(ARRAY_COUNT(CommandBuffer) - 1)); 
	TCHAR Line[256];

	// iterate over the line, breaking up on |'s
	while (ParseLine(&Command, Line, ARRAY_COUNT(Line)))
	{
...
FString AWorldInfo::GetLocalURL()
{
	return GWorld->URL.String();
}

UBOOL AWorldInfo::IsDemoBuild()
{
#if DEMOVERSION
	return true;
#else
 ...
 */
UBOOL AWorldInfo::IsConsoleBuild()
{
#ifdef CONSOLE
	return true;
#else
...
FString AWorldInfo::GetAddressURL()
{
	return FString::Printf( TEXT("%s:%i"), *GWorld->URL.Host, GWorld->URL.Port );
}

USequence* AWorldInfo::GetGameSequence()
{
	return GWorld->GetGameSequence();
}

void AWorldInfo::execAllNavigationPoints _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AWorldInfo::execRadiusNavigationPoints _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass, BaseClass);
	P_GET_OBJECT_REF(ANavigationPoint, OutNav);
	P_GET_VECTOR(Point);
...
void AWorldInfo::execNavigationPointCheck _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_VECTOR(Point);
	P_GET_VECTOR(Extent);
...
void AWorldInfo::execAllControllers _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass, BaseClass);
...
void AWorldInfo::execAllPawns _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass, BaseClass);
...
void AActor::execPollSleep _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AActor::execSetCollision _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AActor::execSetBase _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
UAudioComponent* AActor::CreateAudioComponent(class USoundCue* SoundCue, UBOOL bPlay, UBOOL bStopWhenOwnerDestroyed, UBOOL bUseLocation, FVector SourceLocation, UBOOL bAttachToSelf, BYTE Channel)
{
	return UAudioDevice::CreateComponent(SoundCue, GWorld->Scene, bAttachToSelf ? this : NULL, bPlay, bStopWhenOwnerDestroyed, bUseLocation ? &SourceLocation : NULL, (EAudioChannel)Channel);
}
...
{
	return GWorld->FarMoveActor( this, NewLocation, 0, 1 );
}
...
void AActor::execTrace _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AActor::execTraceComponent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_VECTOR_REF(HitLocation);
	P_GET_VECTOR_REF(HitNormal);
...
void AActor::execFastTrace _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_VECTOR(TraceEnd);
	P_GET_VECTOR_OPTX(TraceStart,Location);
	...
	GWorld->SingleLineCheck( Hit, this, TraceEnd, TraceStart, TRACE_World|TRACE_StopAtAnyHit, BoxExtent );
...
void AActor::execSpawn _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AActor::execDestroy _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void AActor::execAllActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	// Get the parms.
	P_GET_OBJECT(UClass,BaseClass);
...
void AActor::execDynamicActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	// Get the parms.
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
...
void AActor::execChildActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_FINISH;
...
void AActor::execBasedActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_FINISH;
...
void AActor::execComponentList _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
...
void AActor::execAllOwnedComponents _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
...
void AActor::execTouchingActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_FINISH;
...
void AActor::execTraceActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_GET_VECTOR_REF(HitLocation);
...
void AActor::execVisibleActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
...
void AActor::execVisibleCollidingActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_GET_FLOAT(Radius);
...
void AActor::execCollidingActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_GET_FLOAT(Radius);
...
void AActor::execOverlappingActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_GET_FLOAT(Radius);
...
// {{ dEAthcURe|HM
#ifdef EnableHostMigration
void AWeapon::hmSerialize(FArchive& Ar)
{	
	//Autogenerated code by HmSerializeGenerator/generateSerializer.rb

	//Super::hmSerialize(Ar); //왜 죽였는지 확인해보3

	Super::hmSerialize(Ar); // [+] 20070322

	/* test disable 20070323 검증후삭제할것
	if (Ar.IsLoading() ) {
		_hms_defLoading;
		_hms_loadValue(bWeaponPutDown); // BITFIELD bWeaponPutDown:1;
		_hms_loadValue(bInstantHit); // BITFIELD bInstantHit:1;
		_hms_loadValue(bCanThrow); // BITFIELD bCanThrow:1;
		_hms_loadValue(bMeleeWeapon); // BITFIELD bMeleeWeapon:1;
	}
	else {
		_hms_defSaving;
		_hms_saveValue(bWeaponPutDown); // BITFIELD bWeaponPutDown:1;
		_hms_saveValue(bInstantHit); // BITFIELD bInstantHit:1;
		_hms_saveValue(bCanThrow); // BITFIELD bCanThrow:1;
		_hms_saveValue(bMeleeWeapon); // BITFIELD bMeleeWeapon:1;
	}

	Ar << WeaponRange; // FLOAT WeaponRange;
	Ar << PutDownTime; // FLOAT PutDownTime;
	Ar << FireOffset; // FVector FireOffset;
	Ar << EquipTime; // FLOAT EquipTime;
	Ar << CurrentFireMode; // BYTE CurrentFireMode;
	Ar << Priority; // FLOAT Priority;
	Ar << AIRating; // FLOAT AIRating;
	Ar << BotRefireRate; // FLOAT BotRefireRate;
	*/
...
*/
void AInventoryManager::execInventoryActors _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass,BaseClass);
	P_GET_ACTOR_REF(OutActor);
	P_FINISH;
...
*/
void AActor::execLocalPlayerControllers _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_ACTOR_REF(OutActor);
	P_FINISH;
...
UBOOL AActor::TouchingActor( AActor* A )
{
	return this->IsOverlapping(A);

}

void AActor::execPlaySound _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	...
	PlaySound(InSoundCue, bNotReplicated, bNoRepToOwner, bStopWhenOwnerDestroyed, &SoundLocation, bNoRepToRelevant, (EAudioChannel)Channel);
}

void AActor::PlaySound(USoundCue* InSoundCue, UBOOL bNotReplicated/*=FALSE*/, UBOOL bNoRepToOwner/*=FALSE*/, UBOOL bStopWhenOwnerDestroyed/*=FALSE*/, FVector* pSoundLocation/*=NULL*/, UBOOL bNoRepToRelevant/*=FALSE*/, EAudioChannel Channel)
{
	if ( InSoundCue == NULL )
	{
		...
		for( INT iPlayers=0; iPlayers<GWorld->NetDriver->ClientConnections.Num(); iPlayers++ )
		{
			if ( GWorld->NetDriver->ClientConnections(iPlayers) )
			{
				...
				if ( NextPlayer )
				{
					...
					if (NextPlayer != NULL)
					{
						NextPlayer->HearSound(InSoundCue, this, SoundLocation, bStopWhenOwnerDestroyed, Channel);
					}
				}
			}
		}
	...
	if ( GWorld->GetNetMode() != NM_DedicatedServer )
	{
		...
		for( INT iPlayers=0; iPlayers<GEngine->GamePlayers.Num(); iPlayers++ )
		{
			if ( GEngine->GamePlayers(iPlayers) )
			{
				...
				{
					NextListener->HearSound(InSoundCue, this, SoundLocation, bStopWhenOwnerDestroyed, Channel);
				}
			}
		}
	}
...
*/
void APlayerController::HearSound(USoundCue* InSoundCue, AActor* SoundPlayer, const FVector& SoundLocation, UBOOL bStopWhenOwnerDestroyed, EAudioChannel Channel)
{
	INT bIsOccluded = 0;
	...
	{
		eventClientHearSound(InSoundCue, SoundPlayer, SoundLocation, bStopWhenOwnerDestroyed, bIsOccluded, Channel);
	}
}
...
void AActor::ProcessState( FLOAT DeltaSeconds )
{
	...
	&&	!IsPendingKill() )
	{
		...
		if (GetStateFrame()->LatentAction != 0)
		{
			...
			(this->*GNatives[GetStateFrame()->LatentAction])(*GetStateFrame(), (BYTE*)&DeltaSeconds, 0x0, 0x0);
			#else
...
)
{
	...
	else
	{
		...
		BTPrintF(TEXT("InternalProcessRemoteFunction|%p|%d||%s::%s"), Ch->Connection, Bunch.GetNumBytes(), *Actor->GetName(), *Function->GetName());
#endif

		// {{ 20071127 dEAthcURe|ON dd
		#ifdef EnableNetProfile
		extern int G_tt_byteRemoteCall;
		G_tt_byteRemoteCall += Bunch.GetNumBytes();
		//debugf(TEXT("          <---------- InternalProcessRemoteFunction|%p|%d||%s::%s"), Ch->Connection, Bunch.GetNumBytes(), *Actor->GetName(), *Function->GetName()); // 20071127 dEAthcURe|ON dd
		#endif
		//}} 20071127 dEAthcURe|ON dd

		// {{ 20071129 dEAthcURe|ON dd
		#ifdef EnableNetProfile
		extern pfRpc_t GpfRpc;
		GpfRpc.add(Function, *Function->GetName(), Bunch.GetNumBytes());
		#endif
		// }} 20071129 dEAthcURe|ON dd
...
// {{ dEAthcURe|HM
#ifdef EnableHostMigration
void AGameInfo::hmSerialize(FArchive& Ar)
{
	//check 20070207
	//Autogenerated code by HmSerializeGenerator/generateSerializer.rb
	...
	if (Ar.IsLoading() ) {
		_hms_defLoading;
		_hms_loadValue(bHasNetworkError); // BITFIELD bHasNetworkError:1;
		_hms_loadValue(bUseSeamlessTravel); // BITFIELD bUseSeamlessTravel:1;
		_hms_loadValue(bLevelChange); // BITFIELD bLevelChange:1;
		_hms_loadValue(bChangeLevels); // BITFIELD bChangeLevels:1;
		_hms_loadValue(bRestartLevel); // BITFIELD bRestartLevel:1;
		_hms_loadValue(_hm_bSetYaw); // BITFIELD _hm_bSetYaw:1;
		_hms_loadValue(bKickLiveIdlers); // BITFIELD bKickLiveIdlers:1;
		_hms_loadValue(bWaitingToStartMatch); // BITFIELD bWaitingToStartMatch:1;
		_hms_loadValue(bDoFearCostFallOff); // BITFIELD bDoFearCostFallOff:1;
		_hms_loadValue(bOverTime); // BITFIELD bOverTime:1;
		_hms_loadValue(bPauseable); // BITFIELD bPauseable:1;
		_hms_loadValue(bMigratedHost); // BITFIELD bMigratedHost:1;
		_hms_loadValue(bGameRestarted); // BITFIELD bGameRestarted:1;
		_hms_loadValue(bHasEndGameHandshakeBegun); // BITFIELD bHasEndGameHandshakeBegun:1;
		_hms_loadValue(bHasArbitratedHandshakeBegun); // BITFIELD bHasArbitratedHandshakeBegun:1;
		_hms_loadValue(bDelayedStart); // BITFIELD bDelayedStart:1;
		_hms_loadValue(bTeamGame); // BITFIELD bTeamGame:1;
		_hms_loadValue(bUsingArbitration); // BITFIELD bUsingArbitration:1;
		_hms_loadValue(bAllowVehicles); // BITFIELD bAllowVehicles:1;
		_hms_loadValue(bIsEndGameHandshakeComplete); // BITFIELD bIsEndGameHandshakeComplete:1;
		_hms_loadValue(bAdminCanPause); // BITFIELD bAdminCanPause:1;
		_hms_loadValue(bLoggingGame); // BITFIELD bLoggingGame:1;
		_hms_loadValue(bAlreadyChanged); // BITFIELD bAlreadyChanged:1;
		_hms_loadValue(bNeedsEndGameHandshake); // BITFIELD bNeedsEndGameHandshake:1;
		_hms_loadValue(bGameEnded); // BITFIELD bGameEnded:1;
	}
	else {
		_hms_defSaving;
		_hms_saveValue(bHasNetworkError); // BITFIELD bHasNetworkError:1;
		_hms_saveValue(bUseSeamlessTravel); // BITFIELD bUseSeamlessTravel:1;
		_hms_saveValue(bLevelChange); // BITFIELD bLevelChange:1;
		_hms_saveValue(bChangeLevels); // BITFIELD bChangeLevels:1;
		_hms_saveValue(bRestartLevel); // BITFIELD bRestartLevel:1;
		_hms_saveValue(_hm_bSetYaw); // BITFIELD _hm_bSetYaw:1;
		_hms_saveValue(bKickLiveIdlers); // BITFIELD bKickLiveIdlers:1;
		_hms_saveValue(bWaitingToStartMatch); // BITFIELD bWaitingToStartMatch:1;
		_hms_saveValue(bDoFearCostFallOff); // BITFIELD bDoFearCostFallOff:1;
		_hms_saveValue(bOverTime); // BITFIELD bOverTime:1;
		_hms_saveValue(bPauseable); // BITFIELD bPauseable:1;
		_hms_saveValue(bMigratedHost); // BITFIELD bMigratedHost:1;
		_hms_saveValue(bGameRestarted); // BITFIELD bGameRestarted:1;
		_hms_saveValue(bHasEndGameHandshakeBegun); // BITFIELD bHasEndGameHandshakeBegun:1;
		_hms_saveValue(bHasArbitratedHandshakeBegun); // BITFIELD bHasArbitratedHandshakeBegun:1;
		_hms_saveValue(bDelayedStart); // BITFIELD bDelayedStart:1;
		_hms_saveValue(bTeamGame); // BITFIELD bTeamGame:1;
		_hms_saveValue(bUsingArbitration); // BITFIELD bUsingArbitration:1;
		_hms_saveValue(bAllowVehicles); // BITFIELD bAllowVehicles:1;
		_hms_saveValue(bIsEndGameHandshakeComplete); // BITFIELD bIsEndGameHandshakeComplete:1;
		_hms_saveValue(bAdminCanPause); // BITFIELD bAdminCanPause:1;
		_hms_saveValue(bLoggingGame); // BITFIELD bLoggingGame:1;
		_hms_saveValue(bAlreadyChanged); // BITFIELD bAlreadyChanged:1;
		_hms_saveValue(bNeedsEndGameHandshake); // BITFIELD bNeedsEndGameHandshake:1;
		_hms_saveValue(bGameEnded); // BITFIELD bGameEnded:1;
	}

	Ar << GameName; // FStringNoInit GameName;
	Ar << NumBots; // INT NumBots;
	Ar << FearCostFallOff; // FLOAT FearCostFallOff;
	//Ar << NumPlayers; // INT NumPlayers;
	Ar << MaxSpectators; // INT MaxSpectators;
	//Ar << _hm_yaw; // INT _hm_yaw;
	Ar << MinTimeMargin; // FLOAT MinTimeMargin;
	Ar << MaxTimeMargin; // FLOAT MaxTimeMargin;
	Ar << GoalScore; // INT GoalScore;
	Ar << GameDifficulty; // FLOAT GameDifficulty;
	Ar << ArbitrationHandshakeTimeout; // FLOAT ArbitrationHandshakeTimeout;
	Ar << TimeMarginSlack; // FLOAT TimeMarginSlack;
	Ar << MaxLives; // INT MaxLives;
	Ar << GameSpeed; // FLOAT GameSpeed;
	Ar << MaxSpectatorsAllowed; // INT MaxSpectatorsAllowed;
	Ar << DefaultPlayerName; // FStringNoInit DefaultPlayerName;
	Ar << DialogueManagerClass; // FStringNoInit DialogueManagerClass;
	Ar << TimeLimit; // INT TimeLimit;
	Ar << MaxPlayers; // INT MaxPlayers;
	Ar << MaxPlayersAllowed; // INT MaxPlayersAllowed;
	Ar << MaxIdleTime; // FLOAT MaxIdleTime;
	Ar << CurrentID; // INT CurrentID;
	Ar << GoreLevel; // INT GoreLevel;
	//Ar << NumSpectators; // INT NumSpectators;
	*/

	BYTE bGameReplicationInfo = GameReplicationInfo? 1 : 0;
	Ar << bGameReplicationInfo;

	if ( Ar.IsLoading() ) {
		if(bGameReplicationInfo) {
			if(GameReplicationInfo) {
				GameReplicationInfo->hmSerialize(Ar);
			}
			else {
				debugf(TEXT("[dEAthcURe] Invalid object null game replication info"));
			}
		}
	}
	else {
...
FString AGameInfo::GetFullUIPath( const FString& PartialPath )
{
	AWorldInfo* WorldInfo = GWorld ? GWorld->GetWorldInfo() : NULL;
	UClass* NetEntryGameClass = WorldInfo ? WorldInfo->NetEntryGameClass : NULL;

	INT FindIndex = PartialPath.InStr(TEXT("."));
	if( FindIndex == INDEX_NONE )
		return PartialPath;

	FString ResultStr = PartialPath;
	UBOOL bPackageFound = FindPackage(NULL, *PartialPath.Left(FindIndex)) != NULL;
	if( !bPackageFound && NetEntryGameClass )
	{
		AGameInfo* GameInfo = NetEntryGameClass->GetDefaultObject<AGameInfo>();
		if( GameInfo )
			ResultStr = GameInfo->DefaultUIPathPrefix + TEXT("_")+ PartialPath;
	}

	return ResultStr;
}

void AVolume::execEncompasses _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_ACTOR(InActor);
	P_FINISH;
...
void AActor::execIsBasedOn _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AActor::execIsOwnedBy _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_ACTOR(TestActor);
	P_FINISH;
...
void AActor::execSetHardAttach _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
