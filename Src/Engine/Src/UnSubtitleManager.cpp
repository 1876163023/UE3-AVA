 ...
 */
void FSubtitleManager::KillSubtitles( DWORD SubtitleID )
{
 ...
 */
void FSubtitleManager::QueueSubtitles( DWORD SubtitleID, FLOAT Priority, UBOOL bManualWordWrap, FLOAT SoundDuration, TArray<FSubtitleCue>& Subtitles )
{
	check( GEngine );
	check( GWorld );
	...
	const FSubtitleCue Temp = { "", StartTime + SoundDuration };
	NewSubtitle.Subtitles.AddItem( Temp );
}
...
void FSubtitleManager::SplitLinesToSafeZone( FIntRect & SubtitleRegion )
{
	...
	for( TMap<DWORD, FActiveSubtitle>::TIterator It( ActiveSubtitles ); It; ++It )
	{
		...
		const FSubtitleCue Temp = { "", StartTime + SoundDuration };
		Subtitle.Subtitles.AddItem( Temp );
 ...
 */
DWORD FSubtitleManager::FindHighestPrioritySubtitle( FLOAT CurrentTime )
{
	// Tick the available subtitles and find the highest priority one
	FLOAT HighestPriority = -1.0f;
	DWORD HighestPriorityID = 0;
...
void FSubtitleManager::DisplaySubtitles( FCanvas *Canvas, FIntRect & SubtitleRegion )
{
	...
	DWORD HighestPriorityID = FindHighestPrioritySubtitle( GWorld->GetTimeSeconds() );
