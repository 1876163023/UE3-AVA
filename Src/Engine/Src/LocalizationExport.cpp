/*=============================================================================
	LocalizationExport.cpp: Implemention of localized property export.
											 ...
											 UBOOL				bCompareAgainstDefaults)
{
	...
	for( INT i = 0; i < Ptr->Num(); i++ )
	{
		...
		FString RealValue;
		Prop->Inner->ExportTextItem( RealValue, Dest, DefaultData ? DefaultData + DataOffset : NULL, NULL, PPF_Delimited|PPF_LocalizedOnly );
 ...
 * @param	bDumpEmptyProperties		If TRUE, export properties with zero length/empty strings.
 */
void FLocalizationExport::ExportPackage(UPackage* Package, const TCHAR* IntName, UBOOL bCompareAgainstDefaults, UBOOL bDumpEmptyProperties)
{
	...
	for ( FObjectIterator It ; It ; ++It )
	{
		...
		if ( Obj->IsIn( Package ) )
		{
			...
			if ( !Class )
			{
				...
				if( ObjClass->HasAnyClassFlags(CLASS_PerObjectConfig|CLASS_Localized) || Obj->HasAnyFlags(RF_PerObjectLocalized) )
				{
	...
	for ( INT ObjectIndex = 0 ; ObjectIndex < ObjectsToExport.Num() ; ++ObjectIndex )
	{
		...
		else
		{
			...
			const UBOOL bInstancedSubobject = !ObjClass->HasAnyClassFlags(CLASS_PerObjectConfig);
