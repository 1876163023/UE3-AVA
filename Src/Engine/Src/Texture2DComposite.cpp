...
*/
void UTexture2DComposite::UpdateCompositeTextue(INT NumMipsToGenerate)
{
...
INT UTexture2DComposite::GetFirstAvailableMipIndex()
{
	...
	// Apply additional bias for this composite texture.
	MaxLODBias += CompositeLODBias;
...
void UTexture2DComposite::InitValidSourceRegions()
{
	...
	for( INT SrcIdx=0; SrcIdx < SourceRegions.Num(); SrcIdx++ )
	{
		...
		{
			debugf( NAME_Warning, TEXT("FCompositeTexture2DUtil: Source texture size mismatch [%s] - skipping..."), 
				*SourceRegion.Texture2D->GetFullName() );
		}
...
void UTexture2DComposite::InitMips( INT NumMipsToGenerate )
{
	...
	INT MaxTexMips = appCeilLogTwo(Max(SizeX,SizeY));
