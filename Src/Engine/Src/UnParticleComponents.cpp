...
DECLARE_CYCLE_STAT(TEXT("Sprite VB Pack Time"),STAT_SpriteVBPackingTime,STATGROUP_Particles);
...
void AEmitter::execSetTemplate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UParticleLODLevel::UpdateModuleLists()
{
	...
	UpdateModules.Empty();
/***
	if (ModuleArraySize < Modules.Num())
	{
		delete []	SpawnModuleArray;
		delete []	UpdateModuleArray;

		ModuleArraySize		= Modules.Num();
		SpawnModuleCount	= 0;
		UpdateModuleCount	= 0;

		SpawnModuleArray	= new FParticleModuleEntry[ModuleArraySize];
		check(SpawnModuleArray);
		UpdateModuleArray	= new FParticleModuleEntry[ModuleArraySize];
		check(UpdateModuleArray);

		for (INT ii = 0; ii < ModuleArraySize; ii++)
		{
			SpawnModuleArray[ii].Module		= NULL;
			SpawnModuleArray[ii].Offset		= -1;
			UpdateModuleArray[ii].Module	= NULL;
			UpdateModuleArray[ii].Offset	= -1;
		}
	}
***/
	UParticleModule* Module;
...
/***
			if (Module->bSpawnModule)
			{
				SpawnModuleArray[SpawnModuleCount++].Module		= Module;
			}
			if (Module->bUpdateModule)
			{
				UpdateModuleArray[UpdateModuleCount++].Module	= Module;
			}
***/
		}
	}
			...
			if (MeshTD->Mesh)
			{
				if (MeshTD->Mesh->LODModels(0).Elements.Num())
				{
					...
					if (SpriteEmitter)
					{
						...
						if (Element.Material)
						{
							SpriteEmitter->Material	= Element.Material;
						}
					}
				}
			}
...
INT	UParticleLODLevel::CalculateMaxActiveParticleCount()
{
	...
	// Deterine the duration for the emitter
	FLOAT EmitterDuration = RequiredModule->EmitterDuration;
	if (RequiredModule->bEmitterDurationUseRange)
	{
		// Cover the case where Low is actually set to > high
		EmitterDuration	= RequiredModule->EmitterDurationLow + 
			(RequiredModule->EmitterDuration - RequiredModule->EmitterDurationLow);
		if (RequiredModule->EmitterLoops == 0)
		{
			if (RequiredModule->bDelayFirstLoopOnly == FALSE)
			{
				// Take the delay into account... just to be safe?
				EmitterDuration += RequiredModule->EmitterDelay;
			}
		}
	}
	...
	if (RequiredModule->EmitterLoops > 0)
	{
		// Take the number of loops into account
		if (ParticleLifetime > 0)
		{
			// If the duration is longer than the lifetime, don't bother...
			if (EmitterDuration < ParticleLifetime)
			{
				//@todo. Actually determine exact amount that will be alive at the peak...
				// Otherwise, be safe and allocate enough for each loop
				EmitterDuration *= RequiredModule->EmitterLoops;
			}
		}
	}
	else
	{
		// If the emitter will loop infinitely, we need to ensure it won't run out of memory...
		if (ParticleLifetime > 0)
		{
			// Particle lifetime is what we are interested in here...
			EmitterDuration = ParticleLifetime;
		}
	}
	...
	MaxAPC += appFloor(MaxSpawnRate * EmitterDuration) + 2;
	MaxAPC += MaxBurst;
...
{
#if CONSOLE
	// Should NOT be in this function if on console!
#else	//#if CONSOLE
	if (GIsCooking == TRUE)
	{
		// Examine each emitter
		for (INT i = Emitters.Num() - 1; i >= 0; i--)
		{
			UParticleEmitter* Emitter = Emitters(i);
			if (Emitter)
			{
				UParticleSpriteEmitter* SpriteEmitter = CastChecked<UParticleSpriteEmitter>(Emitter);
				// Do not cook out disabled emitter...
				UBOOL bDeleteEmitter = TRUE;
				for (INT LODLevelIndex = 0; (LODLevelIndex < SpriteEmitter->LODLevels.Num()) && (bDeleteEmitter == TRUE); LODLevelIndex++)
				{
				...
				if (bDeleteEmitter)
				{
					...
					debugf(NAME_ParticleWarn, TEXT("Emitter %2d cooked out of PSys %s"), i, *(GetPathName()));
					Emitters(i) = NULL;
				}
			}
		}
...
void UParticleSystem::PostLoad()
{
	...
	for (INT i = Emitters.Num() - 1; i >= 0; i--)
	{
		...
		{
#if CONSOLE
			// Empty emitter slots are ok on consoles
#else	//#if CONSOLE
				...
				if (MeshTD)
				{
					...
					if (Mesh)
					{
						for (INT LODIndex = 0; (LODIndex < Mesh->LODModels.Num()) && (bQuickOut == FALSE); LODIndex++)
						{
							...
							{
								UMaterialInstance* MatInst = LOD->Elements(MatIndex).Material;
								if (MatInst)
								{
...
void UParticleSystemComponent::Attach()
{
	...
	if (Template)
	{
		...
		UBOOL bHasPhysics = FALSE;

		for (INT EmitterIndex = 0; (EmitterIndex < Template->Emitters.Num()) && (bHasPhysics == FALSE); EmitterIndex++)
		{
			...
			if (Emitter)
			{
				for (INT LODIndex = 0; (LODIndex < Emitter->LODLevels.Num()) && (bHasPhysics == FALSE); LODIndex++)
				{
					...
					UParticleLODLevel* TempLOD = Emitter->LODLevels(LODIndex);
					if (TempLOD)
					{
						for (INT ModuleIndex = 0; ModuleIndex < TempLOD->Modules.Num(); ModuleIndex++)
						{
							UParticleModuleCollision* CollisionModule = Cast<UParticleModuleCollision>(TempLOD->Modules(ModuleIndex));
							if (CollisionModule)
							{
...
void UParticleSystemComponent::Tick(FLOAT DeltaTime)
{
	...
	if (GIsGame && ((bOverrideLODMethod ? LODMethod : Template->LODMethod) == PARTICLESYSTEMLODMETHOD_Automatic) && AccumLODDistanceCheckTime == 0.0f)
	{	
		INT	LODIndex = -1;
		...
		if (Template)
		{
			...
			FVector loc;
			if (Player->Actor->ViewTarget != NULL )	loc = Player->Actor->ViewTarget->Location;
			else									loc = Player->Actor->Location;

			FVector	CameraPosition	= loc;
...
//			debugf(TEXT("Perform LOD Determination Check - Dist = %8.5f, PSys %s"), Distance, *GetFullName());
			...
			for (INT LODDistIndex = 1; LODDistIndex < Template->LODDistances.Num(); LODDistIndex++)
			{
//				debugf(TEXT("LOD Level %d %f"), LODDistIndex, Template->LODDistances(LODDistIndex), *GetFullName());
		...
		if ((LODIndex != -1) && (LODIndex != LODLevel))
		{
			SetLODLevel(LODIndex);
		}	
	}
	...
	// removed this for now as it is causing particles to randomly be rendered on and off.
	// we should be using some part of how we LOD particles for this more than likely
#if 1
	// Bail out if MaxSecondsBeforeInactive > 0 and we haven't been rendered the last MaxSecondsBeforeInactive seconds.
	FLOAT MaxSecondsBeforeInactive = Max( SecondsBeforeInactive, Template->SecondsBeforeInactive );
	if ((MaxSecondsBeforeInactive > 0) && GIsGame)
	{
		FLOAT CurrentTimeSeconds = GWorld->GetTimeSeconds();
		if ((LastRenderTime > 0.0f) && (CurrentTimeSeconds > (LastRenderTime + MaxSecondsBeforeInactive)))
		{
			return;
		}
	}
...
void UParticleSystemComponent::InitParticles()
{
	...
	if (Template != NULL)
	{
		...
		if (EmitterInstances.Num() == 0)
		{
			appMemzero(&SMComponents, sizeof(TArray<UStaticMeshComponent*>));
			//appMemzero(&SMMaterialInterfaces, sizeof(TArray<UMaterialInterface*>));
	...
	for (INT i=0; i<EmitterInstances.Num(); i++)
	{
		...
		if (Instance)
		{
			if (Instance->SpriteTemplate != NULL)
			{
				...
				// set to the LOD which is guarenteed to exist
				else
				{
					Instance->CurrentLODLevelIndex = 0;
					Instance->CurrentLODLevel = Instance->SpriteTemplate->LODLevels(Instance->CurrentLODLevelIndex);
