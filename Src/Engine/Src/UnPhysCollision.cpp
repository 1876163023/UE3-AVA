...
UBOOL FKAggregateGeom::LineCheck(FCheckResult& Result, const FMatrix& Matrix, const FVector& Scale3D,  const FVector& End, const FVector& Start, const FVector& Extent, UBOOL bStopAtAnyHit) const
{
	...
	if(ConvexElems.Num() > 0)
	{
		...
		FMatrix WorldToBox = WorldToLocal;
		...
		if(	Extent.IsZero() )
		{
			...
			for(INT i=0; i<ConvexElems.Num(); i++)
			{
				...
				Convex.LineCheck(TempResult, WorldToBox, LocalEnd, LocalStart, BoxExtent);
		...
		if(Result.Time < 1.f)
		{
			...
			const FLOAT LocalToWorldDet = LocalToWorld.Determinant();
...
UBOOL FKAggregateGeom::PointCheck(FCheckResult& Result, const FMatrix& Matrix, const FVector& Scale3D, const FVector& Location, const FVector& Extent) const
{
	...
	if(Scale3D.IsUniform())
	{
		for(INT i=0; i<BoxElems.Num(); i++)
		{
			...
			UBOOL bBoxHit = !BoxElems(i).PointCheck(Result, BestDistance, ElemTM, Scale3D.X, Location, Extent);
	...
	if(ConvexElems.Num() > 0)
	{
		...
		for(INT i=0; i<ConvexElems.Num(); i++)
		{
			...
			if( Convex.ElemBox.Intersect(LocalBox) )
			{
				...
				UBOOL bConvexHit = !ConvexElems(i).PointCheck(Result, BestDistance, WorldToBox, LocalLocation, BoxExtent);
...
static FLOAT DistanceToLine(const FVector& LineStart, const FVector& LineEnd, const FVector& Point)
{
	...
	FVector PointToStart = LineStart - Point;

	FVector Cross = StartToEnd ^ PointToStart;
...
static UBOOL EnsureHullIsValid(TArray<FVector>& InVerts)
{
	...
	{
		FLOAT TestDist = (InVerts(i) - FirstVert).Size();
	...
	for(INT i=1; i<InVerts.Num(); i++)
	{
		...
		{
			FLOAT TestDist = DistanceToLine(FirstVert, InVerts(FurthestVertIndex), InVerts(i));
	...
	FVector Dir1 = InVerts(FurthestVertIndex) - InVerts(0);
	FVector Dir2 = InVerts(ThirdPointIndex) - InVerts(0);
	FVector PlaneNormal = Dir1 ^ Dir2;
	UBOOL bNormalizedOk = PlaneNormal.Normalize();
	if(!bNormalizedOk)
	{
		return FALSE;
	}
	...
	for(INT i=1; i<InVerts.Num(); i++)
	{
		...
		{
			FLOAT PointPlaneDist = Abs((InVerts(i) - InVerts(0)) | PlaneNormal);
...
void MakeCachedConvexDataForAggGeom(FKCachedConvexData* OutCacheData, FKAggregateGeom* InGeom, const FVector& Scale3D, const TCHAR* debugName)
{
	...
	for(INT i=0; i<InGeom->ConvexElems.Num(); i++)
	{
		FKConvexElem* ConvexElem = &(InGeom->ConvexElems(i));
		...
		if(PhysScaleConvexVerts.Num() > 0)
		{
			...
			INT NewElementIndex = OutCacheData->CachedConvexElements.AddZeroed();
			FNxMemoryBuffer Buffer(&(OutCacheData->CachedConvexElements(NewElementIndex).ConvexElementData));
#if USE_QUICKLOAD_CONVEX
			UBOOL bSuccess = NxCookQLConvexMesh(ConvexMeshDesc, Buffer);
#else
			UBOOL bSuccess = NxCookConvexMesh(ConvexMeshDesc, Buffer);
#endif
...
{
#if SHOW_PHYS_INIT_COSTS || defined(SHOW_SLOW_CONVEX)
	...
	else
	{
		...
		{
			debugf( TEXT("WARNING: Cooking Convex For %s (Scale: %f %f %f)"), debugName, uScale3D.X, uScale3D.Y, uScale3D.Z );
		}
#endif

		MakeCachedConvexDataForAggGeom(&TempCacheData, this, uScale3D, debugName);
...
#if USE_QUICKLOAD_CONVEX
		NxConvexMesh* ConvexMesh = NxCreateQLConvexMesh(Buffer);
#else 
...
void MakeCachedPerTriMeshDataForStaticMesh(FKCachedPerTriData* OutCacheData, UStaticMesh* InMesh, const FVector& Scale3D, const TCHAR* DebugName)
{
	...
	FLOAT Det = Scale3D.X * Scale3D.Y * Scale3D.Z;
	UBOOL bFlipTris = (Det < 0.f);

	// Scale all verts into temporary vertex buffer.
	UINT NumVerts = InMesh->LODModels(0).NumVertices;
	...
	NxCookTriangleMesh(StaticMeshDesc, Buffer);
#endif // WITH_NOVODEX
}
	...
	FVector& HitNormal)
{
	...
	ProjectionInterval.Expand(ProjectedExtent * PULLBACK_FACTOR);

	FLOAT ProjectedStart = Axis | LocalStart;
	FLOAT ProjectedEnd = Axis | LocalEnd;
	...
	if(ProjectedStart < ProjectionInterval.Min)
	{
		...
		CheckMinIntersectTime(MinIntersectTime, HitNormal, (ProjectionInterval.Min - ProjectedStart) / ProjectedDirection, -Axis);
		MaxIntersectTime = Min(MaxIntersectTime, (ProjectionInterval.Max - ProjectedStart) / ProjectedDirection);
	}
	else if(ProjectedStart > ProjectionInterval.Max)
	{
		if(ProjectedDirection > -DELTA)
		{
			return FALSE;
		...
		CheckMinIntersectTime(MinIntersectTime, HitNormal, (ProjectionInterval.Max - ProjectedStart) / ProjectedDirection, Axis);
		MaxIntersectTime = Min(MaxIntersectTime, (ProjectionInterval.Min - ProjectedStart) / ProjectedDirection);
	}
	else
	{
		if(ProjectedDirection > DELTA)
...
static UBOOL TestEdgeSeparatingAxis(
	const FOrientedBox& TraceBox, const FOrientedBox& LocalBox,
	...
	FVector& HitNormal)
{
	...
	return TestSeparatingAxis(TraceBox, LocalBox, Axis, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal);
}

static UBOOL TestFaceSeparatingAxis(
	const FOrientedBox& TraceBox, const FOrientedBox& LocalBox,
	...
	FVector& HitNormal)
{
	return TestSeparatingAxis(TraceBox, LocalBox, FaceNormal, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal);
}
 ...
 */
static UBOOL FindSeparatingAxis(const FOrientedBox& TraceBox, const FOrientedBox& LocalBox,
	const FVector& LocalStart, const FVector& LocalEnd,
	FLOAT& MinIntersectTime, FLOAT& MaxIntersectTime,
	FVector& HitNormal)
{
	// Test separating axes from TraceBox faces (TraceBox.AxisX, TraceBox.AxisY, TraceBox.AxisY).
	if (!TestFaceSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisX, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestFaceSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisY, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestFaceSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisZ, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	// Test separating axes from LocalBox faces ([1 0 0], [0 1 0], [0 0 1]).
	if (!TestFaceSeparatingAxis(TraceBox, LocalBox, LocalBox.AxisX, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestFaceSeparatingAxis(TraceBox, LocalBox, LocalBox.AxisY, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestFaceSeparatingAxis(TraceBox, LocalBox, LocalBox.AxisZ, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	// Test separating axes from TraceBox edges x LocalBox edges (cross products of the above).
	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisX, LocalBox.AxisX, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisX, LocalBox.AxisY, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisX, LocalBox.AxisZ, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisY, LocalBox.AxisX, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisY, LocalBox.AxisY, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisY, LocalBox.AxisZ, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisZ, LocalBox.AxisX, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisZ, LocalBox.AxisY, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}

	if (!TestEdgeSeparatingAxis(TraceBox, LocalBox, TraceBox.AxisZ, LocalBox.AxisZ, LocalStart, LocalEnd, MinIntersectTime, MaxIntersectTime, HitNormal))
	{
		return FALSE;
	}
	return MinIntersectTime >= 0.0f;
}
...
UBOOL FKBoxElem::LineCheck(FCheckResult& Result, const FMatrix& Matrix, FLOAT Scale,  const FVector& End, const FVector& Start, const FVector& Extent) const
{
	...
	else
	{
		...
		TraceBox.AxisX = Matrix.InverseTransformNormal(FVector(1.f,0.f,0.f));
		TraceBox.AxisY = Matrix.InverseTransformNormal(FVector(0.f,1.f,0.f));
		TraceBox.AxisZ = Matrix.InverseTransformNormal(FVector(0.f,0.f,1.f));
		...
		FLOAT MaxIntersectTime = 1.0f;
		FVector HitNormal;

		// Check for overlap
		if (FindSeparatingAxis(TraceBox,LocalBox,LocalStart,LocalEnd,MinIntersectTime,MaxIntersectTime,HitNormal))
		{
...
static const FLOAT ParallelRegion = 0.01f;

static UBOOL TestConvexSweptBoxSeparatingAxis(
								...
								FVector& ExitDir)
{
	...
	if( ProjDirMag < 0.01f )
	{
		if( ProjectedStart < ConvexMin && ProjectedStart > (ConvexMin - ParallelRegion) )
		{
			CloseFeatureNormal = -Line;
			bCloseAndParallel = TRUE;
		}
			
		if( ProjectedStart > ConvexMax && ProjectedStart < (ConvexMax + ParallelRegion) )
		{
			CloseFeatureNormal = Line;
			bCloseAndParallel = TRUE;
		}
...
inline UBOOL TestConvexSweptBoxSeparatingAxis(
								const TArray<FVector>& ConvexVerts,
								const TArray<FPlane>& PermutedConvexVerts,
								const FVector& Line,
								const FVector& Start,
...
UBOOL FKConvexElem::LineCheck(FCheckResult& Result, const FMatrix& WorldToBox,  const FVector& LocalEnd, const FVector& LocalStart, const FVector& BoxExtent) const
{
	...
	else
	{
		...
		// Box faces
		if(!TestConvexSweptBoxSeparatingAxis(VertexData, PermutedVertexData, BoxX ^ BoxY, LocalStart, LocalEnd, BoxX, BoxY, BoxZ, BoxExtent, MinIntersectTime, MaxIntersectTime, HitNormal, ExitDir))
			return TRUE;

		if(!TestConvexSweptBoxSeparatingAxis(VertexData, PermutedVertexData, BoxY ^ BoxZ, LocalStart, LocalEnd, BoxX, BoxY, BoxZ, BoxExtent, MinIntersectTime, MaxIntersectTime, HitNormal, ExitDir))
			return TRUE;

		if(!TestConvexSweptBoxSeparatingAxis(VertexData, PermutedVertexData, BoxZ ^ BoxX, LocalStart, LocalEnd, BoxX, BoxY, BoxZ, BoxExtent, MinIntersectTime, MaxIntersectTime, HitNormal, ExitDir))
			return TRUE;
		...
		{
			if(!TestConvexSweptBoxSeparatingAxis(VertexData, PermutedVertexData, FaceNormalDirections(i), LocalStart, LocalEnd, BoxX, BoxY, BoxZ, BoxExtent, MinIntersectTime, MaxIntersectTime, HitNormal, ExitDir))
				return TRUE;
		}
		...
		{
			if(!TestConvexSweptBoxSeparatingAxis(VertexData, PermutedVertexData, EdgeDirections(i) ^ BoxX, LocalStart, LocalEnd, BoxX, BoxY, BoxZ, BoxExtent, MinIntersectTime, MaxIntersectTime, HitNormal, ExitDir))
				return TRUE;

			if(!TestConvexSweptBoxSeparatingAxis(VertexData, PermutedVertexData, EdgeDirections(i) ^ BoxY, LocalStart, LocalEnd, BoxX, BoxY, BoxZ, BoxExtent, MinIntersectTime, MaxIntersectTime, HitNormal, ExitDir))
				return TRUE;

			if(!TestConvexSweptBoxSeparatingAxis(VertexData, PermutedVertexData, EdgeDirections(i) ^ BoxZ, LocalStart, LocalEnd, BoxX, BoxY, BoxZ, BoxExtent, MinIntersectTime, MaxIntersectTime, HitNormal, ExitDir))
				return TRUE;
		}
		...
		if(bCloseAndParallel)
		{
			...
			Result.Normal = (CloseFeatureNormal - (0.05f * CheckDir)).SafeNormal();
		...
		else
		{
			...
			if( (MaxIntersectTime < -MinIntersectTime) && (LineDir | ExitDir) > 0.9f)
			{
				Result.Time = 1.f;
				return TRUE;
			}
			else
			{
				...
				Result.Normal = -(LocalEnd - LocalStart).SafeNormal();
...
UBOOL FKConvexElem::PointIsWithin(const FVector& LocalLocation, FVector& OutLocalNormal, FLOAT& OutBestDistance) const
{
	...
	for(INT i=0; i<FacePlaneData.Num(); i++)
	{
		...
		FLOAT Dist = Plane.PlaneDot(LocalLocation);
		if(Dist > 0.f)
		{
			return FALSE;
		}
...
static const FLOAT DIR_COMPARE_THRESH = 0.01f;
...
UBOOL FKConvexElem::GenerateHullData()
{
	...
	UBOOL bIsValid = EnsureHullIsValid(PhysScaleConvexVerts);
	if(!bIsValid)
	{
		return FALSE;
	}
	...
	if(bSuccess)
	{
		...
		if(TempMesh)
		{
			...
			INT TriCount = TempMesh->getCount(0,NX_ARRAY_TRIANGLES);
			check(TriCount > 0);

			NxInternalFormat IndexFormat = TempMesh->getFormat(0,NX_ARRAY_TRIANGLES);
			...
			{
				INT I0 = IndexBase[(i*3)+0];
				INT I1 = IndexBase[(i*3)+2];
				INT I2 = IndexBase[(i*3)+1];
			...
			INT NumEdges = EdgeStore.Num()/2;
			for(INT i=0; i<NumEdges; i++)
			{
				INT TestEdge0 = EdgeStore((i*2)+0);
				INT TestEdge1 = EdgeStore((i*2)+1);
				...
				{
					FVector Tri0Normal = TriNormals(Tri0);
					FVector Tri1Normal = TriNormals(Tri1);
					FLOAT FaceDot = Tri0Normal | Tri1Normal;
...
void FKConvexElem::PermuteVertexData(void)
{
	...
	INT NumRemaining = VertexData.Num() % 4;
	// Going to do 4 verts at a time
	INT NumToAdd = ((VertexData.Num() / 4) * 3);
	// The number of verts to process as planes without duplicating verts
	INT NumToProcess = VertexData.Num() - NumRemaining;
	// Presize the array
