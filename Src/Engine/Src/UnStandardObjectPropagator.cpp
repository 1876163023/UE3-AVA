...
void FStandardObjectPropagator::ProcessActorMove(AActor* Actor, const FVector& Location, const FRotator& Rotation)
{
	...
	UBOOL OldbStatic = Actor->bStatic;
	UBOOL OldbMovable = Actor->bMovable;
	Actor->bStatic = false;
	Actor->bMovable = true;

	// move and rotate the actor
	FCheckResult Hit; // dummy
	GWorld->FarMoveActor(Actor, Location, FALSE, TRUE, TRUE);
	GWorld->MoveActor(Actor, FVector(0), Rotation, 0, Hit);
	
	// make lights update
	Actor->ConditionalForceUpdateComponents();
	Actor->InvalidateLightingCache();

	// restore it's staticness
	Actor->bStatic = OldbStatic;
	Actor->bMovable = OldbMovable;
...
AActor* FStandardObjectPropagator::ProcessActorCreate(UClass* ActorClass, FName ActorName, const FVector& Location, const FRotator& Rotation, const TArray<FString>& ComponentTemplatesAndNames)
{
	...
	UBOOL OldbNoDelete = ActorClass->GetDefaultActor()->bNoDelete;
	UBOOL OldbStatic = ActorClass->GetDefaultActor()->bStatic;
	ActorClass->GetDefaultActor()->bNoDelete = false;
	ActorClass->GetDefaultActor()->bStatic = false;
	// spawn the actor
	AActor* Actor = GWorld->SpawnActor(ActorClass, ActorName, Location, Rotation);

	// restore the staticness of the class
	ActorClass->GetDefaultActor()->bNoDelete = OldbNoDelete;
	ActorClass->GetDefaultActor()->bStatic = OldbStatic;
...
void FStandardObjectPropagator::ProcessActorDelete(AActor* Actor)
{
	...
	UBOOL OldbStatic = Actor->bStatic;
	UBOOL OldbNoDelete = Actor->bNoDelete;
	Actor->bStatic = false;
