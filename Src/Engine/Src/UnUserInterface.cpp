...
IMPLEMENT_CLASS(UUIRoot);
	...
	IMPLEMENT_CLASS(UUITexture);
		IMPLEMENT_CLASS(UUITileMotionTexture);
...
UUIScene* GRenderWorldDrawPreventer = NULL;

UBOOL IsRenderWorldBanned()
{
	return GRenderWorldDrawPreventer && GRenderWorldDrawPreventer->bDisableWorldRendering;
}
...
void UUISceneClient::InitializeClient( UUISkin* InitialSkin )
{
	...
	eventInitializeSceneClient( InitialSkin );
}
...
void UUISceneClient::execUpdateCanvasToScreen _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void UUISceneClient::execInitializeScene _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
 ...
 */
UBOOL UUISceneClient::OpenScene( UUIScene* Scene, ULocalPlayer* SceneOwner/*=NULL*/, UUIScene** OpenedScene/*=NULL*/, BYTE Priority /*= 1*/ )
{
	return FALSE;
}
void UUISceneClient::execOpenScene _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UUIScene,Scene);
	P_GET_OBJECT_OPTX(ULocalPlayer,SceneOwner,NULL);
	P_GET_OBJECT_OPTX_REF(UUIScene,OpenedScene,NULL);
	P_GET_BYTE_OPTX(Priority,1/*EScenePrior_UIScene_Normal*/)
	P_FINISH;
	*(UBOOL*)Result=OpenScene(Scene,SceneOwner,pOpenedScene,Priority);
}
void UUISceneClient::execIsSceneInitialized _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UUIScene,Scene);
	P_FINISH;
...
void UGameUISceneClient::UpdateActiveControl()
{
	...
	for ( INT SceneIndex = ActiveScenes.Num() - 1; !bHitDetected && SceneIndex >= 0; SceneIndex-- )
	{
		...
		&&	 MousePosition.Y <= Scene->GetPosition(UIFACE_Bottom,EVALPOS_PixelViewport) )
		{
			...
			{
				// @TODO : 에픽 ContainsPoint와 병합 ( Deproject, PixelToCanvas 등등이 합쳐져야함  )
				UUIObject* Widget = RenderStack(RenderIndex);
				FLOAT* RenderBounds = Widget->RenderBounds;
				FMatrix Transform = Widget->GenerateTransformMatrix();
				FVector4 TransformedMousePos = Transform.InverseTransformFVector( FVector(MousePosition.X, MousePosition.Y, 0.0f) );

				if ( Widget != NULL &&
					Widget->ContainsPoint(MousePosition) &&
					Widget->HasInactiveStateOfClass( UUIState_Active::StaticClass() ) )
				{
					...
					if ( Widget->IsActive(TEMP_SPLITSCREEN_INDEX,&ActiveStateIndex) )
					{
						...
						Widget->OnMouseHovering( TransformedMousePos.X, TransformedMousePos.Y );
						bHitDetected = TRUE;
						break;
					}
					else
					{
						...
						if ( ActiveControl != NULL )
						{
							...
							if ( !ActiveControl->HasActiveStateOfClass(UUIState_Active::StaticClass(),Widget->GetBestPlayerIndex()) )
							{
								debugf( NAME_Log, TEXT("ActiveContol has not Active State %s %s" ), *Scene->GetFullName(), *ActiveControl->GetFullName() );
							}


							if ( ActiveControl->DeactivateStateByClass(UUIState_Active::StaticClass(),Widget->GetBestPlayerIndex()) )
							{
								// then activate the "Active" state of the widget that is now under the mouse
								if ( Widget->ActivateStateByClass(UUIState_Active::StaticClass(),Widget->GetBestPlayerIndex()) )
								{
									Widget->PlayUISound(Widget->MouseEnterCue, Widget->GetBestPlayerIndex());
									Widget->OnMouseHovering( TransformedMousePos.X, TransformedMousePos.Y );
									bHitDetected = TRUE;
									break;
								}
						...
						// otherwise, just attempt to change this widget's state to "Active"
						else if ( Widget->ActivateStateByClass(UUIState_Active::StaticClass(),Widget->GetBestPlayerIndex()) )
						{
							Widget->PlayUISound(Widget->MouseEnterCue, Widget->GetBestPlayerIndex());
							Widget->OnMouseHovering( TransformedMousePos.X, TransformedMousePos.Y );
							bHitDetected = TRUE;							
			...
			if ( Scene->bPreventChildSceneMouseHover )
				break;
		}
	}
 ...
 * @param	OpenedScene		the scene that was actually opened.  If Scene is located in a content package, OpenedScene will be
 *							the copy of the scene that was created.  Otherwise, OpenedScene will be the same as the scene passed in.
 * @param	Priority		씬의 우선순위 결정 (우선순위가 높은것이 위로 오도록), 언젠가 Weight란 이름으로 에픽엔진에 추가되면 변경바람.
 *
 * @return TRUE if the scene was successfully opened
 */
UBOOL UGameUISceneClient::OpenScene( UUIScene* Scene, ULocalPlayer* SceneOwner/*=NULL*/, UUIScene** OpenedScene/*=NULL*/, BYTE Priority/*= EScenePrior_Script_Low*/ )
{
	UBOOL bResult = FALSE;
#ifdef DEBUG_UIPERF
	DOUBLE StartTime = appSeconds();
	...
	if ( Scene != NULL )
	{
		...
		if ( ExistingScene == NULL )
		{
			...
			{
				ActivateScene(ExistingScene, Priority);

				// if the caller needs a reference to the scene copy that was activated, set that now
				if ( OpenedScene != NULL )
				{
					if ( ExistingScene->FadeInTimeWhenOpen > 0.0f )
					{
						if ( latestOpendScene != NULL )
							latestOpendScene->Opacity = OpendSceneDefaultOpacity;
						latestOpendScene			= ExistingScene;
						OpendSceneDefaultOpacity	= ExistingScene->Opacity;
						ExistingScene->Opacity	= 0.0;
						OpendSceneTime				= ExistingScene->FadeInTimeWhenOpen;
					}
...
UBOOL UGameUISceneClient::CloseScene( UUIScene* Scene )
{
	...
	if ( Scene != NULL )
	{
		...
		if ( ExistingScene != NULL )
		{
			if ( latestOpendScene == ExistingScene )
				latestOpendScene = NULL;
			//debugf(NAME_Warning, TEXT("CloseScene: InputScene '%s' ExistingScene '%s'"), *Scene->GetFullName(), *ExistingScene->GetFullName());
 ...
 */
void UGameUISceneClient::ActivateScene( UUIScene* SceneToActivate , BYTE Priority )
{
#ifdef DEBUG_UIPERF
	DOUBLE StartTime = appSeconds();
#endif

//	원래 코드- 맨뒤에 새로 생성된 씬을 추가
//	ActiveScenes.AddItem(SceneToActivate);	
	
//  수정된 코드 - 우선순위가 같거나 자신보다 낮을 때 그 Scene의 뒤에다 붙임.
	INT InsertIndex = 0;
	for( INT i = ActiveScenes.Num() - 1 ; i >= 0 ; i-- )
	{
		UUIScene* Scene = ActiveScenes(i);
		if( Scene == NULL )
			continue;
	...
	SceneToActivate->Priority = Priority;
	ActiveScenes.InsertItem( SceneToActivate, InsertIndex );
	...
	debugf(NAME_DevUI, TEXT("%-24s %-30s %-10i >>>>>>"), TEXT(">>>>>> OpenScene"), *SceneToActivate->GetWidgetPathName(), PlayerIndex);
	...
	if (SceneToActivate->bDisableWorldRendering)
	{
...
UBOOL UGameUISceneClient::DeactivateScene( UUIScene* SceneToDeactivate )
{
	...
	if (SceneToDeactivate->bDisableWorldRendering)
	{
		ULocalPlayer* LP = GEngine->GamePlayers(0);
		if(LP && LP->ViewportClient)
		{
			LP->ViewportClient->bDisableWorldRendering = FALSE;
		}
	}
	...
	if ( SceneStackLocation != INDEX_NONE )
	{
		...
		//	    {
		//		    debugf(NAME_DevUI, TEXT("%-24s %-40s <<<<<<<"), TEXT("<<<<<< CloseScene"), *Scene->GetWidgetPathName());
		...
		if ( ActiveScenes.IsValidIndex(SceneStackLocation) )
		{
		    UUIScene* Scene = ActiveScenes(SceneStackLocation);
		    if (Scene->PlayerOwner == SceneToDeactivate->PlayerOwner)
		    {
			    debugf(NAME_DevUI, TEXT("%-24s %-40s <<<<<<<"), TEXT("<<<<<< CloseScene"), *Scene->GetWidgetPathName());
			    ActiveScenes.Remove(SceneStackLocation);
...
{
	UUIScene* pScene = NULL;
	UBOOL bDisplayCursor = FALSE;
	for ( INT SceneIndex = 0; SceneIndex < ActiveScenes.Num(); SceneIndex++ )
	{
		if ( ActiveScenes(SceneIndex)->bDisplayCursor )
		{
			pScene = ActiveScenes(SceneIndex);
			bDisplayCursor = TRUE;
			break;
		}
	}
	...
	if ( bRenderCursor && CurrentMouseCursor == NULL )
	{
		// 커스텀 켜서를 설정하기 위해서 추가.(2007/05/22 고광록)
		if ( pScene->DisplayCursorName.Len() == 0 )
			ChangeMouseCursor( TEXT("Arrow") );
		else
			ChangeMouseCursor( *(pScene->DisplayCursorName) );
	}
}
...
{
	eventConditionalPause(IsUIActive(SCENEFILTER_PausersOnly|SCENEFILTER_IgnoreTransient));
}
...
{
	return !IsUIActive(SCENEFILTER_PausersOnly|SCENEFILTER_IgnoreTransient);
}
...
void UGameUISceneClient::execCreateScene _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UGameUISceneClient::CreateTransientScene()
{
	...
	if ( TransientScene == NULL )
	{
		...
		ActivateScene(TransientScene, EScenePrior_Script_Low);
	}
}
...
void UGameUISceneClient::Tick(FLOAT DeltaTime)
{
	...
	if ( OpendSceneTime > 0.0 && latestOpendScene != NULL )
	{
		OpendSceneTime -= DeltaTime;
		if ( OpendSceneTime < 0.0 )
		{
			OpendSceneTime				= 0.0;
			latestOpendScene->Opacity	= OpendSceneDefaultOpacity;
		}
		...
		{
			latestOpendScene->Opacity	= OpendSceneDefaultOpacity - OpendSceneTime/latestOpendScene->FadeInTimeWhenOpen;
		}
	}
 ...
 */
UBOOL UGameUISceneClient::IsUIActive( DWORD Flags/*=SCENEFILTER_IgnoreTransient*/ ) const
{
	UBOOL bResult = FALSE;
	...
	for (INT SceneIndex = 0; SceneIndex < ActiveScenes.Num() ; SceneIndex++)
	{
		...
		if ( Scene != NULL )
		{
			if ( (Flags&SCENEFILTER_IgnoreTransient) == 0 && Scene == GetTransientScene() )
			{
				bResult = TRUE;
				break;
			}

			if ( (Flags&SCENEFILTER_InputProcessorOnly) != 0 && Scene->delegateGetSceneInputMode() != INPUTMODE_None )
			{
				bResult = TRUE;
				break;
			}
			...
			// @TODO : UIScene bUsesPrimitives, bEnableScenePostProcessing 플래그 추가
 ...
 */
UBOOL UGameUISceneClient::InputAxis( INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
	UBOOL bResult = FALSE;
 ...
 */
UBOOL UGameUISceneClient::InputChar(INT ControllerId, const FInputCompositionStringData& CompStrData)
{
	UBOOL bResult = FALSE;
	...
	{
		UUIScene* Scene = ActiveScenes(SceneIndex);
		if ( Scene->InputChar(ControllerId, CompStrData) )
		{
			bResult = TRUE;
			break;
		}
...
//  [9/28/2006 otterrrr], Information for Drawing IME Candidate Window
UBOOL UGameUISceneClient::InputCandidate(INT ControllerId, const FInputCandidateStringData& CandStrData )
{
	UBOOL bResult = FALSE;
	...
	{
		UUIScene* Scene = ActiveScenes(SceneIndex);
		if( Scene->InputCandidate(ControllerId, CandStrData))
		{
			bResult =TRUE;
			break;
		}
...
/**
 * CHT, CHS에서 ReadingString을 보여줘야할 때 들어옴
 * 
 * @param ControllerId - The controller which the axis movement is from.
 * @param ReadStrData - ReadingString only
 *
 * @return TRUE to consume the candidate string
 */
UBOOL UGameUISceneClient::InputReadingString(INT ControllerId, const FInputReadingStringData& ReadStrData )
{
	UBOOL bResult = FALSE;
	...
	{
		UUIScene* Scene = ActiveScenes(SceneIndex);
		if( Scene->InputReadingString(ControllerId, ReadStrData))
		{
			bResult =TRUE;
			break;
		}
 ...
 * @param	WidgetIsParent	indicates whether the base values are relative to the widget face or the widget's owner's face. 
 *							in absolute pixel values
 */
void FUIScreenValue::CalculateBaseValue( const UUIScreenObject* OwnerWidget, EUIWidgetFace Face, EPositionEvalType Type, FLOAT& BaseValue, FLOAT& BaseExtent, UBOOL OwnerIsParent /*= FALSE*/ )
{
	...
	case EVALPOS_PercentageOwner:
		{
			...
			const UUIScreenObject* WidgetParent = OwnerIsParent == TRUE ? OwnerWidget : OwnerWidget->GetParent();
	...
	if ( OwnerWidgetContainer != NULL )
	{
		...
		{
		case UIFACE_Left:
			BaseValue = OwnerWidgetContainer->GetPosition(Face, EVALPOS_PixelViewport, TRUE);
			if ( Type != EVALPOS_PixelOwner && Type != EVALPOS_PixelScene )
			{
				// type must be either PercentageScene or PercentageOwner
				BaseExtent = OwnerWidgetContainer->GetPosition(UIFACE_Right, EVALPOS_PixelViewport, TRUE) - BaseValue;
				if ( OwnerIsParent )
				{
					BaseValue = 0.f;
				}
		...
		case UIFACE_Top:
			...
			{
				// type must be either PercentageScene or PercentageOwner
				BaseExtent = OwnerWidgetContainer->GetPosition(UIFACE_Bottom, EVALPOS_PixelViewport, TRUE) - BaseValue;
				if ( OwnerIsParent )
				{
					BaseValue = 0.f;
				}
		...
		case UIFACE_Right:
		case UIFACE_Bottom:
			BaseValue = OwnerWidget->GetPosition(UUIRoot::GetOppositeFace(Face), EVALPOS_PixelViewport, TRUE );
			if ( Type != EVALPOS_PixelOwner && Type != EVALPOS_PixelScene )
			{
				// type must be either PercentageScene or PercentageOwner
				...
				BaseExtent = OwnerWidgetContainer->Position.GetBoundsExtent(OwnerWidgetContainer, Orientation, EVALPOS_PixelViewport);
				if ( OwnerIsParent )
				{
					BaseValue = 0.f;
				}
 ...
 */
FLOAT FUIScreenValue::GetValue( const UUIScreenObject* OwnerWidget, EPositionEvalType OutputType/*=EVALPOS_None*/ ) const
{
	FLOAT Result = Value;
	if ( OwnerWidget == NULL || OutputType == EVALPOS_None )
		return Result;

	EUIWidgetFace Face = Orientation == UIORIENT_Horizontal ? Face = UIFACE_Left : Face = UIFACE_Top;
	...
	if ( OutputType == EVALPOS_PixelViewport || 
		 OutputType == EVALPOS_PixelScene ||
		 OutputType == EVALPOS_PixelOwner)
	{
		Result = appRound(Result);
	}
...
void FUIScreenValue::SetValue( class UUIScreenObject* OwnerWidget, FLOAT NewValue, EPositionEvalType InputType/*=EVALPOS_PixelViewport*/ )
{
	...
	if (ScaleType == EVALPOS_PixelViewport || 
		ScaleType == EVALPOS_PixelScene ||
		ScaleType == EVALPOS_PixelOwner)
	{
		ConvertedValue = appRound(ConvertedValue);
	}
 ...
 * @return	the actual value for this UIScreenValue, in pixels or percentage, for the face specified.
 */
FLOAT FUIScreenValue_Bounds::GetPositionValue( const UUIScreenObject* OwnerWidget, EUIWidgetFace Face, EPositionEvalType OutputType/*=EVALPOS_None*/ ) const
{
	...
	// first, determine the actual value of this screen position
	if ( IsPositionCurrent(Face) && OwnerWidget->IsA(UUIObject::StaticClass()) )
	{
		Result = ((UUIObject*)OwnerWidget)->RenderBounds[Face];
	}
	else
	{
	...
	{
		// the output type might be 
		FUIScreenValue::CalculateBaseValue(OwnerWidget, Face, OutputType, BaseValue, BaseExtent);
...
void FUIScreenValue_Bounds::SetPositionValue( UUIScreenObject* OwnerWidget, FLOAT NewValue, EUIWidgetFace Face, EPositionEvalType InputType/*=EVALPOS_PixelOwner*/, UBOOL bResolveChange/*=TRUE*/ )
{
	...
	if (ScaleType[Face] == EVALPOS_PixelViewport || 
		ScaleType[Face] == EVALPOS_PixelScene ||
		ScaleType[Face] == EVALPOS_PixelOwner)
	{
		ConvertedValue = appRound(ConvertedValue);
	}
 ...
 */
FLOAT FUIScreenValue_Bounds::GetBoundsExtent( const UUIScreenObject* OwnerWidget, EUIOrientation Dimension, EPositionEvalType OutputType/*=EVALPOS_PixelOwner*/ ) const
{
	FLOAT MinValue = GetPositionValue(OwnerWidget, Dimension == UIORIENT_Horizontal ? UIFACE_Left : UIFACE_Top, OutputType);
	FLOAT MaxValue = GetPositionValue(OwnerWidget, Dimension == UIORIENT_Horizontal ? UIFACE_Right : UIFACE_Bottom, OutputType);
 ...
 */
void FUIScreenValue_AutoSizeRegion::CalculateBaseValue( EUIOrientation Orientation, EUIAutoSizeEvalType EvaluationType, UUIScreenObject* OwnerWidget, FLOAT& BaseExtent )
{
	checkSlow(Orientation<UIORIENT_MAX);
	checkSlow(EvaluationType<UIAUTOSIZEEVAL_MAX);
	...
	if ( EvaluationType == UIAUTOSIZEEVAL_PercentViewport )
	{
		FVector2D ViewportSize;
		if ( OwnerWidget->GetViewportSize(ViewportSize) )
		{
	...
	else
	{
		...
		if ( BaseWidget == NULL || EvaluationType == UIAUTOSIZEEVAL_PercentScene )
		{
			BaseWidget = OwnerWidget->GetScene();
		}

		checkSlow(BaseWidget);
		BaseExtent = BaseWidget->Position.GetBoundsExtent(BaseWidget, Orientation, EVALPOS_PixelViewport);
	}
}
 ...
 */
FLOAT FUIScreenValue_AutoSizeRegion::GetValue( EUIAutoSizeConstraintType ValueType, EUIOrientation Orientation, EUIAutoSizeEvalType OutputType, UUIScreenObject* OwnerWidget ) const
{
	checkSlow(ValueType<UIAUTOSIZEREGION_MAX);
	checkSlow(OutputType<UIAUTOSIZEEVAL_MAX);
	checkSlow(Orientation<UIORIENT_MAX);
	checkSlow(OwnerWidget);
	...
	if ( Result > 0 && EvalType[ValueType] != OutputType )
	{
		...
		if ( EvalType[ValueType] != UIAUTOSIZEEVAL_Pixels )
		{
			CalculateBaseValue(Orientation, static_cast<EUIAutoSizeEvalType>(EvalType[ValueType]), OwnerWidget, BaseExtent);
			Result *= BaseExtent;
		}

		// then, convert the value into the expected output type if it isn't pixels
		if ( OutputType != UIAUTOSIZEEVAL_Pixels )
		{
 ...
 */
FLOAT FAutoSizeData::GetPaddingValue( EUIOrientation Dimension, EPositionEvalType OutputType, class UUIScreenObject* OwnerWidget ) const
{
	//  [2006/11/9 YTS, modify] 'Padding' is accessible without 'AutoSize' when you set the member 'bUsePadding' true
	return ( bAutoSizeEnabled || bUsePadding )? Padding.GetValue( Dimension, OutputType, OwnerWidget ) : 0.0f;
}
 ...
 */
FLOAT FAutoSizeData::GetMinValue( EUIAutoSizeEvalType OutputType, EUIOrientation Orientation, UUIScreenObject* OwnerWidget ) const
{
	FLOAT Result = 0.f;
 ...
 *			is not enabled.
 */
FLOAT FAutoSizeData::GetMaxValue( EUIAutoSizeEvalType OutputType, EUIOrientation Orientation, UUIScreenObject* OwnerWidget ) const
{
	FLOAT Result = 0.f;
...
void FUIDockingSet::UpdateDockingSet( EUIWidgetFace Face )
{
	...
	if ( bResolved[Face] == 0 || !OwnerWidget->Position.IsPositionCurrent(Face) )
	{
		...
		if ( IsDocked(Face) )
		{
			...
			if ( DockTarget != NULL )
			{
				TargetValue = DockTarget->GetPositionExtent( DockTargetFace );
			}
			else
			{
			...
			if ( !IsDocked(OppositeFace) )
			{
				...
				else if ( (Face == UIFACE_Right && IsWidthLocked()) || (Face == UIFACE_Bottom && IsHeightLocked()) )
				{
					...
					// 위의 엔진 새 코드를 대체해서 사용.
					// 위의 엔진 새 코드로 하면 전혀 상관없는 RenderBounds를 계산에 끌어들인다. (RenderBounds는 계산의 결과이지 과정에 포함이 안된다)
					Extent = OwnerWidget->GetBounds( Face == UIFACE_Right ? UIORIENT_Horizontal : UIORIENT_Vertical, EVALPOS_PixelViewport);
...
{
	SetLastFocusedControl(GetFocusedControl());
	...
	else
	{
		...
		if ( Material != NULL )
		{
			...
			if(SceneClient)
			{
				...
				::DrawTile(Canvas,X, Y, XL, YL, U, V, UL, VL, SceneClient->OpacityParameter->GetInstanceInterface(0));
...
void UUITexture::Render_Texture( FCanvas* Canvas, const FRenderParameters& Parameters )
{
	...
	if ( Image != NULL && ImageStyleData.IsInitialized() )
	{
		...
		FRenderParameters ImageParameters = Parameters;
		FUICombinedStyleData ImageStyleData = this->ImageStyleData;
		ImageStyleData.ImageColor.A *= Parameters.DrawAlpha;
		...
		if ( AdjustedParameters.DrawCoords.IsZero() && bUsingFallbackTexture
			&& ImageStyleData.FallbackImage )
		{
			AdjustedParameters.DrawCoords = ImageStyleData.AtlasCoords;
			ImageParameters.ImageExtent = FVector2D(AdjustedParameters.DrawCoords.UL, AdjustedParameters.DrawCoords.VL);
		}
		AdjustRenderParameters(Image, ImageStyleData, ImageParameters, AdjustedParameters);
	...
	else if ( Image != NULL )
	{
		...
		StyleData.ImageColor = FLinearColor::White;
		StyleData.ImageColor.A *= Parameters.DrawAlpha;
		DrawTile( Canvas, Image, StyleData, Parameters.DrawX, Parameters.DrawY, Parameters.DrawXL, Parameters.DrawYL, Parameters.DrawCoords.U, Parameters.DrawCoords.V, Parameters.DrawCoords.UL, Parameters.DrawCoords.VL );
	}
}
...
void UUITexture::CalculateExtent( FLOAT& out_SizeX, FLOAT& out_SizeY ) const
{
	...
	if ( ImageTexture != NULL )
	{
		...
		{
			UMaterialInstance* Material = Cast<UMaterialInstance>(ImageTexture);
			if ( Material != NULL )
			{
...
* @param	Parameters	the bounds for the region that this texture can render to.
*/
void UUITileMotionTexture::Render_Texture( FCanvas* Canvas, const FRenderParameters& Parameters )
{
	FRenderParameters RenderParameters = Parameters;

	const INT MaxTileCount = TileDimension[UIORIENT_Horizontal] * TileDimension[UIORIENT_Vertical];
	FLOAT TimeElapsed = GWorld->GetTimeSeconds() - UpdateTime;
	INT LinearIndex = OffsetIndex;
	if( MotionPeriod > 0.f && ((MotionRepeat == 0) || TimeElapsed < (MotionPeriod * MotionRepeat)) )
	{
		NumActivated = Clamp( NumActivated, 0, MaxTileCount );
		FLOAT Ratio = TimeElapsed / MotionPeriod;
		LinearIndex += ( NumActivated *  (Ratio - (FLOAT)appTrunc(Ratio)) );
	}

	if( LinearIndex > 0 )
	{
		LinearIndex = LinearIndex % MaxTileCount;
		INT DimDiff[] = { LinearIndex % TileDimension[UIORIENT_Horizontal] , LinearIndex / TileDimension[UIORIENT_Horizontal] };
		RenderParameters.DrawCoords.U += ( DimDiff[UIORIENT_Horizontal] * RenderParameters.DrawCoords.UL);
		RenderParameters.DrawCoords.V += ( DimDiff[UIORIENT_Vertical] * RenderParameters.DrawCoords.VL);
	}
	
	Super::Render_Texture( Canvas, RenderParameters );
}
...
* @param	Extent	set to the width/height of this UITexture's source texture
*/
void UUITileMotionTexture::CalculateExtent( FVector2D& Extent ) const
{
	CalculateExtent(Extent.X, Extent.Y);
}
...
* @param	SizeY	[out] filled in with the height of this texture's material
*/
void UUITileMotionTexture::CalculateExtent( FLOAT& out_SizeX, FLOAT& out_SizeY ) const
{
	Super::CalculateExtent( out_SizeX, out_SizeY );
}
			...
			for( INT StateIdx = 0; StateIdx < CurrentClassInputAliasMap->WidgetStates.Num(); StateIdx++ )
			{
				...
				for(INT ActionIdx = 0; ActionIdx < StateInputAliasList.StateInputAliases.Num(); ActionIdx++)
				{
					...
					for ( INT KeyIdx = 0; KeyIdx < InputAliasKeyList.LinkedInputKeys.Num(); KeyIdx++ )
					{
						...
						InputMap->InputAliasLookupTable.Add(InputKeyData.InputKeyName, FUIInputAliasValue(InputKeyData.ModifierKeyFlags, InputAliasKeyList.InputAliasName));
					}
				}
			}
		}
...
void UUIState::CreateStateSequence( FName SequenceName/*=NAME_None*/ )
{
	...
	else if ( Owner->HasAnyFlags(RF_ArchetypeObject) || Owner->IsInUIPrefab() )
	{
		Flags |= RF_Public;
	}
...
void UUIState::OnActivate( UUIScreenObject* Target, INT PlayerIndex, UBOOL bPushState )
{
	...
	if ( bPushState == TRUE )
	{
//		debugf(TEXT("OnActivate(%s) Push(%s)"), *Target->GetName(), *GetName());
		// add the state to the target's list of states if it doesn't contain this state already
		Target->StateStack.Push(this);
	}
...
void UUIState::OnDeactivate( UUIScreenObject* Target, INT PlayerIndex, UBOOL bPopState )
{
	...
	if ( GIsGame && Target->EventProvider != NULL )
	{
		//@todo - this could also be done in the Activated method of the UIEvent_OnLeaveState, but I'm not sure 
		// which way is better...
...
UBOOL UUIState::DeactivateStateNative(class UUIScreenObject* Target,INT PlayerIndex)
{
	return DeactivateState(Target, PlayerIndex);
}

void UUIState::OnDeactivateNative(class UUIScreenObject* Target,INT PlayerIndex,UBOOL bPoppedState)
{
}
...
void UUIState_Focused::OnActivate( UUIScreenObject* Target, INT PlayerIndex, UBOOL bPushState )
{
	if ( bPushState == TRUE )
	{
		// add the state to the target's list of states
//		Target->StateStack.Push(this);

		// 마우스로 클릭이 된 상태면 Focused, Active가 동시에 있게 되는데
		// Active가 더 Top으로 가서 CurrentState로 처리되도록 앞쪽에 넣어준다.(2007/01/31 고광록)
		
		INT ActiveIndex;
		if ( Target->HasActiveStateOfClass(UUIState_Active::StaticClass(), PlayerIndex, &ActiveIndex) )
		{
			Target->StateStack.InsertItem(this, ActiveIndex);
		}
		else
		{
			Target->StateStack.Push(this);
