...
DECLARE_MEMORY_STAT(TEXT("Pool Alloc Size"),STAT_TexturePoolAllocatedSize,STATGROUP_Streaming);
DECLARE_MEMORY_STAT(TEXT("Pool Avail Size"),STAT_TexturePoolAvailableSize,STATGROUP_Streaming);
...
DECLARE_CYCLE_STAT(TEXT("Audio Resource Creation"),STAT_AudioResourceCreationTime,STATGROUP_Streaming);
...
void FStreamingManagerTexture::UpdateResourceStreaming( FLOAT DeltaTime )
{
	...
	if( bRHISupportsMemoryStats )
	{
		...
		SET_DWORD_STAT(STAT_TexturePoolAvailableSize,AvailableMemorySize);
	}
	else
	{
		SET_DWORD_STAT(STAT_TexturePoolAllocatedSize,0);
		SET_DWORD_STAT(STAT_TexturePoolAvailableSize,0);		
	}
...
INT FStreamingHandlerTextureStatic::GetWantedMips( UTexture2D* Texture, const TArray<FStreamingViewInfo>& ViewInfos, FLOAT FudgeFactor, INT LODAllowedMips )
{
	...
	if( ViewInfos.Num() )
	{
		...
		for( INT LevelIndex=0; LevelIndex<GWorld->Levels.Num() && !bShouldAbortLoop; LevelIndex++ )
		{
			...
			TArray<FStreamableTextureInstance>* TextureInstances	= Level->TextureToInstancesMapPerPlatform[GRHIShaderPlatform].Find( Texture );
...
INT FStreamingHandlerTextureLevelForced::GetWantedMips( UTexture2D* Texture, const TArray<FStreamingViewInfo>& ViewInfos, FLOAT FudgeFactor, INT LODAllowedMips )
{
	...
	{
		ULevel* Level = GWorld->Levels(LevelIndex);
		if( Level->ForceStreamTexturesPerPlatform[GRHIShaderPlatform].Find( Texture ) )
		{
