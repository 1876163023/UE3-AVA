...
void UDecalManager::Init()
{
	...
	{
		UClass* Class = *It;
		if ( Class->IsChildOf( UDecalLifetime::StaticClass() ) && !(Class->ClassFlags & CLASS_Abstract) )
		{
			LifetimePolicies.AddItem( (UDecalLifetime*)StaticConstructObject( Class, GetOuter() ) );
		}
	}
}
...
{
	/// !!!! 기존에 저장되어 있던 DecalManager를 Invalidate!
	if (LifetimePolicies.Num() < 3) const_cast<UDecalManager*>(this)->Init();
