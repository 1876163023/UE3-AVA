...
void UPolys::StaticConstructor()
{
	...
	DWORD SkipIndexIndex = Class->EmitStructArrayBegin( STRUCT_OFFSET( UPolys, Element ), sizeof(FPoly) );
	Class->EmitObjectReference( STRUCT_OFFSET( FPoly, Actor ) );
	Class->EmitObjectReference( STRUCT_OFFSET( FPoly, Material ) );
