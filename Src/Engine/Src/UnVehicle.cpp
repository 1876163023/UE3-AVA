...
UBOOL AVehicle::moveToward(const FVector &Dest, AActor *GoalActor )
{
	...
	if ( GoalActor && Controller->CurrentPath )
	{
		...
		// check if on next path already - FIXMESTEVE - want this for regular pawns as well! - FIXME - also should work when no currentpath (moving to initial path on network)
	...
	else
	{
		...
		else if ( WorldInfo->TimeSeconds - AIMoveCheckTime > 0.2f )
		{
			...
			if ( Rise == 0.f )
			{
				...
				GWorld->SingleLineCheck(Hit, this, Location + 500.f * DirNormal, Location, TRACE_World|TRACE_Pawns);
				if ( Hit.Actor )
				{
...
void AVehicle::SteerVehicle(FVector Direction)
{
	...
	if ( bTurnInPlace )
	{
		...
		if ( ViewDot < 0.9f )
		{
			if ( (ViewDot < -0.9f) && (Distance > 0.5f * CylinderComponent->CollisionRadius)  )
				Throttle = -1.f;
			else
				Throttle = 0.f;
		}
...
{
	if ( !GWorld->GetGameInfo() || !GWorld->GetGameInfo()->bAllowVehicles || !bCollideActors )
		return false;
...
*/
FLOAT AVehicle::SecondRouteAttempt(ANavigationPoint* Anchor, ANavigationPoint* EndAnchor, NodeEvaluator NodeEval, FLOAT BestWeight, AActor *goal, FVector GoalLocation, FLOAT StartDist, FLOAT EndDist)
{
	if ( Driver )
	{
		...
		ANavigationPoint* BestDest = Driver->BestPathTo(NodeEval,Anchor,&BestWeight, false);
 ...
 */
FVector AVehicle::GetTargetLocation(AActor *RequestedBy)
{
	if ( Mesh )
	{
...
*/
UBOOL AVehicle::IsRelevancyOwnerFor(AActor* ReplicatedActor, AActor* ActorOwner)
{
