...
void UFaceFXAsset::UnmountFaceFXAnimSet( UFaceFXAnimSet* AnimSet )
{
	...
	if( InternalActor && AnimSet )
	{
		...
		if( InternalAnimSet )
		{
			...
			FxBool bResult = InternalActor->UnmountAnimSet(SetName);
		}
	}
...
FString UFaceFXAsset::GetDesc()
{
	...
	if( Actor )
	{
		NumFaceGraphNodes = static_cast<INT>(Actor->GetFaceGraph().GetNumNodes());
...
void UFaceFXAsset::Serialize(FArchive& Ar)
{
	...
	if( Ar.Ver() < VER_FACEFX_1_5_UPGRADE )
	{
		USkeletalMesh* DummyMesh;
		Ar << DummyMesh;
	}
	...
	else if( Ar.IsLoading() )
	{
        ...
        if( Ar.Ver() < VER_FACEFX_1_5_UPGRADE )
		{
			TArray<FString> PreviewMorphSetPaths;
			Ar << PreviewMorphSetPaths;
		}
