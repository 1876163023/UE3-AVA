...
INT _NET_BALANCE_VALUE			= 3000;
INT _NET_SPEED_CALC_BYTES		= 1000;
INT _NET_SPEED_MIN				= 1500;
INT _NET_SPEED_MAX				= 6000;
FLOAT _NET_SPEED_SUM_RATE		= 0.4f;
INT _NET_MAX_QUEUED_BYTES		= 100;
FLOAT _NET_DELTA_BYTES_RATE		= 1.3f;

UBOOL UNetConnection::bIgnoreNetReadyCheck = FALSE;


#ifdef _BT_TEST_BY_CRAZY_LOG
...
//	{
//		static TCHAR buf[MAX_PATH+1];
//		_tcsncpy(buf, _GetAppPath(), sizeof(buf)/sizeof(TCHAR));
//		TCHAR* dot = _tcsrchr(buf, TEXT('\\'));
//		*dot = TEXT('\0');
//		path = buf;
//	}
//	return path;
...
LPCTSTR _GetComputerName()
{
	static LPCTSTR name = NULL;
	if (name == NULL)
	{
		static TCHAR buf[MAX_COMPUTERNAME_LENGTH+1];
		DWORD len = MAX_COMPUTERNAME_LENGTH;
		::GetComputerName(buf, &len);
		buf[MAX_COMPUTERNAME_LENGTH] = TEXT('\0');
		name = buf;
	}
	return name;
}

LPCTSTR _GetTimestamp()
{
	static TCHAR buf[64];
	SYSTEMTIME st;
	::GetLocalTime(&st);
	_stprintf(buf, TEXT("%02d%02d%02d_%02d%02d%02d_%03d"), st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
	return buf;
}

class _BandwidthProfiler
{
...
public:
	...
	_BandwidthProfiler() : fp(NULL), bLog(FALSE)
	{
		GConfig->GetBool(TEXT("avaNet"), TEXT("EnableBTTestProfile"), bLog, GNetIni);

		_sntprintf(fn, MAX_PATH, TEXT("BandwidthTest_%s_%s.log"), _GetComputerName(), _GetTimestamp());
		fn[MAX_PATH] = TEXT('\0');
		_sntprintf(path, MAX_PATH, TEXT("%s%sfn"), appGameLogDir(), fn);
		path[MAX_PATH] = TEXT('\0');

		if (bLog)
		{
			fp = _tfopen(path, TEXT("wt"));
		}
	}
	~_BandwidthProfiler()
	{
		if (bLog && fp)
		{
			...
			if ( !GConfig->GetString(TEXT("avaNet"), TEXT("BTTestProfilePath"), SavePath, GNetIni) )
			{
				SavePath = "D:\\";
			}

			if (SavePath[SavePath.Len() - 1] != TEXT('\\'))
				SavePath += TEXT('\\');

			_sntprintf(np, MAX_PATH, TEXT("%s%s"), *SavePath, fn);
			::MoveFile(path, np);
		}
		else if (fp)
		{
	...
	void PrintV(LPCTSTR fmt, va_list arg)
	{
		if (!bLog || fp == NULL)
			return;
		_ftprintf(fp, TEXT("%s|%.15f|"), _GetComputerName(), appSeconds());
		_vftprintf(fp, fmt, arg);
		_ftprintf(fp, TEXT("\n"));
		fflush(fp);
	}
...
_BandwidthProfiler& BandwidthProfiler()
{
	static _BandwidthProfiler bp;
	return bp;
}

void BTPrintF(LPCTSTR fmt, ...)
{
	va_list arg;
	va_start(arg, fmt);
	BandwidthProfiler().PrintV(fmt, arg);
	va_end(arg);
}
...
,	OutAckPacketId		( -1 )
#ifdef _BT_TEST_BY_CRAZY
//,	OutLagPacketLen		( NULL )
,	NetFrameBytes		( 0 )
,	NetBytes			( 0 )
,	LastNewNetSpeedTime	( 0 )
,	NewNetSpeed			( 6000 )
#endif
{
}
...
void UNetConnection::StaticConstructor()
{
	...
	Class->EmitObjectReference( STRUCT_OFFSET( UNetConnection, PackageMap ) );
	Class->EmitFixedArrayBegin( STRUCT_OFFSET( UNetConnection, Channels ), sizeof(UChannel*), MAX_CHANNELS );
	Class->EmitObjectReference( STRUCT_OFFSET( UNetConnection, Channels ) );
	Class->EmitFixedArrayEnd();
	Class->EmitObjectReference( STRUCT_OFFSET( UNetConnection, Download ) );
	Class->EmitObjectArrayReference( STRUCT_OFFSET( UNetConnection, Children ) );
}
...
#ifdef _BT_TEST_BY_CRAZY
	{
		for (int i = 0; i < ARRAY_COUNT(OutLagPacketId); i++)
		{
			OutLagPacketId[i] = -1;
		}
	}
...
void UNetConnection::SendPackageInfo(const FPackageInfo& Info)
{
	Logf
	(
		TEXT("USES GUID=%s PKG=%s FLAGS=%i SIZE=%i GEN=%i BASEPKG=%s"),
		*Info.Guid.String(),
...
#ifdef _BT_TEST_BY_CRAZY
		OutLagPacketLen[Index] = Out.GetNumBytes() + PacketOverhead;
#endif
...
#ifdef _BT_TEST_BY_CRAZY
		BTPrintF(TEXT("ReceivedNak|%p|1|%d"), this, NakPacketId);
#endif
...
{
	AssertValid();
	#ifdef EnableReliableAck
	BYTE bReliable = 0x0; // 20071206 dEAthcURe|ON unreliable packet만 있으면 ack를 보내지 않는다.
	#endif
	...
	while( !Reader.AtEnd() && State!=USOCK_Closed )
	{
...
#ifdef _BT_TEST_BY_CRAZY
				OutLagPacketId[Index] = -1;
				NetFrameBytes += OutLagPacketLen[Index];
#endif
		...
		else
		{
			...
			#ifdef EnableReliableAck
			bReliable |= Bunch.bReliable    = Reader.ReadBit(); // 20071206 dEAthcURe|ON unreliable packet만 있으면 ack를 보내지 않는다.
			#else
	...
	// Acknowledge the packet.
	#ifdef EnableReliableAck
	if(bReliable || GWorld && GWorld->GetNetMode() == NM_Client) SendAck( PacketId ); // 20071206 dEAthcURe|ON unreliable packet만 있으면 ack를 보내지 않는다.
	#else
	SendAck( PacketId );
	#endif
}
...
#ifdef _BT_TEST_BY_CRAZY
	if (!bIgnoreNetReadyCheck && QueuedBytes > _NET_MAX_QUEUED_BYTES)
		QueuedBytes = _NET_MAX_QUEUED_BYTES;
	FLOAT DeltaBytes = NewNetSpeed * DeltaTime * _NET_DELTA_BYTES_RATE;
#else
...
#ifdef _BT_TEST_BY_CRAZY
	NetBytes += NetFrameBytes;
	NetFrameBytes = 0;
	if (NetBytes > _NET_SPEED_CALC_BYTES)
	{
		DOUBLE NewNetSpeedDT = Driver->Time - LastNewNetSpeedTime;
		LastNewNetSpeedTime = Driver->Time;
		DOUBLE NetSpeed = NetBytes / NewNetSpeedDT;
		if (NetSpeed < _NET_SPEED_MIN)
			NetSpeed = _NET_SPEED_MIN;
		if (NetSpeed > _NET_SPEED_MAX)
			NetSpeed = _NET_SPEED_MAX;
		NewNetSpeed = appTrunc((NewNetSpeed * (1.0f - _NET_SPEED_SUM_RATE)) + (NetSpeed * _NET_SPEED_SUM_RATE));
		NetBytes = 0;
		BTPrintF(TEXT("NewNetSpeed|%p|1|%d"), this, NewNetSpeed);
		BTPrintF(TEXT("CurrentNetSpeed|%p|1|%d"), this, CurrentNetSpeed);
	}
#endif
}
...
void UChildConnection::HandleClientPlayer(APlayerController* PC)
{
	...
	for (FPlayerIterator It(Cast<UGameEngine>(GEngine)); It; ++It)
	{
		...
		else if (Parent->Actor != It->Actor)
		{
			UBOOL bHasChildConnection = FALSE;
			for (INT i = 0; i < Parent->Children.Num(); i++)
			{
				if (Parent->Children(i)->Actor == It->Actor)
				{
					bHasChildConnection = TRUE;
				}
			}
			if (!bHasChildConnection)
			{
				NewPlayer = *It;
				break;
			}
	...
	{
#if !FINAL_RELEASE
		debugf(NAME_Error, TEXT("Failed to find LocalPlayer for received PlayerController '%s'. PlayerControllers:"), *PC->GetName());
...
UBOOL UNetConnection::ParseSettings(const TCHAR* Cmd, FOutputDevice& Ar)
{
	UBOOL Res = FALSE;
	INT IntValue;
	FLOAT FloatValue;

	if( ParseCommand(&Cmd, TEXT("BtTestVar")) )
	{
		Ar.Logf(TEXT("_NET_BALANCE_VALUE = %d"), _NET_BALANCE_VALUE);
		Ar.Logf(TEXT("_NET_SPEED_CALC_BYTES = %d"), _NET_SPEED_CALC_BYTES);
		Ar.Logf(TEXT("_NET_SPEED_MIN = %d"), _NET_SPEED_MIN);
		Ar.Logf(TEXT("_NET_SPEED_MAX = %d"), _NET_SPEED_MAX);
		Ar.Logf(TEXT("_NET_SPEED_SUM_RATE = %.3f"), _NET_SPEED_SUM_RATE);
		Ar.Logf(TEXT("_NET_MAX_QUEUED_BYTES = %d"), _NET_MAX_QUEUED_BYTES);
		Ar.Logf(TEXT("_NET_DELTA_BYTES_RATE = %.3f"), _NET_DELTA_BYTES_RATE);
		return TRUE;
	}

	if( Parse(Cmd,TEXT("BtBalanceValue="), IntValue) )
	{
		_NET_BALANCE_VALUE = IntValue;
		Ar.Logf(TEXT("_NET_BALANCE_VALUE set to %d"), IntValue);
		debugf(TEXT("_NET_BALANCE_VALUE set to %d"), IntValue);

		Res = TRUE;
	}
	if( Parse(Cmd,TEXT("BtSpeedCalcBytes="), IntValue) )
	{
		_NET_SPEED_CALC_BYTES = IntValue;
		Ar.Logf(TEXT("_NET_SPEED_CALC_BYTES set to %d"), IntValue);
		debugf(TEXT("_NET_SPEED_CALC_BYTES set to %d"), IntValue);

		Res = TRUE;
	}
	if( Parse(Cmd,TEXT("BtSpeedMin="), IntValue) )
	{
		_NET_SPEED_MIN = IntValue;
		Ar.Logf(TEXT("_NET_SPEED_MIN set to %d"), IntValue);
		debugf(TEXT("_NET_SPEED_MIN set to %d"), IntValue);

		Res = TRUE;
	}
	if( Parse(Cmd,TEXT("BtSpeedMax="), IntValue) )
	{
		_NET_SPEED_MAX = IntValue;
		Ar.Logf(TEXT("_NET_SPEED_MAX set to %d"), IntValue);
		debugf(TEXT("_NET_SPEED_MAX set to %d"), IntValue);

		Res = TRUE;
	}
	if( Parse(Cmd,TEXT("BtSpeedSumRate="), FloatValue) )
	{
		_NET_SPEED_SUM_RATE = FloatValue;
		Ar.Logf(TEXT("_NET_SPEED_SUM_RATE set to %.3f"), FloatValue);
		debugf(TEXT("_NET_SPEED_SUM_RATE set to %.3f"), FloatValue);

		Res = TRUE;
	}
	if( Parse(Cmd,TEXT("BtMaxQueuedBytes="), IntValue) )
	{
		_NET_MAX_QUEUED_BYTES = IntValue;
		Ar.Logf(TEXT("_NET_MAX_QUEUED_BYTES set to %d"), IntValue);
		debugf(TEXT("_NET_MAX_QUEUED_BYTES set to %d"), IntValue);

		Res = TRUE;
	}
	if( Parse(Cmd,TEXT("BtDeltaBytesRate="), FloatValue) )
	{
		_NET_DELTA_BYTES_RATE = FloatValue;
		Ar.Logf(TEXT("_NET_DELTA_BYTES_RATE set to %.3f"), FloatValue);
		debugf(TEXT("_NET_DELTA_BYTES_RATE set to %.3f"), FloatValue);
