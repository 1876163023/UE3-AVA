...
IMPLEMENT_SHADER_TYPE(template<>,TDOFAndBloomGatherPixelShader<NumFPFilterSamples>,TEXT("DOFAndBloomGatherPixelShader"),TEXT("Main"),SF_Pixel,VER_FIX_BLOOM_BURST_BLACK_PIXEL,0);
...
IMPLEMENT_SHADER_TYPE(template<>,TDOFAndBloomGatherPixelShader<MAX_FILTER_SAMPLES>,TEXT("DOFAndBloomGatherPixelShader"),TEXT("Main"),SF_Pixel,VER_FIX_BLOOM_BURST_BLACK_PIXEL,0);
...
IMPLEMENT_SHADER_TYPE(,FDOFAndBloomGatherFallbackPixelShader,TEXT("DOFAndBloomGatherPixelShader"),TEXT("SM2Main"),SF_Pixel,VER_FIX_BLOOM_BURST_BLACK_PIXEL,0);
...
void FDOFAndBloomPostProcessSceneProxy::RenderDOFAndBloomGatherPass(FCommandContextRHI* Context, FViewInfo& View)
{
	...
	//FLOAT RealBloomScale = BloomScale;
	FSceneViewState* ViewState = (FSceneViewState*)(View.State);		
	if (ViewState && ViewState->ExposureData.bUseDynamicTonemapping)
	{
		BloomThreshold = ViewState->ExposureData.LuminanceHistogram.BloomThreshold;
		//RealBloomScale /= BloomThreshold;
	}
	...
	if (bUseHighQualityBloom)
	{
		...
		if (IsSM2Platform( GRHIShaderPlatform ))
		{
			...
			SetPixelShaderValue(Context,GatherPixelShader->GetPixelShader(),GatherPixelShader->BloomThresholdParameter,BloomThreshold);
