		...
		if(NumVertices)
		{
			...
			for(INT PolyIndex = 0;PolyIndex < Model->Polys->Element.Num();PolyIndex++)
			{
				...
				for(INT VertexIndex = 0;VertexIndex < Poly.Vertices.Num();VertexIndex++)
				{
					...
					DestVertex->TangentBasis[0] = FVector(1,0,0);
					DestVertex->TangentBasis[1] = FVector(0,1,0);
					DestVertex->TangentBasis[2] = FVector(0,0,1);
...
public:
		...
		PropertyColor(255,255,255)
	{
		...
		{
			VertexFactoryData->TangentBasisComponents[AxisIndex] =
				STRUCTMEMBER_VERTEXSTREAMCOMPONENT(&WireVertexBuffer,FModelWireVertex,TangentBasis[AxisIndex],VET_PackedNormal);
		}
	...
	virtual void DrawDynamicElements(FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DepthPriorityGroup)
	{
		...
		if(IsCollisionView(View))
		{
			if(ShouldDrawCollision(View))
			{
				if(ConvexVertexBuffer.Vertices.Num() > 0 && ConvexIndexBuffer.Indices.Num() > 0)
				{
					...
					Mesh.MaterialInstance = &CollisionMaterialInstance;
		...
		else
		{
			if(WireIndexBuffer.GetNumEdges() && WireVertexBuffer.GetNumVertices())
			{
				...
				FColoredMaterialInstance WireframeMaterial(
				...
				Mesh.MaterialInstance = &WireframeMaterial;
				Mesh.LocalToWorld = LocalToWorld;
				Mesh.WorldToLocal = LocalToWorld.Inverse();				
				Mesh.FirstIndex = 0;
	...
	virtual FPrimitiveViewRelevance GetViewRelevance(const FSceneView* View)
	{
		...
		if(bVolume && IsCollisionView(View) && (View->Family->ShowFlags & (SHOW_Volumes | SHOW_Game)))
		{
 ...
 */
INT UBrushComponent::AddConvexVolumes( TArray<FConvexVolume>& ConvexVolumes, UBOOL bConvertToWorldSpace )
{
	for( INT ConvexElementIndex=0; ConvexElementIndex<BrushAggGeom.ConvexElems.Num(); ConvexElementIndex++ )
	{
		// Get convex volume planes for brush from physics data.
		const FKConvexElem& ConvexElement	= BrushAggGeom.ConvexElems(ConvexElementIndex);
		...
		if( bConvertToWorldSpace )
		{
			for( INT PlaneIndex=0; PlaneIndex<BrushPlanes.Num(); PlaneIndex++ )
			{
				...
				BrushPlane = BrushPlane.TransformBy( LocalToWorld );
			}
		}
		// Add convex volume.
		ConvexVolumes.AddItem( FConvexVolume( BrushPlanes ) );
	}
	return BrushAggGeom.ConvexElems.Num();
}
