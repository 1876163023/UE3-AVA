...
void FLightPrimitiveInteraction::Create(FLightSceneInfo* LightSceneInfo,FPrimitiveSceneInfo* PrimitiveSceneInfo)
{
	...
	if(bRelevant)
	{
		...
		LightSceneInfo->AttachPrimitive(PrimitiveSceneInfo);

		// Create the light interaction.
		new FLightPrimitiveInteraction(LightSceneInfo,PrimitiveSceneInfo,bDynamic,bLightMapped);
	}
}
...
FLightPrimitiveInteraction::~FLightPrimitiveInteraction()
{
	...
	LightSceneInfo->DetachPrimitive(PrimitiveSceneInfo);
...
UBOOL FPrimitiveSceneInfo::IsStillValid() const
{
	return (
		bStaticShadowing == Component->HasStaticShadowing() &&
		bCastDynamicShadow == (Component->bCastDynamicShadow && Component->CastShadow) &&
		bCastStaticShadow == Component->CastShadow &&
		bAcceptsLights == Component->bAcceptsLights &&
		bAcceptsDynamicLights == Component->bAcceptsDynamicLights &&				
		bAllowApproximateOcclusion == Component->bAllowApproximateOcclusion &&
		SeeThroughGroupIndex == Component->SeeThroughGroupIndex &&
		LightingChannels.Bitfield == Component->LightingChannels.Bitfield &&
		ShadowParentComponent == Component->ShadowParent && 
		OcclusionGroupComponent == Component->OcclusionGroup && 
		LightEnvironmentSceneInfo == ((Component->LightEnvironment && Component->LightEnvironment->bEnabled) ? Component->LightEnvironment->SceneInfo : NULL) &&
		bUseAsOccluder == Component->bUseAsOccluder);
}
...
FPrimitiveSceneInfo::FPrimitiveSceneInfo(UPrimitiveComponent* InComponent,FPrimitiveSceneProxy* InProxy,FScene* InScene):
	...
	Id(INDEX_NONE),
	ShadowVolumeId( INDEX_NONE ),	// ava specific ; 2008. 1. 23 changmin
	ShadowVolumeVerticesId( INDEX_NONE ),
	...
	SeeThroughGroupIndex(InComponent->SeeThroughGroupIndex),
	...
	ShadowParentComponent(InComponent->ShadowParent),
	ShadowParent(InComponent->ShadowParent ? InComponent->ShadowParent->SceneInfo : NULL),
	OcclusionGroupComponent(InComponent->OcclusionGroup),
	OcclusionGroup(InComponent->OcclusionGroup ? InComponent->OcclusionGroup->SceneInfo : NULL),
	FirstShadowChild(NULL),
	NextShadowChild(NULL),
	FirstOcclusionChild(NULL),
	NextOcclusionChild(NULL),
	EnvCube(NULL),
	OcclusionTesterSetFrame(-1)
{
	appMemzero( &IrradianceSH, sizeof(IrradianceSH) );
	check(Component);
	check(Proxy);
	...
	if( InScene && InScene->World && InScene->World->PersistentLevel )
	{
	...
	if( bUsePVSInfo )
	{
		UModelComponent *ModelComponent = Cast<UModelComponent>( InComponent );
		UStaticMeshComponent *StaticMeshComponent = Cast<UStaticMeshComponent>( InComponent );
		if( ModelComponent )
		{
			FirstLeafIndex = ModelComponent->GetClusterNumber();
			LeafCount = -1;
		}
		else if( StaticMeshComponent )
		{
			if( StaticMeshComponent->FirstLeaf != -1 && StaticMeshComponent->LeafCount > 0 )
			{
				FirstLeafIndex = StaticMeshComponent->FirstLeaf;
				LeafCount = StaticMeshComponent->LeafCount;
			}
		}
	}
...
void FPrimitiveSceneInfo::AddToScene()
{
	...
	for(INT ElementIndex = 0;ElementIndex < StaticMeshes.Num();ElementIndex++)
	{
		FStaticMesh* StaticMesh = &StaticMeshes(ElementIndex);
		const FMaterial* Material = StaticMesh->MaterialInstance->GetMaterial();
		...
		Scene->StaticMeshes(SceneArrayAllocation.Index) = StaticMesh;
		StaticMesh->Id = SceneArrayAllocation.Index;

		// not all platforms need this
		if ( bRequiresHitProxies )
		{
			// Add the static mesh to the DPG's hit proxy draw list.
			FHitProxyDrawingPolicyFactory::AddStaticMesh(Scene,StaticMesh);
		}

		if(!StaticMesh->IsTranslucent())
		{
			if(StaticMesh->DepthPriorityGroup == SDPG_World)
			{
				// Add the static mesh to the depth-only draw list.
				FDepthDrawingPolicyFactory::AddStaticMesh(Scene,StaticMesh);

				if ( !bStaticShadowing )
				{
					FVelocityDrawingPolicyFactory::AddStaticMesh(Scene,StaticMesh);
				}
			}

			// Add the static mesh to the DPG's emissive draw list.
			FEmissiveDrawingPolicyFactory::AddStaticMesh(Scene,StaticMesh);
		}		
	}
	...
	if(LightEnvironmentSceneInfo)
	{
		...
		//<@ ava specific ; 2007. 11 5 changmin
		// add cascaded shadow map
		// insert( scene, light ) -> insert( scene, lightenv ) 일 경우, FLightPrimitiveInteraction이 안생김...
		extern UBOOL GUseCascadedShadow;
		if( GUseCascadedShadow )
		{
			for(TSparseArray<FLightSceneInfo*>::TIterator LightIt(Scene->Lights);LightIt;++LightIt)
			{
				FLightSceneInfo* LightSceneInfo = *LightIt;
				if( LightSceneInfo->bUseCascadedShadowmap == 1)
				{
					FLightPrimitiveInteraction::Create(LightSceneInfo, this);
				}
			}
	...
	// cache static sun shadow volume
	extern UBOOL GUseCascadedShadow;
	...
	if( GUseCascadedShadow && Scene->bHasSunLightShadow )
	{
		...
		&& Component->HasStaticShadowing())
		{
			...
			CreateShadowFrustum( Scene->SunLight, CompactPrimitiveSceneInfo.Bounds.GetBox(), &ShadowVolume, &ShadowMatrix );
			...
			INT Id = Scene->StaticSunShadowVolumes.AddItem(ShadowVolume);
			INT VerticesId = Scene->StaticSunShadowVertices.AddItem(ShadowVolumeVertices);
			ShadowVolumeId = Id;
			ShadowVolumeVerticesId = VerticesId;
		}
	}
	//>@ ava
}
...
void FPrimitiveSceneInfo::RemoveFromScene()
{
	...
	//<@ ava specific ; 2008. 1. 23 changmin
	// cache static sun shadow volume
	if( ShadowVolumeId != INDEX_NONE )
	{
		Scene->StaticSunShadowVolumes.Remove(ShadowVolumeId);
	}
	if( ShadowVolumeVerticesId != INDEX_NONE )
	{
		Scene->StaticSunShadowVertices.Remove(ShadowVolumeVerticesId);
	}
...
void FPrimitiveSceneInfo::LinkOcclusionGroup()
{
	if(OcclusionGroup)
	{
		NextOcclusionChild = OcclusionGroup->FirstOcclusionChild;
		OcclusionGroup->FirstOcclusionChild = this;
	}
}

void FPrimitiveSceneInfo::UnlinkOcclusionGroup()
{
	// Remove the primitive from its Occlusion parent's linked list of children.
	if(OcclusionGroup)
	{
		FPrimitiveSceneInfo** OcclusionChildPtr = &OcclusionGroup->FirstOcclusionChild;
		while(*OcclusionChildPtr && *OcclusionChildPtr != this)
			OcclusionChildPtr = &(*OcclusionChildPtr)->NextOcclusionChild;
		*OcclusionChildPtr = NextOcclusionChild;
		NextOcclusionChild = NULL;
	}

	// Disassociate Occlusion children.
	FPrimitiveSceneInfo** OcclusionChildPtr = &FirstOcclusionChild;
	while(*OcclusionChildPtr)
	{
		FPrimitiveSceneInfo** PrevOcclusionChildLink = OcclusionChildPtr;
		(*PrevOcclusionChildLink)->OcclusionGroup = NULL;
		OcclusionChildPtr = &(*OcclusionChildPtr)->NextOcclusionChild;
		*PrevOcclusionChildLink = NULL;
	};
}
...
FMeshDrawingPolicy::FMeshDrawingPolicy(const FVertexFactory* InVertexFactory,const FMaterialInstance* InMaterialInstance):
	VertexFactory(InVertexFactory),
	MaterialInstance(InMaterialInstance),
	bIsTwoSidedMaterial(InMaterialInstance->GetMaterial()->IsTwoSided()),
	bIsWireframeMaterial(InMaterialInstance->GetMaterial()->IsWireframe()),
	bIsNonDirectionalMaterial(InMaterialInstance->GetMaterial()->GetLightingModel() == MLM_NonDirectional)
{}
	...
	) const
{
	const FRasterizerStateInitializerRHI Initializer = {
		...
		(IsNonDirectional() && IsTwoSided()) ? CM_None :
...
void FMeshDrawingPolicy::DrawMesh(FCommandContextRHI* Context,const FMeshElement& Mesh) const
{
	if (Mesh.UseDynamicData)
	{
		...
		else if (Mesh.ParticleType == PET_Sprite)
		{
			...
			{
				RHIDrawSubUVParticles(Context,Mesh);
			}
		else if (Mesh.ParticleType == PET_Batch)
		{
			extern void ParticleRendering_DrawMesh( FCommandContextRHI* );
			
			ParticleRendering_DrawMesh( Context );			
		}
	}
	else
	{		
		if(Mesh.IndexBuffer)
		{
			...
			RHIDrawIndexedPrimitive(
				...
				Mesh.Type,
				Mesh.BaseVertexIndex,	//<@ ava specific ; 2007. 2. 26 changin : use vertex offet for dynamic bsp drawing //>@
				Mesh.MinVertexIndex,
				Mesh.MaxVertexIndex - Mesh.MinVertexIndex + 1,
				Mesh.FirstIndex,
				Mesh.NumPrimitives
