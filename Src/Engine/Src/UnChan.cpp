...
void UActorChannel::StaticConstructor()
{
	...
	Class->EmitObjectReference(STRUCT_OFFSET(UActorChannel,Actor));
	Class->EmitObjectReference(STRUCT_OFFSET(UActorChannel,ActorClass));
}
...
void UActorChannel::ReceivedBunch( FInBunch& Bunch )
{
	...
	if( Actor==NULL )
	{
		...
		if (Connection->Driver != NULL && Connection == Connection->Driver->ServerConnection && Actor->GetAPlayerController() != NULL)
		{
			// If this is the player's channel and the connection was pending, mark it open.
			if (Connection->State == USOCK_Pending)
			{
				Connection->HandleClientPlayer( (APlayerController*)(Actor) ); 
			}
			// Create a child connection for the client if the socket is already open and we just received a new player controller.
			else if (Connection->State == USOCK_Open && Actor != Connection->Actor)
			{
				// create a split connection on the client
				UChildConnection* Child = Connection->Driver->CreateChild(Connection); 
				Child->HandleClientPlayer((APlayerController*)(Actor));
	...
	while (FieldCache || bJustSpawned)
	{
		...
		while( FieldCache && (It=Cast<UProperty>(FieldCache->Field,CLASS_IsAUProperty))!=NULL )
		{
			...
			//debugf( NAME_Log, TEXT("Receive Actor : %s Field : %s	TimeStamp : %.3f"), *Actor->GetName(), *It->GetName(), GWorld->GetTimeSeconds() );
		...
		if( FieldCache && Cast<UFunction>(FieldCache->Field,CLASS_IsAUFunction) )
		{
			...
			debugf( NAME_DevNetTraffic, TEXT("      Received RPC: %s"), *Message.ToString() );
			...
			for( TFieldIterator<UProperty,CLASS_IsAUProperty> It(Function); It && (It->PropertyFlags & (CPF_Parm|CPF_ReturnParm))==CPF_Parm; ++It )
			{
				if( Connection->PackageMap->SupportsObject(*It) )
				{
					if( Cast<UBoolProperty>(*It,CLASS_IsAUBoolProperty) || Bunch.ReadBit() ) 
					{
						...
						{
							It->NetSerializeItem(Bunch, Connection->PackageMap, Parms + It->Offset + (i * It->ElementSize));
						}
					}
				}
			}
			...
			{
				debugfSuppressed(NAME_DevNetTraffic, TEXT("Rejected unwanted function %s in %s"), *Message.ToString(), *Actor->GetFullName());
			}
...
void UActorChannel::ReplicateActor()
{
	...
	debugf(NAME_DevNetTraffic, TEXT("Replicate %s, bNetDirty: %d, bNetInitial: %d, bNetOwner: %d"), *Actor->GetName(), Actor->bNetDirty, Actor->bNetInitial, Actor->bNetOwner );
	...
	if ( Actor->bNetDirty )
	{
		...
		{
			debugf(NAME_DevNetTraffic, TEXT(" DelayScriptReplication() and setting bActorMustStayDirty.") );
	...
	for( INT* iPtr=Reps; iPtr<LastRep; iPtr++ )
	{
		...
		//debugf(NAME_Log,TEXT("Actor : %s   Send : %s %i	TimeStamp : %.3f"),*Actor->GetName(),*It->GetName(),Mapped,GWorld->GetTimeSeconds());
		if( !Bunch.IsError() )
		{
			// Update recent value.
			if( Recent.Num() )
	...
	if( Bunch.GetNumBits() )
	{
		...
		BTPrintF(TEXT("ReplicateActor|%p|%d"), Connection, Bunch.GetNumBytes());
#endif
				...
				for( INT i=0; i<Connection->PackageMap->List.Num(); i++ )
				{
					...
					if (Info.Guid == Guid && Info.PackageName != NAME_None)
					{
						if ( Connection->Driver->MaxDownloadSize > 0 &&
							GFileManager->FileSize(*Info.PackageName.ToString()) > Connection->Driver->MaxDownloadSize )
						{
							break;							
						}
						appStrncpy(SrcFilename, *Info.PackageName.ToString(), ARRAY_COUNT(SrcFilename));
