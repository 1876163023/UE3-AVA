...
/** distortion depth shader type implementations */
//IMPLEMENT_MATERIAL_SHADER_TYPE(template<>,TDistortionMeshVertexShader<FDistortMeshDepthPolicy>,TEXT("DistortDepthVertexShader"),TEXT("Main"),SF_Vertex,0,0);
//IMPLEMENT_MATERIAL_SHADER_TYPE(template<>,TDistortionMeshPixelShader<FDistortMeshDepthPolicy>,TEXT("DistortDepthPixelShader"),TEXT("Main"),SF_Pixel,0,0);
...
IMPLEMENT_SHADER_TYPE(,FDistortionApplyScreenPixelShader,TEXT("DistortApplyScreenPixelShader"),TEXT("Main"),SF_Pixel,VER_DOFBLOOMGATHER_DISTORTION_SHADER_RECOMPILE,0);
...
UBOOL TDistortionMeshDrawingPolicyFactory<DistortMeshPolicy>::IsMaterialIgnored(const FMaterialInstance* MaterialInstance)
{
...
UBOOL FDistortionPrimSet::DrawAccumulatedOffsets(FCommandContextRHI* Context,const class FViewInfo* ViewInfo,UINT DPGIndex)
{
	...
	if( Prims.Num() )
	{
		...
		ParticleRendering_EndBatch(&Drawer, ViewInfo, DPGIndex);
...
/** 
* Iterate over the distortion prims and draw their closest depths
* @param Context - command context
* @param ViewInfo - current view used to draw items
* @param DPGIndex - current DPG used to draw items
* @return TRUE if anything was drawn
*/
UBOOL FDistortionPrimSet::DrawClosestDepths(FCommandContextRHI* Context,const class FViewInfo* ViewInfo,UINT DPGIndex)
{
	UBOOL bDirty=FALSE;

#if GEMINI_TODO
	// Draw the view's elements with the translucent drawing policy.
	bDirty |= DrawViewElements<TDistortionMeshDrawingPolicyFactory<FDistortMeshDepthPolicy> >(
		Context,
		ViewInfo,
		TDistortionMeshDrawingPolicyFactory<FDistortMeshDepthPolicy>::ContextType(),
		DPGIndex,
		FALSE // Distortion is rendered post fog.
		);
	...
	{
		// For drawing scene prims with dynamic relevance.
		TDynamicPrimitiveDrawer<TDistortionMeshDrawingPolicyFactory<FDistortMeshDepthPolicy> > Drawer(
			Context,
			ViewInfo,
			DPGIndex,
			TDistortionMeshDrawingPolicyFactory<FDistortMeshDepthPolicy>::ContextType(),
		...
		for( INT PrimIdx=0; PrimIdx < Prims.Num(); PrimIdx++ )
		{
			...
			else if( ViewRelevance.bStaticRelevance )
			{
				...
				for( INT StaticMeshIdx=0; StaticMeshIdx < PrimitiveSceneInfo->StaticMeshes.Num(); StaticMeshIdx++ )
				{
					...
					if( StaticMesh.IsTranslucent() )
					{
						bDirty |= TDistortionMeshDrawingPolicyFactory<FDistortMeshDepthPolicy>::DrawStaticMesh(
							Context,
							ViewInfo,
							TDistortionMeshDrawingPolicyFactory<FDistortMeshDepthPolicy>::ContextType(),
							StaticMesh,
