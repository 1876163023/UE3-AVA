...
#define SET_FILTER_SHADER_TYPE(NumSamples) \
	...
	{ \
		...
		{ \
			static FBoundShaderStateRHIRef BoundShaderState; \
			...
			{ \
				...
				BoundShaderState = RHICreateBoundShaderState(GFilterVertexDeclaration.VertexDeclarationRHI, Strides, VertexShader->GetVertexShader(), PixelShader->GetPixelShader()); \
			} \
...
void GaussianBlurFilterBuffer(FCommandContextRHI* Context, UINT SizeX,UINT SizeY,FLOAT KernelRadius)
{
	...
	//@AVA : Reference resolution 1024x768
	// The effect blur scale is specified in pixel widths at a reference resolution of 1024x?.
	const FLOAT ResolutionFraction = SizeX * FilterDownsampleFactor / 1024.0f;
