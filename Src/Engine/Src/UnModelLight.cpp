...
FBSPSurfaceStaticLighting::FBSPSurfaceStaticLighting(
	...
	FStaticLightingMesh(InMapping.TriangleVertexIndices.Num() / 3,InMapping.Vertices.Num(),TRUE,InMapping.RelevantLights,InMapping.BoundingBox),
	Mapping(InMapping),
	bComplete(FALSE),
	LightMapData(NULL),
...
UBOOL FBSPSurfaceStaticLighting::DoesLineSegmentIntersect(const FVector& Start,const FVector& End) const
{
...
public:
	 ...
	 */
	FSurfaceStaticLightingGroup(UINT InSizeX,UINT InSizeY,UMaterialInstance* InMaterial,INT InBrightnessGroup):
		Material(InMaterial),
	...
	UBOOL AddSurface(FBSPSurfaceStaticLighting* SurfaceStaticLighting)
	{
		...
		if(TextureLayout.AddElement(&PaddedSurfaceBaseX,&PaddedSurfaceBaseY,SurfaceStaticLighting->SizeX,SurfaceStaticLighting->SizeY))
		{
...
void UModelComponent::ApplyStaticLighting(FModelComponentStaticLighting* ComponentStaticLighting)
{
	...
	for(INT GroupIndex = 0;GroupIndex < SurfaceGroups.Num();GroupIndex++)
	{
		...
		Element->LightMap = FLightMap2D::AllocateLightMap(this,GroupLightMapData, GroupLightMapData, FLightMap2D::NeedsBumpedLightmap(SurfaceGroup.Material),Bounds, FALSE);	// 컴파일 되도록 수정, 사용되지 않는 코드영역
