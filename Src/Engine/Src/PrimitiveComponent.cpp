...
UBOOL GDoNotApplyCullDistances = FALSE;

FPrimitiveSceneProxyCacheInfo::FPrimitiveSceneProxyCacheInfo(const UPrimitiveComponent* Component)
: LocalToWorld(Component->LocalToWorld),
LocalToWorldDeterminant(Component->LocalToWorldDeterminant),
Bounds(Component->Bounds)
{
	TotalScale3D = Component->Scale * Component->Scale3D;
...
void FPrimitiveSceneProxy::Touch( const FPrimitiveSceneProxyCacheInfo& PRI ) 
{
	LocalToWorld = PRI.LocalToWorld;
	LocalToWorldDeterminant = PRI.LocalToWorldDeterminant;		
	TotalScale3D = PRI.TotalScale3D;
	Bounds = PRI.Bounds;	
}

UBOOL FPrimitiveSceneProxy::IsCacheable() const
{
	return FALSE;
}

UBOOL FPrimitiveSceneProxy::IsStillValid( const UPrimitiveComponent* InComponent ) const
{
	if (
		...
		CullDistanceEx == InComponent->CullDistanceEx)
	{
		if(InComponent->GetOwner() && (bOnlyOwnerSee || bOwnerNoSee || bUseViewOwnerDepthPriorityGroup))
		{
			// Check whether owners are same as previous frame
			INT NumOwnersLeft = Owners.Num();

			INT CurrentOwnerIndex = 0;

			for(const AActor* Owner = InComponent->GetOwner();Owner;Owner = Owner->Owner)
			{
				if (NumOwnersLeft-- == 0 || Owners(CurrentOwnerIndex++) != Owner) return FALSE;
			}

			return NumOwnersLeft == 0;
		}
		else
			return TRUE;
	}
...
FPrimitiveSceneProxyCacheInfo(InComponent),
...
bFadingOut(InComponent->bFadingOut),
FadeStart(InComponent->FadeStart), 
FadeEnd(InComponent->FadeEnd),
bCastSunShadow(InComponent->bCastSunShadow)	//<@ ava specific ; 2007. 11. 12 changmin ; add cascaded shadow
{
...
UBOOL FPrimitiveSceneProxy::IsShown(const FSceneView* View) const
{
	...
	FVector Delta = PrimitiveSceneInfo->Bounds.Origin - View->ViewOrigin;	
	if(View->ViewOrigin.W > 0.0f)
	{
		DistanceSquared = FVector2D(Delta.X, Delta.Y).SizeSquared();
	}
	...
	{
		UBOOL ShouldUpdateCullDistance( const UPrimitiveComponent* );
		return ShouldUpdateCullDistance( PrimitiveSceneInfo->Component );
	}
	...
	if (bFadingOut && DistanceSquared > Square(FadeEnd / View->LODDistanceFactor))
	{
		return FALSE;
	}

	if (GDoNotApplyCullDistances)
	{
		return TRUE;
	}	

	// Cull the primitive if the view is farther than its cull distance.
	const FLOAT CullDistanceValue = CullDistanceEx.Value[FSmartCullDistance::FindIndex(Delta.X,Delta.Y)];
	if(CullDistanceValue > 0 && DistanceSquared > Square(CullDistanceValue))
	{
		return FALSE;
	}
...
UBOOL FPrimitiveSceneProxy::IsSeenThrough(const FSceneView* View) const
{
	return (PrimitiveSceneInfo->SeeThroughGroupIndex != 0);
}
...
UBOOL FPrimitiveSceneProxy::IsShadowCast(const FSceneView* View) const
{
...
#endif
	{
		...
		//<@ ava specific ; 2008. 1. 9 changmin
		// add cascaded shadow
		// 우리는 1인칭 그림자를 조정하기 위해서 이 flag를 사용한다. 그래서 다음과 같이 사용한다.
		//UBOOL bCastShadow = PrimitiveSceneInfo->bCastHiddenShadow;
		extern UBOOL GUseCascadedShadow;
		UBOOL bCastShadow = PrimitiveSceneInfo->bCastHiddenShadow && PrimitiveSceneInfo->Scene->bHasSunLightShadow && GUseCascadedShadow;
		//>@ ava
...
void UPrimitiveComponent::Attach()
{
	...
	// If the primitive isn't hidden add it to the scene.
	const UBOOL bShowInEditor = !HiddenEditor && (!Owner || !Owner->bHiddenEd);
	//const UBOOL bShowInGame = !HiddenGame && (!Owner || !Owner->bHidden || bCastHiddenShadow); // const 제거하기 위해 아래로 옮김 ; 2008. 1. 2 changmin

	//<@ ava specific ; 2008. 1. 2 changmin
	// add cascaded shadow
	UBOOL bShowInGame = !HiddenGame && (!Owner || !Owner->bHidden || bCastHiddenShadow);
	extern UBOOL GUseCascadedShadow;
	if( GUseCascadedShadow )
		bShowInGame = bShowInGame || bCastHiddenShadow;
	//>@ ava

	if( (GIsGame && bShowInGame) || (!GIsGame && bShowInEditor) )
	{
		Scene->AddPrimitive(this);
	}

	// Issue a decal update request if we're in the editor (not PIE) and the component accepts decals.
	if ( GIsEditor && !GIsPlayInEditorWorld && bAcceptsDecals )
	{
		// Only issue the request if the component is level placed (not a component used in the editor).
		if ( World == GWorld )
		{
...
void UPrimitiveComponent::execSetTraceBlocking _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetActorCollision _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetOwnerNoSee _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetOnlyOwnerSee _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetHidden _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetShadowParent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetOcclusionGroup _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UPrimitiveComponent,NewOcclusionGroup);
	P_FINISH;
	SetOcclusionGroup(NewOcclusionGroup);
}

void UPrimitiveComponent::SetOcclusionGroup(UPrimitiveComponent* NewOcclusionGroup)
{		
	OcclusionGroup = NewOcclusionGroup;
	BeginDeferredReattach();
}

void UPrimitiveComponent::execSetSeeThroughGroupIndex _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(NewSeeThroughGroupIndex);
	P_FINISH;
	SetSeeThroughGroupIndex(NewSeeThroughGroupIndex);
}

void UPrimitiveComponent::SetSeeThroughGroupIndex(INT NewSeeThroughGroupIndex)
{		
	if (SeeThroughGroupIndex != NewSeeThroughGroupIndex)
	{
		SeeThroughGroupIndex = NewSeeThroughGroupIndex;
		BeginDeferredReattach();
	}	
}

void UPrimitiveComponent::execSetLightEnvironment _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetLightingChannels _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetDepthPriorityGroup _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetViewOwnerDepthPriorityGroup _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetTranslation _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetRotation _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetScale _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
//
void UPrimitiveComponent::execSetScale3D _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UPrimitiveComponent::execSetAbsolute _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
UBOOL UMeshComponent::HasUnlitDistortion() const
{
	UBOOL Result=FALSE;
	// iterate over all materials for this primitive
	const INT NumElements = GetNumElements();
	for( INT MaterialIdx=0; MaterialIdx < NumElements ;MaterialIdx++ )
	{
		UMaterialInstance* MaterialInstance = GetMaterial(MaterialIdx);
		if(MaterialInstance)
		{
...
UBOOL UMeshComponent::HasUnlitTranslucency() const
{
	UBOOL Result=FALSE;
	// iterate over all materials for this primitive
	const INT NumElements = GetNumElements();
	for( INT MaterialIdx=0; MaterialIdx < NumElements ;MaterialIdx++ )
	{
		UMaterialInstance* MaterialInst = GetMaterial(MaterialIdx);
...
/** Returns true if the prim is using a material with translucency */
UBOOL UMeshComponent::HasTranslucency() const
{
	UBOOL Result=FALSE;
	// iterate over all materials for this primitive
	const INT NumElements = GetNumElements();
...
*/
UBOOL UMeshComponent::UsesSceneColor() const
{
	UBOOL Result=FALSE;
	// iterate over all materials for this primitive
	const INT NumElements = GetNumElements();
	for( INT MaterialIdx=0; MaterialIdx < NumElements ;MaterialIdx++ )
	{
		UMaterialInstance* MaterialInst = GetMaterial(MaterialIdx);
		UMaterial* Material = MaterialInst ? MaterialInst->GetMaterial() : NULL;
		// check for scene color sampling by the material
		if( Material &&
...
void UMeshComponent::execGetMaterial _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(SkinIndex);
	P_FINISH;
...
void UMeshComponent::SetMaterial(INT SkinIndex,UMaterialInstance* Material)
{
	if (Materials.Num() <= SkinIndex || Materials(SkinIndex) != Material)
	{
...
void UMeshComponent::execGetNumElements _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void UCylinderComponent::execSetSize _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
public:
	...
	{}
		  ...
		  )
	  {
		...
		if(InDepthPriorityGroup == PrimitiveDPG)
   		{
			DrawDirectionalArrow(PDI,LocalToWorld,ArrowColor,ArrowSize * 3.0f,1.0f,PrimitiveDPG);
		}
	}
	virtual FPrimitiveViewRelevance GetViewRelevance(const FSceneView* View)
	{
...
FPrimitiveSceneProxy* UDrawSphereComponent::CreateSceneProxy()
{
	...
	public:
			  ...
			  )
		{
			...
			{
				  DrawSphere(PDI,LocalToWorld.GetOrigin(), FVector(SphereRadius), SphereSides, SphereSides/2, SphereMaterial->GetInstanceInterface(FALSE), SDPG_World);
			}
		}
	...
	private:
		...
		const UMaterialInstance*	SphereMaterial;
