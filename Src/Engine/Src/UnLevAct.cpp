...
)
{
	...
	check(GWorld == this || GIsCooking || GIsCookingForAva);
...
{
// {{ 20071001 dEAthcURe|HM invalid actor면 무시
#ifdef EnableHmFastLoading	
	if(0x0 == ThisActor || FALSE == ThisActor->IsValid()) {		
		return FALSE;
	}
#else
	check(ThisActor);
...
APlayerController* UWorld::SpawnPlayActor( UPlayer* Player, ENetRole RemoteRole, const FURL& URL, FString& Error )
{
	...
	#ifdef EnableHostMigration
	APlayerController* Actor = 0x0;
	//TCHAR* playerName = g_hostMigration.parseOption(TEXT("Name"), URL);
	const TCHAR* playerName=URL.GetOption(TEXT("Name="),NULL);
	if(playerName) {		
		phmPlayerBase_t pPlayerBase = g_hostMigration.findPlayerData(playerName);
		debugf(TEXT("[UWorld::SpawnPlayActor] playerName=%s pPlayerBase=0x%x HM?%s Listen?%s"), playerName, pPlayerBase, URL.HasOption(TEXT("HM"))?TEXT("true"):TEXT("false"), URL.HasOption(TEXT("Listen"))?TEXT("true"):TEXT("false"));
			
		if(pPlayerBase && (URL.HasOption(TEXT("HM")) || URL.HasOption(TEXT("Listen")))) {
			// {{ 20070509 호스트 이전중 악의적으로 한참 후에 접속하는 사용자를 spectator처리
			if(g_hostMigration.checkDelayedLogin(pPlayerBase)) {
				debugf(TEXT("[UWorld::SpawnPlayActor] player %s is too late."), playerName);
				pPlayerBase->bDead = true;
			}
			// }} 20070509 호스트 이전중 악의적으로 한참 후에 접속하는 사용자를 spectator처리			
			debugf(TEXT("[UWorld::SpawnPlayActor] Spawning host migrated player %s at %f,%f,%f %s"), playerName, pPlayerBase->Location.X, pPlayerBase->Location.Y, pPlayerBase->Location.Z, pPlayerBase->bDead ? TEXT("*dead*") : TEXT("*alive*"));
			Actor = GetGameInfo()->eventHmLogin( *URL.Portal, Options, pPlayerBase->Location, pPlayerBase->Rotation, pPlayerBase->bDead, Error );
			// {{ 20070307 백업된 무기가 없을경우 AddDefault
			if(Actor && pPlayerBase->pi.nInventory == 0) {
				APlayerController* pCtrl = Cast<APlayerController>(Actor);
				if(pCtrl) {
					pCtrl->bHmRestoring = false;
					debugf(TEXT("[dEAthcURe|UWorld::SpawnPlayActor.1] No backuped inventory, enable default."));
				}
			}
			// }} 20070307 백업된 무기가 없을경우 AddDefault
		}		
		...
		if(!Actor) {	
			phmPlayerInfo_t pPi = g_hostMigration.findAllowedPlayerOnNextRound(playerName);
			if(pPi) {
				debugf(TEXT("[UWorld::SpawnPlayActor] Spawning non-spectating player %s"), *pPi->name);
				Actor = GetGameInfo()->eventLogin( *URL.Portal, Options, Error, TRUE );				
				APlayerController* pCtrl = Cast<APlayerController>(Actor);
				if(pCtrl) {
					if(pCtrl->PlayerReplicationInfo) {
						extern bool _hm_updatePlayerInfo(APlayerReplicationInfo* pPRI, phmPlayerInfo_t pPi);
						if(_hm_updatePlayerInfo(pCtrl->PlayerReplicationInfo, pPi)) {
							debugf(TEXT("-- GPI successfully updated for %s"), *pPi->name);
						}
						else debugf(TEXT("-- hmPlayer_t::updateGamePlayerInfo failed for %s"), *pPi->name);
					}
					else debugf(TEXT("-- No proper PRI for %s"), *pPi->name);

					// {{ 20070307 백업된 무기가 없을경우 AddDefault
					if(pPi->nInventory == 0) {
						pCtrl->bHmRestoring = false;
						debugf(TEXT("[dEAthcURe|UWorld::SpawnPlayActor.2] No backuped inventory, enable default."));
					}
					// }} 20070307 백업된 무기가 없을경우 AddDefault
				}
				else debugf(TEXT("-- No proper APlayerController for %s"), *pPi->name);
				g_hostMigration.removeAllowedPlayerOnNextRound(pPi->name);
			}
		}		
		// }} 20070305 dEAthcURe|HM
	}
	...
	if(!Actor) {
		debugf(TEXT("[UWorld::SpawnPlayActor] Spawning player %s normally"), playerName);
		Actor = GetGameInfo()->eventLogin( *URL.Portal, Options, Error );
	}
	#else
...
UBOOL UWorld::CheckSlice(FVector& Location, const FVector& Extent, INT& bKeepTrying)
{
	...
	if( !EncroachingWorldGeometry(Hit, Location, SliceExtent) )
	{
		...
		SingleLineCheck(Hit, NULL, Location - 2.f*Down, Location, TRACE_World|TRACE_Tesselation, SliceExtent);
		...
		if( !Hit.Actor || (Hit.Time > 0.5f) )
		{
			...
			if( !EncroachingWorldGeometry(Hit,Location, Extent) )
			{
				...
				SingleLineCheck( Hit, NULL, Location + Down, Location, TRACE_World|TRACE_Tesselation, Extent );
				if( Hit.Actor )
				{
...
UBOOL UWorld::FindSpot(const FVector& Extent, FVector& Location, UBOOL bUseComplexCollision)
{
	...
	SingleLineCheck(Hit, NULL, Location, StartLoc, TRACE_World|TRACE_Tesselation);
	if( Hit.Actor )
	{
		return FALSE;
	}
	...
	if (!EncroachingWorldGeometry(Hit,Location, Extent, bUseComplexCollision) || CheckSlice(Location, Extent, bKeepTrying))
	{
		...
		SingleLineCheck(Hit, NULL, StartLoc + 0.2f * (StartLoc - Location), Location, TRACE_World|TRACE_Tesselation, Extent);
		if( Hit.Actor )
		{
			Location = Hit.Location;
		}
...
)
{
	...
	else
	{
			...
			(DeltaSize != 0.f) )
		{
			...
			{
				TraceFlags |= TRACE_World | TRACE_Tesselation;
			}
		...
		if ( bMovementOccurred )
		{
			...
			// This will not fix all problems with components being out of date though - for example, attachments of an Actor are not 
			// updated at this point.
			Actor->ForceUpdateComponents( GWorld->InTick && GWorld->TickGroup != TG_PostAsyncWork );
		}
	}
...
)
{
	...
	FMemMark Mark(GMem);
	FCheckResult* FirstHit = Hash ? Hash->ActorEncroachmentCheck( GMem, Actor, TestLocation, TestRotation, TRACE_AllColliding|TRACE_Tesselation ) : NULL;	
		...
		&&	Actor->IsBlockedBy( Test->Actor, Test->Component ) )
		{
			...
			if ( Actor->IsEncroacher() && !Test->Actor->IsEncroacher() )
			{
				...
				if ( bStillEncroaching && Test->Actor->GetAPawn() )
				{
					// try moving away
					FCheckResult Hit(1.f);
					Actor->ActorLineCheck(Hit, Actor->Location, Test->Actor->Location,FVector(0.f,0.f,0.f), TRACE_AllColliding|TRACE_Tesselation);
					if ( Hit.Time < 1.f )
					{
...
{
	FMemMark Mark(GMem);
	FCheckResult* Hits = MultiPointCheck( GMem, Location, Extent, TRACE_World | TRACE_StopAtAnyHit | (bUseComplexCollision ? TRACE_ComplexCollision : 0) );
	if ( !Hits )
	{
		Mark.Pop();
		return false;
 ...
 */
AReverbVolume* AWorldInfo::GetReverbSettings(const FVector& ViewLocation, UBOOL bUseVolumes)
{
...
void AWorldInfo::SetMapNeedsGeometryFullyRebuilt(UBOOL bInMapNeedsGeometryFullyRebuilt )
{
	check(IsInGameThread());
	if( bMapNeedsGeometryFullyRebuilt != bInMapNeedsGeometryFullyRebuilt )
	{
		Modify();
		bMapNeedsGeometryFullyRebuilt = bInMapNeedsGeometryFullyRebuilt;
		MarkPackageDirty();
	}
}

IMPLEMENT_CLASS(AAmbientCubeVolume);

static FSHVectorRGB Zero;

const FAmbientCubeVolumeInfo* AWorldInfo::DetermineAmbientCubeVolume( const FVector& Location ) const
{
	if (AmbientCubeVolumeInfos.Num() == 0) 
	{
		return NULL;
	}

	const FVector Extent( 16, 16, 16 );
	FBox Box( Location - Extent, Location + Extent );

	const FAmbientCubeVolumeInfo* Info = &AmbientCubeVolumeInfos(0);

	for (INT i=1; i<AmbientCubeVolumeInfos.Num(); ++i)
	{
		const FAmbientCubeVolumeInfo* NewInfo = &AmbientCubeVolumeInfos(i);

		if (NewInfo->Priority > Info->Priority)
		{
			FBox box( NewInfo->AmbientCubeMin, NewInfo->AmbientCubeMax );

			if (box.Intersect( Box ))
			{
				Info = NewInfo;
			}			
		}
...
const FSHVectorRGB& AWorldInfo::GetAmbientCube( const FVector& Location ) const
{
	const FAmbientCubeVolumeInfo* Info = DetermineAmbientCubeVolume( Location );
	if (!Info) 
	{
		return Zero;
	}

	return GetAmbientCube( *Info, Location );
}

const FSHVectorRGB& AWorldInfo::GetAmbientCube( const FAmbientCubeVolumeInfo& Info, const FVector& Location ) const
{
	FVector Diff = (Location - Info.AmbientCubeMin) / (Info.AmbientCubeMax - Info.AmbientCubeMin);

	INT X = Clamp( appFloor(Diff.X * (Info.NumAmbientCubeX-1)), 0, Info.NumAmbientCubeX-1 );
	INT Y = Clamp( appFloor(Diff.Y * (Info.NumAmbientCubeY-1)), 0, Info.NumAmbientCubeY-1 );
	INT Z = Clamp( appFloor(Diff.Z * (Info.NumAmbientCubeZ-1)), 0, Info.NumAmbientCubeZ-1 );

	INT AmbientCubeIndex = X + (Y + Z * Info.NumAmbientCubeY) * Info.NumAmbientCubeX + Info.SHStartIndex;	

	return AmbientCubeIndex < AmbientSHs.Num() ? AmbientSHs(AmbientCubeIndex) : Zero;
}

void AWorldInfo::InterpolateAmbientCube( const FVector& Location, FAmbientSH& Result ) const
{
	const FAmbientCubeVolumeInfo* Info = DetermineAmbientCubeVolume( Location );
	if (!Info) 
	{
	...
	InterpolateAmbientCube( *Info, Location, Result );
}
...
void EncodeIrradiance( BYTE c[4], FVector v )
{
	INT i;		
	FLOAT max = 0.0f;				
	for( i = 0; i < 3; i++ )
	...
	FLOAT scalar = appPow( 2.0f, -exponent );
	...
	c[0] = ( unsigned char )v[0];
	c[1] = ( unsigned char )v[1];
	c[2] = ( unsigned char )v[2];
	c[3] = ( signed char )exponent;
}

void DecodeIrradiance( FVector& IrradianceVec, const BYTE Irradiance[4] )
{	
...
#define AMBIENTCUBE_PRECISE		0x1
#define AMBIENTCUBE_IRRADIANCE	0x2
#define AMBIENTCUBE_SH			0x4

static INT GAmbientFetchFlags = 0;

void BeginFetchIrradiance()
{
	GAmbientFetchFlags = AMBIENTCUBE_IRRADIANCE;
}

void BeginFetchSH()
{
	GAmbientFetchFlags = AMBIENTCUBE_PRECISE | AMBIENTCUBE_SH;
}


struct FNativeAmbientSH
{
public :
	FSHVectorRGB SH;
	BYTE Irradiance[4];
	...
	FNativeAmbientSH( const FAmbientSH& Src )
	{
		check( sizeof(FAmbientSH) == sizeof(FNativeAmbientSH) );

		if (GAmbientFetchFlags & AMBIENTCUBE_SH)
		{
			appMemcpy( &SH, Src.SHValue, sizeof(SH) );
		}
		if (GAmbientFetchFlags & AMBIENTCUBE_IRRADIANCE)
		{
			appMemcpy( Irradiance, Src.Irradiance, sizeof(Irradiance) );
		}		
	}

	void Lerp( const FNativeAmbientSH& A, const FNativeAmbientSH& B, FLOAT u )
	{
		FVector IVA, IVB, IV;
		...
		if (GAmbientFetchFlags & AMBIENTCUBE_IRRADIANCE)
		{
			DecodeIrradiance( IVA, A.Irradiance );
			DecodeIrradiance( IVB, B.Irradiance );

			IV = IVA + (IVB - IVA) * u;
			
			if( !GIsEditor && GIsLowEndHW )
			{
				const ULocalPlayer* Player = GEngine->GamePlayers(0);
				if( Player )
				{
					FSceneViewState* ViewState = (FSceneViewState*)(Player->ViewState);
					FLuminanceHistogram& Histogram = ViewState->ExposureData.LuminanceHistogram;
					Histogram.Tonemap(IV);
				}
			}
			
			EncodeIrradiance( Irradiance, IV );
		}
	}

	void CopyTo( FAmbientSH& Dest )
	{
		check( sizeof(FAmbientSH) == sizeof(FNativeAmbientSH) );
		
		if (GAmbientFetchFlags & AMBIENTCUBE_SH)
		{
			appMemcpy( Dest.SHValue, &SH, sizeof(SH) );
		}
		if (GAmbientFetchFlags & AMBIENTCUBE_IRRADIANCE)
		{
			appMemcpy( Dest.Irradiance, Irradiance, sizeof(Irradiance) );
		}		
	}
};

void AWorldInfo::InterpolateAmbientCube( const FAmbientCubeVolumeInfo& Info, const FVector& Location, FAmbientSH& Result ) const
{
	memset( &Result, 0, sizeof(Result) );

	if (Info.NumAmbientCubeX * Info.NumAmbientCubeY * Info.NumAmbientCubeZ == 0) 
	{		
		return;
	}

	FVector Size = (Info.AmbientCubeMax - Info.AmbientCubeMin);
	FVector Diff = (Location - Info.AmbientCubeMin);

	Diff *= FVector( (Info.NumAmbientCubeX-1) / Size.X, (Info.NumAmbientCubeY-1) / Size.Y, (Info.NumAmbientCubeZ-1) / Size.Z );

	INT BaseX = appFloor(Diff.X), BaseY = appFloor(Diff.Y), BaseZ = appFloor(Diff.Z);

	Diff -= FVector( BaseX, BaseY, BaseZ );

	if (BaseX >= Info.NumAmbientCubeX-1) { BaseX = Info.NumAmbientCubeX-1; Diff.X = 0.0f; } else if (BaseX < 0) { BaseX = 0; Diff.X = 0.0f; }
	if (BaseY >= Info.NumAmbientCubeY-1) { BaseY = Info.NumAmbientCubeY-1; Diff.Y = 0.0f; } else if (BaseY < 0) { BaseY = 0; Diff.Y = 0.0f; }
	if (BaseZ >= Info.NumAmbientCubeZ-1) { BaseZ = Info.NumAmbientCubeZ-1; Diff.Z = 0.0f; } else if (BaseZ < 0) { BaseZ = 0; Diff.Z = 0.0f; }

	FLOAT Weight = 0;

	FNativeAmbientSH SHs[8 + 4 + 2 + 1];
	UBOOL SHValidity[8 + 4 + 2 + 1];

	for (INT i=0; i<8; ++i)
	{
		SHValidity[i] = FALSE;

		INT X = (i&1) ? (BaseX+1) : BaseX;
		INT Y = (i&2) ? (BaseY+1) : BaseY;
		INT Z = (i&4) ? (BaseZ+1) : BaseZ;

		if (X < 0 || X >= Info.NumAmbientCubeX ||
			Y < 0 || Y >= Info.NumAmbientCubeY ||
			Z < 0 || Z >= Info.NumAmbientCubeZ)
			continue;					

		INT AmbientCubeIndex = X + (Y + Z * Info.NumAmbientCubeY) * Info.NumAmbientCubeX + Info.SHStartIndex;

		const FAmbientSH& Sample = AmbientSHs(AmbientCubeIndex);

		FCheckResult Hit;						

		FVector BoxCenter = Info.AmbientCubeMin + FVector( Size.X * (X) / (Info.NumAmbientCubeX-1), Size.Y * (Y) / (Info.NumAmbientCubeY-1), Size.Z * (Z) / (Info.NumAmbientCubeZ-1) );

		if (GAmbientFetchFlags & AMBIENTCUBE_PRECISE)
		{
			GWorld->SingleLineCheck( Hit, NULL, BoxCenter, Location, TRACE_Level | TRACE_Terrain | TRACE_LevelGeometry | TRACE_LightVisibility | TRACE_StopAtAnyHit );
		}		
	...
	for (INT Phase=0; Phase<3; ++Phase)
	{
		...
		for (INT i=0; i<Num; ++i)
		{
			const UBOOL ValidityA = SHValidity[Src + i*2];
			const UBOOL ValidityB = SHValidity[Src + i*2 + 1];

			const FNativeAmbientSH& SHA = SHs[Src + i*2];
			const FNativeAmbientSH& SHB = SHs[Src + i*2 + 1];

			SHValidity[Dst + i] = (ValidityA || ValidityB);
			if (!SHValidity[Dst + i])
				continue;
				
			if (ValidityA && ValidityB)
			{				
				SHs[Dst + i].Lerp( SHA, SHB, Diff[Phase] );
			}
			else if (ValidityA)
			{
				SHs[Dst + i] = SHA;
	...
	if (!SHValidity[14]) return;

	SHs[14].CopyTo( Result );	
}
...
void AWorldInfo::Serialize( FArchive& Ar )
{
	...
	if (Ar.IsSaving() || Ar.IsLoading() && Ar.LicenseeVer() >= VER_AVA_COLORCORRECTION_WORLD)
	{
		Ar << ColorCorrections;		
	}
	...
	if (Ar.IsSaving())	
	{
		INT NumSHs = AmbientSHs.Num();

		Ar << NumSHs;

		if (NumSHs > 0) 
		{			
			Ar.SerializeCompressed( &AmbientSHs(0), NumSHs * sizeof(FAmbientSH), COMPRESS_LZO );
		}		
	}
	else if (Ar.IsLoading())
	{
		if (Ar.LicenseeVer() < VER_AVA_SHAMBIENT)
		{
			AmbientSHs.Empty( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ );
			AmbientSHs.AddZeroed( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ  );

			TArray<FVector> LegacyAmbientCubes;

			LegacyAmbientCubes.Empty( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ * 6 );
			LegacyAmbientCubes.AddZeroed( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ * 6 );

			if (NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ > 0) 
			{
				Ar.Serialize( &LegacyAmbientCubes(0), NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ * sizeof(FVector) * 6 );
			}					

			// Add the light to the dynamic light environment SH.
			for (INT i=0; i<NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ; ++i)			
			{
				FSHVectorRGB& NewStaticLightEnvironment = (FSHVectorRGB&)AmbientSHs(i);

				const FVector* AmbientCube = &LegacyAmbientCubes( i * 6 );

				NewStaticLightEnvironment += PointLightSH( FVector( +1, 0, 0 ) ) * FLinearColor( AmbientCube[0].X, AmbientCube[0].Y, AmbientCube[0].Z );
				NewStaticLightEnvironment += PointLightSH( FVector( 0, +1, 0 ) ) * FLinearColor( AmbientCube[2].X, AmbientCube[2].Y, AmbientCube[2].Z );
				NewStaticLightEnvironment += PointLightSH( FVector( 0, 0, +1 ) ) * FLinearColor( AmbientCube[4].X, AmbientCube[4].Y, AmbientCube[4].Z );
				NewStaticLightEnvironment += PointLightSH( FVector( -1, 0, 0 ) ) * FLinearColor( AmbientCube[1].X, AmbientCube[1].Y, AmbientCube[1].Z );
				NewStaticLightEnvironment += PointLightSH( FVector( 0, -1, 0 ) ) * FLinearColor( AmbientCube[3].X, AmbientCube[3].Y, AmbientCube[3].Z );
				NewStaticLightEnvironment += PointLightSH( FVector( 0, 0, -1 ) ) * FLinearColor( AmbientCube[5].X, AmbientCube[5].Y, AmbientCube[5].Z );

				AmbientSHs(i).Irradiance[0] = 0;
				AmbientSHs(i).Irradiance[1] = 0;
				AmbientSHs(i).Irradiance[2] = 0;
				AmbientSHs(i).Irradiance[3] = 0;
			}		
		}		
		else if (Ar.LicenseeVer() < VER_AVA_MULTIPLE_AMBIENTCUBEVOLUMES)
		{
			AmbientSHs.Empty( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ );
			AmbientSHs.AddZeroed( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ  );

			//<@ changmin 2007. 4. 26
			struct FLegacyAmbientSH
			{
				FLOAT SHValue[12];
			};
			TArray<FLegacyAmbientSH> LegacyAmbientSHs;
			LegacyAmbientSHs.Empty( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ );
			LegacyAmbientSHs.AddZeroed( NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ  );
			//>@ changmin

			if (NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ > 0) 
			{
				Ar.Serialize( &LegacyAmbientSHs(0), NumAmbientCubeX * NumAmbientCubeY * NumAmbientCubeZ * sizeof(FLegacyAmbientSH) );
			}

			//<@ changmin : 2007. 4. 26
			for( INT SHIndex = 0; SHIndex < AmbientSHs.Num(); ++SHIndex )
			{
				appMemcpy( AmbientSHs(SHIndex).SHValue, LegacyAmbientSHs(SHIndex).SHValue, sizeof(FLOAT) * 12 );
				AmbientSHs(SHIndex).Irradiance[0] = 0;
				AmbientSHs(SHIndex).Irradiance[1] = 0;
				AmbientSHs(SHIndex).Irradiance[2] = 0;
				AmbientSHs(SHIndex).Irradiance[3] = 0;
			}
			//>@ changmin

			FAmbientCubeVolumeInfo Info;

			Info.NumAmbientCubeX = NumAmbientCubeX;
			Info.NumAmbientCubeY = NumAmbientCubeY;
			Info.NumAmbientCubeZ = NumAmbientCubeZ;
			Info.AmbientCubeMin = AmbientCubeMin;
			Info.AmbientCubeMax = AmbientCubeMax;
			Info.SHStartIndex = 0;

			AmbientCubeVolumeInfos.AddItem( Info );

			MarkPackageDirty();
		}
		else if( Ar.LicenseeVer() < VER_AVA_ADD_IRRADIANCE_TO_AMBIENTCUBE )	// changmin 2007. 4. 26
		{
			INT NumSHs;

			Ar << NumSHs;

			AmbientSHs.Empty( NumSHs );
			AmbientSHs.AddZeroed( NumSHs );
			...
			if (NumSHs > 0) 
			{
				...
				TArray<FLegacyAmbientSH> LegacyAmbientSHs;
				LegacyAmbientSHs.Empty( NumSHs);
				LegacyAmbientSHs.AddZeroed( NumSHs );

				Ar.Serialize( &LegacyAmbientSHs(0), NumSHs * sizeof(FLegacyAmbientSH) );

				for( INT SHIndex = 0; SHIndex < NumSHs; ++SHIndex )
				{
					appMemcpy( AmbientSHs(SHIndex).SHValue, LegacyAmbientSHs(SHIndex).SHValue, sizeof(FLOAT) * 12 );
					AmbientSHs(SHIndex).Irradiance[0] = 0;
					AmbientSHs(SHIndex).Irradiance[1] = 0;
		...
		else if ( Ar.LicenseeVer() < VER_AVA_COMPRESSED_AMBIENTCUBE)
		{
			INT NumSHs;
			...
			if (NumSHs > 0) 
			{
				Ar.Serialize( &AmbientSHs(0), NumSHs * sizeof(FAmbientSH) );
			}
		}
		else
		{
