...
void FKSphereElem::DrawElemSolid(class FPrimitiveDrawInterface* PDI, const FMatrix& ElemTM, FLOAT Scale, const FMaterialInstance* MaterialInstance)
{
	DrawSphere(PDI, ElemTM.GetOrigin(), FVector( this->Radius * Scale ), DrawCollisionSides, DrawCollisionSides/2, MaterialInstance, SDPG_World );
}
...
void FKBoxElem::DrawElemSolid(class FPrimitiveDrawInterface* PDI, const FMatrix& ElemTM, FLOAT Scale, const FMaterialInstance* MaterialInstance)
{
	DrawBox(PDI, ElemTM, 0.5f * FVector(X, Y, Z), MaterialInstance, SDPG_World );
}
...
void FKSphylElem::DrawElemSolid(FPrimitiveDrawInterface* PDI, const FMatrix& ElemTM, FLOAT Scale, const FMaterialInstance* MaterialInstance)
{
	...
	for(INT RingIdx=0; RingIdx<NumRings+1; RingIdx++)
	{
		...
		ArcVert->TangentY = FVector(0.0f, -SpherePos.Z, SpherePos.Y);
	...
	for(INT SideIdx=0; SideIdx<NumSides+1; SideIdx++)
	{
		...
		for(INT VertIdx=0; VertIdx<NumRings+1; VertIdx++)
		{
			...
			Verts[VIx].TangentX = ArcRot.TransformNormal( ArcVerts[VertIdx].TangentX );
			Verts[VIx].TangentY = ArcRot.TransformNormal( ArcVerts[VertIdx].TangentY );
			Verts[VIx].TangentZ = ArcRot.TransformNormal( ArcVerts[VertIdx].TangentZ );
	...
	MeshBuilder.Draw(PDI, ElemTM, MaterialInstance, SDPG_World);
...
static UBOOL DirIsFaceEdge(const FVector& InDir, const FKConvexElem* InElem)
{
	FVector UnitDir = InDir.SafeNormal();

	for(INT i=0; i<InElem->EdgeDirections.Num(); i++)
	{
		FLOAT EdgeDot = UnitDir | InElem->EdgeDirections(i);
...
void FKConvexElem::DrawElemWire(FPrimitiveDrawInterface* PDI, const FMatrix& ElemTM, const FVector& Scale3D, const FColor Color)
{
	...
	for(INT i=0; i<NumTris; i++)
	{
		...
		if( DirIsFaceEdge(VertexData(I0)-VertexData(I1), this) )
			PDI->DrawLine( TransformedVerts(I0), TransformedVerts(I1), Color, SDPG_World );

		if( DirIsFaceEdge(VertexData(I1)-VertexData(I2), this) )
			PDI->DrawLine( TransformedVerts(I1), TransformedVerts(I2), Color, SDPG_World );

		if( DirIsFaceEdge(VertexData(I2)-VertexData(I0), this) )
...
void FKConvexElem::AddCachedSolidConvexGeom(TArray<FDynamicMeshVertex>& VertexBuffer, TArray<INT>& IndexBuffer)
{
	...
	for(INT i=0; i<NumTris; i++)
	{
		...
		for(INT j=0; j<3; j++)
		{
			...
			Vert.TangentX = Edge0.SafeNormal();
			Vert.TangentY = (Normal ^ Edge0).SafeNormal();
			Vert.TangentZ = Normal.SafeNormal();
...
void FKAggregateGeom::DrawAggGeom(FPrimitiveDrawInterface* PDI, const FMatrix& ParentTM, const FVector& Scale3D, const FColor Color, const FMaterialInstance* MatInst, UBOOL bPerHullColor, UBOOL bDrawSolid)
{
		...
		if(bDrawSolid)
		{
			...
			if(!RenderInfo)
			{
				...
				for(INT i=0; i<ConvexElems.Num(); i++)
				{
					...
					ConvexElems(i).AddCachedSolidConvexGeom(RenderInfo->VertexBuffer->Vertices, RenderInfo->IndexBuffer->Indices);
				}

				RenderInfo->VertexBuffer->Init();
				RenderInfo->IndexBuffer->Init();
                
				RenderInfo->CollisionVertexFactory = new FConvexCollisionVertexFactory(RenderInfo->VertexBuffer);
				RenderInfo->CollisionVertexFactory->Init();
			}
			...
			// Draw the mesh.
			FMeshElement Mesh;
			Mesh.IndexBuffer = RenderInfo->IndexBuffer;
			Mesh.VertexFactory = RenderInfo->CollisionVertexFactory;
			Mesh.MaterialInstance = MatInst;
			Mesh.LocalToWorld = LocalToWorld;
			Mesh.WorldToLocal = LocalToWorld.Inverse();
			// previous l2w not used so treat as static			
			Mesh.FirstIndex = 0;
			Mesh.NumPrimitives = RenderInfo->IndexBuffer->Indices.Num() / 3;
			Mesh.MinVertexIndex = 0;
			Mesh.MaxVertexIndex = RenderInfo->VertexBuffer->Vertices.Num() - 1;
...
FMatrix GetSkelBoneMatrix(INT BoneIndex, const TArray<FMatrix>& SpaceBases, const FMatrix& LocalToWorld)
{
...
void UPhysicsAsset::DrawCollision(FPrimitiveDrawInterface* PDI, const USkeletalMesh* SkelMesh, const TArray<FMatrix>& SpaceBases, const FMatrix& LocalToWorld, FLOAT Scale)
{
...
void UPhysicsAsset::DrawConstraints(FPrimitiveDrawInterface* PDI, const USkeletalMesh* SkelMesh, const TArray<FMatrix>& SpaceBases, const FMatrix& LocalToWorld, FLOAT Scale)
{
	for( INT i=0; i<ConstraintSetup.Num(); i++ )
	{
		...
		{
			SkelComp->LimitMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_JointLimitMaterial"), NULL, LOAD_None, NULL);
		}
...
static void DrawLinearLimit(FPrimitiveDrawInterface* PDI, const FVector& Origin, const FVector& Axis, const FVector& Orth, FLOAT LinearLimitRadius, UBOOL bLinearLimited)
{
	if(bLinearLimited)
	{
		...
		PDI->DrawLine(  Start - (0.2f * LimitRenderSize * Orth), Start + (0.2f * LimitRenderSize * Orth), JointLimitColor, SDPG_World );
		PDI->DrawLine(  End - (0.2f * LimitRenderSize * Orth), End + (0.2f * LimitRenderSize * Orth), JointLimitColor, SDPG_World );
	}
	else
	{
		...
		// Draw arrow heads.
		PDI->DrawLine(  Start, Start + (0.2f * LimitRenderSize * Axis) + (0.2f * LimitRenderSize * Orth), JointLimitColor, SDPG_World );
		PDI->DrawLine(  Start, Start + (0.2f * LimitRenderSize * Axis) - (0.2f * LimitRenderSize * Orth), JointLimitColor, SDPG_World );

		PDI->DrawLine(  End, End - (0.2f * LimitRenderSize * Axis) + (0.2f * LimitRenderSize * Orth), JointLimitColor, SDPG_World );
		PDI->DrawLine(  End, End - (0.2f * LimitRenderSize * Axis) - (0.2f * LimitRenderSize * Orth), JointLimitColor, SDPG_World );
	}
}
										 ...
										 const FMatrix& Con1Frame, const FMatrix& Con2Frame, UBOOL bDrawAsPoint)
{
	...
	if(bDrawSelected)
	{
		DrawOrientedStar( PDI, Con1Frame, JointRenderSize, JointFrame1Color );
		DrawOrientedStar( PDI, Con2Frame, JointRenderSize, JointFrame2Color );
	}
	else
	{
		DrawOrientedStar( PDI, Con1Frame, JointRenderSize, JointUnselectedColor );
		DrawOrientedStar( PDI, Con2Frame, JointRenderSize, JointUnselectedColor );
	}
	...
	if(!bLinearXLocked)
		DrawLinearLimit(PDI, Con2Frame.GetOrigin(), Con2Frame.GetAxis(0), Con2Frame.GetAxis(2), LinearLimitRadius, bLinearLimited);

	if(!bLinearYLocked)
		DrawLinearLimit(PDI, Con2Frame.GetOrigin(), Con2Frame.GetAxis(1), Con2Frame.GetAxis(2), LinearLimitRadius, bLinearLimited);

	if(!bLinearZLocked)
		DrawLinearLimit(PDI, Con2Frame.GetOrigin(), Con2Frame.GetAxis(2), Con2Frame.GetAxis(0), LinearLimitRadius, bLinearLimited);
	...
	if(bSwingLimited)
	{
		...
		if(bLockAllSwing)
		{
			...
			PDI->DrawLine( ConeLimitTM.GetOrigin(), ConeLimitTM.TransformFVector( 0.3f * LimitRenderSize * FVector(1,1,0) ), JointLockedColor, SDPG_World);
			PDI->DrawLine( ConeLimitTM.GetOrigin(), ConeLimitTM.TransformFVector( 0.3f * LimitRenderSize * FVector(1,-1,0) ), JointLockedColor, SDPG_World);
		}
		else
		{
			...
			FMatrix ConeToWorld = FScaleMatrix( FVector(LimitRenderSize) ) * ConeLimitTM;
			DrawCone(PDI, ConeToWorld, ang1, ang2, DrawConeLimitSides, true, JointLimitColor, LimitMaterial->GetInstanceInterface(false), SDPG_World );
	...
	if(bTwistLimited)
	{
		if(bLockTwist)
		{
			...
			PDI->DrawLine(  RefLineEnd + Con1Frame.TransformNormal( 0.3f * LimitRenderSize * FVector(0.f,-0.5f,-0.5f) ), 
							RefLineEnd + Con1Frame.TransformNormal( 0.3f * LimitRenderSize * FVector(0.f, 0.5f, 0.5f) ), JointLockedColor, SDPG_World);
		...
		else
		{
			...
			PDI->DrawLine( RefLineEnd, RefLineEnd + ChildTwistRef * LimitRenderSize, JointRefColor, SDPG_World );
			...
			PDI->DrawLine( RefLineEnd, RefLineEnd + WLimit * LimitRenderSize, JointLimitColor, SDPG_World );

			RotateLimit = FQuat( Con1Frame.GetAxis(0), -TwistLimitAngle * (PI/180.0f) );
			WLimit = RotateLimit.RotateVector(ChildTwistLimit);
			PDI->DrawLine( RefLineEnd, RefLineEnd + WLimit * LimitRenderSize, JointLimitColor, SDPG_World );

			DrawArc(RefLineEnd, ChildTwistLimit, -ChildTwistLimit ^ Con1Frame.GetAxis(0), -TwistLimitAngle, TwistLimitAngle, 0.8f * LimitRenderSize, 8, JointLimitColor, PDI);
		}
	}
	else
	{
		...
		if(bLockAllSwing)
			PDI->DrawLine(  Con2Frame.GetOrigin() - LimitRenderSize * Con2Frame.GetAxis(0), 
							Con2Frame.GetOrigin() + LimitRenderSize * Con2Frame.GetAxis(0), JointLimitColor, SDPG_World );
	}
...
public:
		...
		FPrimitiveSceneProxy(InComponent)
	{
		...
		if(!LimitMaterial)
		{
			LimitMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_JointLimitMaterial"), NULL, LOAD_None, NULL);
		}
	...
	*/
	virtual void DrawDynamicElements(FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex,DWORD Flags)
	{
		...
		Setup->DrawConstraint(PDI, 1.f, TRUE, TRUE, LimitMaterial, Con1Frame, Con2Frame, FALSE);
