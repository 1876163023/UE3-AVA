 ...
 */
FString UTextureRenderTarget2D::GetDetailedDescription( INT Index )
{
 ...
 */
UTexture2D* UTextureRenderTarget2D::ConstructTexture2D(UObject* Outer, const FString& NewTexName, EObjectFlags InFlags)
{
		...
		SizeY != 0 && !(SizeY & (SizeY-1)) )
	{
		...
		if( RenderTarget )
		{
			...
			Result = CastChecked<UTexture2D>( 
				StaticConstructObject(UTexture2D::StaticClass(), Outer, FName(*NewTexName), InFlags) );
			...
			DWORD* TextureData = (DWORD*)Mip.Data.Lock(LOCK_WRITE);
