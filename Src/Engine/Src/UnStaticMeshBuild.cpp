...
void UStaticMesh::Build()
{
	...
	for(INT i=0;i<LODModels.Num();i++)
	{
		...
		LODModels(i).Build(kDOPBuildTriangles,(i==0));
	}
...
void FStaticMeshRenderData::GetKDOPTriangles(TArray<FkDOPBuildCollisionTriangle>& kDOPBuildTriangles) 
{
	for(INT TriangleIndex = 0; TriangleIndex < IndexBuffer.Indices.Num(); TriangleIndex += 3)
	{
		...
		new (kDOPBuildTriangles) FkDOPBuildCollisionTriangle(IndexOne,
...
//	UStaticMesh::Build
//
void FStaticMeshRenderData::Build(TArray<FkDOPBuildCollisionTriangle>& kDOPBuildTriangles, UBOOL BuildKDop)
{
#if !CONSOLE
	// Load raw data.
	...
	for(INT TriangleIndex = 0;TriangleIndex < RawTriangles.GetElementCount();TriangleIndex++)
	{
		...
		if(Element.EnableCollision && BuildKDop)
		{
			...
			new (kDOPBuildTriangles) FkDOPBuildCollisionTriangle(VertexIndices[0],
