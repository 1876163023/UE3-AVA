...
void UTextureMovie::Serialize(FArchive& Ar)
{
	...
	{
		Data.SerializeLikeLazyArray( Ar, this );
	}
	else
	{
...
void UTextureMovie::InitDecoder()
{
	...
	if( Decoder )
	{
			...
			MovieStreamSource == MovieStream_File )		
		{
			...
			ValidStream = Decoder->Open( GetLinker()->Filename, Data.GetBulkDataOffsetInFile(), Data.GetBulkDataSize() );
		}
		else
		{
			...
			ValidStream = Decoder->Open( CopyOfData, Data.GetBulkDataSize() );		
		}
 ...
 */
FString UTextureMovie::GetDetailedDescription( INT Index )
{
