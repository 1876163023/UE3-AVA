	...
	FMyShadowVolumeDrawInterface( FCommandContextRHI* InContext, const FLightSceneInfo* InLightSceneInfo,UINT InDepthPriorityGroup ):
	  ...
	  */
	  void SetView( const FViewInfo *InView )
	  {
		  ...
		  LightSceneInfo->SetDepthBounds(Context,View);
	  }
	  ...
	  */
	  void DrawShadowVolume( FLightPrimitiveInteraction *Interaction )
	  {
		  ...
		  {		
			  ...
			  if( LightSceneInfo->GetRadius() == 0.f 
				  ||  LightSceneInfo->GetRadius() > GEngine->ShadowVolumeLightRadiusThreshold )
			  {
	...
	if( UEngine::ShadowVolumesAllowed() )
	{
		...
		for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
		{
				...
				(LightSceneInfo->LightShadowMode == LightShadow_Modulate && VisibleLightDPGInfo.bVisibleInFrustum) )
			{
				...
				// Reset the scissor rectangle and depth bounds test.
