...
public:
	...
	void SetParameters(FCommandContextRHI* Context,const FVertexFactory* VertexFactory,const FMaterialInstance* MaterialInstance,const FSceneView* View, const FMaterialInstance* OriginalMaterialInstance)
	{
		...
		const FMaterial* OriginalMaterial = OriginalMaterialInstance->GetMaterial();
	...
	)
{
	...
	if ( TextureLookupInfo.Num() > 0 )
	{
		...
		FTextureDensityDrawingPolicy DrawingPolicy( Mesh.VertexFactory, MaterialInstance, OriginalMaterialInstance );
...
UBOOL FTextureDensityDrawingPolicyFactory::DrawStaticMesh( FCommandContextRHI* Context, const FSceneView* View, const FStaticMesh* Mesh, UBOOL bDrawnShared )
{
	// Only draw opaque materials in the depth pass.
	const FMaterialInstance* OriginalMaterialInstance = Mesh->MaterialInstance;
	const FMaterialInstance* MaterialInstance = OriginalMaterialInstance;
	...
	if ( TextureLookupInfo.Num() > 0 )
	{
		...
		FTextureDensityDrawingPolicy DrawingPolicy( MeshElement.VertexFactory, MaterialInstance, OriginalMaterialInstance );
...
UBOOL FSceneRenderer::RenderTextureDensities(UINT DPGIndex)
{
	...
	FTextureDensityDrawingPolicyFactory PolicyFactory;
