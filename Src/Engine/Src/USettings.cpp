...
void FSettingsData::SetData(const TCHAR* InData)
{
	...
	if (InData != NULL)
	{
		...
		{
			// Copy the data
			appMemcpy((void*)Value2,(void*)InData,(Value1 + 1) * sizeof(TCHAR));
		}
		else
		{
...
void FSettingsData::SetData(DOUBLE InData)
{
	CleanUp();
	Type = SDT_Double;
	// Get it into a QWORD so we can grab high/low parts
	QWORD FullData = *(QWORD*)&InData;
	// Copy it into high/low parts
	Value1 = (FullData >> 32) & 0xFFFFFFFF;
	Value2 = (INT*)(PTRINT)(FullData & 0xFFFFFFFF);
}
...
void FSettingsData::SetData(QWORD InData)
{
	...
	Type = SDT_Int64;
	// Copy it into high/low parts
	Value1 = (InData >> 32) & 0xFFFFFFFF;
	Value2 = (INT*)(PTRINT)(InData & 0xFFFFFFFF);
}
 ...
 * @param InData2 the second part to assign
 */
void FSettingsData::GetData(INT& InData1,INT& InData2)
{
 ...
 * NOTE: this string must persist for the life of the data object or there
 * will be crashes due to accesses on freed memory
 *
 ...
 */
INT USettings::GetStringSettingValue(INT StringSettingId,INT& ValueIndex)
{
	...
	debugf(NAME_Error,TEXT("Requested localized string setting %d was not found"),
		StringSettingId);
	return FALSE;
}
 ...
 * @return true if found, false otherwise
 */
UBOOL USettings::GetStringSettingValueNames(INT StringSettingId,TArray<FName>& Values)
{
	...
	for (INT Index = 0; Index < LocalizedSettingsMappings.Num(); Index++)
	{
		if (LocalizedSettingsMappings(Index).Id == StringSettingId)
		{
			...
			{
				Values.AddItem(LocalizedSettingsMappings(Index).ValueMappings(Index2).Name);
			}
			return TRUE;
		}
	}
...
void USettings::SetStringSettingValueByName(FName StringSettingName,INT ValueIndex,UBOOL bShouldAutoAdd)
{
	...
	debugf(NAME_Error,TEXT("Requested localized string setting %s was not found"),
		*StringSettingName.ToString());
}
...
UBOOL USettings::IsWildcardStringSetting(INT StringSettingId)
{
	...
	for (INT Index = 0; Index < LocalizedSettingsMappings.Num(); Index++)
	{
		if (LocalizedSettingsMappings(Index).Id == StringSettingId)
		{
			...
			{
				const FLocalizedStringSetting& Setting = LocalizedSettings(Index);
...
UBOOL USettings::SetPropertyFromStringByName(FName PropertyName,const FString& NewValue)
{
	...
	if (GetPropertyId(PropertyName,PropertyId))
	{
		...
		for (INT Index = 0; Index < Properties.Num(); Index++)
		{
			if (Properties(Index).PropertyId == PropertyId)
			{
				return Properties(Index).Data.FromString(NewValue);
			}
		}
	}
	return FALSE;
...
/**
 * Sets a property of type SDT_Int64 to the value specified. Does nothing
 * if the property is not of the right type.
 *
 * @param PropertyId the property to change the value of
 * @param Value the new value to assign
 */
void USettings::SetQwordProperty(INT PropertyId,QWORD Value)
{
	// Find the specified property
	FSettingsProperty* Property = FindProperty(PropertyId);
	if (Property != NULL && Property->Data.Type == SDT_Int64)
	{
		// Set the value
		Property->Data.SetData(Value);
	}
...
/**
 * Reads a property of type SDT_Int64 into the value specified. Does nothing
 * if the property is not of the right type.
 *
 * @param PropertyId the property to change the value of
 * @param Value the out value containing the property's value
 ...
 * @return true if found and is the right type, false otherwise
 */
UBOOL USettings::GetQwordProperty(INT PropertyId,QWORD& Value)
{
	UBOOL bSet = FALSE;
	// Find the specified property
	FSettingsProperty* Property = FindProperty(PropertyId);
...
/**
 * Sets a property of type SDT_Double to the value specified. Does nothing
 * if the property is not of the right type.
 *
 * @param PropertyId the property to change the value of
 * @param Value the new value to assign
 */
void USettings::SetDoubleProperty(INT PropertyId,DOUBLE Value)
{
	// Find the specified property
	FSettingsProperty* Property = FindProperty(PropertyId);
	if (Property != NULL && Property->Data.Type == SDT_Double)
	{
		// Set the value
		Property->Data.SetData(Value);
	}
...
/**
 * Reads a property of type SDT_Double into the value specified. Does nothing
 * if the property is not of the right type.
 *
 * @param PropertyId the property to change the value of
 * @param Value the out value containing the property's value
 ...
 * @return true if found and is the right type, false otherwise
 */
UBOOL USettings::GetDoubleProperty(INT PropertyId,DOUBLE& Value)
{
	UBOOL bSet = FALSE;
	// Find the specified property
	FSettingsProperty* Property = FindProperty(PropertyId);
