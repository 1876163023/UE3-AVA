...
//@todo joeg - remove later
#if PS3
// small font is too unreadable on TVs so use medium
#define STAT_FONT	GEngine->MediumFont
	...
	for (FStatGroup* Group = FirstGroup; Group != NULL; Group = Group->NextGroup)
	{
		...
		if (Group->GroupId != STATGROUP_Default && Group->bShowGroup == TRUE)
		{
			...
			if (bShowCounters)
			{
				...
				if (Group->FirstMemoryCounter != NULL)
				{
				...
				// Don't show the headings if there aren't any
				if (Group->FirstFloatAccumulator != NULL ||
					Group->FirstDwordAccumulator != NULL ||
					Group->FirstFloatCounter ||
					Group->FirstDwordCounter)
				{
