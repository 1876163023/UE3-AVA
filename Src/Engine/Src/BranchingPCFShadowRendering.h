...
public:
	...
	*/
		...
		)
	{
		...
		FVector2D InvTexSize = FVector2D(1.0f / FLOAT(GEngine->RandomAngleTexture->SizeX), 1.0f / FLOAT(GEngine->RandomAngleTexture->SizeY));
		SetPixelShaderValue(Context,GetPixelShader(),InvRandomAngleTextureSize, InvTexSize);
		...
		const FLOAT TexelRadius = GEngine->ShadowFilterRadius * InvBufferResolution;
	...
	*/
	...
	/**
	* Put the low quality versions in their own group for faster iterations
	*/
	virtual EShaderRecompileGroup GetRecompileGroup() const
	{
		if (BranchingPCFPolicy::NumEdgeSamples == BPCFEdge_Low 
			|| BranchingPCFPolicy::NumEdgeSamples == BHardwarePCFEdge_Low
			|| BranchingPCFPolicy::NumEdgeSamples == BFetch4PCFEdge_Low)
		{
			return SRG_GLOBAL_BPCF_SHADOW_LOW;
		}
		else
		{
			return SRG_GLOBAL_BPCF_SHADOW_MEDIUM_HIGH;
		}
	}
private:
	...
	FShaderParameter RandomAngleTextureParameter;
...
public:
	...
	*/
	...
	{
		FBranchingPCFProjectionPixelShader< BranchingPCFPolicy >::SetParameters(Context,View,ShadowInfo);		
	...
	*/
	...
	{
		FBranchingPCFProjectionPixelShader< BranchingPCFPolicy >::Serialize(Ar);
	...
	*/
	static void ModifyCompilationEnvironment(FShaderCompilerEnvironment& OutEnvironment)
	{
		FBranchingPCFProjectionPixelShader< BranchingPCFPolicy >::ModifyCompilationEnvironment(OutEnvironment);
	}
...
public:
	...
	*/
	static void ModifyCompilationEnvironment(FShaderCompilerEnvironment& OutEnvironment)
	{
		FBranchingPCFModProjectionPixelShader<BranchingPCFPolicy>::ModifyCompilationEnvironment(OutEnvironment);
	...
	*/
	...
	{
		FBranchingPCFModProjectionPixelShader<BranchingPCFPolicy>::SetParameters(Context,View,ShadowInfo);
	...
	*/
	void Serialize(FArchive& Ar)
	{
		FBranchingPCFModProjectionPixelShader<BranchingPCFPolicy>::Serialize(Ar);
...
*							based on the current setting of GEngine->ShadowFilterQuality, and sets its parameters.
*
* @param Context - current Context
* @param View - current View
* @param ShadowInfo - current FProjectedShadowInfo
* @param ShadowProjectionTechnique - the shadow projection technique of the requesting light, a member of EShadowProjectionTechnique
...
*				based on the current setting of GEngine->ShadowFilterQuality and ShadowProjectionTechnique, and returns a pointer to it.
* 
* @param ShadowProjectionTechnique - the shadow projection technique of the requesting light, a member of EShadowProjectionTechnique
* @return FBranchingPCFProjectionPixelShaderInterface - a pointer to the chosen instance
*/
template<class LightTypePolicy>
FBranchingPCFProjectionPixelShaderInterface* GetBranchingPCFModProjPixelShaderRef(BYTE ShadowProjectionTechnique)
{
	...
	if (ShadowProjectionTechnique == ShadowProjTech_Default && GEngine->ShadowFilterQuality == SFQ_Low
		|| ShadowProjectionTechnique == ShadowProjTech_BPCF_Low) 
	{
		//use the appropriate version based on hardware capablities
	...
	else if (ShadowProjectionTechnique == ShadowProjTech_Default && GEngine->ShadowFilterQuality == SFQ_Medium
...
/** ChooseBPCFBoundShaderState - allows the BPCF implementation to choose which bound shader state should be used
*
* @param ShadowProjectionTechnique - the shadow technique of the light in question, a member of EShadowProjectionTechnique
* @param LowQualityBoundShaderState - the bound shader state corresponding to a low quality BPCF instance
* @param MediumQualityBoundShaderState - the bound shader state corresponding to a medium quality BPCF instance
* @param HighQualityBoundShaderState - the bound shader state corresponding to a high quality BPCF instance
* @return FGlobalBoundShaderStateRHIRef - the bound shader state chosen
*/
extern FGlobalBoundShaderStateRHIRef* ChooseBPCFBoundShaderState(
	BYTE ShadowProjectionTechnique,
	FGlobalBoundShaderStateRHIRef* LowQualityBoundShaderState,
	FGlobalBoundShaderStateRHIRef* MediumQualityBoundShaderState,
	FGlobalBoundShaderStateRHIRef* HighQualityBoundShaderState);
