...
void AActor::execSetPhysics _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
FLOAT AActor::GetGravityZ()
{
	if ( Physics == PHYS_RigidBody )
	{
		if ( PhysicsVolume )
		{
			return WorldInfo->RBPhysicsGravityScaling * PhysicsVolume->GetGravityZ();
		}
...
void AActor::execMoveSmooth _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_VECTOR(Delta);
	P_FINISH;
...
void AActor::FindBase()
{
	...
	GWorld->SingleLineCheck(Hit, this, ColLocation + FVector(0,0,-8), ColLocation, TRACE_AllBlocking|TRACE_Tesselation, GetCylinderExtent());
...
void APawn::performPhysics(FLOAT DeltaSeconds)
{
	...
	// 여기서 OldZ 를 갱신하면 Remote 의 경우 경사면이나 계단에서 eyeHeight 가 떨리게 된다.
...
FVector APawn::CheckForLedges(FVector AccelDir, FVector Delta, FVector GravDir, int &bCheckedFall, int &bMustJump )
{
	...
	FLOAT TestDown = LEDGECHECKTHRESHOLD + CylinderComponent->CollisionHeight + ::Max( MaxStepHeight, CylinderComponent->CollisionRadius + DesiredDist);
	// try a point trace
	GWorld->SingleLineCheck(Hit, this, Destn + TestDown * GravDir, Destn , TRACE_AllBlocking);
...
void APawn::physWalking(FLOAT deltaTime, INT Iterations)
{
	...
	FVector DesiredMove = Velocity + PhysicsVolume->ZoneVelocity * 25.f * deltaTime;
	DesiredMove.Z = 0.f;
	...
	while ( (remainingTime > 0.f) && (Iterations < 8) && (Controller || bRunPhysicsWithNoController) )
	{
		...
		AActor *NewBase = NULL;
		UBOOL bZeroDelta = Delta.IsNearlyZero();
		...
		else
		{
			...
			UBOOL bCheckLedges = Controller ? Controller->WantsLedgeCheck() : 0;
			if( bCheckLedges )
			{
			...
			else
			{
				...
				// fix by deif :)
				// 미묘하게 파고 드는 것을 좀 완화시키기 위해-
				if (Hit.Time < 1.f)
				{
					FCheckResult PenetrationPreventorHit(1.f);
					FCheckResult PenetrationPreventorHit2(1.f);

					GWorld->MoveActor(this, 2*Hit.Normal, Rotation, 0, PenetrationPreventorHit);
					GWorld->MoveActor(this, -2*Hit.Normal, Rotation, 0, PenetrationPreventorHit2);
				}
			}
		...
		if (Base != NULL && !Base->bCollideActors && Base != GWorld->GetWorldInfo())
		{
		...
		else if( Hit.Time< 1.f && (Hit.Actor!=Base || FloorDist>MAXFLOORDIST) )
		{
			...
			// Slope Down 시 떨리는 현상 수정 Code....
			//FLOAT AdjustDistance = Max(0.f,FloorDist-0.5f*(MINFLOORDIST+MAXFLOORDIST));

			//if ( AdjustDistance > 0.0 )
			//{
			//	FVector realNorm = Hit.Normal;
			//	GWorld->MoveActor(this, Down.SafeNormal()*AdjustDistance, Rotation, 0, Hit);
			//	if ( (Hit.Actor != Base) && (Physics == PHYS_Walking) ) {
			//		SetBase(Hit.Actor, Hit.Normal);
			//	}
			//	Hit.Time = 0.f;
		...
		if( !bMustJump && Hit.Time<1.f && Hit.Normal.Z>=WalkableFloorZ )
		{
			if( (Hit.Normal.Z < 0.99f) && ((Hit.Normal.Z * PhysicsVolume->GroundFriction) < 3.3f) ) 
			{
				...
				//@todo steve - test
				FVector Slide(0.f, 0.f, (deltaTime * GetGravityZ()/(2.f * ::Max(0.5f, PhysicsVolume->GroundFriction))) * deltaTime);
		...
		else
		{
			...
			if ( !bMustJump && !bCanJump && (OldBase == NULL || (!OldBase->bCollideActors && OldBase != GWorld->GetWorldInfo())))
			{
			...
			if( !bCheckedFall && 
				...
				!bMustJump && 
				(!bCanJump || 
					(!bCanWalkOffLedges && (bIsWalking || bIsCrouched))) )
			{
			...
			else
			{
				...
				if (Physics == PHYS_Walking)
				{
					...
					else
					{
						...
						if ((GWorld->HasBegunPlay() == TRUE) && (GWorld->GetTimeSeconds() >= 1.f))
						{
							setPhysics(PHYS_Falling); //default behavior if script didn't change physics
						}
					}
...
void APawn::CalcVelocity(FVector &AccelDir, FLOAT DeltaTime, FLOAT MaxSpeed, FLOAT Friction, INT bFluid, INT bBrake, INT bBuoyant)
{
	 ...
	 */
	const FVector	RootMotionTranslation	= bDoRootMotion ? Mesh->RootMotionDelta.Translation : FVector(0.f);	
	 ...
	 */
	const FLOAT MaxAccel = bDoRootMotionAccel ? (RootMotionMag / DeltaTime) : (bDoRootMotionVelocity ? Max(AccelRate * SpeedModifier, (RootMotionMag / DeltaTime)) : AccelRate * SpeedModifier);
	...
	if( !bDoRootMotionAccel && Controller && Controller->bPreciseDestination )
	{
		...
		if( bForceMaxAccel )
		{
			const FVector Dir = (Controller->Destination - Location).SafeNormal();
...
void AActor::processHitWall(FVector HitNormal, AActor *HitActor, UPrimitiveComponent *HitComp)
{
	if (HitActor != NULL)
	{
	...
	eventHitWall(HitNormal, HitActor, HitComp);
}

void AProjectile::processHitWall(FVector HitNormal, AActor *HitActor, UPrimitiveComponent *HitComp)
{
	if ( HitActor )
	{
...
void APawn::processHitWall(FVector HitNormal, AActor *HitActor, UPrimitiveComponent *HitComp)
{
	if ( !HitActor )
		return;
	...
	if (HitPawn != NULL)
	{
		...
		FVector Dir = 1.2f * HitPawn->CylinderComponent->CollisionRadius * Cross.SafeNormal();
...
UBOOL AProjectile::ShrinkCollision(AActor *HitActor, const FVector &StartLocation)
{
	if ( bSwitchToZeroCollision	)
	{
		...
		{
			FCheckResult Hit(1.f);
			FVector Dir = (Location - StartLocation).SafeNormal();

			GWorld->SingleLineCheck(Hit, this, Location + SHORTTRACETESTDIST*Dir, Location, TRACE_AllBlocking|TRACE_StopAtAnyHit|TRACE_Tesselation);
			if ( Hit.Time == 1.f )
				GWorld->SingleLineCheck(Hit, this, Location, Location + SHORTTRACETESTDIST*Dir, TRACE_AllBlocking|TRACE_StopAtAnyHit|TRACE_Tesselation);

			if ( Hit.Time == 1.f )
			{
				bSwitchToZeroCollision = false;
				CylinderComponent->SetSize(0.f,0.f);
...
void AProjectile::GrowCollision()
{
	...
	{
		FCheckResult Hit(1.f);
		UCylinderComponent* CylComp = Cast<UCylinderComponent>(GetClass()->GetDefaultActor()->CollisionComponent);
		if ( CylComp &&
			(ZeroCollider->CollisionComponent
			? ZeroCollider->CollisionComponent->PointCheck(Hit, Location, FVector(CylComp->CollisionRadius,CylComp->CollisionRadius,CylComp->CollisionHeight), bCollideComplex ? TRACE_ComplexCollision : 0)
			: GWorld->BSPPointCheck(Hit, NULL, Location, FVector(CylComp->CollisionRadius,CylComp->CollisionRadius,CylComp->CollisionHeight))) )
		{
			ZeroCollider = NULL;
			bSwitchToZeroCollision = true;
			SetCollisionSize(CylComp->CollisionRadius, CylComp->CollisionHeight);
		}
	}
}
...
void AActor::processLanded(FVector HitNormal, AActor *HitActor, FLOAT remainingTime, INT Iterations)
{
	...
	{
		Velocity = PhysicsVolume->ZoneVelocity + FVector(0.f, 0.f, 2.f * UCONST_ACTORMAXSTEPHEIGHT);
		return;
	}
...
void APawn::processLanded(FVector HitNormal, AActor *HitActor, FLOAT remainingTime, INT Iterations)
{
	...
	GWorld->SingleLineCheck(Hit, this, ColLocation - FVector(0,0,0.2f * CylinderComponent->CollisionHeight + 2.f*LEDGECHECKTHRESHOLD),
		ColLocation, TRACE_Actors|TRACE_World|TRACE_StopAtAnyHit|TRACE_Tesselation, 0.9f * GetCylinderExtent());

	if ( Hit.Time == 1.f ) //Not a valid landing
	{
		FVector Adjusted = Location;
		if (GWorld->FindSpot(1.1f * GetCylinderExtent(), Adjusted, bCollideComplex) && (Adjusted != Location))
		{
...
void APawn::physFalling(FLOAT deltaTime, INT Iterations)
{
	...
	if( !bDoRootMotion && TickAirControl > 0.05f )
	{
		...
		{
			FVector ColLocation = CollisionComponent ? Location + CollisionComponent->Translation : Location;
			GWorld->SingleLineCheck( Hit, this, ColLocation + TestWalk, ColLocation, TRACE_World|TRACE_StopAtAnyHit|TRACE_Tesselation, FVector( CylinderComponent->CollisionRadius, CylinderComponent->CollisionRadius, CylinderComponent->CollisionHeight ) );
			if( Hit.Actor )
			{
				TickAirControl = 0.f;
			}
		...
		else if ( Hit.Time < 1.f )
		{
			...
			else
			{
				FVector HitNormal = Hit.Normal;
				{
					...
					FVector Old = Location;

					GWorld->MoveActor(this, HitNormal, Rotation, MOVE_TraceHitMaterial, Hit);

					FVector Delta = Location - Old;
...
void AActor::physFalling(FLOAT deltaTime, INT Iterations)
{
	...
	while ( (remainingTime > 0.f) && (Iterations < 8) )
	{
		...
		{
			Adjusted = (Velocity + PhysicsVolume->ZoneVelocity) * timeTick * (1.f - Hit.Time);
			GWorld->MoveActor(this, Adjusted, Rotation, 0, Hit);
			if ( bDeleteMe )
				return;
		}
...
void APawn::physFlying(FLOAT deltaTime, INT Iterations)
{
	...
	FVector OldLocation = Location;
	const FVector Adjusted = (Velocity + PhysicsVolume->ZoneVelocity) * deltaTime;
...
INT AVolume::Encompasses(FVector point)
{
	...
	if (Brush != NULL)
	{
		...
		return !Brush->PointCheck(Hit,this,	NULL, point, FVector(0.f,0.f,0.f));
	}
	else
	{
...
void APawn::physLadder(FLOAT deltaTime, INT Iterations)
{
	...
	{
		FCheckResult Hit(1.f);
		UBOOL bRotClimbUp	= ((Controller->Rotation.Vector() | (OnLadder->ClimbDir + OnLadder->LookDir)) > 0.f);
		UBOOL bAccelClimbUp = ((Acceleration | (OnLadder->ClimbDir + OnLadder->LookDir)) > 0.f);

		UBOOL bClimbUp = bRotClimbUp;
		if ( bAccelClimbUp == false )	bClimbUp = !bClimbUp;
		...
		else if ( (Hit.Time < 1.f) && Hit.Actor->bWorldGeometry )
		{
			...
			// 2007/2/7 Location 이 엉뚱한 값을 가르켜서 Velocity 가 엄청커지는 경우가 생김... 로켓 사다리 버그 fix 를 위해서 주석 처리...
...
#endif
{
	GWorld->SingleLineCheck(Hit, 0, Location - MaxStepHeight * Dir, Location, TRACE_World|TRACE_Tesselation, GetCylinderExtent());
	if (Hit.Time < 1.f)
	{
		SetBase(Hit.Actor, Hit.Normal);
		return 1;
...
void APawn::physSpider(FLOAT deltaTime, INT Iterations)
{
	...
	while ( (remainingTime > 0.f) && (Iterations < 8) )
	{
		...
		if( bZeroMove )
		{
			...
			GWorld->SingleLineCheck( Hit, this, Foot - 20 * Floor, Foot, TRACE_World|TRACE_Tesselation );
		...
		if( !bZeroMove )
		{
			//drop to floor
			FVector ColLocation = CollisionComponent ? Location + CollisionComponent->Translation : Location;
			GWorld->SingleLineCheck( Hit, this, ColLocation - Floor * (MaxStepHeight + MAXSTEPHEIGHTFUDGE), ColLocation, TRACE_AllBlocking|TRACE_Tesselation, GetCylinderExtent() );
			if ( Hit.Time == 1.f )
			{
...
void AActor::physInterpolating(FLOAT DeltaTime)
{
	...
	if( FindInterpMoveTrack(&MoveTrack, &MoveInst, &Seq) )
	{
		...
		FLOAT CurTime = Seq->Position;

		if(MoveTrack->bDisableTrack)
		{
			CurTime = 0.0f;	
		}

		MoveTrack->GetLocationAtTime(MoveInst, CurTime, NewPos, NewRot);

		FVector OldLocation = Location;
		FRotator OldRotation = Rotation;

		// @fixme laurent, hack for milestone. This needs to be removed and made into a separate physics mode for controlled characters.
		// see if we're controlling a pawn
		APawn *P = GetAPawn();

		// If the mesh wants to use the root motion for velocity
		if( P && P->Mesh && P->Mesh->RootMotionMode == RMM_Velocity && P->Mesh->PreviousRMM != RMM_Ignore )
		{
			FVector DeltaMove	= NewPos - Location;
			FLOAT	Dist		= DeltaMove.Size();

			if( Dist > KINDA_SMALL_NUMBER && Dist < 200.f )
			{
				// limit velocity by root motion magnitude
				Dist	= ::Min(Dist, P->Mesh->RootMotionDelta.Translation.Size());
				NewPos	= Location + DeltaMove.SafeNormal() * Dist;
				//debugf(TEXT("Dist: %f"), Dist);

				// if we're using root motion, then clear accumulated root motion
				// See USkeletalMeshComponent::UpdateSkelPose() for details.
				P->Mesh->RootMotionDelta.Translation = FVector(0.f);
			}
		}
		...
		bMovingNow = ((OldRotation != Rotation) || (OldLocation != Location));
