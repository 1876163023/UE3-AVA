...
IMPLEMENT_CLASS(USeqAct_AudioStream);
...
static void AddItemAwareOfArrayDim(TArray<FName>& OutNames, FName Org, UProperty* Property)
{
	if (Property->ArrayDim == 1)
	{
		OutNames.AddItem( Org );
	}
	else
	{
		for (INT i=0; i<Property->ArrayDim; ++i)
		{
			OutNames.AddItem( *FString::Printf(TEXT("%s[%d]"), *Org.ToString(), i ) );
		}		
	}
}

template <class T>
void GetInterpPropertyNames(UClass* Class, TArray<FName>& OutNames)
{
	// first search for any float properties in this actor
	for (TFieldIterator<UFloatProperty> It(Class); It; ++It)
	{
		if (It->PropertyFlags & CPF_Interp)
		{
			// add the property name
			AddItemAwareOfArrayDim( OutNames, *It->GetName(), *It );
		}
	}
	...
	for(TMap<FName,UComponent*>::TIterator It(Class->ComponentNameToDefaultObjectMap);It;++It)
	{
		...
		for (TFieldIterator<T> FieldIt(ComponentClass); FieldIt; ++FieldIt)
		{
			if (FieldIt->PropertyFlags & CPF_Interp)
			{
				// add the property name, mangled to note the component
				AddItemAwareOfArrayDim( OutNames, FName(*FString::Printf(TEXT("%s.%s"),*ComponentName.ToString(), *FieldIt->GetName())), *FieldIt );
			}
		}
	}

	// Iterate over structs marked as 'interp' looking for float properties within them.
	for (TFieldIterator<UStructProperty> It(Class); It; ++It)
	{
		if(It->PropertyFlags & CPF_Interp)
		{
			for(TFieldIterator<T> FIt(It->Struct); FIt; ++FIt)
			{
				if (FIt->PropertyFlags & CPF_Interp)
				{
					// add the property name, plus the struct name
					AddItemAwareOfArrayDim( OutNames, FName(*FString::Printf(TEXT("%s.%s"), *It->GetName(), *FIt->GetName())), *FIt );										
				}
			}
		}
	}
...
void GetInterpFloatPropertyNames(UClass* Class, TArray<FName>& OutNames)
{
	GetInterpPropertyNames<UFloatProperty>(Class,OutNames);	
}

void GetInterpStructPropertyNames(UClass* Class, TArray<FName> &OutNames, FName StructName)
{
	// first search for any vector properties in this actor
	for (TFieldIterator<UStructProperty, CLASS_IsAUStructProperty> It(Class); It; ++It)
	{
		if ((It->PropertyFlags & CPF_Interp) && It->Struct->GetFName() == StructName)
		{
			// add the property name
			AddItemAwareOfArrayDim( OutNames, *It->GetName(), *It );
		}
	...
	for(TMap<FName,UComponent*>::TIterator It(Class->ComponentNameToDefaultObjectMap);It;++It)
	{
		...
		{
			if ((FieldIt->PropertyFlags & CPF_Interp) && FieldIt->Struct->GetFName() == StructName)
			{
				// add the property name, mangled to note the component
				AddItemAwareOfArrayDim( OutNames, FName(*FString::Printf(TEXT("%s.%s"),*ComponentName.ToString(), *FieldIt->GetName())), *FieldIt );
			}
	...
	for (TFieldIterator<UStructProperty> It(Class); It; ++It)
	{
		if(It->PropertyFlags & CPF_Interp)
		{
			...
			{
				if ((VIt->PropertyFlags & CPF_Interp) && VIt->Struct->GetFName() == StructName)
				{
					// add the property name, plus the struct name
					AddItemAwareOfArrayDim( OutNames, FName(*FString::Printf(TEXT("%s.%s"), *It->GetName(), *VIt->GetName())), *VIt );										
				}
			}
		}
	}
...
void GetInterpVectorPropertyNames(UClass* Class, TArray<FName> &OutNames)
{
	GetInterpStructPropertyNames(Class,OutNames,NAME_Vector);
}

void GetInterpRotatorPropertyNames(UClass* Class, TArray<FName> &OutNames)
{
	GetInterpStructPropertyNames(Class,OutNames,NAME_Rotator);
}
...
{
	::GetInterpFloatPropertyNames(GetClass(), OutNames);
}

void UUIObject::GetInterpFloatPropertyNames(TArray<FName> &OutNames)
{
	::GetInterpFloatPropertyNames(GetClass(), OutNames);
}
...
{
	::GetInterpVectorPropertyNames(GetClass(), OutNames);
}

void UUIObject::GetInterpVectorPropertyNames(TArray<FName> &OutNames)
{
	::GetInterpVectorPropertyNames(GetClass(), OutNames);
}

/** 
 *	Get the names of any rotator properties of this UIObject which are marked as 'interp'.
 *	Will also look in components of this Actor, and makes the name in the form 'componentname.propertyname'.
 */
void UUIObject::GetInterpRotatorPropertyNames(TArray<FName>& OutNames)
{
	::GetInterpRotatorPropertyNames(GetClass(), OutNames);
}


// Check property is either a float or vector
static UBOOL PropertyIsFloatOrStruct(UProperty* Prop)
{
 ...
 */
static UObject* FindObjectAndPropOffset(INT& OutPropOffset, UObject* InObject, FName InPropName)
{
	FString PropName, ArrayIndexString;
	INT ArrayIndex = 0;
	if(InPropName.ToString().Split(TEXT("["), &PropName, &ArrayIndexString ))
	{
		ArrayIndex = appAtoi( *(ArrayIndexString.Left( ArrayIndexString.Len() - 1 )) );
	}
	else
	{
		PropName = InPropName.ToString();
	}

	FString CompString, PropString;	
	if(PropName.Split(TEXT("."), &CompString, &PropString))
	{
		// STRUCT
		// First look for a struct with first part of name.
		UStructProperty* StructProp = FindField<UStructProperty>( InObject->GetClass(), *CompString );
		if(StructProp)
		{
			// Look 
			UProperty* Prop = FindField<UProperty>( StructProp->Struct, *PropString );
			if(Prop && PropertyIsFloatOrStruct(Prop))
			{
				ArrayIndex = Clamp( ArrayIndex, 0, Prop->ArrayDim-1 );
				OutPropOffset = StructProp->Offset + Prop->Offset + Prop->ElementSize * ArrayIndex;
				return InObject;
			}
			else
			{
		...
		if(!OutObject)
		{
			...
			InObject->CollectComponents(Components,FALSE);
		...
		if(OutObject)
		{
			...
			{
				ArrayIndex = Clamp( ArrayIndex, 0, Prop->ArrayDim-1 );
				OutPropOffset = Prop->Offset + Prop->ElementSize * ArrayIndex;
				return OutObject;
			}
	...
	{
		UProperty* Prop = FindField<UProperty>( InObject->GetClass(), FName( *PropName ) );
		if(Prop && PropertyIsFloatOrStruct(Prop))
		{
			ArrayIndex = Clamp( ArrayIndex, 0, Prop->ArrayDim-1 );
			OutPropOffset = Prop->Offset + Prop->ElementSize * ArrayIndex;
...
* =====================================================
*/
FLOAT* GetInterpFloatPropertyRef(UObject* InObject, FName InPropName)
{
	// get the property name and the Object its in. handles property being in a component.
	INT PropOffset;
	UObject* PropObject = FindObjectAndPropOffset(PropOffset, InObject, InPropName);
	if (PropObject != NULL)
	{
...
{	
	return ::GetInterpFloatPropertyRef(this, InPropName);
}

FLOAT* UUIObject::GetInterpFloatPropertyRef(FName InPropName)
{	
	return ::GetInterpFloatPropertyRef(this, InPropName);
}
...
* =====================================================
*/
FVector* GetInterpVectorPropertyRef(UObject* InObject, FName InPropName)
{
	// get the property name and the Object its in. handles property being in a component.
	INT PropOffset;
	UObject* PropObject = FindObjectAndPropOffset(PropOffset, InObject, InPropName);
...
{
	return ::GetInterpVectorPropertyRef(this, InPropName);
}

FVector* UUIObject::GetInterpVectorPropertyRef(FName InPropName)
{
	return ::GetInterpVectorPropertyRef(this, InPropName);
}
...
FRotator* GetInterpRotatorPropertyRef(UObject* InObject, FName InPropName)
{
	// get the property name and the Object its in. handles property being in a component.
	INT PropOffset;
	UObject* PropObject = FindObjectAndPropOffset(PropOffset, InObject, InPropName);
	if (PropObject != NULL)
	{
		return ((FRotator*)(((BYTE*)PropObject) + PropOffset));
	}
	return NULL;
}

FRotator* UUIObject::GetInterpRotatorPropertyRef(FName inPropName)
{
	return ::GetInterpRotatorPropertyRef(this, inPropName);
}
...
void USequenceOp::CheckForErrors()
{
	...
	for (INT Idx = 0; Idx < VariableLinks.Num(); Idx++)
	{
		...
		if (VariableLinks(Idx).PropertyName == NAME_None &&
			Idx < DefaultOp->VariableLinks.Num() &&
			VariableLinks(Idx).LinkDesc == DefaultOp->VariableLinks(Idx).LinkDesc &&
			DefaultOp->VariableLinks(Idx).PropertyName != NAME_None )
		{
			Modify();
			VariableLinks(Idx).PropertyName = DefaultOp->VariableLinks(Idx).PropertyName;
		}
	}
...
void USequenceOp::execGetObjectVars _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USequenceOp::execGetBoolVars _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USequenceOp::execLinkedVariables _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USequence::CheckForErrors()
{
	...
	if (bOrphaned ||
		(ParentSequence != NULL &&
		 !ParentSequence->SequenceObjects.ContainsItem(this)))
	{
 ...
 */
UBOOL USequence::ReferencesObject(const UObject* InObject) const
{
...
void USequence::execFindSeqObjectsByClass _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
 ...
 */
void USequenceEvent::ActivateEvent(AActor *InOriginator, AActor *InInstigator, TArray<INT> *ActivateIndices, UBOOL bPushTop/*=FALSE*/)
{
 ...
 */
void USequenceEvent::execCheckActivate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USeqEvent_Touch::execCheckTouchActivate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_ACTOR(InOriginator);
	P_GET_ACTOR(InInstigator);
	P_GET_UBOOL_OPTX(bTest,FALSE);
...
UBOOL USeqEvent_Touch::CheckUnTouchActivate(AActor *InOriginator, AActor *InInstigator, UBOOL bTest)
{
	...
	ActivationTime = 0.f;
	UBOOL bActivated = CheckActivate(InOriginator,InInstigator,bTest) && TouchedList.FindItem(InInstigator,TouchIdx);
...
void USeqVar_Bool::PopulateValue(USequenceOp *Op, UProperty *Property, FSeqVarLink &VarLink)
{
	if (Op != NULL && Property != NULL)
	{
		...
		{
			// bValue == 2 이거나 bValue == 4와 같은 경우가 발생
			// bValue가 위와같이 1이 아니면 (bValue == TRUE)가 FALSE인 경우가 생긴다.
...
UBOOL USeqVar_Vector::SupportsProperty(UProperty *Property)
{
	...
	return (StructProperty->Struct->GetFName() == NAME_Vector);
}
...
void USeqVar_Vector::PublishValue(USequenceOp *Op, UProperty *Property, FSeqVarLink &VarLink)
{
	if (Op != NULL && Property != NULL)
	{
		...
		if (StructProperty && StructProperty->Struct->GetFName() == NAME_Vector)
		{
			// first calculate the value
			FVector Value(0.f);
		...
		{
			StructProperty = Cast<UStructProperty>( ((UArrayProperty*)Property)->Inner );
			...
			if (StructProperty && StructProperty->Struct->GetFName() == NAME_Vector)
			{
				...
				DestArray->Empty(ElementSize, DEFAULT_ALIGNMENT, VectorVars.Num());
				DestArray->AddZeroed(VectorVars.Num(), ElementSize, DEFAULT_ALIGNMENT);
				for (INT Idx = 0; Idx < VectorVars.Num(); Idx++)
				{
	...
	if (Op != NULL && Property != NULL)
	{
		...
		if ( Property->IsA(UNameProperty::StaticClass()))
		{
			...
			*(FName*)((BYTE*)Op + Property->Offset) = NameValue;
			...
			((UArrayProperty*)Property)->Inner->IsA(UNameProperty::StaticClass()))
		{
			// grab the array
			UArrayProperty *ArrayProp = (UArrayProperty*)Property;
			INT ElementSize = ArrayProp->Inner->ElementSize;
			FArray *DestArray = (FArray*)((BYTE*)Op + ArrayProp->Offset);
			// resize it to fit the variable count
			DestArray->Empty(ElementSize, DEFAULT_ALIGNMENT, NameVars.Num());
			DestArray->AddZeroed(NameVars.Num(), ElementSize, DEFAULT_ALIGNMENT);
			for (INT Idx = 0; Idx < NameVars.Num(); Idx++)
			{
				FName* NameVar = NameVars(Idx);
				if ( NameVar != NULL )
				{
					// assign to the array entry
					*(FName*)((BYTE*)DestArray->GetData() + Idx * ElementSize) = *NameVar;
				}
			}
		}
	}
...
void USeqVar_Name::PopulateValue(USequenceOp *Op, UProperty *Property, FSeqVarLink &VarLink)
{
	if (Op != NULL && Property != NULL)
	{
				...
				((UArrayProperty*)Property)->Inner->IsA(UNameProperty::StaticClass()))
		{
			...
			FArray *SrcArray = (FArray*)((BYTE*)Op + ArrayProp->Offset);
			// write out as many entries as are attached
			for (INT Idx = 0; Idx < NameVars.Num() && Idx < SrcArray->Num(); Idx++)
			{
				*(NameVars(Idx)) = *(FName*)((BYTE*)SrcArray->GetData() + Idx * ElementSize);
			}
		}
	}
}
...
void USeqVar_Named::UpdateStatus()
{
	...
	while (Seq != NULL)
	{
		...
		if (Vars.Num() > 0)
		{
			...
			// {{ 20070503 dEAthcURe|HM Hm??? 상속관계 때문에 기본타입과 매칭이 안됨
			//debugf(TEXT("Var %s %s ExpectedType %s %s"), *Var->GetName(), *Var->GetClass()->GetName(), *ExpectedType->GetName(), *ExpectedType->GetName());
			if(!appStricmp(*Var->GetClass()->GetName(), TEXT("avaSeqVar_Hm")) && 
				!appStricmp(*ExpectedType->GetName(), TEXT("SeqVar_Bool"))) {
				bStatusIsOk = TRUE;
			}
			if(!appStricmp(*Var->GetClass()->GetName(), TEXT("avaSeqVar_HmInt")) && 
				!appStricmp(*ExpectedType->GetName(), TEXT("SeqVar_Int"))) {
				bStatusIsOk = TRUE;
			}
			// }} 20070503 dEAthcURe|HM Hm??? 상속관계 때문에 기본타입과 매칭이 안됨
			return;
...
void USeqAct_AudioStream::Activated()
{
	return;
	...
	if (InputLinks(0).bHasImpulse)
	{
		...
		if (AudioDevice)
		{
			...
			{
			case ASO_Play :
				AudioDevice->PlayStream( Stream );
				break;

			case ASO_Stop :
				AudioDevice->StopStream( Stream );
				break;
			}
		}
	}
			...
			if (RemainingDelay <= 0.f)
			{
				...
				for (INT Count = 0; Count < SpawnPoints.Num() && NewSpawn == NULL; Count++)
				{
					...
					if( Point )
					{
						...
						NewSpawn = Factory->CreateActor(&(Point->Location),bOverrideRotation ? &Rotation : &(Point->Rotation), this);
...
void USeqCond_SwitchClass::Activated()
{
	Super::Activated();
	// grab all associated Objects
	...
	for( INT ObjIdx = 0; ObjIdx < ObjList.Num(); ObjIdx++ )
	{
		...
		for( INT OutIdx = 0; OutIdx < ClassArray.Num() && !bExit; OutIdx++ )
		{
			if (ClassArray(OutIdx).ClassName == NAME_Default)
			{
				...
				OutputLinks(OutIdx).ActivateOutputLink();
				bExit = TRUE;
				break;
			}
			else
...
void USeqCond_SwitchClass::UpdateDynamicLinks()
{
	Super::UpdateDynamicLinks();

	// If there are too many output Links
	if( OutputLinks.Num() > ClassArray.Num() )
	{
		...
		{
			INT j = -1;
			for( j = ClassArray.Num() - 1; j >= 0; j-- )
			{
				if( appStricmp( *OutputLinks(i).LinkDesc, *ClassArray(j).ClassName.ToString() ) == 0 )
				{
					break;
				}
			}
	...
	while( OutputLinks.Num() < ClassArray.Num() )
	{
	...
	if( appStricmp( *OutputLinks(Num).LinkDesc, TEXT("Default") ) != 0 )
	{
		OutputLinks.AddZeroed();
		ClassArray.AddZeroed();
	}

	// Ensure last entry is always default
	Num = OutputLinks.Num() - 1;
	OutputLinks(Num).LinkDesc = TEXT("Default");
	ClassArray(Num).ClassName = FName( TEXT("Default"), FNAME_Find );
	ClassArray(Num).bFallThru = 0;

	// Match up descriptions with each class entry
	for( INT i = 0; i < OutputLinks.Num(); i++ )
	{
		OutputLinks(i).LinkDesc = ClassArray(i).ClassName.ToString();
	}
}	
...
static void BuildSoundTargets(TArray<UObject**> ObjVars, TArray<UObject*> &Targets)
{
	for (INT Idx = 0; Idx < ObjVars.Num(); Idx++)
	{
...
void USeqAct_PlaySound::Activated()
{
	...
	if (PlaySound != NULL)
	{
		...
		if (InputLinks(0).bHasImpulse)
		{
			...
			if (wave != NULL)
			{
				SoundDuration = wave->Duration + ExtraDelay;
			}
			else
			{
				SoundDuration = 0.f;
...
void USeqAct_LevelStreamingBase::ActivateLevel( ULevelStreaming* LevelStreamingObject )
{
	if( LevelStreamingObject != NULL )
	{
		...
		if( InputLinks(0).bHasImpulse )
		{
			debugf( NAME_DevStreaming, TEXT("Streaming in level %s (%s)..."),*LevelStreamingObject->GetName(),*LevelStreamingObject->PackageName.ToString());
		...
		{
			debugf( NAME_DevStreaming, TEXT("Streaming out level %s (%s)..."),*LevelStreamingObject->GetName(),*LevelStreamingObject->PackageName.ToString());
...
void USeqAct_LevelVisibility::Activated()
{
	...
	if( LevelStreamingObject != NULL )
	{
		...
		if( InputLinks(0).bHasImpulse )
		{
			debugf( NAME_DevStreaming, TEXT("Making level %s (%s) visible."), *LevelStreamingObject->GetName(), *LevelStreamingObject->PackageName.ToString() );
		...
		{
			debugf( NAME_DevStreaming, TEXT("Hiding level %s (%s)."), *LevelStreamingObject->GetName(), *LevelStreamingObject->PackageName.ToString() );
			LevelStreamingObject->bShouldBeVisible	= FALSE;
		}
...
void USeqAct_Nuke::Activated()
{
	for (INT Idx = 0; Idx < GWorld->Levels.Num(); Idx++)
	{
		GWorld->Levels(Idx)->iFirstDynamicActor = 0;
	}
	TArray<UObject**> ObjVars;
	GetObjectVars(ObjVars,TEXT("Target"));
	for (INT Idx = 0; Idx < ObjVars.Num(); Idx++)
	...
	for (INT Idx = 0; Idx < GWorld->Levels.Num(); Idx++)
	{
		GWorld->Levels(Idx)->SortActorList();
	}
}
...
void USeqAct_ForceMaterialMipsResident::Activated()
{
	...
	{
		UMaterialInstance* Mat = ForceMaterials(i);
		if(Mat)
		{	
