/*=============================================================================
	...
	Copyright 2000-2006 Epic Games, Inc. All Rights Reserved.
=============================================================================*/
...
// operator for MemCount only, so it only serializes the array that needs to be counted
...
void USkeletalMesh::PreSave()
{
	...
	// merge 중.... PerPolyKDOPs는 이번에 Merge 대상이 아니어서 제외.. 2007. 6. 21 changmin
...
*/
void ASkeletalMeshActor::PreviewSetAnimPosition(FName SlotName, INT ChannelIndex, FName InAnimSeqName, FLOAT InPosition)
{
...
*/
void ASkeletalMeshActorMAT::MAT_SetAnimPosition(FName SlotName, INT ChannelIndex, FName InAnimSeqName, FLOAT InPosition, UBOOL bFireNotifies)
{
	...
	for(INT i=0; i<SlotNodes.Num(); i++)
	{
		...
		{	
			SlotNode->MAT_SetAnimPosition(ChannelIndex, InAnimSeqName, InPosition, bFireNotifies);
		}
	}
}
...
* PreviewSetAnimPosition 
*/
void ASkeletalMeshActorMAT::PreviewSetAnimPosition(FName SlotName, INT ChannelIndex, FName InAnimSeqName, FLOAT InPosition)
{
	MAT_SetAnimPosition(SlotName, ChannelIndex, InAnimSeqName, InPosition, FALSE);
...
UBOOL FSkeletalMeshSceneProxy::IsCacheable() const
{
	return TRUE;
}

UBOOL FSkeletalMeshSceneProxy::IsStillValid( const UPrimitiveComponent* InComponent ) const
{
	const USkeletalMeshComponent* Component = static_cast<const USkeletalMeshComponent*>(InComponent);
	...
	return 
		...
		bForceNotToDraw == Component->bForceNotToDraw;
}
		...
		,	bForceNotToDraw(Component->bForceNotToDraw)
{
	...
	for(INT i=0; i<SkeletalMesh->LODModels.Num(); i++)
	{
		...
		for(INT SectionIndex = 0;SectionIndex < LODModel.Sections.Num();SectionIndex++)
		{
			...
			FSectionElementInfo ElementInfo;
			...
			UMaterialInstance* Material = Component->GetMaterial(UseMaterialIndex);
			ElementInfo.Material = (Material && Material->UseWithSkeletalMesh()) ? Material : GEngine->DefaultMaterial;

			LODSection.SectionElements.AddItem(ElementInfo);
		}
	}
 ...
 */
void FSkeletalMeshSceneProxy::DrawDynamicElements(FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex)
{
	if( !MeshObject )
	{
		return;
	...
	if( PrimitiveDPG == DPGIndex )
	{
		...
		else
		{
			...
			for(INT SectionIndex = 0;SectionIndex < LODModel.Sections.Num();SectionIndex++)
			{
				...
				Mesh.MaterialInstance = SectionElementInfo.Material->GetInstanceInterface(bSelected);
				Mesh.LCI = NULL;
												   ...
												   UBOOL bDrawingDynamicLights)
{
	...
	if( PrimitiveDPG == DepthPriorityGroup )
	{
			...
			for ( INT DecalIndex = 0 ; DecalIndex < DPGDecals.Num() ; ++DecalIndex )
			{
				...
				if ( TRUE )//DecalState.QuadToClippedScreenSpaceAABB( View, MinCorner, MaxCorner ) )
				{
					...
					for(INT SectionIndex = 0;SectionIndex < LODModel.Sections.Num();SectionIndex++)
					{
						...
						Mesh.DynamicVertexData = NULL;
						Mesh.MaterialInstance = DecalState.DecalMaterial->GetInstanceInterface(FALSE);
						Mesh.LCI = NULL;
						GetWorldMatrices( View, Mesh.LocalToWorld, Mesh.WorldToLocal );						
						Mesh.FirstIndex = Section.BaseIndex;
						Mesh.NumPrimitives = Section.NumTriangles;
