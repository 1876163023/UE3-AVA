...
{
#if PS3
	// The Cooker stores WZYX but we want XYZW (Intel byte order).
	Ar << N.Vector.W << N.Vector.Z << N.Vector.Y << N.Vector.X;
#else
	Ar << N.Vector.Packed;
#endif
	return Ar;
...
FPixelFormatInfo	GPixelFormats[] =
{
	...
	{ TEXT("DepthStencil"),		1,			1,			1,			0,			1,				0,				0,				1			},	//	PF_DepthStencil
	{ TEXT("ShadowDepth"),		1,			1,			1,			0,			1,				0,				0,				0			},	//	PF_ShadowDepth
...
/** A map from texture group indices to group names. */
FString TextureGroupNames[]=
{
	TEXT("World"),
	TEXT("WorldNormalMap"),
	TEXT("Character"),
	TEXT("CharacterNormalMap"),
	TEXT("Weapon"),
	TEXT("WeaponNormalMap"),
	TEXT("Vehicle"),
	TEXT("VehicleNormalMap"),
	TEXT("Effects"),
	TEXT("Skybox"),
	TEXT("UI"),
	TEXT("LightAndShadowMap"),
	TEXT("RenderTarget"),
};
//
...
void DrawBox(FPrimitiveDrawInterface* PDI,const FMatrix& BoxToWorld,const FVector& Radii,const FMaterialInstance* MaterialInstance,BYTE DepthPriorityGroup)
{
	...
	MeshBuilder.Draw(PDI,FScaleMatrix(Radii) * BoxToWorld,MaterialInstance,DepthPriorityGroup);
}



void DrawSphere(FPrimitiveDrawInterface* PDI,const FVector& Center,const FVector& Radii,INT NumSides,INT NumRings,const FMaterialInstance* MaterialInstance,BYTE DepthPriority)
{
	...
	FDynamicMeshBuilder MeshBuilder;
	{
		...
		for(INT i=0; i<NumRings+1; i++)
		{
			...
			ArcVert->TangentY = FVector(0.0f,-ArcVert->Position.Z,ArcVert->Position.Y);
	...
	MeshBuilder.Draw(PDI, FScaleMatrix( Radii ) * FTranslationMatrix( Center ), MaterialInstance, DepthPriority);
}

void DrawCone(FPrimitiveDrawInterface* PDI,const FMatrix& ConeToWorld, FLOAT Angle1, FLOAT Angle2, INT NumSides, UBOOL bDrawSideLines, const FColor& SideLineColor, const FMaterialInstance* MaterialInstance, BYTE DepthPriority)
{
	...
	MeshBuilder.Draw(PDI, ConeToWorld, MaterialInstance, DepthPriority);
...
UBOOL IsRichView(const FSceneView* View)
{
	...
	const EShowFlags RichShowFlags2 = SHOW_LeafColoration | SHOW_SolidBspOnly;
	if( View->Family->ShowFlags & RichShowFlags2 )
	{
		return TRUE;
	}
	//>@ ava
...
//@HACK deif
UBOOL GDrawingShadowDepth = FALSE;
	...
	)
{
	...
	//<@ ava speicifc ; 206. 11. 30 changmin
	EShowFlags ShowFlags = PDI->View->Family->ShowFlags;
	if( PrimitiveInfo == NULL )
	{
		ShowFlags &= ~(SHOW_LightComplexity | SHOW_LightInfluences);
	}
	//>@ ava
	...
	if(ShowFlags & SHOW_Wireframe)
	{
		...
		const FColoredMaterialInstance WireframeMaterialInstance(
		...
		ModifiedMesh.MaterialInstance = &WireframeMaterialInstance;
	...
	else if(ShowFlags & SHOW_LightComplexity)
	{
		...
		FColoredMaterialInstance LightComplexityMaterialInstance(
			GEngine->SolidColorMaterial->GetInstanceInterface(FALSE),
			Color );

		// Draw the mesh colored by light complexity.
		FMeshElement ModifiedMesh = Mesh;
		ModifiedMesh.MaterialInstance = &LightComplexityMaterialInstance;
		PDI->DrawMesh(ModifiedMesh);
	}
	else if(!(ShowFlags & SHOW_Materials))
	{
		...
		FMeshElement ModifiedMesh = Mesh;
		ModifiedMesh.MaterialInstance = GEngine->LightingOnlyMaterial->GetInstanceInterface(bSelected);
		PDI->DrawMesh(ModifiedMesh);
	}
	else
	{
		...
		if(ShowFlags & SHOW_PropertyColoration)
		{
			...
			const UMaterial* PropertyColorationMaterial = (ShowFlags & SHOW_ViewMode_Lit) ? GEngine->PropertyColorationLitMaterial : GEngine->PropertyColorationUnlitMaterial;

			const FColoredMaterialInstance PropertyColorationMaterialInstance(
				PropertyColorationMaterial->GetInstanceInterface(FALSE),
				GetSelectionColor(PropertyColor,bSelected)
				);
			FMeshElement ModifiedMesh = Mesh;
			ModifiedMesh.MaterialInstance = &PropertyColorationMaterialInstance;
			PDI->DrawMesh(ModifiedMesh);
		}
		else if ( ShowFlags & SHOW_LevelColoration )
		{
			...
			const FColoredMaterialInstance LevelColorationMaterialInstance(
				LevelColorationMaterial->GetInstanceInterface(FALSE),
				GetSelectionColor(LevelColor,bSelected)
				);
			FMeshElement ModifiedMesh = Mesh;
			ModifiedMesh.MaterialInstance = &LevelColorationMaterialInstance;
			PDI->DrawMesh(ModifiedMesh);
		}
				...
				&&	PrimitiveInfo->Component->IsA(UModelComponent::StaticClass()) )
		{
			...
			const FColoredMaterialInstance BSPSplitMaterialInstance(
				BSPSplitMaterial->GetInstanceInterface(FALSE),
				GetSelectionColor(BSPSplitColor,bSelected)
				);
			FMeshElement ModifiedMesh = Mesh;
			ModifiedMesh.MaterialInstance = &BSPSplitMaterialInstance;
			PDI->DrawMesh(ModifiedMesh);
		}
		else if( ShowFlags & SHOW_Collision_Any)
		{
			...
			FMeshElement ModifiedMesh = Mesh;
			ModifiedMesh.MaterialInstance = &CollisionMaterialInstance;
			PDI->DrawMesh(ModifiedMesh);
		}
		else 
		{
			//@HACK deif :)
			if (GDrawingShadowDepth && Mesh.MaterialInstance && Mesh.MaterialInstance->GetMaterial() && Mesh.MaterialInstance->GetMaterial()->GetBlendMode() != BLEND_Opaque)
				return;
