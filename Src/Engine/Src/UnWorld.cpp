...
#include "hostMigration.h" // 20071016 dEAthcURe|HM
/*-----------------------------------------------------------------------------
...
:	URL(InURL)
,	DecalManager(NULL)
//<@ ava specific ; 2006 / 9 / 5
,   bDrawEmissiveDynamicPrimitives(TRUE)
//>@ ava
#if !FINAL_RELEASE
...
void UWorld::StaticConstructor()
{
	...
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, PersistentLevel ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, SaveGameSummary ) );
	Class->EmitObjectArrayReference( STRUCT_OFFSET( UWorld, Levels ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, CurrentLevel ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, CurrentLevelPendingVisibility ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, NetDriver ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, DemoRecDriver ) );
	Class->EmitObjectArrayReference( STRUCT_OFFSET( UWorld, NewlySpawned ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, LineBatcher ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, PersistentLineBatcher ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, SampleDrawer ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWorld, DecalManager ) );
}
...
void UWorld::PostLoad()
{
	...
	// 이전버전의 data는 structural model 이 persitent level에 없다.
	if(PersistentLevel->StructuralModel == NULL )
	{
		PersistentLevel->StructuralModel = new (PersistentLevel) UModel( NULL, 1 );
	}
	//>@ ava
}
...
UBOOL UWorld::SaveWorld( const FString& Filename, UBOOL bForceGarbageCollection, UBOOL bAutosaving, UBOOL bPIESaving )
{
	...
	GhmId = -1; // 20070731 dEAthcURe|HM

	UBOOL bWasSuccessful = false;

	// Check for spaces in the filename as the engine doesn't correctly handle loading/ saving them.
	if( Filename.InStr( TEXT(" ") ) == INDEX_NONE )
	{
		// pause propagation
	...
	{
		appMsgf( AMT_OK, *LocalizeUnrealEd("Error_FilenameContainsSpaces") );
	}
...
void UWorld::Init()
{
	...
	if( GIsGame )
	{
		bDrawEmissiveDynamicPrimitives = FALSE;
	}
	//>@ ava
...
void UWorld::CreateNew()
{
	...
	//<@ ava specific ; 2007. 2. 1 changmin
	GWorld->PersistentLevel->StructuralModel = new (GWorld->PersistentLevel) UModel( NULL, 1 );
	//>@ ava
	...
	//<@ ava specific ; 2007. 2. 1 changmin
	// transactional 할 필요가 없어보입니다..
	//GWorld->PersistentLevel->StructuralModel->SetFlags( RF_Transactional );
	//>@ ava
...
void UWorld::RemoveActor(AActor* Actor, UBOOL bShouldModifyLevel)
{
	...
	// {{ 20070821 dEAthcURe|HM level에 없는놈 제거
	#ifdef EnableHostMigration
	#ifdef EnableHmFastLoading	
	if(GEnableHmFastLoading && !bSuccessfulRemoval) {
		for(int lpp=0;lpp<UObject::GObjObjects.Num();lpp++) {
			UObject* pObject = UObject::GObjObjects(lpp);
			AActor* pActor = Cast<AActor>(pObject);
			if(pActor) {
				UObject::GObjObjects(lpp) = NULL;		
				bSuccessfulRemoval = TRUE;
				debugf(TEXT("Removing actor beyond the level %s"), *pActor->GetFullName());
				break;
			}					
		}
	}	
...
void UWorld::ClearComponents()
{
	...
	//!{ 2006-06-07	허 창 민
 	if( SampleDrawer )
 	{
 		SampleDrawer->ConditionalDetach();
 	}
 	//!} 2006-06-07	허 창 민
...
void UWorld::UpdateComponents(UBOOL bCurrentLevelOnly)
{
	...
	PersistentLineBatcher->ConditionalAttach(Scene,NULL,FMatrix::Identity);
 	...
 	if( !SampleDrawer )
 	{
 		SampleDrawer = ConstructObject<USampleComponent>(USampleComponent::StaticClass());
 	}
 	SampleDrawer->ConditionalDetach();
 	SampleDrawer->ConditionalAttach(Scene, NULL, FMatrix::Identity);
 	//!} 2006-06-07	허 창 민
...
static UBOOL IsTimeLimitExceeded( const TCHAR* CurrentTask, DOUBLE StartTime, ULevel* Level )
{
	...
	if( GIsEditor == FALSE )
	{
		...
		if( DeltaTime > 5 )
		{
			...
			if( DeltaTime > 20 )
			{
				debugf( NAME_DevStreaming, TEXT("UWorld::AddToWorld: %s for %s took (less than) %5.2f ms"), CurrentTask, *Level->GetOutermost()->GetName(), DeltaTime );
			}
			return TRUE;
		}
	}
...
void UWorld::RemoveFromWorld( ULevelStreaming* StreamingLevel )
{
	...
	if( CurrentLevelPendingVisibility == NULL )
	{
		...
		debugf( NAME_DevStreaming, TEXT("UWorld::RemoveFromWorld for %s took %5.2f ms"), *Level->GetOutermost()->GetName(), (appSeconds()-StartTime) * 1000.0 );
	}
}
...
struct FLevelStreamingGCHelper
{
	...
	static void PrepareStreamedOutLevelsForGC()
	{
		...
		// The Editor always fully loads packages and we can't nuke reference in edited levels so we can't use GIsGame here.
		if( !GIsEditor )
		{
			...
			for( INT LevelStreamingIndex=0; LevelStreamingIndex<LevelStreamingObjects.Num(); LevelStreamingIndex++ )
			{
				...
				debugf( NAME_DevStreaming, TEXT("PrepareStreamedOutLevelsForGC called on '%s'"), *Level->GetOutermost()->GetName() );

				// Make sure that this package has been unloaded after GC pass.
				LevelPackageNames.AddItem( Level->GetOutermost()->GetFName() );

				// Mark level as pending kill so references to it get nuked.
				Level->MarkPendingKill();

				// Mark all model components as pending kill so GC nukes references to them.
				...
				// Mark all actors and their components as pending kill so GC will nuke references to them.
				for( INT ActorIndex=0; ActorIndex<Level->Actors.Num(); ActorIndex++ )
				{
					AActor* Actor = Level->Actors(ActorIndex);
					if( Actor )
				...
				// Remove reference so GC can nuke it.
	...
	else if( LevelPackage )
	{
		...
		if( Level )
		{
			...
			for( INT LevelIndex=0; LevelIndex<WorldInfo->StreamingLevels.Num(); LevelIndex++ )
			{
				...
				{
					// Propagate loaded level so garbage collection doesn't nuke it in case async loading was flushed by GC.
	...
	for( INT LevelIndex=0; LevelIndex<WorldInfo->StreamingLevels.Num(); LevelIndex++ )
	{
		...
		&&	!StreamingLevel->LoadedLevel )
		{
			if( !StreamingLevel->bHasLoadRequestPending )
			{
				...
				if( bNeedToLoad && bAllowLevelLoadRequests  )
				{
					...
					if( GIsEditor || !GEngine->bUseBackgroundLevelStreaming )
					{
						...
						{
							debugf( NAME_DevStreaming, TEXT("Failed to load %s"), *StreamingLevel->PackageName.ToString() );
						}
					}
			...
			if( StreamingLevel->bHasLoadRequestPending && bShouldBlockOnLoad && !GIsEditor )
			{
				...
				debugf( NAME_DevStreaming, TEXT("Requested blocking on load for level %s"), *StreamingLevel->PackageName.ToString() );
			}
		}
...
UBOOL UWorld::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else if( ParseCommand( &Cmd, TEXT("DEMOPLAY") ) )
	{
		...
		if( ParseToken( Cmd, Temp, 0 ) )
		{
			...
			INT i = Temp.InStr(TEXT(".demo"), FALSE, TRUE);
			if( i != -1)
			{
				Temp = Temp.Left(i) + Temp.Mid(i+6);
			}

			Temp = appGameDir() * TEXT("Demos") * Temp;

			UGameEngine* GameEngine = CastChecked<UGameEngine>(GEngine);
			FURL URL(NULL, *GameEngine->LastURL.String(TRUE), TRAVEL_Absolute);
			...
			URL.Map = FString::Printf(TEXT("%s.demo"),*Temp);
	...
	else if( ExecRBCommands( Cmd, &Ar ) )
	{
 	...
 	//!{ 2006-07-05	허 창 민
 	else if( ParseCommand( &Cmd, TEXT("SHOWRADIOSITYSAMPLES")) )
 	{
 		AvaRadiosityAdapter ra;
 		SampleDrawer->Samples.Empty();
 		ra.GetRadiositySamples( &SampleDrawer->Samples );
 		return 1;
 	}
#endif
#if !FINAL_RELEASE
	else if(ExecSmartCullDistanceCommands( Cmd, Ar ))
	{
		return 1;
	}	
#endif
	...
	else if( ParseCommand( &Cmd, TEXT("DumpBspTree")) )
	{
		if( PersistentLevel && PersistentLevel->Model )
		{
			PersistentLevel->Model->DumpBspTree();
		}
		return 1;
	}
	else if ( ParseCommand( &Cmd, TEXT("DumpBspInfo")) )
	{
		if( PersistentLevel && PersistentLevel->Model )
		{
			PersistentLevel->Model->DumpBspInfo();
		}
		return 1;
	}
	else if( ParseCommand( &Cmd, TEXT("DumpAmbientCubeInfo")) )
	{
		if( GWorld )
		{
			const INT NumAmbientCubes = GWorld->GetWorldInfo()->AmbientSHs.Num();
			struct FAmbientSH2
			{
				FAmbientSH	SH;
				BYTE		Irradiance[4];
			...
			const INT AmbientCubeMemSize = NumAmbientCubes * sizeof(FAmbientSH) / 1000;
			const INT IrrAmbientCubeMemSize = NumAmbientCubes * sizeof(FAmbientSH2 ) / 1000;
			debugf(NAME_Log, TEXT("Ambient Cube : %d( %d kb -> %d kb)"), NumAmbientCubes, AmbientCubeMemSize, IrrAmbientCubeMemSize );
		}
		return 1;
	}
	else if (ParseCommand( &Cmd, TEXT("CCReimport")))
	{
		if (GWorld)
		{
			AWorldInfo* WorldInfo = GWorld->GetWorldInfo();

			for (INT i=0; i<WorldInfo->ColorCorrections.Num(); ++i)
			{
				FColorCorrectionData& Data = WorldInfo->ColorCorrections(i);
...
void UWorld::BeginPlay(const FURL& InURL, UBOOL bResetTime)
{
	...
	dLoadingProgress(lpdUpdateComponents); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe
	...
	InitWorldRBPhys();
	dLoadingProgress(lpdInitWorldRBPhys); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe
	...
	dLoadingProgress(lpdInitLevelBSPPhysMesh); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe
	PersistentLevel->IncrementalInitActorsRBPhys(0);
	dLoadingProgress(lpdIncrementalInitActorsRBPhys); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe
	...
	if( !HasBegunPlay() )
	{
		...
		// Lock the level.
		debugf( NAME_Log, TEXT("Bringing %s up for play (%i)..."), *GetFullName(), appRound(GEngine->GetMaxTickRate(0,FALSE)) );
		if (bResetTime)
		{
		...
		dLoadingProgress(lpdInitializeActors); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe
		...
		dLoadingProgress(lpdEventInitGame); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe

		// Route various begin play functions and set volumes.
		PersistentLevel->RouteBeginPlay();
		dLoadingProgress(lpdRouteBeginPlay); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe
	...
	dLoadingProgress(lpdSortActorList); // LoadingProgress(); // g_loadingProgress.progress(); // dEAthcURe

	//<@ ava specific ; 2006. 9. 13 changmin
	...
	if( MinimapVolume )
	{
		...
		Info->MinimapOffset = Box.GetCenter();
		Info->MinimapOffset.Z = Box.Max.Z;
	}
	else
	{
		Info->MinimapOffset = FVector(0,0,0);
	}
	//>@ ava
...
#ifdef EnableHostMigration
void UWorld::HmBeginPlay(const FURL& InURL, UBOOL bResetTime)
{
	check(bIsWorldInitialized);
	DOUBLE StartTime = appSeconds();
	...
	if( !HasBegunPlay() )
	{
		...
		// Route various begin play functions and set volumes.
		PersistentLevel->HmRouteBeginPlay();
...
//
void UWorld::FindRelevantLights(UPrimitiveComponent* Primitive,UBOOL bShadowingLightsOnly,TArray<ULightComponent*>& OutRelevantLights)
{
	for(TObjectIterator<ULightComponent> LightIt;LightIt;++LightIt)
	{
		// Only use light which are in this world.
		ULevel* const LightLevel = LightIt->GetOwner() ? Cast<ULevel>(LightIt->GetOwner()->GetOuter()) : NULL;
		const UBOOL bLightIsInWorld = LightLevel && Levels.ContainsItem(LightLevel);
		const UBOOL bStaticShadowing = ((LightIt->UseDirectLightMap || LightIt->IsShadowCast(Primitive)) && LightIt->HasStaticShadowing());
		const UBOOL bHasDesiredShadowing = !bShadowingLightsOnly || bStaticShadowing;
		if(	bLightIsInWorld &&
			LightIt->AffectsPrimitive(Primitive) &&
			bHasDesiredShadowing &&
			!LightIt->IsA(USkyLightComponent::StaticClass()))
		{
			OutRelevantLights.AddItem(*LightIt);
		}
	}
}
...
UBOOL UWorld::IsValidWorldInfo() const
{
	return PersistentLevel &&
		PersistentLevel->Actors.Num() > 0 &&
		PersistentLevel->Actors(0) != NULL &&
		PersistentLevel->Actors(0)->IsA(AWorldInfo::StaticClass());
}
...
void UWorld::NotifyReceivedText( UNetConnection* Connection, const TCHAR* Text )
{
	...
	else if( NetDriver->ServerConnection )
	{
		...
		else if ( ParseCommand(&Text,TEXT("BRAWL")) )
		{
			//-- alcor
			if (IUnNetErrorHandler::pHandler)
				IUnNetErrorHandler::pHandler->OnBrawl();
			//-- alcor
