...
void FlushDeferredDeletion()
{
	...
	ENQUEUE_UNIQUE_RENDER_COMMAND( BlockUntilGPUIdle, { GPS3Gcm->InsertFence(); GPS3Gcm->BlockUntilGPUIdle(); } );
	for( INT i=0; i<2; i++ )
	{
		// Call DeleteUnusedPS3Resources twice, since it's double-buffering.
		ENQUEUE_UNIQUE_RENDER_COMMAND( DeleteResources, { DeleteUnusedPS3Resources(); DeleteUnusedPS3Resources(); } );
	}
	FlushRenderingCommands();
#endif
}
