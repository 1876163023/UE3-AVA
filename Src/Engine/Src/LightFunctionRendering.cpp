...
IMPLEMENT_SHADER_TYPE(,FLightFunctionPixelShader,TEXT("LightFunctionPixelShader"),TEXT("Main"),SF_Pixel,VER_FP_BLENDING_FALLBACK,0);
...
UBOOL FSceneRenderer::CheckForLightFunction( const FLightSceneInfo* LightSceneInfo, UINT DPGIndex )
{
		...
		LightSceneInfo->LightShadowMode != LightShadow_Modulate )
	{
		for (INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
		{
			...
			if(View.VisibleLightInfos(LightSceneInfo->Id).DPGInfo[DPGIndex].NumVisibleLitPrimitives > 0)
			{
				return TRUE;
			}
		}
