						   ...
						   )
{
	...
	// convert to Z-up coords
	static const FMatrix UpMatrix(
		FPlane(0,	0,	1,	0),
		FPlane(1,	0,	0,	0),
		FPlane(0,	1,	0,	0),
		FPlane(0,	0,	0,	1));

	// move the camera back based on view location
	FMatrix ViewMatrix = FTranslationMatrix(-ViewLocation) * UpMatrix;
...
void FCanvasScene::DetachAllComponents()
{
	...
	for( INT Idx=0; Idx < Primitives.Num(); Idx++ )
	{
		Primitives(Idx)->ConditionalDetach();
	}
	// Remove all the attached lights
	for( INT Idx=0; Idx < Lights.Num(); Idx++ )
	{
		Lights(Idx)->ConditionalDetach();
	}
	// empty lists
	Primitives.Empty();
	Lights.Empty();
