...
void FVertexFactory::Set(FCommandContextRHI* Context) const
{
	...
	for(UINT StreamIndex = 0;StreamIndex < Streams.Num();StreamIndex++)
	{
		...
		RHISetStreamSource(Context,StreamIndex,Stream.VertexBuffer->VertexBufferRHI,Stream.Stride);
	}
}
...
void FVertexFactory::SetVertexLightMap(FCommandContextRHI* Context,const FVertexBuffer* LightMapVertexBuffer) const
{
	for(UINT StreamIndex = 0;StreamIndex < Streams.Num();StreamIndex++)
	{
		const FVertexStream& Stream = Streams(StreamIndex);
	...
	// Set the shadow-map vertex stream.
	RHISetStreamSource(Context,Streams.Num(),LightMapVertexBuffer->VertexBufferRHI,sizeof(FQuantizedLightSample));
}
...
void FVertexFactory::ReleaseRHI()
{
	...
	VertexLightMapDeclaration.Release();
	Streams.Empty();
}
...
* @param OutStreamStrides - output array of # MaxVertexElementCount stream strides to fill
*/
void FVertexFactory::GetVertexLightMapStreamStrides(DWORD *OutStreamStrides) const
{
	UINT StreamIndex = GetStreamStrides(OutStreamStrides, FALSE);
	OutStreamStrides[StreamIndex ++] = sizeof(FQuantizedLightSample);
	// Pad stream strides with 0's to be safe (they can be used in hashes elsewhere)
	for (;StreamIndex < MaxVertexElementCount;++StreamIndex)
	{
		OutStreamStrides[StreamIndex] = 0;
...
void FVertexFactory::InitDeclaration(FVertexDeclarationElementList& Elements, UBOOL bCreateShadowMapDeclaration, UBOOL bCreateLightMapDeclaration)
{
	...
	if (bCreateLightMapDeclaration)
	{
		// Create the vertex declaration for rendering the factory with a vertex light-map.
		FVertexDeclarationElementList LightMapElements = Elements;
		LightMapElements.AddItem(FVertexElement(Streams.Num(),STRUCT_OFFSET(FQuantizedLightSample,Coefficients[0]),VET_Color,VEU_TextureCoordinate,5));
		LightMapElements.AddItem(FVertexElement(Streams.Num(),STRUCT_OFFSET(FQuantizedLightSample,Coefficients[1]),VET_Color,VEU_TextureCoordinate,6));
		LightMapElements.AddItem(FVertexElement(Streams.Num(),STRUCT_OFFSET(FQuantizedLightSample,Coefficients[2]),VET_Color,VEU_TextureCoordinate,7));
		VertexLightMapDeclaration = RHICreateVertexDeclaration(LightMapElements);
	}
}

void FVertexFactory::InitPositionDeclaration(FVertexDeclarationElementList& Elements)
{
	// Create the vertex declaration for rendering the factory normally.
	PositionDeclaration = RHICreateVertexDeclaration(Elements);
}

void FVertexFactory::SwitchVertexBuffer( FVertexBuffer& VertexBuffer )
{
	for (UINT i=0; i<Streams.Num(); ++i)
	{
		if (Streams(i).VertexBuffer)
		{
			Streams(i).VertexBuffer = &VertexBuffer;
		}
		else
			break;
	}		
