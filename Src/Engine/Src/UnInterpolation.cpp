...
IMPLEMENT_CLASS(UInterpTrackRotatorProp);
IMPLEMENT_CLASS(UInterpTrackInstRotatorProp);
...
UBOOL AActor::FindInterpMoveTrack(UInterpTrackMove** OutMoveTrack, UInterpTrackInstMove** OutMoveTrackInst, USeqAct_Interp** OutSeq)
{
	for(INT i=0; i<LatentActions.Num(); i++)
	{
		...
		if(InterpAct)
		{
			UInterpGroupInst* GrInst = InterpAct->FindGroupInst(this,NULL);
...
{
	// 역방향에 대해서 카메라 위치를 0이 아닌 InterpData->InterpLength값으로 변경.(2007/02/13 고광록)
	if ( InterpData )
	{
		// Jump to specific location if desired.
		if(bForceReversePos && !bIsPlaying)
			UpdateInterp(ForceReversePosition, FALSE, TRUE);
	}
...
void USeqAct_Interp::StepInterp(FLOAT DeltaSeconds, UBOOL bPreview)
{
	...
	if (!bSkipUpdate)
	{
		...
		if (ReplicatedActor != NULL)
		{
			...
			// {{ 20070724 dEAthcURe|HM 원코드는 replication안하지만 HM을 위해서 해야한다.
			//debugf(TEXT("dEAthcURe|TEST fabs diff %f %f"), fabs(ReplicatedActor->Position - NewPosition), ReplicatingDiffPosition);
			if(fabs(NewPosition - prevPosition) >= ReplicatingDiffPosition) {
				ReplicatedActor->bNetDirty = TRUE;		
				if(GWorld) {
					AWorldInfo* WorldInfo = GWorld->GetWorldInfo();
					if(WorldInfo) {
						ReplicatedActor->NetUpdateTime = WorldInfo->TimeSeconds - 1.0f;
					}
				}
				//debugf(TEXT("dEAthcURe|TEST Updating position %f %f"), NewPosition, ReplicatingDiffPosition);
	...
	if(InterpData)
	{
		for(INT i=0; i<InterpData->InterpGroups.Num(); i++)
		{
				...
				if(GIsGame)
				{
					...
					for (AController *Controller = GWorld->GetFirstController(); Controller != NULL; Controller = Controller->NextController)
					{
						...
						if (PC != NULL)
						{
							...
							NewGroupInstDir->InitGroupInst(DirGroup, PC, NULL);
							bCreatedGroup = TRUE;
						}
					}
					...
					if(bCreatedGroup == FALSE)
					{
						...
						GroupInst.AddItem(NewGroupInstDir);
						// and initialize the instance
						NewGroupInstDir->InitGroupInst(DirGroup, NULL, NULL);
					}
				}
				else
				{
			...
			else
			{
				...
				{
					AActor* Actor = NULL;
					UUIObject* Object = NULL;

					if( j < ObjectVars.Num() )
					{
						UObject *Obj = *(ObjectVars(j));
						Actor = Cast<AActor>(Obj);
						Object = Cast<UUIObject>(Obj);

						// See if there is already a group working on this actor.
						UInterpGroupInst* TestInst = FindGroupInst(Actor,Object);
						if(TestInst)
						{
							debugf( NAME_Warning, TEXT("Skipping instancing group - an Actor may only be connected to one Group!") );
							//Actor = NULL;
						}
					}
					...
					NewGroupInst->InitGroupInst(Group, Actor, Object);
				}
			}
		}
	}
...
UInterpGroupInst* USeqAct_Interp::FindGroupInst(AActor* Actor,UUIObject* Object)
{
	if((!Actor || Actor->bDeleteMe) && !Object)
	{
		return NULL;
	}
	...
	for(INT i=0; i<GroupInst.Num(); i++)
	{
		if( GroupInst(i)->GetGroupActor() == Actor && Actor != NULL ||
			GroupInst(i)->GetGroupObject() == Object && Object != NULL )
		{
			return GroupInst(i);
		}
	}
...
UBOOL UInterpGroup::HasAnimControlTrack() const
{
	...
	for(INT i=0; i<InterpTracks.Num(); i++)
	{
		if( InterpTracks(i)->IsA(UInterpTrackAnimControl::StaticClass()) )
		{
			bHasAnimTrack = TRUE;
		}
	}
...
UUIObject* UInterpGroupInst::GetGroupObject()
{
	return GroupObject;	
}
...
*/
void UInterpGroupInst::InitGroupInst(UInterpGroup* InGroup, AActor* InGroupActor, UUIObject* InGroupObject)
{
	check(InGroup);
	Group = InGroup;
...
UUIObject* UInterpTrackInst::GetGroupObject()
{
	UInterpGroupInst* GrInst = CastChecked<UInterpGroupInst>( GetOuter() );
	return GrInst->GetGroupObject();
}
...
void UInterpTrackMove::GetKeyTransformAtTime(FLOAT Time, FVector& OutPos, FRotator& OutRot)
{
	FQuat KeyQuat;
	...
	{
		OutRot = FRotator::MakeFromEuler( EulerTrack.Eval(Time, FVector(0.f)) );
	}

	OutPos = PosTrack.Eval(Time, FVector(0.f));
}
...
void UInterpTrackMove::GetLocationAtTime(UInterpTrackInst* TrInst, FLOAT Time, FVector& OutPos, FRotator& OutRot)
{
	...
	FVector RelPos;
	FRotator RelRot;
	GetKeyTransformAtTime(Time, RelPos, RelRot);
	...
	OutRot = WorldTM.Rotator() + FRotator::MakeFromEuler(RelKeyWindEuler);
...
void UInterpTrackInstMove::CalcInitialTransform(UInterpTrack* Track, UBOOL bZeroFromHere)
{
	...
	if(bZeroFromHere)
	{
		MoveTrack->GetKeyTransformAtTime(0.f, RelPos, RelRot);
	}
	else
	{
		// Find the current relative position according to the interpolation.
		MoveTrack->GetKeyTransformAtTime(Seq->Position, RelPos, RelRot);
	}
...
void UInterpTrackFloatProp::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	...
	UUIObject* Object = TrInst->GetGroupObject();
	if (Object)
	{
		UInterpTrackInstFloatProp* PropInst = CastChecked<UInterpTrackInstFloatProp>(TrInst);
		if(!PropInst->FloatProp)
...
{
	AActor* Actor = GetGroupActor();
	UUIObject* Object = GetGroupObject();
	if(!Actor && !Object)
	{
		return;
	}
...
void UInterpTrackInstFloatProp::InitTrackInst(UInterpTrack* Track)
{
	...
	if (Actor != NULL)
	{
		FloatProp = ::GetInterpFloatPropertyRef(Actor,TrackProp->PropertyName);
	}	
	else
	{
		FloatProp = ::GetInterpFloatPropertyRef(Object,TrackProp->PropertyName);
	}
}

/*-----------------------------------------------------------------------------
UInterpTrackRotatorProp
-----------------------------------------------------------------------------*/

/** Add a new keyframe at the speicifed time. Returns index of new keyframe. */
INT UInterpTrackRotatorProp::AddKeyframe(FLOAT Time, UInterpTrackInst* TrInst)
{
	UInterpTrackInstRotatorProp* PropInst = CastChecked<UInterpTrackInstRotatorProp>(TrInst);
	if( !PropInst->RotatorProp )
		return INDEX_NONE;
...
/** Change the value of an existing keyframe. */
void UInterpTrackRotatorProp::UpdateKeyframe(INT KeyIndex, UInterpTrackInst* TrInst)
{
	UInterpTrackInstRotatorProp* PropInst = CastChecked<UInterpTrackInstRotatorProp>(TrInst);
	if( !PropInst->RotatorProp )
		return;
	...
	const FRotator& Rotator = *((FRotator*)(PropInst->RotatorProp));

	VectorTrack.Points(KeyIndex).OutVal = FVector( Rotator.Pitch * 360.0f / 65536.0f, Rotator.Yaw * 360.0f / 65536.0f, Rotator.Roll * 360.0f / 65536.0f );
...
*  This is called in the editor, when scrubbing/previewing etc.
*/
void UInterpTrackRotatorProp::PreviewUpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst)
{
	UpdateTrack(NewPosition, TrInst, FALSE);
}
...
*  @param bJump	Indicates if this is a sudden jump instead of a smooth move to the new position.
*/
void UInterpTrackRotatorProp::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	AActor* Actor = TrInst->GetGroupActor();
	UUIObject* Object = TrInst->GetGroupObject();
	if(!Actor && !Object)
	...
	FRotator& Rotator = *((FRotator*)(PropInst->RotatorProp));

	FVector NewRotatorValue = VectorTrack.Eval( NewPosition, FVector( Rotator.Pitch * 360.0f / 65536.0f, Rotator.Yaw * 360.0f / 65536.0f, Rotator.Roll * 360.0f / 65536.0f ) );
	Rotator = FRotator( NewRotatorValue.X * 65536.0f / 360.0f, NewRotatorValue.Y * 65536.0f / 360.0f, NewRotatorValue.Z * 65536.0f / 360.0f);
...
/** Get the name of the class used to help out when adding tracks, keys, etc. in UnrealEd.
* @return	String name of the helper class.*/
const FString	UInterpTrackRotatorProp::GetEdHelperClassName() const
{
	return FString( TEXT("UnrealEd.InterpTrackRotatorPropHelper") );
}


/*-----------------------------------------------------------------------------
UInterpTrackInstRotatorProp
-----------------------------------------------------------------------------*/

/** Called before Interp editing to put object back to its original state. */
void UInterpTrackInstRotatorProp::SaveActorState(UInterpTrack* Track)
{
	AActor* Actor = GetGroupActor();
	UUIObject* Object = GetGroupObject();
	if(!Actor && !Object)
	...
	// Remember current value of property for when we quite Matinee
	ResetRotator = *((FRotator*)(RotatorProp));
}

/** Restore the saved state of this Actor. */
void UInterpTrackInstRotatorProp::RestoreActorState(UInterpTrack* Track)
{
	AActor* Actor = GetGroupActor();
	UUIObject* Object = GetGroupObject();
	if(!Actor && !Object)
	...
	// Restore original value of property
	*((FRotator*)(RotatorProp)) = ResetRotator;
...
/** Initialise this Track instance. Called in-game before doing any interpolation. */
void UInterpTrackInstRotatorProp::InitTrackInst(UInterpTrack* Track)
{
	AActor* Actor = GetGroupActor();
	UUIObject* Object = GetGroupObject();
	if(!Actor && !Object)
	...
	UInterpTrackRotatorProp* TrackProp = Cast<UInterpTrackRotatorProp>(Track);

	if (Actor)
	{
		RotatorProp = ::GetInterpRotatorPropertyRef(Actor,TrackProp->PropertyName);
	}
	else
	{
		RotatorProp = ::GetInterpRotatorPropertyRef(Object,TrackProp->PropertyName);
	}
}
...
void UInterpTrackInstVectorProp::InitTrackInst(UInterpTrack* Track)
{
	...
	if (Actor)
	{
		VectorProp = ::GetInterpVectorPropertyRef(Actor,TrackProp->PropertyName);
	}
	else
	{
		VectorProp = ::GetInterpVectorPropertyRef(Object,TrackProp->PropertyName);
	}
}
...
void UInterpTrackAnimControl::GetAnimForTime(FLOAT InTime, FName& OutAnimSeqName, FLOAT& OutPosition)
{
	if(AnimSeqs.Num() == 0)
	{
...
void UInterpTrackAnimControl::PreviewUpdateTrack(FLOAT NewPosition, class UInterpTrackInst* TrInst)
{
	...
	GetAnimForTime(NewPosition, NewAnimSeqName, NewAnimPosition);

	if(NewAnimSeqName != NAME_None)
	{
		Actor->PreviewSetAnimPosition(SlotName, ChannelIndex, NewAnimSeqName, NewAnimPosition);
	}
}
...
void UInterpTrackAnimControl::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	...
	if(AnimSeqs.Num() == 0 || NewPosition <= AnimInst->LastUpdatePosition || bJump)
	{
		...
		Actor->eventSetAnimPosition(SlotName, ChannelIndex, NewAnimSeqName, NewAnimPosition, FALSE);
	}
	...
	else
	{
		...
		while(CurrentSeqIndex <= EndSeqIndex)
		{
			...
			{
				Actor->eventSetAnimPosition( SlotName, ChannelIndex, AnimSeqs(0).AnimSeqName, AnimSeqs(0).AnimStartOffset, FALSE );
			}
			// If we are within an anim.
			else
			{
				// Find the name and starting time
				FName CurrentAnimName = AnimSeqs(CurrentSeqIndex).AnimSeqName;
				FLOAT CurrentSeqStart = AnimSeqs(CurrentSeqIndex).StartTime;
				FLOAT CurrentStartOffset = AnimSeqs(CurrentSeqIndex).AnimStartOffset;
				FLOAT CurrentEndOffset = AnimSeqs(CurrentSeqIndex).AnimEndOffset;
				FLOAT CurrentRate = AnimSeqs(CurrentSeqIndex).AnimPlayRate;
				...
				if(AnimSeqs(CurrentSeqIndex).bLooping)
				{
					...
					if(Seq)
					{
						...
						FLOAT SeqLength = ::Max((Seq->SequenceLength - (CurrentStartOffset + CurrentEndOffset)) / CurrentRate, 0.01f);
						...
						{
							Actor->eventSetAnimPosition(SlotName, ChannelIndex, CurrentAnimName, SeqEnd + KINDA_SMALL_NUMBER, TRUE);
							Actor->eventSetAnimPosition(SlotName, ChannelIndex, CurrentAnimName, CurrentStartOffset, FALSE);
						}

						FLOAT AnimPos = appFmod((ToTime - CurrentSeqStart) * CurrentRate, SeqLength);
						Actor->eventSetAnimPosition(SlotName, ChannelIndex, CurrentAnimName, AnimPos + CurrentStartOffset, TRUE);
					}
					// If we failed to find the sequence, just use simpler method.
					else
					{
						Actor->eventSetAnimPosition(SlotName, ChannelIndex, CurrentAnimName, ((ToTime - CurrentSeqStart) * CurrentRate) + CurrentStartOffset, TRUE);
					}
				}
				// No looping - its easy - wind to desired time.
				else
				{
					...
					Actor->eventSetAnimPosition(SlotName, ChannelIndex, CurrentAnimName, AnimPos, TRUE);
				}

				// If we are not yet at target anim, set position at start of next anim.
				if(CurrentSeqIndex < EndSeqIndex)
				{
					Actor->eventSetAnimPosition( SlotName, ChannelIndex, AnimSeqs(CurrentSeqIndex+1).AnimSeqName, AnimSeqs(CurrentSeqIndex+1).AnimStartOffset, FALSE );
				}
			}
...
void UInterpTrackFaceFX::GetSeqInfoForTime(FLOAT InTime, FString& OutGroupName, FString& OutSeqName, FLOAT& OutPosition)
{
...
void UInterpTrackFaceFX::PreviewUpdateTrack(FLOAT NewPosition, class UInterpTrackInst* TrInst)
{
	...
	else
	{
		...
		FLOAT Position;
		GetSeqInfoForTime(NewPosition, GroupName, SeqName, Position);
...
void UInterpTrackFaceFX::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	...
	if(NewPosition > FaceFXTrInst->LastUpdatePosition && !bJump)
	{
		...
		GetSeqInfoForTime(FaceFXTrInst->LastUpdatePosition, OldGroupName, OldSeqName, DummyPos);

		FString NewGroupName, NewSeqName;
		GetSeqInfoForTime(NewPosition, NewGroupName, NewSeqName, DummyPos);
		...
		if((FaceFXTrInst->bFirstUpdate && NewSeqName != TEXT("") ) || NewGroupName != OldGroupName || NewSeqName != OldSeqName)
		{
