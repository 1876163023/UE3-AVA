...
{
	if( Ar.IsSaving() && GIsCooking && !B.bInflucencesByteSwapped )
	{
...
void FSkeletalMeshObjectGPUSkin::UpdateDynamicData_RenderThread(FDynamicSkelMeshObjectData* InDynamicData)
{
	...
	checkSlow( DynamicData->NumWeightedActiveMorphs > 0 ? (LOD.MorphVertexFactories.Num()==LODModel.Chunks.Num()) : (LOD.VertexFactories.Num()==LODModel.Chunks.Num()) );
	...
	{
		checkSlow(LOD.DecalVertexFactories.Num()==LODModel.Chunks.Num());
		for( INT ChunkIdx=0; ChunkIdx < LODModel.Chunks.Num(); ChunkIdx++ )
		{
			FSkelMeshChunk& Chunk = LODModel.Chunks(ChunkIdx);
			FGPUSkinVertexFactory::ShaderDataType& ShaderData = LOD.DecalVertexFactories(ChunkIdx).GetShaderData();
...
const FVertexFactory* FSkeletalMeshObjectGPUSkin::GetVertexFactory(INT LODIndex,INT ChunkIdx) const
{
	...
	checkSlow( LODs(LODIndex).VertexFactories.IsValidIndex(ChunkIdx) );
	checkSlow( DynamicData );
...
FSkelMeshDecalVertexFactoryBase* FSkeletalMeshObjectGPUSkin::GetDecalVertexFactory(INT LODIndex,INT ChunkIdx,const FDecalInteraction* Decal)
{
	...
	checkSlow( LODs.IsValidIndex(LODIndex) );
	checkSlow( LODs(LODIndex).DecalVertexFactories.IsValidIndex(ChunkIdx) );

	//@todo sz - handle morph targets w/ decals
