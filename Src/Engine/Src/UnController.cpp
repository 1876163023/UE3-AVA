/*=============================================================================
  ...
  AI related natives
	...
	Copyright 2000-2005 Epic Games, Inc. This software is a trade secret.
=============================================================================*/
...
void AController::Serialize(FArchive& Ar)
{
	...
	{
		// Marks the Actor as pending kill so garbage collection will nuke references to it and PostBeginPlay won't 
...
{
	return GetAPlayerController() && ((GWorld->GetNetMode() == NM_Standalone) || LocalPlayerController());
}
...
APawn* AController::PickTarget(UClass* TargetClass, FLOAT& bestAim, FLOAT& bestDist, FVector FireDir, FVector projStart, FLOAT MaxRange)
{
	...
	for ( AController *next=GWorld->GetFirstController(); next!=NULL; next=next->NextController )
	{
		...
		if ( NewTarget && (NewTarget != Pawn) )
		{
			...
			if ( NewTarget->GetClass()->IsChildOf(TargetClass) && NewTarget->IsValidTargetFor(this) )
			{
...
{
	// only am valid target if not dead and not on same team
	return ( C != NULL && Health > 0 && !bDeleteMe
			&& (!PlayerReplicationInfo || !C->PlayerReplicationInfo
			|| (WorldInfo->Game && !WorldInfo->Game->bTeamGame)
...
*/
void AController::execWaitForLanding _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AController::execPollWaitForLanding _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	if( Pawn && (Pawn->Physics != PHYS_Falling) )
	{
		GetStateFrame()->LatentAction = 0;
...
INT APlayerController::FindStairRotation(FLOAT deltaTime)
{
	...
	GWorld->SingleLineCheck(Hit, this, EyeSpot + 2 * height * Dir, EyeSpot, TRACE_World|TRACE_Tesselation, CollisionSlice);
	...
	if (Dist > 0.8 * height)
	{
		...
		FLOAT Down = 3 * height;
		GWorld->SingleLineCheck(Hit, this, Spot - FVector(0,0,Down), Spot, TRACE_World|TRACE_Tesselation, CollisionSlice);
		if (Hit.Time < 1.f)
		{
			...
			{
				GWorld->SingleLineCheck(Hit, this, Pawn->Location + 0.9*Dist*Dir, Pawn->Location, TRACE_World|TRACE_StopAtAnyHit|TRACE_Tesselation);
				if (Hit.Time == 1.f)
				{
					Spot = EyeSpot + Dist * Dir;
					GWorld->SingleLineCheck(Hit, this, Spot - FVector(0,0,Down), Spot, TRACE_World|TRACE_Tesselation, CollisionSlice);
...
void AActor::execSuggestTossVelocity _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
*/
AActor* AController::FindPath( FVector point, AActor* goal, UBOOL bWeightDetours, FLOAT MaxPathLength, UBOOL bReturnPartial )
{
	if ( !Pawn )
	{
...
void AController::execLineOfSightTo _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_ACTOR(Other);
...
*/
void AController::execMoveTo _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_VECTOR(dest);
	P_GET_ACTOR_OPTX(viewfocus, NULL);
	P_GET_UBOOL_OPTX(bShouldWalk, (Pawn != NULL) ? Pawn->bIsWalking : NULL);
	P_FINISH;
...
void AController::execPollMoveTo _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
*/
void AController::execMoveToward _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_ACTOR(goal);
	P_GET_ACTOR_OPTX(viewfocus, NULL);
...
void AController::execPollMoveToward _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void AController::execPollFinishRotation _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
AActor* AController::HandleSpecial(AActor *bestPath)
{
	...
	if ( newGoal && (newGoal != bestPath) )
	{
		...
		{
			debugf(NAME_DevPath, TEXT("Failed to find path to special goal %s on the way to %s"), *newGoal->GetName(), *bestPath->GetName());
			bPreparingMove = bOldPrep;
		}
	}
	return bestPath;
