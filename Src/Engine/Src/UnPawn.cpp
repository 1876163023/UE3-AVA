/*=============================================================================
  ...
  AI related natives
	...
	Copyright 1997-2005 Epic MegaGames, Inc. This software is a trade secret.
=============================================================================*/
...
#ifdef EnableHostMigration
// {{ dEAthcURe|HM
void APawn::hmSerialize(FArchive& Ar)
{
	//check 20070207
	...
	if ( Ar.IsLoading() ) {
		...
		_hms_loadValue(bUpAndOut); // Ar << value; bUpAndOut = value ? 1 : 0; // Ar << (bUpAndOut ? one:zero);
		_hms_loadValue(bIsWalking); // Ar << value; bIsWalking = value ? 1 : 0; // Ar << (bIsWalking ? one:zero);
		_hms_loadValue(bWantsToCrouch); // Ar << value; bWantsToCrouch = value ? 1 : 0; // Ar << (bWantsToCrouch ? one:zero);
		_hms_loadValue(bIsCrouched); // Ar << value; bIsCrouched = value ? 1 : 0; // Ar << (bIsCrouched ? one:zero);
		_hms_loadValue(bTryToUncrouch); // Ar << value; bTryToUncrouch = value ? 1 : 0; // Ar << (bTryToUncrouch ? one:zero);
		_hms_loadValue(bCanCrouch); // Ar << value; bCanCrouch = value ? 1 : 0; // Ar << (bCanCrouch ? one:zero);
		_hms_loadValue(bCrawler); // Ar << value; bCrawler = value ? 1 : 0; // Ar << (bCrawler ? one:zero);
		_hms_loadValue(bReducedSpeed); // Ar << value; bReducedSpeed = value ? 1 : 0; // Ar << (bReducedSpeed ? one:zero);
		_hms_loadValue(bJumpCapable); // Ar << value; bJumpCapable = value ? 1 : 0; // Ar << (bJumpCapable ? one:zero);
		_hms_loadValue(bCanJump); // Ar << value; bCanJump = value ? 1 : 0; // Ar << (bCanJump ? one:zero);
		_hms_loadValue(bCanWalk); // Ar << value; bCanWalk = value ? 1 : 0; // Ar << (bCanWalk ? one:zero);
		_hms_loadValue(bCanSwim); // Ar << value; bCanSwim = value ? 1 : 0; // Ar << (bCanSwim ? one:zero);
		_hms_loadValue(bCanFly); // Ar << value; bCanFly = value ? 1 : 0; // Ar << (bCanFly ? one:zero);
		_hms_loadValue(bCanClimbLadders); // Ar << value; bCanClimbLadders = value ? 1 : 0; // Ar << (bCanClimbLadders ? one:zero);
		_hms_loadValue(bCanStrafe); // Ar << value; bCanStrafe = value ? 1 : 0; // Ar << (bCanStrafe ? one:zero);
		_hms_loadValue(bAvoidLedges); // Ar << value; bAvoidLedges = value ? 1 : 0; // Ar << (bAvoidLedges ? one:zero);
		_hms_loadValue(bStopAtLedges); // Ar << value; bStopAtLedges = value ? 1 : 0; // Ar << (bStopAtLedges ? one:zero);
		_hms_loadValue(bCountJumps); // Ar << value; bCountJumps = value ? 1 : 0; // Ar << (bCountJumps ? one:zero);
		_hms_loadValue(bSimulateGravity); // Ar << value; bSimulateGravity = value ? 1 : 0; // Ar << (bSimulateGravity ? one:zero);
		_hms_loadValue(bIgnoreForces); // Ar << value; bIgnoreForces = value ? 1 : 0; // Ar << (bIgnoreForces ? one:zero);
		_hms_loadValue(bCanWalkOffLedges); // Ar << value; bCanWalkOffLedges = value ? 1 : 0; // Ar << (bCanWalkOffLedges ? one:zero);
		_hms_loadValue(bCanBeBaseForPawns); // Ar << value; bCanBeBaseForPawns = value ? 1 : 0; // Ar << (bCanBeBaseForPawns ? one:zero);
		_hms_loadValue(bClientCollision); // Ar << value; bClientCollision = value ? 1 : 0; // Ar << (bClientCollision ? one:zero);
		_hms_loadValue(bSimGravityDisabled); // Ar << value; bSimGravityDisabled = value ? 1 : 0; // Ar << (bSimGravityDisabled ? one:zero);
		_hms_loadValue(bDirectHitWall); // Ar << value; bDirectHitWall = value ? 1 : 0; // Ar << (bDirectHitWall ? one:zero);
		_hms_loadValue(bPushesRigidBodies); // Ar << value; bPushesRigidBodies = value ? 1 : 0; // Ar << (bPushesRigidBodies ? one:zero);
		_hms_loadValue(bForceFloorCheck); // BITFIELD bForceFloorCheck:1;//!
		_hms_loadValue(bForceKeepAnchor); // BITFIELD bForceKeepAnchor:1;//!
		_hms_loadValue(bCanMantle); // Ar << value; bCanMantle = value ? 1 : 0; // Ar << (bCanMantle ? one:zero);
		_hms_loadValue(bCanClimbCeilings); // Ar << value; bCanClimbCeilings = value ? 1 : 0; // Ar << (bCanClimbCeilings ? one:zero);
		_hms_loadValue(bCanSwatTurn); // Ar << value; bCanSwatTurn = value ? 1 : 0; // Ar << (bCanSwatTurn ? one:zero);
		_hms_loadValue(bIsFemale); // Ar << value; bIsFemale = value ? 1 : 0; // Ar << (bIsFemale ? one:zero);
		_hms_loadValue(bCanPickupInventory); // Ar << value; bCanPickupInventory = value ? 1 : 0; // Ar << (bCanPickupInventory ? one:zero);
		_hms_loadValue(bAmbientCreature); // Ar << value; bAmbientCreature = value ? 1 : 0; // Ar << (bAmbientCreature ? one:zero);
		_hms_loadValue(bLOSHearing); // Ar << value; bLOSHearing = value ? 1 : 0; // Ar << (bLOSHearing ? one:zero);
		_hms_loadValue(bMuffledHearing); // Ar << value; bMuffledHearing = value ? 1 : 0; // Ar << (bMuffledHearing ? one:zero);
		_hms_loadValue(bAroundCornerHearing); // Ar << value; bAroundCornerHearing = value ? 1 : 0; // Ar << (bAroundCornerHearing ? one:zero);
		_hms_loadValue(bDontPossess); // Ar << value; bDontPossess = value ? 1 : 0; // Ar << (bDontPossess ? one:zero);
		_hms_loadValue(bAutoFire); // Ar << value; bAutoFire = value ? 1 : 0; // Ar << (bAutoFire ? one:zero);
		_hms_loadValue(bRollToDesired); // Ar << value; bRollToDesired = value ? 1 : 0; // Ar << (bRollToDesired ? one:zero);
		_hms_loadValue(bStationary); // Ar << value; bStationary = value ? 1 : 0; // Ar << (bStationary ? one:zero);
		_hms_loadValue(bCachedRelevant); // Ar << value; bCachedRelevant = value ? 1 : 0; // Ar << (bCachedRelevant ? one:zero);
		_hms_loadValue(bSpecialHUD); // Ar << value; bSpecialHUD = value ? 1 : 0; // Ar << (bSpecialHUD ? one:zero);
		_hms_loadValue(bNoWeaponFiring); // Ar << value; bNoWeaponFiring = value ? 1 : 0; // Ar << (bNoWeaponFiring ? one:zero);
		_hms_loadValue(bCanUse); // Ar << value; bCanUse = value ? 1 : 0; // Ar << (bCanUse ? one:zero);
		_hms_loadValue(bRunPhysicsWithNoController); // BITFIELD bRunPhysicsWithNoController:1;//!
		_hms_loadValue(bForceMaxAccel); // Ar << value; bForceMaxAccel = value ? 1 : 0; // Ar << (bForceMaxAccel ? one:zero);
		_hms_loadValue(bForceRMVelocity); // Ar << value; bForceRMVelocity = value ? 1 : 0; // Ar << (bForceRMVelocity ? one:zero);
		_hms_loadValue(bPlayedDeath); // Ar << value; bPlayedDeath = value ? 1 : 0; // Ar << (bPlayedDeath ? one:zero);
	}
	else {
		_hms_defSaving;

		_hms_saveValue(bUpAndOut); // Ar << (bUpAndOut ? one:zero);
		_hms_saveValue(bIsWalking); // Ar << (bIsWalking ? one:zero);
		_hms_saveValue(bWantsToCrouch); // Ar << (bWantsToCrouch ? one:zero);
		_hms_saveValue(bIsCrouched); // Ar << (bIsCrouched ? one:zero);
		_hms_saveValue(bTryToUncrouch); // Ar << (bTryToUncrouch ? one:zero);
		_hms_saveValue(bCanCrouch); // Ar << (bCanCrouch ? one:zero);
		_hms_saveValue(bCrawler); // Ar << (bCrawler ? one:zero);
		_hms_saveValue(bReducedSpeed); // Ar << (bReducedSpeed ? one:zero);
		_hms_saveValue(bJumpCapable); // Ar << (bJumpCapable ? one:zero);
		_hms_saveValue(bCanJump); // Ar << (bCanJump ? one:zero);
		_hms_saveValue(bCanWalk); // Ar << (bCanWalk ? one:zero);
		_hms_saveValue(bCanSwim); // Ar << (bCanSwim ? one:zero);
		_hms_saveValue(bCanFly); // Ar << (bCanFly ? one:zero);
		_hms_saveValue(bCanClimbLadders); // Ar << (bCanClimbLadders ? one:zero);
		_hms_saveValue(bCanStrafe); // Ar << (bCanStrafe ? one:zero);
		_hms_saveValue(bAvoidLedges); // Ar << (bAvoidLedges ? one:zero);
		_hms_saveValue(bStopAtLedges); // Ar << (bStopAtLedges ? one:zero);
		_hms_saveValue(bCountJumps); // Ar << (bCountJumps ? one:zero);
		_hms_saveValue(bSimulateGravity); // Ar << (bSimulateGravity ? one:zero);
		_hms_saveValue(bIgnoreForces); // Ar << (bIgnoreForces ? one:zero);
		_hms_saveValue(bCanWalkOffLedges); // Ar << (bCanWalkOffLedges ? one:zero);
		_hms_saveValue(bCanBeBaseForPawns); // Ar << (bCanBeBaseForPawns ? one:zero);
		_hms_saveValue(bClientCollision); // Ar << (bClientCollision ? one:zero);
		_hms_saveValue(bSimGravityDisabled); // Ar << (bSimGravityDisabled ? one:zero);
		_hms_saveValue(bDirectHitWall); // Ar << (bDirectHitWall ? one:zero);
		_hms_saveValue(bPushesRigidBodies); // Ar << (bPushesRigidBodies ? one:zero);
		_hms_saveValue(bForceFloorCheck); // BITFIELD bForceFloorCheck:1;//!
		_hms_saveValue(bForceKeepAnchor); // BITFIELD bForceKeepAnchor:1;//!
		_hms_saveValue(bCanMantle); // Ar << (bCanMantle ? one:zero);
		_hms_saveValue(bCanClimbCeilings); // Ar << (bCanClimbCeilings ? one:zero);
		_hms_saveValue(bCanSwatTurn); // Ar << (bCanSwatTurn ? one:zero);
		_hms_saveValue(bIsFemale); // Ar << (bIsFemale ? one:zero);
		_hms_saveValue(bCanPickupInventory); // Ar << (bCanPickupInventory ? one:zero);
		_hms_saveValue(bAmbientCreature); // Ar << (bAmbientCreature ? one:zero);
		_hms_saveValue(bLOSHearing); // Ar << (bLOSHearing ? one:zero);
		_hms_saveValue(bMuffledHearing); // Ar << (bMuffledHearing ? one:zero);
		_hms_saveValue(bAroundCornerHearing); // Ar << (bAroundCornerHearing ? one:zero);
		_hms_saveValue(bDontPossess); // Ar << (bDontPossess ? one:zero);
		_hms_saveValue(bAutoFire); // Ar << (bAutoFire ? one:zero);
		_hms_saveValue(bRollToDesired); // Ar << (bRollToDesired ? one:zero);
		_hms_saveValue(bStationary); // Ar << (bStationary ? one:zero);
		_hms_saveValue(bCachedRelevant); // Ar << (bCachedRelevant ? one:zero);
		_hms_saveValue(bSpecialHUD); // Ar << (bSpecialHUD ? one:zero);
		_hms_saveValue(bNoWeaponFiring); // Ar << (bNoWeaponFiring ? one:zero);
		_hms_saveValue(bCanUse); // Ar << (bCanUse ? one:zero);
		_hms_saveValue(bRunPhysicsWithNoController); // BITFIELD bRunPhysicsWithNoController:1;//!
		_hms_saveValue(bForceMaxAccel); // Ar << (bForceMaxAccel ? one:zero);
		_hms_saveValue(bForceRMVelocity); // Ar << (bForceRMVelocity ? one:zero);
		_hms_saveValue(bPlayedDeath); // Ar << (bPlayedDeath ? one:zero);
	}
    ...
    Ar << AvgPhysicsTime;
    Ar << MeleeRange;	
    Ar << FindAnchorFailedTime;
    Ar << LastValidAnchorTime;
    Ar << DestinationOffset;
    Ar << NextPathRadius;
    Ar << SerpentineDir;
    Ar << SerpentineDist;
    Ar << SerpentineTime;
    ...
    Ar << JumpZ;
    Ar << OutofWaterZ;
    Ar << MaxOutOfWaterStepHeight;
    Ar << AirControl;
    Ar << WalkingPct;
    Ar << CrouchedPct;
    Ar << MaxFallSpeed;
    Ar << AIMaxFallSpeedFactor;
    Ar << BaseEyeHeight;
    Ar << EyeHeight;
    Ar << Floor;
    Ar << SplashTime;
    ...
    Ar << DamageScaling;
    Ar << MenuName;	
    Ar << LandMovementState;
    Ar << WaterMovementState;	
    Ar << LastStartTime;
    Ar << TakeHitLocation;	
    Ar << TearOffMomentum;	
    Ar << RBPushRadius;
    Ar << RBPushStrength;	
    Ar << AlwaysRelevantDistanceSquared;
    Ar << VehicleCheckRadius;	
    Ar << ViewPitchMin;
    Ar << ViewPitchMax;
    Ar << AllowedYawError;	
    Ar << FlashLocation;
    Ar << ShotCount;
	//FPointer PhysicsPushBody[2]; //? 해야되는지 확인해보3
	*/

	BYTE bPlayerReplicationInfo = PlayerReplicationInfo? 1 : 0;
	Ar << bPlayerReplicationInfo;

	if ( Ar.IsLoading() ) {
		if(bPlayerReplicationInfo) {
			if(PlayerReplicationInfo) {
				PlayerReplicationInfo->hmSerialize(Ar);
			}
			else {
				debugf(TEXT("[dEAthcURe] Invalid object null player replication info"));
			}
		}
	}
	else {
...
/** returns true if this pawn is the instigator, or should be considered the instigator for purposes of collision determination.
See AProjectile::IgnoreBlockingBy().  */
UBOOL APawn::UseInstigatorBlockingFor( const AActor *Other) const
{
	return Other == this;
}
...
{
	int seen = 0;
	if ( (GWorld->GetNetMode() == NM_Standalone) || (GWorld->GetNetMode() == NM_Client) )
	{
		// just check local player visibility
		seen = (GWorld->GetTimeSeconds() - LastRenderTime < 1);
	}
	else
	{
...
void APawn::Crouch(INT bClientSimulation)
{
	...
	// 언리얼에서 APawn::PostNetReceiveLocation() 에서 
	// EncroachingWorldGeometry 이 실패하면 CollisionHeight 와 CollisionRadius 를 -1 씩 해버린다.
	// 그래서 Adjust 가 잘못 적용돼 Pawn 이 점점 가라앉는다. 그래서 수정하기 위해서 들어간 Code 이다.
	// 2006/04/13 
	//		by OZ
	
	APawn* DefaultPawn = (APawn*)(GetClass()->GetDefaultActor());
	FLOAT OldHeight = CylinderComponent->CollisionHeight;
	//FLOAT OldHeight = DefaultPawn->CylinderComponent->CollisionHeight;
...
void APawn::UnCrouch(INT bClientSimulation)
{
	...
	//FLOAT	HeightAdjust	= DefaultPawn->CylinderComponent->CollisionHeight - CrouchHeight;		
	...
	CylinderComponent->SetSize(DefaultPawn->CylinderComponent->CollisionHeight,DefaultPawn->CylinderComponent->CollisionRadius);
	...
	if( !bClientSimulation )
	{
		...
		if( bEncroached )
		{
			CylinderComponent->SetSize(CrouchHeight,CrouchRadius);
...
{
	// get the pawn's normal height (might be crouching or a Scout, so use the max of current/default)
	FLOAT FullHeight = Max<FLOAT>(CylinderComponent->CollisionHeight, ((APawn*)(GetClass()->GetDefaultObject()))->CylinderComponent->CollisionHeight);
	FLOAT UpThreshold = UpThresholdAdjust + FullHeight + FullHeight - CylinderComponent->CollisionHeight;
			...
			if ( Hit.Normal.Z < WalkableFloorZ )
			{
				...
				if (bMustFall)
				{
					...
					if ( bFailed )
					{
						...
						APhysicsVolume *CurrentPhysicsVolume = GWorld->GetWorldInfo()->GetPhysicsVolume(Hit.Location,NULL,false);
...
void APawn::SetTempLocation( FVector NewLocation, FRotator NewRotation )
{
	FCheckResult Hit(1.0f);

	Location = NewLocation;
	Rotation = NewRotation;
	ForceUpdateComponents( GWorld->InTick && GWorld->TickGroup != TG_PostAsyncWork );


	//GWorld->MoveActor( this, NewLocation - Location, NewRotation, 0, Hit );
	//Location = NewLocation;
	//Rotation = NewRotation;
	//ForceUpdateComponents( GWorld->InTick );
}
