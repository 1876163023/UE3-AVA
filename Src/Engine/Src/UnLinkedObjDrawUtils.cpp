...
FIntPoint FLinkedObjDrawUtils::GetVariableConnectorsSize(FCanvas* Canvas, const FLinkedObjDrawInfo& ObjInfo)
{
	...
	{
		INT XL, YL;
		StringSize( GEngine->SmallFont, XL, YL, *ObjInfo.Variables(i).Name );
	...
	{
		INT XL, YL;
		StringSize( GEngine->SmallFont, XL, YL, *ObjInfo.Events(idx).Name );

		MaxVarX = Max(XL, MaxVarX);
		MaxVarY = Max(YL, MaxVarY);
	}

	const INT NeededX = ((MaxVarX + LO_DESC_X_PADDING) * (ObjInfo.Variables.Num() + ObjInfo.Events.Num())) + (2*LO_TEXT_BORDER);
	INT const NeededY = MaxVarY + LO_TEXT_BORDER;

	return FIntPoint(NeededX, NeededY);
}


void FLinkedObjDrawUtils::DrawVariableConnectors(FCanvas* Canvas, FLinkedObjDrawInfo& ObjInfo, const FIntPoint& Pos, const FIntPoint& Size)
{
	...
	const INT VarWidth	= MaxVarX + LO_DESC_X_PADDING;
	const INT VarRangeX	= Size.X - (2*LO_TEXT_BORDER);
	const INT CenterX	= Pos.X + LO_TEXT_BORDER + VarRangeX / 2;

	const INT SpacingX	= VarRangeX/(ObjInfo.Variables.Num()+ObjInfo.Events.Num());
	const INT StartX	= CenterX - (ObjInfo.Variables.Num()+ObjInfo.Events.Num()-1) * SpacingX / 2;
	const INT LabelY	= Pos.Y + Size.Y - LO_TEXT_BORDER - MaxVarY;

	// Draw variables.
	ObjInfo.VariableX.Add( ObjInfo.Variables.Num() );
	for(INT i=0; i<ObjInfo.Variables.Num(); i++)
	{
		const INT VarX			= StartX + i * SpacingX;
		ObjInfo.VariableX(i)	= VarX;

		if ( bSufficientlyZoomedIn )
		{
			if(bHitTesting) Canvas->SetHitProxy( new HLinkedObjConnectorProxy(ObjInfo.ObjObject, LOC_VARIABLE, i) );
			if (ObjInfo.Variables(i).bOutput)
			{
				FIntPoint Vert0, Vert1, Vert2;
				Vert0.X = -2 + VarX - LO_CONNECTOR_WIDTH/2;
				...
				DrawTriangle2D(Canvas,
								...
								Vert1,FVector2D(0,0),
								Vert2,FVector2D(0,0),
								ObjInfo.Variables(i).Color);
			}
			else
			{
				DrawTile( Canvas, VarX - LO_CONNECTOR_WIDTH / 2, Pos.Y + Size.Y, LO_CONNECTOR_WIDTH, LO_CONNECTOR_LENGTH, 0.f, 0.f, 0.f, 0.f, ObjInfo.Variables(i).Color );
			}
			if(bHitTesting) Canvas->SetHitProxy( NULL );

			INT XL, YL;
			StringSize( GEngine->SmallFont, XL, YL, *ObjInfo.Variables(i).Name );
			if ( AABBLiesWithinViewport( Canvas, VarX - XL / 2, LabelY, XL, YL ) )
			{
				DrawShadowedString( Canvas, VarX - XL / 2, LabelY, *ObjInfo.Variables(i).Name, GEngine->SmallFont, FLinearColor::White );
			}
		}
	}
	...
	for (INT idx = 0; idx < ObjInfo.Events.Num(); idx++)
	{
		const INT VarX		= StartX + ((ObjInfo.Variables.Num()+idx) * SpacingX);
		ObjInfo.EventX(idx)	= VarX;

		if ( bSufficientlyZoomedIn )
		{
			if(bHitTesting) Canvas->SetHitProxy( new HLinkedObjConnectorProxy(ObjInfo.ObjObject, LOC_EVENT, idx) );
			DrawTile( Canvas, VarX - LO_CONNECTOR_WIDTH / 2, Pos.Y + Size.Y, LO_CONNECTOR_WIDTH, LO_CONNECTOR_LENGTH, 0.f, 0.f, 0.f, 0.f, ObjInfo.Events(idx).Color );

			// draw an extra bar to indicate non-optional links
			DrawTile( Canvas, VarX - LO_CONNECTOR_WIDTH / 2 - 1, Pos.Y + Size.Y, LO_CONNECTOR_WIDTH + 2, 2, 0.f, 0.f, 0.f, 0.f, FColor(0,0,0) );
			if(bHitTesting) Canvas->SetHitProxy( NULL );
			...
			if ( AABBLiesWithinViewport( Canvas, VarX - XL / 2, LabelY, XL, YL ) )
			{
				DrawShadowedString( Canvas, VarX - XL / 2, LabelY, *ObjInfo.Events(idx).Name, GEngine->SmallFont, FLinearColor::White );
			}
		}
	}
}
...
void FLinkedObjDrawUtils::DrawLinkedObj(FCanvas* Canvas, FLinkedObjDrawInfo& ObjInfo, const TCHAR* Name, const TCHAR* Comment, const FColor& BorderColor, const FColor& TitleBkgColor, const FIntPoint& Pos)
{
	...
	DrawVariableConnectors(Canvas, ObjInfo, Pos + FIntPoint(0, TitleSize.Y + 1 + LogicSize.Y), FIntPoint(ObjInfo.DrawWidth, VarSize.Y));
}
 ...
 * Convenience function for filtering calls to FRenderInterface::DrawTile via AABBLiesWithinViewport.
 */
void FLinkedObjDrawUtils::DrawTile(FCanvas* Canvas,FLOAT X,FLOAT Y,FLOAT SizeX,FLOAT SizeY,FLOAT U,FLOAT V,FLOAT SizeU,FLOAT SizeV,FMaterialInstance* MaterialInstance)
{
	if ( AABBLiesWithinViewport( Canvas, X, Y, SizeX, SizeY ) )
	{
		::DrawTile(Canvas,X,Y,SizeX,SizeY,U,V,SizeU,SizeV,MaterialInstance);
	}
}
