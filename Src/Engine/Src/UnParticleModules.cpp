...
void UParticleModule::GetCurveObjects(TArray<UObject*>& OutCurves)
{
	for (TFieldIterator<UStructProperty> It(GetClass()); It; ++It)
	{
		...
		if (Distribution)
		{
			OutCurves.AddItem(Distribution);
		}
	}
}
...
void UParticleModule::AddModuleCurvesToEditor(UInterpCurveEdSetup* EdSetup)
{
	...
	for (TFieldIterator<UStructProperty> It(GetClass()); It; ++It)
	{
		...
		if (Distribution)
		{
			EdSetup->AddCurveToCurrentTab(Distribution, It->GetName(), ModuleEditorColor, bCurvesAsColor);
		}
	}
}
...
void UParticleModule::ChangeEditorColor(FColor& Color, UInterpCurveEdSetup* EdSetup)
{
	...
	for (INT TabIndex = 0; TabIndex < EdSetup->Tabs.Num(); TabIndex++)
	{
		...
		for (INT CurveIndex = 0; CurveIndex < Tab->Curves.Num(); CurveIndex++)
		{
			...
			{
				if (Curves(MyCurveIndex) == Entry->CurveObject)
				{
...
UBOOL UParticleModule::ConvertFloatDistribution(UDistributionFloat* FloatDist, UDistributionFloat* SourceFloatDist, FLOAT Percentage)
{
	...
	if (DistConstant)
	{
		...
		DistConstant->Constant	= SourceConstant->Constant * Multiplier;
//		DistConstant->Constant *= Multiplier;
	}
	else
...
***/
	...
	if (DistUniform)
	{
		DistUniform->Min	*= Multiplier;
		DistUniform->Max	*= Multiplier;
	}
	else
	if (DistUniformCurve)
	{
...
UBOOL UParticleModule::ConvertVectorDistribution(UDistributionVector* VectorDist, UDistributionVector* SourceVectorDist, FLOAT Percentage)
{
	...
	if (DistConstant)
	{
//		UDistributionVectorConstant*	SourceConstant	= Cast<UDistributionVectorConstant>(SourceVectorDist);
//		check(SourceConstant);
//		DistConstant->Constant	= SourceConstant->Constant * Multiplier;
	...
	if (DistConstantCurve)
	{
//		UDistributionVectorConstantCurve* SourceConstantCurve	= Cast<UDistributionVectorConstantCurve>(SourceVectorDist);
//		check(SourceConstantCurve);
//		SourceConstantCurve->ConstantCurve;
		for (INT KeyIndex = 0; KeyIndex < DistConstantCurve->GetNumKeys(); KeyIndex++)
		{
			for (INT SubIndex = 0; SubIndex < DistConstantCurve->GetNumSubCurves(); SubIndex++)
			{
...
void UParticleModuleSizeScale::UpdateEditor(FParticleEmitterInstance* Owner, INT Offset, FLOAT DeltaTime, UParticleModule* LowerLODModule, FLOAT Multiplier)
{
	...
	BEGIN_UPDATE_LOOP;
		...
		FVector ScaleFactorB	= SizeScale.GetValue(Particle.RelativeTime, Owner->Component);
...
UINT UParticleModuleCollision::RequiredBytes(FParticleEmitterInstance* Owner)
{
	...
	// INT		MaxCollisions;
	return (2 * sizeof(FVector)) + sizeof(INT);	
}
...
void UParticleModuleCollision::Spawn(FParticleEmitterInstance* Owner, INT Offset, FLOAT SpawnTime)
{
	...
	PARTICLE_ELEMENT(FVector,	UsedDampingFactorRotation);
	PARTICLE_ELEMENT(INT,		UsedMaxCollisions);
	UsedDampingFactor			= DampingFactor.GetValue(Owner->EmitterTime, Owner->Component);
	UsedDampingFactorRotation	= DampingFactorRotation.GetValue(Owner->EmitterTime, Owner->Component);
	UsedMaxCollisions			= appTrunc(MaxCollisions.GetValue(Owner->EmitterTime, Owner->Component));
}
		...
		if (!Owner->Component->SingleLineCheck(Hit, Actor, Location + Direction * Size / DirScalar, OldLocation, TRACE_ProjTargets | TRACE_AllBlocking, Extent))
		{
			...
			if (UsedMaxCollisions > 0)
			{
				if (LODLevel->RequiredModule->bUseLocalSpace)
				{
					...
					BaseVelocity			= BaseVelocity.MirrorByVector(Hit.Normal) * UsedDampingFactor;

					Particle.BaseVelocity		= Owner->Component->LocalToWorld.Inverse().TransformNormal(BaseVelocity);
					Particle.BaseRotationRate	= Particle.BaseRotationRate * UsedDampingFactorRotation.X;
					if (pkMeshInst && pkMeshInst->MeshRotationActive)
					{
						FMeshRotationPayloadData* PayloadData = (FMeshRotationPayloadData*)((BYTE*)&Particle + pkMeshInst->MeshRotationOffset);
						PayloadData->RotationRateBase *= UsedDampingFactorRotation;
					}

					// Reset the current velocity and manually adjust location to bounce off based on normal and time of collision.
					FVector NewVelocity	= Direction.MirrorByVector(Hit.Normal) * (Location - OldLocation).Size() * UsedDampingFactor;
				...
				else
				{
					...
					// Reset the current velocity and manually adjust location to bounce off based on normal and time of collision.
					FVector vNewVelocity	= Direction.MirrorByVector(Hit.Normal) * (Location - OldLocation).Size() * UsedDampingFactor;
					Particle.Velocity		= FVector(0.f);
...
void UParticleModuleCollision::SpawnEditor(FParticleEmitterInstance* Owner, INT Offset, FLOAT SpawnTime, UParticleModule* LowerLODModule, FLOAT Multiplier)
{
	...
	UsedDampingFactor			= (UsedDampingFactorA * Multiplier) + (UsedDampingFactorB * (1.0f - Multiplier));
	UsedDampingFactorRotation	= (UsedDampingFactorRotationA * Multiplier) + (UsedDampingFactorRotationB * (1.0f - Multiplier));
	UsedMaxCollisions			= appTrunc((MaxCollisionsA * Multiplier) + (MaxCollisionsB * (1.0f - Multiplier)));
}
...
{
	// disable emitters with collision at the lowest LOD level
