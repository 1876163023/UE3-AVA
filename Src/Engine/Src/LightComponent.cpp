...
UBOOL ULightComponent::AffectsPrimitive(const UPrimitiveComponent* Primitive) const
{
	...
	UPackage* PrimitiveOutermost	= Primitive->GetOutermost();
	...
	if( (PrimitiveOutermost->PackageFlags & PKG_SelfContainedLighting) && HasStaticShadowing() )
	{
		...
		if( LightOutermost != PrimitiveOutermost )
		{
			return FALSE;
		}
	}
	...
	if( bOnlyAffectSameAndSpecifiedLevels )
	{
		...
		FName PrimitiveLevelName		= PrimitiveOutermost->GetFName();
...
UBOOL ULightComponent::IsDepthDrawingLight() const
{
	return FALSE;
}
...
void ULightComponent::PostEditChange(UProperty* PropertyThatChanged)
{
	...
	do 
	{	
		/// For sunlight component :)
		if (PropertyName == TEXT("MinAutoExposure") ||
			PropertyName == TEXT("MaxAutoExposure") ||
			PropertyName == TEXT("KeyValue") ||
			PropertyName == TEXT("bUseDynamicTonemapping"))
		{
			break;
		}
...
void ULightComponent::UpdateVolumes()
{
	...
	ExclusionConvexVolumes.Empty();
	for( INT VolumeIndex=0; VolumeIndex<ExclusionVolumes.Num(); VolumeIndex++ )
	{
		ABrush*& Brush = ExclusionVolumes(VolumeIndex);
		if( Brush )
		{
			// Make sure there's no chance of cross-level references.
			if( Brush->GetOutermost() != GetOutermost() )
			{
				Brush = NULL;
			}
			// Convert the volumes to a series of convex volumes.
			else if( Brush->BrushComponent )
			{
				Brush->BrushComponent->AddConvexVolumes( ExclusionConvexVolumes, TRUE );
			}
		}
	}

	// Re-populate convex inclusion volumes.
	InclusionConvexVolumes.Empty();
	for( INT VolumeIndex=0; VolumeIndex<InclusionVolumes.Num(); VolumeIndex++ )
	{
		...
		if( Brush )
		{
			...
			{
				Brush->BrushComponent->AddConvexVolumes( InclusionConvexVolumes, TRUE );
			}
		}
	}
}
...
static void UpdateLightEnvironments(ULightComponent* Light)
{
	if (GIsGame)
		return;
 ...
 */
void ULightComponent::execSetEnabled _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void ULightComponent::execSetLightProperties _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void ULightComponent::execGetOrigin _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void ULightComponent::execGetDirection _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
