...
IMPLEMENT_CLASS(UUIAction);
	...
	IMPLEMENT_CLASS(UUIAction_Scene);
		...
		//<@ ava specific ; 2006. 9. 14 changmin
		IMPLEMENT_CLASS(UUIAction_SetSceneDisplayCursorProperty);
		//>@ ava
	...
	IMPLEMENT_CLASS(UUIAction_GetValue);
		IMPLEMENT_CLASS(UUIAction_GetLabelText);
...
IMPLEMENT_CLASS(UUIEvent);
	...
	IMPLEMENT_CLASS(UUIEvent_OnRightClick);		/** 오른쪽 마우스 클릭 */
	IMPLEMENT_CLASS(UUIEvent_OnDoubleClick);	/**< 2006/12/14 */
	IMPLEMENT_CLASS(UUIEvent_Show);				/**< 2006/12/15 */
	IMPLEMENT_CLASS(UUIEvent_Hidden);			/**< 2006/12/15 */
	IMPLEMENT_CLASS(UUIEvent_ValueChanged);
		...
		IMPLEMENT_CLASS(UUIEvent_ListActiveIndexChanged);
		IMPLEMENT_CLASS(UUIEvent_ListTopIndexChanged);	//!< 2007/01/25
		IMPLEMENT_CLASS(UUIEvent_ComboboxValueChanged);
		IMPLEMENT_CLASS(UUIEvent_OnChar);	/**< 2007/04/09 */
		IMPLEMENT_CLASS(UUIEvent_OnGainFocus); /** 포커스 얻음 */
		IMPLEMENT_CLASS(UUIEvent_OnLoseFocus); /** 포커스 잃음 */
...
void UUISequence::execAddSequenceObject _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
/** Unrealscript stubs */
void UUIEvent::execCanBeActivated _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIEvent::execConditionalActivateUIEvent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(ControllerIndex);
	P_GET_OBJECT(UUIScreenObject,InEventOwner);
	P_GET_OBJECT_OPTX(UObject,InEventActivator,NULL);
...
void UUIEvent::execActivateUIEvent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(ControllerIndex);
	P_GET_OBJECT(UUIScreenObject,InEventOwner);
	P_GET_OBJECT_OPTX(UObject,InEventActivator,NULL);
 ...
 * @return	TRUE if the event was activated successfully, FALSE if the event couldn't be activated.
 */
UBOOL UUIEvent::ActivateUIEvent( INT ControllerIndex, UUIScreenObject* InEventOwner, UObject* InEventActivator/*=NULL*/, UBOOL bActivateImmediately/*=FALSE*/, const TArray<INT>* IndicesToActivate/*=NULL*/ )
{
...
void UUIEvent_MetaObject::GenerateActionDescription(const FInputKeyAction &InAction, FString& OutDescription)
{
	...
	UUIInteraction* Interaction = UUIInteraction::StaticClass()->GetDefaultObject<UUIInteraction>();
	UUIState* State = GetOwnerState();
	...
	FUIInputAliasClassMap* WidgetKeyMapping = Interaction->WidgetInputAliasLookupTable.FindRef(OwnerWidget->GetClass());
...
void UUIEvent_State::InitializeLinkedVariableValues()
{
	...
	if ( EventOwner != NULL )
	{
		UUIState* CurrentState = EventOwner->GetCurrentState(PlayerIndex);
		if ( CurrentState != NULL )
		{
...
* member variable of this op, or for VariableLinks that are used in the execution of this ops logic.
*
* Initializes the value of the "New Value" VariableLink
*/
void UUIEvent_CheckValueChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();
	...
	UUICheckbox* CheckboxTarget = CastChecked<UUICheckbox>(EventActivator);
	...
	{
		*(BoolVars(Idx)) = CheckboxTarget->bIsChecked;
	}
}

/* ==========================================================================================================
	UUIEvent_SliderValueChanged
========================================================================================================== */
/**
 * Allows the operation to initialize the values for any VariableLinks that need to be filled prior to executing this
 * op's logic.  This is a convenient hook for filling VariableLinks that aren't necessarily associated with an actual
 ...
 * Initializes the value of the "New Value" VariableLink
 */
void UUIEvent_SliderValueChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();

	// only sliders may activate this event
	UUISlider* SliderTarget = Cast<UUISlider>(EventActivator);

	if ( SliderTarget != NULL )
	{
		const FLOAT SliderValue = SliderTarget->GetValue();
		...
		{
			*(FloatVars(Idx)) = SliderValue;
		}
	}
}

/* ==========================================================================================================
	UUIEvent_ProgressBarValueChanged
========================================================================================================== */
/**
 * Allows the operation to initialize the values for any VariableLinks that need to be filled prior to executing this
 * op's logic.  This is a convenient hook for filling VariableLinks that aren't necessarily associated with an actual
 ...
 * Initializes the value of the "New Value" VariableLink
 */
void UUIEvent_ProgressBarValueChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();

	// only progressbars may activate this event
	UUIProgressBar* Target = Cast<UUIProgressBar>(EventActivator);

	if ( Target != NULL )
	{
		const FLOAT ProgressBarValue = Target->GetValue();
		...
		for (INT Idx = 0; Idx < FloatVars.Num(); Idx++)
		{
			*(FloatVars(Idx)) = ProgressBarValue;
		}
	}
}

/* ==========================================================================================================
	UUIEvent_TextValueChanged
========================================================================================================== */
/**
 * Allows the operation to initialize the values for any VariableLinks that need to be filled prior to executing this
 * op's logic.  This is a convenient hook for filling VariableLinks that aren't necessarily associated with an actual
 * member variable of this op, or for VariableLinks that are used in the execution of this ops logic.
 *
 * Initializes the value of the "Value" VariableLink
 */
void UUIEvent_TextValueChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();

	// only UIEditboxes may activate this event
	UUIEditBox* EditboxTarget = CastChecked<UUIEditBox>(EventActivator);

	// copy the current value of the editbox into the "Value" variable link
	TArray<FString*> StringVars;
	GetStringVars(StringVars,TEXT("Value"));

	for (INT Idx = 0; Idx < StringVars.Num(); Idx++)
	{
		*(StringVars(Idx)) = EditboxTarget->StringRenderComponent->UserText;
	}
}

/* ==========================================================================================================
	UUIEvent_ListIndexChanged
========================================================================================================== */
/**
 * Allows the operation to initialize the values for any VariableLinks that need to be filled prior to executing this
 * op's logic.  This is a convenient hook for filling VariableLinks that aren't necessarily associated with an actual
 ...
 * Initializes the value of the "New Value" VariableLink
 */
void UUIEvent_ListIndexChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();

	// only sliders may activate this event
	UUIList* ListTarget = CastChecked<UUIList>(EventActivator);

	TArray<INT*> CurrentIndexVars, PreviousIndexVars;
	GetIntVars(PreviousIndexVars,TEXT("Old Index"));
	GetIntVars(CurrentIndexVars,TEXT("New Index"));

	// copy the current value of PreviousIndex into the "Old Index" variable link
	for ( INT VarIndex = 0; VarIndex < PreviousIndexVars.Num(); VarIndex++ )
	{
		*PreviousIndexVars(VarIndex) = PreviousIndex;
	}

	// copy the current value of CurrentIndex into the "New Index" variable link
	for ( INT VarIndex = 0; VarIndex < CurrentIndexVars.Num(); VarIndex++ )
	{
		*CurrentIndexVars(VarIndex) = CurrentIndex;
	}
}


void UUIEvent_ListActiveIndexChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();
...
void UUIEvent_ListTopIndexChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();
	...
	TArray<INT*> CurrentTopIndexVars, PreviousTopIndexVars;
	GetIntVars(PreviousTopIndexVars,TEXT("Old TopIndex"));
	GetIntVars(CurrentTopIndexVars,TEXT("New TopIndex"));

	// copy the current value of PreviousIndex into the "Old Index" variable link
	for ( INT VarIndex = 0; VarIndex < PreviousTopIndexVars.Num(); VarIndex++ )
	{
		*PreviousTopIndexVars(VarIndex) = PreviousTopIndex;
	}

	// copy the current value of CurrentIndex into the "New Index" variable link
	for ( INT VarIndex = 0; VarIndex < CurrentTopIndexVars.Num(); VarIndex++ )
	{
		*CurrentTopIndexVars(VarIndex) = CurrentTopIndex;
	}
}

/* ==========================================================================================================
	UIEvent_ComboboxValueChanged
========================================================================================================== */
/**
 * Allows the operation to initialize the values for any VariableLinks that need to be filled prior to executing this
 * op's logic.  This is a convenient hook for filling VariableLinks that aren't necessarily associated with an actual
 ...
 */
void UUIEvent_ComboboxValueChanged::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();

	// only UIComboBox may activate this event
	UUIComboBox* ComboboxActivator = CastChecked<UUIComboBox>(EventActivator);

	if ( ComboboxActivator->ComboEditbox != NULL && ComboboxActivator->ComboEditbox->StringRenderComponent != NULL )
	{
		// copy the current value of the editbox into the "Text Value" variable link
		TArray<FString*> StringVars;
		GetStringVars(StringVars,TEXT("Text Value"));
		for (INT Idx = 0; Idx < StringVars.Num(); Idx++)
		{
			*(StringVars(Idx)) = ComboboxActivator->ComboEditbox->StringRenderComponent->UserText;
		}
	}

	if ( ComboboxActivator->ComboList != NULL )
	{	
		INT CurrentItem = INDEX_NONE, CurrentIndex = INDEX_NONE;

		// if this event was activated because the user pressed Enter or selected a list item, fill in the index values with valid values
		if ( OutputLinks(1).bHasImpulse )
		{
			CurrentItem = ComboboxActivator->ComboList->GetCurrentItem();
			CurrentIndex = ComboboxActivator->ComboList->Index;
		}

		// combo the value of the list's selected items into the "List Data Source Index" variable link
		TArray<INT*> IntVars;
		...
		IntVars.Empty();
		GetIntVars(IntVars, TEXT("Current List Index"));
		for (INT Idx = 0; Idx < IntVars.Num(); Idx++)
		{
 ...
 */
void UUIEvent_SubmitTextData::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();

	// see if any Value variables are attached
	TArray<FString*> StringVars;
	GetStringVars(StringVars,TEXT("Value"));

	// if so, initialize the value of the linked variable with the current value of "Value"
	for ( INT Idx = 0; Idx < StringVars.Num(); Idx++ )
	{
 ...
 */
void UUIEvent_SubmitListData::InitializeLinkedVariableValues()
{
	Super::InitializeLinkedVariableValues();

	UUIList* ActivatorList = Cast<UUIList>(EventActivator);
	if ( ActivatorList != NULL )
	{
		SelectedItem = ActivatorList->GetCurrentItem();

		// see if any int variables were attached to the Selected Item variable link
		TArray<INT*> IntVars;
		GetIntVars(IntVars,TEXT("Selected Item"));

		// if so, initialize the value of the linked variables with the activator list's currently selected item
		for ( INT VarIndex = 0; VarIndex < IntVars.Num(); VarIndex++ )
		{
			*(IntVars(VarIndex)) = SelectedItem;
		}
	}
}
...
void UUIAction_OpenScene::Activated()
{
	...
	if ( SceneClient != NULL )
	{
		...
		{
			AGameInfo* GameInfo = AGameInfo::StaticClass()->GetDefaultObject<AGameInfo>();
			check(GameInfo);
			UUIScene* SceneTemplate = Cast<UUIScene>( StaticLoadObject( UUIScene::StaticClass(), NULL, *GameInfo->GetFullUIPath(SceneName), NULL, LOAD_NoWarn, NULL ) );
			if (SceneTemplate != NULL)
			{
				PlayerOwner = SceneTemplate->GetPlayerOwner(PlayerIndex);

				bSceneOpenedSuccessfully = SceneClient->OpenScene(SceneTemplate,PlayerOwner,&OpenedScene,Priority);
			}		
			else if( Scene != NULL )
			{
		...
		if (!bSceneOpenedSuccessfully)
		{
			...
			{
				bSceneOpenedSuccessfully = SceneClient->OpenScene(Scene,PlayerOwner,&OpenedScene,Priority);
			}			
		}
	}
...
void UUIAction_OpenScene::PostLoad()
{
	Super::PostLoad();

	//debugf(TEXT("PostLoad@UIAction_OpenScene : FullPath = %s, Scene = %s, SceneName = %s"), *GetPathName(), Scene ? *Scene->GetPathName() : GNone, *SceneName);
...
void UUIAction_CloseScene::Activated()
{
	...
	if ( SceneClient != NULL )
	{
		TArray<FName> SceneTagsToClose;
		SceneTagsToClose.Append( SceneTags );
		if( Scene != NULL )
			SceneTagsToClose.AddItem(Scene->SceneTag);

		for( INT SceneTagIndex = 0 ; SceneTagIndex < SceneTagsToClose.Num() ; SceneTagIndex++ )
		{
			UUIScene* SceneToClose = SceneClient->FindSceneByTag(SceneTagsToClose(SceneTagIndex));
			if( SceneToClose != NULL )
			{
				bSceneClosedSuccessfully = bSceneClosedSuccessfully || SceneClient->CloseScene(SceneToClose);
				if( bSceneClosedSuccessfully )
					debugf( NAME_Log, TEXT("Scene %s is closed successfully"), SceneToClose->SceneTag.GetName() );
			}
		}

		if( Scene == NULL && SceneTagsToClose.Num() == 0 )
		{
			UUIScreenObject* OwnerWidget = GetOwner();
			if( bAutoTargetOwner && OwnerWidget != NULL )
			{
				UUIScene* SceneToClose = OwnerWidget->GetScene();
				if( SceneToClose != NULL )
		...
		//if ( SceneTag != NAME_None && SceneToClose == NULL )
		//{			
		//	SceneToClose = SceneClient->FindSceneByTag( SceneTag );

		//	debugf( NAME_Log, TEXT("Trying to close scene by SceneTag '%s'"), *SceneTag.ToString() );
		...
		//		if (bSceneClosedSuccessfully)
		//		{
		//			debugf( NAME_Log, TEXT("Successfully scene closed") );
		//		}				
		//	}
		//}
		//else 
...
void UUIAction_GetListIndex::Activated()
{
	...
	TArray<INT*> IntVars;
	GetIntVars(IntVars,TEXT("Current Item"));
	TArray<INT*> IndexVars;
	GetIntVars(IndexVars,TEXT("Current Index"));
	TArray<INT*> TopIndexVars;
	GetIntVars( TopIndexVars, TEXT("Top Index"));
	...
	else
	{
		...
		for ( INT Index = 0; Index < TargetLists.Num(); Index++ )
		{
			...
			if ( TopIndexVars.IsValidIndex(Index) )
			{
				INT& TopIndexValue = *TopIndexVars(Index);
				TopIndexValue = SourceList->TopIndex;
			}
			// CurrentIndex에 대한 처리를 추가
			// CurrentIndex는 리스트의 첫 엘리먼트를 0으로 매 엘리먼트마다 1씩 증가하는
			// 가장 일반적?인 리스트인덱스이다.
			if( IndexVars.IsValidIndex(Index))
			{
				INT& IndexValue = *IndexVars(Index);
				IndexValue = SourceList->Index;
			}
...
void UUIAction_AddDataField::Activated()
{
	...
	if ( ResolveMarkup(FieldProvider, FieldName) )
	{
		...
		{
			bSuccess = DynamicFieldProvider->AddField(*FieldName, bPersistentField);
		}
	}
...
#else
	// Just say the player has all access on PC.
	OutputLinks(2).ActivateOutputLink();
#endif
...
void UUIAction_GetLoggedInPlayerCount::Activated(void)
{
	...
	eventGetLoginStatus();
#else
	//@todo: Just say all players are logged in for PC for now
	TotalLoggedIn=MaxPlayers;
	NumOnlineEnabled=MaxPlayers;
	NumLocalOnly=0;
#endif
	Super::Activated();
}
...
/* ==========================================================================================================
	UUIAction_GetLabelText
========================================================================================================== */
// added by alcor

void UUIAction_GetLabelText::Activated()
{
	if (Targets.Num() > 0)
	{
		UUILabel *Label = Cast<UUILabel>(Targets(0));
		if ( Label != NULL )
		{
			OutText = Label->StringRenderComponent->GetValue();
			return;
		}

		UUILabelButton *Button = Cast<UUILabelButton>(Targets(0));
		if ( Button != NULL )
		{
			OutText = Button->StringRenderComponent->GetValue();
			return;
		}

		UUIEditBox *EditBox = Cast<UUIEditBox>(Targets(0));
		if ( EditBox != NULL )
		{
			OutText = EditBox->GetValue();
			return;
		}
	}
...
/**
* Set Scene's bDisplayCursor Property
*/
void UUIAction_SetSceneDisplayCursorProperty::Activated()
{
	Super::Activated();
	...
	{
		// if we didn't specify a scene, close the scene that this UIAction belongs to
		UUIScene* SceneToSetProperty = Scene;
		if ( SceneToSetProperty == NULL )
		{
			UUIScreenObject* OwnerWidget = GetOwner();
			if ( bAutoTargetOwner && OwnerWidget != NULL )
			{
				SceneToSetProperty = OwnerWidget->GetScene();
			}
		}

		if( SceneToSetProperty )
		{
			SceneToSetProperty->bDisplayCursor = bDisplayCursor;
			bResult = TRUE;
		}
	}
