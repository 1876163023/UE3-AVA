...
void AutoInitializeRegistrantsEngine( INT& Lookup )
{
	...
	AUTO_INITIALIZE_REGISTRANTS_ENGINE_DSP
 ...
 */
void LoadSpecialMaterial(const FString& MaterialName, UMaterial*& Material)
{
	...
	if (Material == NULL)
	{
		...
		else if (!Material->bUsedAsSpecialEngineMaterial) 
		{
			...
			Material->MaterialResource->InitShaderMap();
...
void UEngine::InitializeObjectReferences()
{
	...
	LoadSpecialMaterial(DefaultMaterialName, DefaultMaterial);
	LoadSpecialMaterial(WireframeMaterialName, WireframeMaterial);
	LoadSpecialMaterial(LightingOnlyMaterialName, LightingOnlyMaterial);
	LoadSpecialMaterial(SolidColorMaterialName, SolidColorMaterial);
	LoadSpecialMaterial(ColoredNodeMaterialName, ColoredNodeMaterial);
	LoadSpecialMaterial(EmissiveTexturedMaterialName, EmissiveTexturedMaterial);
	LoadSpecialMaterial(MissingCollisionMaterialName, MissingCollisionMaterial);
	LoadSpecialMaterial(LevelColorationLitMaterialName, LevelColorationLitMaterial);
	LoadSpecialMaterial(LevelColorationUnlitMaterialName, LevelColorationUnlitMaterial);
	LoadSpecialMaterial(PropertyColorationLitMaterialName, PropertyColorationLitMaterial);
	LoadSpecialMaterial(PropertyColorationUnlitMaterialName, PropertyColorationUnlitMaterial);
	LoadSpecialMaterial(SceneCaptureReflectActorMaterialName, SceneCaptureReflectActorMaterial);
	LoadSpecialMaterial(SceneCaptureCubeActorMaterialName, SceneCaptureCubeActorMaterial);
	LoadSpecialMaterial(TerrainCollisionMaterialName, TerrainCollisionMaterial);
	LoadSpecialMaterial(EnvCubeActorMaterialName, EnvCubeActorMaterial);

	if( DefaultUICaretMaterial == NULL )
	{
		DefaultUICaretMaterial = LoadObject<UMaterial>(NULL, *DefaultUICaretMaterialName, NULL, LOAD_None, NULL);	
	}
	...
	if (GIsEditor == TRUE)
	{
		...
		{
			GeomMaterial = LoadObject<UMaterial>(NULL, *GeomMaterialName, NULL, LOAD_None, NULL);	
		}
		if ( TickMaterial == NULL )
		{
			TickMaterial = LoadObject<UMaterial>(NULL, *TickMaterialName, NULL, LOAD_None, NULL);	
		}
		if ( CrossMaterial == NULL )
		{
			CrossMaterial = LoadObject<UMaterial>(NULL, *CrossMaterialName, NULL, LOAD_None, NULL);	
		}
		if ( EditorBrushMaterial == NULL )
		{
			EditorBrushMaterial = LoadObject<UMaterial>(NULL, *EditorBrushMaterialName, NULL, LOAD_None, NULL);	
		}
	}
	else
	{
		if ( GeomMaterial == NULL )
		{
			GeomMaterial = LoadObject<UMaterial>(NULL, *DefaultMaterialName, NULL, LOAD_None, NULL);	
		}
		if ( TickMaterial == NULL )
		{
			TickMaterial = LoadObject<UMaterial>(NULL, *DefaultMaterialName, NULL, LOAD_None, NULL);	
		}
		if ( CrossMaterial == NULL )
		{
			CrossMaterial = LoadObject<UMaterial>(NULL, *DefaultMaterialName, NULL, LOAD_None, NULL);	
		}
		if ( EditorBrushMaterial == NULL )
		{
			EditorBrushMaterial = LoadObject<UMaterial>(NULL, *DefaultMaterialName, NULL, LOAD_None, NULL);	
		}
	}
...
void UEngine::CleanupGameViewport()
{
	...
	for(FPlayerIterator It(this);It;++It)
	{
		...
		{
			if (GEngine->GameViewport == It->ViewportClient)
			{
...
UBOOL UEngine::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
...
#if USING_REMOTECONTROL
	if( RemoteControlExec	&&	RemoteControlExec->Exec	(Cmd,Ar) ) return 1;
#endif
	if( GavaNetClient		&&	GavaNetClient->Exec	(Cmd,Ar) ) return 1;
	...
	else if( ParseCommand(&Cmd,TEXT("RecompileShaders")) )
	{
		...
		public:
			...
			~FTestTimer()
			{
				...
				debugf(TEXT("[%s] took [%4.2f] seconds"),*InfoStr,ElapsedTime);
			}
	...
	else if( ParseCommand(&Cmd,TEXT("TOGGLEVSM")) )
	{
		...
		debugf( TEXT("VSM Shadows %s"),bEnableVSMShadows ? TEXT("ENABLED") : TEXT("DISABLED") );
		return 1;
	}
	else if( ParseCommand(&Cmd,TEXT("TOGGLEBPCF")) )
	{
		...
		debugf( TEXT("Branching PCF Shadows %s"),bEnableBranchingPCFShadows ? TEXT("ENABLED") : TEXT("DISABLED") );
		return 1;
	} 
	else if( ParseCommand(&Cmd,TEXT("SHADOWQUALITY")) )
	{
		FString FlagStr(ParseToken(Cmd, 0));
		if( FlagStr.Len() > 0 )
		{
			if( appStricmp(*FlagStr,TEXT("0"))==0)
			{
				ShadowFilterQuality = 0;
			}
			else if( appStricmp(*FlagStr,TEXT("1"))==0)
			{
				ShadowFilterQuality = 1;
			}
			else if( appStricmp(*FlagStr,TEXT("2"))==0)
			{
				ShadowFilterQuality = 2;
			}

			warnf( TEXT("New Shadow Quality %i"), ShadowFilterQuality);
		} 
		else
		{
			Ar.Logf( TEXT("Format is 'shadowquality [0-2]'"));
		}
		return 1; 
	}
	...
	else if( ParseCommand(&Cmd,TEXT("SHADOWDEPTHBIAS")) )
	{
		...
		if(NewDepthBias > 0.0f)
		{
			...
			debugf( TEXT("New Depth Bias %f"), DepthBias);
		} 
		else
		{
			Ar.Logf( TEXT("Format is 'shadowdepthbias [float]'"));
		}
		return 1;
	}
...
#endif
	//<@ ava specific 2007. 1. 4n changmin
	else if( ParseCommand( &Cmd, TEXT("LockPvs")) )
	{
		if( ParseCommand(&Cmd, TEXT("TRUE")) )
		{
			if( GWorld->PersistentLevel->StructuralModel->LeafBytes > 0 )
			{
				LockPvs = TRUE;
			}
			else
			{
				appMsgf( AMT_OK, TEXT("Invalid operation. No PVS data in this level.") );
				LockPvs = FALSE;
			}
		}
	...
	// {{ 20071016 dEAthcURe|HM
	#ifdef EnableHmFastLoading
	else if (ParseCommand(&Cmd, TEXT("ENABLEFASTHM"))) {
		extern bool GEnableHmFastLoading;
		GEnableHmFastLoading = true;
		Ar.Logf(TEXT("   FastHM enabled."));		
		return TRUE;
	}
	else if (ParseCommand(&Cmd, TEXT("DISABLEFASTHM"))) {
		extern bool GEnableHmFastLoading;
		GEnableHmFastLoading = false;
		Ar.Logf(TEXT("   FastHM disabled."));		
		return TRUE;
	}
	#endif
	// }} 20071016 dEAthcURe|HM
