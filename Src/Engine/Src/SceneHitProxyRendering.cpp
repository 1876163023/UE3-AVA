...
public:
	...
	{
		// Only compile the hit proxy shaders for the default material on PC D3D.
		return IsPCPlatform( Platform ) && (Material->IsSpecialEngineMaterial() || Material->IsMasked() || Material->IsTwoSided());
	}
...
void FHitProxyDrawingPolicyFactory::AddStaticMesh(FScene* Scene,FStaticMesh* StaticMesh,ContextType)
{
	...
	if( !StaticMesh->bRenderMesh ) return;
	//>@ ava
	...
	Scene->DPGs[StaticMesh->DepthPriorityGroup].HitProxyDrawList.AddMesh(
		...
		FHitProxyDrawingPolicy(StaticMesh->VertexFactory,MaterialInstance)
		);
}
	...
	)
{
	...
	FHitProxyDrawingPolicy DrawingPolicy( Mesh.VertexFactory, MaterialInstance );
...
void FSceneRenderer::RenderHitProxies()
{
	...
	for(UINT DPGIndex = 0;DPGIndex < SDPG_MAX_SceneRender;DPGIndex++)
	{
		...
		for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
		{
			...
			// Draw the scene's opaque emissive draw lists.
			Scene->DPGs[DPGIndex].HitProxyDrawList.DrawVisible(GlobalContext,&View,View.StaticMeshVisibilityMap);

			// Draw all dynamic primitives using an emissive drawing policy.
