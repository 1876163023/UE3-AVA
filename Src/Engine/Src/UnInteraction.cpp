...
void UInteraction::execInit _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
	Init();
}

void UInteraction::execInputKey _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(ControllerId);
	P_GET_NAME(Key);
	P_GET_BYTE(Event);
	P_GET_FLOAT_OPTX(AmountDepressed,1.f);
	P_GET_UBOOL_OPTX(bGamepad,FALSE);
	P_FINISH;
	*(UBOOL*)Result = InputKey(ControllerId,Key,(EInputEvent)Event,AmountDepressed,bGamepad);
}

void UInteraction::execInputAxis _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(ControllerId);
	P_GET_NAME(Key);
	P_GET_FLOAT(Delta);
	...
	P_FINISH;
	*(UBOOL*)Result = InputAxis(ControllerId,Key,Delta,DeltaTime);
}

void UInteraction::execInputChar _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(ControllerId);
	P_GET_STRUCT_REF(struct FInputCompositionStringData,CompStrData);
	P_FINISH;
//	check(Character.Len() == 1);
	*(UBOOL*)Result = InputChar(ControllerId,CompStrData);
}

void UInteraction::execInputCandidate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(ControllerId);
	P_GET_STRUCT_REF(struct FInputCandidateStringData,CandStrData);
	P_FINISH;

	*(UBOOL*)Result = InputCandidate(ControllerId, CandStrData);
}

void UInteraction::execInputReadingString _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(ControllerId);
	P_GET_STRUCT_REF(struct FInputReadingStringData	,ReadStrData);
	P_FINISH;

	*(UBOOL*)Result = InputReadingString(ControllerId, ReadStrData);
}
...
void UUIInteraction::execGetPlayerCount _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void UUIInteraction::execGetPlayerIndex _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIInteraction::execGetPlayerControllerId _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIInteraction::execGetDataStoreClient _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
UBOOL UUIInteraction::InputKey(INT ControllerId,FName Key,EInputEvent Event,FLOAT AmountDepressed/*=1.f*/,UBOOL bGamepad/*=FALSE*/)
{
	...
	{
		bResult = SceneClient->InputKey(ControllerId,Key,Event,AmountDepressed,bGamepad);
	}
 ...
 */
UBOOL UUIInteraction::InputAxis(INT ControllerId,FName Key,FLOAT Delta,FLOAT DeltaTime)
{
	UBOOL bResult = FALSE;
	...
	if ( bProcessInput == TRUE && SceneClient != NULL )
	{
		...
		if ( EmulationDef != NULL && EmulationDef->bEmulateButtonPress == TRUE )
		{
			...
			if ( PlayerIndex >= 0 && PlayerIndex < ARRAY_COUNT(AxisInputEmulation) && AxisInputEmulation[PlayerIndex].bEnabled == TRUE )
			{
				...
				{
					bResult = SceneClient->InputKey(ControllerId, EmulatedEventParms.InputKeyName, (EInputEvent)EmulatedEventParms.EventType);
				}
			}
		}

		bResult = bResult || SceneClient->InputAxis(ControllerId,Key,Delta,DeltaTime);
	}
 ...
 */
UBOOL UUIInteraction::InputChar(INT ControllerId, const FInputCompositionStringData& CompStrData )
{
	UBOOL bResult = FALSE;

	if ( bProcessInput && SceneClient != NULL )
	{
		bResult = SceneClient->InputChar(ControllerId, CompStrData );
	}

	return bResult;
}

UBOOL UUIInteraction::InputCandidate(INT ControllerId, const FInputCandidateStringData& CandStrData)
{
	UBOOL bResult = FALSE;

	if ( bProcessInput && SceneClient != NULL)
		bResult = SceneClient->InputCandidate(ControllerId, CandStrData);

	return bResult;
}

UBOOL UUIInteraction::InputReadingString(INT ControllerId, const FInputReadingStringData& ReadStrData)
{
	UBOOL bResult = FALSE;

	if ( bProcessInput && SceneClient != NULL)
		bResult = SceneClient->InputReadingString(ControllerId, ReadStrData);
...
*
* @param CanvasScene - scene for rendering 3D prims
*/
void UUIInteraction::InitUIPrimitiveScene( FCanvasScene* CanvasScene )
{
	check(CanvasScene);
...
* @param CanvasScene - scene for rendering 3D prims
*/
void UUIInteraction::UpdateUIPrimitiveScene( FCanvasScene* CanvasScene )
{
	check(CanvasScene);
