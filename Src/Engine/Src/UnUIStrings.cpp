...
void UUIString::StringSize( FRenderParameters& Parameters, const TCHAR* pText, const TCHAR* EOL/*=NULL*/ )
{
	...
	if( Parameters.DrawFont )
	{
		...
		const FLOAT CharIncrement = Parameters.DrawFont->Kerning * ScaleX;
		while ( *pCurrentPos )
		{
			...
			TCHAR Char = *pCurrentPos++;
			Parameters.DrawFont->GetCharSize(Char, CharWidth, CharHeight, 0, PrevChar);
			PrevChar = Char;		
...
void UUIString::ClipString( FRenderParameters& Parameters, const TCHAR* pText, FString& out_ResultString, EUIAlignment ClipAlignment/*=UIALIGN_Left*/ )
{
	...
	FString ClipStr = pText;

	//if ( *pText == 0 )
	//	return;
	if( Parameters.DrawFont == NULL || ClipStr.Trim().Len() == 0 )
		return;
	...
	const FLOAT CharSpacing = Parameters.DrawFont->Kerning;
	...
	DefaultCharWidth *= Parameters.Scaling.X;
	DefaultCharHeight *= Parameters.Scaling.Y;

	//const INT SourceStringLength = appStrlen(pText);
	const INT SourceStringLength = ClipStr.Len();
	...
	// 프로그램이 죽는걸 수정한 코드. ( 변경 2007.3.29 )
	// 과정이 많이 바뀌었음. 문자길이가 아직 남았는데도 글씨가 남은칸까지 그려지지 않는다면 버그리포팅 바람.
	//
	// tracks the current width/height of the output text...once this overflows ClipX, the function returns
	FLOAT LineXL = 0.f;
	FLOAT LineYL = DefaultCharHeight * Parameters.Scaling.Y;
	TCHAR PrevChar = '\0';
	INT ClippedNum = ClipStr.Len();

	if( ClipAlignment == UIALIGN_Left )
	{
		for(INT i = 0 ; i < ClipStr.Len() ; i++ )
		{
			FLOAT CharWidth = 0.f;
			FLOAT CharHeight = 0.f;
			Parameters.DrawFont->GetCharSize(ClipStr[i], CharWidth, CharHeight, 0, PrevChar);

			// 클리핑 사이즈를 넘어섰다. (지금 체크하는 문자부터는 잘려나간다)
			if( LineXL + CharWidth * Parameters.Scaling.X > ClipX )
			{
				ClippedNum = i;
				ClipStr = ClipStr.Left(ClippedNum);
				break;
			}

			// 아직 체크해볼 문자가 남아있다면 
			// 문자와 문자 사이에 들어가는 CharSpacing를 추가해준다.
			if( i < ClipStr.Len() -1)
				CharWidth += CharSpacing;

			LineXL += CharWidth * Parameters.Scaling.X;
			LineYL = Max( CharHeight , LineYL );
			PrevChar = ClipStr[i];
		}
	}
	else if ( ClipAlignment == UIALIGN_Right )
	{
		for(INT i = ClipStr.Len() - 1 ; i >= 0 ; i-- )
		{
			FLOAT CharWidth = 0.f;
			FLOAT CharHeight = 0.f;
			Parameters.DrawFont->GetCharSize(ClipStr[i], CharWidth, CharHeight, 0, PrevChar);
			...
			if( LineXL + CharWidth * Parameters.Scaling.X > ClipX )
			{
				ClippedNum = ClipStr.Len() - 1 - i;
				ClipStr = ClipStr.Right(ClippedNum);
				break;
			}
			...
			LineXL += CharWidth * Parameters.Scaling.X;
			LineYL = Max( CharHeight * Parameters.Scaling.Y , LineYL);
			PrevChar = ClipStr[i];
		}
	}
	else
	{
		checkf(FALSE, TEXT("Invalid clipAligment parameter"));
	}
	...
	//		pChar += CharIncrement;
	//		//! CharIncrement값이 음수일 경우도 있기 때문에, 모두 검사해 봐야 한다(2007/02/09 고광록).
	//		//! 근데 버그가 있네..ㅠㅠ
	////		if ( pChar < FirstCharacter || pChar > LastCharacter )
	...
	//		FLOAT CharWidth, CharHeight;
	//		Parameters.DrawFont->GetCharSize(CurrentCharacter, CharWidth, CharHeight, 0, PrevChar);
	...
	//		LineXL += CharWidth * Parameters.Scaling.X;
	//		LineYL = Max<FLOAT>(LineYL, CharHeight * Parameters.Scaling.Y);
	//
...
void UUIString::ApplyFormatting( FRenderParameters& Parameters, UBOOL bIgnoreMarkup )
{
	...
	for ( NodeIndex = 0; NodeIndex < Nodes.Num(); NodeIndex++ )
	{
		...
		if (StringClipMode == CLIP_Scaled || StringClipMode == CLIP_ScaledBestFit)
		{
	...
	if ( StringClipMode == CLIP_Scaled || StringClipMode == CLIP_ScaledBestFit )
	{
		...
		FLOAT ActualYL = Parameters.DrawYL - Parameters.DrawY;


		YScale = ActualYL / RawExtents.Y;
		XScale = Parameters.DrawXL / RawExtents.X;
		
		if ( StringClipMode == CLIP_ScaledBestFit )
		{
			if ( RawExtents.Y * XScale > ActualYL )
			{
				XScale = YScale;
			}
			else
	...
	for ( NodeIndex = 0; NodeIndex < Nodes.Num(); NodeIndex++ )
	{
		...
		if ( StringNode->Extent.X > 0 )
		{
			if ( StringNode->IsTextNode() )
			{
				...
				const UBOOL bOverflowsBounds = CurX + (StringNode->Extent.X * XScale) > ClipX + KINDA_SMALL_NUMBER;
				...
				if ( bOverflowsBounds || bShouldWrapNode )
				{
					...
					else if ( StringClipMode == CLIP_Ellipsis )
					{
						if ( bOverflowsBounds )
						{
					...
					else if ( bOverflowsBounds && StringClipMode == CLIP_Normal )
					{
					...
					{
						CurX += (StringNode->Extent.X * XScale);
						LastYL = Max( (StringNode->Extent.Y * YScale), LastYL);
					}
				}
				else	// bNeedsWrap
				{
				...
				if ( StringNode )
				{
					StringNode->Scaling.X = XScale;
					StringNode->Scaling.Y = YScale;
					StringNode->Extent.X *= XScale;
					StringNode->Extent.Y *= YScale;
				}
			}
			else
			{
				...
				if ( CurX + StringNode->Extent.X > ClipX + KINDA_SMALL_NUMBER )
				{
					...
					else if ( StringClipMode == CLIP_Normal )
					{
						// stop here
						CurX += StringNode->Extent.X;
...
void UUIString::Render_String( FCanvas* Canvas, const FRenderParameters& Parameters )
{
	...
	UBOOL bInvertedDraw = ( StringClipMode == CLIP_Wrap && StringClipAlignment == UIALIGN_Right );
	...
	{
		FUIStringNode* StringNode = bInvertedDraw ? Nodes(Nodes.Num() - i - 1) : Nodes(i);

		if ( !bInvertedDraw )
		{
			if ( i > 0 && StringClipMode == CLIP_Wrap )
			{
	...
	UBOOL bUIListString = IsA(UUIListString::StaticClass());
	if( bUIListString )
	{
		FLOAT WholeLineExtentY = 0.f;
		for ( INT LineIndex = 0 ; LineIndex < NodeLines.Num() ; LineIndex ++ )
		{
			...
			WholeLineExtentY += pCurrentExtent ? pCurrentExtent->Y : 0.0f;
		}
		switch(NodeParameters.TextAlignment[UIORIENT_Vertical])
		{
		case UIALIGN_Default:
		case UIALIGN_Left:		break;
		case UIALIGN_Center:	NodeParameters.DrawY += (NodeParameters.DrawYL - WholeLineExtentY) * 0.5f;	break;
		case UIALIGN_Right:		NodeParameters.DrawY += (NodeParameters.DrawYL - WholeLineExtentY);	break;
		default:	check(false);
		}
	}
...
//			NodeParameters.DrawYL = Max(Parameters.DrawYL / NodeLines.Num(), CurrentLineExtent.Y);
			...
			for ( INT NodeIndex = 0; NodeIndex < CurrentLineRef.Num(); NodeIndex++ )
			{
				...
				NodeParameters.Scaling *= StringNode->Scaling;		// NodeParameter의 Scaling 파라메터를 반영하도록 수정

				// List를 억지로 쓰기 위해 추가한 부분이므로 새로운 UIListString 드로잉코드가 오면 교체하시기 바랍니다
				// 텍스트 수평, 수직 정렬이 안되어 출력되는 경우가 있다. (예를 들면 UIList)
				// 따라서 텍스트 노드의 경우 NodeParameter에 Style TextAlignment를 적용해준다
				if( bUIListString )
				{
					FRenderParameters ListNodeParameters = NodeParameters;
					if( StringNode->IsTextNode())
					{
						FUIStringNode_Text* TextNode = (FUIStringNode_Text*)StringNode;

						// 더이상 스타일데이터로 정렬을 해줄 필요가 없다. ( 리스트 셀이 가진 정보데이터가 할당되어 들어온다 )
						//for( INT i = 0 ; i < ARRAY_COUNT(NodeParameters.TextAlignment) ; i++)
						//	ListNodeParameters.TextAlignment[i] = TextNode->GetNodeStyleData().TextAlignment[i];

						// DrawYL은 드로잉하는 높이가 아니라 드로잉할 수 있는 영역을 나타낸다. (단 TextNode에 한해)
						// 따라서 대략 15줄 위에 있는 DrawYL = CurrentLineExtent.Y 구문은 UIList TextNode에 바람직한 값이 아님
						//ListNodeParameters.DrawYL = (Parameters.DrawYL / NodeLines.Num());

						//TextNode는 FUIStringNode_Text::Render_Node안에서 Horizontal Align처리를 한다 따라서 여기서 하지 않음.
						//switch( ListNodeParameters.TextAlignment[UIORIENT_Horizontal] )
						//{
						//case UIALIGN_Left:
						//case UIALIGN_Default:	break;
						//case UIALIGN_Center:	ListNodeParameters.DrawX += ((ListNodeParameters.DrawXL - ListNodeParameters.ImageExtent.X) * 0.5f );	break;
						//case UIALIGN_Right:		ListNodeParameters.DrawX += (ListNodeParameters.DrawXL - ListNodeParameters.ImageExtent.X);	break;
						//}
					}
					else if( StringNode->IsImageNode() )
					{
						FUIStringNode_Image* ImageNode = (FUIStringNode_Image*)StringNode;
						if( ImageNode != NULL && ImageNode->RenderedImage != NULL)
						{
							FUICombinedStyleData NewStyleData;
							NewStyleData = ImageNode->RenderedImage->GetImageStyle();
							NewStyleData.ImageColor = NewStyleData.TextColor;
							for( INT i = 0 ; i < UIORIENT_MAX ; i++ )
								NewStyleData.AdjustmentType[i].AdjustmentType = ADJUST_Normal;
							ImageNode->RenderedImage->SetImageStyle(NewStyleData);
						}

						//FLOAT LineDrawYL = Parameters.DrawYL/ NodeLines.Num();
						FLOAT LineDrawYL = NodeParameters.DrawYL;
						//FLOAT LineDrawXL = Parameters.DrawXL;
						switch ( ListNodeParameters.TextAlignment[UIORIENT_Vertical] )
						{
						case UIALIGN_Left:
						case UIALIGN_Default:	break;
						case UIALIGN_Center:	ListNodeParameters.DrawY += ( ( LineDrawYL - ListNodeParameters.ImageExtent.Y) * 0.5f);	break;
						case UIALIGN_Right:		ListNodeParameters.DrawY += ( LineDrawYL - ListNodeParameters.ImageExtent.Y);	break;
						}
						//switch( ListNodeParameters.TextAlignment[UIORIENT_Horizontal] )
						//{
						//case UIALIGN_Left:
						//case UIALIGN_Default:	break;
						//case UIALIGN_Center:	ListNodeParameters.DrawX += ((LineDrawXL - ListNodeParameters.ImageExtent.X) * 0.5f );	break;
						//case UIALIGN_Right:		ListNodeParameters.DrawX += (LineDrawXL - ListNodeParameters.ImageExtent.X);	break;
						//}
					}

					// ListNodeParameters.DrawX에 정수가 아닌 소수가 들어올 경우 그려진 타일이 조금씩 뭉개진다. 따라서 정수아닌 소수를 정수로 만들어주었다.
					// 우선은 ListString이 뭉개지지 않기위해 추가하였으나 필요하다면 DrawTile에서 직접 적용해야할것으로 보인다.
					ListNodeParameters.DrawX = appFloor(ListNodeParameters.DrawX);
					ListNodeParameters.DrawY = appFloor(ListNodeParameters.DrawY);

					StringNode->Render_Node(Canvas, ListNodeParameters);
				}
				else
				{
...
UBOOL UUIString::SetValue( const FString& InputString, UBOOL bIgnoreMarkup )
{
	...
	//@fixme - clearing the nodes breaks UIStrings that are persistent, since the nodes are setup in the editor
	ClearNodes();
 ...
 */
FString UUIString::GetValue( UBOOL bReturnProcessedText/*=TRUE*/ )
{
	FString Result;
	for ( INT i = 0; i < Nodes.Num(); i++ )
	{
...
static FString StripEscapeCharacters( const TCHAR* InputText )
{
	...
	// 에디트 박스에 '\\'가 들어갈 경우 '\\'문자가 들어가면
	// AdjustNodeExtent에서	CaretOffset을 0으로 잘못 계산한다.
	
	// 게다가 아래있는 \\문자 처리는 단지 \뒤에 나타나는 문자를 없애준다
	// 처리예) "\\\\" -> "\\"
	// @ TODO : Escaped Character Handling ( + AdjustNodeExtent )
...
void UUIEditboxString::AdjustNodeExtents( FNodeFormattingData& FormatData )
{
	...
	if ( OwnerEditbox->StringRenderComponent != NULL )
	{
		...
		OwnerEditbox->StringRenderComponent->CaretOffset = OwnerEditbox->StringRenderComponent->CalculateCaretOffset(FormatData.Parameters);
		RequiredCaretPixels = OwnerEditbox->StringRenderComponent->CaretOffset.X;
		//RequiredCaretPixels = OwnerEditbox->StringRenderComponent->CalculateCaretOffset(FormatData.Parameters).X;
		...
		for ( NodeIndex = 0; NodeIndex < Nodes.Num(); NodeIndex++ )
		{
			...
			if ( LeadingPixels + NodeWidth > RequiredLeadPixels )
			{
				if ( StringNode->IsTextNode() && !StringNode->IsFormattingParent() )
				{
					...
					UUIString::ClipString(ClipParameters, *TextNode->RenderedText, NodeRenderText, (EUIAlignment)StringClipAlignment);
		...
		OwnerEditbox->StringRenderComponent->CaretOffset.X = CaretPixels;
 ...
 */
const TCHAR* FUIStringNode::GetValue( UBOOL bProcessedValue )
{
	if ( !bProcessedValue )
	{
 ...
 * @return	the value of this UIStringNode, or NULL if this node has no value
 */
const TCHAR* FUIStringNode_Text::GetValue( UBOOL bProcessedValue )
{
	if ( bProcessedValue )
	{
...
void FUIStringNode_Image::Render_Node( FCanvas* Canvas, const FRenderParameters& Parameters )
{
	if ( RenderedImage != NULL )
	{
		...
		// UIDataStore에서 만드는 StringNode_Image는 텍스쳐 좌표를 강제할 수 없도록 되어있다
		// 그러나 현재의 UI는 텍스쳐 하나에 많은 아이콘들이 들어가있으므로 텍스쳐좌표를 설정해야한다.
		FRenderParameters ImageRenderParameters = Parameters;
		if( ! ForcedDrawCoords.IsZero() )
			ImageRenderParameters.DrawCoords = ForcedDrawCoords;

		RenderedImage->Render_Texture(Canvas, ImageRenderParameters );
	}
}
...
/**
 * @param	TargetWidget			MarkupString을 가지고 있는 (이 UIDataStoreBinding을 가지고 있는) Widget
 ...
 */
UBOOL FUIDataStoreBinding::ResolveMarkup( class UUIObject* TargetWidget, class UUIDataProvider*& out_ResolvedProvider, FString& out_FieldName, class UUIDataStore** out_ResolvedDataStore/*=NULL*/ )
{
	UBOOL bResult = FALSE;

	UUIScene* TargetScene = TargetWidget != NULL ? TargetWidget->GetScene() : NULL;
	if( TargetScene != NULL && MarkupString.Len() > 0 )
	{
			...
			if ( MarkupChunk->IsMarkup() )
			{
				...
				if ( MarkupChunk->GetDataStoreMarkup(DataStoreTag, DataStoreValue) )
				{
					...
					if ( ResolvedDataStore != NULL )
					{
						...
						bResult = ResolvedDataStore->ParseDataStoreReference(DataStoreValue, out_ResolvedProvider, out_FieldName, UnusedArrayIndex);
...
UBOOL FUIStringNodeModifier::AddTextColor( FColor& NewColor )
{
	check( ModifierStack.Num() > 0 );

	FModifierData& ActiveModifier = ModifierStack.Last();

	ActiveModifier.InlineTextColorStack.AddItem(NewColor);
	CustomStyleData.TextColor = NewColor;

	return TRUE;
}

UBOOL FUIStringNodeModifier::RemoveTextColor()
{
	if( ModifierStack.Num() > 0  )
	{
		FModifierData& ActiveModifier = ModifierStack.Last();
		if( ActiveModifier.InlineTextColorStack.Num() > 0 )
		{
			ActiveModifier.InlineTextColorStack.Remove(ActiveModifier.InlineTextColorStack.Num() - 1);
		}

		if( ActiveModifier.InlineTextColorStack.Num() > 0 )
		{
			CustomStyleData.TextColor = ActiveModifier.InlineTextColorStack.Last();
		}
		else
		{
			if(ActiveModifier.Style != NULL)
			{
				UUIStyle_Combo* ComboStyle = Cast<UUIStyle_Combo>(ActiveModifier.Style);
				UUIStyle_Text* TextStyle = ComboStyle ? Cast<UUIStyle_Text>(ComboStyle->TextStyle.GetStyleData()) : Cast<UUIStyle_Text>(ActiveModifier.Style);
				CustomStyleData.TextColor = TextStyle ? TextStyle->StyleColor : FLinearColor::White;
			}
			else
			{
				check(BaseStyleData.IsInitialized());
...
UBOOL FUIStringNodeModifier::AddImageColor( FColor& NewColor )
{
	check( ModifierStack.Num() > 0 );

	FModifierData& ActiveModifier = ModifierStack.Last();

	ActiveModifier.InlineImageColorStack.AddItem(NewColor);
	CustomStyleData.ImageColor = NewColor;

	return TRUE;
}

UBOOL FUIStringNodeModifier::RemoveImageColor()
{
	if( ModifierStack.Num() > 0  )
	{
		FModifierData& ActiveModifier = ModifierStack.Last();
		if( ActiveModifier.InlineImageColorStack.Num() > 0 )
		{
			ActiveModifier.InlineImageColorStack.Remove(ActiveModifier.InlineTextColorStack.Num() - 1);
		}

		if( ActiveModifier.InlineImageColorStack.Num() > 0 )
		{
			CustomStyleData.ImageColor = ActiveModifier.InlineImageColorStack.Last();
		}
		else
		{
			if(ActiveModifier.Style != NULL)
			{
				UUIStyle_Combo* ComboStyle = Cast<UUIStyle_Combo>(ActiveModifier.Style);
				UUIStyle_Image* ImageStyle = ComboStyle ? Cast<UUIStyle_Image>(ComboStyle->ImageStyle.GetStyleData()) : Cast<UUIStyle_Image>(ActiveModifier.Style);
				CustomStyleData.ImageColor = ImageStyle ? ImageStyle->StyleColor : FLinearColor::White;
			}
			else
			{
				check(BaseStyleData.IsInitialized());
