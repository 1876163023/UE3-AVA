...
UBOOL UAnimNodeSequence::GetCachedResults(TArray<FBoneAtom>& OutAtoms, FBoneAtom& OutRootMotionDelta, INT& bOutHasRootMotion)
{
	check(SkelComponent);
...
{
	//! @note script에서 OnAnimEnd함수가 호출이 먼저 되도록 한다.(2007/06/26 고광록)
	/*!
		AnimNodeRandom의 경우 OnAnimEnd()에서 다음 애니메이션을 Play하기 때문에
		아래처럼 Animation1번이 끝났다고 알리기도 전에 Animation2가 시작된다고 
		알려주게 된다.
...
void AActor::execPollFinishAnim _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
