...
UUISequence* UUIComp_Event::CreateEventContainer( FName SequenceName/*=NAME_None*/ ) const
{
	...
	UUISequence* Result = ConstructObject<UUISequence>(UUISequence::StaticClass(), const_cast<UUIComp_Event*>(this), SequenceName, 
...
UBOOL UUIComp_DrawComponents::UpdateFade(FLOAT& NewOpacity)
{
	...
	check(WorldInfo);
	FLOAT CurrentTime = WorldInfo->TimeSeconds * WorldInfo->TimeDilation;

	if( FadePresetCurve == NULL )
		return FALSE;

	if ( FadeType == EFT_Fading && FadeUpdateTime > 0.f )
	{
		FLOAT TimeElapsed = WorldInfo->TimeSeconds - FadeUpdateTime;
		FLOAT Ratio = Clamp( TimeElapsed / FadeDuration, 0.f, 1.f);

		if( (FadeUpdateTime + FadeDuration) > WorldInfo->TimeSeconds )
		{
			NewOpacity = GetCurveValue( FadePresetCurve, Ratio);
		}
		else
		{
			FadeUpdateTime = -1.f;
	...
	else if ( FadeType == EFT_Loop )
	{
		FLOAT FadeRange = 1.f;
		FLOAT FadeRatio = Abs( appSin( ( PI / (WorldInfo->TimeSeconds - FadeUpdateTime) ) * CurrentTime) );

		NewOpacity = FadeRatio * FadeRange;
	}
	else
	{
		return FALSE;
...
void UUIComp_DrawComponents::Fade( BYTE FadeType, class UCurveEdPresetCurve* PresetCurve, FLOAT Duration )
{
	this->FadeType 		= FadeType;
	this->FadePresetCurve = PresetCurve;
	this->FadeDuration 	= Duration;

	// Prime Last Render time
	this->FadeUpdateTime = GWorld->GetWorldInfo()->TimeSeconds;
}
...
UBOOL UUIComp_DrawComponents::UpdateScaleRender(FRenderParameters& RenderParameters )
{
	if( ! ScaleRenderParam.bEnabled )
		return FALSE;

	FLOAT TimeElapsed = GWorld->GetTimeSeconds() - ScaleRenderParam.ScaleUpdateTime;
	FLOAT TimeLeft = ScaleRenderParam.ScaleDuration - TimeElapsed;

	if( TimeLeft < 0.f )
	{
		ScaleRenderParam.Reset();
		if ( DELEGATE_IS_SET(OnScaleRenderComplete) )
			delegateOnScaleRenderComplete(this);
	}
	else
	{
		FLOAT Ratio = TimeElapsed / ScaleRenderParam.ScaleDuration;
		FLOAT ScaleX = ScaleRenderParam.ScalePresetCurve[UIORIENT_Horizontal] ? GetCurveValue( ScaleRenderParam.ScalePresetCurve[UIORIENT_Horizontal], Ratio) : 1.f;
		FLOAT ScaleY = ScaleRenderParam.ScalePresetCurve[UIORIENT_Vertical] ? GetCurveValue( ScaleRenderParam.ScalePresetCurve[UIORIENT_Vertical], Ratio) : 1.f;

		// Axis는 Ratio이다. 예시) AxisX = 0.5, AxisY = 0.5 이면 중심을 가리킨다. 
		FLOAT AxisX = ScaleRenderParam.ScaleAxis.X;	
		FLOAT AxisY = ScaleRenderParam.ScaleAxis.Y;
		...
		FVector4 NewPos(-(AxisX * RenderParameters.DrawXL),			//left
						-(AxisY * RenderParameters.DrawYL),			//top
						(1.0f - AxisX) * RenderParameters.DrawXL,	//right
						(1.0f - AxisY) * RenderParameters.DrawYL);	//bottom

		//FLOAT ScaleX = (ScaleRenderParam.ScaleTarget.X - ScaleRenderParam.ScaleValue.X) * FactorX + ScaleRenderParam.ScaleValue.X;
		//FLOAT ScaleY = (ScaleRenderParam.ScaleTarget.Y - ScaleRenderParam.ScaleValue.Y) * FactorY + ScaleRenderParam.ScaleValue.Y;

		// 변환된 상태에서 중심을 기준으로 스케일을 바꾼다.
		FVector4 NewScaledPos( NewPos.X * ScaleX, NewPos.Y * ScaleY, NewPos.Z * ScaleX, NewPos.W * ScaleY );
		
		// 바뀐 스케일을 적용
		RenderParameters.DrawX = CenterPosX + NewScaledPos.X;
		RenderParameters.DrawY = CenterPosY + NewScaledPos.Y;
		RenderParameters.DrawXL = NewScaledPos.Z - NewScaledPos.X;
		RenderParameters.DrawYL = NewScaledPos.W - NewScaledPos.Y;
		
		if ( GetClass() == UUIComp_DrawString::StaticClass() )
		{
			RenderParameters.Scaling.X *= ScaleX;
			RenderParameters.Scaling.Y *= ScaleY;
		}
	}
...
void UUIComp_DrawComponents::ScaleRender(class UCurveEdPresetCurve* PresetCurveHorz,class UCurveEdPresetCurve* PresetCurveVert,FLOAT ScaleAxisX,FLOAT ScaleAxisY,FLOAT ScaleDuration)
{
	ScaleRenderParam.ScaleAxis.X = ScaleAxisX;
	ScaleRenderParam.ScaleAxis.Y = ScaleAxisY;
	ScaleRenderParam.ScalePresetCurve[UIORIENT_Horizontal] = PresetCurveHorz;
	ScaleRenderParam.ScalePresetCurve[UIORIENT_Vertical] = PresetCurveVert;

	ScaleRenderParam.bEnabled = true;
	ScaleRenderParam.ScaleDuration = ScaleDuration;
	ScaleRenderParam.ScaleUpdateTime = GWorld->GetTimeSeconds();
}

void UUIComp_DrawComponents::ResetScaleRender()
{
	ScaleRenderParam.Reset();
}

FLOAT UUIComp_DrawComponents::GetCurveValue( class UCurveEdPresetCurve* PresetCurve, FLOAT Ratio)
{
	return PresetCurve ? PresetCurve->EvalRatio(Clamp(Ratio, 0.f, 1.f)) : Ratio;
}

void UUIComp_DrawComponents::PostEditChange( FEditPropertyChain& PropertyThatChanged )
{
	if ( PropertyThatChanged.Num() > 0 )
	{
		...
		if ( MemberProperty != NULL )
		{
			...
			if( PropertyName == TEXT("ScalingTrigger"))
			{
				ScalingTrigger = FALSE;
				ScaleRender( ScaleRenderParam.ScalePresetCurve[UIORIENT_Horizontal], ScaleRenderParam.ScalePresetCurve[UIORIENT_Vertical],ScaleRenderParam.ScaleAxis.X, ScaleRenderParam.ScaleAxis.Y , ScaleRenderParam.ScaleDuration);
			}
			else if ( PropertyName == TEXT("FadingTrigger"))
			{
				FadingTrigger = FALSE;
				Fade( FadeType, FadePresetCurve, FadeDuration );
			}
		}
	}
}

void UUIComp_DrawComponents::ApplyDrawCompStyle(class UUIStyle_Data* StyleData)
{
	if( StyleData == NULL )
		return;

	if( StyleData->FadeType != EFT_None && StyleData->FadeDuration > 0.f)
	{
		Fade( StyleData->FadeType, StyleData->FadePresetCurve, StyleData->FadeDuration);
	}

	if( StyleData->ScaleDuration && StyleData->ScaleDuration > 0.f )
	{
		ScaleRender( StyleData->ScalePresetCurve[UIORIENT_Horizontal], StyleData->ScalePresetCurve[UIORIENT_Vertical], StyleData->ScaleAxis[UIORIENT_Horizontal], StyleData->ScaleAxis[UIORIENT_Vertical], StyleData->ScaleDuration );
	}
}

void UUIComp_DrawComponents::RefreshDrawCompStyle()
{
	ScaleRender( ScaleRenderParam.ScalePresetCurve[UIORIENT_Horizontal], ScaleRenderParam.ScalePresetCurve[UIORIENT_Vertical],ScaleRenderParam.ScaleAxis.X, ScaleRenderParam.ScaleAxis.Y , ScaleRenderParam.ScaleDuration);
	if( FadeType != EFT_Loop )
		Fade( FadeType, FadePresetCurve, FadeDuration );
}
...
void UUIComp_DrawImage::execGetAppliedImageStyle _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UUIComp_DrawImage::CustomizeAppliedStyle( FUICombinedStyleData& CustomizedStyleData ) const
{
	...
	StyleCustomization.CustomizeOpacity(CustomizedStyleData.ImageColor);
...
void UUIComp_DrawImage::RenderComponent( FCanvas* Canvas, FRenderParameters Parameters )
{
	if ( ImageRef != NULL && Canvas != NULL )
	{
		// Fading
		FLOAT FadeOpacity;
		FRenderParameters ScaleRenderParameters = Parameters;

		if ( UpdateFade(FadeOpacity) )
		{
			SetOpacity( FadeOpacity);
		}
		if ( UpdateScaleRender( ScaleRenderParameters ) )
		{
			Parameters = ScaleRenderParameters;
		}
...
UBOOL UUIComp_DrawImage::NotifyResolveStyle( UUISkin* ActiveSkin, UBOOL bClearExistingValue, UUIState* CurrentMenuState/*=NULL*/, const FName StylePropertyName/*=NAME_None*/ )
{
	...
	if ( StylePropertyName == NAME_None || StylePropertyName == TEXT("ImageStyle") )
	{
		...
		if ( ImageRef != NULL )
		{
			...
			ApplyDrawCompStyle( ImageStyleData );
...
UBOOL UUIComp_DrawString::GetStringFormatParameters( EUIWidgetFace Face, FRenderParameters& out_Parameters ) const
{
	...
	if ( bFormattingConditionsMet )
	{
		...
		// RemoveChild, RemoveChildren에 의해 지워진 자식들에 대해서도 현재 메소드 GetStringFormatParameters가 불린다.
		// 따라서 지워진 자식들에 대해 Owner, OwnerScene이 없을 수 있고 밑에서 얻는 값은 별로 의미없는 값임.
		UUIScreenObject* OwnerScene = Owner ? Owner->GetScene() : NULL;
		if ( !OwnerScene || !OwnerScene->GetViewportOrigin(ViewportOrigin) )
		{
			ViewportOrigin.X = 0.f;
			ViewportOrigin.Y = 0.f;
		}
		if ( !OwnerScene || !OwnerScene->GetScene()->GetViewportSize(ViewportSize) )
		{
		...
		const FLOAT LeftPadding		= AutoSizeParameters[UIORIENT_Horizontal].GetPaddingValue( UIORIENT_Horizontal, EVALPOS_PixelOwner, Owner );
		const FLOAT RightPadding	= AutoSizeParameters[UIORIENT_Horizontal].GetPaddingValue( UIORIENT_Vertical, EVALPOS_PixelOwner, Owner );
		const FLOAT TopPadding		= AutoSizeParameters[UIORIENT_Vertical].GetPaddingValue( UIORIENT_Horizontal, EVALPOS_PixelOwner, Owner );
		const FLOAT BottomPadding	= AutoSizeParameters[UIORIENT_Vertical].GetPaddingValue( UIORIENT_Vertical, EVALPOS_PixelOwner, Owner );
		...
		if ( bAutoSizeHorz )
		{
			...
			const FLOAT MinRegionWidth = AutoSizeParameters[UIORIENT_Horizontal].GetMinValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Horizontal, Owner );
			const FLOAT MaxRegionWidth = AutoSizeParameters[UIORIENT_Horizontal].GetMaxValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Horizontal, Owner );
		...
		if ( bAutoSizeVert )
		{
			...
			// if the user has specified an auto-size constraint range, adjust the available bounding region
			const FLOAT MinRegionWidth = AutoSizeParameters[UIORIENT_Vertical].GetMinValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Vertical, Owner );
			const FLOAT MaxRegionWidth = AutoSizeParameters[UIORIENT_Vertical].GetMaxValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Vertical, Owner );
			if ( MinRegionWidth > KINDA_SMALL_NUMBER )
			{
...
void UUIComp_DrawString::UpdateOwnerBounds( FRenderParameters& Parameters )
{
	...
	if ( IsAutoSizeEnabled(UIORIENT_Vertical) )
	{
		...
		//TargetHeight += TopPadding + BottomPadding;		// 글씨의 크기가 없을때는 Padding도 값이 없다
		TargetHeight += TargetHeight > KINDA_SMALL_NUMBER ? (TopPadding + BottomPadding) : 0;

		const FLOAT MinTargetHeight = AutoSizeParameters[UIORIENT_Vertical].GetMinValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Vertical, Owner );
		const FLOAT MaxTargetHeight = AutoSizeParameters[UIORIENT_Vertical].GetMaxValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Vertical, Owner );
	...
	if ( IsAutoSizeEnabled(UIORIENT_Horizontal) )
	{
		...
		//TargetWidth += LeftPadding + RightPadding;	// 글씨의 크기가 없을때는 Padding도 값이 없다
		TargetWidth += TargetWidth > KINDA_SMALL_NUMBER ? LeftPadding + RightPadding : 0;

		// now verify that the auto-size region falls within the constraints of the auto-size min/max
		const FLOAT MinTargetWidth = AutoSizeParameters[UIORIENT_Horizontal].GetMinValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Horizontal, Owner );
		const FLOAT MaxTargetWidth = AutoSizeParameters[UIORIENT_Horizontal].GetMaxValue( UIAUTOSIZEEVAL_Pixels, UIORIENT_Horizontal, Owner );
...
void UUIComp_DrawString::execGetAppliedStringStyle _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT_OPTX(UUIState,DesiredMenuState,NULL);
	P_FINISH;
...
UBOOL UUIComp_DrawString::NotifyResolveStyle( UUISkin* ActiveSkin, UBOOL bClearExistingValue, UUIState* CurrentMenuState/*=NULL*/, const FName StylePropertyName/*=NAME_None*/ )
{
	...
	if ( StylePropertyName == NAME_None || StylePropertyName == TEXT("StringStyle") )
	{
		...
		UUIStyle_Text* TextStyleData = Cast<UUIStyle_Text>(ComboStyleData->TextStyle.GetStyleData());
		if( TextStyleData != NULL )
		{
			ApplyDrawCompStyle( TextStyleData );
		}
...
void UUIComp_DrawString::SetStringStyle( UUIStyle_Combo* NewComboStyle )
{
	...
	if ( ValueString != NULL )
	{
		...
		UBOOL bNeedsReformat =
			ValueString->SetStringStyle(NewComboStyle)
			|| ValueString->StringClipMode != WrapMode
			|| ValueString->StringClipAlignment != ClipAlignment;
...
void UUIComp_DrawString::CustomizeAppliedStyle( FUICombinedStyleData& CustomizedStyleData ) const
{
	...
	TextStyleCustomization.CustomizeOpacity(CustomizedStyleData.TextColor);
	...
	//FIXME RON - This is a temp fix for mangled alignment issues on the console.  
...
void UUIComp_DrawString::Render_String( FCanvas* Canvas )
{
	...
	FVector2D ValueStringExtent(ValueString->StringExtent.X * Canvas->FontRatioFactorX, 
								ValueString->StringExtent.Y * Canvas->FontRatioFactorY);
	...
	case UIALIGN_Center:
		...
		Parameters.DrawX = LabelX + (LabelWidth * 0.5f - ValueStringExtent.X * 0.5f);
		Parameters.DrawXL = ValueStringExtent.X;
		break;

	case UIALIGN_Right:
		// reduce the bounding region to the width of the longest line of the UIString (the value of StringExtent.X)
		Parameters.DrawX = LabelX2 - ValueStringExtent.X;
	...
	case UIALIGN_Center:
		...
		Parameters.DrawY = LabelY + (LabelHeight * 0.5f - ValueStringExtent.Y * 0.5f);
		Parameters.DrawYL = ValueStringExtent.Y;
		break;

	case UIALIGN_Right:
		// reduce the bounding region to the combined height of all lines in the UIString (the value of StringExtent.Y)
		Parameters.DrawY = LabelY2 - ValueStringExtent.Y;
...
void UUIComp_DrawString::SetValue( const FString& NewText )
{
	...
	if ( ValueString != NULL )
	{
		...
		//// 타이핑하는것처럼 하기위해 문자열을 변환
		//DraTypingString.Reset();
		//for( INT i = 0; i < NewText.Len() ; i++ )
		//{
		//	TCHAR Unicode = NewText[i];
		//	UBOOL bHangulSyllable = FALSE;
		//	UBOOL bHangulJamo = FALSE;
		//	//한글이라면
		//	if( (bHangulSyllable = (0xAC00 <= Unicode && Unicode <= 0xD7A3)) ||
		//		(bHangulJamo = (0x1100 <= Unicode && Unicode <= 0x11ff)) )
		//	{
		//		if( bHangulSyllable )
		//		{
		//			TCHAR EndChar = Unicode % 28;
		//			TCHAR MedChar = (( Unicode - EndChar )/28)%21;
		//			TCHAR FirstChar = (( Unicode - EndChar)/28)/21;
		//			UBOOL bHasEndChar = (EndChar != 0);

		//			FDrawTypingCharInfo CompCharInfo;
		//			CompCharInfo.UnicodeChar = FirstChar + 0x1100;
		//			CompCharInfo.bComposing = TRUE;
		//			DrawTypingString.AddItem(CompCharInfo);
		//			CompCharInfo.UnicodeChar = MedChar + 0x1161;
		//			CompCharInfo.bComposing = bHasEndChar ? TRUE : FALSE;
		//			DrawTypingString.AddItem(CompCharInfo);
		//			if( bHasEndChar )
		//			{
		//				CompCharInfo.UnicodeChar = EndChar + 0x11A8 - 1;
		//				CompCharInfo.bComposing = FALSE;
		//				DrawTypingString.AddItem(CompCharInfo);
		//			}
		//		}
		//		else if ( bHangulJamo )
		//		{
		//			// 'ㄱ','ㄴ','ㄷ', ....
		//			if( 0x1100 <= Unicode && Unicode < 0x1161 )
		//			{
		//				CompCharInfo.UnicodeChar = 

		//			}
		//			// 'ㅏ','ㅐ','ㅒ',...
		//			else if( 0x1100 <= Unicode && Unicode < 0x11A8 )
		//			{
 ...
 */
void UUIComp_DrawString::ReapplyFormatting()
{
	...
	// on the component!
	GetOuterUUIObject()->RequestSceneUpdate(FALSE,TRUE);
}
...
void UUIComp_DrawString::PostEditChange( FEditPropertyChain& PropertyThatChanged )
{
	if ( PropertyThatChanged.Num() > 0 )
	{
		...
		{
			FName PropertyName = MemberProperty->GetFName();
			if ( PropertyName == TEXT("AutoSizeParameters") || PropertyName == TEXT("WrapMode") || PropertyName == TEXT("ClipAlignment") || PropertyName == TEXT("bIgnoreMarkup") )
			{
				// this recalculates the extents for each of the nodes contained by the string
				ReapplyFormatting();
				GetOuterUUIObject()->RequestSceneUpdate(TRUE,TRUE,TRUE);
			}
...
void UUIComp_DrawString::PostLoad()
{
	...
	// FIXME RON - This needs to be disabled.  For some reason on console, this will overwrite the instanced data and it shouldnt.
...
UBOOL UUIComp_DrawStringEditbox::GenerateCaretMarkup( FString& out_CaretMarkupString )
{
	...
	//  [10/16/2006 YTS] Comment 'StringCaret.bDisplayCaret' code below.
	//
	// RENDER CARET CONDITION : StringCaret.bDisplayCaret == true && CaretNode != NULL
	// the commented code below blocks the condition 'CaretNode != NULL' at first time the editbox shown.
	//
	// but I worried about that commented code. it is likely to be another reason to check this condition. What there??
	...
	if ( /*StringCaret.bDisplayCaret && */ StringCaret.CaretType != UIPEN_MAX )
	{
		...
		//	UTexture* CaretImage = UGameUISceneClient::StaticClass()->GetDefaultObject<UGameUISceneClient>()->DefaultUITexture[StringCaret.CaretType];
		//	if ( CaretImage != NULL )
		//	{
		//		if ( StringCaret.CaretMaterial == NULL )
		//		{
		//			UMaterialInstanceConstant* CaretInstance = ConstructObject<UMaterialInstanceConstant>(UMaterialInstanceConstant::StaticClass());
		...
		if( CaretImage != NULL )
		{
			out_CaretMarkupString = FString::Printf(TEXT("%s;XL=%f"), *CaretImage->GetPathName(), StringCaret.CaretWidth );
			bResult = TRUE;
		}
	}
...
void UUIComp_DrawStringEditbox::ApplyCaretStyle( UUIState* CurrentWidgetState/*=NULL*/ )
{
	if ( CurrentWidgetState == NULL )
	{
		...
		CurrentWidgetState = GetOuterUUIObject()->GetCurrentState();
	}
	...
	if ( CurrentWidgetState != NULL )
	{
		...
		{
			debugf(NAME_Warning, TEXT("%s::ApplyCaretStyle: no caret style configured!"), *GetPathName());
		}
	}
	else
	{
 ...
 */
UBOOL UUIComp_DrawStringEditbox::SetCaretPosition( INT NewCaretPosition, UBOOL bSetInsertPosition /*=TRUE*/ )
{
	...
	if( bSetInsertPosition )
		InsertPos = StringCaret.CaretPosition;
...
void UUIComp_DrawStringEditbox::UpdateFirstVisibleCharacter()
{
	...
	//	// get a string containing all characters up to the caret position
	//	FString CaretBoundString = UserText.Left(StringCaret.CaretPosition);
	...
	//	// bounding region
	//	UUIString::ClipString(Parameters, *CaretBoundString, CaretClippedString, (EUIAlignment)ClipAlignment);
 ...
 */
FVector2D UUIComp_DrawStringEditbox::CalculateCaretOffset( FRenderParameters Parameters ) const
{
	// start at the first visible character
	INT StartPosition = Clamp(FirstCharacterPosition, 0, UserText.Len());

	// 조합중일때는 조합하는중인 문자까지 계산해야한다
	INT EndPosition = Clamp(StringCaret.CaretPosition + (bComposing ? CurrCompStr.Len() : 0), StartPosition, UserText.Len());
	...
	{
		TArray<FWrappedStringElement> WrapStrings;
		Parameters = FRenderParameters( Parameters.DrawX, 0, Parameters.DrawX + Parameters.DrawXL - StringCaret.CaretWidth, 0, Parameters.DrawFont);
		UUIString::WrapString( Parameters, Parameters.DrawX, *VisibleString, WrapStrings, TEXT("\n"));

		// Empty Line 삭제
		for( INT LineIndex = WrapStrings.Num() - 1 ; LineIndex >= 0 ; LineIndex-- )
		{
			FWrappedStringElement& WrapStrElem = WrapStrings(LineIndex);
			if( WrapStrElem.Value.Len() == 0 )
			{
				if( WrapStrings(LineIndex).LineExtent.Y == 0.f )
					WrapStrings.Remove(LineIndex);
			}
		}

		check( ValueString );
		FLOAT DefaultHeight = ValueString->GetDefaultLineHeight();

		// 마지막 문자가 개행 문자면 한줄 추가해줘야 커서위치를 계산할 수있다
		// (문자와 인접한 개행문자는 그리고 그 이후 문자들이 모두 개행문자이면 
		// 그 개행문자는WrapString에 포함되지 않는다)
		const TCHAR* pText = *VisibleString;
		if( WrapStrings.Num() >0 && VisibleString.Len() > 0 &&
			(WrapStrings(WrapStrings.Num() - 1).Value.Len() == 0 ||
			appIsLinebreak(VisibleString[VisibleString.Len() - 1])) )
		{
			new(WrapStrings) FWrappedStringElement(TEXT(""), 0, DefaultHeight);
		}

		Parameters.DrawXL = Parameters.DrawYL = 0;
		for( INT LineIndex = 0 ; LineIndex < WrapStrings.Num() ; LineIndex++ )
		{
			Parameters.DrawXL = 0;
			FWrappedStringElement& WrapStrElem = WrapStrings(LineIndex);
			FString& CurrValue = WrapStrElem.Value;
			if( CurrValue.Len() > 0 )
			{
				Parameters.DrawXL += WrapStrElem.LineExtent.X;
			}
			
			if( WrapStrings.IsValidIndex(LineIndex - 1) )
				Parameters.DrawYL += WrapStrings(LineIndex - 1).LineExtent.Y;
		}
	}
	return FVector2D(Parameters.DrawXL, Parameters.DrawYL);
}
...
{
	UUIEditBox* EditBox = Cast<UUIEditBox>(GetOuterUUIObject());
	if ( !EditBox->bPasswordMode )
	{
		return UserText;
	}
...
{
	UBOOL bTextChanged = UserText != NewText;
...
FString UUIComp_DrawStringEditbox::GetValue( UBOOL bReturnInputText/*=TRUE*/ ) const
{
	...
	return Super::GetValue(bReturnInputText);
}
 ...
 * Refreshes the UIString's extents and appearance based on the string's current style and formatting options.
 */
void UUIComp_DrawStringEditbox::ReapplyFormatting()
{
	bRecalculateFirstCharacter = TRUE;
	Super::ReapplyFormatting();
}
 ...
 */
INT UUIComp_DrawStringEditbox::InsertCompStr( FString& TargetStr, const FString& CompStr )
{
	UUIEditBox* EditBox = Cast<UUIEditBox>(GetOuter());
	check(EditBox);

	const INT AppCompLen = UIMEComposition::StaticGetImeLanguage() == KBDINPUTLANG_Korean ?  CompStr.Len() - 1 : 0;
	FString BackupString = TargetStr.Mid(InsertPos + Max(AppCompLen,0), CertainEndPos - InsertPos );
	INT ResultLength = TargetStr.Len() + CompStr.Len();
	...
	// MaxCharacters 도달했는지 , 렌더바운드 넘어가는지 검사
	const INT MaxCharacters = EditBox->MaxCharacters;
	INT Exceed = MaxCharacters > 0 ? ResultLength - EditBox->MaxCharacters : 0;
	if( EditBox->bLimitTextInRenderBound )
	{
		check( ValueString );
		FRenderParameters LimitParms( ValueString->StringStyleData.DrawFont, 1.f,1.f);
		EditBox->GetPositionExtents(LimitParms.DrawX, LimitParms.DrawXL, LimitParms.DrawY, LimitParms.DrawYL, FALSE);
		FVector2D Padding( AutoSizeParameters[UIORIENT_Horizontal].GetPaddingValue(UIORIENT_Horizontal,EVALPOS_PixelOwner,EditBox),
			AutoSizeParameters[UIORIENT_Vertical].GetPaddingValue(UIORIENT_Vertical, EVALPOS_PixelOwner,EditBox));
		LimitParms.DrawX += Padding.X;
		LimitParms.DrawXL -= Padding.X;
		LimitParms.DrawY += Padding.Y;
		LimitParms.DrawYL -= Padding.Y;

		FString TestString = TargetStr.Left(InsertPos) + CompStr + BackupString;
		FString ClipString;
		UUIString::ClipString( LimitParms, *TestString,  ClipString);
		Exceed = Max( TestString.Len() - ClipString.Len(), Exceed );
	}

	if( Exceed > 0 )
	{
		INT CurrentIMELang = UIMEComposition::StaticGetImeLanguage();
		if( (CurrentIMELang == KBDINPUTLANG_TraditionalChinese || CurrentIMELang == KBDINPUTLANG_SimplifiedChinese) )
		{
			UIMEComposition::StaticNotifyAction( IMENOTIFY_CompositionStr, IMENOTIFYCOMPSTR_Complete, 0);
			TargetStr = TargetStr.Left(InsertPos) + CompStr + BackupString;
			TargetStr = TargetStr.Left(TargetStr.Len() - Exceed);
			InsertCount = CompStr.Len() - Exceed;
		}
		else
		{
			UIMEComposition::StaticNotifyAction( IMENOTIFY_CompositionStr, IMENOTIFYCOMPSTR_Cancel, 0);
			InsertCount = 0;
		}
	}
	else
	{
		TargetStr = TargetStr.Left(InsertPos) + CompStr + BackupString;
		InsertCount = CompStr.Len();
	}
	// @TODO : verifying the 'MaxCharacter' bound

	return InsertCount;
}

/**
 * CompType이 IME_COMPOSITION일 때 즉,
 *
 * 조합문자(CompStr)가 들어왔거나
 * 조합이 완료된 문자(ResultStr)가 들어왔거나
 * 단일 문자가 들어왔거나(영어소문자대문자, 컨트롤문자) 처리
 *
 * @param CompStrData Contains info about CompType, CompStr, Attrs, Clauses, and so on.
 * @return	composition-string consumed or not
 */
UBOOL UUIComp_DrawStringEditbox::OnImeComposition( const FInputCompositionStringData& CompStrData )
{
	UBOOL bResult = FALSE;

	LastCompStrData = CompStrData;

	if( bComposing )
		UserText = BackupUserText;

	UUIEditBox* EditBox = Cast<UUIEditBox>(GetOuterUUIObject());
	check(EditBox);
	INT MaxCharacters = EditBox ? EditBox->MaxCharacters : 0;
	BYTE CurrIMELang = UIMEComposition::StaticGetImeLanguage();

	TCHAR FirstCharacter = CompStrData.CompStr.Len() > 0 ? CompStrData.CompStr[0] : TEXT('\0');
	if( CompStrData.CompStr.Len() == 0 /*backspace when composing*/ ||
		EditBox->IsValidCharacter( FirstCharacter ) )
	{
		INT CaretPosition = StringCaret.CaretPosition;
		if( CompStrData.CompType == IMECOMPOSITION_Composition )
		{
			// ignore level  3 feature ( Chinese QuanPin, ZhengMa, and so on ... )
			if( UIMEComposition::StaticGetImeProperty() & UCONST_IMEPROP_AT_CARET )
			{
				CurrCompStr = CompStrData.CompStr;
				if( CurrIMELang == KBDINPUTLANG_TraditionalChinese )
				{
					ShowInvert = TRUE;
					INT TempIncr = 0;
					for( INT i = CompStrData.CompCursorPos ; i < CompStrData.CompAttr.Num() ; i++ )
					{
						if( CompStrData.CompAttr(i) == 1 )
							TempIncr++;
						else
							break;
					}

					InvertStart = InsertPos + CompStrData.CompCursorPos;
					InvertEnd = InvertStart + TempIncr;
				}

				INT Increase = InsertCompStr( UserText, CompStrData.CompStr );
				if( Increase > 0 && CurrIMELang != KBDINPUTLANG_Korean )
				{
					if( CurrIMELang == KBDINPUTLANG_Japanese )
						SetCaretPosition( InsertPos + CompStrData.CompStr.Len(), FALSE );
					else
						SetCaretPosition( InsertPos + CompStrData.CompCursorPos, FALSE );
				}
			}
		}
		...
		{
			ShowInvert = FALSE;
			if( CompStrData.CompStr.Len() > 0 )
			{
				INT Increase = InsertCompStr( UserText, CompStrData.CompStr );
				BackupUserText = UserText;
				SetCaretPosition( InsertPos + Increase );
				CertainEndPos += Increase;
			}
		}
		//FString DebugOutputStr = FString::Printf(TEXT("OnImeComposition (%s), %s, %d"),
		//	*UserText,
		//	CompStrData.CompType == IMECOMPOSITION_Result ? TEXT("Res") : CompStrData.CompType == IMECOMPOSITION_Composition ? TEXT("Comp") : TEXT("Unknown"),
		//	CompStrData.CompCursorPos);

		//debugf(*DebugOutputStr);

		EditBox->SetValue(UserText);
		bResult = TRUE;
	}
	// @TODO yts - I wouldn't fix it until finishing the ime composition functionality.
	////@todo - add support for selecting substrings of the text
	else if ( EditBox->bDisableCopyAndPaste == FALSE )
	{
		TCHAR ResultChar = CompStrData.CompStr.Len() > 0 ? CompStrData.CompStr[0] : TEXT('\0');
		switch ( ResultChar )
		{
		case 3:		// Ctrl-C
		...
		case 22:	//	Ctrl-V
			{
				...
				{
					EditBox->SetValue( UserText + appClipboardPaste() );
				}
				else
				{
					EditBox->SetValue(UserText.Left(CaretPosition) + appClipboardPaste() + UserText.Mid(CaretPosition) );
				}

				if ( MaxCharacters != 0 && UserText.Len() > MaxCharacters )
				{
					EditBox->SetValue( UserText.Left(MaxCharacters) );
				}
			}
			bResult = TRUE;
			break;
	...
	// End of Character Handling .
	// There are changes in the user-text.
	//if( bResult == TRUE )
	//{
	//	FString OutString = UserText;
	//	if( EditBox->bLimitTextInRenderBound )
	//	{
	//		check( ValueString);
	//		FRenderParameters LimitParms( ValueString->StringStyleData.DrawFont, 1.f,1.f);
	...
	//		FRenderParameters ClipParms = LimitParms;
	//		UUIString::ClipString( ClipParms, *UserText, OutString );
	//	}
	//	if( MaxCharacters > 0 && OutString.Len() > MaxCharacters )
	//	{
	//		OutString = OutString.Left( MaxCharacters );
	//	}

	//	if( OutString != UserText )
	//	{
	//		SetCaretPosition( Min(OutString.Len(), StringCaret.CaretPosition) );
	//	}
	//	EditBox->SetValue( OutString );
	//}
...
{	
	UUIObject* Owner = GetOuterUUIObject();
	UUIEditBox* EditBox = Owner ? Cast<UUIEditBox>(Owner) : NULL;

	check(EditBox && ValueString);

	BYTE CurrIMELang = UIMEComposition::StaticGetImeLanguage();

	check(ValueString);
	FRenderParameters DefaultSize(0,0,0,0,ValueString->StringStyleData.DrawFont);
	UUIString::StringSize(DefaultSize, TEXT("A"));

	// Render ReadingString

	// Render Composing Indicator
	if( CaretNode != NULL && 
		bComposing &&
		CurrIMELang != KBDINPUTLANG_Korean &&
		(UIMEComposition::StaticGetImeProperty() & UCONST_IMEPROP_AT_CARET) )
	{
		FVector2D StartPos(Parameters.DrawX,Parameters.DrawY);
		FVector2D EndPos = StartPos;

		if( CurrIMELang == KBDINPUTLANG_Japanese )
		{
			// @TODO : Japanese Support.
		}
		else if ( CurrIMELang == KBDINPUTLANG_TraditionalChinese ||
				CurrIMELang == KBDINPUTLANG_SimplifiedChinese )
		{
			// @TODO : Multi-line support.
			// draw an underline of composing characters;
			FString FrontString = UserText.Left(InsertPos);
			INT StrXL, StrYL;
			StringSize( ValueString->StringStyleData.DrawFont, StrXL, StrYL, *FrontString );
			StartPos.X += StrXL;
			StartPos.Y += DefaultSize.DrawYL;

			StringSize( ValueString->StringStyleData.DrawFont, StrXL, StrYL, *CurrCompStr );
			EndPos.X = StartPos.X + StrXL;
			EndPos.Y = StartPos.Y;
		}
		else
		{
			// unexpected condition
			checkf(FALSE,TEXT("Unexpected Language Type = %d"),CurrIMELang);
		}

		DrawLine2DStyled( Canvas, StartPos, EndPos, FLinearColor::White, DLS_DASH, appTrunc(GWorld->GetTimeSeconds() * 8.f));
		//CaretNode->Render_Node(Canvas, CompParams);
	}

	// Caret Rendering ( Line Caret & Box Caret)
	if ( CaretNode != NULL && StringCaret.bDisplayCaret && !EditBox->bReadOnly)
	{
		FRenderParameters CaretParams = Parameters;
		FString CaretStr = UserText.Mid(StringCaret.CaretPosition,CurrCompStr.Len());

		// now render the caret
		CaretParams.DrawX += (CaretOffset.X - 1);
		CaretParams.DrawY += CaretOffset.Y;
		CaretParams.DrawXL = StringCaret.CaretWidth;
		CaretParams.ImageExtent = CaretNode->Extent;
		
		// 새 캐럿의 너비와 높이를 결정한다.
		INT CaretStrXL, CaretStrYL;
		StringSize(ValueString->StringStyleData.DrawFont , CaretStrXL, CaretStrYL, *CaretStr);
		if( CaretStrYL == 0 )
		{
			if( CaretParams.DrawYL == 0 )
				CaretParams.DrawY -= DefaultSize.DrawYL/2;					// Need an Offset When DrawYL == 0
			CaretParams.DrawYL = DefaultSize.DrawYL;
		}
		else
		{
			CaretParams.DrawYL = CaretStrYL;
		}

		// 문자를 조합중일때는 캐럿을 박스로 그려준다
		if( bComposing )
		{
			if( CurrIMELang == KBDINPUTLANG_Korean )
			{
				if( EditBox->bPasswordMode )
				{
					FRenderParameters AsteriskSize(0,0,0,0,ValueString->StringStyleData.DrawFont);
					UUIString::StringSize(AsteriskSize, TEXT("*"));
					CaretParams.DrawXL = AsteriskSize.DrawXL;
				}
				else
				{
					CaretParams.DrawXL = CaretStrXL + 1;
				}
			}
			// UUIComp_DrawStringEditbox::CalculateCaretOffset()
			// 캐럿이 현재 커서를 포함해 왼쪽에 있는 모든 텍스트에 대해 CaretOffset을 계산하므로
			// 조합중일때는 캐럿 크기만큼 왼쪽으로 이동해줘야한다.
			CaretParams.DrawX -= CaretStrXL;
		}

		CaretParams.DrawAlpha = Abs( appCos( GWorld->GetTimeSeconds() * PI));
		CaretNode->Render_Node(Canvas, CaretParams);
	}
...
/**
 * 스키마를 포함한 열데이터의 모든 Alignment 필드값을 @param Alignment값으로 변경한다.
 */
void UUIComp_ListPresenter::SetColumnAlignment(INT ColumnIndex, EUIOrientation Orientation , EUIAlignment Alignment)
{
	if( ElementSchema.Cells.IsValidIndex(ColumnIndex) && 
		0 <= Orientation && Orientation < UIORIENT_MAX &&
		0 <= Alignment && Alignment < UIALIGN_MAX )
	{
		ElementSchema.Cells(ColumnIndex).Alignment[Orientation] = Alignment;
		for( INT ListItemIndex = 0 ; ListItemIndex < ListItems.Num() ; ListItemIndex++ )
		{
			if( ListItems(ListItemIndex).Cells.IsValidIndex(ColumnIndex))
			{
				struct FUIListElementCell& ElementCell = ListItems(ListItemIndex).Cells(ColumnIndex);
				ElementCell.Alignment[Orientation] = Alignment;
			}
		}
	}
}
...
/**
 * 스키마를 포함한 열데이터의 모든 ClipMode 값을 변경한다.
 */
void UUIComp_ListPresenter::SetColumnClipMode( INT ColumnIndex, ETextClipMode ClipMode )
{
	if( ElementSchema.Cells.IsValidIndex(ColumnIndex) && 
		0 <= ClipMode && ClipMode < CLIP_MAX )
	{		
		ElementSchema.Cells(ColumnIndex).ClipMode = ClipMode;
		for( INT ListItemIndex = 0 ; ListItemIndex < ListItems.Num() ; ListItemIndex++ )
		{
			if( ListItems(ListItemIndex).Cells.IsValidIndex(ColumnIndex))
			{
				struct FUIListElementCell& ElementCell = ListItems(ListItemIndex).Cells(ColumnIndex);
				ElementCell.ClipMode = ClipMode;
			}
		}
	}
}
...
/**
* 리스트 내부에서 사용하는 UIObject들을 갱신해준다.
*/
void UUIComp_ListPresenter::RefreshInternalUIObjectSubscriberValue( INT BindingIndex /*= INDEX_NONE*/ )
{
	for( INT ItemIndex = 0 ; ItemIndex < InternalUIObjects.Num() ; ItemIndex++ )
	{
		for( INT InstIndex = 0 ; InstIndex < InternalUIObjects(ItemIndex).Instances.Num() ; InstIndex++ )
		{
			UUIObject* Instance = InternalUIObjects(ItemIndex).Instances(InstIndex);
			IUIDataStoreSubscriber* DataStoreSubscriber = NULL;
			if( Instance != NULL &&
				(DataStoreSubscriber = (IUIDataStoreSubscriber*)Instance->GetInterfaceAddress( IUIDataStoreSubscriber::UClassType::StaticClass() ) ) != NULL)
			{
	...
	if ( SchemaProvider && CellDataBinding != NAME_None && BindingIndex >= INDEX_NONE )
	{
		...
		if ( BindingIndex >= ElementSchema.Cells.Num() )
		{
			...
			for ( INT CellIndex = ElementSchema.Cells.Num(); CellIndex <= BindingIndex; CellIndex++ )
			{
				...
				eventConstructElementSchemaCell(CellIndex);
				FUIListElementCellTemplate* EmptyCell = &ElementSchema.Cells(CellIndex);
...
//			FUIListElementCellTemplate* Cell = new(ElementSchema.Cells,InsertIndex) FUIListElementCellTemplate(EC_EventParm);
			eventConstructElementSchemaCell(InsertIndex);
			FUIListElementCellTemplate* Cell = &ElementSchema.Cells(InsertIndex);
			if ( Cell != NULL )
			{
				Cell->OnCellCreated(OwnerList);
...
void UUIComp_ListPresenter::RefreshElementSchema()
{
	...
	if ( CellProvider )
	{
		...
		if(!GIsGame)
		{
			...
			// 저장된 원래의 ColumnHeaderText가 새 ColumnHeaderText와 다르다면 수정해줘야한다
			// 예 ) LobbyRooms.RoomID -> LobbyRooms.방번호
			TArray<FString> ColumnHeaders;
			AvailableCellTags.GenerateValueArray(ColumnHeaders);
			UBOOL bColumnHeaderTextChanged = TRUE;
			for( INT CellIndex = 0 ; CellIndex < ElementSchema.Cells.Num() ; CellIndex++)
			{
				const FString* ColumnHeaderText;
				if( (ColumnHeaderText = AvailableCellTags.Find(ElementSchema.Cells(CellIndex).CellDataField)) != NULL &&
					*ColumnHeaderText != ElementSchema.Cells(CellIndex).ColumnHeaderText)
				{
					ElementSchema.Cells(CellIndex).ColumnHeaderText = *ColumnHeaderText;
					bColumnHeaderTextChanged = TRUE;
				}
			}

			if( bColumnHeaderTextChanged )
				Modify();
		}
 ...
 * @param	ElementToInsert		the element that was inserted into the list.
 *
 * @return	the index where the new element was inserted, or INDEX_NONE if the element wasn't added to the list.
 */
INT UUIComp_ListPresenter::InsertElement( INT InsertIndex, INT ElementToInsert )
{
	UUIList* OwnerList = GetOuterUUIList();
	if ( OwnerList->DataProvider )
	{
		...
		// get the schema provider, which will probably be the default object for the class that is implementing this UIListElementProvider interface
		TScriptInterface<IUIListElementCellProvider> CellProvider = OwnerList->DataProvider->GetElementCellValueProvider(OwnerList->DataSource.DataStoreField, ElementToInsert);

		//@fixme - check that CellProvider considers ElementToInsert a valid index....otherwise we'll have entries for elements in the list, but there won't be any actual data rendering
		FUIListItemDataBinding DataBinding(CellProvider, OwnerList->DataSource.DataStoreField, ElementToInsert);
		...
		//@fixed by deif
		//ListItems pointer 자체가 변경되었을 가능성이 있는데 그냥 유지하면 안된다
		for ( INT ItemIndex = 0; ItemIndex < ListItems.Num(); ItemIndex++ )
		{
			FUIListItem& ListItem = ListItems(ItemIndex);			
...
{
	UUIList* OwnerList = GetOuterUUIList();
	check(OwnerList != NULL);

	INT HitCellIndex = -1;
	if( ShouldRenderColumnHeaders() )
	{
		INT CursorX, CursorY;
		FCellHitDetectionInfo CellHitInfo;
		OwnerList->GetCursorPosition( CursorX, CursorY );
		OwnerList->CalculateCellFromPosition( FIntPoint(CursorX, CursorY), CellHitInfo);
		// 헤더를 그려줘야한다면 헤더에 맞는 해당셀만 스타일을 적용해줘야한다.
		switch(	OwnerList->CellLinkType )
		{
		case LINKED_None:
		case LINKED_Rows:		HitCellIndex = CellHitInfo.HitColumn == INDEX_NONE ? CellHitInfo.HitRow : INDEX_NONE;		break;
		case LINKED_Columns:	HitCellIndex = CellHitInfo.HitRow == INDEX_NONE ? CellHitInfo.HitColumn : INDEX_NONE;		break;
		}
	}

	UBOOL bValidSortCondition = OwnerList->SortComponent && OwnerList->SortComponent->PrimarySortColumn != INDEX_NONE;
	...
	for ( INT CellIndex = 0; CellIndex < ElementSchema.Cells.Num(); CellIndex++ )
	{
		...
		{
			if( bValidSortCondition )
			{
				UClass* DesiredState = HitCellIndex != CellIndex ? (OwnerList->IsEnabled() ? UUIState_Enabled::StaticClass() : UUIState_Disabled::StaticClass()) : NULL;
				CellTemplate.ApplyHeaderStyleData(ResolvedStyle, DesiredState);
			}
			else
			{
				CellTemplate.ApplyHeaderStyleData(ResolvedStyle, UUIState_Enabled::StaticClass());
			}
		}
	}
...
UBOOL UUIComp_ListPresenter::SetElementState( INT ElementIndex, EUIListElementState NewElementState )
{
	...
	if ( ListItems.IsValidIndex(ElementIndex) )
	{
		...
		{
			FUIListElementCell& Cell = Element.Cells(CellIndex);
			Cell.SetCellState(NewElementState);
			bResult = TRUE;
		}
...
/**
 * 셀바운드를 다시 계산할 필요가 있음
 * ( TopIndex가 바뀌었거나 셀크기가 변했거나 등 리스트의 크기는 변하지 않았는데 보여주는 셀의 형태가 변했을때 )
 */
void UUIComp_ListPresenter::RecalcCellBounds()
{
	bRecalcCellBounds = TRUE;
	...
	case LINKED_Rows:
		{
			...
			if ( ColumnWidth > 0 )
			{
				...
				const FLOAT ColumnSpacing = Owner->CellSpacing.GetValue(Owner, EVALPOS_PixelViewport);
				...
				Owner->MaxVisibleItems = TotalVisibleItems;
			}
			else
			{
	...
	case LINKED_Columns:
		{
			...
			// @TODO : 리스트내에서 항목의 높이나 너비가 다른 경우 보이는 아이템의 갯수도 변하므로 변한값을 반영
			// 채팅 내용을 길게 적어넣었을 경우 리스트의 Wrap기능으로 다음줄로 넘어가 한항목의 크기가 다른항목보다 길어질 수 있음
			if ( Owner->RowAutoSizeMode == CELLAUTOSIZE_Constrain )
			{
				FLOAT ListHeightLeft = Parameters.DrawYL - (ShouldRenderColumnHeaders() ? Owner->GetHeaderSize() : 0.f);
				INT TotalVisibleItems = 0;
				for( INT Index = Owner->TopIndex ; Index < ListItems.Num() ; Index++ )
				{
					FLOAT RowHeight = Owner->GetRowHeight(Index);
					if( ListHeightLeft - RowHeight < 0 )
						break;
					ListHeightLeft -= RowHeight;
					ListHeightLeft -= Owner->CellSpacing.GetValue(Owner, EVALPOS_PixelViewport);
					TotalVisibleItems++;
				}
				Owner->MaxVisibleItems = TotalVisibleItems;
			}
			...
			{
				const FLOAT ListHeight = Parameters.DrawYL - (ShouldRenderColumnHeaders() ? Owner->GetHeaderSize() : 0.f);
				INT TotalVisibleItems = appFloor(ListHeight / RowHeight);

				// now we have to adjust the number for spacing.  Until we know how many items would be visible, we can't take spacing into account
				// so we need to do that step now
				const FLOAT RowSpacing = Owner->CellSpacing.GetValue(Owner, EVALPOS_PixelViewport);
...
static TArray<FUIListElementCell*> ConvertElementCellArray( FUIListElementCell& Cell )
{
...
void UUIComp_ListPresenter::ResolveFacePosition( EUIWidgetFace Face )
{
	...
	if( Owner->Position.IsPositionCurrent() )
		bRecalcCellBounds = TRUE;
	...
	if ( bReapplyFormatting )
	{
		...
		{
			FRenderParameters RenderParameters = Parameters;

			// format the list elements
			ApplyListFormatting(RenderParameters);
	...
	if( bRecalcCellBounds )
	{
		FRenderParameters Parameters;
		if( GetListRenderParameters(Face, Parameters) )
		{
			Parameters.DrawXL += Parameters.DrawX;
			Parameters.DrawYL += Parameters.DrawY;
			
			// 셀들의 영역을 미리 계산
			UpdateCellBounds(Parameters);

			// 셀들의 미리계산된 영역을 바탕으로 내부 UIObject를 만들고 배치함
			// (내부 UIObject의 예) 상점의 리스트안에 들어가는 버튼
			UpdateInternalUIObject(Parameters);

			bRecalcCellBounds = FALSE;
		}
	}
}
...
UBOOL UUIComp_ListPresenter::GetListRenderParameters( EUIWidgetFace Face, FRenderParameters& out_Parameters ) const
{
	...
	if ( bFormattingConditionsMet )
	{
		//@todo ronp - move this into a method, like GetViewportDimension(out_ViewportOrigin, out_ViewportSize)
		FVector2D ViewportOrigin, ViewportSize;
		if ( !Owner->GetScene()->GetViewportOrigin(ViewportOrigin) )
		{
			ViewportOrigin.X = 0.f;
			ViewportOrigin.Y = 0.f;
		}
		if ( !Owner->GetScene()->GetViewportSize(ViewportSize) )
		{
			ViewportSize.X = UCONST_DEFAULT_SIZE_X;
			ViewportSize.Y = UCONST_DEFAULT_SIZE_Y;
		}
		...
		//{
		//	BoundingRegionWidth -= Owner->VerticalScrollbar->GetBounds(UIORIENT_Horizontal,EVALPOS_PixelViewport);
		//}
...
//		const INT NumRows = GetTotalRowCount() + ((Owner->CellLinkType != LINKED_Rows && ShouldRenderColumnHeaders()) ? 1 : 0);
		FLOAT TargetHeight = ((Owner->CellLinkType != LINKED_Rows) && ShouldRenderColumnHeaders()) ? Owner->GetHeaderSize() : 0.f;
		const INT NumRows = GetTotalRowCount();
		const FLOAT Spacing = Owner->CellLinkType != LINKED_Rows ? Owner->CellSpacing.GetValue(Owner, EVALPOS_PixelViewport) : 0.f;
	...
	if ( ShouldAdjustListBounds(UIORIENT_Horizontal) )
	{
		...
		FLOAT TargetWidth = (Owner->CellLinkType != LINKED_Columns && ShouldRenderColumnHeaders()) ? Owner->GetHeaderSize() : 0.f;
		const INT NumCols = GetTotalColumnCount();
		const FLOAT Spacing = Owner->CellLinkType != LINKED_Columns ? Owner->CellSpacing.GetValue(Owner, EVALPOS_PixelViewport) : 0.f;
...
/**
 * Store the widget's cell-bounds according to the current list setting.
 */
void UUIComp_ListPresenter::UpdateCellBounds( struct FRenderParameters& Parameters )
{
	UUIList* Owner = GetOuterUUIList();
	...
	if ( Owner->VerticalScrollbar != NULL && Owner->VerticalScrollbar->IsVisible() )
	{
		Parameters.DrawXL -= Owner->VerticalScrollbar->GetBounds(UIORIENT_Horizontal,EVALPOS_PixelViewport);
	}

	FRenderParameters CellParameters = Parameters;
	BYTE CellLinkType = Owner->CellLinkType;
	...
	if ( ShouldRenderColumnHeaders() )
	{
		...
		{
		case LINKED_None:
			CellParameters.DrawXL = CellParameters.DrawX + Owner->GetColumnWidth();
			CellParameters.DrawYL = CellParameters.DrawY + Owner->GetRowHeight();
			break;

		case LINKED_Rows:
			CellParameters.DrawXL = CellParameters.DrawX + Owner->GetHeaderSize();
			break;

		case LINKED_Columns:
			CellParameters.DrawYL = CellParameters.DrawY + Owner->GetHeaderSize();
			break;
		}
		...
		for( INT CellIndex = 0 ; CellIndex < ElementSchema.Cells.Num() ; CellIndex++ )
		{
			CalculateCellBounds( INDEX_NONE, CellIndex, CellParameters);
		}

		// 헤더 영역을 그리기 가능 영역에서 빼준다
		CellParameters = Parameters;
		switch( CellLinkType )
		{
		case LINKED_Rows:
			CellParameters.DrawX += Owner->GetHeaderSize();
			break;
		case LINKED_Columns:
		case LINKED_None:
			CellParameters.DrawY += Owner->GetHeaderSize();
			break;
		}
	}

	// 빈 항목을 그릴 EmptyCell을 초기화
	ElementSchema.EmptyCells.Reset();
	
	FRenderParameters ElementParameters = CellParameters;
	INT TopIndex = ListItems.IsValidIndex(Owner->TopIndex) ? Owner->TopIndex : 0;
	for( INT ElementIndex = TopIndex ; ElementIndex < TopIndex + Owner->MaxVisibleItems ; ElementIndex++ )
	{
		// 내용이 있는 셀의 경계정보 획득
		if( ListItems.IsValidIndex( ElementIndex ) )
		{
			FUIListItem& ListItem = ListItems(ElementIndex);
			for( INT CellIndex = 0 ; CellIndex < ListItem.Cells.Num() ; CellIndex++ )
			{
				CalculateCellBounds( ElementIndex, CellIndex, ElementParameters );
			}
			
			// LINKED_Column일 때는 한 행 전체의 경계정보를
			// LINKED_Rows일 때는 한 열 전체의 경계정보를 저장한다 (bCellUnitDrawing이 아닐 때 사용한다)
			FUIListElementCell& ElemCell = ListItem.ElementCell;
			for( INT FaceIndex = 0 ; FaceIndex < ARRAY_COUNT(ElemCell.Bounds) ; FaceIndex++ )
				ElemCell.Bounds[FaceIndex] = 0;
			if( ListItem.Cells.Num() > 0 )
			{
				for( INT FaceIndex = 0 ; FaceIndex < ARRAY_COUNT(ElemCell.Bounds) ; FaceIndex++ )
					ElemCell.Bounds[FaceIndex] = ListItem.Cells(0).Bounds[FaceIndex];
				for( INT CellIndex = 1 ; CellIndex < ListItem.Cells.Num() ; CellIndex++ )
				{
					const FUIListElementCell& Cell = ListItem.Cells(CellIndex);
					for( INT FaceIndex = 0 ; FaceIndex < UIFACE_MAX ; FaceIndex++ )
					{
						if( FaceIndex == UIFACE_Left || FaceIndex == UIFACE_Top )
							ElemCell.Bounds[FaceIndex] = Min(Cell.Bounds[FaceIndex], ElemCell.Bounds[FaceIndex]);
						else
							ElemCell.Bounds[FaceIndex] = Max(Cell.Bounds[FaceIndex], ElemCell.Bounds[FaceIndex]);
					}
				}
				ElemCell.bVisible = TRUE;
			}
		}
		// 내용이 없는 셀의 경계정보 획득
		else
		{
			...
			CalculateCellBounds( ElementIndex, INDEX_NONE, ElementParameters );
		}

		// LINKED_Columns의 경우 한 행을 LINKED_Rows경우 한 열을 넘어가야하므로
		// 다음 행 또는 열의 값으로 CellParameter를 갱신해준다
		FLOAT CellSpacing = Owner->CellSpacing.GetValue(Owner, EVALPOS_PixelViewport);
		switch(CellLinkType)
		{
		case LINKED_Columns:
			ElementParameters.DrawX = CellParameters.DrawX;
			ElementParameters.DrawY += (Owner->GetRowHeight(ElementIndex) + CellSpacing);
			break;
		case LINKED_Rows:
			ElementParameters.DrawX += (Owner->GetColumnWidth(ElementIndex) + CellSpacing);
			ElementParameters.DrawY = CellParameters.DrawY;
			break;
		case LINKED_None:
			{
				const INT MaxColumns = GetMaxNumVisibleColumns();
				const INT DefaultElemWidth = Owner->GetColumnWidth();
				const INT DefaultElemHeight = Owner->GetRowHeight();

				INT ColumnIndex = ( Owner->TopIndex + ElementIndex + 1) % MaxColumns;
				if ( ColumnIndex == 0 )
				{
					ElementParameters.DrawX = Owner->RenderBounds[UIFACE_Left];
					ElementParameters.DrawY += DefaultElemHeight;
					if( ElementIndex < Owner->GetItemCount() - MaxColumns )
						ElementParameters.DrawY += CellSpacing;
				}
				else
				{
					ElementParameters.DrawX = Parameters.DrawX + (DefaultElemWidth + CellSpacing) * ColumnIndex;
				}
			}
			break;
		}
...
UBOOL UUIComp_ListPresenter::CalculateCellBounds( INT ElementIndex, INT CellIndex, FRenderParameters& Parameters)
{
	UUIList* Owner = GetOuterUUIList();
	...
	// 2. 실제 내용을 담고 있는 셀
	// 3. 빈 내용을 표시해줄 셀
	UBOOL bCellTemplate = ElementIndex == INDEX_NONE;
	UBOOL bEmptyCell = !bCellTemplate && !ListItems.IsValidIndex(ElementIndex);

	if( !bCellTemplate && !bEmptyCell && !ListItems(ElementIndex).Cells.IsValidIndex(CellIndex) )
		return FALSE;

	if( bCellTemplate && !ElementSchema.Cells.IsValidIndex(CellIndex) )
		return FALSE;

	if( bEmptyCell )
	{
		INT Index = ElementSchema.EmptyCells.AddZeroed();
	}

	FUIListElementCell& Cell = bCellTemplate ? ElementSchema.Cells(CellIndex) :
								bEmptyCell ? ElementSchema.EmptyCells(ElementSchema.EmptyCells.Num() - 1) :
								ListItems(ElementIndex).Cells(CellIndex);

	for( INT FaceIndex = 0 ; FaceIndex < ARRAY_COUNT(Cell.Bounds) ; FaceIndex++ )
		Cell.Bounds[FaceIndex] = 0;
	Cell.bVisible = TRUE;
	...
	if( bEmptyCell )
	{
		...
		{
			CellWidth = Parameters.DrawXL - Parameters.DrawX;
			CellHeight = Owner->GetRowHeight(ElementIndex);
		}
		else if ( CellLinkType == LINKED_Rows )
		{
			CellWidth = Owner->GetColumnWidth( ElementIndex );
			CellHeight = Parameters.DrawYL -  Parameters.DrawY;
		}
		else if( CellLinkType == LINKED_None )
		{
		...
		{
			checkf(FALSE,TEXT("Inacceptable linktype %d"),CellLinkType);
		}
		
		Cell.Bounds[UIFACE_Left] = Parameters.DrawX;
		Cell.Bounds[UIFACE_Top] = Parameters.DrawY;
		Cell.Bounds[UIFACE_Right] = (Parameters.DrawX + CellWidth);
		Cell.Bounds[UIFACE_Bottom] = (Parameters.DrawY + CellHeight);

		// Cell Bound가 List RenderBound를 넘지 않았나 검사
		switch( CellLinkType )
		{
		case LINKED_Columns:
		case LINKED_None:
			if ( Cell.Bounds[UIFACE_Bottom] > Parameters.DrawYL )
				Cell.bVisible = FALSE;
			break;
		case LINKED_Rows:
			if ( Cell.Bounds[UIFACE_Right] > Parameters.DrawXL )
				Cell.bVisible = FALSE;
			break;
		default:
			checkf(FALSE,TEXT("Inacceptable linktype %d"),CellLinkType);
	...
	else
	{
		...
		if ( CellLinkType == LINKED_Columns )
		{
			CellHeight = bCellTemplate ? Owner->GetHeaderSize() : Owner->GetRowHeight(ElementIndex);
			CellWidth = Owner->GetColumnWidth( CellIndex );
		}
		else if ( CellLinkType == LINKED_Rows )
		{
			CellHeight = Owner->GetRowHeight( CellIndex );
			CellWidth = bCellTemplate ? Owner->GetHeaderSize() : Owner->GetColumnWidth( ElementIndex );
		}
		else if( CellLinkType == LINKED_None )
		{
			CellWidth = Owner->GetColumnWidth( CellIndex );
		...
		Cell.Bounds[UIFACE_Left] = Parameters.DrawX;
		Cell.Bounds[UIFACE_Top] = Parameters.DrawY;
		Cell.Bounds[UIFACE_Right] = (CellLinkType == LINKED_Columns || CellLinkType == LINKED_None) ? (Parameters.DrawX += CellWidth) : (Parameters.DrawX + CellWidth);
		Cell.Bounds[UIFACE_Bottom] = (CellLinkType == LINKED_Rows) ? (Parameters.DrawY += CellHeight) : (Parameters.DrawY + CellHeight);
		...
		// 마지막 셀은 경계부분까지 늘여서 그린다
		UBOOL bLastCell = CellIndex == ( bCellTemplate ? ElementSchema.Cells.Num() - 1 : ListItems(ElementIndex).Cells.Num() - 1 );
		if( bLastCell )
		{
			if( CellLinkType == LINKED_Columns )
				Cell.Bounds[UIFACE_Right] = Parameters.DrawXL;
			//else if( CellLinkType == LINKED_None )
			//{
			//	const INT MaxColumns = GetMaxNumVisibleColumns();
			//	const INT DefaultColumnWidth = Owner->GetColumnWidth();

			//	INT ColumnIndex = ( Owner->TopIndex + ElementIndex + 1) % MaxColumns;
			//	Cell.Bounds[UIFACE_Right] = Parameters.DrawXL - (MaxColumns - ColumnIndex - 1)*DefaultColumnWidth;
			//}
			//Cell.Bounds[UIFACE_Bottom] = (CellLinkType == LINKED_Rows) ? Parameters.DrawYL : Cell.Bounds[UIFACE_Bottom];
		}
	}

	return Cell.bVisible;
}

void UUIComp_ListPresenter::UpdateInternalUIObject( struct FRenderParameters& Parameters )
{
	UUIList* Owner = GetOuterUUIList();

	INT TopIndex = Owner->TopIndex == INDEX_NONE ? 0 : Owner->TopIndex;
	INT VisibleItems = Owner->MaxVisibleItems;
	const INT VisibleListItemCount = Min( ListItems.Num() - TopIndex, VisibleItems );
	const INT InstanceCount = VisibleListItemCount;

	for( INT ObjectIndex = 0 ; ObjectIndex < InternalUIObjects.Num() ; ObjectIndex++ )
	{
		FInternalUIObjectItem& InternalUIItem = InternalUIObjects(ObjectIndex);
		UUIObject* ArcheType = InternalUIItem.Archetype;
		UUIObject* Parent = (GIsEditor && !GIsGame) ? ArcheType : Owner;

		if( ArcheType == NULL )
			continue;

		// @TODO : 기존에 만들어놓은 Instance가 있다면 그대로 쓰도록 고쳐야한다.
		// InternalUIObject를 리셋 Reset
		if( InternalUIItem.Instances.Num() != InstanceCount )
		{
			Parent->RemoveChildren(InternalUIItem.Instances);
			InternalUIItem.Instances.Empty();
		}

		IUIDataStoreSubscriber* DataStoreSubscriberSource = (IUIDataStoreSubscriber*)ArcheType->GetInterfaceAddress( IUIDataStoreSubscriber::UClassType::StaticClass() );

		// 개수가 모자를 때 CreateWidget으로 채워넣음
		if( InternalUIItem.Instances.Num() == 0 )
		{
			for( INT ChildIndex = 0 ; ChildIndex < InstanceCount ; ChildIndex++ )
			{				
				UUIObject* WidgetCreated = CastChecked<UUIObject>(StaticDuplicateObject(ArcheType,ArcheType, UUIObject::GetTransientPackage(), TEXT("")) );
				WidgetCreated->SetFlags( RF_Transient | RF_Obsolete );
				WidgetCreated->eventSetVisibility(TRUE);
				InternalUIItem.Instances.AddItem(WidgetCreated);
			}

			for( INT ChildIndex = 0 ; ChildIndex < InternalUIItem.Instances.Num() ; ChildIndex++ )
			{
				if( InternalUIItem.Instances(ChildIndex) != NULL )
				{
					UUIObject* Child = InternalUIItem.Instances(ChildIndex);
	...
	TArray<FVector2D> CellDiffs(InternalUIObjects.Num());
	{
		FVector2D FirstCellPos = ListItems.IsValidIndex(TopIndex) ? FVector2D(ListItems(TopIndex).ElementCell.Bounds[UIFACE_Left],ListItems(TopIndex).ElementCell.Bounds[UIFACE_Top]) : FVector2D(0,0);
		for( INT ObjectIndex = 0 ; ObjectIndex < InternalUIObjects.Num() ; ObjectIndex++ )
		{
			UUIObject* ArcheType = InternalUIObjects(ObjectIndex).Archetype;
			FVector2D ArcheTypePos = ArcheType ? FVector2D( ArcheType->GetPosition(UIFACE_Left, EVALPOS_PixelViewport), ArcheType->GetPosition(UIFACE_Top,EVALPOS_PixelViewport)) : FVector2D(0,0);
			CellDiffs(ObjectIndex) = ArcheTypePos - FirstCellPos;

			if( (!GIsEditor || GIsGame) && ArcheType && ArcheType->IsVisible() )
				ArcheType->eventSetVisibility(FALSE);
		}
	}
	
	// Internal UIObject들이 제자리에 위치하도록 수정
	for( INT ElementIndex = TopIndex ; ElementIndex < (TopIndex + VisibleItems) ; ElementIndex++)
	{
		if( !ListItems.IsValidIndex(ElementIndex) )
			break;

		FUIListItem& ListItem = ListItems(ElementIndex);
		FUIListElementCell& Cell = ListItem.ElementCell;

		for( INT ObjectIndex = 0 ; ObjectIndex < InternalUIObjects.Num() ; ObjectIndex++ )
		{
			INT InstIndex = ElementIndex - TopIndex;
			
			FInternalUIObjectItem& UIObjectItem = InternalUIObjects(ObjectIndex);
			if( ! UIObjectItem.Instances.IsValidIndex( InstIndex ) )
				continue;

			UUIObject* Instance = UIObjectItem.Instances( InstIndex );
			if( Instance )
			{
				FVector2D DiffPixel = CellDiffs.IsValidIndex(ObjectIndex) ? CellDiffs(ObjectIndex) : FVector2D(0,0);
				Instance->SetPosition( Cell.Bounds[UIFACE_Left] + DiffPixel.X, UIFACE_Left, EVALPOS_PixelViewport );
				Instance->SetPosition( Cell.Bounds[UIFACE_Top] + DiffPixel.Y, UIFACE_Top, EVALPOS_PixelViewport );
				
				// @TODO : UIObject가 임의의의 파라메터를 가질 수 있도록 수정.
				// 우선 ListButton에 한해서 파라메터를 사용하도록 하였음.
				UUIListButton* ListButton = Cast<UUIListButton>(Instance);
				if( ListButton )
					ListButton->ElementIndex = ElementIndex;
			}
		}
	}
}
...
//			if ( ElementIndex % MaxColumns == 0 && (ElementIndex > 0 || MaxColumns == 1) )
			if ( ( Owner->TopIndex + ElementIndex + 1) % MaxColumns == 0 )
			{
				Parameters.DrawX = Owner->RenderBounds[UIFACE_Left];
				//Parameters.DrawY += Owner->GetRowHeight(ElementIndex);
				Parameters.DrawY += Owner->GetRowHeight();
				if ( ElementIndex < Owner->GetItemCount() - MaxColumns )
				{
					Parameters.DrawY += Owner->CellSpacing.GetValue(Owner,EVALPOS_PixelViewport);
				}
			}
			else
			{
				// otherwise, move to the next column
				//Parameters.DrawX += Owner->GetColumnWidth(ElementIndex) + Owner->CellSpacing.GetValue(Owner,EVALPOS_PixelViewport);
				Parameters.DrawX += Owner->GetColumnWidth() + Owner->CellSpacing.GetValue(Owner,EVALPOS_PixelViewport);
			}
		}
	}
}
...
void UUIComp_ListPresenter::ApplyCellFormatting( TArray<FUIListElementCell*> Cells, FRenderParameters& Parameters )
{
	...
	if ( Cells.Num() > 0 )
	{
		...
		const FLOAT CellPadding = Owner->CellPadding.GetValue(Owner, EVALPOS_PixelViewport);
		...
		for ( INT CellIndex = 0; CellIndex < Cells.Num(); CellIndex++ )
		{
			...
			if ( Owner->CellLinkType == LINKED_Columns )
			{
				...
				if ( ClipX - CellParameters.DrawX < appFloor(CellWidth) )
				{
					if (ElementCell.ValueString == NULL
						...
						||	ElementCell.ValueString->StringClipMode == CLIP_None || ElementCell.ValueString->StringClipMode == CLIP_Wrap )
					{
			...
			if ( ElementCell.ValueString != NULL )
			{
				...
				ElementCell.ValueString->StringClipMode = ElementCell.ClipMode;
		...
		{
		case LINKED_Rows:
			Parameters.DrawX += CellWidth + Owner->CellSpacing.GetValue(Owner,EVALPOS_PixelViewport);
			break;

		case LINKED_Columns:
			Parameters.DrawY += CellHeight + Owner->CellSpacing.GetValue(Owner,EVALPOS_PixelViewport);
			break;
		}
	}
	else
...
void UUIComp_ListPresenter::Render_List( FCanvas* Canvas )
{
	...
	if ( ShouldRenderColumnHeaders() )
	{
		...
		case LINKED_Rows:
			ColumnHeaderParameters.DrawXL = Parameters.DrawX + Owner->GetHeaderSize();
			break;

		case LINKED_Columns:
			ColumnHeaderParameters.DrawYL = Parameters.DrawY + Owner->GetHeaderSize();
			break;
		}

		// render the column header (this only works for 
		//Render_Cells(Canvas, INDEX_NONE, ColumnHeaderParameters );
		Render_Cells(Canvas, ConvertElementCellArray(ElementSchema.Cells), TRUE);
	...
	UBOOL bDrawVislbles = Owner->bDrawVisibleItems && !Owner->bCellUnitDrawing;
	const INT NumItems = Owner->GetItemCount();
	for ( INT ListIndex = Owner->TopIndex; ListIndex < Owner->TopIndex + Owner->MaxVisibleItems; ListIndex++ )
	{
		INT ItemIndex = ListIndex;
	...
	if( bDrawVislbles )
	{
		for( INT CellIndex = 0 ; CellIndex < ElementSchema.EmptyCells.Num() ; CellIndex++ )
			Render_Cells(Canvas, ConvertElementCellArray(ElementSchema.EmptyCells(CellIndex)),FALSE,TRUE);
	}
}
...
void UUIComp_ListPresenter::Render_ListElement( FCanvas* Canvas, INT ElementIndex, FRenderParameters& Parameters )
{
	...
	const FLOAT ClipY = Parameters.DrawYL;

	UBOOL bCellUnitDraw = Owner->bCellUnitDrawing;

	if ( ListItems.IsValidIndex(ElementIndex) )
	{
		/*const*/ FUIListItem& ListItem = ListItems(ElementIndex);

		struct FCellPriority
		{
			FCellPriority( INT InCellIndex, INT InPriority ) : CellIndex(InCellIndex), Priority(InPriority) {}
			INT CellIndex;
			INT Priority;
			UBOOL operator<( const FCellPriority& rhs ) { return Priority < rhs.Priority; }
		};

		TArray<FCellPriority> PriorityList;
		for( INT CellIndex = 0 ; CellIndex < ElementSchema.Cells.Num() ; CellIndex++ )
		{
			if( ElementSchema.Cells.IsValidIndex(CellIndex) )
			{
				const FUIListElementCellTemplate& Cell = ElementSchema.Cells(CellIndex);
				new(PriorityList) FCellPriority(CellIndex, Cell.DrawingPriority);
			}
		}

		ava::sort( &PriorityList(0), &PriorityList(0) + PriorityList.Num() );

		TArray<FUIListElementCell*> SortedCells;
		for( INT ListIndex = 0 ; ListIndex < PriorityList.Num() ; ListIndex++ )
		{
			SortedCells.AddItem( &ListItem.Cells(PriorityList(ListIndex).CellIndex) );
		}

		// 셀단위 배경이 아니라 항목단위 배경이여야할 때는 항목 배경을 그린다.
		if( !bCellUnitDraw )
		{
			ListItem.ElementCell.CellState = ListItem.Cells.Num() > 0 ? ListItem.Cells(0).CellState : ELEMENT_Normal;
			Render_Cells( Canvas, ConvertElementCellArray(ListItem.ElementCell), FALSE, TRUE );
		}
		// 배경과 해당 문자열을 그린다.
		Render_Cells( Canvas, SortedCells );
	}
}
 ...
 * @param	Canvas			the canvas to use for rendering
 * @param	Cells			the list of cells to render
 * @param	bCellTemplate	지금 그리려고 하는 셀들이 셀 템플릿(스키마, 항목표시줄의 셀)인지 아닌지 알려준다.
 * @param	bForceDraw		UUIList.bCellUnitDrawing일때도 강제로 항목을 그려줌
 */
void UUIComp_ListPresenter::Render_Cells( FCanvas *Canvas, const TArray<struct FUIListElementCell*> Cells, UBOOL bCellTemplate /*= FALSE*/, UBOOL bForceDraw /*= FALSE*/ )
{
	if( Cells.Num() == 0 )
		return;
	...
	// 항목표시줄은 언제나 Cell단위로 그려야한다.
	UBOOL bCellUnitDraw = Owner->bCellUnitDrawing || bCellTemplate;

	for( INT CellIndex = 0 ; CellIndex < Cells.Num() ; CellIndex++ )
	{
		const FUIListElementCell& Cell = *Cells(CellIndex);
		if( !Cell.bVisible )
			continue;

		EUIListElementState CellState = static_cast<EUIListElementState>(Cell.CellState);
		FRenderParameters CellParameters(Cell.Bounds[UIFACE_Left], Cell.Bounds[UIFACE_Top], Cell.Bounds[UIFACE_Right] - Cell.Bounds[UIFACE_Left], Cell.Bounds[UIFACE_Bottom] - Cell.Bounds[UIFACE_Top]);

		if( Owner->ShouldRenderDataBindings() )
		{
			// render a box which displays the outline for this cell
			FVector2D StartLoc(CellParameters.DrawX, CellParameters.DrawY);
			FVector2D EndLoc(CellParameters.DrawX + CellParameters.DrawXL, CellParameters.DrawY + CellParameters.DrawYL);
			DrawBox2D(Canvas, StartLoc, EndLoc, FColor(0,255,255));	// draw an aqua box to show the bounds of this label

			if( GIsEditor && !GIsGame &&
				Owner->bDebugShowBounds && Cell.ValueString )
				DrawString(Canvas, CellParameters.DrawX, CellParameters.DrawY + 10, *FString::Printf(TEXT("X=%.2f,Y=%.2f,XL=%.2f,YL=%.2f"),Cell.Bounds[UIFACE_Left], Cell.Bounds[UIFACE_Top], Cell.Bounds[UIFACE_Right], Cell.Bounds[UIFACE_Bottom]), Cell.ValueString->StringStyleData.DrawFont,FLinearColor::White);
		}

		if( bForceDraw || bCellUnitDraw )
		{
			if( bCellTemplate )
			{
				FUIListElementCellTemplate* CellTemplate = (FUIListElementCellTemplate*)&Cell;
				if( CellTemplate->Background != NULL )
					CellTemplate->Background->Render_Texture( Canvas, CellParameters );
			}
			else
			{
				if( CellState == ELEMENT_Active && Owner->SelectionOverlayComponent )
					Owner->SelectionOverlayComponent->RenderComponent(Canvas, CellParameters);
				else
					ListItemOverlay[CellState]->Render_Texture(Canvas, CellParameters);
			}
		}

		if( Cell.ValueString )
		{
			FLOAT CellPadding = Owner->CellPadding.GetValue(Owner, EVALPOS_PixelViewport);
			FRenderParameters StringParameters(
				CellParameters.DrawX + 0.5f * CellPadding,
				CellParameters.DrawY + 0.5f * CellPadding,
				CellParameters.DrawXL - CellPadding,
				CellParameters.DrawYL - CellPadding);

			for( INT i = 0 ; i < UIORIENT_MAX ; i++ )
				StringParameters.TextAlignment[i] = Cell.Alignment[i];

			Cell.ValueString->Render_String(Canvas, StringParameters);
		}
	}
}
...
void UUIComp_ListPresenter::PostEditChange( FEditPropertyChain& PropertyThatChanged )
{
	...
	if ( PropertyThatChanged.Num() > 0 )
	{
		...
		if ( OutermostProperty != NULL )
		{
			...
			else if ( OuterPropertyName == TEXT("InternalUIObjects") ||
					OuterPropertyName == TEXT("bRefresh") ||
					OuterPropertyName == TEXT("bRemove") )
			{
				for( INT ObjectIndex = 0 ; ObjectIndex < InternalUIObjects.Num() ; ObjectIndex++ )
				{
					FInternalUIObjectItem& UIObjectItem = InternalUIObjects(ObjectIndex);
					UUIObject*& UIObject = UIObjectItem.Archetype;
					if( !UIObject )
						continue;

					if( !UIObject->IsInitialized() )
					{
						UObject* UIObjectArchetype = UIObject->GetClass()->GetDefaultObject<UObject>();
						UIObject = Owner->CreateWidget( Owner, UIObject->GetClass(), UIObjectArchetype);
						for( INT FaceIndex = 0 ; FaceIndex < UIFACE_MAX ; FaceIndex++ )
							UIObject->Position.ChangeScaleType( Owner, (EUIWidgetFace)FaceIndex, EVALPOS_PixelOwner);
						UIObject->SetPosition(100, UIFACE_Right, EVALPOS_PixelOwner);
						UIObject->SetPosition(25, UIFACE_Bottom, EVALPOS_PixelOwner);
						Owner->InsertChild(UIObject);
					}
					if( UIObjectItem.bRefresh )
					{
						UIObject->RemoveChildren( UIObjectItem.Instances );
						UIObjectItem.Instances.Empty();

						UIObjectItem.bRefresh = FALSE;
					}
					if( UIObjectItem.bRemove )
					{
						Owner->RemoveChild(UIObject);
						UIObject = NULL;
						UIObjectItem.Instances.Empty();
						UIObjectItem.bRemove = FALSE;
					}
				}
			}
		}
...
namespace
{
	...
	public:
		...
		{
			const INT IndexA = DataSourceIndices->FindItemIndex(A.DataSource.DataSourceIndex);
	...
	if(ContainerRightFace>ContainerLeftFace)
	{
		...
		for ( INT ChildIndex = 0; ChildIndex < ChildWidgets.Num(); ChildIndex++ )
		{
			...
			if(ChildRightFace>ChildLeftFace)
			{
				...
				Child->SetPosition(ChildNewLeftFace, UIFACE_Left, EVALPOS_PixelViewport, TRUE);
				Child->SetPosition(ChildNewRightFace, UIFACE_Right, EVALPOS_PixelViewport, TRUE);
			}
		}
	}
}
	...
	if(ContainerBottomFace>ContainerTopFace)
	{
		...
		for ( INT ChildIndex = 0; ChildIndex < ChildWidgets.Num(); ChildIndex++ )
		{
			...
			if(ChildBottomFace>ChildTopFace)
			{
				...
				// Modify Child position while preserving its current size, the original scale type of the child face 
				// is not modified by this call, the absolute pixel value will be automatically converted into child face scale type
				Child->SetPosition(ChildNewTopFace, UIFACE_Top, EVALPOS_PixelViewport, TRUE);
				Child->SetPosition(ChildNewBottomFace, UIFACE_Bottom, EVALPOS_PixelViewport, TRUE);
			}
		}
	}
}
...
UUIList* UUIComp_ListComponentBase::GetOuterUUIList() const
{
	return Cast<UUIList>(GetOuter());
}

UUIObject* UUIComp_DrawComponents::GetOuterUUIObject() const 
{
	return Cast<UUIObject>(GetOuter());
}
