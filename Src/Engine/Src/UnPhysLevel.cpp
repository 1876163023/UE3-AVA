...
DECLARE_CYCLE_STAT(TEXT("Effect Dynamics Time"),STAT_EffectDynamicsTime,STATGROUP_Physics);
...
DECLARE_DWORD_COUNTER_STAT(TEXT("Total HW Dynamic Bodies"),STAT_TotalHWDynamicBodies,STATGROUP_Physics);
DECLARE_DWORD_COUNTER_STAT(TEXT("Awake HW Dynamic Bodies"),STAT_AwakeHWDynamicBodies,STATGROUP_Physics);
DECLARE_DWORD_COUNTER_STAT(TEXT("Total Effect Bodies"),STAT_TotalEffectBodies,STATGROUP_Physics);
...
static void DoPhysXRegistryHack(void);
	...
	BYTE GCurrentEpicPhysDataVersion = 14; // deif increased :)
...
FRBPhysScene* CreateRBPhysScene(const FVector& Gravity)
{
...
#if PS3
	// TEMPFIX: Disable SPU broadphase - fixes vehicles on terrain.
	...
	NovodexScene->setActorGroupPairFlags(UNX_GROUP_DEFAULT,			UNX_GROUP_NOTIFYCOLLIDE,	NX_NOTIFY_ON_TOUCH);
	NovodexScene->setActorGroupPairFlags(UNX_GROUP_NOTIFYCOLLIDE,	UNX_GROUP_NOTIFYCOLLIDE,	NX_NOTIFY_ON_TOUCH);
	NovodexScene->setActorGroupPairFlags(UNX_GROUP_MODIFYCONTACT,	UNX_GROUP_NOTIFYCOLLIDE,	NX_NOTIFY_ON_TOUCH | NX_NOTIFY_CONTACT_MODIFICATION);
		...
		{		
			for(UINT i=0; i<nProfileData->numZones; i++)
			{
				...
				if( Stat )
				{
					...
					Stat->Cycles += nZone->hierTime / (1000000.f * GSecondsPerCycle);
				}
...
#if STATS
	FStatGroup* MemGroup = GStatManager.GetGroup(STATGROUP_Physics);
