...
/* Hack!! AVA/deif, Render thread 

TileRenderer에서 Fog가 적용되는 것을 막기 위해
*/
UBOOL GHackForceFogless = FALSE;
...
*/
void FHeightFogVertexShaderParameters::Set(FCommandContextRHI* Context, const FMaterialInstance* MaterialInstance, const FViewInfo& View, FShader* VertexShader) const
{
	// Set the fog constants.
	if ( !GHackForceFogless && !MaterialInstance->GetMaterial()->IsDecalMaterial() )
	{
		SetVertexShaderValue(Context,VertexShader->GetVertexShader(),FogMinHeightParameter,View.FogMinHeight);
		SetVertexShaderValue(Context,VertexShader->GetVertexShader(),FogMaxHeightParameter,View.FogMaxHeight);
		...
		SetVertexShaderValue(Context,VertexShader->GetVertexShader(),FogDistanceScaleParameter,View.FogDistanceScale);
		SetVertexShaderValue(Context,VertexShader->GetVertexShader(),FogExtinctionDistanceParameter,View.FogExtinctionDistance);
		SetVertexShaderValue(Context,VertexShader->GetVertexShader(),FogStartDistanceParameter,View.FogStartDistance);
	}
	else
	{
...
class FHeightFogVertexShader : public FGlobalShader
{
	DECLARE_SHADER_TYPE(FHeightFogVertexShader,Global);
public:
	...
	FHeightFogVertexShader( )	{ }
	FHeightFogVertexShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer):
	...
	{		
		...
		// Set the view constants.
		SetVertexShaderValue(Context,GetVertexShader(),ScreenToWorldParameter,
			FMatrix(
				FPlane(1,0,0,0),
				FPlane(0,1,0,0),
...
IMPLEMENT_SHADER_TYPE(,FHeightFogVertexShader,TEXT("HeightFogVertexShader"),TEXT("Main"),SF_Vertex,0,0);

/** A vertex shader for rendering height fog. */
class FHeightFogPixelShader : public FGlobalShader
{
	DECLARE_SHADER_TYPE(FHeightFogPixelShader,Global);
public:
	...
	FHeightFogPixelShader( )	{ }
	FHeightFogPixelShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer):
		FGlobalShader(Initializer)
	{
	...
	void SetParameters(FCommandContextRHI* Context,const FViewInfo& View)
	{
		// Set the texture containing the scene color.
		...
		if(SceneDepthTextureParameter.IsBound())
		{
			...
			RHISetViewPixelParameters( Context, &View, GetPixelShader(), &SceneDepthCalcParameter, NULL );
		}
...
IMPLEMENT_SHADER_TYPE(,FHeightFogPixelShader,TEXT("HeightFogPixelShader"),TEXT("Main"),SF_Pixel,VER_FP_BLENDING_FALLBACK,0);
...
void FSceneRenderer::InitFogConstants()
{
	for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
	{
		...
		if((View.Family->ShowFlags & SHOW_Fog) && GSystemSettings->bAllowFog)
		{
...
FGlobalBoundShaderStateRHIRef FSceneRenderer::FogBoundShaderState;
...
UBOOL FSceneRenderer::RenderFog(UINT DPGIndex)
{
	if((DPGIndex == SDPG_World) && (Scene->Fogs.Num()))
	{
		...
		// Find the cached shaders.
		TShaderMapRef<FHeightFogVertexShader> VertexShader(GetGlobalShaderMap());
		TShaderMapRef<FHeightFogPixelShader> HeightFogPixelShader(GetGlobalShaderMap());

		SetGlobalBoundShaderState(GlobalContext, FogBoundShaderState, GFogVertexDeclaration.VertexDeclarationRHI, *VertexShader, *HeightFogPixelShader, sizeof(FVector2D));
		...
		for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
		{
			...
			HeightFogPixelShader->SetParameters(GlobalContext,View);
