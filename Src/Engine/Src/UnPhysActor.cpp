...
void AActor::SyncActorToRBPhysics()
{
	...
	else
	{
		...
		{
			debugf(TEXT("AActor::SyncActorToRBPhysics: Invalid or Missing BodyInstance in CollisionComponent. (check to make certain your object actually has a collision volume) for: %s"), *GetFullName());
		}
	}
 ...
 */
// 2006/3/22 Sleeping 이면 RBState 를 가지고 올 수 올 수 없다.
// Sleeping 인 경우에는 return false 를 함으로써 replication 시키지 않기 위함이나,
// Sleeping 인 경우에도 RBState 를 가지고 오며, return 을 원래대로 유지하기 위해서 수정했음
...
UBOOL ADynamicSMActor::HasStaticShadowing() const 
{ 
	return !(LightEnvironment && LightEnvironment->bEnabled); 
}

// {{ 20061130 dEAthcURe|HM
#ifdef EnableHostMigration
void AKActor::hmSerialize(FArchive &Ar)
{
	...
	if (Ar.IsLoading() ) {
	_hms_defLoading;
	_hms_loadValue(bDamageAppliesImpulse); // BITFIELD bDamageAppliesImpulse:1 GCC_BITFIELD_MAGIC;
	_hms_loadValue(bWakeOnLevelStart); // BITFIELD bWakeOnLevelStart:1;
	_hms_loadValue(bCurrentSlide); // BITFIELD bCurrentSlide:1;
	_hms_loadValue(bSlideActive); // BITFIELD bSlideActive:1;
	}
	else {
	_hms_defSaving;
	_hms_saveValue(bDamageAppliesImpulse); // BITFIELD bDamageAppliesImpulse:1 GCC_BITFIELD_MAGIC;
	_hms_saveValue(bWakeOnLevelStart); // BITFIELD bWakeOnLevelStart:1;
	_hms_saveValue(bCurrentSlide); // BITFIELD bCurrentSlide:1;
	_hms_saveValue(bSlideActive); // BITFIELD bSlideActive:1;
	}

	Ar << DrawScaleX; // FLOAT DrawScaleX;
	Ar << DrawScaleY; // FLOAT DrawScaleY;
	Ar << DrawScaleZ; // FLOAT DrawScaleZ;
	Ar << LastImpactTime; // FLOAT LastImpactTime;
	Ar << InitialLocation; // FVector InitialLocation;
	Ar << InitialRotation; // FRotator InitialRotation;
	Ar << AngErrorAccumulator; // FLOAT AngErrorAccumulator;
	Ar << LastSlideTime; // FLOAT LastSlideTime;
	_hms_serializeBytes(RBState); // FRigidBodyState RBState;
	*/
...
void AActor::OnRigidBodyCollision(const FRigidBodyCollisionInfo& Info0, const FRigidBodyCollisionInfo& Info1, const FCollisionImpactData& RigidCollisionData)
{
	...
	for(INT Idx = 0; Idx < GeneratedEvents.Num(); Idx++)
	{
		...
		if (CollideEvent != NULL)
		{
			if(bOriginatorIs0)
			{
				CollideEvent->CheckRBCollisionActivate(Info0, Info1, RigidCollisionData.ContactInfos, VelMag);
			}
			else
			{
				CollideEvent->CheckRBCollisionActivate(Info1, Info0, RigidCollisionData.ContactInfos, VelMag);
			}
		}
	}
}
