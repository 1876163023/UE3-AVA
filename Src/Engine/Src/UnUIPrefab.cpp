...
UBOOL UUIPrefab::CreateWidgetArchetypes( TArray<FArchetypeInstancePair>& WidgetPairs, const FBox& BoundingRegion )
{
	...
	// get a list of all the objects we just created, including nested child widgets, components, subobjects, etc.
	TArray<UObject*> ArchetypeObjects;
	FArchiveObjectReferenceCollector RefCollector(
		&ArchetypeObjects,	//	InObjectArray
		this,				//	LimitOuter
		FALSE,				//	bRequireDirectOuter
		TRUE,				//	bIgnoreReferencesThroughArchetype
		TRUE,				//	bSerializeRecursively
		TRUE);				//	bIgnoreTransientReferences
	Serialize(RefCollector);

	// now we have a list of all objects contained within this archetype wrapper which are referenced by the archetype widgets
	for ( INT ArcIndex = 0; ArcIndex < ArchetypeObjects.Num(); ArcIndex++ )
	{
		ArchetypeObjects(ArcIndex)->SetFlags(RF_Public|RF_ArchetypeObject|RF_Transactional);
	}
...
UUIPrefabInstance* UUIPrefab::InstancePrefab( UUIScreenObject* DestinationOwner, FName DestinationName )
{
	...
	if ( DestinationOwner != NULL )
	{
		...
		PrefabInstance->Rename(*DestinationName.ToString(), NULL, REN_ForceNoResetLoaders);
...
void UUIPrefab::ConvertObjectReferences( TMap<UObject*,UObject*>& ReplacementMap, UBOOL bNullPrivateRefs, TArray<UObject*>* pSourceObjects/*=NULL*/ )
{
	...
	if ( pSourceObjects == NULL )
	{
		...
		TMap<UObject*, UObject*>::TConstIterator it(ReplacementMap);
		for( ; it ; ++it)
			SourceObjects.AddUniqueItem(it.Value());
