...
void UAnimNode::SaveCachedResults(const TArray<FBoneAtom>& NewAtoms, const FBoneAtom& NewRootMotionDelta, INT bNewHasRootMotion)
{
	...
	// Do not cache results for nodes which only have one parent- it is uncessary.
...
void UAnimNodeCrossfader::execPlayOneShotAnim _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void UAnimNodeCrossfader::execBlendToLoopingAnim _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(AnimSeqName);
	P_GET_FLOAT_OPTX(BlendInTime,0.f);
	P_GET_FLOAT_OPTX(Rate,1.f);
...
void UAnimNodeBlendMultiBone::execSetTargetStartBone _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(TargetIdx);
	...
	while( DesiredPos < DesiredNum && RequiredPos < RequiredNum )
	{
		...
		if( AimCompIndex != INDEX_NONE )
		{
			...
			else
			{
				...
				{
					QuaternionOffset	= BiLerpQuat(AimCpnt.CD.Quaternion, AimCpnt.RD.Quaternion, AimCpnt.CC.Quaternion, AimCpnt.RC.Quaternion, SafeAim.X, SafeAim.Y+1.f);
				...
				{
					QuaternionOffset	= BiLerpQuat(AimCpnt.LC.Quaternion, AimCpnt.CC.Quaternion, AimCpnt.LU.Quaternion, AimCpnt.CU.Quaternion, SafeAim.X+1.f, SafeAim.Y);
				...
				{
					QuaternionOffset	= BiLerpQuat(AimCpnt.LD.Quaternion, AimCpnt.CD.Quaternion, AimCpnt.LC.Quaternion, AimCpnt.CC.Quaternion, SafeAim.X+1.f, SafeAim.Y+1.f);
...
void UAnimNodeBlendByBase::TickAnim( FLOAT DeltaSeconds, FLOAT TotalWeight )
{
	...
	if( AOwner && AOwner->Base != CachedBase )
	{
		...
		{
			SetActiveChild(DesiredChildIdx, bJustBecameRelevant ? 0.f : BlendTime);
		}
	}
...
void UAnimNodeSlot::MAT_SetAnimPosition(INT ChannelIndex, FName InAnimSeqName, FLOAT InPosition, UBOOL bFireNotifies)
{
