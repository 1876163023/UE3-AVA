...
{
	// if the static data was never initlaized, do it now
	if (!bInitialized)
	{
		bInitialized = TRUE;		
		...
		Data.TangentBasisComponents[1] = FVertexStreamComponent(
			&VertexBuffer,STRUCT_OFFSET(FMaterialTileVertex,TangentY),sizeof(FMaterialTileVertex),VET_PackedNormal);
		Data.TangentBasisComponents[2] = FVertexStreamComponent(
...
void FTileRenderer::DrawTile(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialInstance* MaterialInstance)
{
	...
	PrepareShaders(Context, View, MaterialInstance, FMatrix::Identity, TRUE);
}

void FTileRenderer::DrawTile(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialInstance* MaterialInstance, FLOAT X, FLOAT Y, FLOAT SizeX, FLOAT SizeY, UBOOL bIsHitTesting, const FHitProxyId HitProxyId)
{
	// @GEMINI_TODO: Fix my matrices below :(
DrawTile(Context, View, MaterialInstance, X, Y, SizeX, SizeY, 0, 0, 1, 1, bIsHitTesting, HitProxyId);
return;
	...
	if (bIsHitTesting)
	{
		DrawTile(View, MaterialInstance, X, Y, SizeX, SizeY, 0, 0, 1, 1, bIsHitTesting, HitProxyId);
		return;
	}
...
void FTileRenderer::DrawTile(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialInstance* MaterialInstance, FLOAT X, FLOAT Y, FLOAT SizeX, FLOAT SizeY, FLOAT U, FLOAT V, FLOAT SizeU, FLOAT SizeV, UBOOL bIsHitTesting, const FHitProxyId HitProxyId)
{
if (bIsHitTesting)
{
	...
	PrepareShaders(Context, View, MaterialInstance, FMatrix::Identity, FALSE, bIsHitTesting, HitProxyId);
}




void FTileRenderer::PrepareShaders(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialInstance* MaterialInstance, const FMatrix& LocalToWorld, UBOOL bUseIdentityViewProjection, UBOOL bIsHitTesting, const FHitProxyId HitProxyId)
{
	...
	/* HACK! AVA */
	extern UBOOL GHackForceFogless;
	GHackForceFogless = TRUE; 
	...
	else
	{
		...
		{
			FEmissiveDrawingPolicyFactory::DrawDynamicMesh(Context, &View, FEmissiveDrawingPolicyFactory::ContextType(), Mesh, FALSE, FALSE, NULL, HitProxyId);
		}
	}		
