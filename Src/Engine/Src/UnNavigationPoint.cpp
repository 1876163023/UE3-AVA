/*=============================================================================
  ...
  NavigationPoint and subclass functions
	...
	Copyright 2001-2005 Epic Games, Inc. All Rights Reserved.
=============================================================================*/
...
{
	return ( !PhysicsVolume->bWaterVolume && !bNotBased && CylinderComponent );
}
	...
	for (ANavigationPoint *Nav = GWorld->GetFirstNavigationPoint(); Nav != NULL; Nav = Nav->nextNavigationPoint)
	{
		if((!bOnlyChanged || bPathsChanged || Nav->bPathsChanged) && Nav != this)
		{
			...
			if( !ProscribePathTo(Nav, Scout) )
			{
				...
				if( !bForcedPath && !bDestinationOnly && CanConnectTo( Nav, TRUE ) && newSpec->defineFor( this, Nav, Scout ) )
				{
...
void ANavigationPoint::ClearForPathFinding()
{
	...
	// check to see if we should nuke our anchored pawn
...
UBOOL ANavigationPoint::FindAlternatePath(UReachSpec* StraightPath, INT AccumulatedDistance)
{
	...
	for (INT i = 0; i < PathList.Num(); i++)
	{
		if ( !PathList(i)->bPruned 
			&& (PathList(i)->End.Nav == StraightPath->End.Nav)
			&& ((StraightDir | (StraightPath->End->Location - Location).SafeNormal()) >= 0.f) 
			)
		{
	...
	{
		if ( !PathList(i)->bPruned 
			&& !PathList(i)->bSkipPrune
			&& (PathList(i)->Distance > 0)
			&& (AccumulatedDistance + PathList(i)->Distance < appTrunc(PATHPRUNING * StraightPath->Distance))
			&& (PathList(i)->End != StraightPath->Start)
			&& (*PathList(i)->End != NULL)
			&& (*PathList(i) <= *StraightPath)
			&& ((StraightDir | (PathList(i)->End->Location - Location).SafeNormal()) > 0.f)
			)
		{
			if( PathList(i)->End->FindAlternatePath(StraightPath, AccumulatedDistance + PathList(i)->Distance) )
			{
...
void ACoverSlotMarker::AddForcedSpecs( AScout *Scout )
{
	...
	if( !Slot )
	{
  	...
  	if( Link->bLooped )
  	{
  		...
  		{
  			// Link to last
  			ACoverSlotMarker* TargetMarker = Link->Slots(LastIdx).SlotMarker;
  			ForcePathTo( TargetMarker, Scout, UForcedReachSpec::StaticClass() );
  		}
  		else 
  		// Otherwise, if last slot
  		if( SlotIdx == LastIdx )
  		{
  			// Link to first
  			ACoverSlotMarker* TargetMarker = Link->Slots(0).SlotMarker;
  			ForcePathTo( TargetMarker, Scout, UForcedReachSpec::StaticClass() );
  		}
  	}
  	INT PrevIdx = SlotIdx - 1;
  	INT NextIdx = SlotIdx + 1;
  	if( PrevIdx >= 0 )
  	{
  		// Link to previous slot
  		ACoverSlotMarker* TargetMarker = Link->Slots(PrevIdx).SlotMarker;
  		ForcePathTo( TargetMarker, Scout, UForcedReachSpec::StaticClass() );
  	}
  	if( NextIdx <= LastIdx )
  	{
  		// Link to next slot
  		ACoverSlotMarker* TargetMarker = Link->Slots(NextIdx).SlotMarker;
  		ForcePathTo( TargetMarker, Scout, UForcedReachSpec::StaticClass() );
  	}
#endif
	...
	{
		TargSlot = ACoverLink::CoverRefToSlotPtr( Slot->TurnTarget(Idx) );
		if( TargSlot && TargSlot->SlotMarker )
		{
			// Remove previous normal paths
			for( INT Idx = 0; Idx < PathList.Num(); Idx++ )
			...
			ForcePathTo( TargSlot->SlotMarker, Scout, USwatTurnReachSpec::StaticClass() );
		}
	}
}
...
UBOOL ACoverSlotMarker::CanConnectTo( ANavigationPoint* Nav, UBOOL bCheckDistance )
{
	...
	// Movement between them will be handled by cover physics
...
UBOOL ACoverLink::FindCoverEdges(const FVector& StartLoc, FVector AxisX, FVector AxisY, FVector AxisZ)
{
	...
	while (!bFoundEdge && IterationCount++ < 150)
	{
			...
			if (!GWorld->SingleLineCheck(Hit,this,End,Start,TRACE_World,FVector(1.f)))
			{
				...
				if (GWorld->SingleLineCheck(Hit,this,End,Start,TRACE_World,FVector(1.f)))
				{
					...
					if (LastSlot.CoverType == CT_Standing)
					{
						...
						if (!GWorld->SingleLineCheck(Hit,this,End,Start,TRACE_World,FVector(1.f)))
						{
							debugf(TEXT(">--+ traced back to find edge"));
		...
		else
		{
			...
			// hit a side wall, throw down a slot and adjust
...
void ACoverLink::EditorAutoSetup(FVector Direction)
{
	...
	if (!GWorld->SingleLineCheck(Hit,this,Location + Direction * 256.f,Location,TRACE_World,FVector(1.f)))
	{
		...
		//@fixme - hook in at a higher level so we get the proxy instead, so that we don't have to attempt to rely on a separate trace?
		if (Abs(Hit.Normal | FVector(0,0,1)) > 0.3f)
		{
			debugf(TEXT("%s invalid surface normal"),*GetName());
			GWorld->DestroyActor(this);
			return;
		}
		//@debug - show trace
		//DrawDebugLine(Hit.Location,Location,0,255,0,1);

		SetRotation((Hit.Normal * -1).Rotation());
		SetLocation(Hit.Location + Hit.Normal * 128.f);
		FindBase();
		...
		if (!FindCoverEdges(Location + AxisX * 96.f,AxisX,AxisY,AxisZ))
		{
			debugf(TEXT("%s failed to place any slots"),*GetName());
			GWorld->DestroyActor(this);
			return;
		}
...
static UBOOL CanSwatTurn( AScout* Scout, ACoverLink* SrcLink, INT SrcSlotIdx, FRotationMatrix& R, INT Direction )
{
	...
	{
		// Set item info
		FCoverReference Info;
		Info.Nav			= BestTargetLink;
		Info.Guid			= BestTargetLink->NavGuid;
		Info.SlotIdx		= BestSlotIdx;
		Info.Direction		= Direction;

		// Add new item
		SrcLink->Slots(SrcSlotIdx).TurnTarget.AddItem( Info );
...
void ACoverLink::BuildSlotInfo(INT SlotIdx)
{
	...
	// if in game be sure to nuke the scout
			...
			// in various directions
			{
				...
				if (BestCheckResult.Actor != NULL)
				{
					...
					if (!GWorld->SingleLineCheck(CheckResultA,
												 this,
												 SlotLocation + (X * (AlignDist*4.f) + Y * 32.f),
												 SlotLocation + Y * 32.f,
												 TRACE_World,
												 FVector(1.f)) &&
					...
					FVector CheckLocation = SlotLocation + FVector(0,0,8);
					INT AdjustCounter = 16;
					UBOOL bFits = FALSE;
					while( !bFits && AdjustCounter-- > 0 )
					{
//						DrawDebugBox( CheckLocation, Scout->GetCylinderExtent(), 255, 0, 0, TRUE );

						// Try to fit scout in check spot
						bFits = GWorld->FarMoveActor( Scout, CheckLocation, TRUE, FALSE, FALSE );
						// If couldn't fit
						if( !bFits )
						{
							// Move check spot out from the wall
							CheckLocation -= NewRotation.Vector() * 4.f;
						}
					}
					Slot.LocationOffset = RotMatrix.InverseTransformFVector(CheckLocation - Location);
					SlotLocation = GetSlotLocation(SlotIdx);
				}
...
void ACoverLink::execGetFireLinkTo _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(SlotIdx);
...
void ACoverLink::execHasFireLinkTo _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(SlotIdx);
			...
			if ( !ProscribePathTo(Nav, Scout) )
			{
				...
				else if ( !bDestinationOnly )
				{
						...
						if ( !Hit.Actor || (Hit.Actor == Nav) )
						{
							...
							else if ( Dist2D < CylinderComponent->CollisionRadius )
							{
								...
								else if ( Location.Z > Nav->Location.Z )
								{
									...
									{
                                        GWorld->SingleLineCheck( Hit, this, Intersect, Nav->Location, TRACE_World|TRACE_StopAtAnyHit, FVector(Scout->PathSizes(SizeIndex).Radius,Scout->PathSizes(SizeIndex).Radius,1.f) );
										if ( !Hit.Actor || (Hit.Actor == Nav) )
										{
					...
					if ( SpecRadius > 0.f )
					{
						...
						debugf(NAME_DevPath, TEXT("***********added new spec from %s to %s size %f %f"),*GetName(),*Nav->GetName(), SpecRadius, SpecHeight);
					}
				}
			}
		}
