...
void* RHILockVertexBuffer(FVertexBufferRHIParamRef VertexBuffer,UINT Offset,UINT Size,UBOOL bAppend)
{
	void* Data = GetStaticBuffer();
	return Data;
}
...
, bIsFullscreen(bInIsFullscreen)
{
	...
	BackBuffer = RHICreateTargetable3DSurface(
		SizeX,
		SizeY,
		PF_A8R8G8B8,
		Texture2D,
...
void* RHILockTexture2D(FTexture2DRHIParamRef Texture,UINT MipIndex,UBOOL bIsDataBeingWrittenTo,UINT& DestStride)
{
	void* Data = GetStaticBuffer();
...
void RHIUnlockTexture2D(FTexture2DRHIParamRef Texture,UINT MipIndex)
{
}

//  [2006/11/20 YTS, temporarily inserted ]
FTexture2DRHIRef RHICreateTexture3D(UINT SizeX,UINT SizeY,UINT SizeZ,BYTE Format,UINT NumMips,DWORD Flags)
{
	FTexture3DRHIRef Texture3D;
	return Texture3D;
}

void* RHILockTexture3D(FTexture3DRHIParamRef Texture,UINT MipIndex,UBOOL bIsDataBeingWrittenTo,UINT& DestRowPitch,UINT& DestSlidePitch)
{
	void* Data = GetStaticBuffer();
	DestRowPitch = 0;
	DestSlidePitch = 0;
...
FSurfaceRHIRef RHICreateTargetableVolumeSurface(
	UINT SizeX,
	UINT SizeY,
	BYTE Format,
