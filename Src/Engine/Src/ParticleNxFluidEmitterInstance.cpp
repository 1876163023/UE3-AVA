...
IncrementPacketCulling( FPhysXFluidTypeData & PhysXFluid )
{
	...
	INT MinPacketStartIndex = FluidPacketData.bufferFluidPackets[0].firstParticleIndex;
	INT MinPacketSize = FluidPacketData.bufferFluidPackets[0].numParticles;

	for(INT i = 1; i < NumParticlePackets; ++i )
	{
		NxFluidPacket& FluidPacket = FluidPacketData.bufferFluidPackets[i];
		if( (INT)FluidPacket.numParticles < MinPacketSize )
		{
			MinPacketSize = FluidPacket.numParticles;
			MinPacketStartIndex = FluidPacket.firstParticleIndex;
		}
	}

	// Cull Min. size packet
	...
	for( INT i = 0; i < MinPacketStartIndex; ++i )
	{
		UpdateData.bufferFlag[i] = 0;
	}
	for( INT i = MinPacketStartIndex; i < MinPacketStartIndex+MinPacketSize; ++i )
	{
		UpdateData.bufferFlag[i] = (NxU32)NX_FP_DELETE;
	}
	for( INT i = MinPacketStartIndex+MinPacketSize; i < PhysXFluid.FluidNumParticles; ++i )
	{
		UpdateData.bufferFlag[i] = 0;
	}
...
void FParticleSpriteNxFluidEmitterInstance::SetParticleData(DWORD NumParticles)
{
	...
	if( MaxActiveParticles < (INT)FluidData.FluidNumParticles )
	{
		Resize( FluidData.FluidMaxParticles );
		check( MaxActiveParticles == FluidData.FluidMaxParticles );
		for( INT i = FluidData.FluidNumParticles; i < MaxActiveParticles; ++i )
		{
			ParticleIndices[i] = i;
