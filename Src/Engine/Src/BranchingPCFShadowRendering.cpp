...
IMPLEMENT_BPCF_SHADER_TYPE(FLowQualityManualPCF,LowQualityShaderName,TEXT("BranchingPCFProjectionPixelShader"),TEXT("Main"),SF_Pixel, VER_EPIC_MERGE_APR_6);

typedef FBranchingPCFProjectionPixelShader<FMediumQualityManualPCF> MediumQualityShaderName;
IMPLEMENT_BPCF_SHADER_TYPE(FMediumQualityManualPCF,MediumQualityShaderName,TEXT("BranchingPCFProjectionPixelShader"),TEXT("Main"),SF_Pixel, VER_EPIC_MERGE_APR_6);

typedef FBranchingPCFProjectionPixelShader<FHighQualityManualPCF> HighQualityShaderName;
IMPLEMENT_BPCF_SHADER_TYPE(FHighQualityManualPCF,HighQualityShaderName,TEXT("BranchingPCFProjectionPixelShader"),TEXT("Main"),SF_Pixel, VER_EPIC_MERGE_APR_6);

// 3 quality levels of hardware PCF
typedef FBranchingPCFProjectionPixelShader<FLowQualityHwPCF> HwPCFLowQualityShaderName;
IMPLEMENT_BPCF_SHADER_TYPE(FLowQualityHwPCF,HwPCFLowQualityShaderName,TEXT("BranchingPCFProjectionPixelShader"),TEXT("HardwarePCFMain"),SF_Pixel, VER_EPIC_MERGE_APR_6);

typedef FBranchingPCFProjectionPixelShader<FMediumQualityHwPCF> HwPCFMediumQualityShaderName;
IMPLEMENT_BPCF_SHADER_TYPE(FMediumQualityHwPCF,HwPCFMediumQualityShaderName,TEXT("BranchingPCFProjectionPixelShader"),TEXT("HardwarePCFMain"),SF_Pixel, VER_EPIC_MERGE_APR_6);

typedef FBranchingPCFProjectionPixelShader<FHighQualityHwPCF> HwPCFHighQualityShaderName;
IMPLEMENT_BPCF_SHADER_TYPE(FHighQualityHwPCF,HwPCFHighQualityShaderName,TEXT("BranchingPCFProjectionPixelShader"),TEXT("HardwarePCFMain"),SF_Pixel, VER_EPIC_MERGE_APR_6);
								   ...
								   BYTE ShadowProjectionTechnique) 
{
	...
	BranchingPCFPixelShader->SetParameters(Context,View,ShadowInfo);
...
UBOOL ShouldUseBranchingPCF(BYTE ShadowProjectionTechnique)
{
	...
	//always use Branching PCF if Hardware PCF is not supported,
	//since the other filters look very bad without Hw PCF
	if (!GSupportsHardwarePCF)
	{
		return TRUE;
	}
	...
	if ((ShadowProjectionTechnique == ShadowProjTech_Default) && GEngine->bEnableBranchingPCFShadows)
	{
		return TRUE;
	}
...
*/
FGlobalBoundShaderStateRHIRef* ChooseBPCFBoundShaderState(
	BYTE ShadowProjectionTechnique,
	...
	FGlobalBoundShaderStateRHIRef* HighQualityBoundShaderState)
{
	...
	if (ShadowProjectionTechnique == ShadowProjTech_Default && GEngine->ShadowFilterQuality == SFQ_Low
		|| ShadowProjectionTechnique == ShadowProjTech_BPCF_Low) 
	{
		return LowQualityBoundShaderState;
	}
	else if (ShadowProjectionTechnique == ShadowProjTech_Default && GEngine->ShadowFilterQuality == SFQ_Medium
		|| ShadowProjectionTechnique == ShadowProjTech_BPCF_Medium)
	{
		return MediumQualityBoundShaderState;
	}
	else 
	{
		return HighQualityBoundShaderState;
	}
}
