...
void UDownload::StaticConstructor()
{
	...
	Class->EmitObjectReference( STRUCT_OFFSET( UDownload, Connection ) );
}
...
void UDownload::DownloadDone()
{
	...
	else
	{
		...
		else
		{
			...
			CacheIni.SetString(TEXT("Cache"), *Info->Guid.String(), *Info->Parent->GetName(), IniName);
...
{
	UClass* Class = GetClass();
	Class->EmitObjectReference( STRUCT_OFFSET( UChannelDownload, Ch ) );
}
 ...
 */
void UChannelDownload::InitializeIntrinsicPropertyValues()
{
