...
// arbitrary min/max
static const FLOAT MAX_FAR_PLANE = 10000.f;
...
EShowFlags USceneCaptureComponent::GetSceneShowFlags()
{
	...
	EShowFlags Result = SHOW_DefaultGame;
...
*/
UBOOL FSceneCaptureProbe::UpdateRequired(FLOAT CurrentTime) const
{
	checkSlow(CurrentTime >= LastCaptureTime);
    return (	(TimeBetweenCaptures == 0 && LastCaptureTime == 0) ||
				(TimeBetweenCaptures > 0 && (CurrentTime - LastCaptureTime) >= TimeBetweenCaptures)		);
}
...
* @param CurrentTime - seconds since start of play in the world
*/
void FSceneCaptureProbe::UpdateLastCaptureTime(FLOAT CurrentTime)
{
...
void FSceneCaptureProbe2D::CaptureScene( FSceneRenderer* MainSceneRenderer )
{
	...
	if( RTResource &&
		...
		UpdateRequired(MainSceneRenderer->ViewFamily.CurrentWorldTime) )
	{	
		UpdateLastCaptureTime(MainSceneRenderer->ViewFamily.CurrentWorldTime);
		...
		RHICopyToResolveTarget(RTResource->RenderTargetSurfaceRHI, FALSE);
...
void FSceneCaptureProbeCube::CaptureScene( FSceneRenderer* MainSceneRenderer )
{
		...
		UpdateRequired(MainSceneRenderer->ViewFamily.CurrentWorldTime) )
	{
		...
		FSceneView* ParentView = (FSceneView*)MainSceneRenderer->ViewFamily.Views(0);
		// projection matrix based on the fov,near/far clip settings
		...
		for( INT FaceIdx=CubeFace_PosX; FaceIdx < CubeFace_MAX; FaceIdx++ )
		{
			...
			RHICopyToResolveTarget(RTResource->RenderTargetSurfaceRHI, FALSE,ResolveParams);
...
void FSceneCaptureProbeReflect::CaptureScene( FSceneRenderer* MainSceneRenderer )
{
	...
	{	
		...
		// reverse global cull mode for mesh rendering
		GInvertCullMode = TRUE;
		// render the scene to the target
		CaptureSceneRenderer->Render();
		// restore global cull mode
		GInvertCullMode = FALSE;
...
void USceneCapturePortalComponent::execSetCaptureParameters _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	...
	// update capture probe by readding it to the scene
	if( Scene )
	{
		Scene->RemoveSceneCapture(this);
		Scene->AddSceneCapture(this);
...
FSceneCaptureProbe* USceneCapturePortalComponent::CreateSceneCaptureProbe() 
{
	...
	FMatrix SrcWorldToLocalM( FTranslationMatrix(-SrcActor->Location) *
		FInverseRotationMatrix( FVector(-SrcActor->Rotation.Vector()).Rotation() ) );
...
void FSceneCaptureProbePortal::CaptureScene( FSceneRenderer* MainSceneRenderer )
{
	...
	{	
		...
		for( INT ViewIdx=0; ViewIdx < MaxViewIdx; ViewIdx++ )
		{
			...
			// use the same projeciton matrix as the portal should be mapped to the screen
			FMatrix ProjMatrix = ParentView->ProjectionMatrix;
...
void APortalTeleporter::PostEditChange(UProperty* PropertyThatChanged)
{
	...
	if( PortalComponent &&
		PropertyThatChanged->GetFName() == FName(TEXT("ViewDestination")) )
	{
	...
	if( PortalComponent &&
		PropertyThatChanged->GetFName() == FName(TEXT("SisterPortal")) )
	{
		PortalComponent->ViewDestination = SisterPortal;
	}
...
UBOOL APortalTeleporter::TransformActor(AActor* A)
{
	...
	if (SisterPortal == NULL || !CanTeleport(A))
	{
		return FALSE;
	}
