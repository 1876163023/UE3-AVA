...
void UUISkin::NotifyStyleModified( UUIStyle* Style )
{
	...
	const FName* OldTag = StyleNameMap.FindKey(Style);
		...
		for ( TMap<UUIState*,UUIStyle_Data*>::TIterator It(StateDataMap); It; ++It )
		{
			...
			if ( StyleDataArchetype != NULL )
			{
				...
				if ( ComboStyleData != NULL )
				{
					...
					else if ( ! ArchetypeComboData->TextStyle.IsCustomStyleDataValid() )
					{
						//// @ TODO : Archetype 복원 코드
						//StyleData->SetArchetype( UUIStyle_Combo::StaticClass()->GetDefaultObject() , TRUE );
						//StyleData->MarkPackageDirty();
					}
					else
					{
						...
						if ( CustomTextDataArchetype != CustomTextData->GetArchetype() )
						{
					...
					else if ( !ArchetypeComboData->ImageStyle.IsCustomStyleDataValid() )
					{
						//// @TODO : ArcheType복원 코드
						//StyleData->SetArchetype( UUIStyle_Combo::StaticClass()->GetDefaultObject() , TRUE );
						//StyleData->MarkPackageDirty();
					...
					else
					{
						...
						if ( CustomImageDataArchetype != CustomImageData->GetArchetype() )
						{
...
void UUIStyle::SetDirtiness( UBOOL bIsDirty, UUIStyle_Data* Target/*=NULL*/ )
{
	if ( Target != NULL )
	{
		...
		UUIState* const * StateKey = StateDataMap.FindKey(Target);
...
UBOOL UUIStyle_Data::MatchesStyleData( UUIStyle_Data* StyleToCompare ) const
{
	...
	{
		UBOOL bSameScaleCurve = TRUE;
		for( INT i = 0 ; i < ARRAY_COUNT(this->ScalePresetCurve) ; i++ )
			if( StyleToCompare->ScalePresetCurve[i] != this->ScalePresetCurve[i] )
				bSameScaleCurve = FALSE;

		UBOOL bSameScaleAxis = TRUE;
		for( INT i = 0 ; i < ARRAY_COUNT(this->ScaleAxis) ; i++ )
			if( StyleToCompare->ScaleAxis[i] != this->ScaleAxis[i] )
				bSameScaleAxis = FALSE;

		bResult = bSameScaleCurve && bSameScaleAxis 
				&& StyleToCompare->StyleColor == this->StyleColor
				&& StyleToCompare->FadeDuration == this->FadeDuration
				&& StyleToCompare->FadePresetCurve == this->FadePresetCurve
				&& StyleToCompare->ScaleDuration == this->ScaleDuration;
...
UBOOL FUITextStyleOverride::CustomizeAlignment( EUIOrientation Orientation, EUIAlignment& OriginalAlignment ) const
{
	...
	{
		OriginalAlignment = (EUIAlignment)TextAlignment[Orientation];
		return TRUE;
	}
