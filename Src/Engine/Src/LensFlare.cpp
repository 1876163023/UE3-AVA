...
void ULensFlareComponent::BeginDestroy()
{
	if (SceneInfo)
	{
		...
		if (SceneProxy)
		{
			FLensFlareDynamicData* DynamicData = SceneProxy->GetDynamicData();
			if (DynamicData)
			{
				DynamicData->ReleaseRenderResources(this, SceneProxy);
			}
		}
	}
...
UBOOL ULensFlareComponent::HasUnlitDistortion() const
{
	...
	if (Template->SourceElement.bIsEnabled == TRUE)
	{
		...
		{
			MaterialInstance = Template->SourceElement.LFMaterials(MaterialIndex);
			if (MaterialInstance)
			{
	...
	if (bHasDistortion == FALSE)
	{
		for (INT ElementIndex = 0; ElementIndex < Template->Reflections.Num(); ElementIndex++)
		{
			...
			if (Element.bIsEnabled == TRUE)
			{
				...
				{
					MaterialInstance = Element.LFMaterials(MaterialIndex);
					if (MaterialInstance)
					{
						Material = MaterialInstance->GetMaterial();
						if (Material)
