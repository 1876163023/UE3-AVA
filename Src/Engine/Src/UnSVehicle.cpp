...
// {{ 20070807 dEAthcURe|HM
#ifdef EnableHostMigration
void ASVehicle::hmSerialize(FArchive &Ar)
{
	Super::hmSerialize(Ar);

	if ( Ar.IsLoading() ) {
		float AngErrorAccumulator = 0.0f;
		BYTE oldPhysics = Physics;

		// 값을 적용하기 위해서 잠시 PHYS_RigidBody로 변경하고 함수를 호출해 주고
		// 원래 값으로 복원해 준다. 그렇지 않으면 Location값에 처음 VehicleFactory에서 
		// Spawned Location위치로 갱신되서 처음 위치로 돌아가게 된다.(2006/08/07 고광록)
		Physics = PHYS_RigidBody;

/*
		// 게임중 Host가 나갔다가 다시 들어와서 Host가 된 경우,
		// 탱크가 Stop중이면 HM전에 RBState.Position값이 Spawn위치가 되어서
		// 탱크에 EU군이 닿는 순간 PHYS_RigidBody가 되어 다시 Spawn된 위치로 돌아가게 된다.
		FVector Delta = Location - VState.RBState.Position;
		// 위의 경우를 거리로 판별하여 현재 위치를 복원된 Location으로 갱신해준다.(2006/09/05 고광록)
		if ( Delta.SizeSquared() > 300 * 300 )
		{
//			debugf(TEXT("avaVehicle.hmSerialize - Restore: Pos(%f,%f,%f) = Loc(%f,%f,%f)"), 
//				VState.RBState.Position.X, VState.RBState.Position.Y, VState.RBState.Position.Z,
//				Location.X, Location.Y, Location.Z);
//			FQuat q = Rotation.Quaternion();
//			debugf(TEXT("avaVehicle.hmSerialize - Restore: Quat(%f,%f,%f,%f) = Rot(%f,%f,%f,%f)"), 
//				VState.RBState.Quaternion.X, VState.RBState.Quaternion.Y, VState.RBState.Quaternion.Z, VState.RBState.Quaternion.W, 
//				q.X, q.Y, q.Z, q.W);

			VState.RBState.Position = Location;
			VState.RBState.Quaternion = Rotation.Quaternion();
		}
*/
		...
		SetLocation(LastLocation + FVector(0,0,3));
		SetRotation(LastRotation);

		VState.RBState.Position   = LastLocation + FVector(0,0,3);
		VState.RBState.Quaternion = LastRotation.Quaternion();

		ApplyNewRBState(VState.RBState, &AngErrorAccumulator);
		Physics = oldPhysics;
	}
}
#endif
...
static FMatrix MakeWheelRelRotMatrix(const FMatrix& ActorToWorld, const FMatrix& CompToWorld)
{
	...
	FVector ComponentCapX = CompToWorld.InverseTransformNormal(WorldCapX);
	FVector ComponentCapY = CompToWorld.InverseTransformNormal(WorldCapY);
	FVector ComponentCapZ = CompToWorld.InverseTransformNormal(WorldCapZ);
 ...
 */
void ASVehicle::ModifyNxActorDesc(NxActorDesc& ActorDesc,UPrimitiveComponent* PrimComp)
{
	check(Mesh);
	...
	for(INT i=0; i<Wheels.Num(); i++)
	{
		...
		NxGroupsMask NewMask = CreateGroupsMask(Mesh->RBChannel, &TempChannels);
...
void ASVehicle::OnRigidBodyCollision(const FRigidBodyCollisionInfo& Info0, const FRigidBodyCollisionInfo& Info1, const FCollisionImpactData& RigidCollisionData)
{
...
{
	if(NewPhysics == PHYS_RigidBody || NewPhysics == PHYS_None)
	{
...
void ASVehicle::TickAuthoritative( FLOAT DeltaSeconds )
{
	...
	{
		NetUpdateTime = GWorld->GetTimeSeconds() - 1; // force quick net update
	}
}
...
void ASVehicle::physRigidBody(FLOAT DeltaTime)
{
	...
	for(INT i=0; i<Wheels.Num(); i++)
	{
		...
		if(bHaveContact)
		{
			...
			bVehicleOnWater = (ShapeMask.bits0 & WaterMask.bits0);
	...
	if(Role == ROLE_Authority)
	{
		...
		VState.ServerViewPitch = DriverViewPitch;
		VState.ServerViewYaw = DriverViewYaw;
	}
	...
	else	
	{
		...
		DriverViewPitch = VState.ServerViewPitch;
		DriverViewYaw = VState.ServerViewYaw;
	}
	...
	for(INT i=0; i<Wheels.Num(); i++)
	{
		...
		USkelControlWheel* SkelControl = Cast<USkelControlWheel>( Mesh->FindSkelControl(vw->SkelControlName) );
		if(SkelControl)
		{
			SkelControl->UpdateWheelControl( vw->SuspensionPosition, vw->CurrentRotation, vw->Steer );
		}
		...
		if(vw->WheelParticleComp)
		{
			...
			vw->WheelParticleComp->SetFloatParameter(vw->SlipParticleParamName, UseSlipVel);

			// Update position of particle system to always be at base of tire (point where it hits the ground).
			vw->WheelParticleComp->Translation = CompToActor.TransformFVector(vw->WheelPosition) - FVector(0.f, 0.f, vw->WheelRadius - vw->SuspensionPosition);
			vw->WheelParticleComp->Translation /= TotalScale;
		}
...
void ASVehicle::PostEditChange(UProperty* PropertyThatChanged)
{
 ...
 */
void ASVehicle::InitVehicleRagdoll(USkeletalMesh* RagdollMesh, UPhysicsAsset* RagdollPhysAsset, FVector ActorMove)
{
	if(bDeleteMe)
	{
	...
	Mesh->SetSkeletalMesh(RagdollMesh);
