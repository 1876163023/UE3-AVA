...
void FFinalSkinVertexBuffer::InitDynamicRHI()
{
	...
	for( UINT VertexIdx=0; VertexIdx < LodModel.NumVertices; VertexIdx++ )
	{
		...
		DestVertex->TangentY = SrcVertex->TangentY;
		...
		IsValidRef(MeshLOD.VertexBuffer.VertexBufferRHI) )
	{
		...
		if( DynamicData->ClothPosData.Num() > 0 && LODIndex == 0 && DynamicData->ClothBlendWeight > 0.f)
		{
			...
			for(INT i=0; i<NumSkinnedClothVerts; i++)
			{
				...
				DestVertex[GraphicsIndex].TangentY = TangentY;
...
void FSkeletalMeshObjectCPUSkin::FSkeletalMeshObjectLOD::InitResources()
{
	...
	VertexFactoryData->TangentBasisComponents[1] = FVertexStreamComponent(
		&VertexBuffer,STRUCT_OFFSET(FFinalSkinVertex,TangentY),sizeof(FFinalSkinVertex),VET_PackedNormal);
...
void FSkeletalMeshObjectCPUSkin::FSkeletalMeshObjectLOD::FDecalLOD::InitResources_GameThread(FSkeletalMeshObjectCPUSkin::FSkeletalMeshObjectLOD* LODObject)
{
	...
	VertexFactoryData->TangentBasisComponents[1] = FVertexStreamComponent(
		&LODObject->VertexBuffer,STRUCT_OFFSET(FFinalSkinVertex,TangentY),sizeof(FFinalSkinVertex),VET_PackedNormal);
	VertexFactoryData->TangentBasisComponents[2] = FVertexStreamComponent(
...
{
#if PS3// Xenon does not enjoy null vertex buffers, mmmmkay!
	RHISetStreamSource(NULL,0,FVertexBufferRHIRef(),0);
#endif
...
FORCEINLINE void RebuildTangentBasis( VertexType& DestVertex, FLOAT Det )
{
...
FORCEINLINE void UpdateMorphedVertex( TArray<FActiveMorph>& ActiveMorphs, VertexType& MorphedVertex, VertexType& SrcVertex, INT CurBaseVertIdx, INT LODIndex, TArray<INT>& MorphVertIndices )
{
	...
	FMatrix TangentBasisM( SrcVertex.TangentX, SrcVertex.TangentY, SrcVertex.TangentZ, VecZero );
	// rebuild orthonormal tangents
	RebuildTangentBasis( MorphedVertex, TangentBasisM.Determinant() );
}
...
static FORCEINLINE VectorRegister Unpack( const DWORD *PackedNormal )
{
#if XBOX
...
static FORCEINLINE void Pack( VectorRegister Normal, DWORD *PackedNormal )
{
...
FORCEINLINE void SkinVertices( FFinalSkinVertex* DestVertex, FMatrix* ReferenceToLocal, INT LODIndex, FStaticLODModel& LOD, TArray<FActiveMorph>& ActiveMorphs )
{
	...
	for(INT ChunkIndex = 0;ChunkIndex < LOD.Chunks.Num();ChunkIndex++)
	{
		...
		for(INT VertexIndex = 0;VertexIndex < Chunk.GetNumRigidVertices();VertexIndex++,SrcRigidVertex++,DestVertex++)
		{
			...
			VectorRegister SrcNormals[4] = {0};
			VectorRegister DstNormals[4];
			SrcNormals[0] = VectorLoadFloat3_W1( &MorphedVertex->Position );
			SrcNormals[1] = Unpack( &MorphedVertex->TangentX.Vector.Packed );
			SrcNormals[2] = Unpack( &MorphedVertex->TangentY.Vector.Packed );
			SrcNormals[3] = Unpack( &MorphedVertex->TangentZ.Vector.Packed );
			...
			N_xxxx = VectorReplicate( SrcNormals[3], 0 );
			N_yyyy = VectorReplicate( SrcNormals[3], 1 );
			N_zzzz = VectorReplicate( SrcNormals[3], 2 );
			DstNormals[3] = VectorMultiplyAdd( N_xxxx, M00, VectorMultiplyAdd( N_yyyy, M10, VectorMultiply( N_zzzz, M20 ) ) );

			// Write to 16-byte aligned memory:
			VectorStoreAligned( DstNormals[0], &DestVertex->Position );
			Pack( DstNormals[1], &DestVertex->TangentX.Vector.Packed );
			Pack( DstNormals[2], &DestVertex->TangentY.Vector.Packed );
			Pack( DstNormals[3], &DestVertex->TangentZ.Vector.Packed );
		...
		for(INT VertexIndex = 0;VertexIndex < Chunk.GetNumSoftVertices();VertexIndex++,SrcSoftVertex++,DestVertex++)
		{
			...
			static VectorRegister	SrcNormals[4];
			VectorRegister			DstNormals[4];
