...
public:
	...
	*/
	...
	{
		const FGPUSkinVertexFactory::ShaderDataType& ShaderData = ((FGPUSkinVertexFactory*)VertexFactory)->GetShaderData();
...
{
	// enable the morph blend codepath
...
void FGPUSkinMorphVertexFactory::AddVertexElements(DataType& InData, FVertexDeclarationElementList& OutElements)
{
	...
	OutElements.AddItem(AccessStreamComponent(InData.DeltaPositionComponent,VEU_Position,1));
	OutElements.AddItem(AccessStreamComponent(InData.DeltaTangentZComponent,VEU_Tangent,1));
}
...
IMPLEMENT_VERTEX_FACTORY_TYPE(FGPUSkinMorphVertexFactory, FGPUSkinVertexFactoryShaderParameters, "GpuSkinVertexFactory", TRUE, FALSE, 0);
...
IMPLEMENT_VERTEX_FACTORY_TYPE( FGPUSkinDecalVertexFactory, FGPUSkinDecalVertexFactoryShaderParameters, "GpuSkinDecalVertexFactory", TRUE, FALSE, VER_DECAL_REMOVED_VELOCITY_REGISTERS );
