...
namespace // our anon namespace
{
	...
	{
		// preconditions
		if( MusicTrackToFadeIn.TheSoundCue == NULL )
		{
			return;
		}
		...
		if( AudioComp != NULL )
		{
			...
			AudioComp->bCurrentIsMusic = TRUE;

			//@todo martin: implement DelayBetweenOldAndNewTrack
...
UBOOL USeqAct_CrossFadeMusicTracks::UpdateOp( FLOAT DeltaTime )
{
	...
	{
		CrossFadeMusicTracks_SoundPlayer( this, NextTrackToPlay );

		// finally set the curr track type
		CurrTrackType = NextTrackToPlay.TrackType;
...
{
	// constructors don't exist for these as they were created from .uc land 
	// @todo: turn this into an init function on the struct
// 	FMusicTrackStruct MusicTrackToFadeIn;
// 	MusicTrackToFadeIn.TrackType = NAME_None;
// 	MusicTrackToFadeIn.TheSoundCue = NULL;
// 	MusicTrackToFadeIn.Params.FadeInTime = 0;
// 	MusicTrackToFadeIn.Params.FadeInVolumeLevel = 1;
// 	MusicTrackToFadeIn.Params.DelayBetweenOldAndNewTrack = 0;
// 	MusicTrackToFadeIn.Params.FadeOutTime = 0;
// 	MusicTrackToFadeIn.Params.FadeOutVolumeLevel = 0;
			...
			for( INT Idx = 0; Idx < MTB->MusicTrackBank.Num(); ++Idx )
			{
				...
				if( MTB->MusicTrackBank(Idx).TrackType == *TrackType->StrValue )
				{
					...
					for (AController* C = GWorld->GetWorldInfo()->ControllerList; C != NULL; C = C->NextController)
					{
						...
						if (PC != NULL)
						{
							PC->eventClientCrossFadeMusicTrack(this, MTB->MusicTrackBank(Idx));
						}
					}
					break;
				}
	...
	else if( InputLinks.Num() > 1 && InputLinks(1).bHasImpulse == TRUE )
	{
		...
		if( MT != NULL )
		{
			...
			for (AController* C = GWorld->GetWorldInfo()->ControllerList; C != NULL; C = C->NextController)
			{
				...
				if (PC != NULL)
				{
					PC->eventClientCrossFadeMusicTrack(this, MT->MusicTrack);
				}
			}
		}
	}
...
void USeqAct_CrossFadeMusicTracks::CrossFadeTrack(const FMusicTrackStruct& TrackToPlay)
{
	...
	UBOOL bSameTrack = FALSE;

	if( ( CurrPlayingTrack != NULL )
		&& ( CurrPlayingTrack->SoundCue != NULL )
		...
		)
	{
		...
		if( TrackToPlay.TheSoundCue->GetFullName() == CurrPlayingTrack->SoundCue->GetFullName() )
		{
