 ...
 */
void FPathBuilder::UndefinePaths()
{
...
void FPathBuilder::ReviewPaths()
{
	...
	else
	{
		...
		Scout = GetScout();
		if ( NumStarts < Scout->MinNumPlayerStarts )
			GWarn->MapCheck_Add( MCTYPE_ERROR, GWorld->GetFirstNavigationPoint(), *FString::Printf(TEXT("Only %d PlayerStarts in this level (need at least %d)"),NumStarts,Scout->MinNumPlayerStarts), MCACTION_NONE, TEXT("NeedMorePlayerstarts"));
...
void FPathBuilder::SetPathCollision(UBOOL bEnabled)
{
	if ( bEnabled )
	{
		for( FActorIterator It; It; ++It )
...
void FPathBuilder::PrunePaths(INT NumPaths)
{
	...
	for (ANavigationPoint* Nav = GWorld->GetFirstNavigationPoint(); Nav != NULL; Nav = Nav->nextNavigationPoint)
	{
		...
		if (Nav->GetClass() == APathNode::StaticClass() && Nav->PathList.Num() > 0)
		{
			...
			{	
				...
				if (!Nav->bMergeable && *Nav->PathList(i)->End != NULL && (Nav->bFlyingPreferred || !Nav->PathList(i)->End->bFlyingPreferred))
				{
					bNecessary = FALSE;
					...
					for (INT j = 0; j < Nav->PathList(i)->End->PathList.Num(); j++)
					{
						...
						if (Spec != NULL && *Spec->End != NULL && !Spec->End->bMergeable)
						{
...
void FPathBuilder::UpdateInterpActors(UBOOL &bProblemsMoving, TArray<USeqAct_Interp*> &InterpActs)
{
...
void FPathBuilder::RestoreInterpActors(TArray<USeqAct_Interp*> &InterpActs)
{
...
void FPathBuilder::BuildNavLists()
{
 ...
 */
void FPathBuilder::DefinePaths(UBOOL bReviewPaths, UBOOL bShowMapCheck)
{
	...
	// make sure we start with a new scout
	DestroyScout();
	Scout = GetScout();
	...
	if (!bProblemsMoving && !bBuildCancelled)
	{
		...
		Scout->SetCollision( FALSE, FALSE, Scout->bIgnoreEncroachers );
		...
		BuildNavLists();
		// setup the scout
		Scout->SetCollision(TRUE, TRUE, Scout->bIgnoreEncroachers);
		...
		// Skip add long paths in warfare game
		if( appStricmp(appGetGameName(),TEXT("War")) != 0)
		{
			// do a second pass to add long range reachspecs
			debugf(NAME_DevPath, TEXT("Add long range reachspecs"));
			NumDone = 0;
		...
		if (!bBuildCancelled)
		{
			debugf(TEXT("%s add special paths"),*Scout->GetName());
			Scout->AddSpecialPaths(NumPaths, FALSE);
		}
		...
		if( !bBuildCancelled )
		{
			...
			do 
			{
				...
				for (ANavigationPoint *Nav = GWorld->GetFirstNavigationPoint(); Nav != NULL && !bBuildCancelled; Nav = Nav->nextNavigationPoint)
				{
					if (Nav->NetworkID == -1)
					{
			...
			} while(bEstablishedNetwork  && !bBuildCancelled);
 ...
 */
void FPathBuilder::AdjustCover( UBOOL bFromDefinePaths )
{
	GWarn->BeginSlowTask( *LocalizeUnrealEd(TEXT("AdjustCover")), 1);

	INT NumCover = 0;
	INT NumDone  = 0;
	TArray<ACoverLink*> Links;
	UBOOL bBuildCancelled = FALSE;
 ...
 */
void FPathBuilder::BuildCover( UBOOL bFromDefinePaths )
{
	GWarn->BeginSlowTask( *LocalizeUnrealEd(TEXT("BuildingCover")), 1);
	...
	NumDone = 0;
	Scout = GetScout();
	const FString LocalizeBuildCoverBuildingFireLinksF( LocalizeUnrealEd(TEXT("BuildCoverBuildingFireLinksF")) );
	for( ACoverLink *Link = Info->CoverList; Link != NULL && !bBuildCancelled; Link = Link->NextCoverLink, NumDone++ )
	{
		GWarn->StatusUpdatef( NumDone, NumCover, *LocalizeBuildCoverBuildingFireLinksF, NumDone, NumCover );

		Link->BuildFireLinks( Scout );
		...
		for (ACoverLink *Link = Info->CoverList; Link != NULL; Link = Link->NextCoverLink)
		{
			for (INT SlotIdx = 0; SlotIdx < Link->Slots.Num(); SlotIdx++)
			{
				for (INT Idx = 0; Idx < Link->Slots(SlotIdx).FireLinks.Num(); Idx++)
				{
					...
					if (*FireLink.TargetLink != NULL &&
						FireLink.TargetLink->GetOutermost() != Link->GetOutermost())
					{
						FireLink.TargetLink.Guid = FireLink.TargetLink->NavGuid;
...
void FPathBuilder::DestroyScout()
{
	for (FActorIterator It; It; ++It)
	{
		AScout *Scout = Cast<AScout>(*It);
		if (Scout != NULL)
		{
			if (Scout->Controller != NULL)
			{
				GWorld->DestroyActor(Scout->Controller);
			}
			GWorld->DestroyActor(Scout);
		}
	}
