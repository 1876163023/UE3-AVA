...
public:
	...
	FCoverMeshSceneProxy(const UCoverMeshComponent* InComponent):
	...
	{		
		...
		UBOOL bDrawAsSelected = Link && Link->IsSelected();
	...
	virtual void DrawDynamicElements(FPrimitiveDrawInterface* PDI, const FSceneView* View, UINT InDepthPriorityGroup)
	{
		...
		if(View->Family->ShowFlags & SHOW_Paths)
		{
			FDebugRenderSceneProxy::DrawDynamicElements(PDI, View, InDepthPriorityGroup);
		}
		...
		for(INT i=0; i<CoverMeshes.Num(); i++)
		{
			...
			for(INT ElementIndex = 0;ElementIndex < LODModel.Elements.Num();ElementIndex++)
			{
				...
				MeshElement.MaterialInstance = Element.Material->GetInstanceInterface(CoverInfo.bSelected);
...
{
	return new FCoverMeshSceneProxy(this);
}
