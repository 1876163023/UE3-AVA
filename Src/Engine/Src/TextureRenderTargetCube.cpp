 ...
 */
FString UTextureRenderTargetCube::GetDetailedDescription( INT Index )
{
	...
	if( SizeX != 0 && !(SizeX & (SizeX-1)) )
	{
		...
		if( RenderTarget )
		{
			...
			Result = CastChecked<UTextureCube>( 
				StaticConstructObject(UTextureCube::StaticClass(), Outer, FName(*NewTexName), InFlags) );
			...
			for( INT FaceIdx=0; FaceIdx < CubeFace_MAX; FaceIdx++ )
			{
				...
				UTexture2D* Tex2D = CastChecked<UTexture2D>( 
					StaticConstructObject(UTexture2D::StaticClass(), Outer, FName(*Tex2DName), InFlags) );
