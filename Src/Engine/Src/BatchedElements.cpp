...
public:
		...
		FShader(Initializer)
	{
		...
		SceneColorBiasFactorParam.Bind(Initializer.ParameterMap,TEXT("SceneColorBiasFactor"),TRUE);
	}
	...
	void SetParameters(FCommandContextRHI* Context,const FTexture* Texture,FLOAT Gamma,EBlendMode BlendMode)
	{
		...
		if (GammaParameter.IsBound())
			SetPixelShaderValue(Context,GetPixelShader(),GammaParameter,Gamma);
		SetPixelShaderValue(Context,GetPixelShader(),SceneColorBiasFactorParam,(BlendMode == BLEND_Modulate) ? 1.0f : appPow(2.0f,GCurrentColorExpBias));
	}
	...
	virtual void Serialize(FArchive& Ar)
	{
		...
		Ar << TextureParameter;
		Ar << GammaParameter;
		Ar << SceneColorBiasFactorParam;
	}

private:
	FShaderParameter TextureParameter;
	FShaderParameter GammaParameter;
	FShaderParameter SceneColorBiasFactorParam;
};
...
public:
	...
	static UBOOL ShouldCache(EShaderPlatform Platform) { return IsPCPlatform( Platform ); }
...
{
	if(LineVertices.Num() || Points.Num() || Sprites.Num() || MeshElements.Num() || QuadMeshElements.Num())
	{
		...
		if(LineVertices.Num() || Points.Num())
		{
		...
		for(INT SpriteIndex = 0;SpriteIndex < Sprites.Num();SpriteIndex++)
		{
			...
			SpriteVertices[0] = FSimpleElementVertex(FVector4(Sprite.Position - WorldSpriteX - WorldSpriteY,1),FVector2D(0,0),Sprite.Color,Sprite.HitProxyId);
			SpriteVertices[1] = FSimpleElementVertex(FVector4(Sprite.Position - WorldSpriteX + WorldSpriteY,1),FVector2D(0,1),Sprite.Color,Sprite.HitProxyId);
			SpriteVertices[2] = FSimpleElementVertex(FVector4(Sprite.Position + WorldSpriteX + WorldSpriteY,1),FVector2D(1,1),Sprite.Color,Sprite.HitProxyId);
			SpriteVertices[3] = FSimpleElementVertex(FVector4(Sprite.Position + WorldSpriteX - WorldSpriteY,1),FVector2D(1,0),Sprite.Color,Sprite.HitProxyId);
			// Draw the sprite.
			RHIEndDrawPrimitiveUP(Context);
		}
