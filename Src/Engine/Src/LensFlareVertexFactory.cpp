...
void FLensFlareVertexFactory::FillElementList(FVertexDeclarationElementList& Elements, INT& Offset)
{
	...
	Elements.AddItem(FVertexElement(0,Offset,VET_Float4,VEU_Normal,0));
	Offset += sizeof(FLOAT) * 4;
...
{
	FLensFlareVertexFactory* LensFlareVF = (FLensFlareVertexFactory*)VertexFactory;
...
IMPLEMENT_VERTEX_FACTORY_TYPE(FLensFlareVertexFactory,FLensFlareVertexFactoryShaderParameters,"LensFlareVertexFactory",TRUE,FALSE, VER_SEPARATED_STATIC_MESH_POSITIONS);
