	...
	for (INT i = 0; i < Objects.Num(); i++)
	{
		...
		else
		{
				...
				Spec->supports(Radius, Height, MoveFlags, iMaxFallSpeed))
			{
				...
				if (StartDistSquared < EndDistSquared && Spec->Start->IsUsableAnchorFor(this) && Spec->End->GetReachSpecTo(Spec->Start) != NULL) // only consider Start if there's a reverse ReachSpec
				{
...
FLOAT APawn::findPathToward(AActor *goal, FVector GoalLocation, NodeEvaluator NodeEval, FLOAT BestWeight, UBOOL bWeightDetours, FLOAT MaxPathLength, UBOOL bReturnPartial )
{
	...
	if ( !Anchor || (!EndAnchor && bSpecifiedEnd) )
	{
		if (Anchor == NULL)
		{
			...
			if (Anchor == NULL)
			{
				FindAnchorFailedTime = GWorld->GetTimeSeconds();
				return 0.f;
			}
		...
		if ( EndAnchor == Anchor )
		{
			...
			if ( PassedAnchor )
			{
				if ( Cast<ANavigationPoint>(goal) )
				{
					Controller->RouteCache_AddItem( Cast<ANavigationPoint>(goal) );
				}
			}
			else
			{
		...
		case PST_AStar:
			{
				...
				// Do A* search
				if( FAStarPath::AStarPathTo( this, Anchor, EndAnchor, MaxPathLength, bReturnPartial, BestPath ) )
				{
					// If we allow a partial path OR 
					// the path reaches our destination
					if( bReturnPartial || 
						BestPath.IsPathTo( EndAnchor ) )
					{
						// Set route cache for the controller
						BestPath.SetRouteCache( Controller, StartDist, EndDist );
						// Return length of path as the result
						Result = BestPath.GetPathCost();
					}
				}
			}
			break;
...
//debug
//#define _DEBUG_ASTAR_ 1
//#define _DEBUG_ASTAR_FLAG_ 1

// Consturctor 
// - used to append node to a previous path and calc new costs
UBOOL FAStarPath::SetPath( FAStarPath* PrevPath, UObject* AddObject, ANavigationPoint* Goal, APawn* Pawn, FLOAT MaxPathLength )
{
	// Get info that we need to add to the path 
	//	- add object is either a UReachSpec* or ANavigationPoint* or NULL
	UReachSpec* AddSpec = Cast<UReachSpec>(AddObject);
	ANavigationPoint* AddNode = AddSpec ? AddSpec->End : Cast<ANavigationPoint>(AddObject);

	// If there was a previous path
	if( PrevPath )
	{
		// Keep previous path list
		NavList = PrevPath->NavList;

		// Recompute partial cost as previous partial cost + distance to new node
		PartialPathCost = PrevPath->PartialPathCost;
	}
	else
	{
		// Otherwise, init partial cost to zero
		PartialPathCost = 0.f;
	}

	// If given a reach spec - add cost
	if( AddSpec )
	{ 
		PartialPathCost += AddSpec->CostFor( Pawn );

//		debugf(TEXT("%s SetPath... CostFor %s: %s to %s Cost %f"), *Pawn->GetName(), *AddSpec->GetName(), *AddSpec->Start->GetName(), *AddSpec->End->GetName(), PartialPathCost );
	}
	// Reset estimate cost
	EstimateCost = 0.f;

	// If we have a node to add AND
	// path hasn't exceeded max cost
	INT FullCost = appTrunc(GetPathCost());
	if( AddNode )
	{
		// Estimate cost is distance from new node to goal
		if( AddNode != Goal )
		{
			EstimateCost = (AddNode->Location - Goal->Location).Size();
		}

		// Add new node to path list
		NavList.AddItem( AddNode );
	}

	return (FullCost < MaxPathLength);
}
 ...
 */
UBOOL FAStarPath::ShorterPathExists( TArray<FAStarPath>& PathList )
{
	for( INT Idx = 0; Idx < PathList.Num(); Idx++ )
	{
		// If end of existing path matches end of this path
		if( PathList(Idx).NavList.Top() == NavList.Top() )
		{		
			// If the current path in list is shorter than this path
			if( PathList(Idx).PartialPathCost < PartialPathCost )
			{
				// Exit immediately
				return TRUE;
			}
			...
			{
				// Otherwise, this path is shorter
				// Remove the longer existing path from list
				PathList.Remove( Idx );

				// and exit (only one path to any node - the shortest)
				return FALSE;
			}
		}
	}
 ...
 */
void FAStarPath::SetRouteCache( AController* C, FLOAT StartDist, FLOAT EndDist )
{
	if( NavList.Num() == 0 || !C )
	{
		return;
	...
	if( C->Pawn )
	{
		// While there are items in the path AND 
		// the pawn has reached the first node in the list
		while( NavList.Num() &&
			   C->Pawn->ReachedDestination( NavList(0) ) )
		{
	...
	if( C->Pawn && StartDist > 0.f )
	{
		if( NavList.Num() > 1 )
		{
			ANavigationPoint* First = NavList(0);
			ANavigationPoint* Next	= NavList(1);

			FLOAT DistPawnToNext  = (C->Pawn->Location  - Next->Location).Size();
			FLOAT DistFirstToNext = (First->Location    - Next->Location).Size();
			FLOAT MaxDist = 0.75f * MAXPATHDIST;

			if( Next->IsA(AVolumePathNode::StaticClass()) )
			{
				MaxDist = ::Max( MaxDist, Next->CylinderComponent->CollisionRadius );
			}

			// If...
			if( (DistPawnToNext < MaxDist) &&			// Pawn is within max dist of next node AND
				(DistPawnToNext < DistFirstToNext) &&	// Distance from pawn to next node is less than first to next AND
				((GWorld->GetNetMode() != NM_Standalone) || // (In a network game OR
					(GWorld->GetTimeSeconds() - C->Pawn->LastRenderTime < 5.f) ||	// Pawn was in view recently OR
					(StartDist > 250.f)) )											// Pawn is "far" from start point
			{
				FCheckResult Hit(1.f);

				// Trace to next path node
				GWorld->SingleLineCheck( Hit, C, Next->Location, C->Pawn->Location, TRACE_World | TRACE_StopAtAnyHit );

				// If path is clear and we can reach it
				if( !Hit.Actor && C->Pawn->actorReachable( Next, 1, 1 ) )
				{
	...
	// Set controller route cache to be our path list
	for( INT Idx = 0; Idx < NavList.Num(); Idx++ )
	{
		C->RouteCache_AddItem( NavList(Idx) );
	}
	C->RouteGoal  = NavList.Num() ? NavList.Last() : NULL;

	// If there is more than one path in the list
	if( C->Pawn && C->RouteCache.Num() > 1 )
	{
		// Get reach spec between first and second nodes
		ANavigationPoint *FirstPath = C->RouteCache(0);
		UReachSpec* NextSpec = NULL;
		if( FirstPath )
		{
			ANavigationPoint *SecondPath = C->RouteCache(1);
			if( SecondPath )
			{
		...
		// Set the next path radius for pawn
		if( NextSpec )
		{
			C->Pawn->NextPathRadius = NextSpec->CollisionRadius;
		}
		else
		{
			C->Pawn->NextPathRadius = 0.f;
		}
	}
}
 ...
 */
void FAStarPath::PruneRoute( APawn* Pawn )
{
/// BEGIN - REMOVE NODES BYPASSED BY SLOT SHORT CUT PATHS
#ifdef _DEBUG_ASTAR_
#ifdef _DEBUG_ASTAR_FLAG_
	if( Pawn->bDebug )
	{
#endif
	INT LastCover = 0;

	debugf(TEXT("START PATH - NUM %d Pawn %s"), NavList.Num(), Pawn ? *Pawn->GetName() : TEXT("NULL" ) );
	for( INT T = 0; T < NavList.Num(); T++ )
	{
		debugf( TEXT("Node %d %s"), T, NavList(T)->GetName() );
	}
	debugf(TEXT("END PATH - NUM %d Pawn %s"), NavList.Num(), Pawn ? *Pawn->GetName() : TEXT("NULL" ) );
	for( INT T = 0; T < NavList.Num(); T++ )
	{
		debugf( TEXT("Node %d %s"), T, NavList(T)->GetName() );
	}
...
#endif
#endif // _DEBUG_ASTAR_
/// END - SHORT CUT BYPASS
}


/** Print out a path list */
void FAStarPath::DumpPath()
{
	debugf( TEXT("Print path %x = "), this );
	debugf( TEXT("{") );
	for( INT Idx = 0; Idx < NavList.Num(); Idx++ )
	{
		debugf(TEXT("\t\t\t\t %d %s"), Idx, *NavList(Idx)->GetName() );
	}
	debugf(TEXT("}") );
	debugf( TEXT("End path %x -> Cost %f Partial %f Estimate %f"), this, GetPathCost(), PartialPathCost, EstimateCost );
}

/** Checks for a loop in the path */
UBOOL FAStarPath::HasLoop()
{
	debugf(TEXT("Check for loop in path %x = "), this );

	for( INT Idx = 0; Idx < NavList.Num(); Idx++ )
	{
		for( INT Test = Idx + 1; Test < NavList.Num(); Test++ )
		{
			if( NavList(Idx) == NavList(Test) )
			{
				debugf(TEXT("LOOP FOUND %s"), NavList(Idx) ? *NavList(Idx)->GetName() : TEXT("NULL") );
				return TRUE;
			}
		}
	}

	debugf(TEXT("NO LOOP IN PATH"));
	return FALSE;
}

UBOOL FAStarPath::AStarPathTo( APawn* Pawn, ANavigationPoint *Start, ANavigationPoint* Goal, FLOAT MaxPathLength, UBOOL bReturnPartial, FAStarPath& Result )
{
#ifdef _DEBUG_ASTAR_
#ifdef _DEBUG_ASTAR_FLAG_
	if( Pawn->bDebug )
#endif
	debugf( TEXT("%s ATTEMPT TO FIND PATH TO %s from %s"), *Pawn->GetName(), *Goal->GetName(), *Start->GetName() );
#endif // _DEBUG_ASTAR_

	// Init relevant variables
	UBOOL	bGoalFound = FALSE;
	INT		iRadius	= Pawn->bCanCrouch ? appTrunc(Pawn->CrouchRadius) : appTrunc(Pawn->CylinderComponent->CollisionRadius);
	INT		iHeight = Pawn->bCanCrouch ? appTrunc(Pawn->CrouchHeight) : appTrunc(Pawn->CylinderComponent->CollisionHeight);
	INT		iMaxFallSpeed = appTrunc(Pawn->GetAIMaxFallSpeed());
	INT		moveFlags = Pawn->calcMoveFlags();
	
	// Create list of current paths being evaluated
	TArray<FAStarPath>	PathList;
	// Keep closest partial path
	FAStarPath			PartialPath;

	// Add start point to path list as first path
	FAStarPath FirstPath;
	FirstPath.SetPath( NULL, Start, Goal, Pawn, MaxPathLength );
	PathList.Push( FirstPath );
	...
	// While we still have paths to evaluate and 
	// the "best" path in sorted list hasn't reached our goal
	DOUBLE StartTime = appSeconds();
	UBOOL bAbortSearch = FALSE;
	while( !bAbortSearch && PathList.Num() && !PathList.Top().IsPathTo( Goal ) )
	{
		// Grab first path from the path list and the last node in that path
		FAStarPath		  Path	   = PathList.Pop();
		ANavigationPoint* LastNode = Path.NavList.Top();
	...
	{
#endif
		debugf( TEXT("Dump Top Path %d"), PathCnt++ );
		Path.DumpPath();
#ifdef _DEBUG_ASTAR_FLAG_
	}
#endif
		...
		for( INT PathIdx = 0; PathIdx < LastNode->PathList.Num(); PathIdx++ )
		{
			UReachSpec *Spec = LastNode->PathList(PathIdx);
			if( !Spec || 
				!Spec->End ||
				!Spec->supports( iRadius, iHeight, moveFlags, iMaxFallSpeed ) )
			{
				continue;
			}

			// Grab path node that we want to try to add
			ANavigationPoint* AddNode = LastNode->PathList(PathIdx)->End;

			// If this node is already in the path - skip it
			if( Path.NavList.FindItemIndex( AddNode ) >= 0 )
			{
				continue;
			}
			...
			// Create new path
			FAStarPath NewPath;
			if( NewPath.SetPath( &Path, Spec, Goal, Pawn, MaxPathLength ) )
			{
				// Reject all paths that end at the same node, keep only the shortest one
				if( NewPath.ShorterPathExists( PathList ) )
				{
					continue;
				}

				// If node we are adding is our destination
				if( AddNode == Goal )
				{
					// Set flag
					bGoalFound = TRUE;
				...
				{
#endif
					debugf( TEXT("Add Path to %s"), *AddNode->GetName() );
					NewPath.DumpPath();
#ifdef _DEBUG_ASTAR_FLAG_
				}
#endif
			...
			{
				// Keep partial path that is closest to the goal
				if( PartialPath.GetPathCost() == 0.f ||
					NewPath.EstimateCost < PartialPath.EstimateCost )
				{
					PartialPath = NewPath;
				}
				...
				{
#endif
					debugf( TEXT("Add Partial to %s"), *AddNode->GetName() );
					NewPath.DumpPath();
#ifdef _DEBUG_ASTAR_FLAG_
				}
#endif
...
/**
// Disable for now
		if ((appSeconds() - StartTime) > 0.02)
		{
			debugf(TEXT("Aborting path find for %s, %s -> %s"),*Pawn->GetName(),*Start->GetName(),*Goal->GetName());
			bAbortSearch = TRUE;
		}
*/
		// Sort list by path cost
		Sort<FAStarPath,FAStarPathCompare>( &PathList(0), PathList.Num() );
	}

	// If goal was reached
	if( bGoalFound )
	{
		while( PathList.Top().NavList.Top() != Goal )
		{
			PathList.Pop();
		}
		Result.SetPath( &PathList.Top(), NULL, Goal, Pawn, MaxPathLength );
		...
		{
#endif
			debugf( TEXT("%s FINAL PATH TO %s"), *Pawn->GetName(), *Goal->GetName() );
			Result.DumpPath();
#ifdef _DEBUG_ASTAR_FLAG_
		}
#endif
	...
	// If we failed to find goal but will accept a partial path AND
	// our partial path is valid
	else if( bReturnPartial && 
			 PartialPath.NavList.Num() )
	{
		// Set result to partial path
		Result.SetPath( &PartialPath, NULL, Goal, Pawn, MaxPathLength );
		...
		{
#endif
			debugf( TEXT("FINAL PATH TO %s"), *Goal->GetName() );
			Result.DumpPath();
#ifdef _DEBUG_ASTAR_FLAG_
		}
#endif
		...
		if( Pawn->bDebug )
#endif
			debugf( TEXT("%s FAILED TO FIND PATH TO %s"), *Pawn->GetName(), *Goal->GetName() );
#endif // _DEBUG_ASTAR_
...
void ADynamicAnchor::Initialize(AController* InUser, ANavigationPoint* Point1, ANavigationPoint* Point2, UReachSpec* SpecTemplate)
{
	...
	// Add to nav list and cross level actors & mark as having cross level paths.
	ULevel* Level = GetLevel();
	Level->AddToNavList( this, TRUE );
