...
{
	if(nScene.checkResults(NX_ALL_FINISHED, false))
	{
...
void FNxContactReport::onContactNotify(NxContactPair& pair, NxU32 events)
{
	...
	// First, look for 'sensor' body, used for pushing physics objects.
	NxActor* nSensor = NULL;
	NxActor* nPushed = NULL;
	if(nActor0->readActorFlag(NX_AF_DISABLE_RESPONSE))
	{
		nSensor = nActor0;
		nPushed = nActor1;
	}
	else if(nActor1->readActorFlag(NX_AF_DISABLE_RESPONSE))
	{
		nSensor = nActor1;
		nPushed = nActor0;
	}
	...
	// Sensor case - not a physical contact
	if(nSensor)
	{
		// Only care about collisions with dynamic objects (not static or kinematic).
		if(!nPushed->isDynamic() || nActor0->readBodyFlag(NX_BF_KINEMATIC))
		{
			return;
		}
		else
		{
			...
			check(nSensor->userData); // userData of sensor object should be Pawn.
			PushInfo->Pusher = (APawn*)nSensor->userData;
	...
	else
	{
		...
		NotifyInfo->bCallEvent0 = (nActor0->getGroup() == UNX_GROUP_NOTIFYCOLLIDE);
		CreateCollisionInfo(nActor0, NotifyInfo->Info0);

		NotifyInfo->bCallEvent1 = (nActor1->getGroup() == UNX_GROUP_NOTIFYCOLLIDE);
	...
	while(It.goNextPair())
	{
		...
		while(It.goNextPatch())
		{
			...
			while(It.goNextPoint())
			{
				...
				if(!bIsWheelContact)
				{
					new(*ContactInfoArray)FRigidBodyContactInfo(N2UPosition(nContactPos), N2UVectorCopy(nContactNormal),
						N2UPosition(nActor0->getPointVelocity(nContactPos)), N2UPosition(nActor1->getPointVelocity(nContactPos)),
																PhysMat0, PhysMat1 );
				}
			}
		}	
...
void DestroyDummyKinActor(NxActor* KinActor)
{
	...
	{
		GNovodexPendingKillActor.AddItem(KinActor);
	}
	else
	{
...
void AddForceNoWake(NxActor *Actor, const NxVec3 &force) 
{
	if(!Actor->isSleeping())
	{
		...
		Actor->saveBodyToDesc(BodyDesc);
		addForce(Actor, force);
		Actor->wakeUp(BodyDesc.wakeUpCounter);
	}
}
