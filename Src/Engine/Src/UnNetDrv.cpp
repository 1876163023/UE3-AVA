...
void UPackageMapLevel::StaticConstructor()
{
	...
	Class->EmitObjectReference( STRUCT_OFFSET( UPackageMapLevel, Connection ) );
}
		...
		else
		{
			...
			if (Object != NULL && GWorld != NULL)
			{
				...
				if (Level != NULL && Level != GWorld->PersistentLevel)
				{
					...
					if (!bLevelVisible)
					{
						debugf(NAME_DevNetTraffic, TEXT("Using None instead of replicated reference to %s because the level it's in has not been made visible"), *Object->GetFullName());
						Object = NULL;
					}
				}
			}
...
//@fixme: temp stats for seek free package map
DECLARE_DWORD_COUNTER_STAT(TEXT("Static obj refs sent"), 460, STATGROUP_Net)
DECLARE_DWORD_COUNTER_STAT(TEXT("Static obj refs sent bytes"), 461, STATGROUP_Net)
DECLARE_DWORD_COUNTER_STAT(TEXT("Static obj refs rec"), 462, STATGROUP_Net)
DECLARE_DWORD_COUNTER_STAT(TEXT("Static obj refs rec bytes"), 463, STATGROUP_Net)
DECLARE_DWORD_COUNTER_STAT(TEXT("Name refs sent"), 464, STATGROUP_Net)
DECLARE_DWORD_COUNTER_STAT(TEXT("Name refs sent bytes"), 465, STATGROUP_Net)
DECLARE_DWORD_COUNTER_STAT(TEXT("Name refs rec"), 466, STATGROUP_Net)
DECLARE_DWORD_COUNTER_STAT(TEXT("Name refs rec bytes"), 467, STATGROUP_Net)
...
UBOOL UPackageMapSeekFree::SerializeObject( FArchive& Ar, UClass* Class, UObject*& Object )
{
	if( Ar.IsLoading() )
	{
		...
		if( ChannelIndex == INDEX_NONE )
		{
			...
			if( !Ar.IsError() )
			{
				//@fixme: temp stats for seek free package map
				INC_DWORD_STAT(462);
				INC_DWORD_STAT_BY(463, (ObjectPathName.Len() + 1) * sizeof(TCHAR));
	...
	else if( Ar.IsSaving() )
	{
		...
		else
		{
			...
			//@fixme: temp stats for seek free package map
			INC_DWORD_STAT(460);
			INC_DWORD_STAT_BY(461, (ObjectPathName.Len() + 1) * sizeof(TCHAR));
		}
	}
	return TRUE;
}
UBOOL UPackageMapSeekFree::SerializeName( FArchive& Ar, FName& Name )
{
	if( Ar.IsLoading() )
	{
		...
		//@fixme: temp stats for seek free package map
		INC_DWORD_STAT(464);
		INC_DWORD_STAT_BY(465, (InString.Len() + 1) * sizeof(TCHAR));
	}
	else if( Ar.IsSaving() )
	{
		...
		//@fixme: temp stats for seek free package map
		INC_DWORD_STAT(466);
		INC_DWORD_STAT_BY(467, (OutString.Len() + 1) * sizeof(TCHAR));
	}
	return TRUE;
}
...
int GsulRandomSeed = SUL_DefaultRandomSeed;
#endif
// }} 20080213 dEAthcURe|SUL
...
,   NetUpdateTime(0.0f) // 20071205 dEAthcURe|ON
#endif
// {{ 20080213 dEAthcURe|SUL
#ifdef EnableSecureUdpLayer
,   pSecureUdpLayer(0x0)
...
// }} 20080213 dEAthcURe|SUL
{
	if ( !HasAnyFlags(RF_ClassDefaultObject) )
	{
		...
		// {{ 20080213 dEAthcURe|SUL
		#ifdef EnableSecureUdpLayer
		pSecureUdpLayer    = new secureUdpLayer_t();
		#endif
		// }} 20080213 dEAthcURe|SUL
	}
}
void UNetDriver::StaticConstructor()
{
	...
	UClass* Class = GetClass();
	Class->EmitObjectArrayReference( STRUCT_OFFSET( UNetDriver, ClientConnections ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UNetDriver, ServerConnection ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UNetDriver, MasterMap ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UNetDriver, RoleProperty ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UNetDriver, RemoteRoleProperty ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UNetDriver, NetConnectionClass ) );
}
...
void UNetDriver::TickFlush()
{
	...
	#ifdef EnableNetDriverTickFrequency
	if(GWorld && GWorld->GetNetMode() != NM_Client) {
		AWorldInfo* pWorldInfo = GWorld->GetWorldInfo();
		if(pWorldInfo) {
			if(pWorldInfo->RealTimeSeconds < NetUpdateTime + 1.0f / GNetDriverTickFrequency) {
				return;
			}
			NetUpdateTime = pWorldInfo->RealTimeSeconds;
		}		
	}		
	#endif
	// }} 20071205 dEAthcURe|ON net driver tick frequency
...
UBOOL UNetDriver::InitConnect( FNetworkNotify* InNotify, FURL& URL, FString& Error )
{
	...
	#ifdef EnableSecureUdpLayer
	if(pSecureUdpLayer) {
		pSecureUdpLayer->init(GsulRandomSeed);
	}
	#endif
	// }} 20080213 dEAthcURe|SUL
...
UBOOL UNetDriver::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else if (ParseCommand(&Cmd, TEXT("PACKAGEMAP")))
	{
		...
		if (ServerConnection != NULL)
		{
			...
			{
				Ar.Logf( TEXT("      Package %i: Name - %s, LocalGeneration - %i, RemoteGeneration - %i, BaseIndex - %i, ObjectCount - %i"), i,
							*ServerConnection->PackageMap->List(i).PackageName.ToString(), ServerConnection->PackageMap->List(i).LocalGeneration, ServerConnection->PackageMap->List(i).RemoteGeneration,
							ServerConnection->PackageMap->List(i).ObjectBase, ServerConnection->PackageMap->List(i).ObjectCount );
			}
		}
		for (INT i = 0; i < ClientConnections.Num(); i++)
		{
			...
			{
				Ar.Logf( TEXT("      Package %i: Name - %s, LocalGeneration - %i, RemoteGeneration - %i, BaseIndex - %i, ObjectCount - %i"), i,
							*Connection->PackageMap->List(i).PackageName.ToString(), Connection->PackageMap->List(i).LocalGeneration, Connection->PackageMap->List(i).RemoteGeneration,
							Connection->PackageMap->List(i).ObjectBase, Connection->PackageMap->List(i).ObjectCount );
			}
		}
		return TRUE;
	}
...
#endif
	// {{ 20070205 dEAthcURe
	else if (ParseCommand(&Cmd, TEXT("GETNETSPEED"))) {
		// Print packagemap for open connections
		Ar.Logf(TEXT("GetNetSpeed:"));
		if (ServerConnection) {
			Ar.Logf(TEXT("   Server %s, net speed:%d"), *ServerConnection->LowLevelDescribe(), ServerConnection->CurrentNetSpeed);
		}
		for (INT i = 0; i < ClientConnections.Num(); i++) {
			UNetConnection* Connection = ClientConnections(i);
			Ar.Logf( TEXT("   Client %s, net speed:%d"), *Connection->LowLevelDescribe(), Connection->CurrentNetSpeed );
		}
		return TRUE;
	}
	else if (ParseCommand(&Cmd, TEXT("SETNETSPEED"))) {
		INT newNetSpeed = appAtoi(Cmd);
		if(newNetSpeed ==0) {
			Ar.Logf(TEXT("Àå³­ÇÏ3? SetNewSpeed #{nBytes/sec}"));
			return FALSE;
		}
		Ar.Logf(TEXT("SetNetSpeed: %d"), newNetSpeed);
		if (ServerConnection) {
			Ar.Logf(TEXT("   Server %s, net speed:%d->%d"), *ServerConnection->LowLevelDescribe(), ServerConnection->CurrentNetSpeed, newNetSpeed);
			ServerConnection->CurrentNetSpeed = newNetSpeed;
		}
		for (INT i = 0; i < ClientConnections.Num(); i++) {
			UNetConnection* Connection = ClientConnections(i);
			Ar.Logf( TEXT("   Client %s, net speed:%d->%d"), *Connection->LowLevelDescribe(), Connection->CurrentNetSpeed, newNetSpeed);
			Connection->CurrentNetSpeed = newNetSpeed;
		}
		return TRUE;
	}
	// }} 20070205 dEAthcURe
#ifdef _BT_TEST_BY_CRAZY
	else if (UNetConnection::ParseSettings(Cmd, Ar))
	{
		return TRUE;
	}
#endif
