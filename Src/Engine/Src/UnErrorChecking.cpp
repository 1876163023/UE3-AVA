...
void AWorldInfo::CheckForErrors()
{
	...
	{
		GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf(TEXT("World have no pvs information") ) );
	}

	// check full build
	//if( GWorld->PersistentLevel && GWorld->PersistentLevel->Model->)
	//>@ ava
...
void AActor::CheckForErrors()
{
	...
	if( Physics == PHYS_RigidBody && !CollisionComponent)
	{
