 ...
 */
UBOOL GHackCompressTexture = FALSE;

void FTexture2DMipMap::Serialize( FArchive& Ar, UObject* Owner )
{
	if (Ar.IsSaving() && Ar.IsPersistent() && GHackCompressTexture)
	{
		// 압축이 되어 있지 않다?
		if ((Data.GetSavedBulkDataFlags() & BULKDATA_SerializeCompressed) == 0)
		{
			// Memory로 load해 놓는다. memory에 없으면 flag가 바뀌면서 나중에 compressed된 건 줄 알고 load하다가 뻑난다.
			Data.Lock(LOCK_READ_WRITE);
			Data.Unlock();
		}		

		Data.StoreCompressedOnDisk( COMPRESS_LZO );
	}
 ...
 */
FString UTexture2D::GetDetailedDescription( INT Index )
{
...
{
	INT RequestStatus = PendingMipChangeRequestStatus.GetValue();
//	check( RequestStatus != TEXTURE_PENDING_INITIALIZATION );
 ...
 */
INT UTexture2D::GetSize( INT MipCount )
{
	INT Size		= 0;
...
FTextureResource* UTexture2D::CreateResource()
{
	...
	&&	LODGroup != TEXTUREGROUP_UI )
	{
		...
		if( !GIsEditor && GPackageFileCache->FindPackageFile( *GetOutermost()->GetName(), NULL, Filename ) )
		{
		...
		else
		{
			...
			if( !GIsEditor && GPackageFileCache->FindPackageFile( *GetOutermost()->GetName(), &PackageGuid, Filename ) )
			{
				bIsStreamable = TRUE;
			}
			else
	...
	// LightMap만 Streaming안되게 해준다.(2007/10/02 고광록)
	// (UnLightMap.cpp Ln222, UnShadwoMap.cpp Ln272 적용했지만 새로 굽기 전까지만 처리해준다)
	if ( LODGroup == TEXTUREGROUP_LightAndShadowMap )
		bIsStreamable = FALSE;
...
,	IORequestCount( 0 )
{
	...
	//<@ ava specific ; 2007. 5. 25 changmin
	// lightmap은 필요한 mip level만 loading합니다.
	if( Owner->LODGroup == TEXTUREGROUP_LightAndShadowMap )
	{
...
void FTexture2DResource::InitRHI()
{
	...
	// cmheo : lightmap
	// 필요한 mip level만 가진 texture 를 만들 겁니다...
	if( Owner->LODGroup == TEXTUREGROUP_LightAndShadowMap )
	{
		Texture2DRHI	= RHICreateTexture2D( SizeX, SizeY, Owner->Format, 1, TexCreateFlags );
	}
	else
	{
	...
	//<@ ava specific ; 2007. 5. 25 changmin
	// 필요한 mip만 texture로 만듭니다...
	if( Owner->LODGroup == TEXTUREGROUP_LightAndShadowMap )
	{
		// Read the resident mip-levels into the RHI texture.
		const INT MipIndex = FirstMip;
		if( MipData[MipIndex] != NULL )
		{
			UINT DestPitch;
			void* MipData = RHILockTexture2D( Texture2DRHI, 0, TRUE, DestPitch );
			GetData( MipIndex, MipData, DestPitch );
	...
	else
	{
		...
		for( INT MipIndex=FirstMip; MipIndex<Owner->Mips.Num(); MipIndex++ )
		{
			...
			{
				UINT DestPitch;
				void* MipData = RHILockTexture2D( Texture2DRHI, MipIndex - FirstMip, TRUE, DestPitch );
				GetData( MipIndex, MipData, DestPitch );
...
void FTexture2DResource::GetData( UINT MipIndex,void* Dest,UINT DestPitch )
{
	...
	else
	{
		...
		check( (INT(Src) - INT(MipData[MipIndex])) == MipMap.Data.GetBulkDataSize() );
	}
#else
...
void FTexture2DResource::UpdateMipCount()
{
	...
	for( INT MipIndex=0; MipIndex<FirstSharedMip; MipIndex++ )
	{
		...
		// Lock new texture.
		UINT DestStride;
		void* MipData = RHILockTexture2D( IntermediateTextureRHI, MipIndex, TRUE, DestStride );
