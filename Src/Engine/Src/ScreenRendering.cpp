...
FBoundShaderStateRHIRef ScreenBoundShaderState;

/**
 * Draws a texture rectangle on the screen using normalized (-1 to 1) device coordinates.
 */
void DrawScreenQuad( FCommandContextRHI* Context, FLOAT X0, FLOAT Y0, FLOAT U0, FLOAT V0, FLOAT X1, FLOAT Y1, FLOAT U1, FLOAT V1, const FTexture* Texture )
{ 
    ...
    ScreenPixelShader->SetParameters(Context,Texture);

    if (!IsValidRef(ScreenBoundShaderState))
    {
        DWORD Strides[MaxVertexElementCount];
        appMemzero(Strides, sizeof(Strides));
        Strides[0] = sizeof(FScreenVertex);

        ScreenBoundShaderState = RHICreateBoundShaderState(GScreenVertexDeclaration.VertexDeclarationRHI,Strides,ScreenVertexShader->GetVertexShader(),ScreenPixelShader->GetPixelShader());
    }

    RHISetBoundShaderState(Context, ScreenBoundShaderState);
    ...
    RHIDrawPrimitiveUP(Context,PT_TriangleStrip,2,Vertices,sizeof(Vertices[0]));
}
