 ...
 */
static void RecursiveFindWaves(USoundNode* Node, TArray<USoundNodeWave*> &OutWaves)
{
	if ( Node )
	{
 ...
 */
FString USoundCue::GetDetailedDescription( INT Index )
{
	FString Description = TEXT( "" );
	switch( Index )
	{
...
void USoundCue::Serialize(FArchive& Ar)
{
	Super::Serialize( Ar );
	Ar << EditorData;
	// Empty when loading for cooking or for game.
	if ( (!GIsEditor || GIsCooking) && Ar.IsLoading() )
	{
		EditorData.Empty();
	}
}
...
void USoundNode::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	for( INT i=0; i<ChildNodes.Num() && i<GetMaxChildNodes(); i++ )
	{
		if( ChildNodes(i) )
		{
			ChildNodes(i)->ParseNodes( this, i, AudioComponent, WaveInstances );
		}
	}
}
...
void USoundNodeMixer::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
...
void USoundNodeDistanceCrossFade::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
 ...
 */
FString USoundNodeWave::GetDesc()
{
	FString Channels;
 ...
 */
FString USoundNodeWave::GetDetailedDescription( INT Index )
{
	FString Description = TEXT( "" );
	switch( Index )
	{
	...
	case 1:
		...
		break;
	case 2:
		Description = FString::Printf( TEXT( "%d%%" ), CompressionQuality );
		break;
	case 3:
...
void USoundNodeWave::Serialize( FArchive& Ar )
{
	...
	if( Ar.Ver() >= VER_UPDATED_SOUND_NODE_WAVE )
	{
		Ar << ChannelOffsets;
		Ar << ChannelSizes;
	}
	...
	if( Ar.Ver() >= VER_ADDED_NUM_CHANNELS )
	{
		Ar << NumChannels;
	}
...
void USoundNodeWave::PostLoad()
{
	...
	// most likely cause us to run out of memory given that the Editor fully loads all open packages
	// unless -fastload is specified.
 ...
 */
UBOOL USoundNodeWave::IsLocalizedResource()
{
	UBOOL bIsLocalized = Super::IsLocalizedResource() || Subtitles.Num() > 0;
	if ( !bIsLocalized )
	{
		// @hack: Gears: Be sure to consider Locust sounds as requiring localization, even though they have no subtitles.
		// @hack: Gears: Need a better solution for this after ship.
		const FString PackageName = GetOutermost()->GetName();
		if ( PackageName.StartsWith(TEXT("Locust_Boomer_Chatter"))
			||PackageName.StartsWith(TEXT("Locust_Drone_Chatter"))
			||PackageName.StartsWith(TEXT("Locust_Raam_Chatter"))
			||PackageName.StartsWith(TEXT("Locust_Theron_Chatter"))
			)
		{
			bIsLocalized = TRUE;
			//warnf( NAME_Warning, TEXT("!!!!! %s is localized by package"), *GetPathName() );
		}
	}
	return bIsLocalized;
}
...
void USoundNodeWave::PostEditChange( UProperty* PropertyThatChanged )
{
	...
	{
		if( PropertyThatChanged->GetName() == FString( TEXT( "CompressionQuality" ) ) 
			|| PropertyThatChanged->GetName() == FString( TEXT( "bCompressedSample" ) ))
		{
 ...
 */
void USoundNodeWave::FinishDestroy()
{
	FreeResources();
...
void USoundNodeWave::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	...
	{
		if( ComponentWaveInstances(i)->ParentGUID == ParentGUID )
		{
	...
	if( WaveInstance == NULL )
	{
		...
		if( !AudioComponent->bSuppressSubtitles )
		{
			...
			FSubtitleManager::GetSubtitleManager()->QueueSubtitles( ( DWORD )WaveInstance, AudioComponent->SubtitlePriority, bManualWordWrap, Duration, Subtitles );
		}
	}
	...
	if( !WaveInstance->bIsFinished )
	{
		...
		WaveInstance->Priority					= AudioComponent->CurrentVolume + AudioComponent->CurrentPriorityMultiplier;
		...
		WaveInstance->SoundModeVoice			= AudioComponent->CurrentSoundModeVoice;
		WaveInstance->bIsMusic					= AudioComponent->bCurrentIsMusic;
		WaveInstance->bNoReverb					= AudioComponent->bCurrentNoReverb;
		...
		WaveInstance->bHeadRelative				= AudioComponent->CurrentHeadRelative;
...
FLOAT USoundNodeWave::GetDuration()
{
...
void USoundNodeAttenuation::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	if( TRUE || AudioComponent->bAllowSpatialization )
	{
		if( bAttenuate )
		{
			...
			{
				UsedMinRadius = MinRadius.GetValue(0.f, AudioComponent);
				UsedMaxRadius = MaxRadius.GetValue(0.f, AudioComponent);
			...
			{
				FLOAT DistanceFraction = Distance / UsedMaxRadius;
				...
				else if( DistanceModel == ATTENUATION_Logarithmic )
				{
					...
					{
						Constant = -1.0f / log( UsedMinRadius / UsedMaxRadius );
					}
					AudioComponent->CurrentVolume *= Min( Constant * -log( DistanceFraction ), 1.0f );
				}
				else if( DistanceModel == ATTENUATION_Inverse )
				{
					...
					AudioComponent->CurrentVolume *= Min( Constant * ( 0.02f / DistanceFraction ), 1.0f );
				}
				else if( DistanceModel == ATTENUATION_LogReverse )
				{
					...
					AudioComponent->CurrentVolume *= Max( 1.0f - Constant * log( 1.0f / ( 1.0f - DistanceFraction ) ), 0.0f );
				}
			}
		}
...
void USoundNodeLooping::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
...
void USoundNodeOscillator::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
...
void USoundNodeModulator::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	RETRIEVE_SOUNDNODE_PAYLOAD( sizeof(FLOAT) + sizeof(FLOAT) );
...
void USoundNodeModulatorContinuous::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	RETRIEVE_SOUNDNODE_PAYLOAD( sizeof(FLOAT) + sizeof(FLOAT) );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedVolumeModulation );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedPitchModulation );
...
void USoundNodeAmbient::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	RETRIEVE_SOUNDNODE_PAYLOAD( sizeof(FLOAT) + sizeof(FLOAT) + sizeof(FLOAT) + sizeof(FLOAT) );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedMinRadius );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedMaxRadius );
	...
	if( Wave )
	{
		INT PreviousNum = WaveInstances.Num();
		Wave->ParseNodes( this, 0, AudioComponent, WaveInstances );
...
void USoundNodeAmbientNonLoop::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	RETRIEVE_SOUNDNODE_PAYLOAD( sizeof(FLOAT) + sizeof(FLOAT) + sizeof(FLOAT) + sizeof(FLOAT) + sizeof(INT) );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedMinRadius );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedMaxRadius );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedVolumeModulation );
	DECLARE_SOUNDNODE_ELEMENT( FLOAT, UsedPitchModulation );
	...
	if(AudioComponent->PlaybackTime >= NextSoundTime)
	{
		...
		if( SlotIndex < SoundSlots.Num() )
		{
			...
			{
				SoundSlots(SlotIndex).Wave->ParseNodes( this, SlotIndex, AudioComponent, WaveInstances );
			}
...
void USoundNodeWaveParam::ParseNodes(USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances)
{
	USoundNodeWave* NewWave = NULL;
	AudioComponent->GetWaveParameter(WaveParameterName, NewWave);
	if (NewWave != NULL)
	{
		NewWave->ParseNodes( this, -1, AudioComponent, WaveInstances );
	}
	else
	{
...
void USoundNodeRandom::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	...
	{
		ChildNodes(NodeIndex)->ParseNodes( this, NodeIndex, AudioComponent, WaveInstances );	
	}
}
 ...
 *
 **/
void USoundNodeDelay::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	RETRIEVE_SOUNDNODE_PAYLOAD( sizeof(FLOAT) + sizeof(FLOAT) );
...
void USoundNodeConcatenator::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	RETRIEVE_SOUNDNODE_PAYLOAD( sizeof(INT) );
	DECLARE_SOUNDNODE_ELEMENT( INT, NodeIndex );
	...
	if( NodeIndex < ChildNodes.Num() )
	{
		...
		if( ChildNode )
		{
			...
			ChildNode->ParseNodes( this, NodeIndex, AudioComponent, WaveInstances );
			SavedState.Restore( AudioComponent );
		}
	}
}
 ...
 */
void USoundNodeMature::ParseNodes( USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	RETRIEVE_SOUNDNODE_PAYLOAD( sizeof(INT) );
	DECLARE_SOUNDNODE_ELEMENT( INT, NodeIndex );
