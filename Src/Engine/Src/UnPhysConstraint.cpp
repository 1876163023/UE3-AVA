...
void ARB_ConstraintActor::CheckForErrors()
{
	...
	if( ConstraintSetup == NULL )
	{
		GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf(TEXT("%s : RB_ConstraintActor actor has NULL ConstraintSetup property - please delete!"), *GetName() ), MCACTION_DELETE, TEXT("ConstraintSetupNull") );
	}
	if( ConstraintInstance == NULL )
	{
		GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf(TEXT("%s : RB_ConstraintActor actor has NULL ConstraintInstance property - please delete!"), *GetName() ), MCACTION_DELETE, TEXT("ConstraintInstanceNull") );
	}

	// Make sure constraint actors are not both NULL.
	if ( ConstraintActor1 == NULL && ConstraintActor2 == NULL )
	{
		GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf(TEXT("%s : both of RB_ConstraintActor's ConstraintActors are NULL"), *GetName() ), MCACTION_NONE, TEXT("BothConstraintsNull") );
	}
	else
	{
		...
		if ( ConstraintActor1 != NULL && ConstraintActor2 != NULL )
		{
			if ( ConstraintActor1->bStatic && ConstraintActor2->bStatic )
			{
				GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf(TEXT("%s : RB_ConstraintActor actor connects two static actors!"), *GetName() ), MCACTION_NONE, TEXT("BothConstraintsStatic") );
			}
		}
		else
		{
			...
			if ( ( ConstraintActor1 == NULL && ConstraintActor2->bStatic ) ||
				 ( ConstraintActor2 == NULL && ConstraintActor1->bStatic ) )
			{
				GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf(TEXT("%s : RB_ConstraintActor actor is connected to a single static actor"), *GetName() ), MCACTION_NONE, TEXT("ConstraintStaticOnly") );
			}
		}
	}
}
...
void URB_ConstraintInstance::InitConstraint(UPrimitiveComponent* PrimComp1, UPrimitiveComponent* PrimComp2, class URB_ConstraintSetup* Setup, FLOAT Scale, AActor* InOwner, UPrimitiveComponent* InPrimComp, UBOOL bMakeKinForBody1)
{
	...
	if(bMakeKinForBody1)
	{
		// Must specify NULL for PrimComp1 if using bMakeKinForBody1 flag
		if(!FirstActor)
		{
			return;
		}
		...
		FMatrix KinActorTM = N2UTransform(FirstActor->getGlobalPose());
		FirstActor = CreateDummyKinActor(&(FirstActor->getScene()), KinActorTM);
	...
	SetLinearPositionDrive(bLinearXPositionDrive, bLinearYPositionDrive, bLinearZPositionDrive);
	SetLinearVelocityDrive(bLinearXVelocityDrive, bLinearYVelocityDrive, bLinearZVelocityDrive);
	...
	SetAngularPositionDrive(bSwingPositionDrive, bTwistPositionDrive);
	SetAngularVelocityDrive(bSwingVelocityDrive, bTwistVelocityDrive);
...
void URB_ConstraintInstance::execTermConstraint _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void URB_ConstraintInstance::SetLinearPositionDrive(UBOOL bEnableXDrive, UBOOL bEnableYDrive, UBOOL bEnableZDrive)
{
	...
	if (Joint && Joint->getState() != NX_JS_BROKEN)
	{
		...
		if(D6Joint)
		{
			...
			if(bEnableXDrive)
				Desc.xDrive.driveType = Desc.xDrive.driveType | NX_D6JOINT_DRIVE_POSITION;
			else
				Desc.xDrive.driveType = Desc.xDrive.driveType & ~NX_D6JOINT_DRIVE_POSITION;

			if(bEnableYDrive)
				Desc.yDrive.driveType = Desc.yDrive.driveType | NX_D6JOINT_DRIVE_POSITION;
			else
				Desc.yDrive.driveType = Desc.yDrive.driveType & ~NX_D6JOINT_DRIVE_POSITION;

			if(bEnableZDrive)
				Desc.zDrive.driveType = Desc.zDrive.driveType | NX_D6JOINT_DRIVE_POSITION;
			else
				Desc.zDrive.driveType = Desc.zDrive.driveType & ~NX_D6JOINT_DRIVE_POSITION;
...
void URB_ConstraintInstance::SetLinearVelocityDrive(UBOOL bEnableXDrive, UBOOL bEnableYDrive, UBOOL bEnableZDrive)
{
	...
	if (Joint && Joint->getState() != NX_JS_BROKEN)
	{
		...
		if(D6Joint)
		{
			...
			if(bEnableXDrive)
				Desc.xDrive.driveType = Desc.xDrive.driveType | NX_D6JOINT_DRIVE_VELOCITY;
			else
				Desc.xDrive.driveType = Desc.xDrive.driveType & ~NX_D6JOINT_DRIVE_VELOCITY;

			if(bEnableYDrive)
				Desc.yDrive.driveType = Desc.yDrive.driveType | NX_D6JOINT_DRIVE_VELOCITY;
			else
				Desc.yDrive.driveType = Desc.yDrive.driveType & ~NX_D6JOINT_DRIVE_VELOCITY;

			if(bEnableZDrive)
				Desc.zDrive.driveType = Desc.zDrive.driveType | NX_D6JOINT_DRIVE_VELOCITY;
			else
				Desc.zDrive.driveType = Desc.zDrive.driveType & ~NX_D6JOINT_DRIVE_VELOCITY;
...
void URB_ConstraintInstance::SetAngularPositionDrive(UBOOL bEnableSwingDrive, UBOOL bEnableTwistDrive)
{
	...
	if (Joint && Joint->getState() != NX_JS_BROKEN)
	{
		...
		if(D6Joint)
		{
			...
			if(bEnableSwingDrive)
				Desc.swingDrive.driveType = Desc.swingDrive.driveType | NX_D6JOINT_DRIVE_POSITION;
			else
				Desc.swingDrive.driveType = Desc.swingDrive.driveType & ~NX_D6JOINT_DRIVE_POSITION;

			if(bEnableTwistDrive)
				Desc.twistDrive.driveType = Desc.twistDrive.driveType | NX_D6JOINT_DRIVE_POSITION;
			else
				Desc.twistDrive.driveType = Desc.twistDrive.driveType & ~NX_D6JOINT_DRIVE_POSITION;
			...
			if(bEnableSwingDrive && bEnableTwistDrive)
				Desc.slerpDrive.driveType = Desc.slerpDrive.driveType | NX_D6JOINT_DRIVE_POSITION;
			else
				Desc.slerpDrive.driveType = Desc.slerpDrive.driveType & ~NX_D6JOINT_DRIVE_POSITION;
...
void URB_ConstraintInstance::SetAngularVelocityDrive(UBOOL bEnableSwingDrive, UBOOL bEnableTwistDrive)
{
	...
	if (Joint && Joint->getState() != NX_JS_BROKEN)
	{
		...
		if(D6Joint)
		{
			...
			if(bEnableSwingDrive)
				Desc.swingDrive.driveType = Desc.swingDrive.driveType | NX_D6JOINT_DRIVE_VELOCITY;
			else
				Desc.swingDrive.driveType = Desc.swingDrive.driveType & ~NX_D6JOINT_DRIVE_VELOCITY;

			if(bEnableTwistDrive)
				Desc.twistDrive.driveType = Desc.twistDrive.driveType | NX_D6JOINT_DRIVE_VELOCITY;
			else
				Desc.twistDrive.driveType = Desc.twistDrive.driveType & ~NX_D6JOINT_DRIVE_VELOCITY;

			// If both drives are on, turn slerp drive on (in case its being used)
			if(bEnableSwingDrive && bEnableTwistDrive)
				Desc.slerpDrive.driveType = Desc.slerpDrive.driveType | NX_D6JOINT_DRIVE_VELOCITY;
			else
				Desc.slerpDrive.driveType = Desc.slerpDrive.driveType & ~NX_D6JOINT_DRIVE_VELOCITY;
...
void URB_ConstraintInstance::SetLinearDriveParams(FLOAT InSpring, FLOAT InDamping, FLOAT InForceLimit)
{
	...
	if (Joint && Joint->getState() != NX_JS_BROKEN)
	{
		...
		if(D6Joint)
		{
			...
			Desc.xDrive.spring = InSpring * SpringScale;
			Desc.xDrive.damping = InDamping * DampScale;
			Desc.xDrive.forceLimit = InForceLimit * ForceScale;

			Desc.yDrive.spring = InSpring * SpringScale;
			Desc.yDrive.damping = InDamping * DampScale;
			Desc.yDrive.forceLimit = InForceLimit * ForceScale;

			Desc.zDrive.spring = InSpring * SpringScale;
			Desc.zDrive.damping = InDamping * DampScale;
			Desc.zDrive.forceLimit = InForceLimit * ForceScale;
...
void URB_ConstraintInstance::SetAngularPositionTarget(FQuat InPosTarget)
{
#if WITH_NOVODEX
	NxJoint* Joint = (NxJoint*)ConstraintData;
	if (Joint)
...
{
	//  [10/30/2006 otterrrr] BUG 
	// 아래가 주석처리 되지 않으면 절대 SetDriveAngularVelocity에 도달할 수 없다.
	// SetAngularVelocityTarget( AngularVelocityTarget )와 같이 이 메소드가 불리기 때문이다.
	// 결과적으로 DriveAngularVelocity가 적용되지 않으므로 모터도 동작 안한다.
...
void URB_ConstraintInstance::SetAngularDriveParams(FLOAT InSpring, FLOAT InDamping, FLOAT InForceLimit)
{
	...
	if (Joint && Joint->getState() != NX_JS_BROKEN)
	{
		...
		if(D6Joint)
		{
			...
			Desc.swingDrive.spring = InSpring * SpringScale;
			Desc.swingDrive.damping = InDamping * DampScale;
			Desc.swingDrive.forceLimit = InForceLimit * ForceScale;

			Desc.twistDrive.spring = InSpring * SpringScale;
			Desc.twistDrive.damping = InDamping * DampScale;
			Desc.twistDrive.forceLimit = InForceLimit * ForceScale;

			Desc.slerpDrive.spring = InSpring * SpringScale;
			Desc.slerpDrive.damping = InDamping * DampScale;
			Desc.slerpDrive.forceLimit = InForceLimit * ForceScale;
...
void URB_ConstraintInstance::MoveKinActorTransform(FMatrix& NewTM)
{
	...
	if(DummyKinActor)
	{
		...
		NewTM.RemoveScaling();
		nActor->moveGlobalPose( U2NTransform(NewTM) );
	}
#endif
}
