...
//! 소유자가 유무에 따라서 LastRenderTime을 얻어온다.(2007/07/09)
inline FLOAT GetOwnerLastRenderTime(USkeletalMeshComponent *SkelMeshComp)
{
	// 소유자가 있다면 소유자의 LastRenderTime을 리턴.
	// 없다면 자기자신의 LastRenderTimer을 리턴.
	return SkelMeshComp->GetOwner() ? SkelMeshComp->GetOwner()->LastRenderTime : SkelMeshComp->LastRenderTime;
}

inline UBOOL IsRecentlyRendered(USkeletalMeshComponent *SkelMeshComp)
{
	return ( GetOwnerLastRenderTime(SkelMeshComp) > (GWorld->GetWorldInfo()->TimeSeconds - SkelMeshComp->SkelVisibleTime ) );
}
...
#define SHOW_SKELETAL_MESH_COMPONENT_TICK_TIME 1

#define SHOW_ANIMNODE_TICK_TIMES	1
...
void USkeletalMeshComponent::Attach()
{
	if( SkeletalMesh )
	{
		if (!GIsUCC)
		{
			...
			const UBOOL bSupportsGPUMorphing = !IsSM2Platform( GRHIShaderPlatform );
			//>@ ava 
			...
			//@AVA HACK
			// AVA에서는 CPU skin을 사용할 경우가 없음!
			if(bIsCPUSkinned && (GIsEditor || !bForceNotToUseCPUSkinInGame))
			{
	...
	if( !FaceFXActorInstance && SkeletalMesh )
	{
		if( SkeletalMesh->FaceFXAsset )
		{
			...
			FaceFXActorInstance->GetAnimPlayer().SetWantBlendInAndOut(FxFalse);
		}
	}
#endif // WITH_FACEFX
...
void USkeletalMeshComponent::UpdateTransform()
{
	...
	const UBOOL bRecentlyRendered = IsRecentlyRendered( this );
	...
	if(PhysicsAssetInstance && !bHasHadPhysicsBlendedIn && GWorld->InTick && GWorld->TickGroup == TG_PostAsyncWork)
	{
		extern UBOOL GUseCascadedShadow;	// ava specific
		if(bRecentlyRendered || bUpdateSkelWhenNotRendered || (bCastHiddenShadow && GUseCascadedShadow) )	//<@ ava specific ; add cascaded shadow ; 2008. 1. 3 changmin
		{
			BlendInPhysics();
		}
	...
	if((PhysicsAssetInstance || (ParentAnimComponent && ParentAnimComponent->PhysicsAssetInstance)) && GWorld->InTick && GWorld->TickGroup != TG_PostAsyncWork )
	{
		bFinalUpdate = FALSE;
	}
	...
	// add cascaded shadow
	// todo -> bCastHiddenShadow flag로 전환..
	extern UBOOL GUseCascadedShadow;
	if( bFinalUpdate && MeshObject && ( bRecentlyRendered || MeshObject->bHasBeenUpdatedAtLeastOnce == FALSE || (bCastHiddenShadow && GUseCascadedShadow)))
	{
...
void USkeletalMeshComponent::TickAnimNodes(FLOAT DeltaTime)
{
	...
	for(INT i=0; i<AnimTickArray.Num(); i++)
	{
		...
		if( Node->bRelevant || !Node->bSkipTickWhenZeroWeight && !bPauseAnims )
		{
...
void USkeletalMeshComponent::Tick(FLOAT DeltaTime)
{
	...
	extern UBOOL GUseCascadedShadow;	// 2008. 1. 7 changmin ; add cascaded shadow
	...
	if( MeshObject && Animations && bHasBegunPlay )//&& (bRecentlyRendered || bUpdateSkelWhenNotRendered) || ( bCastHiddenShadow && GUseCascadedShadow ) )
	{		
	...
	INT		PrevLODLevel = PredictedLODLevel;	
	UBOOL	bLODHasChanged = UpdateLODStatus();

	
	if ((bRecentlyRendered || bUpdateSkelWhenNotRendered) || bLODHasChanged || ( bCastHiddenShadow && GUseCascadedShadow ) )	// 2008. 1. 7 changmin ; add cascaded shadow
	{
#if !FINAL_RELEASE && SHOW_SKELETAL_MESH_COMPONENT_TICK_TIME
		...
		if(Owner && Owner->Physics == PHYS_RigidBody && (BodyInstance || PhysicsAssetInstance))
		{
			...
			if( bAsleep )
			{
				if ( PrevLODLevel > PredictedLODLevel && FramesPhysicsAsleep >= 5 )
		...
		{
			debugf( TEXT( "SkelMesh:  %s  Owner: %s, UpdateSkelPose: \t%fms" ), *SkeletalMesh->GetPathName(), *GetOwner()->GetName(), UpdateSkelPoseStop );
		}
#endif
...
void USkeletalMeshComponent::AttachComponentToSocket(UActorComponent* Component,FName SocketName)
{
	if( SkeletalMesh != NULL )
	{
		...
		else
		{
			...
			debugf( TEXT("AttachComponentToSocket : Could not find socket '%s' in %s  attaching: %s"), *SocketName.ToString(), *GetOwner()->GetName(), *Component->TemplateName.ToString() );
#endif
		}
	}
	else
	{
#if !PS3 // hopefully will be found on PC
		debugf( TEXT("AttachComponentToSocket : no SkeletalMesh could not attach socket '%s'  attaching: %s"), *SocketName.ToString(), *Component->TemplateName.ToString() );
#endif
	}
}
...
void USkeletalMeshComponent::ComposeSkeleton(TArray<FBoneAtom>& LocalTransforms, const TArray<BYTE>& RequiredBones)
{
	...
	const UBOOL bRenderedRecently = IsRecentlyRendered( this );
...
void USkeletalMeshComponent::UpdateFaceFX( TArray<FBoneAtom>& LocalTransforms, UBOOL bTickFaceFX )
{
	...
	if( FaceFXActorInstance )
	{
		...
		if( FaceFXActor )
		{
			...
			FxAnimPlayer& AnimPlayer = FaceFXActorInstance->GetAnimPlayer();
			if( AnimPlayer.IsPlaying() || 
				FaceFXActorInstance->GetAllowNonAnimTick() || 
				FaceFXActor->IsOpenInStudio() || 
				!bTickFaceFX ) 
			{
				...
				if(bTickFaceFX)
				{
					...
					FxAnimPlayerState PlayerState = APS_Stopped;
					{
						...
						PlayerState = FaceFXActorInstance->Tick(AppTime, AudioOffset);
					}

					if( APS_StartAudio == PlayerState )
					{
						if( CachedFaceFXAudioComp )
						{
							if( CachedFaceFXAudioComp->SoundCue )
				...
				FxFaceGraph& FaceGraph = FaceFXActor->GetFaceGraph();

				{
					SCOPE_CYCLE_COUNTER(STAT_FaceFX_MorphPassTime);
					// Update any morph nodes that may be in the Face Graph.
					FxFaceGraph::Iterator MorphNodeIter    = FaceGraph.Begin(FUnrealFaceFXMorphNode::StaticClass());
					FxFaceGraph::Iterator MorphNodeEndIter = FaceGraph.End(FUnrealFaceFXMorphNode::StaticClass());
					for( ; MorphNodeIter != MorphNodeEndIter; ++MorphNodeIter )
					{
   						FUnrealFaceFXMorphNode* MorphNode = FxCast<FUnrealFaceFXMorphNode>(MorphNodeIter.GetNode());
   						if( MorphNode )
   						{
							FFaceFXMorphTargetProxy* Proxy = FxCast<FFaceFXMorphTargetProxy>(MorphNode->GetGenericTargetProxy());
							// If the proxy is invalid create one.
							if( !Proxy )
							{
								FFaceFXMorphTargetProxy* NewMorphTargetProxy = new FFaceFXMorphTargetProxy();
								MorphNode->SetGenericTargetProxy(NewMorphTargetProxy);
								MorphNode->SetShouldLink(FxTrue);
							...
							if( Proxy )
							{
								...
								if( MorphNode->ShouldLink() || bShouldClientRelink )
								{
									const FxFaceGraphNodeUserProperty& MorphTargetNameProperty = 
										MorphNode->GetUserProperty(FX_MORPH_NODE_TARGET_NAME_INDEX);
									Proxy->Link(MorphTargetNameProperty.GetStringProperty());
									MorphNode->SetShouldLink(FxFalse);
								}
								Proxy->SetSkeletalMeshComponent(this);
								MorphNode->UpdateTarget();
							}
						}
					}
				}

				{
					...
					FxFaceGraph::Iterator MaterialParameterNodeIter    = FaceGraph.Begin(FUnrealFaceFXMaterialParameterNode::StaticClass());
					FxFaceGraph::Iterator MaterialParameterNodeEndIter = FaceGraph.End(FUnrealFaceFXMaterialParameterNode::StaticClass());
					for( ; MaterialParameterNodeIter != MaterialParameterNodeEndIter; ++MaterialParameterNodeIter )
					{
   						FUnrealFaceFXMaterialParameterNode* MaterialParameterNode = FxCast<FUnrealFaceFXMaterialParameterNode>(MaterialParameterNodeIter.GetNode());
   						if( MaterialParameterNode )
   						{
							FFaceFXMaterialParameterProxy* Proxy = FxCast<FFaceFXMaterialParameterProxy>(MaterialParameterNode->GetGenericTargetProxy());
							// If the proxy is invalid create one.
							if( !Proxy )
							{
								FFaceFXMaterialParameterProxy* NewMaterialParameterProxy = new FFaceFXMaterialParameterProxy();
								MaterialParameterNode->SetGenericTargetProxy(NewMaterialParameterProxy);
								MaterialParameterNode->SetShouldLink(FxTrue);
							...
							{
								// If the proxy requests to be re-linked, link it up.
								if( MaterialParameterNode->ShouldLink() || bShouldClientRelink )
								{
									const FxFaceGraphNodeUserProperty& MaterialSlotProperty = 
										MaterialParameterNode->GetUserProperty(FX_MATERIAL_PARAMETER_NODE_MATERIAL_SLOT_ID_INDEX);
									const FxFaceGraphNodeUserProperty& ParameterNameProperty =
										MaterialParameterNode->GetUserProperty(FX_MATERIAL_PARAMETER_NODE_PARAMETER_NAME_INDEX);
									Proxy->Link(MaterialSlotProperty.GetIntegerProperty(), ParameterNameProperty.GetStringProperty());
									MaterialParameterNode->SetShouldLink(FxFalse);
								}
								Proxy->SetSkeletalMeshComponent(this);
								MaterialParameterNode->UpdateTarget();
							}
						}
					}
				}

				{
					...
					FxMasterBoneList& MasterBoneList = FaceFXActor->GetMasterBoneList();
					FaceFXActorInstance->UpdateBonePoses();
...
UBOOL USkeletalMeshComponent::PlayFaceFXAnim(UFaceFXAnimSet* FaceFXAnimSetRef, const FString& AnimName, const FString& GroupName)
{
	...
	if( FaceFXActorInstance )
	{
		...
		UBOOL bPlaySuccess = FaceFXActorInstance->GetAnimPlayer().Play(TCHAR_TO_ANSI(*AnimName), TCHAR_TO_ANSI(*GroupName));

		// If that worked, we want to start some audio
		if( bPlaySuccess )
		{
			// If we have an animation playing, and an Owner Actor
			const FxAnim* Anim = FaceFXActorInstance->GetAnimPlayer().GetCurrentAnim();
			if( Anim && Owner )
			{
...
void USkeletalMeshComponent::StopFaceFXAnim( void )
{
	...
	if( FaceFXActorInstance )
	{
		...
		FaceFXActorInstance->GetAnimPlayer().Stop();
	}
#endif // WITH_FACEFX
}
...
UBOOL USkeletalMeshComponent::IsPlayingFaceFXAnim()
{
	...
	return (AnimPlayer.IsPlaying() == FxTrue);
#else
	return FALSE;
#endif
}
	...
	if( FaceFXActorInstance )
	{
		...
		if( pRegMapEntry )
		{
			...
			case FXRO_Add:
				...
				break;
			case FXRO_Multiply:
				RegisterOp = VO_Multiply;
				break;
			case FXRO_Replace:
				RegisterOp = VO_Replace;
				break;
			default:
...
void USkeletalMeshComponent::SetFaceFXRegisterEx( const FString& RegName, BYTE RegOp, FLOAT FirstValue, FLOAT FirstInterpDuration, FLOAT NextValue, FLOAT NextInterpDuration )
{
	...
	if( FaceFXActorInstance )
	{
		...
		if( pRegMapEntry )
		{
			...
			FaceFXActorInstance->SetRegisterEx(pRegMapEntry->FaceFXRegName, RegisterOp, FirstValue, FirstInterpDuration, NextValue, NextInterpDuration);
		}
		else
		{
			debugf(TEXT("FaceFX: WARNING Attempt to write to undeclared register %s"), *RegName);
...
void USkeletalMeshComponent::UpdateSkelPose( FLOAT DeltaTime, UBOOL bTickFaceFX )
{
...
#if !FINAL_RELEASE && ENABLE_GETBONEATOM_STATS
	...
	ComposeSkeleton(LocalAtoms, RequiredBones);
 	...
 	if( GShouldLogOutAFrameOfSkelCompTick == TRUE )
 	{
 		...
 		debugf( TEXT( "ComposeSkeleton_Time: %s \t%f" ), SkeletalMesh?*SkeletalMesh->GetName():TEXT("NONE"), ComposeSkeleton_Time );
 	}
#endif
...
void USkeletalMeshComponent::execGetBoneMatrix _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(BoneIdx);
	P_FINISH;
...
void USkeletalMeshComponent::execMatchRefBone _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
	P_FINISH;
...
FName USkeletalMeshComponent::GetBoneNameByIndex( INT BoneIndex )
{
	if ( SkeletalMesh && BoneIndex >= 0 && BoneIndex < SkeletalMesh->RefSkeleton.Num() )
		return SkeletalMesh->RefSkeleton(BoneIndex).Name;
	else
		return NAME_None;
}

void USkeletalMeshComponent::execGetBoneNameByIndex _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_INT(Index);
	P_FINISH;
	*(FName*)Result = GetBoneNameByIndex(Index);
}

void USkeletalMeshComponent::execGetParentBone _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
	P_FINISH;
...
void USkeletalMeshComponent::execGetBoneNames _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execSetEnableClothSimulation _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execFindAnimSequence _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execFindMorphTarget _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execAttachComponent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execDetachComponent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UActorComponent,Component);
	P_FINISH;
...
void USkeletalMeshComponent::execAttachComponentToSocket _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UActorComponent,Component);
...
void USkeletalMeshComponent::execGetSocketWorldLocationAndRotation _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execGetSocketByName _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(InSocketName);
	P_FINISH;
...
void USkeletalMeshComponent::execFindComponentAttachedToBone _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execIsComponentAttached _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UActorComponent,Component)
...
void USkeletalMeshComponent::execAttachedComponents _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execSetSkeletalMesh _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execSetPhysicsAsset _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execSetParentAnimComponent _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execGetBoneQuaternion _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
...
void USkeletalMeshComponent::execGetBoneLocation _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
	P_GET_INT_OPTX(Space,0);
	P_FINISH;
...
void USkeletalMeshComponent::execGetBoneAxis _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
	P_GET_BYTE(Axis);
	P_FINISH;
 ...
 */
void USkeletalMeshComponent::execTransformToBoneSpace _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
 ...
 */
void USkeletalMeshComponent::execTransformFromBoneSpace _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
	P_GET_VECTOR(InPosition);
	P_GET_ROTATOR(InRotation);
...
void USkeletalMeshComponent::execFindClosestBone _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execSetAnimTreeTemplate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execUpdateParentBoneMap _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void USkeletalMeshComponent::execInitSkelControls _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void USkeletalMeshComponent::execFindSkelControl _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execFindAnimNode _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
 ...
 */
void USkeletalMeshComponent::execAllAnimNodes _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_OBJECT(UClass, BaseClass);
...
void USkeletalMeshComponent::execFindMorphNode _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(InNodeName);
	P_FINISH;
...
void USkeletalMeshComponent::execFindConstraintIndex _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
/** Script-exposed execFindConstraintBoneName function. */
void USkeletalMeshComponent::execFindConstraintBoneName _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execFindBodyInstanceNamed _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_NAME(BoneName);
	P_FINISH;
...
void USkeletalMeshComponent::execForceSkelUpdate _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void USkeletalMeshComponent::execUpdateAnimations _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void USkeletalMeshComponent::execPlayFaceFXAnim _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
...
void USkeletalMeshComponent::execStopFaceFXAnim _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void USkeletalMeshComponent::execIsPlayingFaceFXAnim _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
...
void USkeletalMeshComponent::execDeclareFaceFXRegister _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_STR(RegName);
	P_FINISH;
...
void USkeletalMeshComponent::execGetFaceFXRegister _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_STR(RegName);
	P_FINISH;
...
void USkeletalMeshComponent::execSetFaceFXRegister _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_STR(RegName);
	P_GET_FLOAT(RegVal);
	P_GET_BYTE(RegOp);
...
void USkeletalMeshComponent::execSetFaceFXRegisterEx _ParamList_ScriptFunctionImpl // 20080221 dEAthcURe|CD // ( FFrame& Stack, RESULT_DECL )
{
	P_GET_STR(RegName);
	P_GET_BYTE(RegOp);
...
/**
 * Computes decal texture coordinates for the specified position.
 *
 * @param		Decal			Decal info.
 * @param		InPos			World-space position.
 * @param		OutTexCoords	[out] Decal texture coordinates.
 */
static FORCEINLINE void ComputeTextureCoordinates(const FDecalState* Decal,
												  const FVector& InPos,
												  FVector2D& OutTexCoords)
{
