										   ...
										   FLOAT T1X, FLOAT T1Y, FLOAT T1Z, const FOctreeNodeBounds& Bounds)
{
	...
	for(INT i=0; i<Primitives.Num(); i++)
	{
		...
		if(TestPrimitive->Tag != UPrimitiveComponent::CurrentTag)
		{
			...
			if (o->ChkTraceFlags & TRACE_LightVisibility)
 			{
 				if (!TestPrimitive->CastShadow || !TestPrimitive->bAcceptsLights)
 				{
 					continue;
 				}
 			}
 			else if( !(o->ChkTraceFlags & TRACE_ShadowCast) )
			{
			...
			&&	PrimOwner->ShouldTrace(TestPrimitive,o->ChkActor, o->ChkTraceFlags) )
			{
				...
				if( lineChkRes )
				{
					...
					{
						debugf( TEXT("ZELC: Component '%s' with Owner '%s' returned normal with incorrect length (%f)"),
							*TestPrimitive->GetName(),*PrimOwner->GetName(),Hit.Normal.Size());
					}
#endif
		...
		if(TestPrimitive->Tag != UPrimitiveComponent::CurrentTag)
		{
				...
				PrimOwner->ShouldTrace(TestPrimitive, o->ChkActor, o->ChkTraceFlags) )
			{
				...
				if(lineChkRes)
				{
					...
					//{
					//	debugf(TEXT("NZELC: Component '%s' with Owner '%s' returned normal with incorrect length (%f)"),
	...
	for(INT i=0; i<Primitives.Num(); i++)
	{
			...
			PrimOwner && (PrimOwner->OverlapTag != UPrimitiveComponent::CurrentTag || GIsEditor))
		{
				...
				!((o->ChkActor->Physics == PHYS_Interpolating) && PrimOwner->bWorldGeometry) )
			{
				...
				if(o->ChkActor->IsOverlapping(PrimOwner, &TestHit, TestPrimitive))
				{
#if CHECK_FALSE_NEG
					if(!hitActorBox)
	...
	const FOctreeNodeBounds& Bounds,FRadiusOverlapCheck& Check)
{
	...
	if (Children)
	{
		...
		for(INT i=0; i<numChildren; i++)
		{
			Children[childIXs[i]].ActorOverlapCheck(Octree,FOctreeNodeBounds(Bounds,childIXs[i]),Check);
		}
	}
	...
	const FVector& Location,FLOAT Radius)
{
	...
	// Build the precalculated info
	FRadiusOverlapCheck CheckInfo(Location,Radius);
	RootNode->ActorOverlapCheck(this, RootNodeBounds, CheckInfo);
