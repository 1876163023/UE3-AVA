/*=============================================================================
	...
	Copyright 2002-2005 Epic Games, Inc. All Rights Reserved.
=============================================================================*/
...
UBOOL UStaticMeshComponent::LineCheck(FCheckResult& Result,const FVector& End,const FVector& Start,const FVector& Extent,DWORD TraceFlags)
{
	...
	if( Owner && 
		bWantSimpleCheck && 
//{{DEIF - Sun light visibility check¿ë!
		((TraceFlags & TRACE_LightVisibility) ||	
//}}DEIF
	...
	else if (StaticMesh->kDOPTree.Nodes.Num())
	{
		...
		if (ZeroExtent == TRUE)
		{
			...
			TkDOPLineCollisionCheck<FStaticMeshCollisionDataProvider> kDOPCheck(Start,End,TraceFlags,Provider,&Result);
			// Do the line check
		...
		{
			// Create the check structure with all the local space fun
			TkDOPBoxCollisionCheck<FStaticMeshCollisionDataProvider> kDOPCheck(Start,End,Extent,TraceFlags,Provider,&Result);
...
UBOOL UStaticMeshComponent::PointCheck(FCheckResult& Result,const FVector& Location,const FVector& Extent,DWORD TraceFlags)
{
	...
	{ 
		...
		FStaticMeshCollisionDataProvider Provider(this);
		// Create the check structure with all the local space fun
		TkDOPPointCollisionCheck<FStaticMeshCollisionDataProvider> kDOPCheck(Location,Extent,Provider,&Result);
