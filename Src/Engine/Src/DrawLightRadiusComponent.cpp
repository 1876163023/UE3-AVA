...
FPrimitiveSceneProxy* UDrawLightRadiusComponent::CreateSceneProxy()
{
	...
	public:
		...
		FDrawLightRadiusSceneProxy(const UDrawLightRadiusComponent* InComponent)
			  ...
			  )
		  {
			  ...
			  {
				  DrawSphere(PDI,LocalToWorld.GetOrigin(), FVector(SphereRadius), SphereSides, SphereSides/2, SphereMaterial->GetInstanceInterface(TRUE), SDPG_World);
			  }
		  }
	...
	return IsOwnerSelected() ? new FDrawLightRadiusSceneProxy(this) : NULL;
}
