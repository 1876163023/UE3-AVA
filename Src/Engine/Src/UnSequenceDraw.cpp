...
FIntPoint USequenceObject::GetTitleBarSize(FCanvas* Canvas)
{
	...
	// check for client side objects
	if (IsA(USequenceEvent::StaticClass()) && ((USequenceEvent*)this)->bClientSideOnly)
	{
...
void USequenceOp::DrawVariableConnectors(FCanvas* Canvas, const FIntPoint& Pos, const FIntPoint& Size, INT MouseOverConnType, INT MouseOverConnIndex)
{
	...
	FLinkedObjDrawUtils::DrawVariableConnectors( Canvas, ObjInfo, Pos, Size );
...
void USequenceOp::DrawSeqObj(FCanvas* Canvas, UBOOL bSelected, UBOOL bMouseOver, INT MouseOverConnType, INT MouseOverConnIndex, FLOAT MouseOverTime)
{
	...
	if(bMouseOver)
	{
		...
		DrawVariableConnectors(	Canvas, FIntPoint(ObjPosX, ObjPosY + TitleSize.Y + 1 + LogicSize.Y),	FIntPoint(Width, VarSize.Y), MouseOverConnType, MouseOverConnIndex);
	}
	else
	{
		DrawLogicConnectors(	Canvas, FIntPoint(ObjPosX, ObjPosY + TitleSize.Y + 1),					FIntPoint(Width, LogicSize.Y), -1, INDEX_NONE);
		DrawVariableConnectors(	Canvas, FIntPoint(ObjPosX, ObjPosY + TitleSize.Y + 1 + LogicSize.Y),	FIntPoint(Width, VarSize.Y), -1, INDEX_NONE);
	}
}
...
#define TRIANGLE_SIZE 32
	...
	if(NumVisibleVarLinks > 0)
	{
		...
		if(bMouseOver)
		{
			DrawVariableConnectors(Canvas, FIntPoint(ObjPosX, TriangleTop + TRIANGLE_SIZE * 2 + LogicSize.Y + 3), FIntPoint(MaxWidth, VarSize.Y), MouseOverConnType, MouseOverConnIndex);
		}
		else
		{
			DrawVariableConnectors(Canvas, FIntPoint(ObjPosX, TriangleTop + TRIANGLE_SIZE * 2 + LogicSize.Y + 3), FIntPoint(MaxWidth, VarSize.Y), -1, INDEX_NONE);
		}
	}
...
void USequenceVariable::DrawSeqObj(FCanvas* Canvas, UBOOL bSelected, UBOOL bMouseOver, INT MouseOverConnType, INT MouseOverConnIndex, FLOAT MouseOverTime)
{
	...
	INT XL, YL;
	StringSize(GEngine->SmallFont, *VarString, XL, YL);
...
void USeqVar_MusicTrack::DrawExtraInfo(FCanvas* Canvas, const FVector& CircleCenter)
{
	...
	DrawTile( Canvas, appTrunc(CircleCenter.X-16), appTrunc(CircleCenter.Y-16), 32, 32, 0.f, 0.f, 1.f, 1.f, UseMaterial->GetInstanceInterface(0) );
}
...
void USequenceFrame::DrawFrameBox(FCanvas* Canvas, UBOOL bSelected)
{
	...
	if(bFilled)
	{
		...
		if(FillMaterial)
		{
			...
			if(bTileFill)
			{
				DrawTile(Canvas, ObjPosX, ObjPosY, SizeX, SizeY, 0.f, 0.f, (FLOAT)SizeX/64.f, (FLOAT)SizeY/64.f, FillMaterial->GetInstanceInterface(0) );
			}
			else
			{
				DrawTile(Canvas, ObjPosX, ObjPosY, SizeX, SizeY, 0.f, 0.f, 1.f, 1.f, FillMaterial->GetInstanceInterface(0) );
			}
		}
		else if(FillTexture)
		{
...
FColor USeqCond_SwitchClass::GetConnectionColor( INT ConnType, INT ConnIndex, INT MouseOverConnType, INT MouseOverConnIndex )
{
	if( ConnType == LOC_OUTPUT )
	{
		if( (MouseOverConnType != LOC_OUTPUT || MouseOverConnIndex != ConnIndex) && !ClassArray(ConnIndex).bFallThru ) 
		{
			return FColor(255,0,0);
		}
	}
