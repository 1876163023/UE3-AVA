...
{
	return MaterialInstance->GetMaterial()->IsMasked() ?
MaterialInstance :
	GEngine->DefaultMaterial->GetInstanceInterface(FALSE);
}
...
IMPLEMENT_SHADER_TYPE(template<>,TShadowProjectionPixelShader<F4SampleHwPCF>,TEXT("ShadowProjectionPixelShader"),TEXT("HardwarePCFMain"),SF_Pixel,VER_HARDWARE_PCF,0);
//Shader Model 2 compatible version
IMPLEMENT_SHADER_TYPE(template<>,TShadowProjectionPixelShader<F4SampleManualPCF>,TEXT("ShadowProjectionPixelShader"),TEXT("Main"),SF_Pixel,VER_HARDWARE_PCF,0);
//Full version that uses Hardware PCF
IMPLEMENT_SHADER_TYPE(template<>,TShadowProjectionPixelShader<F16SampleHwPCF>,TEXT("ShadowProjectionPixelShader"),TEXT("HardwarePCFMain"),SF_Pixel,VER_HARDWARE_PCF,0);
...
//Full version
IMPLEMENT_SHADER_TYPE(template<>,TShadowProjectionPixelShader<F16SampleManualPCF>,TEXT("ShadowProjectionPixelShader"),TEXT("Main"),SF_Pixel,VER_HARDWARE_PCF,0);
...
*/
FShadowProjectionPixelShaderInterface* GetProjPixelShaderRef()
{
	...
	)
{
	...
	FShadowDepthDrawingPolicy DrawingPolicy(Mesh.VertexFactory,GetShadowDepthMaterial(Mesh.MaterialInstance),ShadowInfo);
	...
	)
{
	...
	if( bDirectionalLight )
		SubjectMatrix = WorldToFace * FShadowProjectionMatrix(MinLightW,MaxLightW,WAxis);
	else
...
void FProjectedShadowInfo::RenderDepth(FCommandContextRHI* Context, const FSceneRenderer* SceneRenderer,BYTE DepthPriorityGroup)
{
	...
	if (GRHIDebug[5])
	{debugf(TEXT("Dumping shadow"));}
	if (GRHIDebug[5])
		DumpRenderTarget( GPS3RHIDepthTarget );
	if (GRHIDebug[5])
		DumpRenderTarget( GPS3RHIRenderTarget );
#endif // USE_PS3_RHI
#endif // PS3
}
...
void FProjectedShadowInfo::RenderProjection(FCommandContextRHI* Context, const FViewInfo* View) const
{
...
#endif
	{
		...
		if(bPreShadow)
		{
			...
			TDynamicPrimitiveDrawer<FDepthDrawingPolicyFactory> Drawer(Context,View,ShadowDPG,FDepthDrawingPolicyFactory::ContextType(),TRUE);
			for(INT PrimitiveIndex = 0;PrimitiveIndex < ReceiverPrimitives.Num();PrimitiveIndex++)
			{
				...
				ReceiverPrimitives(PrimitiveIndex)->Proxy->DrawDynamicElements(&Drawer,View,ShadowDPG);
			}

			ParticleRendering_EndBatch(&Drawer, View, ShadowDPG);
		}
		else
		{
			// 2007. 5. 10 changmin
			// ati card 중에, shadow stencil 을 그릴때, 뒤의 color buffer도 망가뜨린다.
			// 이를 막기위해... 나중에 덮어써지는 framebuffer를 render target으로 설정한다.
			//extern UBOOL GBlackShadowVolume;
			//if( GBlackShadowVolume )
			//{
			//	RHISetRenderTarget( Context, View->Family->RenderTarget->RenderTargetSurfaceRHI, GSceneRenderTargets.GetSceneDepthSurface() );
			//}

			// 2007. 5. 15 changmin
			// ATI 에서 알려준 정보.
			// FP Buffer 에서 D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING이 지원되지 않으면.. ColorWrite 설정이 작동하지 않는다...
			// 따라서, 현재 SceneColorBuffer대신 다른 Buffer를 설정한다.
			if( !GSupportsFPBlending )
			{
				RHISetRenderTarget( Context, View->Family->RenderTarget->RenderTargetSurfaceRHI, GSceneRenderTargets.GetSceneDepthSurface() );
			}
		...
		extern FLOAT GRHIDebug[20];
		if (GRHIDebug[5])
		{debugf(TEXT("Dumping shadow stencil"));}
		if (GRHIDebug[5])
			DumpRenderTarget( GPS3RHIDepthTarget, 1 );
#endif // USE_PS3_RHI
#endif // PS3
	...
	else
	{
		...
		if( bEnableVSMShadows )
		{
			...
			if (ShouldUseBranchingPCF(LightSceneInfo->ShadowProjectionTechnique))
			{
				...
				FShader* BranchingPCFPixelShader = SetBranchingPCFParameters(Context,View,this,LightSceneInfo->ShadowProjectionTechnique);

				FGlobalBoundShaderStateRHIRef* CurrentBPCFBoundShaderState = ChooseBPCFBoundShaderState(LightSceneInfo->ShadowProjectionTechnique, &BranchingPCFLowQualityBoundShaderState, 
			...
			{
				FShadowProjectionPixelShaderInterface * PixelShader = GetProjPixelShaderRef();
	...
	else
	{
		...
		for(INT MeshIndex = 0;MeshIndex < PrimitiveSceneInfo->StaticMeshes.Num();MeshIndex++)
		{
			...
			SubjectMeshElements.AddMesh(
				...
				FShadowDepthDrawingPolicy(StaticMesh->VertexFactory,GetShadowDepthMaterial(StaticMesh->MaterialInstance),this)
				);
		}
	}
}
...
void FSceneRenderer::CreateProjectedShadow(const FLightPrimitiveInteraction* Interaction,TArray<FProjectedShadowInfo*>& OutPreShadows)
{
	...
	if(Interaction->GetDynamicShadowType() == DST_Projected && !PrimitiveSceneInfo->ShadowParent)
	{
		...
		{
			// Shadowing constants.
			static const FLOAT ShadowResolutionScale = 1.0f;
			static const FLOAT ShadowFadeExponent = 0.3f;
			...
			const FLOAT FadeAlpha = appPow(ShadowResolutionFraction,ShadowFadeExponent);
			...
			if(FadeAlpha > 0.0f)
			{
					...
					)
				{
					...
					{
						const FViewInfo& View = Views(ViewIndex);
						if(View.PrimitiveVisibilityMap(ShadowChild->Id))
						{
							bIsVisible = TRUE;
							break;
						}
...
void FSceneRenderer::InitDynamicShadows()
{
	...
	for(TSparseArray<FLightSceneInfo*>::TConstIterator LightIt(Scene->Lights);LightIt;++LightIt)
	{
		...
		const UBOOL bLightCastsProjectedShadows = LightSceneInfo->bCastDynamicShadow && LightSceneInfo->bProjectedShadows;
		...
		//<@ ava specific ; 2007. 9. 11 changmin
		// add cascaded shadow map
		extern UBOOL GUseCascadedShadow;
		if( LightSceneInfo->bUseCascadedShadowmap && GUseCascadedShadow )
		{
			Ava_CreateCascadedShadows( LightSceneInfo );
		}
		else
		// create project shadows for specific light( flags.... )
		//>@ ava
		if(bLightIsVisibleInAnyView && bLightCastsProjectedShadows)
		{
	...
	// Initialize the views' ShadowVisibilityMaps.
	for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
	{
		FViewInfo& View = Views(ViewIndex);
...
UBOOL FSceneRenderer::CheckForProjectedShadows( const FLightSceneInfo* LightSceneInfo, UINT DPGIndex )
{
	...
	for(INT ShadowIndex = 0;ShadowIndex < ProjectedShadows.Num();ShadowIndex++)
	{
		...
		if(ProjectedShadowInfo->LightSceneInfo == LightSceneInfo)
		{
			...
			{
				const FViewInfo& View = Views(ViewIndex);
				bShadowIsVisible |= ProjectedShadowInfo->GetDepthPriorityGroup(&View) == DPGIndex && View.ShadowVisibilityMap(ShadowIndex);
			}
...
{
	SCOPE_CYCLE_COUNTER(STAT_ShadowDrawTime);
	...
	while(NumShadowsRendered < Shadows.Num())
	{
		...
		// Render the shadow projections.
		{
			...
			UBOOL bResolveAfterEachShadow = LightSceneInfo->LightShadowMode == LightShadow_Modulate && !(GSupportsFPBlending||GIsLowEndHW);
			...
			for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
			{
				...
				for(INT ShadowIndex = 0;ShadowIndex < Shadows.Num();ShadowIndex++)
				{
					...
					if(ProjectedShadowInfo->bAllocated)
					{
						...
						if(ProjectedShadowInfo->GetDepthPriorityGroup(&View) == DPGIndex)
						{
							ProjectedShadowInfo->RenderProjection(GlobalContext, &View);
						}
					}
