...
void USkelControlLookAt::DrawSkelControl3D(const FSceneView* View, FPrimitiveDrawInterface* PDI, USkeletalMeshComponent* SkelComp, INT BoneIndex)
{
	...
	if( bEnableLimit && bShowLimit && SkelComp->SkeletalMesh )
	{
		...
		UMaterialInstance* LimitMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_JointLimitMaterial"), NULL, LOAD_None, NULL);

		DrawCone(PDI,ConeToWorld, MaxAngleRadians, MaxAngleRadians, 40, TRUE, FColor(64,255,64), LimitMaterial->GetInstanceInterface(FALSE), SDPG_World);
	}
}
...
void USkelControlFootPlacement::CalculateNewBoneTransforms(INT BoneIndex, USkeletalMeshComponent* SkelComp, TArray<FMatrix>& OutBoneTransforms)
{
	...
	FVector EndToePos = SkelComp->SpaceBases(BoneIndex).TransformFVector(ToePos);
	FVector WorldEndToePos = SkelComp->LocalToWorld.TransformFVector(EndPos);

	FVector WorldToeDelta = WorldEndToePos - WorldEndPos;
	...
	UBOOL bHit = SkelComp->LegLineCheck( WorldToeDelta + WorldRootPos, WorldToeDelta + CheckEndPos, HitLocation, HitNormal);

	FLOAT LegAdjust = 0.f;
	if(bHit)
	{
		// Find how much we are adjusting the foot up or down. Postive is down, negative is up.
		LegAdjust = ((HitLocation - WorldEndToePos) | LegDir);
