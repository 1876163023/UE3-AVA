/*=============================================================================
UberPostProcessEffect.cpp: combines DOF, bloom, material coloring, and
possibly town-mapping.
...
IMPLEMENT_SHADER_TYPE(,FUberPostProcessBlendPixelShader,TEXT("UberPostProcessBlendPixelShader"),TEXT("Main"),SF_Pixel,VER_FP_BLENDING_FALLBACK,0);
...
public:
	...
	*/
	UBOOL Render(FCommandContextRHI* Context, UINT InDepthPriorityGroup,FViewInfo& View,const FMatrix& CanvasTransform)
	{
		...
		{
			RHISetRenderTarget(Context,View.Family->RenderTarget->RenderTargetSurfaceRHI, FSurfaceRHIRef()); // Render to the final render target
		}
		else
		{
		...
		BlendPixelShader->GammaParameters.Set(
			Context,
			*BlendPixelShader,
			View.Family->RenderTarget->GetDisplayGamma(),
			View.ColorScale,
			View.OverlayColor
			);
		...
		if (!IsValidRef(UberBlendBoundShaderState))
		{
			DWORD Strides[MaxVertexElementCount];
			appMemzero(Strides, sizeof(Strides));
			Strides[0] = sizeof(FFilterVertex);

			UberBlendBoundShaderState = RHICreateBoundShaderState(GFilterVertexDeclaration.VertexDeclarationRHI, Strides, BlendVertexShader->GetVertexShader(), BlendPixelShader->GetPixelShader());
		}

		RHISetBoundShaderState(Context, UberBlendBoundShaderState);
		...
		if ( FinalEffectInGroup )
		{
			...
			FLOAT UVOffsetX = +0.5f * ScaleX + GPixelCenterOffset / static_cast<float>(TargetSizeX);
			FLOAT UVOffsetY = +0.5f * ScaleY + GPixelCenterOffset / static_cast<float>(TargetSizeY);
			...
			UVOffsetX -= (View.X - View.RenderTargetX) / TargetSizeX;
			UVOffsetY -= (View.Y - View.RenderTargetY) / TargetSizeY;
...
protected:
	...
	static FBoundShaderStateRHIRef UberBlendBoundShaderState;
};

FBoundShaderStateRHIRef FUberPostProcessSceneProxy::UberBlendBoundShaderState;
