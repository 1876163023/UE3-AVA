...
APrefabInstance* AActor::FindOwningPrefabInstance() const
{
	...
	if( Cast<APrefabInstance>(this) )
	{
		return (APrefabInstance*)this;
	}
...
void APrefabInstance::InstanceKismetSequence(USequence* SrcSequence, const FString& InSeqName)
{
	...
	if(RootSeq)
	{
		...
		SequenceInstance = CastChecked<USequence>( StaticDuplicateObject(SrcSequence, SrcSequence, PrefabsSequence, *SeqName) );
		SequenceInstance->ObjName = SequenceInstance->GetName();

		// Turn archetype refs into actual object refs.
		FArchiveReplaceObjectRef<UObject> ReplaceAr(SequenceInstance, ArchetypeToInstanceMap, TRUE, TRUE, TRUE);
...
void APrefabInstance::DestroyKismetSequence()
{
	...
	checkf(PrefabSequenceContainer->ObjName == FString(TEXT("Prefabs")), TEXT("SequenceInstance::%s -- %s"), *SequenceInstance->GetFullName(), *GetFullName() ); // Should always be in the 'Prefabs' subsequence.
...
void APrefabInstance::InstancePrefab(UPrefab* InPrefab)
{
	...
	for(INT i=0; i<TemplatePrefab->PrefabArchetypes.Num(); i++)
	{
		...
		if(Archetype->IsA(AActor::StaticClass()))
		{
			...
			NewObj = GWorld->SpawnActor(Archetype->GetClass(), NAME_None, WorldTM.GetOrigin(), WorldTM.Rotator(), ArcActor, TRUE);
	...
	UPrefab::ResolveInterDependencies(ArchetypeToInstanceMap, false);
	...
	for ( TMap<UObject*,UObject*>::TIterator InstIt(ArchetypeToInstanceMap); InstIt; ++InstIt )
	{
		...
		if(ObjInst)
		{
			...
			if(ActorInst)
			{
				...
				// @todo: Confirm that because Rotators use integers, we don't need to do the same snapping for them.
			}
...
UBOOL APrefabInstance::VerifyMemberArchetypes()
{
	...
	for ( TMap<UObject*,UObject*>::TIterator It(ArchetypeToInstanceMap); It; ++It )
	{
		...
		if ( It.Key() == NULL )
		{
			...
			warnf(NAME_Warning, *LocalizeUnrealEd(TEXT("Prefab_MissingArchetypes")), *MemberInstance->GetFullName(), *GetPathName());
...
void UPrefab::CopySequenceIntoPrefab(USequence* InPrefabSeq, TMap<UObject*,UObject*>& InstanceToArchetypeMap)
{
	...
	PrefabSequence = CastChecked<USequence>( StaticDuplicateObject(InPrefabSeq, InPrefabSeq, this, TEXT("None")) );
	...
	FArchiveReplaceObjectRef<UObject> ReplaceAr(PrefabSequence, InstanceToArchetypeMap, true, true, true);
}
