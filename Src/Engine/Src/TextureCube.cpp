	...
	else
	{
		...
		for( INT FaceIndex=0; FaceIndex<6; FaceIndex++ )
		{
			...
			if( FaceTexture )
			{
				...
				// bug fix : Mip이 없는 texture에 lod bias을 적용해서 texture가 제대로 나오지 않는 경우가 있었다.(ID : 001362)
				if( !FaceTexture->CompressionNoMipmaps )
				//>@ ava
					NumMips = Clamp(FaceTexture->Mips.Num() - GSystemSettings->TextureLODSettings.CalculateLODBias(FaceTexture), 0, NumMips);
 ...
 */
FString UTextureCube::GetDetailedDescription( INT Index )
{
		...
		for( INT FaceIndex=0; FaceIndex<6; FaceIndex++ )
		{
			...
			if( FaceTexture )
			{
				...
				for( INT MipIndex=0; MipIndex<Owner->NumMips; MipIndex++ )
				{
					...
					{
						UINT DestStride;
						void* MipData = RHILockTextureCubeFace( TextureCube, FaceIndex, MipIndex, TRUE, DestStride );
						GetData( FaceIndex, MipIndex+FirstMip, MipData );
...
private:
	 ...
	 */
	void GetData( INT FaceIndex, INT MipIndex, void* Dest )
	{
