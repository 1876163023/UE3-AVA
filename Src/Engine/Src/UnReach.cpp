...
{
	if (End->bBlocked || (End->bBlockedForVehicles && P->IsA(AVehicle::StaticClass())))
	{
		return TRUE;
	}
...
//for superville to look at later
//		FVector CollisionSize = P ? P->GetDefaultCollisionSize() : FVector(CollisionRadius, CollisionRadius, CollisionHeight);
//			!BlockedBy->ActorLineCheck(Hit, End->Location, Start->Location, CollisionSize, TRACE_Pawns | TRACE_Others | TRACE_Blocking) )
...
//		return appTrunc(Distance * SWIMCOSTMULTIPLIER) + End->Cost;
	...
	if (EndNav != NULL)
	{
		...
		if (EndNav->LastAnchoredPawnTime > 0.f)
		{
			...
			//debugf(TEXT("-! %s had anchored pawn %2.3f seconds ago (%2.3f)"),*EndNav->GetName(),Delta,4.f - (3.f * (Delta/5.f)));
			// if it has been less than 5 seconds since something anchored here
			if (Delta <= 5.f)
			{
				// scale based on that duration
				Cost = appTrunc(Cost * 4.f - (3.f * (Delta/5.f)));
			}
			else
			{
		...
		for (FCheckResult* CheckHit = Hits; CheckHit != NULL && !bResult; CheckHit = CheckHit->GetNext())
		{
				...
				(Dir | (CheckHit->Actor->Location - Start->Location).SafeNormal()) > KINDA_SMALL_NUMBER )
			{
				...
				if (CheckHit->Actor->GetComponentsBoundingBox(FALSE).IsInside(End->Location))
				{
					...
					{
						// PlayerPawn이 End영역에 들어있어서 장애물로 취급되는 경우이다.
//						debugf(TEXT("Blocked By Actor(%s), RouteGoal(%s), End(%s), RouteCache.Num()=%d"), 
//							*CheckHit->Actor->GetName(), *C->RouteGoal->GetName(), *End->GetName(), C->RouteCache.Num());

						if (!C->eventHandlePathObstruction(CheckHit->Actor))
						{
							debugf(NAME_DevPath, TEXT("AI goal %s obstructed by bBlocksNavigation Actor %s"), *End->GetName(), *CheckHit->Actor->GetName());
							C->MoveTimer = -1.f;
