...
ATerrain* ATerrain::SplitTerrain( UBOOL SplitOnXAxis, INT RemainingPatches )
{
	...
	NewTerrain->CollisionTesselationLevel	= CollisionTesselationLevel;
...
void ATerrain::PostEditChange(UProperty* PropertyThatChanged)
{
	...
	CollisionTesselationLevel = Clamp(CollisionTesselationLevel,1,MaxTesselationLevel);
...
#if 0//CONSOLE
	// Not supporting dynamic buffers on consoles at the moment...
	MinTessellationLevel = MaxTesselationLevel;
#else	//#if CONSOLE
...
void ATerrain::BeginDestroy()
{
	...
	for (INT WeightMapIndex = 0; WeightMapIndex < WeightedMaterials.Num(); WeightMapIndex += 4)
	{
...
void ATerrain::TouchWeightMapResources()
{
	...
	for (WeightMapIndex = 0; WeightMapIndex < WeightedMaterials.Num(); WeightMapIndex += 4)
	{
		...
		if (TextureIndex >= WeightedTextureMaps.Num())
		{
			check(!TEXT("ATerrain::TouchWeightMapResources> INVALID TEXTURE"));
		}
		else
		{
			Texture = WeightedTextureMaps(TextureIndex);
		...
		INT ii = WeightMapIndex + TextureIndex;
	}
}
		...
		for(INT DecorationIndex = 0; DecorationIndex < DecoLayers(DecoLayerIndex).Decorations.Num(); DecorationIndex++)
		{
			...
			for (INT InstanceIndex = 0; InstanceIndex < Decoration.Instances.Num(); InstanceIndex++)
			{
				...
				if (IsTerrainQuadVisible(IntX, IntY) == TRUE)
				{
					...
					DecorationInstance.Component->BeginDeferredUpdateTransform();
				...
				for(UINT InstanceIndex = 0;InstanceIndex < MaxInstances;InstanceIndex++)
				{
					...
					if (IsTerrainQuadVisible(IntX, IntY) == TRUE)
					{
							...
							(IntX >= MinX) && (IntX <= MaxX) && (IntY >= MinY) && (IntY <= MaxY))
						{
							...
							AttachComponent(DecorationInstance->Component);
						}
					}
				}
			}
 ...
 */
void ATerrain::UpdateMaterialInstance(UMaterialInstance* InMatInst)
{
}
					...
					if (TerrainMaterial->Material == InMat)
					{
						for (INT CachedIndex = 0; CachedIndex < CachedMaterialCount; CachedIndex++)
						{
							...
							if (CachedMaterial)
							{
								...
								for (INT MaterialIndex = 0; MaterialIndex < CheckMask.Num(); MaterialIndex++)
								{
									if (CheckMask.Get(MaterialIndex))
									{
										if (MaterialIndex < WeightedMaterials.Num())
										{
											FTerrainWeightedMaterial* CheckWeightedMaterial = &(WeightedMaterials(MaterialIndex));
...
UBOOL ATerrain::TessellateTerrainUp(INT InTessellationlevel, UBOOL bRegenerateComponents)
{
		...
		(NewNumVerticesY != NumVerticesY))
	{
		...
		CollisionTesselationLevel = NewCollisionTesselationLevel;
 ...
 */
UBOOL ATerrain::GetClosestVertex(const FVector& InLocation, FVector& OutVertex)
{
...
void UTerrainComponent::Serialize( FArchive& Ar )
{
	...
	{
		Ar << CollisionVertices << BoxCheckTree;
	}
										 ...
										 TArray<INT>& Indices,const FVector& ScaleFactor)
{
	...
	// Make sure that the collision setting is valid
	Terrain->CollisionTesselationLevel = Clamp(Terrain->CollisionTesselationLevel,1,Terrain->MaxTesselationLevel);
	...
	INT StepSize = Terrain->MaxTesselationLevel / Terrain->CollisionTesselationLevel;
	...
	for(INT QuadY = 0; QuadY < NumVertsY - 1; QuadY++)
	{
		...
		{
			INT GlobalY = SectionBaseY + (QuadY * StepSize);
			INT	GlobalX = SectionBaseX + (QuadX * StepSize);
			if (Terrain->IsTerrainQuadVisible(GlobalX, GlobalY) == FALSE)
			{
				continue;
			}
...
void UTerrainComponent::BuildCollisionData(void)
{
	...
	// From this we can build our collision triangle list
	TArray<FkDOPBuildCollisionTriangle> kDOPBuildTriangles;
	// Presize the array
	kDOPBuildTriangles.Empty(Indices.Num() / 3);
	// Loop through and convert from indices to build triangle
	for (INT Index = 0; Index < Indices.Num(); Index += 3)
	{
		new (kDOPBuildTriangles) FkDOPBuildCollisionTriangle(
			// Indices and material index
			Indices(Index + 0),Indices(Index + 2),Indices(Index + 1),0,
			// Vertices...
			CollisionVertices(Indices(Index + 0)),CollisionVertices(Indices(Index + 2)),CollisionVertices(Indices(Index + 1)));
	}
	// Now populate the kDOP with the data
	BoxCheckTree.Build(kDOPBuildTriangles);
}
...
{
	/**
	 * The terrain that is being collided against
	 */
	ATerrain* Terrain;
	/**
	 * The component that is being collided against
	 */
...
public:
	/**
	 * Sets the component and terrain members
	 */
	...
	{
		//@todo  Support per triangle material information
		return NULL;
	}

	/**
	 * Returns the kDOPTree for this terrain component
	 */
	FORCEINLINE const TkDOPTree<FTerrainCollisionDataProvider>& GetkDOPTree(void) const
	{
		return Component->BoxCheckTree;
	}

	/**
	 * Returns the local to world for the terrain
	 */
	...
	/**
	 * Returns the world to local for the terrain
	 */
	...
	/**
	 * Returns the world to local transpose adjoint for the terrain
	 */
	...
	/**
	 * Returns the determinant for the terrain
	 */
...
{
	UBOOL Hit = FALSE;
	if (BoxCheckTree.Nodes.Num())
	{ 
		FTerrainCollisionDataProvider Provider(this);
		// Create the check structure with all the local space fun
		TkDOPPointCollisionCheck<FTerrainCollisionDataProvider> kDOPCheck(InPoint,InExtent,Provider,&Result);
		// Check for a hit
		Hit = BoxCheckTree.PointCheck(kDOPCheck);
		if (Hit == TRUE)
		{
			// Transform the hit normal and location to world space if there was a hit
...
UBOOL UTerrainComponent::LineCheck(FCheckResult& Result,const FVector& End,const FVector& Start,const FVector& Extent,DWORD TraceFlags)
{
	...
	{
#if	!CONSOLE
		UBOOL bUseInGameCollision = GWorld->HasBegunPlay();
#else
		// No need for the function call cost on console
		UBOOL bUseInGameCollision = TRUE;
#endif
		// Use the low poly for the majority of the traces and the high poly
		// only when specified by the TRACE_Tesselation flag
		// NOTE: Always use the slow method in the editor
		if (bUseInGameCollision && (TraceFlags & TRACE_Tesselation) == 0 && BoxCheckTree.Nodes.Num() > 0)
		{
			FTerrainCollisionDataProvider Provider(this);
			TkDOPLineCollisionCheck<FTerrainCollisionDataProvider> kDOPCheck(Start,End,TraceFlags,Provider,&Result);
			// Do a line check against the BoxCheck low poly mesh data
			Hit = BoxCheckTree.LineCheck(kDOPCheck);
			if (Hit == TRUE)
			{
		...
		else
		{
			...
			for(INT PatchY = MinY;PatchY <= MaxY;PatchY++)
			{
				...
				for (INT PatchX = MinX; PatchX <= MaxX; PatchX++)
				{
					...
					const FTerrainPatchBounds&	PatchBound = PatchBounds((PatchY - SectionBaseY) * TrueSectionSizeX + (PatchX - SectionBaseX));
					...
					for(INT SubY = 0;SubY < TesselationLevel;SubY++)
					{
						...
						for(INT SubX = 0;SubX < TesselationLevel;SubX++)
						{
							...
							UBOOL			Hit = 0;
								...
								Hit |= LineCheckWithTriangle(Result,V00,V01,V11,LocalStart,LocalEnd,LocalDirection);
								Hit |= LineCheckWithTriangle(Result,V11,V10,V00,LocalStart,LocalEnd,LocalDirection);
	...
	{
		if (BoxCheckTree.Nodes.Num() > 0)
		{
			FTerrainCollisionDataProvider Provider(this);
			TkDOPBoxCollisionCheck<FTerrainCollisionDataProvider> kDOPCheck(Start,End,Extent,TraceFlags,Provider,&Result);
			Hit = BoxCheckTree.BoxCheck(kDOPCheck);
			if (Hit == TRUE)
			{
				// Transform the hit normal to world space if there was a hit
				// This is deferred until now because multiple triangles might get
...
void UTerrainComponent::UpdateBounds()
{
	...
	if( PatchBounds.Num() != TrueSectionSizeX * TrueSectionSizeY )
	{
		UpdatePatchBounds();
	}
	...
	for(INT Y = 0;Y < TrueSectionSizeY;Y++)
	{
		...
		{
			const FTerrainPatchBounds&	Patch = PatchBounds(Y * TrueSectionSizeX + X);
			BoundingBox += FBox(
				FVector(X - Patch.MaxDisplacement,Y - Patch.MaxDisplacement,Patch.MinHeight),
				FVector(
					X + 1 + Patch.MaxDisplacement,Y + 1 + Patch.MaxDisplacement,
					Patch.MaxHeight
					)
				);
		}
...
void UTerrainComponent::UpdatePatchBounds()
{
	...
	PatchBounds.Empty(TrueSectionSizeX * TrueSectionSizeY);
		...
		for (INT X = 0; X < TrueSectionSizeX; X++)
		{
			...
			for (INT SubY = 0; SubY <= 1; SubY++)
			{
				for (INT SubX = 0; SubX <= 1; SubX++)
				{
					...
					const FTerrainPatch& Patch = Terrain->GetPatch(SectionBaseX + X + SubX,SectionBaseY + Y + SubY);
...
public:
	...
	virtual UBOOL DoesLineSegmentIntersect(const FVector& Start,const FVector& End) const
	{
		...
		return !Primitive->LineCheck(Result,End,Start,FVector(0,0,0),TRACE_ShadowCast | TRACE_StopAtAnyHit);
	}
	...
	virtual void Apply(FLightMapData2D* LightMapData,const TMap<ULightComponent*,FShadowMapData2D*>& ShadowMapData)
	{
		...
		Primitive->LightMap = FLightMap2D::AllocateLightMap(
			Primitive,
			LightMapData,
			LightMapData,	// ava specific ; compile 되도록 수정. 실행되지 않는 코드임.
			NULL,
			Primitive->Bounds,
			FALSE
			);
		...
		for(INT LightIndex = 0;LightIndex < RelevantLights.Num();LightIndex++)
		{
			...
			for(INT LightIndex = 0;LightIndex < Primitive->ShadowMaps.Num();LightIndex++)
			{
				if(Primitive->ShadowMaps(LightIndex)->GetLightGuid() == Light->LightGuid)
				{
					bIsInShadowMap = TRUE;
					break;
				}
 ...
 */
FString UTerrainWeightMapTexture::GetDetailedDescription( INT Index )
{
