...
void DSWaitForCondition::Process(UBOOL bOptional) 
{
	...
	if ( !Debugger->IsClosing && Debugger->CallStack->StackDepth && Node->GetLine() > 0 && EvaluateCondition(bOptional) )
	{
		...
		else if ( !GIsRequestingExit || GIsUCC )
		{
			...
			if ( !CallStack->UpdateStack(Debugee, CurrentFrame, LineNumber, InputPos, OpCode) )
			{
				if ( CallStack->StackDepth > 0 )
				{
					...
					if ( IsDebugging && CurrentStackPosition != CallStack->Stack.Num() - 1 )
					{
...
#else
				appErrorf(NAME_FriendlyError,TEXT("Received duplicate NEWSTACK frame.  Verify that all packages have been compiled in debug mode."));
#endif
			}
...
#else
				appErrorf(NAME_FriendlyError,TEXT("Received LATENTNEWSTACK with stack depth.  Verify that all packages have been compiled in debug mode."));
#endif
			}
...
#else
		appErrorf(NAME_FriendlyError,TEXT("Received call to UpdateStack with stack out of sync.  Verify that all packages have been compiled in debug mode."));
#endif
	}
