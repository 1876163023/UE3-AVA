...
INT UBatchExportCommandlet::Main( const FString& Params )
{
	...
	if ( !Package )
	{
		appErrorf( *LocalizeUnrealEd("Error_FailedToLoadPackage"), *Pkg );
	}

	if( !GFileManager->MakeDirectory( *Path, 1 ) )
	{
		warnf( NAME_Warning, *LocalizeUnrealEd("Error_FailedToMakeDirectory"), *Path );
		return 1;
	}
	...
	for( TObjectIterator<UObject> It; It; ++It )
	{
		if( It->IsA(Class) && It->IsIn(Package) )
		{
			...
			{
				appErrorf(*LocalizeUnrealEd("Error_CantExportNameToFile"), *It->GetFullName(),*Filename);
			}
		}
	}
	GIsRequestingExit=1;
