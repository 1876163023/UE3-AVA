...
#define SET_ERROR( ErrorType ) { if( Error ) *Error = ErrorType; }
	...
	EMorphImportError* Error )
{
	...
	else
	{
		...
		{
			SET_ERROR( MorphImport_CantLoadFile );
		}
		else
		{
			// null terminate it
			...
			// (This is pretty ghetto, but saves from duplicating a lot of code for importing psk files. 
			// It's a temp solution since we will load from Collada files eventually anyway).	
			USkeletalMeshFactory* SkelMeshFact = new USkeletalMeshFactory();
			USkeletalMesh* TargetSkelMesh = (USkeletalMesh*)SkelMeshFact->FactoryCreateBinary( 
			...
			if( !TargetSkelMesh )
			{
				SET_ERROR( MorphImport_InvalidMeshFormat );
			}
			else
			{
				...
				{
					SET_ERROR( MorphImport_MismatchBaseMesh );                
				}
				else
				{
					if( bReplaceExisting )
...
void FMorphTargetBinaryImport::ImportMorphLODModel( UMorphTarget* MorphTarget, const TCHAR* SrcFilename, INT LODIndex, EMorphImportError* Error )
{
	...
	{
		SET_ERROR( MorphImport_InvalidLODIndex );
	}
	// check for valid meta data for the base mesh
	else if( BaseMeshRawData.Indices.Num() && !BaseMeshRawData.WedgePointIndices.Num() )
	{
