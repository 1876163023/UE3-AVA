...
{
							new(GetClass(), TEXT("FRScriptOutputPath"),		RF_Public) UStrProperty		( CPP_PROPERTY(FRScriptOutputPath),		TEXT("Advanced"), CPF_Config );
	...
	UArrayProperty* A	=	new(GetClass(), TEXT("EditPackages"),			RF_Public) UArrayProperty	( CPP_PROPERTY(EditPackages),			TEXT("Advanced"), CPF_Config|CPF_GlobalConfig );
...
void UEditorEngine::Init()
{
	...
	for( INT i=0; i<EditPackages.Num(); i++ )
		if( !LoadPackage( NULL, *EditPackages(i), LOAD_NoWarn ) )
				appErrorf( *LocalizeUnrealEd("Error_CantFindEditPackage"), *EditPackages(i) );
	EndLoad();
...
void UEditorEngine::Tick( FLOAT DeltaSeconds )
{
	...
	for( INT i=0; i<ViewportClients.Num(); i++ )
	{
		...
		if(ViewportClients(i)->bForceAudioUpdate)
		{
...
//#if defined(WITH_FACEFX) && defined(FX_TRACK_MEMORY_STATS)
	...
	if( Client && Client->GetAudioDevice() )
	{
		...
		// Update audio device.
		Client->GetAudioDevice()->Update( bForceAudioUpdate || IsRealtime || (PlayWorld && !PlayWorld->IsPaused()) );
...
void UEditorEngine::SetObjectPropagationDestination(INT Destination)
{
	...
	else
	{
		...
		else
		{
			...
			GObjectPropagator->SetIPAddress(Console->GetIPAddress(), Console->GetIntelByteOrder());
		}
	}
}
...
void UEditorEngine::UpdatePrefabs()
{
	...
	for( FActorIterator It; It; ++It )
	{
		...
		if(	PrefabInst && PrefabInst->TemplatePrefab &&
			!PrefabInst->bDeleteMe && !PrefabInst->IsPendingKill() )
		{
			...
			else if ( PrefabInst->TemplateVersion > SourcePrefab->PrefabVersion )
			{
				...
				PrefabInst->DestroyPrefab(GetSelectedActors());
		...
		if (StreamingBoundsTexture != NULL)
		{
			...
			for(UINT ViewportIndex = 0; ViewportIndex < (UINT)ViewportClients.Num(); ViewportIndex++)
			{
				...
				if (ViewportClient)
				{
					...
					ViewportClient->RedrawRequested( NULL );
				}
			}
		}
		else
 ...
 */
UBOOL GIsPlayInEditorWorld_RenderThread = FALSE;
...
UWorld* SetPlayInEditorWorld( UWorld* PlayInEditorWorld )
{
	...
	ENQUEUE_UNIQUE_RENDER_COMMAND(
		FSetPlayInEditorWorld,		
	{
		GIsPlayInEditorWorld_RenderThread = TRUE;
	});
...
void RestoreEditorWorld( UWorld* EditorWorld )
{
	...
	ENQUEUE_UNIQUE_RENDER_COMMAND(
		FSetPlayInEditorWorld,		
	{
		GIsPlayInEditorWorld_RenderThread = FALSE;
	});
}
...
void CookSimpleWave( USoundNodeWave* SoundNodeWave, FConsoleSoundCooker* SoundCooker, FByteBulkData& DestinationData )
{
	...
	// If it is not a WAVE file format,
	if( !ReadWaveHeader( WaveInfo, RawWaveData, SoundNodeWave->RawData.GetBulkDataSize(), 0 ) )
	{		
		extern FLOAT Audio_GetDuration( const BYTE* Bytes, INT NumBytes );
		FLOAT Duration = Audio_GetDuration( RawWaveData, SoundNodeWave->RawData.GetBulkDataSize() );

		if (Duration > 0.0f)
		{
			// Make place for cooked data.
			DestinationData.Lock( LOCK_READ_WRITE );
			BYTE * RawCompressedData = ( BYTE * )DestinationData.Realloc( SoundNodeWave->RawData.GetBulkDataSize() );

			// Retrieve cooked data.
			appMemcpy( RawCompressedData, RawWaveData, SoundNodeWave->RawData.GetBulkDataSize() );

			DestinationData.Unlock();		

			SoundNodeWave->SampleRate = 44100;
			SoundNodeWave->NumChannels = 2;
			SoundNodeWave->SampleDataSize = SoundNodeWave->RawData.GetBulkDataSize();
			SoundNodeWave->Duration = Duration;
	...
	// Cook the data.
	if( SoundCooker->Cook( ( short * )WaveInfo.SampleDataStart, WaveInfo.SampleDataSize, &WaveFormat, SoundNodeWave->CompressionQuality, SoundNodeWave->bCompressedSample, RawWaveData, SoundNodeWave->RawData.GetBulkDataSize() ) == TRUE ) 
	{
		// Make place for cooked data.
		DestinationData.Lock( LOCK_READ_WRITE );
 ...
 */
void CookSoundNodeWave( USoundNodeWave* SoundNodeWave, FConsoleSoundCooker* SoundCooker, FByteBulkData& DestinationData )
{
	check( SoundCooker );
 ...
 * @param	SoundNodeWave			Wave file to cook
 */
void CookSoundNodeWave( USoundNodeWave* SoundNodeWave, UE3::EPlatformType Platform )
{
	if( Platform == UE3::PLATFORM_Unknown || Platform == UE3::PLATFORM_Xenon )
	{
		...
		if( XenonSupport )
		{
			CookSoundNodeWave( SoundNodeWave, XenonSupport->GetGlobalSoundCooker(), SoundNodeWave->CompressedXbox360Data );
		}
	}
	...
	if( Platform == UE3::PLATFORM_Unknown || Platform == UE3::PLATFORM_PS3 )
	{
		...
		if( PS3Support )
		{
			CookSoundNodeWave( SoundNodeWave, PS3Support->GetGlobalSoundCooker(), SoundNodeWave->CompressedPS3Data );
		}
	}

	if( Platform == UE3::PLATFORM_Unknown || Platform == UE3::PLATFORM_Windows )
	{
		CookSoundNodeWave( SoundNodeWave, GetPCSoundCooker(), SoundNodeWave->CompressedPCData );
	}
}
...
void SoundNodeWaveQualityPreview( USoundNodeWave* SoundNode, FPreviewInfo* PreviewInfo )
{
	...
	{
		debugfSlow( NAME_DevAudio, TEXT( "PC decompression failed" ) );
	}
	...
	if( XenonSupport )
	{
		...
		{
			debugfSlow( NAME_DevAudio, TEXT( "Xenon decompression failed" ) );
		}
	}
	...
	if( PS3Support )
	{
		...
		{
			debugfSlow( NAME_DevAudio, TEXT( "PS3 decompression failed" ) );
		}
	}
	// PCF End
}
