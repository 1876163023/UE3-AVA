	...
	)
{
	...
	INT StrLineLength = appStrlen(SourceText);
	...
	if (GEditor->IsImportingT3D)
	{
		PortFlags |= PPF_AttemptNonQualifiedSearch;
	}
	if ( GIsUCCMake )
	{
		PortFlags |= PPF_RestrictImportTypes;
	}
	...
	while( ParseLine( &SourceText, StrLine, StrLineLength, TRUE ) )
	{
		const TCHAR* Str = StrLine;
		...
		else if( GetBEGIN(&Str,TEXT("Object")))
		{
			...
			if( TemplateClass->IsChildOf(UComponent::StaticClass()) && SubobjectOuter->HasAnyFlags(RF_ClassDefaultObject))
			{
				...
				if(OverrideComponent)
				{
					...
					check(OverrideComponent==BaseTemplate);
				}
			...
			else
			{
				...
				if (Parse(Str, TEXT("Archetype="), ArchetypeName))
				{
					...
					if (ArchetypeClass)
					{
						...
						// @fixme subobjects: this _may_ need StaticLoadObject, but there is currently a bug in StaticLoadObject that it can't take a non-package pathname properly
						Archetype = UObject::StaticFindObject(ArchetypeClass, ANY_PACKAGE, *Refs(1));
//						Archetype = UObject::StaticLoadObject(ArchetypeClass, ANY_PACKAGE, *Refs(1), NULL, 0, NULL);
					}
				}
				...
				if ( Archetype != NULL && !Archetype->HasAnyFlags(RF_ClassDefaultObject) && TemplateObject->IsA(UComponent::StaticClass()) )
				{
		...
		else
		{
			...
			if( *Str )
			{
				...
				if(*Str == '.')
				{
					...
					else if (ParseCommand(&Str,TEXT("Sort")))
					{
						ArrayOp = ADO_Sort;
					}
				}
				...
				UObject* ImportTextParent = GEditor->Bootstrapping 
					? ObjectStruct->IsA(UClass::StaticClass()) 
						? Cast<UClass>(ObjectStruct)->GetDefaultObject()
						: ObjectStruct
					: (UObject*)DestData;
				...
				if( !Property )
				{
					...
					if( !Property )
					{
						Warn->Logf( NAME_Warning, TEXT("Unknown property in defaults: %s"), StrLine);
						continue;
					}
				}

				if ( !GIsUCCMake && (Property->PropertyFlags&(CPF_NoImport|CPF_DuplicateTransient)) != 0 )
				{
				...
				if ( ArrayOp != ADO_None )
				{
					...
					if (GEditor->Bootstrapping && ArrayProperty->Inner->IsA(UComponentProperty::StaticClass()))
					{
					...
					if(ArrayOp == ADO_Empty)
					{
						...
						Array->Empty(ArrayProperty->Inner->ElementSize, DEFAULT_ALIGNMENT);
					}
					else if( ArrayOp == ADO_Add || ArrayOp == ADO_Remove || ArrayOp == ADO_Replace )
					{
						FArray*	Array = (FArray*)(DestData + Property->Offset);
						...
						if(ArrayOp == ADO_Add)
						{
							...
							INT	Index = Array->AddZeroed(1, Size, DEFAULT_ALIGNMENT);
							...
							if ( Result == NULL || Result == Str )
							{
								...
								Array->Remove(Index, 1, Size, DEFAULT_ALIGNMENT);
							}
						}
							...
							else
							{
								...
								for(UINT Index = 0;Index < (UINT)Array->Num();Index++)
								{
									...
									if(ArrayProperty->Inner->Identical(Temp,DestData))
									{
										...
										Array->Remove(Index--,1,ArrayProperty->Inner->ElementSize, DEFAULT_ALIGNMENT);
									}
								}
							}
						...
						else if ( ArrayOp == ADO_Replace )
						{
							const TCHAR* CompleteValue = Str;

							INT Size = ArrayProperty->Inner->ElementSize;

							BYTE* SearchValue = new BYTE[Size];
							BYTE* ReplaceValue = new BYTE[Size];

							appMemzero(SearchValue, Size);
							appMemzero(ReplaceValue, Size);

							TCHAR* CommaPos = appStrchr(Str,TCHAR(','));
							if ( CommaPos != NULL )
							{
								// export the value specified to a temporary buffer
								const TCHAR* SearchResult = ArrayProperty->Inner->ImportText(Str,SearchValue,PortFlags,ImportTextParent);
								if ( SearchResult == NULL || SearchResult == Str )
								{
									Warn->Logf(NAME_Warning, TEXT("Unable to parse parameter value '%s' in defaultproperties array operation: %s"), Str, StrLine);
								}
								else
								{
									Str = CommaPos + 1;
									const TCHAR* ReplaceResult = ArrayProperty->Inner->ImportText(Str,ReplaceValue,PortFlags,ImportTextParent);
									if ( ReplaceResult == NULL || ReplaceResult == Str )
									{
										Warn->Logf(NAME_Warning, TEXT("Unable to parse parameter value '%s' in defaultproperties array operation: %s"), Str, StrLine);
									}
									else
									{
										...
										{
											BYTE* ElementData = (BYTE*)Array->GetData() + Index * Size;
											if(ArrayProperty->Inner->Identical(SearchValue,ElementData))
											{
												if ( RemovedComponents && bIsComponentsArray )
												{
												...
												bFound = TRUE;
												ArrayProperty->Inner->CopyCompleteValue(ElementData, ReplaceValue);
											}
										}

										if ( !bFound )
										{
											FString SearchText = CompleteValue;
											INT pos = SearchText.InStr(TEXT(","));
											if ( pos != INDEX_NONE )
												SearchText = SearchText.Left(pos);

											Warn->Logf(NAME_Error, TEXT("No values matching '%s' in array '%s': %s"), *SearchText, *ArrayProperty->GetName(), StrLine);
										}
									}
								}
							}
							...
							{
								Warn->Logf(NAME_Error, TEXT("Syntax for Replace operation is .Replace(CurrentValue,ReplacementValue): %s"), StrLine);
							}

							ArrayProperty->Inner->DestroyValue(SearchValue);
							ArrayProperty->Inner->DestroyValue(ReplaceValue);
							delete SearchValue;
							delete ReplaceValue;
						}
					}
					else if (ArrayOp == ADO_RemoveIndex)
					{
						...
						Array->Remove(removeIdx,1,ArrayProperty->Inner->ElementSize, DEFAULT_ALIGNMENT);
					}
					//  [10/9/2006 otterrrr], Sort Default Properties by the KeyElement of StructProperty.
					else if (ArrayOp == ADO_Sort)
					{
						FArray* Array = (FArray*)(DestData + Property->Offset);
						...
						if(!StructProperty)
						{
							Warn->Logf( NAME_ExecWarning, TEXT("Syntax for Replace operation is .Sort(KeyName of StructProperty): %s"), StrLine );
							continue;
						}
						...
						FName GotName( *StrKey, FNAME_Find, TRUE );
						if( GotName == NAME_None )
						{
							Warn->Logf( NAME_ExecWarning, TEXT("No field matching '%s' in struct '%s' : %s"),*GotName.GetName(),*StructProperty->GetName(),StrLine);
							continue;							
						}

						UBOOL bMatch = FALSE;
						UBOOL bIsStrKey = FALSE;
						INT nKeyOffset = 0;
						for( TFieldIterator<UProperty, CLASS_IsAUProperty> It(StructProperty->Struct); It; ++It )
						{
							...
							{
								bMatch = TRUE;
								bIsStrKey = Cast<UStrProperty>(Property) ? TRUE : FALSE;
								nKeyOffset = It->Offset;
								break;
							}
						}
						if(!bMatch)
						...
						if(!bIsStrKey)
						{
							Warn->Logf( NAME_ExecWarning, TEXT("Only StringType can be a SearchKey of Sort : %s"),StrLine);
							continue;
						}

						// Shell Sort.
						INT Incr = Array->Num()/2;
						while( Incr > 0)
						{
							for( INT i = 0 ; i < Array->Num() ; i++)
							{
								INT Curr = i;
								BYTE* PropData = (BYTE*)Array->GetData() + Curr * ElementSize;
								BYTE* DescData = (BYTE*)Array->GetData() + (Curr - Incr) * ElementSize;
								while( Curr >= Incr &&  *(FString*)(DescData + nKeyOffset) > *(FString*)(PropData + nKeyOffset) )
								{
									// a temporal copy is not allowed because of some constraints
									Array->Swap( Curr ,Curr - Incr ,ElementSize );
									PropData -= Incr*ElementSize;
									DescData -= Incr*ElementSize;
									Curr -= Incr;
								}
							}
							Incr /= 2;
				...
				else
				{
					...
					if (*Str == '{')
					{
						...
						do
						{
							...
							{
								// parse a new line
								if (ParseLine(&SourceText,StrLine,StrLineLength,TRUE))
								{
									if ( ContextSupplier != NULL )
										ContextSupplier->CurrentLine++;
					...
					{
						FullText = FString::Printf(TEXT("%s"),Str);
					}
					...
					if( appStricmp(*Property->GetName(),TEXT("Name"))!=0 )
					{
						...
						if (Index > -1 && ArrayProperty != NULL) //set single dynamic array element
						{
							...
							if (Index>=Array->Num())
							{
								...
								Array->AddZeroed(NumToAdd, ArrayProperty->Inner->ElementSize, DEFAULT_ALIGNMENT);
								UStructProperty* StructProperty = Cast<UStructProperty>(ArrayProperty->Inner, CLASS_IsAUStructProperty);
							...
							const TCHAR* Result = ArrayProperty->Inner->ImportText( Str, (BYTE*)Array->GetData() + Index * ArrayProperty->Inner->ElementSize, PortFlags, ImportTextParent, &ImportError );
						...
						else
						{
							...
							// if we're compiling a class, we want to pass in that class for the parent of the component property, even if the property is inside a nested component
							if (GEditor->Bootstrapping && Property->IsA(UComponentProperty::StaticClass()))
							{
								ImportTextParent = SubobjectOuter;
							}
	...
	)
{
	...
	if ( LineNumber != INDEX_NONE )
	{
		...
		{
			checkf(!GIsUCCMake || ContextSupplier, TEXT("NULL context supplier encountered while importing defaults for %s: SubobjectRoot:%s  SubobjectOuter:%s   LineNumber:%i"), *ObjectStruct->GetFullName(), *SubobjectRoot->GetFullName(), SubobjectOuter ? *SubobjectOuter->GetFullName() : TEXT(""), LineNumber);
			if ( ContextSupplier != NULL )
			{
	...
	if ( SubobjectOuter != NULL && GIsUCCMake == FALSE )
	{
		SubobjectOuter->PreEditChange(NULL);
	}

	FObjectInstancingGraph* CurrentInstanceGraph = InInstanceGraph;
	if ( SubobjectRoot != NULL && SubobjectRoot != UObject::StaticClass()->GetDefaultObject() )
	{
		if ( CurrentInstanceGraph == NULL )
		{
			CurrentInstanceGraph = new FObjectInstancingGraph;
		}
		CurrentInstanceGraph->SetDestinationRoot(SubobjectRoot);
	}
	...
	if ( SubobjectOuter != NULL )
	{
 		...
 		if ( !SubobjectRoot->HasAnyFlags(RF_ClassDefaultObject) )
		{
			...
			ObjectStruct->InstanceComponentTemplates(DestData, DefaultData, SubobjectArchetype ? SubobjectArchetype->GetClass()->GetPropertiesSize() : NULL,
				SubobjectOuter, CurrentInstanceGraph);
		}

		if ( !GIsUCCMake )
		{
			// notify the object that it has just been imported
			SubobjectOuter->PostEditImport();

			// notify the object that it has been edited
			SubobjectOuter->PostEditChange(NULL);
		}
	}
	...
	if ( CurrentInstanceGraph != NULL && InInstanceGraph == NULL )
	{
