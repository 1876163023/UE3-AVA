...
//
extern INT         GErrors;        // Errors encountered in Csg operation.
extern INT			GDiscarded;		// Number of polys discarded and not added.
extern INT         GNode;          // Node AddBrushToWorld is adding to.
extern INT         GLastCoplanar;	// Last coplanar beneath GNode at start of AddWorldToBrush.
extern INT         GNumNodes;		// Number of Bsp nodes at start of AddWorldToBrush.
extern UModel		*GModel;		// Level map Model we're adding to.
...
(
	...
	DWORD		NodeFlags, 
	FPoly*		EdPoly,
	//<@ ava specific ; 2007. 1. 31 changmin
	INT			ClusterNumber
	//>@ ava
)
{
	...
	if( Surf->PolyFlags & PF_Hint )					NodeFlags |= NF_Hint | NF_NotVisBlocking | NF_NotCsg;
	//>@ ava
	...
	if( EdPoly->Vertices.Num() > FBspNode::MAX_NODE_VERTICES )
	{
		...
		*EdPoly2 = *EdPoly;
		...
		//<@ ava specific ; 2007. 1. 31 changmin
		// cluster number를 지정할 수 있도록 확장
		INT iNode = bspAddNode( Model, iParent, NodePlace, NodeFlags, EdPoly1, ClusterNumber ); // Add this poly first.
		bspAddNode( Model, iNode,   NODE_Plane, NodeFlags, EdPoly2, ClusterNumber ); // Then add other (may be bigger).
		//>@ ava
...
(
	...
	,FBox&				Bound
	,FPoly*				GridSplitPoly
	,UBOOL				NeedsGridSplit
	,FLOAT*				SplitPlaneDist
	//>@ ava
)
{
	check(NumPolys>0);
	...
	#define	DIST_EPSILON	0.01
		...
		for( int i = 0; i < 2; ++i )
		{
			FLOAT Dist = BLOCK_SIZE * ( appFloor( Bound.Min[i] / BLOCK_SIZE) + 1);
			if( Bound.Max[i] > Dist )
			{
				Normal[i] = 1;
				if( fabs(Dist - appFloor(Dist + 0.5f)) < DIST_EPSILON )
				{
					Dist = appFloor(Dist + 0.5f);
				}

				*SplitPlaneDist = Dist;
				
				//FPoly *Poly = FindPolyWithPlane( Normal, Dist, NumPolys, PolyList );
				FPoly *Poly;
				...
				//if( !Poly )
				{
					...
					// Set up the FPoly.
					GridSplitPoly->Init();
					GridSplitPoly->Normal		= Normal;
					GridSplitPoly->Base			= Normal * Dist;
					GridSplitPoly->PolyFlags	= PF_Hint | PF_NotSolid;

					new(GridSplitPoly->Vertices) FVector(GridSplitPoly->Base + Axis1*WORLD_MAX + Axis2*WORLD_MAX);
					new(GridSplitPoly->Vertices) FVector(GridSplitPoly->Base - Axis1*WORLD_MAX + Axis2*WORLD_MAX);
					new(GridSplitPoly->Vertices) FVector(GridSplitPoly->Base - Axis1*WORLD_MAX - Axis2*WORLD_MAX);
					new(GridSplitPoly->Vertices) FVector(GridSplitPoly->Base + Axis1*WORLD_MAX - Axis2*WORLD_MAX);
	...
	for( i=0; i<NumPolys; i+=Inc )
	{
		...
		do
		{
			...
			&& ( (Poly->PolyFlags & PF_AddLast) && !(Poly->PolyFlags & PF_Portal) && !(Poly->PolyFlags & PF_Hint) )	// ava specific : 2006. 11. 27 changmin. PF_Hint 추가
			&& !AllSemiSolids );
		if( Index>=i+Inc || Index>=NumPolys )
			continue;

		UBOOL HintSplit = FALSE;
		...
		for( j=0; j<NumPolys; j+=Inc ) if( j != Index )
		{
			...
			switch( OtherPoly->SplitWithPlaneFast( FPlane( Poly->Vertices(0), Poly->Normal), NULL, NULL ) )
			{
				...
				case SP_Split:
					...
					//<@ ava specific ; 2006. 11. 26 changmin
					if ( !(Poly->PolyFlags & PF_Hint) && (OtherPoly->PolyFlags & PF_Hint ) )	// hint 가 아니면, hint를 자르지 못한다...
						HintSplit = TRUE;
					//>@ ava
					break;
			}
		...
		if( HintSplit )
		{
			Score = 9999999;	// big value
		}
		if( Poly->PolyFlags & PF_Hint )
		{
			Score -= (100.0f - float(Balance)) *Splits;	// decrease split score, so hint will has higher priority.
		}
		//>@ ava
		if( Score<BestScore || !Best )
		{
			...
			//<@ ava specific ; 2006. 11. 28 changmin
			*SplitPlaneDist = Poly->Normal | Poly->Base;
			//>@ ava
		}
	}
	check(Best);
...
)
{
	...
	const UBOOL NeedsGridSplit = (RebuildSimplePolys == 0) && bGridSplitting;
	FLOAT Dist;
	FPoly *SplitPoly = FindBestSplit( NumPolys, PolyList, Opt, Balance, PortalBias, Bound, &GridSplitPoly, NeedsGridSplit, &Dist );
	//>@ ava

	//<@ ava specific ; 2006. 11 .28 changmin
	if( SplitPoly == &GridSplitPoly )
	{
	...
	//<@ ava specific ; 2006. 11. 28 changmin
	FBox FrontBound = Bound;
	FBox BackBound = Bound;
	for( INT i = 0; i < 3; ++i )
	{
		if( SplitPoly->Normal[i] == 1 )
		{
			FrontBound.Min[i]	= Dist;
			BackBound.Max[i]	= Dist;
			break;
		}
	}
	//>@ ava
	...
	if( NumFront > 0 ) SplitPolyList( Model, iOurNode, NODE_Front, NumFront, FrontList, Opt, Balance, PortalBias, RebuildSimplePolys, FrontBound, bGridSplitting );	// ava specific ; 2006. 12. 28 chanmin : add bound, gridsplitting
	if( NumBack  > 0 ) SplitPolyList( Model, iOurNode, NODE_Back,  NumBack,  BackList,  Opt, Balance, PortalBias, RebuildSimplePolys, BackBound, bGridSplitting );	// ava specific ; 2006. 12. 28 chanmin : add bound, gridsplitting
...
)
{
	...
	if( Model->Polys->Element.Num() )
	{
		...
		//<@ ava specific ; 2007. 1. 4 changmin
		// bsp 용 bound 계산.
		// model의 treebound는 portal 생성용 bound
		FBox TreeBound;
		TreeBound.Init();
		for( int i=0; i<Model->Polys->Element.Num(); i++ )
		{
			...
			{
				TreeBound += poly.Vertices(vertexIndex);
			}
		}
		//>@ ava

		// Now split the entire Bsp by splitting the list of all polygons.
		//const UBOOL bNeedGridSplitting = bComputePVS;
		const UBOOL bNeedGridSplitting = FALSE;
		SplitPolyList
		(
			Model,
			INDEX_NONE,
			...
			PortalBias,
			RebuildSimplePolys
			//<@ ava specific ; 2006. 12. 28 changmin
			, TreeBound
			, bNeedGridSplitting
			//>@ ava
		);
 ...
 */
void UEditorEngine::RebuildBrush(UModel* Brush)
{
...
)
{
	...
	UMaterialInstance* SelectedMaterialInstance = GetSelectedObjects()->GetTop<UMaterialInstance>();
	for( i=0; i<Brush->Polys->Element.Num(); i++ )
	{
		...
		{
			UMaterialInstance*& PolyMat = CurrentPoly.Material;
...
)
{
	...
	//!{ 2006-05-24	허 창 민
	// T - junction을 갖지 않는 두 side와 연결된 vertex를 찾아 start vertex로 만들어 보자~~~
	if( VertexNumber == 0 || VertexNumber == 1 )
	{
		INT StartVertex = VertexNumber + Node.NumVertices - 2;

		INT iOldVert2 = Node.iVertPool;
		Node.iVertPool = Model->Verts.Add( Node.NumVertices );

		for( INT i=0; i < Node.NumVertices; ++i )
		{
			Model->Verts(Node.iVertPool + i) = Model->Verts( iOldVert2 + ( i + StartVertex ) % Node.NumVertices );
		}
	}
