...
INT UMakeCommandlet::Main( const FString& Params )
{
	...
	if( ParseParam(appCmdLine(), TEXT("COOK")) )
	{
		bCook = TRUE;
		bDeletePackage = TRUE;
	}
	...
	if (Success && bCook)
	{
		...
		for( INT PackageIndex=0; PackageIndex<CookEntries.Num(); PackageIndex++ )
		{
			const FCookEntry& Entry = CookEntries(PackageIndex);		

			// write a message indicating that we have finished the "compilation" phase and are beginning the "saving" part, so that crashes in SavePackage aren't
			// mistakenly assumed to be related to script compilation.
			warnf(TEXT("Resaving package '%s'"), *Entry.Filename);
			SavePackage( Entry.Package, NULL, RF_Standalone, *Entry.Filename, GError, NULL );
		}
	}
