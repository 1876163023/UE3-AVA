...
INT FindMaterialIndex(TArray<FStaticMeshElement>& Materials,UMaterialInstance* Material)
{
...
struct FASEMaterialInfo
{
	...
	FASEMaterialInfo( TCHAR* InName, INT InWidth, INT InHeight, FLOAT InUTiling, FLOAT InVTiling, UMaterialInstance* InMaterial )
	{
...
)
{
	...
	if(appStricmp(Type,TEXT("ASE")) == 0)
	{
		...
		TArray<UMaterialInstance*>	ExistingElementMaterial;
		...
		//!{ 2006-06-27	Çã Ã¢ ¹Î
		UBOOL						ExistingHasRadiosityGeometry = false;
		//!} 2006-06-27	Çã Ã¢ ¹Î
		...
		if(ExistingMesh)
		{
			...
			//!{ 2006-06-27	Çã Ã¢ ¹Î
			ExistingHasRadiosityGeometry = ExistingMesh->HasRadiosityGeometry();
			//!} 2006-06-27	Çã Ã¢ ¹Î
		...
		while( ParseLine( &Buffer, StrLine ) )
		{
			...
			if( appStrstr(Str,TEXT("*3DSMAX_ASCIIEXPORT")) )
			{
				...
				UMaterialInstance* CurrentMaterial = NULL;
				...
				while( ParseLine( &Buffer, StrLine ) )
				{
					...
					if( StrLine.StartsWith( TEXT("*GEOMOBJECT") ) )
					{
						...
						if( CollisionFaceIdx.Num() )
						{
							...
							// For convex, we create a CollisionModel and put the triangles into that.
							// Otherwise, we just put them in a local array.
							TArray<FPoly>* Tris;
							TArray<FPoly> CollisionTris;

							if(GeomObjType == GOT_Convex && !bOneConvexPerUCXObject)
							{
								Tris = &(TempModel->Polys->Element);
							}
							else
							{
								Tris = &CollisionTris;
							...
							{
								FPoly*	Poly = new(*(Tris)) FPoly();

								Poly->Init();

								new(Poly->Vertices) FVector(CollisionVertices(CollisionFaceIdx(x + 2)) * FVector(1,-1,1));
								new(Poly->Vertices) FVector(CollisionVertices(CollisionFaceIdx(x + 1)) * FVector(1,-1,1));
								new(Poly->Vertices) FVector(CollisionVertices(CollisionFaceIdx(x + 0)) * FVector(1,-1,1));
								Poly->iLink = x / 3;
							...
							if(GeomObjType == GOT_Convex)
							{
								...
								else
								{
									...
									const UBOOL bSuccess = KModelToHulls( &StaticMesh->BodySetup->AggGeom, TempModel );
									if ( !bSuccess )
									{
							...
							{
								AddBoxGeomFromTris( *Tris, &StaticMesh->BodySetup->AggGeom, *NodeName );
							}
				...
				//!{ 2006-06-27	Çã Ã¢ ¹Î
				if( bRestoreExistingData && ExistingHasRadiosityGeometry )
				{
