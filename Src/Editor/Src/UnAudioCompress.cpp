...
bool FPCSoundCooker::CookMP3( short* SrcBuffer, DWORD SrcBufferSize, void* WaveFormat, INT Quality, BYTE* RawWaveData, DWORD RawWaveDataSize )
{
	...
	CompressionQuality = Clamp( 9 - CompressionQuality * 9, 0.0f, 9.0f );

	// Write source file
	TCHAR TempFilename[256];
	appCreateTempFilename( TEXT("."), TempFilename );

	FString WaveFilename = FString::Printf( TEXT("%s.wav"), TempFilename );
	FString MP3Filename = FString::Printf( TEXT("%s.mp3"), TempFilename );

	// Lock raw wave data.	
	FArchive* Ar = GFileManager->CreateFileWriter( *WaveFilename );	
	Ar->Serialize( RawWaveData, RawWaveDataSize );
	Ar->Close();	

	delete Ar;	

	FString Parameters = FString::Printf( TEXT("-V %d %s %s"), appTrunc(CompressionQuality), *WaveFilename, *MP3Filename );

	HANDLE hProcess = (HANDLE)appCreateProc( TEXT("lame.exe"), *Parameters );

	if (hProcess == NULL)
	{
		GFileManager->Delete( *WaveFilename );
		return false;
	}
	...
	INT FileSize = GFileManager->FileSize( *MP3Filename );
	...
	Ar = GFileManager->CreateFileReader( *MP3Filename );
	if (!Ar)
	{
		return false;
	}	

	CompressedDataStore.AddZeroed( FileSize );

	Ar->Serialize( &CompressedDataStore(0), FileSize );
	Ar->Close();
	delete Ar;

	GFileManager->Delete( *MP3Filename );
...
*/
bool FPCSoundCooker::Cook( short* SrcBuffer, DWORD SrcBufferSize, void* WaveFormat, INT Quality, UBOOL bCompressedSample, BYTE* RawWaveData, DWORD RawWaveDataSize )
{
	...
	UBOOL bEncodeAsMP3 = TRUE;

	if (bCompressedSample && RawWaveDataSize > 0 && RawWaveData != NULL)
	{
		if (CookMP3( SrcBuffer, SrcBufferSize, WaveFormat, Quality, RawWaveData, RawWaveDataSize ))
			return true;
...
*/
bool FPCSoundCooker::CookSurround( short* SrcBuffers[8], DWORD SrcBufferSize, void* WaveFormat, INT Quality )
{
	ogg_stream_state	os;		// take physical pages, weld into a logical stream of packets 
	ogg_page			og;		// one ogg bitstream page.  Vorbis packets are inside
	ogg_packet			op;		// one raw packet of data for decode
...
bool FPCSoundCooker::Decompress( const BYTE* CompressedData, DWORD CompressedDataSize, void* AdditionalInfo )
{
	...
	{
		if (CompressedDataSize > 3)
		{
			if (CompressedData[0] == 'I' &&
				CompressedData[1] == 'D' &&
				CompressedData[2] == '3')
			{
				PCMData.AddZeroed( CompressedDataSize );
				appMemcpy( &PCMData(0), CompressedData, CompressedDataSize );
				return( true );
			}
		}
