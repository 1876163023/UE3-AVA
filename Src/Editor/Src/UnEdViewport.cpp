...
AActor* UEditorEngine::AddActor(UClass* Class, const FVector& Location, UBOOL bSilent)
{
	...
	AActor* Actor = NULL;
	{
		...
		if( Actor )
		{
			...
			{
				csgCopyBrush( (ABrush*)Actor, (ABrush*)Class->GetDefaultActor(), 0, 0, 1, TRUE );
			}
...
namespace
{
	...
	static void ClickActor(FEditorLevelViewportClient* ViewportClient,AActor* Actor,const FViewportClick& Click)
	{
		...
		// Handle selection.
		if( Click.GetKey() == KEY_RightMouseButton && !Click.IsControlDown() && !ViewportClient->Viewport->KeyState(KEY_LeftMouseButton) )
		{
			{
	...
	static void ClickGeomVertex(FEditorLevelViewportClient* ViewportClient,HGeomVertexProxy* InHitProxy,const FViewportClick& Click)
	{
		...
		if( Click.GetKey() == KEY_LeftMouseButton || Click.GetKey() == KEY_RightMouseButton)
		{
			mode->GetCurrentTool()->StartTrans();

			// Disable Ctrl+clicking for selection if selecting with RMB.
			const UBOOL bControlDown = (Click.GetKey() == KEY_RightMouseButton ? FALSE : Click.IsControlDown());
			if( !bControlDown )
			{
				mode->GetCurrentTool()->SelectNone();
			}
		...
		if( Click.GetKey() == KEY_RightMouseButton )
		{
			...
			const FVector World = InHitProxy->GeomObject->GetActualBrush()->LocalToWorld().TransformFVector( InHitProxy->GeomObject->VertexPool( InHitProxy->VertexIndex ) );
	...
	static void ClickSurface(FEditorLevelViewportClient* ViewportClient,UModel* Model,INT iSurf,const FViewportClick& Click)
	{
		...
		if( Click.GetKey() == KEY_LeftMouseButton && Click.IsShiftDown() && Click.IsControlDown() )
		{
		...
		else if( Click.GetKey() == KEY_LeftMouseButton && Click.IsShiftDown() )
		{
			...
			UMaterialInstance* SelectedMaterialInstance = GEditor->GetSelectedObjects()->GetTop<UMaterialInstance>();
		...
		{
			// Grab the texture.
			GEditor->GetSelectedObjects()->SelectNone(UMaterialInstance::StaticClass());
			if(Surf.Material)
			{
		...
		else if( Click.IsAltDown() && Click.GetKey() == KEY_LeftMouseButton)
		{
			...
			Surf.Material = GEditor->GetSelectedObjects()->GetTop<UMaterialInstance>();
			if( Click.IsControlDown() )
			{
	...
	static void ClickBackdrop(FEditorLevelViewportClient* ViewportClient,const FViewportClick& Click)
	{
		...
		if( Click.GetKey() == KEY_LeftMouseButton && ViewportClient->Viewport->KeyState(KEY_A) )
		{
			// Create an actor of the selected class.
			UClass* SelectedClass = GEditor->GetSelectedObjects()->GetTop<UClass>();
			if( SelectedClass )
...
UBOOL FEditorLevelViewportClient::InputKey(FViewport* Viewport,INT ControllerId,FName Key,EInputEvent Event,FLOAT/*AmountDepressed*/,UBOOL/*Gamepad*/)
{
	...
	{
		Viewport->ShowCursor( !bMouseButtonDown );
		Viewport->LockMouseToWindow( bMouseButtonDown );
	}
	...
	if ( bMouseButtonDown && (Event == IE_Pressed || Event == IE_Released) && (bMouseButtonEvent || bCtrlButtonEvent || bAltButtonEvent || bShiftButtonEvent) )
	{
		...
		bCtrlWasDownOnMouseDown |= ControlDown & !AltDown;
		bIsTracking = TRUE;
		return TRUE;
	}
...
UBOOL FEditorLevelViewportClient::CheckForShowFlagInput(FName Key, UBOOL bControlDown, UBOOL bAltDown, UBOOL bShiftDown)
{
	...
	const EShowFlags CurrentViewFlags = ShowFlags & SHOW_ViewMode_Mask;
...
UBOOL FEditorLevelViewportClient::InputAxis(FViewport* Viewport,INT ControllerId,FName Key,FLOAT Delta,FLOAT DeltaTime)
{
	if( GCurrentLevelEditingViewportClient != this )
	{
		//GCallbackEvent->Send( CALLBACK_RedrawAllViewports );
...
void FEditorLevelViewportClient::ApplyDeltaToActor( AActor* InActor, const FVector& InDeltaDrag, const FRotator& InDeltaRot, const FVector& InDeltaScale )
{
	...
	const UBOOL bIsBrushNotVolume = ( InActor->IsBrush() && !Cast<AVolume>(InActor) );
	if( bIsBrushNotVolume )
	{
	...
	const UBOOL bRotatingActor = !InDeltaRot.IsZero();
	if( bRotatingActor )
	{
		if( bIsBrushNotVolume )
		{
			...
			if(Brush->BrushComponent->Brush && Brush->BrushComponent->Brush->Polys)
			{
				for( INT poly = 0 ; poly < Brush->BrushComponent->Brush->Polys->Element.Num() ; poly++ )
				{
					...
					{
						Poly->Vertices(vertex) = Brush->PrePivot + FRotationMatrix( InDeltaRot ).TransformNormal( Poly->Vertices(vertex) - Brush->PrePivot );
					}
		...
		{
			InActor->EditorApplyRotation( InDeltaRot, AltDown, ShiftDown, ControlDown );
		}
	...
	// ** Translation
	// **
	InActor->EditorApplyTranslation( InDeltaDrag, AltDown, ShiftDown, ControlDown );


	// **
	// ** Scale
	// **
	const UBOOL bScalingActor = !InDeltaScale.IsNearlyZero(0.000001f);
	...
	if( bScalingActor )
	{
		...
		else
		{
			...
			InActor->EditorApplyScale( InDeltaScale,
										matrix,
										GEditorModeTools().CoordSystem == COORD_World ? &GEditorModeTools().PivotLocation : NULL,
										AltDown,
										ShiftDown,
										ControlDown );
		}
...
void FEditorLevelViewportClient::RedrawRequested(FViewport* Viewport)
{
...
void FEditorLevelViewportClient::Draw(FViewport* Viewport,FCanvas* Canvas)
{
	...
	FSceneViewFamilyContext ViewFamily(Canvas->GetRenderTarget(),GetScene(),ShowFlags,GWorld->GetTimeSeconds(),GWorld->GetRealTimeSeconds(),NULL);
	FSceneView* View = CalcSceneView( &ViewFamily );
	...
	if (View->Family->ShowFlags & SHOW_Ambientcubes)
	{
		DrawAmbientCubes( Viewport, View, Canvas );
	}
	...
	//!{ 2006-08-11	Çã Ã¢ ¹Î
	// Possibly a hack. Lines may get added without the scene being rendered etc.
	if( GWorld->SampleDrawer )
	{
		GWorld->SampleDrawer->ConditionalUpdateTransform();
	}
	//!} 2006-08-11	Çã Ã¢ ¹Î
...
static FSHVector PointLightSH(const FVector& Direction)
{
	...
	Result /= appSqrt(Dot(Result,Result));
...
void FEditorLevelViewportClient::DrawAmbientCubes(FViewport* Viewport, const FSceneView* View, FCanvas* Canvas)
{
	AWorldInfo* WI = GWorld->GetWorldInfo();

	if (!WI) return;

	for (INT i=0; i<WI->AmbientCubeVolumeInfos.Num(); ++i)
	{
		const FAmbientCubeVolumeInfo& Info = WI->AmbientCubeVolumeInfos(i);
		INT NX = Info.NumAmbientCubeX;
		INT NY = Info.NumAmbientCubeY;
		INT NZ = Info.NumAmbientCubeZ;

		FVector Org = Info.AmbientCubeMin;
		FVector Size = (Info.AmbientCubeMax - Info.AmbientCubeMin);

		FMatrix InvViewMatrix = View->ViewMatrix.Inverse();	
		FMatrix InvProjMatrix = View->ProjectionMatrix.Inverse();			

		FVector CubeExtent( 4, 4, 4 );
		...
		for (INT X = 0; X < NX; ++X)
		{
			for (INT Y = 0; Y < NY; ++Y)
			{
				for (INT Z = 0; Z < NZ; ++Z)
				{
					FVector BoxCenter = Org + FVector( Size.X * (X) / (NX-1), Size.Y * (Y) / (NY-1), Size.Z * (Z) / (NZ-1) );
					FVector BoxExtents( 1, 1, 1 );

					FLOAT Distance = (BoxCenter - View->ViewOrigin).Size();

					static const FLOAT HotSpot = 16 * 3 * 3;
					static const FLOAT FallOff = 16 * 3 * 3;

					if (Distance > HotSpot + FallOff) continue;				

					// Project center of bounding box onto screen.
					const FPlane ProjBoxCenter = View->Project(BoxCenter);								

					// Do nothing if behind camera
					if(ProjBoxCenter.W > 0.f && fabsf(ProjBoxCenter.X) < 1.2f && fabsf(ProjBoxCenter.Y) < 1.2f)
					{
						FCheckResult Hit;						

						GWorld->SingleLineCheck( Hit, NULL, BoxCenter, View->ViewOrigin, TRACE_World | TRACE_LightVisibility | TRACE_StopAtAnyHit );

						if (Hit.Actor != NULL)				
							continue;

						/*
						FVector TestPoint = BoxCenter;

						// check if fits at desired location					
						for (INT i=0; i<100; ++i)							
						{
							FCheckResult Hit(1.f);
							if (!GWorld->EncroachingWorldGeometry(Hit, TestPoint, CubeExtent ))
								break;

							TestPoint.Z += CubeExtent.Z * 2;
						}
						*/

						//const FPlane ProjTest = View->Project(TestPoint);													

						const INT HalfX = 0.5f * Viewport->GetSizeX();
						const INT HalfY = 0.5f * Viewport->GetSizeY();					

						INT AmbientCubeIndex = X + (Y + Z * NY) * NX + Info.SHStartIndex;
						FVector Direction = (BoxCenter - View->ViewOrigin).SafeNormal();
						FVector Squared = Direction * Direction;

						FAmbientSH AmbientCube;
						GWorld->GetWorldInfo()->InterpolateAmbientCube( Info, BoxCenter, AmbientCube );
						const FSHVectorRGB& Colors = WI->AmbientSHs(AmbientCubeIndex);

						// Calculate the light intensity for this direction.
						FSHVector UnitLightSH = PointLightSH(Direction);
						FLinearColor DrawColor = GetPositiveColor(Dot(AmbientCube,UnitLightSH));
						FLinearColor DrawColor2= GetPositiveColor(Dot(Colors,UnitLightSH));					

						if (Distance > HotSpot)
						{
							DrawColor.A = 1.0f - (Distance - HotSpot) / FallOff;
							DrawColor2.A = 1.0f - (Distance - HotSpot) / FallOff;
						}
						
						// Draw a box when considering the current level.
						DrawTile( Canvas, HalfX * ( 1 + ProjBoxCenter.X) - 8, HalfY * ( 1 - ProjBoxCenter.Y) - 8, 8.0f, 16.0f, 0, 0, 1, 1, DrawColor );	
						DrawTile( Canvas, HalfX * ( 1 + ProjBoxCenter.X), HalfY * ( 1 - ProjBoxCenter.Y) - 8, 8.0f, 16.0f, 0, 0, 1, 1, DrawColor2 );	
						/*DrawLine2D( Canvas, 
							FVector2D( HalfX * ( 1 + ProjBoxCenter.X), HalfY * ( 1 - ProjBoxCenter.Y) ),
							FVector2D( HalfX * ( 1 + ProjTest.X), HalfY * ( 1 - ProjTest.Y) ),
							DrawColor );	*/

						// This is an approximation which only takes into account first and second order spherical harmonics.
						FSHVector Luminance = ((const FSHVectorRGB&)AmbientCube).GetLuminance();
						FVector MaxDirection( -Luminance.V[3], -Luminance.V[1], Luminance.V[2] );
						
						const FVector DiffVec = (View->Project(BoxCenter + MaxDirection) - ProjBoxCenter).SafeNormal();
						FVector2D StartPos( HalfX * ( 1 + ProjBoxCenter.X), HalfY * ( 1 - ProjBoxCenter.Y) );

						DrawLine2D( Canvas, 
							StartPos,
							StartPos + 16.0f * FVector2D( DiffVec.X, -DiffVec.Y ),
							DrawColor );	

						if (((FVector)(View->ViewOrigin - BoxCenter)).Size() < 128)
						{
							DrawString( Canvas, StartPos.X, StartPos.Y, *FString::Printf( TEXT("%d %d %d"), X, Y, Z ), GEngine->GetTinyFont(), FLinearColor::White );
							DrawString( Canvas, StartPos.X, StartPos.Y+16, *FString::Printf( TEXT("%5.2f %5.2f %5.2f"), BoxCenter.X, BoxCenter.Y, BoxCenter.Z ), GEngine->GetTinyFont(), FLinearColor::White );						
						}						
					}				
				}
			}
	...
	FAmbientSH AmbientCube;
	GWorld->GetWorldInfo()->InterpolateAmbientCube( View->ViewOrigin, AmbientCube );	

	// Calculate the light intensity for this direction.
	FSHVector UnitLightSH = PointLightSH(View->ViewMatrix.GetAxis(2));
	FLinearColor DrawColor = GetPositiveColor(Dot(AmbientCube,UnitLightSH));	

	const INT HalfX = 0.5f * Viewport->GetSizeX();
	const INT HalfY = 0.5f * Viewport->GetSizeY();
	const INT Size = 16;
	DrawTile( Canvas, HalfX - Size/2, HalfY - Size/2, Size, Size, 0, 0, 1, 1, DrawColor );	

	UFont* Font = GEngine->GetSmallFont();
	INT XL, YL;
	StringSize( Font, XL, YL, TEXT("Qp") );
	FLOAT Y = HalfY + YL;
	...
	for(TSparseArray<ULightComponent*>::TConstIterator LightIt(GWorld->StaticLightList);LightIt;++LightIt)
	{
		...
		// Skip lights which are in a different light environment.
		if(!Light->bAffectsDefaultLightEnvironment)
		{
			continue;
		}		
		...
		if(ConvertedLightingChannels.Dynamic)
		{
			// Skip lights which don't affect the primitive's bounds.
			if(!Light->AffectsBounds(FBoxSphereBounds(View->ViewOrigin,FVector(16,16,16),16)))
			{
				continue;
			}
			...
			if(Light->CastShadows && Light->CastStaticShadows)
			{
				...
				FVector LightVector = (FVector)LightPosition - View->ViewOrigin * LightPosition.W;
				const UBOOL bLightIsVisible = GWorld->SingleLineCheck(
					Hit,
					NULL,
					View->ViewOrigin,
					View->ViewOrigin + LightVector,
			...
			DrawString( Canvas, HalfX, Y, Light->GetOwner() ? *Light->GetOwner()->GetName() : *Light->GetFullName(), Font, FLinearColor::White );
			Y += YL;
		}
	}
}
