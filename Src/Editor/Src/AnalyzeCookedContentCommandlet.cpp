...
extern void GetNativeScriptPackageNames(TArray<FString>& PackageNames, UBOOL bExcludeGamePackages, UBOOL bIncludeLocalizedSeekFreePackages);
...
{
	FFilename CookedPackageWildcard = (appGameDir() + COOKED_FOLDER) * TEXT("*.*");
	...
	GetNativeScriptPackageNames(NativeScriptPackageNames, FALSE, TRUE);
...
UBOOL FResourceStatReporter_TotalMemoryPerAsset::CreateReport( const TArray<FPackageResourceStat>& ResourceStats )
{
	...
	for ( INT ClassIndex = 0; ClassIndex < MaxDesiredResults; ClassIndex++ )
	{
		...
		warnf(NAME_Console, TEXT("%*i) %*s:  %I64i %s"), IndexIndent, ClassIndex, ClassNameIndent, *ClassSize.ClassName, 
...
UBOOL FResourceStatReporter_AssetDuplication::CreateReport( const TArray<FPackageResourceStat>& ResourceStats )
{
	...
	for ( TMap<INT,FDuplicatedResource>::TIterator It(DuplicatedResources); It && ResultCount++ < MaxDesiredResults; ++It )
	{
		...
		warnf(NAME_Console, TEXT("Min/Max/Total: %i%s / %i%s / %I64i%s across %i packages for %s"), 
	...
	for ( TMap<INT,FDuplicatedResource>::TIterator It(DuplicatedResources); It && ResultCount++ < MaxDesiredResults; ++It )
	{
		...
		warnf(NAME_Console, TEXT("%s %I64i%s (total in %i packages)"), *DuplicateResource.GetResourceFullName(),
