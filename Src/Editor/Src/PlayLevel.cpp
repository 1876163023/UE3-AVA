...
UBOOL UEditorPlayer::Exec(const TCHAR* Cmd,FOutputDevice& Ar)
{
	...
	{
#if !FINAL_RELEASE
		/* Finishing Update cull distances */
		if (GPrecomputingFlags && GUpdatingCullDistances)
		{		
			GEngine->Exec(TEXT("UpdateCullDistances"), *GLog);		
		}
#endif
...
void UEditorEngine::EndPlayMap()
{
	...
	if (GPrecomputingFlags)
	{			
		GEngine->Exec(TEXT("ApplyCullDistances"), *GLog);
	}
}

void UEditorEngine::PlayMap(FVector* StartLocation, FRotator* StartRotation, INT Destination, INT PrecomputingFlags )
{
	...
	// remember precomputing flags
	PlayWorldPrecomputingFlags = PrecomputingFlags;
...
void UEditorEngine::StartQueuedPlayMapRequest()
{
	...
	if (PlayWorldDestination == -1)
		PlayInEditor(PlayWorldPrecomputingFlags);
	else
...
UBOOL UEditorEngine::SavePlayWorldPackages( const TCHAR* Prefix, UBOOL bPlayOnConsole )
{
	...
	if (bPlayOnConsole)
	{
		appSaveAllWorlds( *FullAutoSaveDir, Prefix, TRUE, FALSE, TRUE );
	}
	else
	{
		...
		appSaveWorlds( WorldsNeedingPIESave, *FullAutoSaveDir, Prefix, TRUE, FALSE, TRUE );
...
void UEditorEngine::PlayInEditor( INT PrecomputingFlags )
{
	// Precomputing :) ;; 
	GPrecomputingFlags = PrecomputingFlags;
