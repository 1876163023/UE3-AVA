...
public:
	...
	virtual	INT EnvCube( INT CoordinateIdx) 
	{ 
		return INDEX_NONE;
	}
			...
			if (Package != NULL)
			{
				for(TObjectIterator<UMaterial> MaterialIt;MaterialIt;++MaterialIt)
				{
					if(MaterialIt->IsIn(Package))
					{
						...
						MaterialIt->MaterialResource->CompileProperty(MP_DiffuseColor,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_SpecularColor,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_Normal,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_EmissiveColor,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_Opacity,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_OpacityMask,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_Distortion,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_TwoSidedLightingMask,&GatherExpressionsCompiler);
						MaterialIt->MaterialResource->CompileProperty(MP_CustomLighting,&GatherExpressionsCompiler);
