...
UBOOL UTerrainExporterT3D::ExportHeightMapData(ATerrain* Terrain, FOutputDevice& Ar, FFeedbackContext* Warn, DWORD dwPortFlags)
{
	...
	{
		GConfig->GetString(TEXT("Editor.EditorEngine"), TEXT("HeightMapExportClassName"), Check, GEditorIni);
	}
	if (Check.Len() == 0)
	{
		...
		if ((dwPortFlags & PPF_Copy) == 0)
		{
			GConfig->SetString(TEXT("Editor.EditorEngine"), TEXT("HeightMapExportClassName"), *Check, GEditorIni);
		}
	}
	else
	{
