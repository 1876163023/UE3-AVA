...
#ifndef	AVA_INCLUDE_FSTATICBRUSHITERATOR
// Globals:
 ...
 */
void UEditorEngine::PostEditImportVolume(AVolume* Actor)
{
	csgPrepMovingBrush( Actor );
}
...
FString UEditorEngine::csgGetName( ECsgOper CSG )
{
	return (FindObjectChecked<UEnum>( ANY_PACKAGE, TEXT("ECsgOper") ) )->GetEnum(CSG).ToString();
}
...
// Repartition the bsp.
//
void UEditorEngine::bspRepartition( INT iNode, INT Simple )
{
	bspBuildFPolys( GWorld->GetModel(), 1, iNode );
	bspMergeCoplanars( GWorld->GetModel(), 0, 0 );
	//bspBuild( GWorld->GetModel(), BSP_Good, 12, 70, Simple, iNode );
	bspBuild( GWorld->GetModel(), BSP_Optimal, 12, 70, Simple, iNode );
	bspRefresh( GWorld->GetModel(), 1 );
}
...
void UEditorEngine::csgRebuild()
{
	...
	//<@ ava specific ; 2007. 1. 3 changmin
	// build tree bounds to generating portal.
	UModel *Bsp = GWorld->PersistentLevel->Model;
	Bsp->TreeBounds.Init();
	...
	for( FStaticBrushIterator It; It; ++It )
	{
		...
		if( !BrushActor->IsABuilderBrush() )
		{
			...
			if
				(  !(BrushActor->PolyFlags&PF_Semisolid)
	...
	Bsp->TreeBounds = StructuralBound;
	//>@ ava
	...
	{	
		...
		if( !Brush->IsABuilderBrush() )
		{
			...
			||  (Brush->PolyFlags&PF_Hint) )
			{
				...
				//>@
				BrushCount++;
				GWarn->StatusUpdatef( BrushCount, BrushTotal, *LocalizeUnrealEd(TEXT("ApplyingStructuralBrushF")), BrushCount, BrushTotal );
...
#define AVA_DETAIL 1
#if AVA_DETAIL
	//<@ ava specific ; 2006. 12. 21 changmin
	// detail model을 만든다. ( csg operations between semisolid and subtract )
	// We keep no reference to this, so it will be GC'd at some point.
	UModel *DetailModel = new UModel( NULL, 1 );
	for( FStaticBrushIterator It; It; ++It )
	{
		ABrush* Brush = CastChecked<ABrush>(*It);
		if( (!Brush->IsABuilderBrush() && (Brush->PolyFlags&PF_Semisolid) && !(Brush->PolyFlags&PF_Portal) && !(Brush->PolyFlags&PF_Hint) && (Brush->CsgOper==CSG_Add) )
		||	(!Brush->IsABuilderBrush() && (Brush->CsgOper==CSG_Subtract))
		)
		{
			...
			bspBrushCSG( Brush, DetailModel, Brush->PolyFlags, (ECsgOper)Brush->CsgOper, FALSE, TRUE, FALSE );
			if( bIsDetail )
			{
				BrushCount++;
				GWarn->StatusUpdatef( BrushCount, BrushTotal, *LocalizeUnrealEd(TEXT("ApplyingDetailBrushF")), BrushCount, BrushTotal );
	...
	bspAddDetailToStructure( GWorld->GetModel(), DetailModel );
#else
	...
	GWarn->StatusUpdatef( 0, 3,  *LocalizeUnrealEd(TEXT("RebuildCSGOptimizingSubBSPs")) );
	INT iNode;
	...
	for( FStaticBrushIterator It; It; ++It )
	{
		...
		if( !Brush->IsABuilderBrush() && Brush->IsSelected() )
		{
			if( PropertiesMask & MSB_PolyFlags )
			{
				...
				// brush 의 flag가 polygon의 flag로 전파되지 않는다.
				// semi solid로 갔다가.. solid로 전환되지 않는 버그 수정
				if( Brush->Brush )
				{
					Brush->Brush->Polys->Modify();
					for( INT i = 0; i < Brush->Brush->Polys->Element.Num(); ++i )
					{
						FPoly& CurrentPoly = Brush->Brush->Polys->Element(i);
						CurrentPoly.PolyFlags = (CurrentPoly.PolyFlags & ~ClearPolyFlags) | SetPolyFlags;
					}
				}
...
#endif	// #ifndef AVA_INCLUDE_FSTATICBRUSHITERATOR
