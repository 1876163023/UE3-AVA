 ...
 */
void FWidget::Render_Axis( const FSceneView* View, FPrimitiveDrawInterface* PDI, EAxis InAxis, FMatrix& InMatrix, UMaterialInstance* InMaterial, FColor* InColor, FVector2D& OutAxisEnd, float InScale, UBOOL bDrawWidget )
{
	...
	if( bDrawWidget )
	{
		...
		// Draw the arrow mesh.
		MeshBuilder.Draw(PDI,ArrowToWorld,InMaterial->GetInstanceInterface(FALSE),SDPG_UnrealEdForeground);
...
void FWidget::Render_Scale( const FSceneView* View,FPrimitiveDrawInterface* PDI, const FVector& InLocation, UBOOL bDrawWidget )
{
	...
	Render_Axis( View, PDI, AXIS_X, XMatrix, Material, Color, XAxisEnd, Scale, bDrawWidget );
	Render_Axis( View, PDI, AXIS_Y, YMatrix, Material, Color, YAxisEnd, Scale, bDrawWidget );
	Render_Axis( View, PDI, AXIS_Z, ZMatrix, Material, Color, ZAxisEnd, Scale, bDrawWidget );
...
void FWidget::Render_ScaleNonUniform( const FSceneView* View,FPrimitiveDrawInterface* PDI, const FVector& InLocation, UBOOL bDrawWidget )
{
	...
	if( DrawAxis&AXIS_X )	Render_Axis( View, PDI, AXIS_X, XMatrix, XMaterial, XColor, XAxisEnd, Scale, bDrawWidget );
	if( DrawAxis&AXIS_Y )	Render_Axis( View, PDI, AXIS_Y, YMatrix, YMaterial, YColor, YAxisEnd, Scale, bDrawWidget );
	if( DrawAxis&AXIS_Z )	Render_Axis( View, PDI, AXIS_Z, ZMatrix, ZMaterial, ZColor, ZAxisEnd, Scale, bDrawWidget );
