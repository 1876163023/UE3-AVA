...
void UCookPackagesCommandlet::CookTexture( UTexture2D* Texture2D )
{
	...
	if( !Texture2D->IsTemplate( RF_ClassDefaultObject ) )
	{
		...
		TextureCookTime += appSeconds() - StartTime;
	}
}
...
void UCookPackagesCommandlet::CookMovieTexture( UTextureMovie* TextureMovie )
{
	...
	if( !TextureMovie->IsTemplate(RF_ClassDefaultObject) )
	{
		if( ShouldByteSwapData() )
		{
			...
			UBOOL Result = FALSE;
			if( TextureMovie->DecoderClass == UCodecMovieTheora::StaticClass() )
			{
				Result = TRUE;				
			}
			else
...
void UCookPackagesCommandlet::CookSoundNodeWave( USoundNodeWave* SoundNodeWave )
{
	...
	SoundCookTime += appSeconds() - StartTime;
}
...
{	
	...
	)
	{
		...
		if(Material->MaterialResource->Compile( ShaderPlatform, MaterialShaderMapRef, FALSE ))
		{
...
UBOOL UCookPackagesCommandlet::SetPlatform(const FString& Params)
{
	...
	if (Parse(*Params, TEXT("PLATFORM="), PlatformStr))
	{
		...
		else if (PlatformStr == TEXT("pc_sm2"))
		{
			Platform = PLATFORM_Windows;
			ShaderPlatform = SP_PCD3D_SM2;
			GCreationFlags = TextureCreate_NoPackedMip;
		}
		else if (PlatformStr == TEXT("pc_sm2_poor"))
		{
			Platform = PLATFORM_Windows;
...
/**
 * Precreate all the .ini files that the Xenon will use at runtime
 */
void UCookPackagesCommandlet::CreateIniFiles()
{
	if( Platform == PLATFORM_PS3 || Platform == PLATFORM_Xenon )
	{
		...
		GConfig->CoalesceFilesFromDisk(*(appGameConfigDir() + GetConfigOutputDirectory()), ShouldByteSwapData());
...
UBOOL UCookPackagesCommandlet::Init( const TCHAR* Params )
{
	...
	// create the ini files for the xbox
	CreateIniFiles();
	...
	if( Switches.FindItemIndex(TEXT("FULL")) != INDEX_NONE )
	{
		...
		FString CookedFiles = FString::Printf( TEXT("%s") PATH_SEPARATOR TEXT("*.*"), *CookedDir );
		TArray<FString> Found; 
		GFileManager->FindFiles( Found, *CookedFiles, TRUE, FALSE );
		for( INT i=0; i < Found.Num(); i++ )
		{
			FString CookedFileName = FString::Printf( TEXT("%s") PATH_SEPARATOR TEXT("%s"), *CookedDir, *Found(i) );
			warnf( NAME_Log, TEXT("Deleting: %s"), *CookedFileName );
			if( !GFileManager->Delete( *CookedFileName ) )
			{
				appDebugMessagef(TEXT("Couldn't delete %s"), *CookedFileName);
				return FALSE;
			}
		}
	}
	...
	// here we need to look in the .ini to see which packages we need to load which are not referenced (e.g. .ini config, DLO )
	...
	for( TMultiMap<FString,FString>::TIterator It(*PackagesToAlwaysCook); It; ++It )
	{
		...
		{
			warnf( NAME_Log, TEXT("AlwaysCookedPackage added:  %s"), *PackageName );
			RequiredPackages.AddItem( FString(*PackageName).ToUpper() );
			LoadPackage( NULL, *PackageFileName, LOAD_None );
		}
	}

	// here we need to look in the .ini to see which packages should NEVER get moved into seek free packages (ie fonts due to localization)
	TMultiMap<FString,FString>* PackagesToExcludeFromSeekFreeMap = GConfig->GetSectionPrivate( TEXT("Engine.PackagesToExcludeFromSeekFree"), 0, 1, GEngineIni );
	// keep the package pointers around
	// @todo josh: make this a package flag for faster checking int the FObjectIterator below
	TArray<UPackage*> PackagesToExcludeFromSeekFree;
	// this ini setting isn't required
	if (PackagesToExcludeFromSeekFreeMap)
	{
		for( TMultiMap<FString,FString>::TIterator It(*PackagesToExcludeFromSeekFreeMap); It; ++It )
		{
			FString& PackageName = It.Value();
			FString PackageFileName;
			if( GPackageFileCache->FindPackageFile( *PackageName, NULL, PackageFileName ) == TRUE )
			{
				warnf( NAME_Log, TEXT("Excluding from seek free:  %s"), *PackageName );
				UPackage* Package = LoadPackage( NULL, *PackageFileName, LOAD_None );
				// make sure this and its objects aren't garbage collected so we always know it's to be excluded
				Package->AddToRoot();
				// keep a list of excluded packages for checking later
				PackagesToExcludeFromSeekFree.AddItem(Package);
			// Package is required so we can't strip it.
			RequiredPackages.AddItem( FString(Package->GetName()).ToUpper() );

				// mark all objects in the pacakge as not seekfreeable and put them in the root to keep from 
				// getting GC'd
				for( FObjectIterator It; It; ++It )
				{
					UObject*		Object				= *It;
	...
	// Check whether we only want to cook passed in packages and their dependencies.
	bOnlyCookDependencies = Tokens.Num() > 0 ? TRUE : FALSE;
	...
	PackageLoadTime += appSeconds() - StartTime;
 ...
 *
 * @param [out] FirstRequiredIndex		index of first required package in returned array, untouched if there are none
 ...
 */
TArray<UCookPackagesCommandlet::FPackageCookerInfo> UCookPackagesCommandlet::GeneratePackageList( INT& FirstRequiredIndex, INT& FirstScriptIndex, INT& FirstGameScriptIndex, INT& FirstMapIndex, INT& FirstMPMapIndex )
{
	...
	appGetEngineScriptPackageNames( EngineScriptPackageNames, FALSE );
	appGetGameNativeScriptPackageNames( GameNativeScriptPackageNames, FALSE );
	appGetGameScriptPackageNames( GameScriptPackageNames, FALSE );
	...
	// get all the startup packages that the xbox will use (so we need to pass it the xbox engine config name)
	...
	for( INT PackageIndex = 0; PackageIndex < PackageList.Num(); PackageIndex++ )
	{
		...
		FFilename			DstFilename	= CookedDir + SrcFilename.GetBaseFilename() + TEXT(".xxx");
		...
		UBOOL	bIsEngineScriptFile		= EngineScriptPackageNames.FindItemIndex( SrcFilename.GetBaseFilename() ) != INDEX_NONE;
		UBOOL	bIsGameNativeScriptFile	= GameNativeScriptPackageNames.FindItemIndex( SrcFilename.GetBaseFilename() ) != INDEX_NONE;
		UBOOL	bIsNativeScriptFile		= bIsEngineScriptFile || bIsGameNativeScriptFile;
		UBOOL	bIsNonNativeScriptFile	= !bIsGameNativeScriptFile && GameScriptPackageNames.FindItemIndex( SrcFilename.GetBaseFilename() ) != INDEX_NONE;
		UBOOL	bIsEditorScriptFile		= !(bIsNativeScriptFile || bIsNonNativeScriptFile) && AllScriptPackageNames.FindItemIndex( SrcFilename.GetBaseFilename() ) != INDEX_NONE;
		UBOOL	bIsMapFile				= (SrcFilename.GetExtension() == FURL::DefaultMapExt) || (SrcFilename.GetBaseFilename() == TEXT("entry"));
		UBOOL	bIsStartupPackage		= AllStartupPackageNames.FindItemIndex( SrcFilename.GetBaseFilename() ) != INDEX_NONE;
		...
		if( FindObject<UWorld>( NULL, *FString::Printf(TEXT("%s.TheWorld"),*SrcFilename.GetBaseFilename()) ) )
		{
			bIsMapFile = TRUE;
		}
		// Special case handling for Entry.upk.
		if( SrcFilename.GetBaseFilename() == TEXT("Entry") )
		{
			bIsMapFile = TRUE;
		}
		...
		else if( bIsNonNativeScriptFile )
		{
			continue;
		}
		...
		if( bIsMapFile )
		{
			...
			if ( bIsMultiplayerMap )
			{
				...
				MPMapFilenamePairs.AddItem( FPackageCookerInfo( *SrcFilename, *DstFilename, TRUE, FALSE, bIsStartupPackage ) );
			}
			else
			{
				MapFilenamePairs.AddItem( FPackageCookerInfo( *SrcFilename, *DstFilename, TRUE, FALSE, bIsStartupPackage ) );
			}
		}
		// Package is a script file.
		else if( bIsNativeScriptFile )
		{
			ScriptFilenamePairs.AddItem( FPackageCookerInfo( *SrcFilename, *DstFilename, TRUE, TRUE, bIsStartupPackage ) );
		}
		// Package is a required package.
		else if( RequiredPackages.FindItemIndex( SrcFilename.GetBaseFilename().ToUpper() ) != INDEX_NONE )
		{
			RegularFilenamePairs.AddItem( FPackageCookerInfo( *SrcFilename, *DstFilename, FALSE, FALSE, bIsStartupPackage ) );
		}
		// Package is not required and can be stripped of everything but texture miplevels.
		else
		{
			NotRequiredFilenamePairs.AddItem( FPackageCookerInfo( *SrcFilename, *DstFilename, FALSE, FALSE, bIsStartupPackage ) );
		}
	}
 ...
 */
void UCookPackagesCommandlet::SaveCookedPackage( UPackage* Package, UObject* Base, EObjectFlags TopLevelFlags, const TCHAR* DstFilename, UBOOL bStripEverythingButTextures )
{
	...
	// Update time it took to save.
	PackageSaveTime += appSeconds() - SaveStartTime;
}
...
static void GenerateMPObjectList(const TArray<FString>& MPCueNames, TArray<FString>& OutMPObjectNames)
{
	...
	for ( INT Index = 0 ; Index < References.Num() ; ++Index )
	{
		...
		if( !Object->HasAnyFlags( RF_Transient ) &&	!Object->IsIn( UObject::GetTransientPackage() ) )
		{
			...
			debugf( NAME_DevCooking, TEXT("MPObjects: %s (%s)"), *ObjectPath, *Object->GetClass()->GetName() );
...
void UCookPackagesCommandlet::CookMPSoundPackages(const TArray<FString>& MPCueNames)
{
	if ( bSeparateSharedMPResources && MPCueNames.Num() > 0 )
	{
		...
		FFilename DstFilename	= CookedDir + MPSounds_PackageName + TEXT(".xxx");
...
INT UCookPackagesCommandlet::Main( const FString& Params )
{
	...
	if (!bIniFilesOnly)
	{
		...
		TArray<FPackageCookerInfo> PackageList = GeneratePackageList( FirstRequiredIndex, FirstScriptIndex, FirstGameScriptIndex, FirstMapIndex, FirstMPMapIndex );
		...
		for( INT PackageIndex=0; PackageIndex<PackageList.Num(); PackageIndex++ )
		{
			...
			// Save the shader cache after each package has been verified.
			SaveLocalShaderCaches();

			const FFilename& SrcFilename			= PackageList(PackageIndex).SrcFilename;
			const FFilename& DstFilename			= PackageList(PackageIndex).DstFilename;
			...
			UBOOL bIsStartupPackage					= PackageList(PackageIndex).bIsStartupPackage;
			UBOOL bShouldBeFullyCompressed			= bIsNativeScriptFile && bPreloadPackagesFromMemory;
			const FString PackageName				= SrcFilename.GetBaseFilename( TRUE );
			...
			const UBOOL bStripEverythingButTextures	= PackageIndex < FirstRequiredIndex;
			...
			// Load package and keep track of time spent loading.
			DOUBLE LoadStartTime = appSeconds();
			warnf( NAME_Log, TEXT("Loading %s"), *SrcFilename );
			UPackage* Package = Cast<UPackage>(UObject::LoadPackage( NULL, *SrcFilename, LOAD_None ));
			PackageLoadTime += appSeconds() - LoadStartTime;
			...
			if( Package )
			{
				...
				// make a local shader cache in each of the seekfree and startup packages
				if (bShouldBeSeekFree || bIsStartupPackage)
				{
				...
				if( World && World->PersistentLevel )
				{
					...
					debugf( NAME_DevCooking, TEXT("COOKEDPHYSICS: Brush: %f KB"), ((FLOAT)BrushPhysDataBytes)/1024.f);
				}
				...
				{
					SaveCookedPackage( Package, World, RF_Standalone, *DstFilename, bStripEverythingButTextures );
				}
				...
				if( bShouldBeSeekFree )
				{
					...
					for( FObjectIterator It; It; ++It )
					{
						...
						// Avoid object from being put into subsequent script or map packages.
						if( bIsNativeScriptFile && Object->HasAnyFlags( RF_Saved ) )
						{
							Object->SetFlags( RF_MarkedByCooker );
						}
						...
						if( bIsNativeScriptFile && ShaderCache && Material )
						{
							AlreadyHandledMaterials.Set( Material, TRUE );
						}
					}
		...
		// Print out detailed timing stats.
		warnf( NAME_Log, TEXT("Cooking Textures took : %6.2f seconds"),TextureCookTime);
		warnf( NAME_Log, TEXT("Cooking Sounds took   : %6.2f seconds"),SoundCookTime);
		warnf( NAME_Log, TEXT("PrepareForSaving took : %6.2f seconds"),PrepareForSavingTime);
		warnf( NAME_Log, TEXT("Loading packages took : %6.2f seconds"),PackageLoadTime);
		warnf( NAME_Log, TEXT("Saving packages took  : %6.2f seconds"),PackageSaveTime);
