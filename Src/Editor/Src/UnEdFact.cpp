...
UObject* UTextureCubeFactoryNew::FactoryCreateNew( UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn )
{
...
/*	@todo FIXME RON - temporarily commented out because of UTGame script compile loaded
...
)
{
	...
	while( ParseLine(&Buffer,StrLine) )
	{
		...
		else if( GetBEGIN(&Str,TEXT("SURFACE")) )
		{
			...
			UMaterialInstance* SrcMaterial = NULL;
			...
			do
			{
				if( ParseCommand(&Buffer,TEXT("TEXTURE")) )
				{
					...
					{
						SrcMaterial = Cast<UMaterialInstance>(StaticLoadObject( UMaterialInstance::StaticClass(), NULL, *TextureName, NULL, LOAD_NoWarn, NULL ));
					}
					Check++;
				}
			...
			if( Check == 6 )
			{
				...
				for( INT i = 0 ; i < GWorld->PersistentLevel->Model->Surfs.Num() ; i++ )
				{
					...
					if( DstSurf->PolyFlags & PF_Selected )
					{
						...
						DstSurf->pBase = GEditor->bspAddPoint( GWorld->PersistentLevel->Model, &NewBase, 1 );
						DstSurf->vTextureU = GEditor->bspAddVector( GWorld->PersistentLevel->Model, &NewTextureU, 0 );
						DstSurf->vTextureV = GEditor->bspAddVector( GWorld->PersistentLevel->Model, &NewTextureV, 0 );
	...
	for( FActorIterator It; It; ++It )
	{
		...
		if( bActorChanged && Brush && Brush->Brush )
		{
			...
			if( bIsStaticBrush )
			{
				GEditor->bspValidateBrush( Brush->Brush, TRUE, FALSE );
			}
			else
			{
				GEditor->RebuildBrush( Brush->Brush );
			}
		}
...
struct FASEMaterial
{
	...
	FASEMaterial( const TCHAR* InName, INT InWidth, INT InHeight, FLOAT InUTiling, FLOAT InVTiling, UMaterialInstance* InMaterial, UBOOL InTwoSided, UBOOL InUnlit, UBOOL InAlphaTexture, UBOOL InMasked, UBOOL InTranslucent )
	{
 ...
 * is initialized against its archetype, but before any objects of this class are created.
 */
void USoundFactory::InitializeIntrinsicPropertyValues()
{
	SupportedClass = USoundNodeWave::StaticClass();
	new(Formats)FString(TEXT("wav;Sound"));
	new(Formats)FString(TEXT("mp3;Sound"));
	bCreateNew = 0;
...
{
	if(	appStricmp(FileType, TEXT("WAV"))==0 || appStricmp(FileType, TEXT("MP3"))==0 )
	{
		...
		Sound->Duration = Audio_GetDuration( Buffer, BufferEnd - Buffer );		
		
		// if we're auto creating a default cue
		if( bAutoCreateCue )
		{
			// then first create the actual sound cue
			FString SoundCueName = FString::Printf( TEXT("%sCue"), *Name.ToString() );
...
USoundSurroundFactory::USoundSurroundFactory()
{
	bEditorImport = TRUE;
}
...
)
{
	...
	{
		DecoderClass = UObject::StaticLoadClass( UCodecMovie::StaticClass(), NULL, TEXT("Engine.CodecMovieTheora"), NULL, LOAD_None, NULL );
	}
	else
	{
		// Unknown format.
...
void UTextureFactory::InitializeIntrinsicPropertyValues()
{
	...
	new(Formats)FString(TEXT("pnga;Texture"));		// blue값을 존재하는 텍스쳐의 alpha로 설정해준다.
	//}
	bCreateNew = 0;
	bEditorImport = 1;
}
...
)
{
	...
	if( appStricmp( Type, TEXT("FLOAT") ) == 0 )
	{
		...
		{
			//{{ 2006. 7. 19 changmin.
			// test for 1024x512 mode
			SizeX = 1024;
			SizeY = 512;

			if( SizeX * SizeY * SrcComponents * sizeof( FLOAT ) != Length )
			{
				// test for 2048x1024 mode
				SizeX = 2048;
				SizeY = 1024;
	...
	// PNG File
	//
	else if( Length >= sizeof(FPNGHeader) && appMemcmp(PNG->IHDR, (void*)"IHDR", 4) == 0 )
	{
		...
		// RGB거 아니면 무조건 리턴.
		if ( PNG->ColorType != PNG_COLOR_TYPE_RGB && PNG->ColorType != PNG_COLOR_TYPE_GRAY )
			return NULL;
...
void UDSPPresetFactoryNew::StaticConstructor()
{
	new(GetClass()->HideCategories) FName(TEXT("Object"));
}
...
* is initialized against its archetype, but before any objects of this class are created.
*/
void UDSPPresetFactoryNew::InitializeIntrinsicPropertyValues()
{
	SupportedClass	= UDSPPreset::StaticClass();
	bCreateNew		= 1;
	Description		= TEXT("DSP Preset");
}
//
//	UCurveEdPresetCurveFactoryNew::FactoryCreateNew
//
UObject* UDSPPresetFactoryNew::FactoryCreateNew(UClass* Class,UObject* InParent,FName Name,EObjectFlags Flags,UObject* Context,FFeedbackContext* Warn)
{
	return StaticConstructObject(Class,InParent,Name,Flags);
}

IMPLEMENT_CLASS(UDSPPresetFactoryNew);

void UReverbPropertyFactoryNew::StaticConstructor()
{
	new(GetClass()->HideCategories) FName(TEXT("Object"));
}
...
* is initialized against its archetype, but before any objects of this class are created.
*/
void UReverbPropertyFactoryNew::InitializeIntrinsicPropertyValues()
{
	SupportedClass	= UReverbProperty::StaticClass();
	bCreateNew		= 1;
	Description		= TEXT("Reverb Preset");
}
//
//	UCurveEdPresetCurveFactoryNew::FactoryCreateNew
//
UObject* UReverbPropertyFactoryNew::FactoryCreateNew(UClass* Class,UObject* InParent,FName Name,EObjectFlags Flags,UObject* Context,FFeedbackContext* Warn)
{
	return StaticConstructObject(Class,InParent,Name,Flags);
}

IMPLEMENT_CLASS(UReverbPropertyFactoryNew);

void UTrueTypeFontFactoryNew::StaticConstructor()
{
	new(GetClass()->HideCategories) FName(TEXT("Object"));

	new(GetClass(),TEXT("FontResource"),      RF_Public)UStrProperty  (CPP_PROPERTY(FontResource     ), TEXT(""), CPF_Edit );
	new(GetClass(),TEXT("Height"),            RF_Public)UIntProperty  (CPP_PROPERTY(Height			 ), TEXT(""), CPF_Edit );
	new(GetClass(),TEXT("bAntialised"),       RF_Public)UBoolProperty (CPP_PROPERTY(bAntialiased	 ), TEXT(""), CPF_Edit );
	new(GetClass(),TEXT("bPrecacheAlphabets"),RF_Public)UBoolProperty (CPP_PROPERTY(bPrecacheAlphabets), TEXT(""), CPF_Edit );	
}
...
* is initialized against its archetype, but before any objects of this class are created.
*/
void UTrueTypeFontFactoryNew::InitializeIntrinsicPropertyValues()
{
	bCreateNew			= 1;
	// don't allow importing
	bEditorImport		= 0;

	SupportedClass	= UTrueTypeFont::StaticClass();	
	Description		= TEXT("True Type Font");
...
//	UCurveEdPresetCurveFactoryNew::FactoryCreateNew
//
UObject* UTrueTypeFontFactoryNew::FactoryCreateNew(UClass* Class,UObject* InParent,FName Name,EObjectFlags Flags,UObject* Context,FFeedbackContext* Warn)
{
	UTrueTypeFont* Font = Cast<UTrueTypeFont>( StaticConstructObject(Class,InParent,Name,Flags) );

	if (Font)
	{
		Font->FontResource = FontResource;
		Font->Height = Height;
		Font->Quality = bAntialiased ? TTQ_Antialiased : TTQ_Default;
		Font->bPrecacheAlphabets = bPrecacheAlphabets;
	}

	return Font;
}

IMPLEMENT_CLASS(UTrueTypeFontFactoryNew);

/*------------------------------------------------------------------------------
UReverbPropertyFactory implementation.
------------------------------------------------------------------------------*/
/**
* Initializes property values for intrinsic classes.  It is called immediately after the class default object
* is initialized against its archetype, but before any objects of this class are created.
*/
void UReverbPropertyFactory::InitializeIntrinsicPropertyValues()
{
	SupportedClass = UReverbProperty::StaticClass();
	new(Formats)FString(TEXT("reverb;Unreal ReverbProperty"));
	bCreateNew = 0;
	bText = 1;
	bEditorImport = 1;
}
UReverbPropertyFactory::UReverbPropertyFactory()
{
	bEditorImport = 1;
}

/**
*	Create a UReverbProperty from text.
*
*	@param	InParent	Usually the parent ReverbProperty, but might be a package for example. Used as outer for created ReverbPropertyObjects.
*	@param	Flags		Flags used when creating ReverbPropertyObjects
*	@param	Type		If "paste", the newly created ReverbPropertyObjects are added to the selected object set.
*	@param	Buffer		Text buffer with description of ReverbProperty
*	@param	BufferEnd	End of text info buffer
*	@param	Warn		Device for squirting warnings to
*
*	
*	Note that we assume that all subobjects of a UReverbProperty are ReverbPropertyObjects, and will be explicitly added to its ReverbPropertyObjects array ( ReverbPropertyObjects(x)=... lines are ignored )
*	This is because objects may already exist in parent ReverbProperty etc. It does mean that if we ever add non-ReverbPropertyObject subobjects to ReverbProperty it won't work. Hmm..
*/
UObject* UReverbPropertyFactory::FactoryCreateText
(
 UClass*				UnusedClass,
 UObject*			InParent,
 FName				UnusedName,
...
{
	UReverbProperty* ParentReverb = Cast<UReverbProperty>(InParent);

	// We keep a mapping of new, empty ReverbProperty objects to their property text.
	// We want to create all new ReverbPropertyObjects first before importing their properties (which will create links)
	TArray<UReverbProperty*>		NewReverbObjs;
	TMap<UReverbProperty*,FString>	PropMap;
	TMap<UReverbProperty*,FString>		SubObjMap;

	UBOOL bIsPasting = (appStricmp(Type,TEXT("paste"))==0);

	UReverbProperty* OutReverbProperty = NULL;
	...
	while( ParseLine(&Buffer,StrLine) )
	{
		...
		if( GetBEGIN(&Str,TEXT("OBJECT")) )
		{
			UClass* ReverbObjClass;
			if( ParseObject<UClass>( Str, TEXT("CLASS="), ReverbObjClass, ANY_PACKAGE ) )
			{
				//check( ReverbObjClass->IsChildOf(UReverbProperty::StaticClass()) );
				if (!ReverbObjClass->IsChildOf(UReverbProperty::StaticClass()))
				{
					continue;
				}

				FName ReverbObjName(NAME_None);
				Parse( Str, TEXT("NAME="), ReverbObjName );				

				UReverbProperty* NewReverbObj = ConstructObject<UReverbProperty>( ReverbObjClass, InParent, ReverbObjName, Flags );				

				// Get property text for the new ReverbProperty object.
				FString PropText, PropLine;
				FString SubObjText;
				INT ObjDepth = 1;
				while ( ParseLine( &Buffer, PropLine ) )
				...
				// Save property text and possibly sub-object text in the case of sub-ReverbProperty.
				PropMap.Set( NewReverbObj, *PropText );
				NewReverbObjs.AddItem(NewReverbObj);				
			}
		}
	}
	...
	if(bIsPasting)
	{
		GEditor->GetSelectedObjects()->SelectNone( UReverbProperty::StaticClass() );
	}

	for(INT i=0; i<NewReverbObjs.Num(); i++)
	{
		UReverbProperty* NewReverbObj = NewReverbObjs(i);
		FString* PropText = PropMap.Find(NewReverbObj);
		check(PropText); // Every new object should have property text.		

		// Now import properties into newly created ReverbProperty object. Should create links correctly...
		const TCHAR* PropStr = **PropText;

		//debugf( TEXT("-----PROP STRING------") );
		//debugf( TEXT("%s"), PropStr );

		NewReverbObj->PreEditChange(NULL);

		ImportObjectProperties( (BYTE*)NewReverbObj, PropStr, NewReverbObj->GetClass(), NewReverbObj, InParent, Warn, 0 );				
	}
	...
	for(INT i=0; i<NewReverbObjs.Num(); i++)
	{
		...
		// If this is a paste, add the newly created ReverbProperty objects to the selection list.
		if(bIsPasting)
		{
			GEditor->GetSelectedObjects()->Select(NewReverbObj);
		}

		NewReverbObj->PostEditChange(NULL);
	}

	return NewReverbObjs.Num() ? NewReverbObjs(0) : NULL;
}

IMPLEMENT_CLASS(UReverbPropertyFactory);

/*------------------------------------------------------------------------------
UUIAnimationSeqFactoryNew implementation.
------------------------------------------------------------------------------*/

//
//	UUIAnimationSeqFactoryNew::StaticConstructor
//
void UUIAnimationSeqFactoryNew::StaticConstructor()
{
	new(GetClass()->HideCategories) FName(TEXT("Object"));
}
...
* is initialized against its archetype, but before any objects of this class are created.
*/
void UUIAnimationSeqFactoryNew::InitializeIntrinsicPropertyValues()
{
	SupportedClass	= UUIAnimationSeq::StaticClass();
	bCreateNew		= 1;
	Description		= TEXT("UIAnimationSeq");
}
//
//	UUIAnimationSeqFactoryNew::FactoryCreateNew
//
UObject* UUIAnimationSeqFactoryNew::FactoryCreateNew(UClass* Class,UObject* InParent,FName Name,EObjectFlags Flags,UObject* Context,FFeedbackContext* Warn)
{
	UUIAnimationSeq* AnimSeq = Cast<UUIAnimationSeq>(StaticConstructObject(Class,InParent,Name,Flags));
	if( AnimSeq )
	{
		AnimSeq->SeqName = Name;
	}

	return AnimSeq;
}

IMPLEMENT_CLASS(UUIAnimationSeqFactoryNew);