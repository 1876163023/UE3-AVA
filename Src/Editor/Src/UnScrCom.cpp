...
INT FScriptCompiler::EmitOuterContext()
{
	...
	WORD wSize=5;		// hardcoded value: since we're always emiting Outer here, we shouldn't have to worry about this ever changing.
	Writer << wSize;
		...
		if ( !NoConsts )
		{
			...
			if( PeekSymbol(TEXT("'")) )
			{
					...
					&&	MatchSymbol(TEXT("'")) )
				{
					...
					// the literal object reference.....otherwise we can end up in a situtation where script that compiled fine suddenly fails to compile if
			...
			if( OperLinks.Num()>0 && Precedence<MaxPrecedence )
			{
				...
				if( NumParms==3 || IsPreOperator )
				{
					...
					if( NewResultType.Type == CPT_None )
						appThrowf( TEXT("Bad or missing expression after '%s'"), *OperToken.TokenName.ToString() );
				}
 ...
 )
{
	...
	for( ; ; )
	{
		...
		// {{ 20070319 dEAthcURe
		//#ifdef EnableHostMigration // 비활성상태로둘것
		else if( Specifier.Matches( TEXT("hmserialize") ))
		{
			Flags      |= CPF_HmSerialize;
			IsVariable  = TRUE;
		}
		//#endif // 비활성상태로둘것
		// }} 20070319 dEAthcURe
		...
		//#ifdef EnableCipherData // 비활성상태로둘것
		else if( Specifier.Matches( TEXT("cipher") ))
		{
			Flags      |= CPF_Cipher;
			IsVariable  = TRUE;
		}
 ...
 )
{
	...
	else if( !Skip )
	{
		...
		{
			if ( Class->HasAnyClassFlags(CLASS_Native) && Scope->Children == NULL )
			{
				UStruct* SuperScope = Scope->GetSuperStruct();
				while ( SuperScope != NULL && SuperScope->Children == NULL )
				{
...
void FScriptCompiler::CompileCommand( FToken& Token, UBOOL& NeedSemicolon )
{
	...
	else if( Token.Matches(NAME_ForEach) )
	{
		...
		GetToken(TypeToken);
		UArrayProperty* ArrayProp = Cast<UArrayProperty>(FindField(TopNode, TypeToken.Identifier, TRUE, UArrayProperty::StaticClass(), NULL, NULL), CLASS_IsAUProperty);
		if (ArrayProp != NULL)
		{
			// write out iterator op
			Writer << EX_DynArrayIterator;

			// compile the array prop
			FToken ResultToken;
			FPropertyBase Required(ArrayProp->Inner,CPRT_SimpleReference);
			CompileFieldExpr(TopNode,Required,TypeToken,ResultToken,0,(TopNest->Allow&ALLOW_Instance)!=0 );

			RequireSymbol(TEXT("("), TEXT("'foreach array'"));
			// out item
			CompileExpr( FPropertyBase(CPT_None,CPRT_AssignValue), TEXT("'foreach array'"), NULL, MAXINT, NULL, FALSE, NULL, TRUE );
			BYTE IndexByte = 0;
			if (MatchSymbol(TEXT(",")))
			{
				// index is present
		...
		else
		{
			...
			SupressDebugInfo++;
			CompileExpr( FPropertyBase(CPT_None,CPRT_AssignmentReference), TEXT("'ForEach'"), NULL, MAXINT, NULL, FALSE, NULL, TRUE );
