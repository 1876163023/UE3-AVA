	...
	if( GEditor->bShowBrushMarkerPolys )
	{
		...
		for( FActorIterator It; It; ++ It )
		{
			...
			if( Brush && Brush->Brush && (Brush->IsABuilderBrush() || Brush->IsVolumeBrush()) && GEditor->GetSelectedActors()->IsSelected(Brush) )
			{
				...
				MeshBuilder.Draw(PDI, Brush->LocalToWorld(), new(GEngineMem) FColoredMaterialInstance(GEngine->EditorBrushMaterial->GetInstanceInterface(FALSE),Brush->GetWireColor()), SDPG_World);
			}
		}
	}
		...
		if( ClipMarkers.Num() > 1 )
		{
			...
			if( bDrawOK && !NormalPoly.CalcNormal(1) )
			{
				...
				// Draw a triangle.
				{
					...
					MeshBuilder.Draw(PDI, FMatrix::Identity, new(GEngineMem) FColoredMaterialInstance(GEngine->EditorBrushMaterial->GetInstanceInterface(FALSE),GEngine->C_BrushWire), SDPG_World);
				}
			}
		}
	}
...
{
	// Selected
	FMaterialInstance* SelectedColorInstance = new(GEngineMem) FColoredMaterialInstance(GEngine->GeomMaterial->GetInstanceInterface(FALSE),FColor(255,128,64));
	...
	for( INT ObjectIdx = 0 ; ObjectIdx < GeomObjects.Num() ; ++ObjectIdx )
	{
		...
		FMaterialInstance* UnselectedColorInstance = new(GEngineMem) FColoredMaterialInstance(GEngine->GeomMaterial->GetInstanceInterface(FALSE),FLinearColor(GeomObject->GetActualBrush()->GetWireColor()) * 0.5f);
...
void FEdModeTerrainEditing::DrawTool( FPrimitiveDrawInterface* PDI, class ATerrain* Terrain, 
	FVector& Location, FLOAT InnerRadius, FLOAT OuterRadius, TArray<ATerrain*>& Terrains )
{
	FTerrainToolSettings* ToolSettings = (FTerrainToolSettings*)(GetSettings());
	if (bVertexEditing == TRUE)
	{
		DrawToolGrid(PDI, Terrain, Location, InnerRadius, OuterRadius, Terrains);
		DrawToolGridBallAndSticks(PDI, Terrain, Location, InnerRadius, OuterRadius, Terrains);
	}
	else
	{

		DrawToolCircle(PDI, Terrain, Location, InnerRadius, Terrains);
		if (OuterRadius != InnerRadius)
		{
			DrawToolCircle(PDI, Terrain, Location, OuterRadius, Terrains);
		}
		// Draw the 'ball' at the exact vertex...
		DrawToolCircleBallAndSticks(PDI, Terrain, Location, InnerRadius, OuterRadius, Terrains);
		...
		if ((TerrainTool->SupportsMirroring() == TRUE) && (ToolSettings->MirrorSetting != FTerrainToolSettings::TTMirror_NONE))
		{
            ...
            DrawToolCircle(PDI, Terrain, MirrorWorld, InnerRadius, Terrains, TRUE);
			if (OuterRadius != InnerRadius)
			{
				DrawToolCircle(PDI, Terrain, MirrorWorld, OuterRadius, Terrains, TRUE);
			}
			// Draw the 'ball' at the exact vertex...
			DrawToolCircleBallAndSticks(PDI, Terrain, MirrorWorld, InnerRadius, OuterRadius, Terrains);
		}
	}
}

#define CIRCLESEGMENTS 16
void FEdModeTerrainEditing::DrawToolCircle( FPrimitiveDrawInterface* PDI, ATerrain* Terrain, FVector& Location, FLOAT Radius, TArray<ATerrain*>& Terrains, UBOOL bMirror )
{
	FTerrainToolSettings* ToolSettings = (FTerrainToolSettings*)(GetSettings());
    ...
    for( INT s=0;s<=CIRCLESEGMENTS;s++ )
	{
		...
		if (!GWorld->SingleLineCheck(Result,NULL,TraceEnd,TraceStart,TRACE_Terrain|TRACE_TerrainIgnoreHoles))
		{
...
void FEdModeTerrainEditing::DrawToolCircleBallAndSticks( FPrimitiveDrawInterface* PDI, class ATerrain* Terrain, FVector& Location, FLOAT InnerRadius, FLOAT OuterRadius, TArray<ATerrain*>& Terrains, UBOOL bMirror )
{
	FVector OutVertex;
	...
	for (INT TerrainIndex = 0; TerrainIndex < Terrains.Num(); TerrainIndex++)
	{
		...
		if (CheckTerrain->GetClosestVertex(Location, OutVertex) == TRUE)
		{
			//@todo. Scale the size of the ball based on the screen-space size??
			FVector Start = OutVertex;
			FVector End = OutVertex + FVector(0.0f, 0.0f, 40.0f);
			FLinearColor Color(1.0f, 0.0f, 0.0f, 1.0f);
			PDI->DrawLine(Start, End, Color, SDPG_Foreground);
			DrawWireStar(PDI, End, 20.0f, FColor(255,0,0), SDPG_Foreground);
		}
	}
}

void FEdModeTerrainEditing::DrawToolGrid( FPrimitiveDrawInterface* PDI, class ATerrain* Terrain, FVector& Location, FLOAT InnerRadius, FLOAT OuterRadius, TArray<ATerrain*>& Terrains, UBOOL bMirror )
{
}

void FEdModeTerrainEditing::DrawToolGridBallAndSticks( FPrimitiveDrawInterface* PDI, class ATerrain* Terrain, FVector& Location, FLOAT InnerRadius, FLOAT OuterRadius, TArray<ATerrain*>& Terrains, UBOOL bMirror )
{
}
...
//			ToolTerrains.AddItem(Terrain);
			ToolTerrains.AddUniqueItem(Terrain);

			if(Terrain)
			{
				DrawTool(PDI,Terrain,ToolHitLocation,CurrentSettings->RadiusMin,CurrentSettings->RadiusMax,ToolTerrains);

				// Allow the tool to render anything custom.
				((FModeTool_Terrain*)GEditorModeTools().GetCurrentTool())->Render( Terrain, ToolHitLocation, PDI );
			}
		}
	}
}
...
UBOOL FEdModeCoverEdit::InputKey(FEditorLevelViewportClient* ViewportClient,FViewport* Viewport,FName Key,EInputEvent Event)
{
	...
	else if (Key == KEY_MiddleMouseButton)
	{
		if (Event == IE_Pressed)
		{
			...
			PathBuilder.SetPathCollision(TRUE);
			...
			PathBuilder.SetPathCollision(FALSE);
...
void FEdModeCoverEdit::DrawHUD(FEditorLevelViewportClient* ViewportClient,FViewport* Viewport,const FSceneView* View,FCanvas* Canvas)
{
	...
	FColor DrawColor(255,255,255,255);
}
	...
	switch( CoordSystem )
	{
		case COORD_Local:
		{
			...
			if( !GetCurrentMode()->GetCustomDrawingCoordinateSystem( matrix, NULL ) )
			{
				...
				if( Num == 1 )
				{
					matrix = FRotationMatrix( GEditor->GetSelectedActors()->GetTop<AActor>()->Rotation );
				}
				else
				{
