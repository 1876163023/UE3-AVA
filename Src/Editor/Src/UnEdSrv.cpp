...
UBOOL UEditorEngine::Exec_StaticMesh( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("SAVEBRUSHASCOLLISION")) )
	{
		...
		GEditor->bspBuild(TempModel,BSP_Good,15,70,1,0);
		GEditor->bspRefresh(TempModel,1);
		GEditor->bspBuildBounds(TempModel);
...
UBOOL UEditorEngine::Exec_Brush( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if (ParseCommand(&Str,TEXT("ADD")))
	{
		ABrush* NewBrush = NULL;
		{
			...
			NewBrush = csgAddOperation( GWorld->GetBrush(), DWord1, CSG_Add );
			if( NewBrush )
			{
	...
	else if (ParseCommand(&Str,TEXT("SUBTRACT"))) // BRUSH SUBTRACT
	{
		ABrush* NewBrush = NULL;
		{
			...
			FinishAllSnaps();
			NewBrush = csgAddOperation(GWorld->GetBrush(),0,CSG_Subtract); // Layer
			if( NewBrush )
			{
	...
	else if( ParseCommand (&Str,TEXT("LOAD")) ) // BRUSH LOAD
	{
		if( Parse( Str, TEXT("FILE="), TempFname, 256 ) )
		{
			...
			bspValidateBrush( GWorld->GetBrush()->Brush, 0, 1 );
	...
	else if( ParseCommand(&Str,TEXT("MERGEPOLYS")) ) // BRUSH MERGEPOLYS
	{
		...
		for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
		{
			...
			{
				ABrush* Brush = static_cast<ABrush*>( Actor );
				bspValidateBrush( Brush->Brush, 1, 1 );
				Brush->MarkPackageDirty();
				LevelDirtyCallback.Request();
			}
		}
...
void UEditorEngine::RebuildBSP()
{
	...
	bspBuild( GWorld->GetModel(), BSP_Optimal, 15, 70, 0, 0 );
	...
	GWorld->GetWorldInfo()->SetMapNeedsGeometryFullyRebuilt( TRUE );
	//>@ ava
}
...
UBOOL UEditorEngine::Map_Rebuild(const TCHAR* Str, FOutputDevice& Ar)
{
	...
	//<@ ava specific ; 2007. 2. 5 changmin
	bFullBuildGeometry = FALSE;
	PvsComputeOptions = NULL;
	RebuildMapAndVis();
	GWorld->GetWorldInfo()->SetMapNeedsGeometryFullyRebuilt( FALSE );
	//>@ ava

	return TRUE;
}
//<@ ava specific ; 2007. 1. 2. changmin
void UEditorEngine::FullBuildGeometry( const FFullBuildGeometryOptions &Options, UBOOL bComputePVS )
{
	bFullBuildGeometry = TRUE;

	if( bComputePVS )
		PvsComputeOptions = &Options;
	else
		PvsComputeOptions = NULL;
...
void UEditorEngine::RebuildMapAndVis()
{
	const FScopedBusyCursor BusyCursor;

	FlushRenderingCommands();

	Trans->Reset( *LocalizeUnrealEd(TEXT("FullBuildGeometryMsg")) );
	GWarn->BeginSlowTask( *LocalizeUnrealEd(TEXT("FullBuildGeometryMsg")), 1);

	AWorldInfo*	WorldInfo = GWorld->GetWorldInfo();
	if ( WorldInfo->bPathsRebuilt )
	{
		debugf(TEXT("Full Build Geometry Clear paths rebuilt"));
	}
...
void UEditorEngine::CheckForTrashcanReferences(const TArray<UPackage*>& Packages, TArray<UObject*>& OutObjects, TArray<UObject*>& OutTrashcanObjects)
{
	...
	for ( TObjectIterator<UObject> It ; It ; ++It )
	{
		...
		for ( INT PackageIndex = 0 ; PackageIndex < Packages.Num() ; ++PackageIndex )
		{
			...
			if ( (! (Object->GetFlags() & RF_Transient) ) && Object->IsIn( SelectedPackage ) )
			{
...
public:
	...
	*/
	void MoveActorsToLevel(TArray<AActor*>& OutNewActors, TArray<APrefabInstance*>& OutNewPrefabInstances, ULevel* DestLevel, ULevel* BufferLevel)
	{
		...
		GEditor->edactCopySelected( FALSE );
		GEditor->edactDeleteSelected();

		if ( BufferLevel )
		{
			// Paste to the buffer level.
			GWorld->CurrentLevel = BufferLevel;
			GEditor->edactPasteSelected( TRUE, FALSE );
	...
	if ( CopyJobs.Num() > 0 )
	{
...

		{
			...
			for ( CopyJobMap::TIterator It( CopyJobs ) ; It ; ++It )
			{
				...
				{
					Job->MoveActorsToLevel( NewActors, NewPrefabInstances, DestLevel, BufferLevel );
				}
			}
	...
	for ( INT BrushIndex = 0 ; BrushIndex < Brushes.Num() ; ++BrushIndex )
	{
		...
		for ( INT PolyIndex = 0 ; PolyIndex < Actor->Brush->Polys->Element.Num() ; ++PolyIndex )
		{
			...
			else
			{
				...
				UMaterialInstance*& ReferencedMaterial = Actor->Brush->Polys->Element(PolyIndex).Material;
 ...
 */
UBOOL UEditorEngine::Map_Check(const TCHAR* Str, FOutputDevice& Ar, UBOOL bCheckDeprecatedOnly)
{
	const FScopedBusyCursor BusyCursor;
...
UBOOL UEditorEngine::Exec_Poly( const TCHAR* Str, FOutputDevice& Ar )
{
	if( ParseCommand(&Str,TEXT("SELECT")) ) // POLY SELECT [ALL/NONE/INVERSE] FROM [LEVEL/SOLID/GROUP/ITEM/ADJACENT/MATCHING]
	{
		...
		else if( ParseCommand(&Str,TEXT("MATCHING")) )
		{
			...
			else if( ParseCommand(&Str,TEXT("SELECTEDMATERIAL")))
			{
				Ava_polySelectMatchingSelectedMaterial( FALSE );
			}
			//>@ ava
			GCallbackEvent->Send( CALLBACK_SelChange );
			return TRUE;
...
UBOOL UEditorEngine::Exec_Obj( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( ParseCommand( &Str, TEXT( "SavePackage" ) ) )
	{
		...
		if( Parse( Str, TEXT( "FILE=" ), TempFname, 256 ) && ParseObject<UPackage>( Str, TEXT( "Package=" ), Pkg, NULL ) )
		{
			...
			{
				if( appMsgf( AMT_OK, *LocalizeUnrealEd( "Error_FilenameContainsSpaces" ) ) )
				{
					return FALSE;
				}
			}
...
UBOOL UEditorEngine::MoveActorToFloor( AActor* InActor, UBOOL InAlign )
{
	...
	if( !GWorld->SingleLineCheck( Hit, InActor, StartLocation + Direction*WORLD_MAX, StartLocation, TRACE_World, Extent ) )
	{
		...
		GWorld->FarMoveActor( InActor, NewLocation );
		if( InAlign )
		{
...
UBOOL UEditorEngine::Exec( const TCHAR* Stream, FOutputDevice& Ar )
{
	...
	else if (ParseCommand(&Str,TEXT("MAP")))
	{
		...
		{
			return Map_Check( Str, Ar );
		}
		else if (ParseCommand (&Str,TEXT("CHECKDEP")))
		{
			return Map_Check( Str, Ar, TRUE );
		}
		else if (ParseCommand (&Str,TEXT("SCALE")))
		{
