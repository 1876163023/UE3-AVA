...
BEGIN_COMMANDLET(AnalyzeReferencedContent,Editor)
	...
	struct FMaterialStats
	{
		...
		/** Emissive no lightmap instruction count.										*/
		INT NumInstructionsEmissiveNoLightmap;
		/** Emissive with vertex lightmap instruction count.							*/
		INT NumInstructionsEmissiveAndLightmap;
	...
	/** Shader type for translucency pixel shader. */
	FShaderType*	ShaderTypeTranslucent;
	/** Shader type for additive pixel shader. */
	FShaderType*	ShaderTypeAdditive;
	/** Shader type for modulate pixel shader. */
	FShaderType*	ShaderTypeModulate;
	/** Shader type for emissive pixel shader (no lightmap).  */
	FShaderType*	ShaderTypeEmissiveNoLightmap;
	/** Shader type for emissive pixel shader (including lightmap). */
	FShaderType*	ShaderTypeEmissiveAndLightmap;
...
/** DLL handle to platform-specific xxxTools.dll												*/
	...
	/** Time spent cooking textures.															*/
	DOUBLE							TextureCookTime;
	/** Time spent cooking sounds.																*/
	DOUBLE							SoundCookTime;
	/** Time spent in PrepareForSaving function.												*/
	DOUBLE							PrepareForSavingTime;
	/** Time spent loading packages.															*/
	DOUBLE							PackageLoadTime;
	/** Time spent saving packages.																*/
	...
	struct FPackageCookerInfo
	{
		...
		UBOOL bIsStartupPackage;

		/** Constructor, initializing member variables with passed in ones */
		FPackageCookerInfo( const TCHAR* InSrcFilename, const TCHAR* InDstFilename, UBOOL InbShouldBeSeekFree, UBOOL InbIsNativeScriptFile, UBOOL InbIsStartupPackage )
		...
		,	bIsStartupPackage( InbIsStartupPackage )
		{}
	};
	...
	*/
	 ...
	 */
	TArray<FPackageCookerInfo> GeneratePackageList( INT& FirstRequiredIndex, INT& FirstScriptIndex, INT& FirstGameScriptIndex, INT& FirstMapIndex, INT& FirstMPMapIndex );
	/**
	 ...
	 */
	void SaveCookedPackage( UPackage* Package, UObject* Base, EObjectFlags TopLevelFlags, const TCHAR* DstFilename, UBOOL bStripEverythingButTextures );
	/**
...
BEGIN_COMMANDLET(FindSpecTexturesWithoutLODBiasOfTwo,Editor)
END_COMMANDLET
...
//<@ ava specific ; 2006. 09. 09 changmin
BEGIN_COMMANDLET(RemoveUITextureMips,Editor)
END_COMMANDLET

BEGIN_COMMANDLET(UpdateCullDistances,Editor)
END_COMMANDLET
//>@ ava
