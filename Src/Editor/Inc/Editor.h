...
#include <winsock2.h> // 20060926 dEAthcURe
...
class UEditorEngine : public UEngine
{
	...
	/** Manned Precomputing flags */
	INT						PlayWorldPrecomputingFlags;
	...
	BITFIELD					bFullBuildGeometry:1;
	BITFIELD					bSolidOnly:1;
	const FFullBuildGeometryOptions	*PvsComputeOptions;
	//>@ ava
	...
	*/
	...
	//<@ ava specific ; 2007. 1. 2 changmin
	void RebuildMapAndVis();
	void FullBuildGeometry( const class FFullBuildGeometryOptions &Options, UBOOL bComputePVS );
	void AVA_CheckBrushIntersection(UBOOL bSolidBrushOnly);
	//>@ ava
	...
	*/
	 ...
	 */
	virtual void edactCopySelected(UBOOL bReselectPrefabActors) {}
	 ...
	 */
	virtual void edactPasteSelected(UBOOL bDuplicate, UBOOL bOffsetLocations) {}
	 ...
	 */
	virtual UBOOL edactDeleteSelected() { return TRUE; }
	...
	virtual void csgCopyBrush( ABrush* Dest, ABrush* Src, DWORD PolyFlags, EObjectFlags ResFlags, UBOOL NeedsPrep, UBOOL bCopyPosRotScale );
	virtual ABrush*	csgAddOperation( ABrush* Actor, DWORD PolyFlags, ECsgOper CSG );
	virtual void csgRebuild();
	virtual FString csgGetName( ECsgOper CsgOper );
	...
	virtual void Ava_polySelectMatchingSelectedMaterial( UBOOL bCurrentLevelOnly );
	//>@ ava
	...
	// bsp node를 추가할 때, cluster number를 지정할 수 있도록 확장
	virtual INT	bspAddNode( UModel* Model, INT iParent, enum ENodePlace ENodePlace, DWORD NodeFlags, FPoly* EdPoly, INT ClusterNumber = INDEX_NONE );
	//>@ ava

	//<@ ava specific ; 2007. 1. 5 changmin
	virtual void bspAddDetailToStructure( UModel *Structure, UModel *Detail );
	virtual void AssignDetailClusters( UModel *Bsp );
	virtual void FilterStructuralNodes( UModel *Bsp, UModel *Detail );
	virtual void Ava_csgRebuild();
	virtual void Ava_bspRepartition( UModel *Bsp, INT iNode, INT Simple );
	virtual void Ava_AssignClusterNumber( UModel *Structural, UModel *WorldModel );
	//>@ ava
	 ...
	 */
	virtual void RebuildBrush(UModel* Brush);
	...
	// Scripts.
	virtual int MakeScripts( UClass* BaseClass, FFeedbackContext* Warn, UBOOL MakeAll, UBOOL Booting, UBOOL MakeSubclasses, UPackage* LimitOuter = NULL, UBOOL bParseOnly=0 );
	 ...
	 * @param	bPrecomputing			manned map navigation [ precomputating step ]
	 */
	virtual void PlayMap(FVector* StartLocation = NULL, FRotator* StartRotation = NULL, INT DestinationConsole = -1, INT PrecomputingFlags = 0 );
	 ...
	 */
	virtual void PlayInEditor( INT PrecomputingFlags = 0 );
...
private:
	...
	UBOOL Map_Check(const TCHAR* Str, FOutputDevice& Ar, UBOOL bCheckDeprecatedOnly=FALSE);
...
public:
	...
	~FReimportHandler(){ if(GConfig) FReimportManager::Instance()->Handlers.RemoveItem(this); }
	/**
	* Reimports specified resource from its source, if the meta-data exists
	* @param Package texture to reimport
	* @return TRUE if handled
 ...
 */
void CookSoundNodeWave( USoundNodeWave* SoundNodeWave, UE3::EPlatformType Platform = UE3::PLATFORM_Unknown );
