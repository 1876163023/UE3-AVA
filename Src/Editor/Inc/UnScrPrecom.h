...
public:
	...
	*/
	FString CurrentLocation() const
	{
		...
		{
			return FString::Printf(TEXT("%s:%d"), *Filename, InputLine+1);
		}
		else 
		{
			return FString();
...
public:
	...
	*/
	...
	TCHAR PeekChar() const
	{
		...
		return ActiveProvider->PeekChar();
	}
...
private:
	...
	UBOOL Lookup(const FName& macroName, FScriptMacroDefinition*& definition) 
	{
		...
		FScriptMacroDefinition* Macro = GlobalSymbols.Find(macroName);
		if ( Macro == NULL )
			return false;
	...
	*/
	...
	/** Global symbol table - this holds all the macro definitions available to the current uc file */
	TMap<FName, FScriptMacroDefinition> GlobalSymbols;
};
