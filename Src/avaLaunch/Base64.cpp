...
void CBase64::AllocEncode(DWORD nSize)
{
  if(m_nEBufLen < nSize)
  {
	...
	m_pEBuffer = (PBYTE)appRealloc(m_pEBuffer, m_nEBufLen);
  }
  
  appMemzero(m_pEBuffer, m_nEBufLen);
  m_nEDataLen = 0;
}
...
void CBase64::AllocDecode(DWORD nSize)
{
  if(m_nDBufLen < nSize)
  {
	  ...
	  m_pDBuffer = (PBYTE)appRealloc(m_pDBuffer, m_nDBufLen);
  }
  
  appMemzero(m_pDBuffer, m_nDBufLen);
  m_nDDataLen = 0;
}
...
{
  AllocDecode(nBufLen);
  appMemcpy(m_pDBuffer, pBuffer, nBufLen);
  m_nDDataLen = nBufLen;
}
...
void CBase64::Encode(const PBYTE pBuffer, DWORD nBufLen)
{
  ...
  {
    Raw.Clear();
    appMemcpy(&Raw, m_pDBuffer + nIndex, 3);
    Raw.nSize = 3;
  ...
  if(nBufLen > nIndex)
  {
    ...
    appMemcpy(&Raw, m_pDBuffer + nIndex, nBufLen - nIndex);
    _EncodeToBuffer(Raw, m_pEBuffer + m_nEDataLen);
    m_nEDataLen += 4;
  }
}
