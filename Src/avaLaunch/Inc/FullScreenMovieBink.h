/*=============================================================================
FullscreenMovieBink.h: Fullscreen movie playback default Bink
...
/**
* Bink movie implementation. Simply clears the screen
*/
...
public:
	...
	*/
	static FFullScreenMovieSupport* StaticInitialize();
	...
	*/
	virtual void GameThreadPlayMovie(EMovieMode InMovieMode, const TCHAR* MovieFilename, INT StartFrame=0);
...
protected:
	...
	void PlayMovie( EMovieMode InMovieMode, const TCHAR* MovieFilename, INT StartFrame );
	void StopMovie( UBOOL bResetFlag );
	void WaitForMovie();	
	...
	*/
	...
	void GameThreadFetchNewMovie();
	void GameThreadInternalPlayMovie( const TCHAR* MovieFilename );

	

	
	FString PlayingMovieFilename;

	INT CurrentFrame;
	LONG PlayCount;
	INT CurrentMovieIndex;
	UBOOL bPlaying;			// Only used by RenderingThread		

	FCriticalSection* CriticalSection;

	struct FMoviePlayEntry
	{	
	...
	TArray<FMoviePlayEntry> QueuedMovie;
	
	// 재생 중이지 않다면 queue된 맨처음 것을 재생
	void EnsureMoviePlaying();

	// 현재 재생 중인 Movie를 멈추고 다음 것으로 넘어감
	void SkipMovie();
};
