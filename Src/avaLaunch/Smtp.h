...
#define MESSAGE_CHARSET _T("euc-kr") //_T("iso-8859-1")
#endif
...
{
public:
  FString Encoding;  // Content encoding.  Leave blank to let the system discover it
  FString Charset;   // Character set for text attachments
  FString ContentId; // Unique content ID, leave blank to let the system handle it
...
public: // Operators
  ...
  const CSmtpAddress& operator=(const FString& strAddress);
...
public: // Operators
  ...
  const CSmtpAttachment& operator=(const FString& strFilename);

public: // Member Variables
  FString FileName;  // Fully-qualified path and filename of this attachment
  FString AltName;   // Optional, an alternate name for the file to use when sending
...
public: // Operators
  ...
  const CSmtpMessageBody& operator=(const FString& strBody);

public: // Member Variables
  FString       Data;             // Message body;
};
...
public: // Member Variables
  ...
  FString                        Subject;          // The message subject
  TArray<CSmtpMessageBody>       Message;          // An array of message bodies
  TArray<CSmtpAddress>           CC;               // Carbon Copy recipients
  TArray<CSmtpAddress>           BCC;              // Blind Carbon Copy recipients
  TArray<CSmtpAttachment>        Attachments;      // An array of attachments
  TMap<FString,FString>          Headers;          // Optional headers to include in the message
  SYSTEMTIME                     Timestamp;        // Timestamp of the message
  MimeTypeEnum                   MimeType;         // Type of MIME message this is
  FString                        MessageId;        // Optional message ID

private: // Private Member Variables
  int                            GMTOffset;        // GMT timezone offset value

public: // Public functions
  void Parse(FString& strDest);
...
private: // Private functions to finalize the message headers & parse the message
	...
	void EncodeMessage(EncodingEnum code, FString& strMsg, FString& strMethod, LPBYTE pByte = NULL, DWORD dwSize = 0);
	void Make7Bit(FString& strDest, FString& strSrc);
  void CommitHeaders();
	void BreakMessage(FString& strDest, FString& strSrc, size_t nLength = 76);
	void EncodeQuotedPrintable(FString& strDest, FString& strSrc);
};
...
public: // Member Variables.  Feel free to modify these to change the system's behavior
  ...
  FString  m_strUser;         // Username for authentication
  FString  m_strPass;         // Password for authentication

private: // Private Member Variables
  SOCKET   m_hSocket;         // Socket being used to communicate to the SMTP server
  FString  m_strResult;       // String result from a SendCmd()
