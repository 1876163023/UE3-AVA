...
const CSmtpMessageBody& CSmtpMessageBody::operator=(const FString& strBody)
{
  Data = strBody;
  return *this;
}
...
const CSmtpAttachment& CSmtpAttachment::operator=(const FString& strFilename)
{
...
const CSmtpAddress& CSmtpAddress::operator=(const FString& strAddress)
{
  Address = strAddress;
  return *this;
}
...
void CSmtpMessage::CommitHeaders()
{
  ...
  if (Headers.FindKey(strHeader) == NULL) Headers.Set(*strHeader,*strValue);
  ...
  Headers.Set(*strHeader,*strValue);
  ...
  strValue.Empty();
  strHeader = _T("To");
  if (Recipient.Name.Len())
  {
    wsprintf(szOut,_T("\"%s\" "),*Recipient.Name);
    strValue += szOut;
  }
  if (Recipient.Address.Len())
  {
    wsprintf(szOut,_T("<%s>"),*Recipient.Address);
    strValue += szOut;
  }
  // Write out all the CC'd names
  for (n = 0;n < CC.Num();n++)
  {
    if (strValue.Len()) strValue += _T(",\r\n\t");
    if (CC(n).Name.Len())
    {
      wsprintf(szOut,_T("\"%s\" "),*CC(n).Name);
      strValue += szOut;
    }
    wsprintf(szOut,_T("<%s>"),*CC(n).Address);
    strValue += szOut;
  }
  Headers.Remove(*strHeader);
  Headers.Set(*strHeader,*strValue);
  ...
  if (Sender.Name.Len())
  {
    wsprintf(szOut,_T("\"%s\" "),*Sender.Name);
    strValue += szOut;
  }
  wsprintf(szOut,_T("<%s>"),*Sender.Address);
  strValue += szOut;
  Headers.Remove(*strHeader);
  Headers.Set(*strHeader,*strValue);
}
...
void CSmtpMessage::Parse(FString& strDest)
{
  FString strHeader;
  FString strValue;
  FString strTemp;
  FString strBoundary;  
  FString strInnerBoundary;
  TCHAR szOut[1024];
  int n;
  ...
  if (n > 1)
  {
    ...
    {
      if (Attachments.Num() == 0) MimeType = mimeAlternative;
  ...
  for (n = 0;n < Message.Num();n++)
  {
    ...
    if (strBoundary.Len() || strInnerBoundary.Len())
    {
      strDest += _T("\r\n--");
      // If we have an inner boundary, write that one.  Otherwise write the outer one
      if (strInnerBoundary.Len()) strDest += strInnerBoundary;
    ...
    if (_tcsnicmp(*Message(n).Encoding,_T("text/"),5) == 0)
    {
      wsprintf(szOut,_T(";\r\n\tcharset=\"%s\""),*Message(n).Charset);
      strDest += szOut;
    }
    strDest += _T("\r\n");
    ...
    if (Message(n).ContentId.Len())
    {
      wsprintf(szOut,_T("\r\nContent-ID: <%s>"),*Message(n).ContentId);
      strDest += szOut;
    }
    strDest += _T("\r\n\r\n");
    strDest += strValue;
  ...
  for (n = 0;n < Attachments.Num();n++)
  {
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
      ...
      pData = (LPBYTE)appMalloc(dwSize + 1);
      ...
      if (ReadFile(hFile,pData,dwSize,&dwBytes,NULL))
      {
        ...
        else pszFile = (LPTSTR)*Attachments(n).AltName;
        ...
        if (pszExt)
        {
          ...
          if(RegOpenKeyEx(HKEY_CLASSES_ROOT,pszExt,0,KEY_READ, &hKey) == ERROR_SUCCESS)
          {
			  ...
			  ULONG nBytes = dwSize * sizeof(TCHAR);
			  DWORD dwType;
			  LONG lRes;
			  lRes = RegQueryValueEx(hKey, _T("Content Type"), NULL, &dwType, reinterpret_cast<LPBYTE>(szType), &nBytes);
        ...
        if (Attachments(n).Encoding.Len())
          lstrcpy(szType,*Attachments(n).Encoding);
        ...
        {
          wsprintf(szOut,_T(";\r\n\tcharset=\"%s\""),*Attachments(n).Charset);
          strDest += szOut;
        }
        wsprintf(szOut,_T(";\r\n\tname=\"%s\"\r\n"),pszFile);
        strDest += szOut;
		...
		debugf(TEXT("%d, %s, %s, %d"), (int)Attachments(n).TransferEncoding, *strValue, *strTemp, dwSize);
        ...
        if (Attachments(n).ContentId.Len())
        {
          wsprintf(szOut,_T("\r\nContent-ID: <%s>"),*Attachments(n).ContentId);
          strDest += szOut;
        }
        strDest += _T("\r\n\r\n");
...
void CSmtpMessage::EncodeQuotedPrintable(FString& strDest, FString& strSrc)
{
  FString strTemp;
  FString strTemp2;
  LPTSTR pszTok1;
...
void CSmtpMessage::BreakMessage(FString& strDest, FString& strSrc, size_t nLength)
{
  FString strTemp = strSrc;
  FString strLine;
  LPTSTR pszTok1;
  LPTSTR pszTok2;
  LPTSTR pszBreak;
...
void CSmtpMessage::Make7Bit(FString& strDest, FString& strSrc)
{
  LPTSTR pszTok;

  strDest = strSrc;

  pszTok = (LPTSTR)*strDest;
  do
  {
...
void CSmtpMessage::EncodeMessage(EncodingEnum code, FString& strMsg, FString& strMethod, LPBYTE pByte, DWORD dwSize)
{
  FString strTemp;
  LPTSTR pszTok1;
  LPTSTR pszTok2;
  ...
  if (!pByte)
  {
    int MBLength = WideCharToMultiByte( CP_ACP, 0, *strMsg, -1, NULL, 0, NULL, NULL );
    pszBuffer = (LPSTR)appMalloc(MBLength + 1);
	WideCharToMultiByte( CP_ACP, 0, *strMsg, -1, pszBuffer, MBLength + 1, NULL, NULL );
    pByte = (LPBYTE)pszBuffer;
    dwSize = (DWORD)strlen(pszBuffer);//MBLength+1;//DWORD(strMsg.Len());
  }
  ...
  case encodeQuotedPrintable:
    ...
    strMsg = ANSI_TO_TCHAR((LPSTR)pByte);
  ...
  if (pszBuffer) appFree(pszBuffer);
...
BOOL CSmtp::Connect(LPTSTR pszServer)
{
  ...
  addr.sin_addr.s_addr = inet_addr(TCHAR_TO_ANSI(pszServer));
  if (addr.sin_addr.s_addr == INADDR_NONE)
  {
    LPHOSTENT pHost = gethostbyname(TCHAR_TO_ANSI(pszServer));
...
int CSmtp::SendCmd(LPTSTR pszCmd)
{
  ...
  if (pszCmd)
  {
    int MBLength = WideCharToMultiByte( CP_ACP, 0, pszCmd, -1, NULL, 0, NULL, NULL );
    pszBuff = (LPSTR)appMalloc(MBLength+1);
	WideCharToMultiByte( CP_ACP, 0, pszCmd, -1, pszBuff, MBLength+1, NULL, NULL );
    ...
    dwPosition = 0;
    dwLen = (DWORD)strlen(pszBuff);	//lstrlen(pszCmd);
    if (dwLen > CMD_BLOCK_SIZE) bReportProgress = TRUE;

    while (dwLen != dwPosition)
    {
      dwMax = Min<DWORD>(CMD_BLOCK_SIZE,dwLen - dwPosition);
  ...
  m_strResult = ANSI_TO_TCHAR(szResult);
  ...
  if (nRet && nRet != SOCKET_ERROR)
  {
    szResult[3] = 0;
    nRet = atoi(szResult);
    SmtpCommandResponse(pszCmd, nRet, (LPTSTR)*m_strResult);
  }
  else nRet = -1;
...
//  int nRecipients = 0;
  ...
  if (!msg.Sender.Address.Len()) return -1;
  
  // Check if we have recipients
  if (!msg.Recipient.Address.Len() && !msg.CC.Num()) return -1;
  ...
  //  if (!msg.Message.Num() && !msg.Attachments.Num()) return -1;

  // Send the sender's address
  nRet = SendFrom((LPTSTR)*msg.Sender.Address);
  if (nRet) return nRet;
  
  // If we have a recipient, send it
  nRecipientCount = 0; // Count of recipients
  if (msg.Recipient.Address.Len())
  {
    nRet = SendTo((LPTSTR)*msg.Recipient.Address);
    if (!nRet) nRecipientCount ++;
  }

  // If we have any CC's, send those.
  for (n = 0;n < msg.CC.Num();n++)
  {
    nRet = SendTo((LPTSTR)*msg.CC(n).Address);
    if (!nRet) nRecipientCount ++;
  }

  // If we have any bcc's, send those.
  for (n = 0;n < msg.BCC.Num();n++)
  {
    nRet = SendTo((LPTSTR)*msg.BCC(n).Address);
    if (!nRet) nRecipientCount ++;
  }
...
int CSmtp::SendMessage(CSmtpAddress &addrFrom, CSmtpAddress &addrTo, LPCTSTR pszSubject, LPTSTR pszMessage, LPVOID pvAttachments, DWORD dwAttachmentCount)
{
  ...
  if (!dwAttachmentCount && pvAttachments)
  {
    ...
    while (lstrlen(pszAttachments))
    {
      ...
      message.Attachments.AddItem(attach);
...
int CSmtp::SendAuthentication()
{
  ...
  while(1)
  {
    ...
    while (1)
    {
      ...
      const TCHAR *pszAuthType = &(*m_strResult)[4];

      cvt.Decode(TCHAR_TO_ANSI(pszAuthType));
      pszTemp = cvt.DecodedMessage();
      
      if (!lstrcmpiA(pszTemp,"Username:"))
        cvt.Encode(TCHAR_TO_ANSI(*m_strUser));
      else if (!lstrcmpiA(pszTemp,"Password:"))
        cvt.Encode(TCHAR_TO_ANSI(*m_strPass));
      else break;
      
      wsprintf(szMsg,_T("%s\r\n"),ANSI_TO_TCHAR(cvt.EncodedMessage()));
      nRet = SendCmd(szMsg);
