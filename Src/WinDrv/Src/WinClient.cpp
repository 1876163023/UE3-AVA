...
UWindowsClient::UWindowsClient()
{
	...
	KeyMapVirtualToName.Set(VK_XBUTTON1,KEY_XMouseButton);
	KeyMapVirtualToName.Set(VK_XBUTTON2,KEY_YMouseButton);

	KeyMapVirtualToName.Set(VK_BACK,KEY_BackSpace);
	KeyMapVirtualToName.Set(VK_TAB,KEY_Tab);
	KeyMapVirtualToName.Set(VK_CLEAR,KEY_NumPadEnter);
	...
	// [10/25/2006 YTS], mapping VK_SNAPSHOT to the Name'PrintScreen'
	KeyMapVirtualToName.Set(VK_SNAPSHOT,KEY_PrintScreen);
	...
	// unassigned 가상키코드 영역을 스크롤에 사용
	KeyMapVirtualToName.Set(0x88,KEY_MouseScrollUp);
	KeyMapVirtualToName.Set(0x89,KEY_MouseScrollDown);
...
void UWindowsClient::StaticConstructor()
{
	...
	Class->EmitObjectReference( STRUCT_OFFSET( UWindowsClient, Engine ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWindowsClient, AudioDeviceClass ) );
	Class->EmitObjectReference( STRUCT_OFFSET( UWindowsClient, AudioDevice ) );
}
			...
			while( SUCCEEDED( DirectInput8Mouse->GetDeviceData(sizeof(DIDEVICEOBJECTDATA),&Event,&Elements,0) ) && (Elements > 0) )
			{
				if ( FocusViewportClient )
				{
					...
					case DIMOFS_BUTTON4:
						...
						{
							FName Key;
							if( Event.dwOfs == DIMOFS_BUTTON0 )
								Key = KEY_LeftMouseButton;							
							else if( Event.dwOfs == DIMOFS_BUTTON1 )
								Key = KEY_RightMouseButton;
							else if( Event.dwOfs == DIMOFS_BUTTON2 )
								Key = KEY_MiddleMouseButton;
							else if( Event.dwOfs == DIMOFS_BUTTON3 )									
								Key = KEY_XMouseButton;
							else if( Event.dwOfs == DIMOFS_BUTTON4 )									
								Key = KEY_YMouseButton;

							/*if (Event.dwOfs == DIMOFS_BUTTON3)
							{
								appMsgf(AMT_OK, TEXT(" Couldn't get CenterCenter pose, this is necessary. Aborting."));
							}*/

							UBOOL bPress = (Event.dwData & 0x80) != 0;
							FocusViewportClient->InputKey( FocusViewport,0,Key,bPress ? IE_Pressed : IE_Released);
						}						
						break;
#endif
					default:
...
UBOOL GPreventTaskSwitching = FALSE;
...
LRESULT CALLBACK LowLevelKeyboardProc( INT Code, WPARAM wParam, LPARAM lParam )
{
	...
	UBOOL bControlKeyDown = GetAsyncKeyState (VK_CONTROL) >> ((sizeof(SHORT) * 8) - 1); // Disable CTRL+ESC 
	...
	if (GIsGame && IsFullscreenDevice && GWindowActive)
	{
		if ((p->vkCode == VK_LWIN) || (p->vkCode == VK_RWIN) ||		// Windows Key
			GPreventTaskSwitching &&
			(
				p->vkCode == VK_ESCAPE && bControlKeyDown ||			// CTRL+Esc
