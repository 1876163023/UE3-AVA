...
{
	INC_DWORD_STAT(STAT_RHICreateTexture);
...
FTexture3DRHIRef RHICreateTexture3D(UINT SizeX, UINT SizeY, UINT SizeZ, BYTE Format, UINT NumMips, DWORD Flags)
{
	INC_DWORD_STAT(STAT_RHICreateTexture);

	FTexture3DRHIRef Texture3D( Flags/*&TexCreate_SRGB*/ );
	VERIFYD3DRESULT(GDirect3DDevice->CreateVolumeTexture(
		SizeX,
		SizeY,
		SizeZ,
		NumMips,
		// Invalid flag - D3DUSAGE_RENDERTARGET
		//(Flags&TexCreate_ResolveTargetable) ? D3DUSAGE_RENDERTARGET : 0,
		0,
		(D3DFORMAT)GPixelFormats[Format].PlatformFormat,
		// Invalid flag - D3DPOOL_DEFAULT
		//(Flags&TexCreate_ResolveTargetable) ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED,
		D3DPOOL_MANAGED,
		Texture3D.GetInitReference(),
		NULL
		));
	return Texture3D;
}

void* RHILockTexture3D(FTexture3DRHIRef Texture, UINT MipIndex, UBOOL bIsDataBeingWrittenTo, UINT& DestRowPitch, UINT& DestSlicePitch)
{
	D3DLOCKED_BOX			LockedBox;
	DWORD					LockFlag = D3DLOCK_NOSYSLOCK;
	if( !bIsDataBeingWrittenTo )
		LockFlag |= D3DLOCK_READONLY;
	VERIFYD3DRESULT(Texture->LockBox(MipIndex,&LockedBox,NULL,LockFlag));
	DestRowPitch = LockedBox.RowPitch;
	DestSlicePitch = LockedBox.SlicePitch;

	return LockedBox.pBits;
}

void RHIUnlockTexture3D(FTexture3DRHIParamRef Texture,UINT MipIndex)
{
	VERIFYD3DRESULT(Texture->UnlockBox(MipIndex));
}
//  [2006/11/20 YTS]
...
FTextureCubeRHIRef RHICreateTextureCube( UINT Size, BYTE Format, UINT NumMips, DWORD Flags )
{
	...
	VERIFYD3DRESULT( GDirect3DDevice->CreateCubeTexture(
		...
		NumMips,
		((Flags&TexCreate_ResolveTargetable) ? D3DUSAGE_RENDERTARGET : 0) |
		((Flags&TexCreate_Dynamic) ? D3DUSAGE_DYNAMIC : 0),
		(D3DFORMAT)GPixelFormats[Format].PlatformFormat,
		(Flags&TexCreate_ResolveTargetable) ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED,
...
void* RHILockTextureCubeFace( FTextureCubeRHIParamRef TextureCube, UINT FaceIndex, UINT MipIndex, UBOOL bIsDataBeingWrittenTo, UINT& DestStride )
{
	D3DLOCKED_RECT LockedRect;
...
void RHIUnlockTextureCubeFace( FTextureCubeRHIParamRef TextureCube, UINT FaceIndex, UINT MipIndex)
{
