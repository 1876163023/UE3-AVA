...
/* ==========================================================================================================
WxEffectAdjustmentPanel
========================================================================================================== */

IMPLEMENT_DYNAMIC_CLASS(WxEffectAdjustmentPanel,wxPanel)
BEGIN_EVENT_TABLE(WxEffectAdjustmentPanel,wxPanel)
//EVT_BUTTON(ID_UI_EDITSTYLE_COLOR,WxEffectAdjustmentPanel::OnClick_ChangeColor)
//EVT_COMMAND_SCROLL(ID_UI_EDITSTYLE_ALPHA, WxEffectAdjustmentPanel::OnScroll_ChangeAlpha)
	EVT_BUTTON(ID_UI_EDITSTYLE_BUTTON_USE_FADECURVE, WxEffectAdjustmentPanel::OnClick_UseSelected)
	EVT_BUTTON(ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEHORZ, WxEffectAdjustmentPanel::OnClick_UseSelected)
	EVT_BUTTON(ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEVERT, WxEffectAdjustmentPanel::OnClick_UseSelected)
	EVT_TEXT_ENTER(ID_UI_EDITSTYLE_TEXT_FADEDURATION, WxEffectAdjustmentPanel::OnText_FadeDuration)
	EVT_TEXT_ENTER(ID_UI_EDITSTYLE_TEXT_SCALEDURATION, WxEffectAdjustmentPanel::OnText_ScaleDuration)
	EVT_TEXT_ENTER(ID_UI_EDITSTYLE_TEXT_SCALEAXISHORZ, WxEffectAdjustmentPanel::OnText_ScaleAxis)
	EVT_TEXT_ENTER(ID_UI_EDITSTYLE_TEXT_SCALEAXISVERT, WxEffectAdjustmentPanel::OnText_ScaleAxis)
	EVT_CHOICE(ID_UI_EDITSTYLE_COMBO_FADETYPE, WxEffectAdjustmentPanel::OnCombo_FadeType)
	EVT_BUTTON(ID_UI_EDITSTYLE_BUTTON_DEL_FADECURVE, WxEffectAdjustmentPanel::OnClick_DelSelected)
	EVT_BUTTON(ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEHORZ, WxEffectAdjustmentPanel::OnClick_DelSelected)
	EVT_BUTTON(ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEVERT, WxEffectAdjustmentPanel::OnClick_DelSelected)
END_EVENT_TABLE()

/** Default constructor for use in two-stage dynamic window creation */
WxEffectAdjustmentPanel::WxEffectAdjustmentPanel(): 
		cmb_FadeType(NULL), txt_FadeDuration(NULL), txt_FadeCurve(NULL), btn_FadeCurveAssign(NULL), btn_FadeCurveBrowse(NULL), btn_FadeCurveDelete(NULL),
		txt_ScaleAxisX(NULL), txt_ScaleAxisY(NULL), txt_ScaleCurveHorz(NULL), txt_ScaleCurveVert(NULL), txt_ScaleDuration(NULL),
		btn_ScaleCurveHorzAssign(NULL), btn_ScaleCurveHorzBrowse(NULL), btn_ScaleCurveHorzDelete(NULL),
		btn_ScaleCurveVertAssign(NULL), btn_ScaleCurveVertBrowse(NULL), btn_ScaleCurveVertDelete(NULL)
{
}
...
* @param	OwnerPropertyGroup  the style property group which contains this panel
*/
void WxEffectAdjustmentPanel::Create( wxWindow* InParent, wxWindowID InID, class UUIStyle_Data* InitialData, class WxStylePropertiesGroup* OwnerPropertyGroup )
{
	check(InitialData);
	check(OwnerPropertyGroup);
...
* Creates the controls and sizers for this panel.
*/
void WxEffectAdjustmentPanel::CreateControls()
{
	wxBoxSizer* MainVSizer = new wxBoxSizer(wxVERTICAL);	
	SetSizer(MainVSizer);
	{
		// 페이드 조절 컨트롤들
		wxBoxSizer* FadeVSizer = new wxBoxSizer(wxVERTICAL);
		MainVSizer->Add(FadeVSizer, 0, wxGROW|wxALL, 0);
		{
			// add the "Fade Adjust:" label
			wxStaticText* lbl_FadeAdjust = new wxStaticText(this, wxID_STATIC, *LocalizeUI(TEXT("StylePropertiesCommon_Label_FadeAdjust")) );
			FadeVSizer->Add(lbl_FadeAdjust, 0, wxALIGN_LEFT|wxLEFT|wxRIGHT|wxTOP|wxADJUST_MINSIZE, 3);

			TArray<FString> TFadeTypes;
			LocalizeUI(TEXT("StylePropertiesCommon_Combo_FadeType")).ParseIntoArray( &TFadeTypes , TEXT(","), TRUE);
			wxArrayString WxFadeTypes;
			for( INT i = 0 ; i < TFadeTypes.Num() ; i++ )
				WxFadeTypes.Add(*TFadeTypes(i));

			wxBoxSizer* FadeHSizer = new wxBoxSizer(wxHORIZONTAL);
			FadeVSizer->Add(FadeHSizer);

			// 페이드 타입 ( 없음, 한번, 루프 ) 콤보박스
			wxStaticText* lbl_FadeType = new wxStaticText(this, wxID_STATIC, *LocalizeUI(TEXT("StylePropertiesCommon_Label_FadeType")));
			cmb_FadeType = new WxUIChoice( this, ID_UI_EDITSTYLE_COMBO_FADETYPE, wxDefaultPosition, wxDefaultSize, WxFadeTypes, wxCB_DROPDOWN | wxCB_READONLY);
			cmb_FadeType->SetSelection(0);
			wxStaticText* lbl_FadeDuration = new wxStaticText(this, wxID_STATIC, *LocalizeUI(TEXT("StylePropertiesCommon_Label_FadeDuration")));
			txt_FadeDuration = new WxUITextCtrl(this, ID_UI_EDITSTYLE_TEXT_FADEDURATION, TEXT("0.0"), wxDefaultPosition, wxSize(30,-1), wxTE_PROCESS_ENTER);
			
			FadeHSizer->Add(lbl_FadeType, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			FadeHSizer->Add(cmb_FadeType, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			FadeHSizer->Add(lbl_FadeDuration, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			FadeHSizer->Add(txt_FadeDuration, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);

			//FadeVSizer->Add(cmb_FadeType,0, wxLEFT | wxRIGHT | wxFIXED_MINSIZE, 3 );
			//FadeVSizer->Add(txt_FadeDuration,0, wxLEFT | wxRIGHT | wxBottom | wxFIXED_MINSIZE, 3);

			txt_FadeCurve = new WxUITextCtrl(this, ID_UI_EDITSTYLE_TEXT_FADECURVE, TEXT(""), wxDefaultPosition, wxSize(200,-1) , wxTE_READONLY | wxTE_PROCESS_ENTER);
			btn_FadeCurveAssign = new wxBitmapButton(this, ID_UI_EDITSTYLE_BUTTON_USE_FADECURVE, GPropertyWindowManager->Prop_UseCurrentBrowserSelectionB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);
			btn_FadeCurveBrowse = new wxBitmapButton(this, ID_PROP_PB_BROWSE, GPropertyWindowManager->Prop_ShowGenericBrowserB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);
			btn_FadeCurveDelete = new wxBitmapButton(this, ID_UI_EDITSTYLE_BUTTON_DEL_FADECURVE,GPropertyWindowManager->Prop_DeleteItemB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);

			FadeHSizer->Add(txt_FadeCurve, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
			FadeHSizer->Add(btn_FadeCurveAssign, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
			FadeHSizer->Add(btn_FadeCurveBrowse, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
			FadeHSizer->Add(btn_FadeCurveDelete, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
		}


		// 스케일 조절 컨트롤들
		wxBoxSizer* ScaleVSizer = new wxBoxSizer(wxVERTICAL);
		MainVSizer->Add(ScaleVSizer,0, wxGROW|wxALL, 0);
		{
			// add "Scale Adjust:" label
			wxStaticText* lbl_ScaleAdjust = new wxStaticText(this, wxID_STATIC, *LocalizeUI(TEXT("StylePropertiesCommon_Label_ScaleAdjust")));
			ScaleVSizer->Add(lbl_ScaleAdjust, 0, wxALIGN_LEFT|wxLEFT|wxRIGHT|wxTOP|wxADJUST_MINSIZE, 3);

			wxBoxSizer* ScaleHSizer = new wxBoxSizer(wxHORIZONTAL);
			ScaleVSizer->Add(ScaleHSizer);
			// add the slider for selecting alpha value
			wxStaticText* lbl_ScaleAxis = new wxStaticText(this, wxID_STATIC, *LocalizeUI(TEXT("StylePropertiesCommon_Label_ScaleAxis")));
			txt_ScaleAxisX = new WxUITextCtrl(this, ID_UI_EDITSTYLE_TEXT_SCALEAXISHORZ, TEXT(""), wxDefaultPosition, wxSize(30,-1),wxTE_PROCESS_ENTER);
			txt_ScaleAxisY = new WxUITextCtrl(this, ID_UI_EDITSTYLE_TEXT_SCALEAXISVERT, TEXT(""), wxDefaultPosition, wxSize(30,-1),wxTE_PROCESS_ENTER);
			wxStaticText* lbl_ScaleDuration = new wxStaticText(this, wxID_STATIC, *LocalizeUI(TEXT("StylePropertiesCommon_Label_ScaleDuration")));
			txt_ScaleDuration = new WxUITextCtrl(this, ID_UI_EDITSTYLE_TEXT_SCALEDURATION, TEXT("0.0"), wxDefaultPosition, wxSize(30,-1),wxTE_PROCESS_ENTER);
	
			ScaleHSizer->Add(lbl_ScaleAxis, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			ScaleHSizer->Add(txt_ScaleAxisX, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			ScaleHSizer->Add(txt_ScaleAxisY, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			ScaleHSizer->Add(lbl_ScaleDuration, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			ScaleHSizer->Add(txt_ScaleDuration, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);

			txt_ScaleCurveHorz = new WxUITextCtrl(this, ID_UI_EDITSTYLE_TEXT_SCALECURVEHORZ, TEXT(""), wxDefaultPosition, wxSize(200,-1), wxTE_READONLY | wxTE_PROCESS_ENTER);
			btn_ScaleCurveHorzAssign = new wxBitmapButton(this, ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEHORZ, GPropertyWindowManager->Prop_UseCurrentBrowserSelectionB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);
			btn_ScaleCurveHorzBrowse = new wxBitmapButton(this, ID_PROP_PB_BROWSE, GPropertyWindowManager->Prop_ShowGenericBrowserB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);
			btn_ScaleCurveHorzDelete = new wxBitmapButton(this, ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEHORZ, GPropertyWindowManager->Prop_DeleteItemB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);

			txt_ScaleCurveVert = new WxUITextCtrl(this, ID_UI_EDITSTYLE_TEXT_SCALECURVEVERT, TEXT(""), wxDefaultPosition, wxSize(200,-1), wxTE_READONLY | wxTE_PROCESS_ENTER);
			btn_ScaleCurveVertAssign = new wxBitmapButton(this, ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEVERT, GPropertyWindowManager->Prop_UseCurrentBrowserSelectionB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);
			btn_ScaleCurveVertBrowse = new wxBitmapButton(this, ID_PROP_PB_BROWSE, GPropertyWindowManager->Prop_ShowGenericBrowserB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);
			btn_ScaleCurveVertDelete = new wxBitmapButton(this, ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEVERT, GPropertyWindowManager->Prop_DeleteItemB, wxDefaultPosition, wxSize(24,24), wxBU_AUTODRAW | wxBU_EXACTFIT);

			ScaleHSizer->Add(txt_ScaleCurveHorz, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			ScaleHSizer->Add(btn_ScaleCurveHorzAssign, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
			ScaleHSizer->Add(btn_ScaleCurveHorzBrowse, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
			ScaleHSizer->Add(btn_ScaleCurveHorzDelete, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);

			ScaleHSizer->Add(txt_ScaleCurveVert, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 5);
			ScaleHSizer->Add(btn_ScaleCurveVertAssign, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
			ScaleHSizer->Add(btn_ScaleCurveVertBrowse, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
			ScaleHSizer->Add(btn_ScaleCurveVertDelete, 0, wxALIGN_CENTER_VERTICAL|wxALL|wxADJUST_MINSIZE, 3);
		}
	}
}
...
* Initialize the values of the controls contained by this panel with the values from the current style.
*/
void WxEffectAdjustmentPanel::InitializeControlValues()
{
	check(StyleDataArray.Num() > 0 && StyleDataArray(0) != NULL);

	UBOOL bSameFadeCurve = TRUE;
	UBOOL bSameScaleCurveX = TRUE;
	UBOOL bSameScaleCurveY = TRUE;
	UBOOL bSameScaleDuration = TRUE;
	UBOOL bSameScaleAxisX = TRUE;
	UBOOL bSameScaleAxisY = TRUE;
	UBOOL bSameFadeDuration = TRUE;
	UBOOL bSameFadeType = TRUE;

	for( INT i = 1 ; i < StyleDataArray.Num() ; i++ )
	{
		UUIStyle_Data* CurrStyleData = StyleDataArray(i);
		UUIStyle_Data* PrevStyleData = StyleDataArray(i - 1);
		check( CurrStyleData != NULL && PrevStyleData != NULL );

		if ( CurrStyleData->FadeDuration != PrevStyleData->FadeDuration )
			bSameFadeDuration = FALSE;
		if ( CurrStyleData->FadeType != PrevStyleData->FadeType )
			bSameFadeType = FALSE;
		if ( CurrStyleData->FadePresetCurve != PrevStyleData->FadePresetCurve )
			bSameFadeCurve = FALSE;
		if ( CurrStyleData->ScaleAxis[UIORIENT_Horizontal] != PrevStyleData->ScaleAxis[UIORIENT_Horizontal] )
			bSameScaleAxisX = FALSE;
		if ( CurrStyleData->ScaleAxis[UIORIENT_Vertical] != PrevStyleData->ScaleAxis[UIORIENT_Vertical] )
			bSameScaleAxisY = FALSE;
		if ( CurrStyleData->ScaleDuration != PrevStyleData->ScaleDuration )
			bSameScaleDuration = FALSE;
		if ( CurrStyleData->ScalePresetCurve[UIORIENT_Horizontal] != PrevStyleData->ScalePresetCurve[UIORIENT_Horizontal] )
			bSameScaleCurveX = FALSE;
		if ( CurrStyleData->ScalePresetCurve[UIORIENT_Vertical] != PrevStyleData->ScalePresetCurve[UIORIENT_Vertical] )
			bSameScaleCurveY = FALSE;
	}

	UUIStyle_Data* FirstStyleData = StyleDataArray(0);

	if( bSameFadeCurve )
	{
		txt_FadeCurve->ClearMultipleValues();
		txt_FadeCurve->SetValue(FirstStyleData->FadePresetCurve ? *FirstStyleData->FadePresetCurve->GetPathName() : TEXT("None"));
	}
	else
		txt_FadeCurve->DisplayMultipleValues();

	if( bSameFadeDuration )
	{
		txt_FadeDuration->ClearMultipleValues();
		txt_FadeDuration->SetValue( *FString::Printf(TEXT("%.2f"), FirstStyleData->FadeDuration) );
	}
	else
		txt_FadeDuration->DisplayMultipleValues();

	if( bSameFadeType )
	{
		cmb_FadeType->ClearMultipleValues();
		cmb_FadeType->SetSelection(FirstStyleData->FadeType);
	}
	else
		cmb_FadeType->DisplayMultipleValues();

	if( bSameScaleCurveX )
	{
		txt_ScaleCurveHorz->ClearMultipleValues();
		txt_ScaleCurveHorz->SetValue(FirstStyleData->ScalePresetCurve[UIORIENT_Horizontal] ? *FirstStyleData->ScalePresetCurve[UIORIENT_Horizontal]->GetPathName() : TEXT("None"));
	}
	else
		txt_ScaleCurveHorz->DisplayMultipleValues();

	if( bSameScaleCurveY )
	{
		txt_ScaleCurveVert->ClearMultipleValues();
		txt_ScaleCurveVert->SetValue(FirstStyleData->ScalePresetCurve[UIORIENT_Vertical] ? *FirstStyleData->ScalePresetCurve[UIORIENT_Vertical]->GetPathName() : TEXT("None"));
	}
	else
		txt_ScaleCurveVert->DisplayMultipleValues();

	if( bSameScaleAxisX )
	{
		txt_ScaleAxisX->ClearMultipleValues();
		txt_ScaleAxisX->SetValue( *FString::Printf(TEXT("%.2f"),FirstStyleData->ScaleAxis[UIORIENT_Horizontal]));
	}
	else
		txt_ScaleAxisX->DisplayMultipleValues();

	if( bSameScaleAxisY)
	{
		txt_ScaleAxisY->ClearMultipleValues();
		txt_ScaleAxisY->SetValue( *FString::Printf(TEXT("%.2f"),FirstStyleData->ScaleAxis[UIORIENT_Vertical]));
	}
	else
		txt_ScaleAxisY->DisplayMultipleValues();

	if( bSameScaleDuration )
	{
		txt_ScaleDuration->ClearMultipleValues();
		txt_ScaleDuration->SetValue( *FString::Printf(TEXT("%.2f"),FirstStyleData->ScaleDuration ) );
	}
	else
		txt_ScaleDuration->DisplayMultipleValues();
...
*	Notifies owner style editor that property has been modified
*/
void WxEffectAdjustmentPanel::NotifyPropertyChanged()
{
	if(OwnerPropertyGroup)
	{
		OwnerPropertyGroup->NotifyStyleModified(TRUE);
...
* @param	SelectedStatesData		Array of data from the currently selected states
*/
void WxEffectAdjustmentPanel::NotifySelectedStatesChanged( const TArray< UUIStyle_Data* > & SelectedStatesData )
{	
	// Update current StyleDataArray to hold currently selected data by making a shallow copy of the passed array
	StyleDataArray = SelectedStatesData;
...
void WxEffectAdjustmentPanel::OnClick_UseSelected( wxCommandEvent& Event )
{
	UCurveEdPresetCurve* SelectedPresetCurve = GEditor->GetSelectedObjects()->GetTop<UCurveEdPresetCurve>();
	if ( SelectedPresetCurve != NULL )
	{
		FString PathName = SelectedPresetCurve->GetPathName();
		if( Event.GetId() == ID_UI_EDITSTYLE_BUTTON_USE_FADECURVE )
			txt_FadeCurve->SetValue(*PathName);
		else if ( Event.GetId() == ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEHORZ )
			txt_ScaleCurveHorz->SetValue(*PathName);
		else if ( Event.GetId() == ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEVERT )
			txt_ScaleCurveVert->SetValue(*PathName);
	}

	UpdatePresetCurveProperty(Event.GetId(), SelectedPresetCurve);
	NotifyPropertyChanged();
}

void WxEffectAdjustmentPanel::OnClick_DelSelected( wxCommandEvent& Event )
{
	if( Event.GetId() == ID_UI_EDITSTYLE_BUTTON_DEL_FADECURVE )
		txt_FadeCurve->SetValue(TEXT(""));
	else if ( Event.GetId() == ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEHORZ )
		txt_ScaleCurveHorz->SetValue(TEXT(""));
	else if ( Event.GetId() == ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEVERT )
		txt_ScaleCurveVert->SetValue(TEXT(""));

	UpdatePresetCurveProperty(Event.GetId(), NULL);
	NotifyPropertyChanged();
}

void WxEffectAdjustmentPanel::UpdatePresetCurveProperty( int WidgetID, UCurveEdPresetCurve* PresetCurve )
{
	for( TArray<UUIStyle_Data*>::TIterator It(StyleDataArray); It; ++It)
	{
		switch(WidgetID)
		{
		case ID_UI_EDITSTYLE_BUTTON_DEL_FADECURVE:
		case ID_UI_EDITSTYLE_BUTTON_USE_FADECURVE:			(*It)->FadePresetCurve = PresetCurve;	break;
		case ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEHORZ:
		case ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEHORZ:		(*It)->ScalePresetCurve[UIORIENT_Horizontal] = PresetCurve;	break;
		case ID_UI_EDITSTYLE_BUTTON_DEL_SCALECURVEVERT:
		case ID_UI_EDITSTYLE_BUTTON_USE_SCALECURVEVERT:		(*It)->ScalePresetCurve[UIORIENT_Vertical] = PresetCurve;	break;
...
void WxEffectAdjustmentPanel::OnText_ScaleAxis( wxCommandEvent& Event )
{
	if( txt_ScaleAxisX == NULL )
		return;

	check( Event.GetId() == ID_UI_EDITSTYLE_TEXT_SCALEAXISHORZ || Event.GetId() == ID_UI_EDITSTYLE_TEXT_SCALEAXISVERT );

	FString ScaleAxisStr = Event.GetId() == ID_UI_EDITSTYLE_TEXT_SCALEAXISHORZ ? txt_ScaleAxisX->GetValue().c_str() : txt_ScaleAxisY->GetValue().c_str();
	if(	ScaleAxisStr.IsNumeric() == FALSE )
		return;

	UpdateScaleAxis( Event.GetId() == ID_UI_EDITSTYLE_TEXT_SCALEAXISHORZ ? UIORIENT_Horizontal : UIORIENT_Vertical , appAtof(*ScaleAxisStr) );
	NotifyPropertyChanged();
}

void WxEffectAdjustmentPanel::UpdateScaleAxis( BYTE Orient, FLOAT AxisX )
{
	check( 0 <= Orient && Orient < UIORIENT_MAX );

	for( TArray<UUIStyle_Data*>::TIterator It(StyleDataArray); It; ++It)
	{
		(*It)->ScaleAxis[Orient] = AxisX; 
		(*It)->SetDirtiness(TRUE);
	}
}

void WxEffectAdjustmentPanel::OnText_ScaleDuration( wxCommandEvent& Event )
{
	if( txt_ScaleDuration == NULL )
		return;

	FString DurStr = txt_ScaleDuration->GetValue().c_str();
	if( DurStr.IsNumeric() == FALSE )
		return;

	UpdateScaleDuration( appAtof(*DurStr) );
	NotifyPropertyChanged();
}

void WxEffectAdjustmentPanel::UpdateScaleDuration( FLOAT ScaleDuration )
{
	for( TArray<UUIStyle_Data*>::TIterator It(StyleDataArray); It; ++It)
	{
		(*It)->ScaleDuration = ScaleDuration;
		(*It)->SetDirtiness(TRUE);
	}
}

void WxEffectAdjustmentPanel::OnText_FadeDuration( wxCommandEvent& Event )
{
	if( txt_FadeDuration == NULL )
		return;

	FString DurStr = txt_FadeDuration->GetValue().c_str();
	if( DurStr.IsNumeric() == FALSE )
		return;

	UpdateFadeDuration( appAtof(*DurStr) );
	NotifyPropertyChanged();
}

void WxEffectAdjustmentPanel::UpdateFadeDuration( FLOAT FadeDuration )
{
	for( TArray<UUIStyle_Data*>::TIterator It(StyleDataArray); It; ++It)
	{
		(*It)->FadeDuration = FadeDuration;
		(*It)->SetDirtiness(TRUE);
	}
}

void WxEffectAdjustmentPanel::OnCombo_FadeType( wxCommandEvent& Event )
{
	INT Index;
	if( (Index = cmb_FadeType->GetSelection()) == wxNOT_FOUND )
		return;

	BYTE FadeType;
	wxString WxStrValue = cmb_FadeType->GetString(Index);
	FString FadeTypeStr = WxStrValue.c_str();
	FadeTypeStr = FadeTypeStr.ToLower();
	if( FadeTypeStr.InStr(TEXT("once")) >= 0 || FadeTypeStr.InStr(TEXT("fade")) >= 0 )
		FadeType = EFT_Fading;
	else if ( FadeTypeStr.InStr(TEXT("loop")) >= 0 || FadeTypeStr.InStr(TEXT("pulse")) >= 0)
		FadeType = EFT_Loop;
	else
		FadeType = EFT_None;

	UpdateFadeType( FadeType );
	NotifyPropertyChanged();
}

void WxEffectAdjustmentPanel::UpdateFadeType( BYTE FadeType  )
{
	for( TArray<UUIStyle_Data*>::TIterator It(StyleDataArray); It; ++It)
	{
		(*It)->FadeType = FadeType;
		(*It)->SetDirtiness(TRUE);
	}
}
...
void WxImageAdjustmentPanel::CreateControls()
{
...

	{
		...
		spin_TextureCoordinates[0] = new WxUISpinCtrl( this, ID_UI_EDITSTYLE_SPIN_ATLASCOORDS_ORIGIN, TEXT("0"), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 8192, 0 );
		CoordinatesSizer->Add(spin_TextureCoordinates[0], 1, wxGROW|wxALIGN_CENTER_VERTICAL|wxRIGHT, 5);

		spin_TextureCoordinates[1] = new WxUISpinCtrl( this, ID_UI_EDITSTYLE_SPIN_ATLASCOORDS_EXTENT, TEXT("0"), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 8192, 0 );
...
void WxStyleTextPropertiesGroup::CreateControls()
{
	...
	// 페이드, 스케일 조절 패널
	pnl_Effect = new WxEffectAdjustmentPanel();
	pnl_Effect->Create(this, wxID_ANY, StyleDataArray(0), this);
	StaticVSizer->Add(pnl_Effect, 0, wxGROW|wxALL, 5);
...
void WxStyleTextPropertiesGroup::InitializeControlValues()
{
	...
	pnl_Effect->InitializeControlValues();
...
void WxStyleTextPropertiesGroup::NotifySelectedStatesChanged( TArray<UUIState*> & SelectedStates )
{
	...
	pnl_Effect->NotifySelectedStatesChanged(BaseData);
