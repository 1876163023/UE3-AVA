	...
	,	SelectedAlgorithm( NULL )
{
	...
	LeftSizerV->Add( AlgorithmList, 1, wxGROW | wxALL, GControlBorder );
	...
	ButtonSizerH->Add( ApplyToSetButton, 1, wxGROW | wxRIGHT, GControlBorder );
	...
	ButtonSizerH->Add( ApplyToSequenceButton, 1, wxGROW | wxLEFT, GControlBorder );

	PropertyWindow = new WxPropertyWindow;
	PropertyWindow->Create( this, NULL );
	TopSizerH->Add( PropertyWindow, 1, wxGROW | wxALL, GControlBorder );
...
void WxDlgAnimationCompression::SetSelectedAlgorithm(UAnimationCompressionAlgorithm* InAlgorithm)
{
	...
	PropertyWindow->SetObject( SelectedAlgorithm, TRUE, FALSE, FALSE );
}
...
void WxDlgAnimationCompression::OnApplyAlgorithmToSet(wxCommandEvent& In)
{
	...
	{
		const UBOOL bProceed = appMsgf( AMT_YesNo, *LocalizeUnrealEd(TEXT("AboutToCompressAnimations_F")), AnimSetViewer->SelectedAnimSet->Sequences.Num() );
		if ( bProceed )
		{
