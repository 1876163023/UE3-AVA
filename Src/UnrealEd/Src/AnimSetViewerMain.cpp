...
static const FColor ExtraMesh4WireColor = FColor(255,215,215,255);
static const FColor ExtraMesh5WireColor = FColor(215,255,215,255);
static const FColor ExtraMesh6WireColor = FColor(215,215,255,255);

static const FLOAT	TranslateSpeed = 0.25f;
static const FLOAT	RotateSpeed = 0.02f;
static const FLOAT	LightRotSpeed = 40.0f;
static const FLOAT	WindStrengthSpeed = 4.f;
	...
	PreviewScene(FRotator(-8192,-8192,0),0.25f,1.f,TRUE)
{
	...
	// 3개를 추가적으로 더 생성한다.
	InASV->PreviewSkelCompAux4 = ConstructObject<USkeletalMeshComponent>(USkeletalMeshComponent::StaticClass());
	InASV->PreviewSkelCompAux4->ParentAnimComponent = InASV->PreviewSkelComp;
	InASV->PreviewSkelCompAux4->WireframeColor = ExtraMesh4WireColor;
	PreviewScene.AddComponent(InASV->PreviewSkelCompAux4,FMatrix::Identity);

	InASV->PreviewSkelCompAux5 = ConstructObject<USkeletalMeshComponent>(USkeletalMeshComponent::StaticClass());
	InASV->PreviewSkelCompAux5->ParentAnimComponent = InASV->PreviewSkelComp;
	InASV->PreviewSkelCompAux5->WireframeColor = ExtraMesh5WireColor;
	PreviewScene.AddComponent(InASV->PreviewSkelCompAux5,FMatrix::Identity);

	InASV->PreviewSkelCompAux6 = ConstructObject<USkeletalMeshComponent>(USkeletalMeshComponent::StaticClass());
	InASV->PreviewSkelCompAux6->ParentAnimComponent = InASV->PreviewSkelComp;
	InASV->PreviewSkelCompAux6->WireframeColor = ExtraMesh6WireColor;
	PreviewScene.AddComponent(InASV->PreviewSkelCompAux6,FMatrix::Identity);
...
void FASVViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	if( AnimSetViewer->SelectedAnimSeq )
	{
		...
		// Draw anim position
		InfoString = FString::Printf(TEXT("Pos: %3.1f%%, Time: %4.4fs, Len: %4.4fs"), 
...
UBOOL FASVViewportClient::InputAxis(FViewport* Viewport, INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
...
public:
	...
	/** 
	 * Skeletal meshes are only drawn as dynamic elements 
	...
	{
		FSkeletalMeshSceneProxy::DrawDynamicElements(PDI, View, DPGIndex);
...
BEGIN_EVENT_TABLE( WxAnimSetViewer, wxFrame )
	...
	EVT_COMBOBOX( IDM_ANIMSET_SKELMESHAUX4COMBO, WxAnimSetViewer::OnAuxSkelMeshComboChanged )
	EVT_COMBOBOX( IDM_ANIMSET_SKELMESHAUX5COMBO, WxAnimSetViewer::OnAuxSkelMeshComboChanged )
	EVT_COMBOBOX( IDM_ANIMSET_SKELMESHAUX6COMBO, WxAnimSetViewer::OnAuxSkelMeshComboChanged )
	...
	EVT_BUTTON( IDM_ANIMSET_SKELMESH_AUX4USE, WxAnimSetViewer::OnAuxSkelMeshUse )
	EVT_BUTTON( IDM_ANIMSET_SKELMESH_AUX5USE, WxAnimSetViewer::OnAuxSkelMeshUse )
	EVT_BUTTON( IDM_ANIMSET_SKELMESH_AUX6USE, WxAnimSetViewer::OnAuxSkelMeshUse )
	...
	,	DlgAnimationCompression( NULL )
{
	...
	LeftPanel->SetSizer(LeftSizerV);
	{
		...
		// SkeletalMesh
		{
			...
			// Extra Mesh 3개 추가.
			{
				wxStaticText* Aux4SkelMeshLabel = new wxStaticText( SkeletalMeshPanel, -1, *LocalizeUnrealEd("ExtraMesh4") );
				SkeletalMeshPanelSizer->Add( Aux4SkelMeshLabel, 0, wxALIGN_LEFT|wxALL|wxADJUST_MINSIZE, ControlBorder );

				wxBoxSizer* Aux4Sizer = new wxBoxSizer( wxHORIZONTAL );
				SkeletalMeshPanelSizer->Add( Aux4Sizer, 0, wxGROW|wxALL, 0 );

				SkelMeshAux4Combo = new WxComboBox( SkeletalMeshPanel, IDM_ANIMSET_SKELMESHAUX4COMBO, TEXT(""), wxDefaultPosition, wxDefaultSize, 0, NULL, wxCB_READONLY | wxCB_SORT );
				Aux4Sizer->Add( SkelMeshAux4Combo, 1, wxGROW|wxALL, ControlBorder );

				wxBitmapButton* Aux4UseButton = new wxBitmapButton( SkeletalMeshPanel, IDM_ANIMSET_SKELMESH_AUX4USE, UseButtonB );
				Aux4UseButton->SetToolTip( *LocalizeUnrealEd("ToolTip_2") );
				Aux4Sizer->Add( Aux4UseButton, 0, wxGROW|wxALL, ControlBorder );
			}

			{
				wxStaticText* Aux5SkelMeshLabel = new wxStaticText( SkeletalMeshPanel, -1, *LocalizeUnrealEd("ExtraMesh5") );
				SkeletalMeshPanelSizer->Add( Aux5SkelMeshLabel, 0, wxALIGN_LEFT|wxALL|wxADJUST_MINSIZE, ControlBorder );

				wxBoxSizer* Aux5Sizer = new wxBoxSizer( wxHORIZONTAL );
				SkeletalMeshPanelSizer->Add( Aux5Sizer, 0, wxGROW|wxALL, 0 );

				SkelMeshAux5Combo = new WxComboBox( SkeletalMeshPanel, IDM_ANIMSET_SKELMESHAUX5COMBO, TEXT(""), wxDefaultPosition, wxDefaultSize, 0, NULL, wxCB_READONLY | wxCB_SORT );
				Aux5Sizer->Add( SkelMeshAux5Combo, 1, wxGROW|wxALL, ControlBorder );

				wxBitmapButton* Aux5UseButton = new wxBitmapButton( SkeletalMeshPanel, IDM_ANIMSET_SKELMESH_AUX5USE, UseButtonB );
				Aux5UseButton->SetToolTip( *LocalizeUnrealEd("ToolTip_2") );
				Aux5Sizer->Add( Aux5UseButton, 0, wxGROW|wxALL, ControlBorder );
			}

			{
				wxStaticText* Aux6SkelMeshLabel = new wxStaticText( SkeletalMeshPanel, -1, *LocalizeUnrealEd("ExtraMesh6") );
				SkeletalMeshPanelSizer->Add( Aux6SkelMeshLabel, 0, wxALIGN_LEFT|wxALL|wxADJUST_MINSIZE, ControlBorder );

				wxBoxSizer* Aux6Sizer = new wxBoxSizer( wxHORIZONTAL );
				SkeletalMeshPanelSizer->Add( Aux6Sizer, 0, wxGROW|wxALL, 0 );

				SkelMeshAux6Combo = new WxComboBox( SkeletalMeshPanel, IDM_ANIMSET_SKELMESHAUX6COMBO, TEXT(""), wxDefaultPosition, wxDefaultSize, 0, NULL, wxCB_READONLY | wxCB_SORT );
				Aux6Sizer->Add( SkelMeshAux6Combo, 1, wxGROW|wxALL, ControlBorder );

				wxBitmapButton* Aux6UseButton = new wxBitmapButton( SkeletalMeshPanel, IDM_ANIMSET_SKELMESH_AUX6USE, UseButtonB );
				Aux6UseButton->SetToolTip( *LocalizeUnrealEd("ToolTip_2") );
				Aux6Sizer->Add( Aux6UseButton, 0, wxGROW|wxALL, ControlBorder );
			}
		}
	...
	SkelMeshAux4Combo->Freeze();
	SkelMeshAux5Combo->Freeze();
	SkelMeshAux6Combo->Freeze();
	...
	SkelMeshAux4Combo->Append( *LocalizeUnrealEd("-None-"), (void*)NULL );
	SkelMeshAux5Combo->Append( *LocalizeUnrealEd("-None-"), (void*)NULL );
	SkelMeshAux6Combo->Append( *LocalizeUnrealEd("-None-"), (void*)NULL );
	...
	for(TObjectIterator<USkeletalMesh> It; It; ++It)
	{
		...
		SkelMeshAux4Combo->Append( *ItSkelMesh->GetName(), ItSkelMesh );
		SkelMeshAux5Combo->Append( *ItSkelMesh->GetName(), ItSkelMesh );
		SkelMeshAux6Combo->Append( *ItSkelMesh->GetName(), ItSkelMesh );
	}
	...
	SkelMeshAux4Combo->Thaw();
	SkelMeshAux5Combo->Thaw();
	SkelMeshAux6Combo->Thaw();
...
void WxAnimSetViewer::NotifyPostChange( void* Src, UProperty* PropertyThatChanged )
{
	if( PropertyThatChanged )
	{
		...
		else if(PropertyThatChanged->GetFName() == FName(TEXT("bForceCPUSkinning")))
		{
			...
			// Extra Mesh 3개 추가.
			FComponentReattachContext ReattachContext4(PreviewSkelCompAux4);
			FComponentReattachContext ReattachContext5(PreviewSkelCompAux5);
			FComponentReattachContext ReattachContext6(PreviewSkelCompAux6);
		}
	}
	...
	if(SelectedAnimSet)
	{
		...
		while(LinkupIndex < SelectedAnimSet->LinkupCache.Num())
		{
			...
			// Find the SkeletalMesh with this Linkup's GUID.
			USkeletalMesh* SkelMesh = NULL;
			for(TObjectIterator<USkeletalMesh> It;It;++It)
			{
				USkeletalMesh* TestMesh = *It;
				if(TestMesh->SkelMeshGUID == Linkup.SkelMeshLinkupGUID)
				{
					SkelMesh = TestMesh;
					continue;
				}
...
void WxAnimSetViewer::ClearSocketPreviews()
{
	...
	for(INT i=0; i<SocketPreviews.Num(); i++)
	{
		...
		// Attach된 메쉬를 Detach해 준다.(2006/12/13 고광록)
