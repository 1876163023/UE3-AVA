...
#if 0
	EditorFrame->OptionProxyInit();
	if ( TwoDeeShapeEditor )
	{
		// If the 2D shape editor was open, reset it to receive the new option proxies.
		TwoDeeShapeEditor->ResetPrimitiveWindow();
	}
#endif
...
void WxUnrealEdApp::Send(ECallbackEventType InType,const FString& InString, UObject* InObject)
{
	switch (InType)
	{
		// save the sahder cache when we save a pacakge
