...
struct FReferencerClassCompare
{
	...
	{
		return FClassNameObjNameCompare::Compare(A.Referencer, B.Referencer);
	}
};
...
struct FReferencerObjCompare
{
	static INT Compare( const WxReferencedAssetsBrowser::FReferencedAssets& A, const WxReferencedAssetsBrowser::FReferencedAssets& B )
	{
		return FObjNameCompare::Compare(A.Referencer, B.Referencer);
	}
};
...
void WxReferencedAssetsBrowser::SortAssetList()
{
	...
	for ( INT ActorIndex = 0; ActorIndex < Referencers.Num(); ActorIndex++ )
	{
		...
		{
			// Sort by class, then object name.
			Sort<UObject*,FClassNameObjNameCompare>(&Asset.AssetList(0), Asset.AssetList.Num());
		}
		else
		{
			// Sort by just object name
			Sort<UObject*,FObjNameCompare>(&Asset.AssetList(0), Asset.AssetList.Num());
		}
	}
}
 ...
 * scrolling.
 */
UBOOL WxReferencedAssetsBrowser::InputKey(FViewport* Viewport,INT ControllerId,FName Key,EInputEvent Event,FLOAT AmountDepressed,UBOOL bGamepad)
{
	...
	if ( ViewMode == RAV_Thumbnail )
	{
		...
		for (INT Index = 0; Index < Referencers.Num(); Index++)
		{
			...
			FString ReferencerTitle = (Asset.Referencer == GWorld->GetModel())
				? FString::Printf(TEXT("BSP.%s"), *Asset.Referencer->GetName())
				: Asset.Referencer->GetName();
					...
					if (YPos + FixedSize + STD_TNAIL_PAD >= 0 && YPos <= (INT)Viewport->GetSizeY())
					{
						...
						else
						{
							...
							RenderInfo->Renderer->Draw(Asset,TPT_Plane,XPos,YPos,
								FixedSize,FixedSize,Viewport,Canvas,FALSE);
...
void WxReferencedAssetsBrowser::UpdateTree()
{
	...
	for ( INT ReferenceIndex = 0; ReferenceIndex < Referencers.Num(); ReferenceIndex++ )
	{
		...
		const wxTreeItemId BaseItemId = ReferenceTree->AppendItem( RootId, *Asset.Referencer->GetName(), -1, -1, new WxTreeObjectWrapper(Asset.Referencer) );
...
void WxReferencedAssetsBrowser::AddReferencedAssets( wxTreeItemId ParentId, UObject* BaseObject, TArray<UObject*>* AssetList )
{
	...
	for ( INT AssetIndex = 0; AssetIndex < AssetList->Num(); AssetIndex++ )
	{
		...
		{
			ItemString = *LocalizeUnrealEd(TEXT("Script"));
		...
		{
			ItemString = *LocalizeUnrealEd(TEXT("Defaults"));
			if ( ReferencedAssets == NULL || ReferencedAssets->Num() == 0 )
			{
...
int wxCALLBACK WxRABrowserListSort_Class( long InItem1, long InItem2, long InSortData )
{
	...
	int Result = appStricmp(*A->GetClass()->GetName(),*B->GetClass()->GetName());
	if ( Result == 0 )
	{
...
void WxReferencedAssetsBrowser::UpdateList()
{
	...
	for ( INT ReferenceIndex = 0; ReferenceIndex < Referencers.Num(); ReferenceIndex++ )
	{
		...
		for ( INT AssetIndex = 0; AssetIndex < Asset.AssetList.Num(); AssetIndex++ )
		{
			...
			const INT item = ListView->InsertItem( COLHEADER_Name, *ReferencedAsset->GetName()/*, GBTCI_Resource*/ );
			...
			ListView->SetItem( item, COLHEADER_Class, *ReferencedAsset->GetClass()->GetName() );
 ...
 */
UBOOL WxReferencedAssetsBrowser::FFindAssetsArchive::ShouldSearchForAssets(
	UObject* Object,const TArray<UClass*>& IgnoreClasses,
	const TArray<UObject*>& IgnorePackages)
{
	...
	for (INT Index = 0; Index < IgnoreClasses.Num() && bShouldSearch == TRUE;
		Index++)
	{
		// Bail if we are on the ignore list
		bShouldSearch = Object->IsA(IgnoreClasses(Index)) == FALSE;
	}
	if (bShouldSearch == TRUE)
	{
		// Check to see if we should ignore it due to package
		for (INT Index = 0; Index < IgnorePackages.Num() && bShouldSearch == TRUE;
			Index++)
		{
			// If this object belongs to this package, bail
			bShouldSearch = Object->IsIn(IgnorePackages(Index)) == FALSE;
		}
	}

	if ( bShouldSearch == TRUE && bIncludeDefaultRefs == FALSE && Object->IsTemplate() )
	{
