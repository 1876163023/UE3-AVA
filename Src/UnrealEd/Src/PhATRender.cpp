...
AssetEditor(InAssetEditor)
{
	...
	// make our own line batcher so we can draw lines
	...
	// Body materials
	ElemSelectedMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_ElemSelectedMaterial"), NULL, LOAD_None, NULL);
	check(ElemSelectedMaterial);

	BoneSelectedMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_BoneSelectedMaterial"), NULL, LOAD_None, NULL);
	check(BoneSelectedMaterial);

	BoneUnselectedMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_UnselectedMaterial"), NULL, LOAD_None, NULL);
	check(BoneUnselectedMaterial);

	BoneNoCollisionMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_NoCollisionMaterial"), NULL, LOAD_None, NULL);
	check(BoneNoCollisionMaterial);

	JointLimitMaterial = LoadObject<UMaterialInstance>(NULL, TEXT("EditorMaterials.PhAT_JointLimitMaterial"), NULL, LOAD_None, NULL);
...
void FPhATViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	FString StatusString = FString::Printf( *LocalizeUnrealEd("BodiesConstraints_F"), AssetEditor->PhysicsAsset->BodySetup.Num(), AssetEditor->PhysicsAsset->ConstraintSetup.Num() );
	...
	if(AssetEditor->bRunningSimulation)
	{
		FString PhysWeightString = FString::Printf( TEXT("Phys Blend: %3.0f%%"), AssetEditor->EditorSkelComp->PhysicsWeight * 100.f );
		INT PWLW, PWLH;
...
void UPhATSkeletalMeshComponent::RenderAssetTools(const FSceneView* View, class FPrimitiveDrawInterface* PDI, UBOOL bHitTest)
{
		...
		if(BoneIndex != INDEX_NONE)
		{
			...
			for(INT j=0; j<AggGeom->SphereElems.Num(); j++)
			{
				...
				if(CollisionViewMode == PRM_Solid)
				{
					...
					AggGeom->SphereElems(j).DrawElemSolid( PDI, ElemTM, 1.f, PrimMaterial->GetInstanceInterface(0) );
				}
			...
			for(INT j=0; j<AggGeom->BoxElems.Num(); j++)
			{
				...
				if(CollisionViewMode == PRM_Solid)
				{
					...
					AggGeom->BoxElems(j).DrawElemSolid( PDI, ElemTM, 1.f, PrimMaterial->GetInstanceInterface(0) );
				}
			...
			for(INT j=0; j<AggGeom->SphylElems.Num(); j++)
			{
				...
				if(CollisionViewMode == PRM_Solid)
				{
					...
					AggGeom->SphylElems(j).DrawElemSolid( PDI, ElemTM, 1.f, PrimMaterial->GetInstanceInterface(0) );
				}
			...
			for(INT j=0; j<AggGeom->ConvexElems.Num(); j++)
			{
				...
				if(CollisionViewMode == PRM_Solid )
				{
					...
					AggGeom->ConvexElems(j).DrawElemSolid( PDI, ElemTM, FVector(1.f), PrimMaterial->GetInstanceInterface(0) );
				}
#endif
	...
	if(ConstraintViewMode != PCV_None)
	{
		for(INT i=0; i<PhysicsAsset->ConstraintSetup.Num(); i++)
		{
			...
			PhysEditor->DrawConstraint(i, 1.f, PDI, PhysEditor->EditorSimOptions->bShowConstraintsAsPoints);
			if(bHitTest) PDI->SetHitProxy(NULL);
		}
	}
...
void WxPhAT::DrawConstraint(INT ConstraintIndex, FLOAT Scale, FPrimitiveDrawInterface* PDI, UBOOL bDrawAsPoint)
{
	...
	cs->DrawConstraint(PDI, Scale, bDrawLimits, bDrawSelected, PhATViewportClient->JointLimitMaterial, Con1Frame, Con2Frame, bDrawAsPoint);
}
...
UMaterialInstance* WxPhAT::GetPrimitiveMaterial(INT BodyIndex, EKCollisionPrimitiveType PrimitiveType, INT PrimitiveIndex)
{
...
void WxPhAT::DrawCurrentWidget(const FSceneView* View, FPrimitiveDrawInterface* PDI, UBOOL bHitTest)
{
	...
	FLOAT WidgetRadius = View->Project(WidgetOrigin).W * (WidgetSize / ZoomFactor);
