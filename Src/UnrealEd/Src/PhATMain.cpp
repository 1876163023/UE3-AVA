...
namespace
{
	static const TCHAR* DockingParent_Name = TEXT("PhAT");
...
BEGIN_EVENT_TABLE( WxPhAT, wxFrame )
...
: wxFrame( InParent, InID, *LocalizeUnrealEd("PhAT"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE | wxFRAME_FLOAT_ON_PARENT | wxFRAME_NO_TASKBAR ),
  FDockingParent( this )
{
	...
	wxPane* PreviewPane = new wxPane( this );
	{
		...
		PreviewPane->SetClient(PreviewWindow);
	}
	...
	wxDockWindow* DockWindowProperties = AddDockingWindow(PropertyWindow, FDockingParent::DH_Right, *LocalizeUnrealEd(TEXT("Properties")));

	// Create tree control
	TreeCtrl = new WxTreeCtrl;
	TreeCtrl->Create( this, ID_PHAT_TREECTRL, NULL, wxTR_HAS_BUTTONS|wxTR_MULTIPLE|wxTR_LINES_AT_ROOT );
	wxDockWindow* DockWindowTree = AddDockingWindow(TreeCtrl, FDockingParent::DH_Right, *LocalizeUnrealEd(TEXT("Tree")));
	...
	{
		appMsgf( AMT_OK, *LocalizeUnrealEd("BodyMissingFromSkelMesh"), *MissingBones );
	}
	...
	case ID_PHAT_SETMATERIAL:
		{
			...
			{
				FString MenuText = FString::Printf(*LocalizeUnrealEd("ApplyPhysicalAssetMaterialF"), *SelectedPhysMaterial->GetName());
				In.SetText(*MenuText);
				In.Enable(TRUE);
			}
			else
...
UBOOL FPhATViewportClient::InputKey(FViewport* Viewport, INT ControllerId, FName Key, EInputEvent Event,FLOAT /*AmountDepressed*/,UBOOL /*Gamepad*/)
{
	...
	// Ignore the keystroke if we've been asked to (from tree control mouse release)
	if( AssetEditor->bIgnoreNextKey )
	{
		AssetEditor->bIgnoreNextKey = FALSE;
		return TRUE;
	}
...
UBOOL FPhATViewportClient::InputAxis(FViewport* Viewport, INT ControllerId,  FName Key, FLOAT Delta, FLOAT DeltaTime)
{
...
void FPhATViewportClient::Tick(FLOAT DeltaSeconds)
{
	...
	if(AssetEditor->bRunningSimulation)
	{
		...
		FLOAT PhysDT = ::Min(UseDelta, MaxDeltaTime);

		TickRBPhysScene(AssetEditor->RBPhysScene, PhysDT, MaxSubstep, FALSE);
		...
		if( NULL != LineBatcher )
		{
			LineBatcher->BatchedLines.Empty(); // remove the novodex debug line draws
...
void WxPhAT::NotifyPreChange( void* Src, UProperty* PropertyAboutToChange )
{
	if(!bRunningSimulation)
	{
		if(EditingMode == PEM_BodyEdit)
			GEditor->Trans->Begin( *LocalizeUnrealEd("EditBodyProperties") );
		else
			GEditor->Trans->Begin( *LocalizeUnrealEd("EditConstraintProperties") );
	}
