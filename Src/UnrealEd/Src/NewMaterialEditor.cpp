 ...
 */
class FExpressionPreview : public FMaterial, public FMaterialInstance
{
public:
	FExpressionPreview()
	{}
	...
	{
		// we only need the unlit, emissive, local vertex factory shaders for drawing an opaque Material Tile
		// @todo: Added a FindShaderType by fname or something"
		return (appStristr(ShaderType->GetName(), TEXT("UnlitPixel")) || appStristr(ShaderType->GetName(), TEXT("Emissive")) && appStristr(ShaderType->GetName(), TEXT("<FNoLightMapPolicy>"))) && VertexFactoryType == FindVertexFactoryType(FName(TEXT("FLocalVertexFactory"), FNAME_Find));
	}
	...
	virtual const FMaterial* GetMaterial() const
	{
		...
		{
			return GEngine->DefaultMaterial->GetInstanceInterface(FALSE)->GetMaterial();
		}
	}

	virtual UBOOL GetVectorValue(const FName& ParameterName,FLinearColor* OutValue) const
	{
		return Expression->GetOuterUMaterial()->GetInstanceInterface(0)->GetVectorValue(ParameterName,OutValue);
	}

	virtual UBOOL GetScalarValue(const FName& ParameterName,FLOAT* OutValue) const
	{
		return Expression->GetOuterUMaterial()->GetInstanceInterface(0)->GetScalarValue(ParameterName,OutValue);
	}

	virtual UBOOL GetTextureValue(const FName& ParameterName,const FTexture** OutValue) const
	{
		return Expression->GetOuterUMaterial()->GetInstanceInterface(0)->GetTextureValue(ParameterName,OutValue);
	}
	...
	virtual UBOOL NeedsBumpedLightmap() const { return FALSE; }
	//>@ ava
 ...
 */
class FPreviewMaterial : public FMaterialResource, public FMaterialInstance
{
public:
	...
	virtual UBOOL ShouldCache(EShaderPlatform Platform, const FShaderType* ShaderType, const FVertexFactoryType* VertexFactoryType) const
	{
		...
		if (appStristr(ShaderType->GetName(), TEXT("TEmissive")))
		{
			if (appStristr(ShaderType->GetName(), TEXT("<FNoLightMapPolicy>")))
			{
				bShaderTypeMatches = TRUE;
			}
		}
		if (appStristr(ShaderType->GetName(), TEXT("TUnlit")))
		{
			bShaderTypeMatches = TRUE;			
		}
		else if (appStristr(ShaderType->GetName(), TEXT("TTranslucency")))
		{
			bShaderTypeMatches = TRUE;
		}
	...
	virtual UBOOL GetVectorValue(const FName& ParameterName,FLinearColor* OutValue) const
	{
		return Material->GetInstanceInterface(0)->GetVectorValue(ParameterName,OutValue);
	}

	virtual UBOOL GetScalarValue(const FName& ParameterName,FLOAT* OutValue) const
	{
		return Material->GetInstanceInterface(0)->GetScalarValue(ParameterName,OutValue);
	}

	virtual UBOOL GetTextureValue(const FName& ParameterName,const FTexture** OutValue) const
	{
		return Material->GetInstanceInterface(0)->GetTextureValue(ParameterName,OutValue);
	}
};
...
static FExpressionInput* GetMaterialInput(UMaterial* Material, INT Index)
{
	...
	case 11: ExpressionInput = &Material->AmbientMask; break;	
...
static void BreakLinksToExpression(UMaterialExpression* InMaterialExpression, UMaterial* Material)
{
	...
	for ( INT ExpressionIndex = 0 ; ExpressionIndex < Material->Expressions.Num() ; ++ExpressionIndex )
	{
		...
		MaterialExpression->RemoveReferenceTo( InMaterialExpression );
	}
...
#define __CLEAR_REFERENCE_TO_EXPRESSION( MatExpr, Mat, Prop ) \
	...
	__CLEAR_REFERENCE_TO_EXPRESSION( InMaterialExpression, Material, AmbientMask );
									...
									UMaterial* Material, TArray<UMaterialExpression*>& OutNewExpressions, TArray<UMaterialExpression*>& OutNewComments)
{
	...
	for( INT SrcExpressionIndex = 0 ; SrcExpressionIndex < SrcExpressions.Num() ; ++SrcExpressionIndex )
	{
		...
		UClass*					SrcExpressionClass	= SrcExpression->GetClass();
		UMaterialExpression*	NewExpression		= ConstructObject<UMaterialExpression>( SrcExpressionClass, Material, NAME_None, RF_Transactional, SrcExpression );

		NewExpression->SetArchetype( SrcExpressionClass->GetDefaultObject() );
	...
	,	ScopedTransaction( NULL )
{
	...
	Material = (UMaterial*)UObject::StaticDuplicateObject(OriginalMaterial, OriginalMaterial, UObject::GetTransientPackage(), NULL, ~0, UPreviewMaterial::StaticClass()); 
	...
	MaterialInputs.AddItem( FMaterialInputInfo( TEXT("AmbientMask"), &Material->AmbientMask ) );
...
void WxMaterialEditor::DrawMaterialExpression(UMaterialExpression* MaterialExpression, UBOOL bExpressionSelected, FCanvas* Canvas)
{
	...
	INT MaxTextureDependencyLength = Material->MaterialResource->GetMaxTextureDependencyLength();
	const INT* TextureDependencyLength = Material->MaterialResource->GetTextureDependencyLengthMap().Find(MaterialExpression);
...
void WxMaterialEditor::DrawObjects(FViewport* Viewport, FCanvas* Canvas)
{
	...
	{
		const TMultiMap<UMaterialExpression*,FString>& CompileErrors = Material->MaterialResource->GetCompileErrors();
		INT ErrorY = 5;

		// Draw the translucent shader instruction count.
		DrawShaderInstructionCount(
			Canvas,
			Material,
			FindShaderTypeByName(TEXT("TTranslucencyPixelShaderFBlendTranslucentPolicyFNoLightMapPolicy")),
			TEXT("Translucency pixel shader"),
			5,
			ErrorY
			);

		// Draw the additive shader instruction count.
		DrawShaderInstructionCount(
			Canvas,
			Material,
			FindShaderTypeByName(TEXT("TTranslucencyPixelShaderFBlendAdditivePolicyFNoLightMapPolicy")),
			TEXT("Additive pixel shader"),
			5,
			ErrorY
			);

		// Draw the modulate shader instruction count.
		DrawShaderInstructionCount(
			Canvas,
			Material,
			FindShaderTypeByName(TEXT("TTranslucencyPixelShaderFBlendModulatePolicyFNoLightMapPolicy")),
			TEXT("Modulate pixel shader"),
			5,
			ErrorY
			);
		...
		DrawShaderInstructionCount(
			Canvas,
			Material,
			FindShaderTypeByName(TEXT("TEmissivePixelShader<FNoLightMapPolicy>")),
			TEXT("Emissive pixel shader w/o light-map"),
			5,
			ErrorY
			);
		...
		DrawShaderInstructionCount(
			Canvas,
			Material,
			FindShaderTypeByName(TEXT("TEmissivePixelShader<FLightMapTexturePolicy>")),
			TEXT("Emissive pixel shader w/ light-map"),
			5,
			ErrorY
			);
		...
		DrawShaderInstructionCount(
			Canvas,
			Material,
			FindShaderTypeByName(TEXT("TLightPixelShaderFPointAndSkyLightPolicy_TwoWayFNoStaticShadowingPolicyTRUE")),
			TEXT("Dynamic mesh pixel shader"),
			5,
			ErrorY
			);
		...
		DrawShaderInstructionCount(
			Canvas,
			Material,
			FindShaderTypeByName(TEXT("TLightPixelShaderFDirectionalLightPolicyFShadowTexturePolicyTRUE")),
		...
		for(TMultiMap<UMaterialExpression*,FString>::TConstIterator ErrorIt(CompileErrors);ErrorIt;++ErrorIt)
		{
			FLinkedObjDrawUtils::DrawShadowedString( Canvas, 5, ErrorY, *ErrorIt.Value(), GEngine->SmallFont, FLinearColor(1,0,0) );
			ErrorY += 20;
		}
	}
	Canvas->PopTransform();
