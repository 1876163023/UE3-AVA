...
void FInterpEdViewportClient::DrawGrid(FViewport* Viewport, FCanvas* Canvas, UBOOL bDrawTimeline)
{
	...
	while( LineNum*GridSpacing < ViewEndTime)
	{
		...
		if(bDrawTimeline)
		{
			...
			DrawLine2D(Canvas, FVector2D(LinePosX, ViewY - TotalBarHeight), FVector2D(LinePosX, ViewY), FColor(125,125,125) );
		...
		else
		{
			...
			DrawLine2D(Canvas, FVector2D(LinePosX, 0), FVector2D(LinePosX, ViewY - TotalBarHeight), FColor(125,125,125) );
		}
...
void FInterpEdViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	for(INT i=0; i<InterpEd->IData->InterpGroups.Num(); i++)
	{
		...
		if(Group->bVisible)
		{
			...
			DrawTile(Canvas, LabelWidth - 26, (0.5*GroupHeadHeight)-8, 16, 16, 0.f, 0.f, 1.f, 1.f, ButtonMat->GetInstanceInterface(0) );
			if(Canvas->IsHitTesting()) Canvas->SetHitProxy( NULL );
			...
			if(!Group->bCollapsed)
			{
				...
				for(INT j=0; j<Group->InterpTracks.Num(); j++)
				{
					...
					check(TrackIconMat);
					DrawTile(Canvas, -LabelWidth + 24, 0.5*(TrackHeight - 16), 16, 16, 0.f, 0.f, 1.f, 1.f, TrackIconMat->GetInstanceInterface(0) );
					...
					if(EventTrack)
					{
						...
						DrawTile(Canvas, -32, TrackHeight-11, 8, 8, 0.f, 0.f, 1.f, 1.f, BackwardMat->GetInstanceInterface(0) );
						if(Canvas->IsHitTesting()) Canvas->SetHitProxy( NULL );

						if(Canvas->IsHitTesting()) Canvas->SetHitProxy( new HInterpEdEventDirBtn(Group, j, IED_Forward) );
						DrawTile(Canvas, -22, TrackHeight-11, 8, 8, 0.f, 0.f, 1.f, 1.f, ForwardMat->GetInstanceInterface(0) );
						if(Canvas->IsHitTesting()) Canvas->SetHitProxy( NULL );
					}
					...
					if( Track->IsA(UInterpTrackFloatBase::StaticClass()) || Track->IsA(UInterpTrackVectorBase::StaticClass()) || Track->IsA(UInterpTrackMove::StaticClass()) )
					{
						...
						DrawTile(Canvas, -22, TrackHeight-11, 8, 8, 0.f, 0.f, 1.f, 1.f, GraphMat->GetInstanceInterface(0) );
						if(Canvas->IsHitTesting()) Canvas->SetHitProxy( NULL );
					}
					...
					if(Canvas->IsHitTesting()) Canvas->SetHitProxy( new HInterpEdTrackDisableTrackBtn(Group, j) );
					{
						...
						if(Track->bDisableTrack == FALSE)
						{
							DrawTile(Canvas, -LabelWidth+4, YPos, DisableIconSize.X, DisableIconSize.Y, 0,0,1,1, DisableTrackMat->GetInstanceInterface(0));
						}
					}			
					if(Canvas->IsHitTesting()) Canvas->SetHitProxy( NULL );
