...
/* Implemented in UnrealEd/EnvCubeRender.cpp */
void EnvCube_Render();

	
#if WITH_FACEFX
	#include "../../../External/FaceFX/FxStudio/Inc/FxStudioApp.h"
#endif // WITH_FACEFX
...
public:
	...
	WxMainMenu()
	{
		...
		// Build menu
		{
			...
			BuildMenu->Append( IDM_AVA_FULL_BUILD_GEOMETRY, *LocalizeUnrealEd("FullBuildGeometry"), *LocalizeUnrealEd("AvaToolTip_01") );
			BuildMenu->Append( IDM_AVA_BUILD_SOLID_GEOMETRY, *LocalizeUnrealEd("BuildSolidGeometry"), *LocalizeUnrealEd("AvaToolTip_02") );
			BuildMenu->Append( IDM_AVA_BUILD_SOLID_GEOMETRY_AND_COMPUTE_PVS, *LocalizeUnrealEd("BuildSolidGeometryAndComputePVS"), *LocalizeUnrealEd("AvaToolTip_03") );
			//>@ ava
			//<@ ava specific ; 2008. 3. 6 changmin
			BuildMenu->Append( IDM_AVA_CHECK_BRUSH_INTERSECTION, *LocalizeUnrealEd("CheckBrushIntersection"), *LocalizeUnrealEd("AvaToolTip_04"));
			BuildMenu->Append( IDM_AVA_CHECK_SOLID_BRUSH_INTERSECTION, *LocalizeUnrealEd("CheckSolidBrushIntersection"), *LocalizeUnrealEd("AvaToolTip_05"));
			//>@ ava
			...
			for (FConsoleSupportIterator It; It && ConsoleIndex < 20; ++It, ConsoleIndex++)
			{
				...
				if (It->GetNumMenuItems() > 0)
				{
					...
					for (INT ItemIndex = 0; ItemIndex < It->GetNumMenuItems() && ConsoleIndex < 40; ItemIndex++)
					{
						...
						{
							ConsoleMenu[ConsoleIndex]->Append(IDM_ConsoleSpecific_START + ConsoleIndex * 40 + ItemIndex, MenuLabel, TEXT(""), Kind);
							ConsoleMenu[ConsoleIndex]->Check(IDM_ConsoleSpecific_START + ConsoleIndex * 40 + ItemIndex, bIsChecked);
						}
					}
		...
		// Tools menu
		{
			ToolsMenu->Append( ID_Tools2DEditor, *LocalizeUnrealEd("2DShapeEditor"), *LocalizeUnrealEd("ToolTip_131") );
...
//<@ ava specific; 2006. 12. 29 changmin
class AvaClusterManager
{
public:
	INT GetNumClusters()
	{
		return ClusterNames_.Num();
	}
	FString	GetClusterName( INT Index )
	{
		check( Index < ClusterNames_.Num() );
		return ClusterNames_(Index);
	}
	UBOOL IsOccupied( INT Index )
	{
		return OccupiedClusters_.ContainsItem( Index );
	}
	void Init()
	{
#define AVA_ROOT L"\\\\avabuild\\avacontext\\rad_mpi\\"
		WIN32_FIND_DATA data;			
		HANDLE hFind = FindFirstFile( AVA_ROOT L"clusters\\*.*", &data );
		if (hFind != INVALID_HANDLE_VALUE)
		{
			do 
			{
				if (!(data.dwFileAttributes & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM)))
				{
					ClusterNames_.AddItem(data.cFileName);
				}
	...
	{
		WIN32_FIND_DATA data;			
		HANDLE hFind = FindFirstFile( AVA_ROOT L"occupied_clusters\\*.*", &data );
		FILETIME ft;
		...
		// 3 mins
		liCurrent.QuadPart -= 1800000000;
		ft.dwLowDateTime	= liCurrent.LowPart;
		ft.dwHighDateTime	= liCurrent.HighPart;
		// update occupied cluster list
		TArray<FString> OccupiedClusterNames;
		if (hFind != INVALID_HANDLE_VALUE)
		{
			do 
			{
				if (!(data.dwFileAttributes & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM)))
				{				
					if (CompareFileTime(&data.ftLastWriteTime,&ft) > 0) // -1 ftCreationTime is
					{
						OccupiedClusterNames.AddItem(data.cFileName);
					}				
				}
		...
		for (INT ClusterIndex = 0; ClusterIndex <ClusterNames_.Num(); ++ClusterIndex)
		{
			INT j;
			if (OccupiedClusterNames.FindItem( ClusterNames_(ClusterIndex), j ))
			{
				OccupiedClusters_.AddItem(ClusterIndex);
			}
		}
	}
private:
	TArray<FString>	ClusterNames_;
	TArray<INT>		OccupiedClusters_;
};
//>@
//<@ ava specific ; 2006. 12. 29 changmin
// pvs options
class AvaDlgPvsComputeOptions : public wxDialog
{
public:
	AvaDlgPvsComputeOptions();
	~AvaDlgPvsComputeOptions()
	{
		delete Timer;
	}
	...
	*/
	UBOOL ShowModal( FFullBuildGeometryOptions& Options );
	...
	// Check box
//	wxCheckBox *ComputePvsCheckBox;
	DECLARE_EVENT_TABLE()
private:
	wxTimer				*Timer;
	TArray<wxCheckBox*> Clusters;
	AvaClusterManager	ClusterManager;
};

BEGIN_EVENT_TABLE(AvaDlgPvsComputeOptions, wxDialog)
EVT_BUTTON( wxID_OK, AvaDlgPvsComputeOptions::OnOK )
END_EVENT_TABLE()

AvaDlgPvsComputeOptions::AvaDlgPvsComputeOptions()
{
	SetExtraStyle(GetExtraStyle()|wxWS_EX_BLOCK_EVENTS);
	wxSize DialogSize( 350, 500 );
	wxDialog::Create( GApp->EditorFrame, wxID_ANY, *LocalizeUnrealEd(TEXT("FBGO_FullBuildGeometryOptions")), wxDefaultPosition, DialogSize, wxCAPTION|wxDIALOG_MODAL|wxTAB_TRAVERSAL );
	wxBoxSizer* BoxSizer2 = new wxBoxSizer(wxVERTICAL);
	SetSizer(BoxSizer2);

	//ComputePvsCheckBox = new wxCheckBox( this, wxID_ANY, *LocalizeUnrealEd(TEXT("FBGO_ComputePvs")), wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
	//ComputePvsCheckBox->SetValue(FALSE);
	//BoxSizer2->Add(ComputePvsCheckBox, 0, wxGROW|wxALL, 5);

	wxStaticBox* StaticBoxSizer3Static = new wxStaticBox(this, wxID_ANY, *LocalizeUnrealEd(TEXT("FBGO_Clusters")));
	...
	// cluster
	wxSizer* ClusterSizer = new wxFlexGridSizer(3);
	StaticBoxSizer3->Add(ClusterSizer, 0, wxEXPAND|wxALL, 2);
	ClusterManager.Init();
	for( INT ClusterIndex = 0; ClusterIndex < ClusterManager.GetNumClusters(); ++ClusterIndex )
	{
		FString ClusterName = ClusterManager.GetClusterName( ClusterIndex );
		wxCheckBox* pCheckBox = new wxCheckBox( this, wxID_ANY, *ClusterName, wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
		pCheckBox->SetValue(FALSE);
		ClusterSizer->Add(pCheckBox, 0, wxGROW|wxALL, 5);
		Clusters.AddItem( pCheckBox );
	}
...
void AvaDlgPvsComputeOptions::UpdateClusters()
{
	ClusterManager.Update();
	for (INT ClusterIndex =0; ClusterIndex <ClusterManager.GetNumClusters(); ++ClusterIndex )
	{
		if( ClusterManager.IsOccupied(ClusterIndex) )
		{
			Clusters(ClusterIndex)->SetForegroundColour( wxColour( 0x0000af ) );
		}
		else
		{
			Clusters(ClusterIndex)->SetForegroundColour( wxColour( 0x00af00 ) );						
		}
	}
	Refresh(false);
}

UBOOL AvaDlgPvsComputeOptions::ShowModal( FFullBuildGeometryOptions &Options )
{
	// Retrieve values from ini. 
	//GConfig->GetBool( TEXT("FullBuildGeometryOptions"), TEXT("ComputePVS"),	Options.bComputePVS, GEditorIni );
	for (INT i=0; i<ClusterManager.GetNumClusters(); ++i)
	{
		UBOOL UseThisCluster = FALSE;
		FString ClusterName = ClusterManager.GetClusterName( i );
		GConfig->GetBool( TEXT("FullBuildGeometryOptions"),	*FString::Printf( TEXT("Cluster_%s"), *ClusterName ), UseThisCluster, GEditorIni );
		// populate dialog with read in values
		Clusters(i)->SetValue( !!UseThisCluster );
	}

	// Populate dialog with read in values
	//ComputePvsCheckBox->SetValue( Options.bComputePVS == TRUE );
	...
	if( wxDialog::ShowModal() == wxID_OK )
	{
		...
		//Options.bComputePVS = ComputePvsCheckBox->GetValue();
		Options.Clusters.Empty();
		for (INT i=0; i<ClusterManager.GetNumClusters(); ++i)
		{			
			if (Clusters(i)->IsChecked() && !ClusterManager.IsOccupied(i) )
			{
				Options.Clusters.AddUniqueItem( ClusterManager.GetClusterName(i) );
			}
		}
	}
	// Save 
	...
	{
		//GConfig->SetBool( TEXT("FullBuildGeometryOptions"), TEXT("ComputePVS"),	Options.bComputePVS, GEditorIni );
		for (INT i=0; i<ClusterManager.GetNumClusters(); ++i)
		{				
			UBOOL UseThisCluster = Clusters(i)->IsChecked();
			FString ClusterName = ClusterManager.GetClusterName(i);
			GConfig->SetBool( TEXT("FullBuildGeometryOptions"),	*FString::Printf( TEXT("Cluster_%s"), *ClusterName ), UseThisCluster, GEditorIni );				
		}
	}
...
public:
	...
	//<@ ava specific ; 2006. 12. 29 changmin
	wxCheckBox* UseRadiosityLightingCheckBox;
	wxCheckBox* UseRayTraceLightingCheckBox;
	wxCheckBox* UseMultiThreadCheckBox;
	wxCheckBox*	BakeAmbientCubesCheckBox;
	wxCheckBox* ExtraCheckBox;
	wxCheckBox* FastCheckBox;
	wxComboBox* AmbientTypeComboBox;
	TArray<wxCheckBox*>	Clusters;
	wxTimer*			Timer;
	AvaClusterManager	ClusterManager;
	void OnTimer( wxTimerEvent& In )
...
{
    SetExtraStyle(GetExtraStyle()|wxWS_EX_BLOCK_EVENTS);
	wxSize DialogSize( 350, 800 );
	wxDialog::Create( GApp->EditorFrame, wxID_ANY, *LocalizeUnrealEd(TEXT("LBO_LightingBuildOptions")), wxDefaultPosition, DialogSize, wxCAPTION|wxDIALOG_MODAL|wxTAB_TRAVERSAL );
	...
	//!{ 2006-05-02	Çã Ã¢ ¹Î
	wxSizer* RadiositySizer = new wxStaticBoxSizer(wxVERTICAL, this, *LocalizeUnrealEd(TEXT("LBO_RadiosityOptions")));
	StaticBoxSizer3->Add(RadiositySizer, 0, wxEXPAND|wxALL, 2);

	UseRadiosityLightingCheckBox = new wxCheckBox( this, wxID_ANY, *LocalizeUnrealEd(TEXT("LBO_UseRadiosityLighting")), wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
	UseRadiosityLightingCheckBox->SetValue(FALSE);
	RadiositySizer->Add(UseRadiosityLightingCheckBox, 0, wxGROW|wxALL, 5);

	UseRayTraceLightingCheckBox = new wxCheckBox( this, wxID_ANY, *LocalizeUnrealEd(TEXT("LBO_UseRaytraceLighting")), wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
	UseRayTraceLightingCheckBox->SetValue(FALSE);
	RadiositySizer->Add(UseRayTraceLightingCheckBox, 0, wxGROW|wxALL, 5);

	BakeAmbientCubesCheckBox= new wxCheckBox( this, wxID_ANY, *LocalizeUnrealEd(TEXT("LBO_BakeAmbientCubes")), wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
	BakeAmbientCubesCheckBox->SetValue(FALSE);
	RadiositySizer->Add(BakeAmbientCubesCheckBox, 0, wxGROW|wxALL, 5);

	UseMultiThreadCheckBox = new wxCheckBox( this, wxID_ANY, *LocalizeUnrealEd(TEXT("LBO_UseMultiThread")), wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
	UseMultiThreadCheckBox->SetValue(FALSE);
	RadiositySizer->Add(UseMultiThreadCheckBox, 0, wxGROW|wxALL, 5);

	ExtraCheckBox = new wxCheckBox( this, wxID_ANY, *LocalizeUnrealEd(TEXT("LBO_Extra")), wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
	ExtraCheckBox->SetValue(FALSE);
	RadiositySizer->Add(ExtraCheckBox, 0, wxGROW|wxALL, 5);

	FastCheckBox = new wxCheckBox( this, wxID_ANY, *LocalizeUnrealEd(TEXT("LBO_Fast")), wxDefaultPosition, wxDefaultSize, wxCHK_2STATE );
	FastCheckBox->SetValue(FALSE);
	RadiositySizer->Add(FastCheckBox, 0, wxGROW|wxALL, 5);

	//<@ ava specific ; 2006. 12. 29 changmin
	wxSizer* AmbientTypeSizer = new wxStaticBoxSizer(wxVERTICAL, this, *LocalizeUnrealEd(TEXT("LBO_AmbientType")));
	AmbientTypeComboBox = new wxComboBox( this, wxID_ANY, TEXT(""), wxDefaultPosition, wxDefaultSize, 0, NULL, wxCB_READONLY );
	AmbientTypeComboBox->AppendString(*LocalizeUnrealEd(TEXT("LBO_AmbientType_None")));
	AmbientTypeComboBox->AppendString(*LocalizeUnrealEd(TEXT("LBO_AmbientType_Simple")));
	AmbientTypeComboBox->AppendString(*LocalizeUnrealEd(TEXT("LBO_AmbientType_Full")));
	AmbientTypeSizer->Add( AmbientTypeComboBox, 0, wxEXPAND | wxALL, 2 );
	RadiositySizer->Add(AmbientTypeSizer, 0, wxGROW|wxALL, 5);
	// cluster
	wxSizer* ClusterSizer = new wxFlexGridSizer(3);
	RadiositySizer->Add(ClusterSizer, 0, wxEXPAND|wxALL, 2);
	ClusterManager.Init();
	for( INT ClusterIndex = 0; ClusterIndex < ClusterManager.GetNumClusters(); ++ClusterIndex )
	{
		FString ClusterName = ClusterManager.GetClusterName( ClusterIndex );
...
//<@ ava speicifc ; deif
void WxDlgLightingBuildOptions::UpdateClusters()
{
	ClusterManager.Update();
	for (INT ClusterIndex =0; ClusterIndex <ClusterManager.GetNumClusters(); ++ClusterIndex )
	{
...
UBOOL WxDlgLightingBuildOptions::ShowModal( FLightingBuildOptions& Options )
{
	...
	//<@ ava specific ; 2006. 12. 29 changmin
	GConfig->GetBool( TEXT("LightingBuildOptions"), TEXT("RadiosityLighting"),		Options.bUseRadiosityLighting,		GEditorIni );
	GConfig->GetBool( TEXT("LightingBuildOptions"), TEXT("RayTraceLighting"),		Options.bUseRayTraceLighting,		GEditorIni );
	GConfig->GetBool( TEXT("LightingBuildOptions"), TEXT("MultiThread"),			Options.bUseMultiThread,			GEditorIni);
	GConfig->GetBool( TEXT("LightingBuildOptions"), TEXT("BakeAmbientCubes"),		Options.bBakeAmbientCubes,			GEditorIni);
	GConfig->GetInt( TEXT("LightingBuildOptions"),	TEXT("AmbientType"),			Options.AmbientType,				GEditorIni);
	GConfig->GetInt( TEXT("LightingBuildOptions"),	TEXT("NumBounce"),				Options.NumBounce,					GEditorIni);
	
	Options.bExtra = TRUE;
	Options.bFast = FALSE;
	GConfig->GetBool( TEXT("LightingBuildOptions"), TEXT("Extra"),					Options.bExtra,						GEditorIni);
	GConfig->GetBool( TEXT("LightingBuildOptions"), TEXT("Fast"),					Options.bFast,						GEditorIni);
	for (INT i=0; i<ClusterManager.GetNumClusters(); ++i)
	{
		UBOOL UseThisCluster = FALSE;
		FString ClusterName = ClusterManager.GetClusterName( i );
		GConfig->GetBool( TEXT("LightingBuildOptions"),	*FString::Printf( TEXT("Cluster_%s"), *ClusterName ), UseThisCluster,		GEditorIni );
		Clusters(i)->SetValue( !!UseThisCluster );		
	}
	//>@ ava
	...
	//<@ ava specific ; 2006. 12 .29 changmin
	UseRadiosityLightingCheckBox->SetValue( Options.bUseRadiosityLighting == TRUE );
	UseRayTraceLightingCheckBox->SetValue( Options.bUseRayTraceLighting == TRUE );
	UseMultiThreadCheckBox->SetValue( Options.bUseMultiThread == TRUE );
	BakeAmbientCubesCheckBox->SetValue( Options.bBakeAmbientCubes == TRUE );
	ExtraCheckBox->SetValue( Options.bExtra == TRUE );
	FastCheckBox->SetValue( Options.bFast == TRUE );
	AmbientTypeComboBox->SetSelection( Options.AmbientType );
	//>@ ava
	...
	if( wxDialog::ShowModal() == wxID_OK )
	{
		...
		//<@ ava specific ; 2006. 12. 29 changmin
		Options.bUseRadiosityLighting		= UseRadiosityLightingCheckBox->GetValue();
		Options.bUseRayTraceLighting		= UseRayTraceLightingCheckBox->GetValue();
		Options.bUseMultiThread				= UseMultiThreadCheckBox->GetValue();
		Options.bBakeAmbientCubes			= BakeAmbientCubesCheckBox->GetValue();
		Options.bExtra						= ExtraCheckBox->GetValue();
		Options.bFast						= FastCheckBox->GetValue();
		Options.AmbientType					= AmbientTypeComboBox->GetCurrentSelection();
		Options.Clusters.Empty();
		for (INT i=0; i<ClusterManager.GetNumClusters(); ++i)
		{			
			if (Clusters(i)->IsChecked() && !ClusterManager.IsOccupied(i) )
		...
		if ( bProceedWithBuild )
		{
			...
			//<@ ava specific ; 2006. 12. 29 changmin
			GConfig->SetBool( TEXT("LightingBuildOptions"), TEXT("RadiosityLighting"),		Options.bUseRadiosityLighting,		GEditorIni );
			GConfig->SetBool( TEXT("LightingBuildOptions"), TEXT("RayTraceLighting"),		Options.bUseRayTraceLighting,		GEditorIni );
			GConfig->SetBool( TEXT("LightingBuildOptions"), TEXT("MultiThread"),			Options.bUseMultiThread,			GEditorIni );
			GConfig->SetBool( TEXT("LightingBuildOptions"), TEXT("BakeAmbientCubes"),		Options.bBakeAmbientCubes,			GEditorIni );			
			GConfig->SetBool( TEXT("LightingBuildOptions"), TEXT("Extra"),					Options.bExtra,						GEditorIni );
			GConfig->SetBool( TEXT("LightingBuildOptions"), TEXT("Fast"),					Options.bFast,						GEditorIni );
			GConfig->SetInt( TEXT("LightingBuildOptions"), TEXT("AmbientType"),				Options.AmbientType,				GEditorIni);
			for (INT i=0; i<ClusterManager.GetNumClusters(); ++i)
			{				
				UBOOL UseThisCluster = Clusters(i)->IsChecked();
				FString ClusterName = ClusterManager.GetClusterName(i);
				GConfig->SetBool( TEXT("LightingBuildOptions"),	*FString::Printf( TEXT("Cluster_%s"), *ClusterName ), UseThisCluster, GEditorIni );				
			}
			//>@ ava
		}
	}
...
BEGIN_EVENT_TABLE( WxEditorFrame, wxFrame )
	...
	EVT_MENU( IDM_BuildPlayInEditor_MannedPrecomputing, WxEditorFrame::MenuBuildPlayInEditor_MannedPrecomputing )
	EVT_MENU( IDM_BakeEnvCubes, WxEditorFrame::MenuBakeEnvCubes )
	...
	//<@ ava specific ; 2007. 2. 2 changmin
	EVT_MENU( IDM_AVA_FULL_BUILD_GEOMETRY, WxEditorFrame::MenuBuild)
	EVT_MENU( IDM_AVA_BUILD_SOLID_GEOMETRY, WxEditorFrame::MenuBuild)
	EVT_MENU( IDM_AVA_BUILD_SOLID_GEOMETRY_AND_COMPUTE_PVS, WxEditorFrame::MenuBuild)	// 2008. 2. 22 changmin
	//>@ ava
	//<@ ava specific ; 2008. 3. 6 changmin
	EVT_MENU( IDM_AVA_CHECK_BRUSH_INTERSECTION, WxEditorFrame::MenuBuild )	// 2008. 3. 6 changmin
	EVT_MENU( IDM_AVA_CHECK_SOLID_BRUSH_INTERSECTION, WxEditorFrame::MenuBuild )	// 2008. 3. 6 changmin
	//>@ ava
	...
	EVT_MENU( IDM_BackDropPopupPlayFromHereInEditor_Precompute, WxEditorFrame::MenuPlayFromHereInEditor_Precompute )
	...
	EVT_MENU( IDM_PUSHVIEW_START, WxEditorFrame::PushViewStart )
	EVT_MENU( IDM_PUSHVIEW_STOP, WxEditorFrame::PushViewStop )
	...
	EVT_MENU( IDMENU_ActorPopupConvertStaticMeshToavaKActor, WxEditorFrame::MenuConvertActors )
	EVT_MENU( IDMENU_ActorPopupConvertMoverToavaKActor, WxEditorFrame::MenuConvertActors )
	...
	EVT_MENU_RANGE (IDM_CREATEMATERIALINSTANCE_START, IDM_CREATEMATERIALINSTANCE_END, WxEditorFrame::MenuCreateMaterialInstance)
	EVT_MENU_RANGE (IDM_EDITMATERIALINSTANCE_START, IDM_EDITMATERIALINSTANCE_END, WxEditorFrame::MenuCreateMaterialInstance)
...
void WxEditorFrame::MenuBuildPlayInEditor_MannedPrecomputing( wxCommandEvent& In )
{
	GUnrealEd->PlayMap(NULL, NULL, -1, 1);
}

void WxEditorFrame::MenuBakeEnvCubes( wxCommandEvent& In )
{
	EnvCube_Render();
}
...
void WxEditorFrame::UpdateUIConsoleSpecific(wxUpdateUIEvent& In)
{
	...
	MainMenuBar->ConsoleMenu[Console]->SetLabel(IDM_ConsoleSpecific_START + Console * 40 + MenuItem, MenuLabel);
...
void WxEditorFrame::MenuBuild( wxCommandEvent& In )
{
	...
	//<@ ava specific ; 2006. 2. 2 changmin
	case IDM_AVA_FULL_BUILD_GEOMETRY:
		{
			FFullBuildGeometryOptions PvsComputeOptions;
			//AvaDlgPvsComputeOptions PvsComputeOptionsDialog;
			//if( PvsComputeOptionsDialog.ShowModal( PvsComputeOptions ) )
			{
				// We can't set the busy cursor for all windows, because lighting
				// needs a cursor for the lighting options dialog.
				const FScopedBusyCursor BusyCursor;
				GUnrealEd->FullBuildGeometry( PvsComputeOptions, TRUE );
			}
			break;
		}
	case IDM_AVA_BUILD_SOLID_GEOMETRY:
		{
			FFullBuildGeometryOptions PvsComputeOptions;
			//AvaDlgPvsComputeOptions PvsComputeOptionsDialog;
			//if( PvsComputeOptionsDialog.ShowModal( PvsComputeOptions ) )
			{
				const FScopedBusyCursor BusyCursor;
				GUnrealEd->bSolidOnly = TRUE;
				GUnrealEd->FullBuildGeometry( PvsComputeOptions, FALSE );
				GUnrealEd->bSolidOnly = FALSE;
			}
			break;
		}
	case IDM_AVA_BUILD_SOLID_GEOMETRY_AND_COMPUTE_PVS:
		{
			FFullBuildGeometryOptions PvsComputeOptions;
			//AvaDlgPvsComputeOptions PvsComputeOptionsDialog;
			//if( PvsComputeOptionsDialog.ShowModal( PvsComputeOptions ) )
	...
	//<@ ava specific ; 2008. 3. 6 changmin
	case IDM_AVA_CHECK_BRUSH_INTERSECTION:
		{
			const FScopedBusyCursor BusyCursor;
			GUnrealEd->AVA_CheckBrushIntersection(FALSE);
			break;
		}
	case IDM_AVA_CHECK_SOLID_BRUSH_INTERSECTION:
		{
			const FScopedBusyCursor BusyCursor;
			GUnrealEd->AVA_CheckBrushIntersection(TRUE);
			break;
		}
	//>@ ava
...
{
	GApp->TwoDeeShapeEditor = new WxTwoDeeShapeEditor( this, -1 );
	GApp->TwoDeeShapeEditor->Show();
}
...
struct FConvertStaticMeshActorInfo
{
	...
	void GetFromActor(AActor* Actor, UStaticMeshComponent* MeshComp)
	{
		...
		CullDistanceEx			= MeshComp->CullDistanceEx;
	...
	void SetToActor(AActor* Actor, UStaticMeshComponent* MeshComp)
	{
		...
		MeshComp->CullDistanceEx		= CullDistanceEx;
...
void WxEditorFrame::MenuConvertActors( wxCommandEvent& In )
{
	...
	const UBOOL bFromStaticMesh = (In.GetId() == IDMENU_ActorPopupConvertStaticMeshToKActor || In.GetId() == IDMENU_ActorPopupConvertStaticMeshToavaKActor || In.GetId() == IDMENU_ActorPopupConvertStaticMeshToMover);
	...
	const UBOOL bFromMover = (In.GetId() == IDMENU_ActorPopupConvertMoverToStaticMesh || In.GetId() == IDMENU_ActorPopupConvertMoverToKActor || In.GetId() == IDMENU_ActorPopupConvertMoverToavaKActor);
	...
	const UBOOL bToavaKActor = (In.GetId() == IDMENU_ActorPopupConvertStaticMeshToavaKActor || In.GetId() == IDMENU_ActorPopupConvertMoverToavaKActor);
	...
	if ( GUnrealEd->edactDeleteSelected() )
	{
		...
		for( INT i = 0 ; i < ConvertInfo.Num() ; ++i )
		{
			...
			else if( bToavaKActor )
			{
				UClass* ava_GetAvaKActorStaticClass();
				// Make current the level into which the new actor is spawned.
				GWorld->CurrentLevel = Info.SourceLevel;
				AKActor* KActor = CastChecked<AKActor>( GWorld->SpawnActor(ava_GetAvaKActorStaticClass(), NAME_None, Info.Location, Info.Rotation) );
				KActor->ClearComponents();
				Info.SetToActor(KActor, KActor->StaticMeshComponent);
				KActor->ConditionalUpdateComponents();
				GEditor->SelectActor( KActor, TRUE, NULL, FALSE );
...
{
	GUnrealEd->Exec( *FString::Printf( TEXT("MAP SETBRUSH CLEARFLAGS=%d SETFLAGS=%d"), (INT)(PF_Semisolid + PF_NotSolid), 0 ) );
}
...
void WxEditorFrame::MenuCreateMaterialInstance( wxCommandEvent &In )
{
	...
	if(EventID >= IDM_CREATEMATERIALINSTANCE_START && EventID <= IDM_CREATEMATERIALINSTANCE_END)
	{
		MaterialIdx = In.GetId() - IDM_CREATEMATERIALINSTANCE_START;
		bCreate = TRUE;
	}
	else
	{
		MaterialIdx = In.GetId() - IDM_EDITMATERIALINSTANCE_START;
	}
	...
	if(SelectedActors->Num()==1)
	{
		...
		if(FirstActor)
		{
			...
			UMaterialInstance* SelectedMaterial = NULL;
			...
			if(SelectedMaterial)
			{
				...
				if(bCreate && InstanceToEdit==NULL)
				{
					...
					wxTextEntryDialog TextDlg(this, *LocalizeUnrealEd("EnterMaterialInstanceName"), 
						*LocalizeUnrealEd("PleaseEnterValue"), *DefaultName);
					if(TextDlg.ShowModal()==wxID_OK)
					{
						...
						{	
							InstanceToEdit = ConstructObject<UMaterialInstanceConstant>(UMaterialInstanceConstant::StaticClass(), LevelPackage, 
								ObjectName.c_str(), RF_Transactional);
							if(InstanceToEdit)
							{
				...
				{
					wxFrame* MaterialInstanceEditor = new WxMaterialInstanceEditor( (wxWindow*)GApp->EditorFrame,-1, InstanceToEdit );
			...
			for(INT ComponentIdx=0;ComponentIdx<FirstActor->Components.Num(); ComponentIdx++)
			{
				...
				if(MeshComponent != NULL)
				{
					...
					else
					{
						...
						{
							UMaterialInstance* InstanceToAssign = MaterialSelection->GetTop<UMaterialInstance>();
...
static void PlayFromHere(INT WhichConsole = -1,INT PrecomputeFlags = 0)
{
	...
	if (PrecomputeFlags & 1)
	{
		extern void BeginUpdatingCullDistances();
	...
	GUnrealEd->PlayMap(&StartLocation, StartRotation, WhichConsole, PrecomputeFlags);
}
...
void WxEditorFrame::MenuPlayFromHereInEditor_Precompute( wxCommandEvent& In )
{
	// start a play from here, in the local editor
	PlayFromHere(-1,1);
}
...
void WxEditorFrame::PushViewStart(wxCommandEvent& In)
{
	GEditor->bIsPushingView = TRUE;
	GEngine->Exec(TEXT("REMOTE PUSHVIEW START"), *GLog);
}

void WxEditorFrame::PushViewStop(wxCommandEvent& In)
{
	GEditor->bIsPushingView = FALSE;
	GEngine->Exec(TEXT("REMOTE PUSHVIEW STOP"), *GLog);
}
