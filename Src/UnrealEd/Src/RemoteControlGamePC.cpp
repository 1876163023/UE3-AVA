...
UBOOL FRemoteControlGamePC::ShowEditActor(const TCHAR* ActorName)
{
	...
	if( !PropertyWindowFrame )
	{
		...
		if( Actor )
		{
			...
			PropertyWindowFrame->Create( NULL, -1, TRUE );
			PropertyWindowFrame->DisallowClose();
			PropertyWindowFrame->SetObject( Actor, FALSE, TRUE, TRUE );
...
void FRemoteControlGamePC::RefreshActorPropertyWindowList()
{
	for(TMap<FString, WxPropertyWindowFrame*>::TIterator It(ActorEditors); It; ++It)
	{
		...
		else
		{
			...
			{
				// Refresh.
				It.Value()->SetObject( Actor, FALSE, TRUE, TRUE );
			}
		}
	}
}
...
namespace
{
		...
		void GetActorListHelper(TArray<FRemoteControlGame::ActorDescription>& Actors, IteratorType& It)
	{
		for( ; It ; ++It )
		{
			...
			ActorDesc.ActorName = Actor->GetName();
			if( Actor->Owner )
			{
...
{
	// Turn into an FName.
	SelectedActorName = MakeFName(ActorName);
}

FString FRemoteControlGamePC::GetSelectedActor() const
{
	return SelectedActorName.ToString();
}
...
{
	return FindActor( MakeFName(ActorName) );
}

/**
 * Finds an actor by name.
 */
AActor* FRemoteControlGamePC::FindActor(const FName& InName) const
{
	// Need to check that the name is valid before sending it to StaticFindObjectFast
	// the checkslow code now works and sending in an uninitialized FName::Index will app assert
	// note: there appears to be an uninitialized index in many packages in core.
	if( !InName.IsValid() )
	{
		return NULL;
	}
	...
	check( World->CurrentLevel == World->PersistentLevel );
	return Cast<AActor>( UObject::StaticFindObjectFast(AActor::StaticClass(), World->PersistentLevel, InName) );
}
...
UBOOL FRemoteControlGamePC::GetPropertyList(const FString& ObjectName, TArray<FString>& OutPropList)
{
	...
	{
		bFoundObject = TRUE;
		for (TFieldIterator<UProperty,CLASS_IsAUProperty> It(Object->GetClass()); It; ++It)
		{
											  ...
											  TArray<FString>& OutArrayObjectList)
{
	...
	{
		FArray* ArrayProp = (FArray*)(((BYTE*)Object) + pProp->Offset);
