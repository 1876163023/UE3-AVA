...
FCascadeEmitterEdViewportClient::FCascadeEmitterEdViewportClient(WxCascade* InCascade) :
	  Cascade(InCascade)
	, IconMatRenderNormal(NULL)
	, IconMatRenderCross(NULL)
	, IconMatRenderPoint(NULL)
	, IconMatRenderNone(NULL)
	, IconMatCurveEdit(NULL)
{
...
void FCascadeEmitterEdViewportClient::DrawHeaderBlock(INT Index, INT XPos, UParticleEmitter* Emitter, FViewport* Viewport,FCanvas* Canvas)
{
	...
	if (SpriteEmitter)
	{
		...
		if (SpriteEmitter->Material && !Canvas->IsHitTesting())
		{
			// Get the rendering info for this object
			FThumbnailRenderingInfo* RenderInfo =
				GUnrealEd->GetThumbnailManager()->GetRenderingInfo(SpriteEmitter->Material);
			// If there is an object configured to handle it, draw the thumbnail
			if (RenderInfo != NULL && RenderInfo->Renderer != NULL)
			{
				RenderInfo->Renderer->Draw(SpriteEmitter->Material,TPT_Plane,
					ThumbPos.X,ThumbPos.Y,ThumbSize,ThumbSize,Viewport,Canvas,FALSE);
			}
		}
		else
		{
			DrawTile(Canvas,ThumbPos.X, ThumbPos.Y, ThumbSize, ThumbSize, 0.f, 0.f, 1.f, 1.f, FLinearColor::Black);		
		}
	}
	...
	//@todo.SAS. Need the DrawTile(..., MaterialInstance*) version of the function!
#if defined(_CASC_USE_ICON_MATERIALS_)
	// Draw enable/disable button
	FMaterialInstance* EnabledIconInst = NULL;
	if (LODLevel->bEnabled)
	{
		EnabledIconInst	= GetIcon(CASC_Icon_Module_Enabled);
	}
	else
	{
		EnabledIconInst	= GetIcon(CASC_Icon_Module_Disabled);
	}
	check(EnabledIconInst);
	if (Canvas->IsHitTesting())
        Canvas->SetHitProxy(new HCascadeEmitterEnableProxy(Emitter));
	DrawTile(Canvas,XPos + 12, 26, 16, 16, 0.f, 0.f, 1.f, 1.f, EnabledIconInst);
	if (Canvas->IsHitTesting())
        Canvas->SetHitProxy(NULL);

	// Draw rendering mode button.
	FMaterialInstance* IconInst	= NULL;
//	switch (Emitter->EmitterRenderMode)
	switch (LODLevel->RequiredModule->EmitterRenderMode)
	{
	case ERM_Normal:
		IconInst	= GetIcon(CASC_Icon_Render_Normal);
		break;
	case ERM_Point:
		IconInst	= GetIcon(CASC_Icon_Render_Point);
		break;
	case ERM_Cross:
		IconInst	= GetIcon(CASC_Icon_Render_Cross);
		break;
	case ERM_None:
		IconInst	= GetIcon(CASC_Icon_Render_None);
		break;
	}
	check(IconInst);
	...
	DrawTile(Canvas,XPos + 32, 26, 16, 16, 0.f, 0.f, 1.f, 1.f, IconInst);
	if (Canvas->IsHitTesting())
        Canvas->SetHitProxy(NULL);
	...
	DrawTile(Canvas,XPos + 52, 26, 16, 16, 0.f, 0.f, 1.f, 1.f, GetIcon(CASC_Icon_CurveEdit));
	if (Canvas->IsHitTesting())
        Canvas->SetHitProxy(NULL);
#else	//#if defined(_CASC_USE_ICON_MATERIALS_)
	// Draw enable/disable button
	...
	if (Canvas->IsHitTesting())
        Canvas->SetHitProxy(NULL);
#endif	//#if defined(_CASC_USE_ICON_MATERIALS_)
...
#if defined(_CASC_USE_ICON_MATERIALS_)
	DrawTile(Canvas,EmitterWidth - 20, 2, 16, 16, 0.f, 0.f, 1.f, 1.f, GetIcon(CASC_Icon_CurveEdit));
#else//#if defined(_CASC_USE_ICON_MATERIALS_)
...
void FCascadeEmitterEdViewportClient::Draw3DDrawButton(UParticleEmitter* Emitter, UParticleModule* Module, UBOOL bHitTesting, FCanvas* Canvas)
{
	...
	{
#if defined(_CASC_USE_ICON_MATERIALS_)
		DrawTile(Canvas,EmitterWidth - 40, 21, 16, 16, 0.f, 0.f, 1.f, 1.f, GetIcon(CASC_Icon_3DDraw_Enabled));
#else	//#if defined(_CASC_USE_ICON_MATERIALS_)
	...
	{
#if defined(_CASC_USE_ICON_MATERIALS_)
		DrawTile(Canvas,EmitterWidth - 40, 21, 16, 16, 0.f, 0.f, 1.f, 1.f, GetIcon(CASC_Icon_3DDraw_Disabled));
#else	//#if defined(_CASC_USE_ICON_MATERIALS_)
...
void FCascadeEmitterEdViewportClient::DrawEnableButton(UParticleEmitter* Emitter, UParticleModule* Module, UBOOL bHitTesting, FCanvas* Canvas)
{
	...
	{
#if defined(_CASC_USE_ICON_MATERIALS_)
		DrawTile(Canvas,EmitterWidth - 20, 21, 16, 16, 0.f, 0.f, 1.f, 1.f, GetIcon(CASC_Icon_Module_Enabled));
#else	//#if defined(_CASC_USE_ICON_MATERIALS_)
	...
	{
#if defined(_CASC_USE_ICON_MATERIALS_)
		DrawTile(Canvas,EmitterWidth - 20, 21, 16, 16, 0.f, 0.f, 1.f, 1.f, GetIcon(CASC_Icon_Module_Disabled));
#else	//#if defined(_CASC_USE_ICON_MATERIALS_)
...
FMaterialInstance* FCascadeEmitterEdViewportClient::GetIcon(Icons eIcon)
{
#if defined(_CASC_USE_ICON_MATERIALS_)
	switch (eIcon)
	{
	case CASC_Icon_Render_Normal:
		return IconMatRenderNormal->GetInstanceInterface(0);
	case CASC_Icon_Render_Cross:
		return IconMatRenderCross->GetInstanceInterface(0);
	case CASC_Icon_Render_Point:
		return IconMatRenderPoint->GetInstanceInterface(0);
	case CASC_Icon_Render_None:
		return IconMatRenderNone->GetInstanceInterface(0);
	case CASC_Icon_CurveEdit:
		return IconMatCurveEdit->GetInstanceInterface(0);
	case CASC_Icon_3DDraw_Enabled:
		return IconMat3DDrawEnabled->GetInstanceInterface(0);
	case CASC_Icon_3DDraw_Disabled:
		return IconMat3DDrawDisabled->GetInstanceInterface(0);
	case CASC_Icon_Module_Enabled:
		return IconMatModuleEnabled->GetInstanceInterface(0);
	case CASC_Icon_Module_Disabled:
		return IconMatModuleDisabled->GetInstanceInterface(0);
	}
#endif	//#if defined(_CASC_USE_ICON_MATERIALS_)
...
{
#if defined(_CASC_USE_ICON_MATERIALS_)
	IconMatRenderNormal		= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_RenderNormal_MAT"), NULL, LOAD_None, NULL);
	check(IconMatRenderNormal);
	IconMatRenderCross		= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_RenderCross_MAT"), NULL, LOAD_None, NULL);
	check(IconMatRenderCross);
	IconMatRenderPoint		= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_RenderPoint_MAT"), NULL, LOAD_None, NULL);
	check(IconMatRenderPoint);
	IconMatRenderNone		= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_RenderNone_MAT"), NULL, LOAD_None, NULL);
	check(IconMatRenderNone);
	IconMatCurveEdit		= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_CurveEd_MAT"), NULL, LOAD_None, NULL);
	check(IconMatCurveEdit);
	IconMat3DDrawEnabled	= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_RenderCross_MAT"), NULL, LOAD_None, NULL);
	check(IconMat3DDrawEnabled);
	IconMat3DDrawDisabled	= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_RenderNone_MAT"), NULL, LOAD_None, NULL);
	check(IconMat3DDrawDisabled);
	IconMatModuleEnabled	= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_ModuleEnable_MAT"), NULL, LOAD_None, NULL);
	check(IconMatModuleEnabled);
	IconMatModuleDisabled	= (UMaterial*)UObject::StaticLoadObject(UMaterial::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_ModuleDisable_MAT"), NULL, LOAD_None, NULL);
	check(IconMatModuleDisabled);
#else	//#if defined(_CASC_USE_ICON_MATERIALS_)
	IconMatRenderNormal		= NULL;
	IconMatRenderCross		= NULL;
	IconMatRenderPoint		= NULL;
	IconMatRenderNone		= NULL;
	IconMatCurveEdit		= NULL;
	IconMat3DDrawEnabled	= NULL;
	IconMat3DDrawDisabled	= NULL;
	IconMatModuleEnabled	= NULL;
	IconMatModuleDisabled	= NULL;
...
//							Emitter->bEnabled	= LODLevel->bEnabled;
...
UBOOL FCascadeEmitterEdViewportClient::InputAxis(FViewport* Viewport, INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
	return FALSE;
}
