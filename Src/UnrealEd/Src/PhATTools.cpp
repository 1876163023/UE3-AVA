...
static const FLOAT  RotateSpeed = static_cast<FLOAT>( 1.0*(PI/180.0) );
...
void WxPhAT::ChangeDefaultSkelMesh()
{
	...
	UBOOL bDoChange = appMsgf( AMT_YesNo, *LocalizeUnrealEd("SureChangeAssetSkelMesh"), *PhysicsAsset->GetName(), *NewSkelMesh->GetName() );
	if(bDoChange)
	{
		...
		for(INT i=0; i<PhysicsAsset->BodySetup.Num(); i++)
		{
			...
			{
				appMsgf( AMT_OK, *LocalizeUnrealEd("BoneMissingFromSkelMesh"), *BodyName.ToString() );
				return;
			}
		}
...
void WxPhAT::SetSelectedBody(INT BodyIndex, EKCollisionPrimitiveType PrimitiveType, INT PrimitiveIndex)
{
	...
	if(SelectedBodyIndex == INDEX_NONE)
	{
		...
		PropertyWindow->SetObject(EditorSimOptions, true, false, true);
	}
	else
	{
		...
		{
			PropertyWindow->SetObject(PhysicsAsset->DefaultInstance->Bodies(SelectedBodyIndex), true, false, true);
		}
		else
		{
			PropertyWindow->SetObject(PhysicsAsset->BodySetup(SelectedBodyIndex), true, false, true);
		}
	}
...
void WxPhAT::SetSelectedConstraint(INT ConstraintIndex)
{
	...
	else
	{
		...
		if(bShowInstanceProps)
		{
			PropertyWindow->SetObject(PhysicsAsset->DefaultInstance->Constraints(SelectedConstraintIndex), true, false, true);
		}
		else
		{
			PropertyWindow->SetObject(PhysicsAsset->ConstraintSetup(SelectedConstraintIndex), true, false, true);
		}
	}	
...
void WxPhAT::WeldBodyToSelected(INT AddBodyIndex)
{
	...
	//UBOOL bDoWeld = appMsgf(1,  *FString::Printf(*LocalizeUnrealEd("Prompt_14"), *AddBoneName, *SelectedBoneName) );
	UBOOL bDoWeld = true;
	if(bDoWeld)
	{
		{
...
void WxPhAT::MakeNewBody(INT NewBoneIndex)
{
	...
	for(INT i=0; i<EditorSkelMesh->RefSkeleton.Num(); i++)
	{
		if( EditorSkelMesh->BoneIsChildOf(i, NewBoneIndex) )
		{
			...
			if(BodyIndex != INDEX_NONE)
			{
				appMsgf(AMT_OK, *LocalizeUnrealEd("Error_CantMakeNewBonePhysical"));
				return;
			}
		}
	}
...
void WxPhAT::ShowSimOptionsWindow()
{
	if(!SimOptionsWindow)
	{
		...
		SimOptionsWindow->Create( this, -1, false, this );
		SimOptionsWindow->SetSize( 64,64, 350,600 );
	}

	SimOptionsWindow->SetObject( EditorSimOptions, true, false, true );
...
void WxPhAT::ToggleInstanceProperties()
{
	...
	if(EditingMode == PEM_ConstraintEdit)
	{
		if(SelectedConstraintIndex != INDEX_NONE)
		{
			...
			if(bShowInstanceProps)
			{
				PropertyWindow->SetObject(ConInstance, true, false, true);
			}
			else
			{
				PropertyWindow->SetObject(ConSetup, true, false, true);
			}
		}
	}
	else if( EditingMode == PEM_BodyEdit )
	{
		if(SelectedBodyIndex != INDEX_NONE)
		{
			...
			if(bShowInstanceProps)
			{
				PropertyWindow->SetObject(BodyInstance, true, false, true);
			}
			else
			{
				PropertyWindow->SetObject(BodySetup, true, false, true);
			}
		}
	}
}
...
static wxTreeItemId CreateTreeItemAndParents(INT BoneIndex, TArray<FMeshBone>& Skeleton, wxTreeCtrl* TreeCtrl, TMap<FName,wxTreeItemId>& BoneTreeItemMap)
{
	...
	{
		wxTreeItemId parentItem = CreateTreeItemAndParents(Bone.ParentIndex, Skeleton, TreeCtrl, BoneTreeItemMap);
...
void WxPhAT::FillTree()
{
	TreeCtrl->Freeze();
	{
		...
		if( NextSelectEvent == PNS_MakeNewBody )
		{
			...
			{
				UBOOL bCanBeSelected = TRUE;

				// Check that this bone has no physics children.
				for(INT i=BoneIndex; i<EditorSkelMesh->RefSkeleton.Num(); i++)
				{
					if( EditorSkelMesh->BoneIsChildOf(i, BoneIndex) )
					{
						INT BodyIndex = PhysicsAsset->FindBodyIndex(EditorSkelMesh->RefSkeleton(i).Name);
						if( BodyIndex != INDEX_NONE )
						{
				...
				wxTreeItemId newItem = CreateTreeItemAndParents(BoneIndex, EditorSkelMesh->RefSkeleton, TreeCtrl, BoneTreeItemMap);
...
void WxPhAT::OnTreeSelChanged( wxTreeEvent& In )
{
	...
	// eat the next keystroke (mouse release)
	bIgnoreNextKey = TRUE;
...
void WxPhAT::StartManipulating(EAxis Axis, const FViewportClick& Click, const FMatrix& WorldToCamera)
{
	...
	if(EditingMode == PEM_BodyEdit)
	{
		...
		GEditor->Trans->Begin( *LocalizeUnrealEd("MoveElement") );
		PhysicsAsset->BodySetup(SelectedBodyIndex)->Modify();
	}
	else
	{
		check(SelectedConstraintIndex != INDEX_NONE);
		GEditor->Trans->Begin( *LocalizeUnrealEd("MoveConstraint") );
...
void WxPhAT::SimMousePress(FViewport* Viewport, UBOOL bConstrainRotation, FName Key)
{
	...
	FSceneViewFamilyContext ViewFamily(Viewport,PhATViewportClient->GetScene(),PhATViewportClient->ShowFlags,GWorld->GetTimeSeconds(),GWorld->GetRealTimeSeconds(),NULL);
	...
	const UBOOL bHit = !PhysicsAsset->LineCheck( Result, EditorSkelComp, Click.GetOrigin(),Click.GetOrigin() + Click.GetDirection() * SimGrabCheckDistance, FVector(0) );
...
{
	// don't start simulation if there are no bodies
	if( PhysicsAsset->BodySetup.Num() == 0 )
	{
		return;  
	}
...
void WxPhAT::DeleteCurrentPrim()
{
	...
	if( bs->AggGeom.GetElementCount() == 1 )
	{
		...
		if(EditorSimOptions->bPromptOnBoneDelete)
			bDoDelete = appMsgf(AMT_YesNo, *LocalizeUnrealEd("Prompt_15"));
