	...
	PreviewScene( FRotator(-45.f*(65535.f/360.f), 180.f*(65535.f/360.f), 0.f), 0.25f, 1.f)
{
	...
	// Load the standard editor cube.
	UStaticMesh* Mesh = (UStaticMesh*)UObject::StaticLoadObject(UStaticMesh::StaticClass(),NULL,TEXT("EditorMeshes.Cascade.BackgroundCube"),NULL,LOAD_None,NULL);
	check(Mesh);
	BackgroundCube = ConstructObject<UStaticMeshComponent>(UStaticMeshComponent::StaticClass());
	check(BackgroundCube);

	BackgroundCube->StaticMesh = Mesh;

	BackgroundMatInst = (UMaterialInstanceConstant*)UObject::StaticLoadObject(UMaterialInstanceConstant::StaticClass(), NULL, TEXT("EditorMaterials.Cascade.CASC_BackgroundCube_MATInst"), NULL, LOAD_None, NULL);
	check(BackgroundMatInst);

	BackgroundMatInst->SetVectorParameterValue(FName(TEXT("Color")), FLinearColor(0.f, 0.f, 0.f, 1.f));

	if (BackgroundCube->Materials.Num() == 0)
	{
		BackgroundCube->Materials.AddZeroed(1);
	}
	BackgroundCube->Materials(0) = BackgroundMatInst;
	BackgroundCube->Scale = 2048.0f;

	BackgroundCube->DepthPriorityGroup = SDPG_World;

	PreviewScene.AddComponent(BackgroundCube,FMatrix::Identity);
...
void FCascadePreviewViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
					...
					if (LODLevel && (LODLevel->RequiredModule->EmitterRenderMode != ERM_None))
					{
						if (bDrawParticleCounts)
						{
							strOutput += FString::Printf(TEXT("%4d/%4d"), 
								Instance->ActiveParticles, LODLevel->PeakActiveParticles);
						}
	...
	{
		UINT Width = Viewport->GetSizeX();
		UINT Height = Viewport->GetSizeY();
		// Read the contents of the viewport into an array.
		TArray<FColor> Bitmap;
		if (Viewport->ReadPixels(Bitmap))
		{
			check(Bitmap.Num() == Width * Height);

			// Create a texture
			UTexture2D* NewShot = ConstructObject<UTexture2D>(UTexture2D::StaticClass(), Cascade->PartSys);
			check(NewShot);
			NewShot->Init(Width, Height, PF_A8R8G8B8);
			
			// Create MipMaps for the texture we created.
			BYTE* MipData = (BYTE*)(NewShot->Mips(0).Data.Lock(LOCK_READ_WRITE));
			for (UINT y = 0; y < Height; y++)
			{
				BYTE* DestPtr = &MipData[y * Width * 4];
				FColor* SrcPtr = const_cast<FColor*>(&Bitmap(y * Width));
			...
			NewShot->Mips(0).Data.Unlock();

			NewShot->UpdateResource();

			Cascade->PartSys->ThumbnailImage = NewShot;
...
void FCascadePreviewViewportClient::Draw(const FSceneView* View,FPrimitiveDrawInterface* PDI)
{
	...
	Cascade->PartSysComp->SetLODLevel(Cascade->PartSys->PreviewLODSetting);
}
...
UBOOL FCascadePreviewViewportClient::InputAxis(FViewport* Viewport, INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
/***
