...
void UMaterialInstanceThumbnailRenderer::Draw(UObject* Object,
	...
	FViewport* Viewport,FCanvas* Canvas,UBOOL bShowBackground)
{
	UMaterialInstance* MatInst = Cast<UMaterialInstance>(Object);
	if (MatInst != NULL)
	{
		...
		FMaterialThumbnailScene	ThumbnailScene(MatInst,PrimType,0,0,Scale,
			bShowBackground);
	...
	FViewport* Viewport,FCanvas* Canvas,UBOOL bShowBackground)
{
	...
	{
		if (GUnrealEd->GetThumbnailManager()->bPSysRealTime == TRUE)
		{
...
const FShader* GetMaterialShader(UMaterial* Material,const TCHAR* ShaderTypeName)
{
													...
													TArray<FString>& OutLabels)
{
	...
	if (MaterialInstance != NULL)
	{
		...
		if ( Material )
		{
			...
			const FShader* EmissiveShader = GetMaterialShader(Material,TEXT("TEmissivePixelShader<FLightMapTexturePolicy>"));			
			const FShader* LightShader = GetMaterialShader(Material,TEXT("TLightPixelShaderFPointAndSkyLightPolicy_TwoWayFNoStaticShadowingPolicyTRUE"));
			const FShader* TranslucentShader = GetMaterialShader(Material,TEXT("TTranslucencyPixelShaderFBlendTranslucentPolicyFNoLightMapPolicy"));
			const FShader* AdditiveShader = GetMaterialShader(Material,TEXT("TTranslucencyPixelShaderFBlendAdditivePolicyFNoLightMapPolicy"));
			const FShader* ModulateShader = GetMaterialShader(Material,TEXT("TTranslucencyPixelShaderFBlendModulatePolicyFNoLightMapPolicy"));

			// Build the instruction count summary string from the available shaders.
			if(EmissiveShader && LightShader)
			{
				new(OutLabels) FString(FString::Printf(
					TEXT("%u/%u instructions"),
					EmissiveShader->GetNumInstructions(),
					LightShader->GetNumInstructions()
					));
			}
			else
			{
				const FShader* PrimaryShader = NULL;
				if(EmissiveShader)
				{
					PrimaryShader = EmissiveShader;
				}
				else if(TranslucentShader)
				{
					PrimaryShader = TranslucentShader;
				}
				else if(AdditiveShader)
				{
					PrimaryShader = AdditiveShader;
				}
				else if(ModulateShader)
				{
					PrimaryShader = ModulateShader;
				}
				...
				if(PrimaryShader)
				{
					new(OutLabels) FString(FString::Printf(
						TEXT("%u instructions"),
						PrimaryShader->GetNumInstructions()
						));
				}
			}
			...
			MaterialInstance->GetTextures(Textures);

			new(OutLabels) FString(FString::Printf(TEXT("%u textures"),Textures.Num()));
...
UBOOL UArchetypeThumbnailRenderer::SupportsThumbnailRendering(UObject* Object,UBOOL bCheckObjectState/*=TRUE*/)
{
	...
	UBOOL bWithinPrefabOrArchetype = FALSE;
	UObject* OuterObj = Object->GetOuter();
	while ( OuterObj && !bWithinPrefabOrArchetype )
	{
		if(OuterObj->IsA(UPrefab::StaticClass()) || OuterObj->HasAnyFlags(RF_ArchetypeObject))
		{
			bWithinPrefabOrArchetype = TRUE;
		}
	...
	return !bWithinPrefabOrArchetype && Object->HasAnyFlags(RF_ArchetypeObject);
}
	...
	FLOAT Zoom,DWORD& OutWidth,DWORD& OutHeight)
{
	...
	///@fix; by deif 2006-2-13, TrueTypeFont는 Texture개수가 0일 수 있음.
	if (Font != NULL && Font->Textures.Num() && Font->Textures(0) != NULL)	
	{
...
void UUISceneThumbnailRenderer::Draw(UObject* Object,EThumbnailPrimType PrimType,
	...
	FCanvas* Canvas,UBOOL bShowBackground)
{
	...
	if(Scene != NULL)
	{
		...
		{
			Super::Draw(Object, PrimType, X, Y, Width, Height, Viewport, Canvas, bShowBackground);
		}
	}
}
...
void ULensFlareThumbnailRenderer::Draw(UObject* Object,EThumbnailPrimType,INT X,INT Y,
									   DWORD Width,DWORD Height,FViewport*,FCanvas* Canvas, UBOOL bShowBackground)
{
	if (GUnrealEd->GetThumbnailManager())
	{
		ULensFlare* LensFlare = Cast<ULensFlare>(Object);
