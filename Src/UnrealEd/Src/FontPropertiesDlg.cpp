...
BEGIN_EVENT_TABLE(WxFontPropertiesDlg, wxDialog)
...
WxFontPropertiesDlg::WxFontPropertiesDlg() :
	wxDialog(NULL,-1,wxString(TEXT("FontProperties"))),
	Viewport(NULL),
	PropertyWindow(NULL),
	ViewportHolder(NULL),
	bIsScrolling(FALSE),
...
//	ItemButton10->Enable(FALSE);
    ...
    wxStaticBox* ItemStaticBoxSizer11Static = new wxStaticBox(this, wxID_ANY, TEXT("FontProperties"));
    wxStaticBoxSizer* ItemStaticBoxSizer11 = new wxStaticBoxSizer(ItemStaticBoxSizer11Static, wxHORIZONTAL);
    ItemBoxSizer3->Add(ItemStaticBoxSizer11, 1, wxGROW|wxALL, 5);

	// Add our property window to the right panel
	PropertyWindow = new WxPropertyWindow;
	PropertyWindow->Create( this, this );
    ItemStaticBoxSizer11->Add(PropertyWindow, 1, wxGROW|wxALL, 5);
...
bool WxFontPropertiesDlg::Show(bool InShow,UFont* InFont)
{
	...
	PropertyWindow->SetObject(Font,1,1,0);
	// Now show the dialog
...
UBOOL WxFontPropertiesDlg::ImportPage(INT PageNum,const TCHAR* FileName)
{
	...
	if (appLoadFileToArray(Data,FileName))
	{
		...
		UTexture2D* NewPage = (UTexture2D*)Factory->FactoryCreateBinary(
			UTexture2D::StaticClass(),Font,NAME_None,0,NULL,TEXT("TGA"),
		...
		if (NewPage != NULL)
		{
				...
				Texture->SizeX == NewPage->SizeX)
			{
				...
				NewPage->CompressionNoMipmaps = Texture->CompressionNoMipmaps;
...
void WxFontPropertiesDlg::OnUpdateAllFontPages(wxCommandEvent& In)
{
	...
	wxDirDialog ChooseDirDialog(this, 
		*FString::Printf( *LocalizeUnrealEd("Save_F"),*Font->GetName()),
		*LastPath);
...
{
	WxFontPreviewDlg *Dlg = new WxFontPreviewDlg(this,Font,FColor(255,255,255),FColor(0,0,0));
	Dlg->Show(1);
}
...
void WxFontPropertiesDlg::Draw(FViewport* Viewport,FCanvas* Canvas)
{
			...
			if (RenderInfo != NULL && RenderInfo->Renderer != NULL)
			{
				...
				if (YPos + Height + PadSize >= 0 && YPos <= (INT)Viewport->GetSizeY())
				{
					...
					else
					{
						...
						// Draw the font texture for this page
						RenderInfo->Renderer->Draw(Texture,TPT_Plane,XPos,YPos,
							Width,Height,Viewport,Canvas,FALSE);
					}
				}
	...
	if (Font->Textures.Num() > 0)
		ScrollSpeed = TotalHeight / (Font->Textures.Num() * 2);
	else
		ScrollSpeed = TotalHeight;
}
/**
...
/**
 * Handles notification of an object being destroyed
 *
 * @param Src the object that was destroyed
 */
void WxFontPropertiesDlg::NotifyDestroy( void* Src )
{
	// See if our window was destroyed
...
void WxFontPropertiesDlg::UpdateSelectedPage(UObject* SelectedObject)
{
	...
	for (INT Index = 0;
		Index < Font->Textures.Num() && CurrentSelectedPage == -1; Index++)
	{
...
BEGIN_EVENT_TABLE(WxFontPropertiesDlg::WxFontPreviewDlg, wxDialog)
	// Button events
	EVT_BUTTON( ID_CLOSE, WxFontPropertiesDlg::WxFontPreviewDlg::OnClose )
...
WxFontPropertiesDlg::WxFontPreviewDlg::WxFontPreviewDlg(wxWindow* Parent,
	...
	wxDialog(Parent,-1,wxString(TEXT("FontPreview"))),
	Font(InFont),
...
void WxFontPropertiesDlg::WxFontPreviewDlg::CreateControls(void)
{
    ...
    wxStaticBox* ItemStaticBoxSizer3Static = new wxStaticBox(this, wxID_ANY, TEXT("Preview"));
    ...
    PreviewTextControl = new wxTextCtrl( this, ID_PREVIEW_EDIT,
		...
		TEXT("DJs flock by when MTV ax quiz prog"),				// shorter than above
    ...
    wxButton* ItemButton8 = new wxButton( this, ID_FOREGROUND, TEXT("Foreground"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	wxButton* ItemButton9 = new wxButton( this, ID_BACKGROUND, TEXT("Background"), wxDefaultPosition, wxDefaultSize, 0 );
    ...
    wxButton* ItemButton11 = new wxButton( this, ID_CLOSE, TEXT("Close"), wxDefaultPosition, wxDefaultSize, 0 );
 ...
 * Ends the dialog
 */
void WxFontPropertiesDlg::WxFontPreviewDlg::OnClose(wxCommandEvent& In)
{
	EndModal(wxID_OK);
}
	...
	FCanvas* Canvas)
{
	...
	FString PreviewText = PreviewTextControl->GetValue().c_str();
	FString TestText = PreviewText;
	const static FString EscapeChar("#");
	INT FindIndex = INDEX_NONE;
	while( (FindIndex = TestText.InStr(EscapeChar, TRUE)) != INDEX_NONE )
	{
		if( (FindIndex + 1) < TestText.Len() && appIsDigit(TestText[FindIndex + 1]) )
		{
			INT StartIndex = FindIndex + 1;
			INT EndIndex = FindIndex + 2;
			while( EndIndex < TestText.Len() && appIsDigit(TestText[EndIndex]) )
				EndIndex++;

			TCHAR CharToReplace = appAtoi( *PreviewText.Mid(StartIndex, EndIndex - StartIndex) );
			PreviewText = PreviewText.Left(FindIndex) + CharToReplace + PreviewText.Mid(EndIndex);
		}

		TestText = PreviewText.Left(FindIndex);
	}

	DrawString(Canvas,PadSize,PadSize, *PreviewText,
