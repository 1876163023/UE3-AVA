...
void FLensFlarePreviewViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	if (Viewport && bCaptureScreenShot)
	{
		...
		if (Viewport->ReadPixels(OrigBitmap))
		{
			...
			LensFlareEditor->LensFlare->ThumbnailImage = FImageUtils::ConstructTexture2D( 
				ScaledWidth, ScaledHeight, ScaledBitmap, LensFlareEditor->LensFlare, TEXT("ThumbnailTexture"), ObjectFlags );
