	...
	while ( ParseLine(&Buffer,CurrentLine) )
	{
		...
		else if ( GetBEGIN(&Str,TEXT("WIDGET")) )
		{
			...
			if ( ParseObject<UClass>(Str, TEXT("CLASS="), WidgetClass, ANY_PACKAGE) )
			{
				...
				UUIObject* NewWidget = ConstructObject<UUIObject>( WidgetClass, WidgetParent, WidgetName, Flags, WidgetArchetype );
				if ( NewWidget != NULL )
				{
		...
		else if ( GetBEGIN(&Str,TEXT("SEQUENCE")) )
		{
			...
			if ( ParseObject<UClass>(Str, TEXT("CLASS="), SeqObjClass, ANY_PACKAGE) )
			{
				...
				{
					UUISequence* ExistingSequence = FindObject<UUISequence>(CurrentImportObject, *SeqObjName.ToString());
					check(ExistingSequence==NULL);
				}
				...
				{
					NewSequence = MainSequenceContainer->CreateEventContainer(SeqObjName);
				}
...
void UUISceneExporter::ExportWidgetProperties( UUIObject* WidgetToExport, FOutputDevice& Ar, DWORD PortFlags )
{
	...
	// the value for EventProvider will not be exported, since none of its properties are editable.
	// This is a special component for widgets, and we want to always export the value of this property
	Ar.Logf(TEXT("%sEventProvider=%s'%s'%s"),
		appSpc(TextIndent),
		*WidgetToExport->EventProvider->GetClass()->GetName(),
		*WidgetToExport->EventProvider->GetName(),
		LINE_TERMINATOR
		);
...
void UUISceneExporter::BuildComponentMap( UObject* ComponentOwner, TArray<UComponent*>& out_Components )
{
	...
	if ( ComponentOwner->GetClass()->HasAnyClassFlags(CLASS_HasComponents) )
	{
		...
		for ( INT ComponentIndex = 0; ComponentIndex < ComponentReferences.Num(); ComponentIndex++ )
		{
			...
			checkSlow(Component->TemplateName != NAME_None);
