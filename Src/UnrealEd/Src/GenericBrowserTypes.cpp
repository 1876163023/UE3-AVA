...
IMPLEMENT_CLASS(UGenericBrowserType_DSPPreset)
IMPLEMENT_CLASS(UGenericBrowserType_ReverbProperty)
IMPLEMENT_CLASS(UGenericBrowserType_UIAnimation)
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UMaterialInstanceConstant::StaticClass(), FColor(0,128,0), new WxMBGenericBrowserContext_MaterialInstance, 0, this ) );
}

UBOOL UGenericBrowserType_MaterialInstance::ShowObjectEditor( UObject* InObject )
{
	// Show the material instance editor.
	UMaterialInstance* MaterialInstance = Cast<UMaterialInstance>(InObject);

	if(MaterialInstance)
	{
		wxFrame* MaterialInstanceEditor = new WxMaterialInstanceEditor( (wxWindow*)GApp->EditorFrame,-1, MaterialInstance );
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( USequence::StaticClass(), FColor(255,255,255), NULL, 0, this ) );
}
...
void AddSupportInfoItem_Sound( TArrayNoInit<struct FGenericBrowserTypeInfo>& SupportInfo, UClass* Class, FColor color )
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( Class, color, new WxMBGenericBrowserContext_Sound ) );
}
...
void UGenericBrowserType_Sounds::Stop( void )
{
	...
	if( AudioComponent )
	{
		...
		AudioComponent->WaveInstances.Empty();
		AudioComponent->SoundNodeData.Empty();
		AudioComponent->SoundNodeWaveMap.Empty();
...
UBOOL UGenericBrowserType_Font::ShowObjectEditor(UObject* InObject)
{
	...
	if (Font != NULL)
	{
		if (Cast<UTrueTypeFont>(InObject))
...
UBOOL UGenericBrowserType_UIArchetype::ShowObjectEditor( UObject* InObject )
{
	...
	if ( ArchetypeWrapper != NULL )
	{
		...
		if ( WrapperScene == NULL )
		{
			...
			WrapperScene = ConstructObject<UUIScene>(UUIScene::StaticClass(), ArchetypeWrapper->GetOutermost(), SceneTitle, RF_Transient|RF_Transactional);
		...
		//@fixme? - add method to UISceneManager - OpenArchetype, for opening an editor window that doesn't have a UIScene as the OwnerContainer
...
void UGenericBrowserType_DSPPreset::Init()
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UDSPPreset::StaticClass(), FColor(128,192,255), NULL ) );
}

UBOOL UGenericBrowserType_DSPPreset::ShowObjectEditor( UObject* InObject )
{
	return Super::ShowObjectEditor( InObject );
}

void UGenericBrowserType_DSPPreset::DoubleClick( UObject* InObject )
{
	UAudioDevice* AudioDevice = GEngine->Client->GetAudioDevice();
	if( AudioDevice )
	{		
		UDSPPreset* Preset = CastChecked<UDSPPreset>(InObject);
...
void UGenericBrowserType_ReverbProperty::Init()
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UReverbProperty::StaticClass(), FColor(128,192,255), NULL ) );
}

UBOOL UGenericBrowserType_ReverbProperty::ShowObjectEditor( UObject* InObject )
{
	return Super::ShowObjectEditor( InObject );
}

void UGenericBrowserType_ReverbProperty::DoubleClick( UObject* InObject )
{
	UAudioDevice* AudioDevice = GEngine->Client->GetAudioDevice();
	if( AudioDevice )
	{
		...
		ReverbSettings.FadeTime = 0;
		ReverbSettings.ReverbType = REVERB_Off;
		ReverbSettings.UserdefinedReverbPreset = CastChecked<UReverbProperty>(InObject);
...
void UGenericBrowserType_UIAnimation::Init()
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UUIAnimationSeq::StaticClass(), FColor(128,192,255), NULL) );
}

UBOOL UGenericBrowserType_UIAnimation::ShowObjectEditor( UObject* InObject )
{
	UUIAnimationSeq *UIAnimSeq = Cast<UUIAnimationSeq>(InObject);

	if ( UIAnimSeq )
	{
		UUISceneManager* SceneManager = GetUISceneManager();
		check(SceneManager);

		UUIScene* TempScene = ConstructObject<UUIScene>(UUIScene::StaticClass(), UObject::GetTransientPackage(), NAME_None, RF_Transient);
		check(SceneManager->OpenScene( TempScene ));

		UUIObject* Widget = TempScene->CreateWidget(TempScene, UUILabelButton::StaticClass(), NULL, TEXT("LabelForUIAnimSeq"));
		TempScene->InsertChild( Widget );

		INT SceneClientIndex = SceneManager->FindSceneIndex(TempScene);
		UEditorUISceneClient* SceneClient = Cast<UEditorUISceneClient>( SceneManager->SceneClients(SceneClientIndex) );
		check( SceneClient && SceneClient->SceneWindow );
		WxUIEditorBase* UIEditor = SceneClient->SceneWindow;

		// @TODO : place widget to a desirable position.

		UInterpCurveEdSetup* CurveEdSetup = ConstructObject<UInterpCurveEdSetup>(UInterpCurveEdSetup::StaticClass(),INVALID_OBJECT, NAME_None, RF_Transient | RF_NotForClient | RF_NotForServer );
		WxUIAnimCurveEd* CurveEd = new WxUIAnimCurveEd( UIEditor , -1, CurveEdSetup, UIAnimSeq);
		CurveEd->SetNotifyObject(CurveEd);
		UIEditor->AddDockingWindow( CurveEd, FDockingParent::DH_Bottom, TEXT("UIAnimSeq"), TEXT("CurveEditor") );
