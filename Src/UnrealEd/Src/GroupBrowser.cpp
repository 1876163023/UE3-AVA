...
/**
 * Iterates over the specified wxCheckListBox object and extracts selected entries to the output string list.
 */
static void GetSelectedGroupNames(const wxCheckListBox* GroupList, TArray<FString>& OutSelectedGroupNames)
{
	...
	{
		OutSelectedGroupNames.AddItem( GroupList->GetString( Selections[SelectionIndex] ).c_str() );
	}
}
...
void WxGroupBrowser::Update()
{
	...
	if ( IsShown() == TRUE && !GIsPlayInEditorWorld && GWorld )
	{
		...
		{
			GroupList->Append( *UniqueGroups(GroupIndex) );
			// Set the group to "off".
			const UBOOL bVisible = GroupArray.ContainsItem( *UniqueGroups(GroupIndex) );
...
void WxGroupBrowser::PopulateActorList()
{
	...
	GetSelectedGroupNames( GroupList, SelectedGroupNames );

	ActorList->Freeze();
	ActorList->ClearAll();
	ActorList->InsertColumn( 0, *LocalizeUnrealEd("Actors") );
	if ( SelectedGroupNames.Num() > 0 )
	{
		...
		for( FActorIterator It ; It ; ++It )
		{
			...
			if( IsValid( Actor ) )
			{
				for ( INT GroupIndex = 0 ; GroupIndex < SelectedGroupNames.Num() ; ++GroupIndex )
				{
					if ( Actor->IsInGroup( *SelectedGroupNames(GroupIndex) ) )
					{
						ActorNames.AddItem( Actor->GetPathName() );
						break;
					}
				}
			}
		}
		Sort<USE_COMPARE_CONSTREF(FString,GroupBrowser)>( ActorNames.GetTypedData(), ActorNames.Num() );
	}
	ActorList->Thaw();
}
