	...
	for( INT i = 0 ; i < InModel->Surfs.Num() ; i++ )
	{
		...
		if( Surf->PolyFlags & PF_Selected )
		{
			...
			switch( InTexAlignType )
			{
				...
				{
					for( INT x = 0 ; x < EdPoly.Vertices.Num() ; x++ )
						PolyBBox += (EdPoly.Vertices(x) + Surf->Actor->Location);
				}
				break;
			}
		}
...
void UTexAlignerFace::AlignSurf( ETexAlign InTexAlignType, UModel* InModel, FBspSurfIdx* InSurfIdx, FPoly* InPoly, FVector* InNormal )
{
	...
	{
		bboxWorld += InSurfIdx->Surf->Actor->LocalToWorld().TransformFVector( InPoly->Vertices(x) );

		FVector vtx = FRotationMatrix( DiffRot ).TransformFVector( InPoly->Vertices(x) );
		bboxPlane += vtx;

		new(NewVerts) FVector(vtx);
	}

	// Find the lowest vertex (the one with the smallest values).  This will be tex coord 0,1

	TArray<FVector> SortedVtx = NewVerts;
	appQsort( &SortedVtx(0), SortedVtx.Num(), sizeof(FVector), (QSORT_COMPARE)CompareVerts );

	INT SortedVtxIdx = -1;
	for( INT x = 0 ; x < InPoly->Vertices.Num() ; ++x )
		if( InPoly->Vertices(x) == SortedVtx(0) )
		{
			SortedVtxIdx = x;
			break;
		}
...
void FTexAlignTools::Init()
{
	...
	Aligners.AddItem( CastChecked<UTexAligner>(UObject::StaticConstructObject(UTexAlignerFace::StaticClass(),UObject::GetTransientPackage(),NAME_None,RF_Public|RF_Standalone) ) );
