...
void WxAnimSetViewer::OnAuxSkelMeshComboChanged(wxCommandEvent& In)
{
	...
	else if(In.GetId() == IDM_ANIMSET_SKELMESHAUX4COMBO)
	{
		AuxComp = PreviewSkelCompAux4;

		if(SkelMeshAux4Combo->GetSelection() != -1)
		{
			NewMesh = (USkeletalMesh*)SkelMeshAux4Combo->GetClientData( SkelMeshAux4Combo->GetSelection() );
		}
	}
	else if(In.GetId() == IDM_ANIMSET_SKELMESHAUX5COMBO)
	{
		AuxComp = PreviewSkelCompAux5;

		if(SkelMeshAux5Combo->GetSelection() != -1)
		{
			NewMesh = (USkeletalMesh*)SkelMeshAux5Combo->GetClientData( SkelMeshAux5Combo->GetSelection() );
		}
	}
	else if(In.GetId() == IDM_ANIMSET_SKELMESHAUX6COMBO)
	{
		AuxComp = PreviewSkelCompAux6;

		if(SkelMeshAux6Combo->GetSelection() != -1)
		{
			NewMesh = (USkeletalMesh*)SkelMeshAux6Combo->GetClientData( SkelMeshAux6Combo->GetSelection() );
		}
	}
...
void WxAnimSetViewer::OnAuxSkelMeshUse( wxCommandEvent& In )
{
	...
	if(SelectedSkelMesh)
	{
		...
		else if(In.GetId() == IDM_ANIMSET_SKELMESH_AUX4USE)
		{
			AuxComp = PreviewSkelCompAux4;
			AuxCombo = SkelMeshAux4Combo;
		}
		else if(In.GetId() == IDM_ANIMSET_SKELMESH_AUX5USE)
		{
			AuxComp = PreviewSkelCompAux5;
			AuxCombo = SkelMeshAux5Combo;
		}
		else if(In.GetId() == IDM_ANIMSET_SKELMESH_AUX6USE)
		{
			AuxComp = PreviewSkelCompAux6;
			AuxCombo = SkelMeshAux6Combo;
		}
...
void WxAnimSetViewer::OnMergeMaterials( wxCommandEvent& In )
{
	...
	TArray<UMaterialInstance*>	NewMaterialArray;
	...
	{
		UMaterialInstance* Inst = SelectedSkelMesh->Materials(i);
