...
void UUnrealEdEngine::SetPivot( FVector NewPivot, UBOOL SnapPivotToGrid, UBOOL DoMoveActors, UBOOL bIgnoreAxis )
{
	...
	if( SnapPivotToGrid )
	{
		...
		Constraints.Snap( GEditorModeTools().SnappedLocation, GEditorModeTools().GridBase, DummyRotator );
	...
	if( Count==1 )
	{
		if( SingleActor->IsA( ABrush::StaticClass() ) && !SingleActor->IsA( AVolume::StaticClass() ) )
		{
			SingleActor->PreEditChange(NULL);
			SingleActor->Modify();

			// Set the new pre pivot.
			const FVector NewPrePivot( SingleActor->PrePivot + (GEditorModeTools().SnappedLocation - SingleActor->Location) );
			SingleActor->PrePivot = NewPrePivot;
			SingleActor->MarkComponentsAsDirty();
			SingleActor->bNetDirty = TRUE;

			// Move the actor.
			FCheckResult Hit;
			GWorld->MoveActor(SingleActor, GEditorModeTools().SnappedLocation - SingleActor->Location, SingleActor->Rotation, 0, Hit);
