...
WxMaterialEditorBase::WxMaterialEditorBase(wxWindow* InParent, wxWindowID InID, UMaterialInstance* InMaterialInstance)
	...
	,	bShowGrid( TRUE )
{
	...
	SetPreviewMaterial( InMaterialInstance );
 ...
 *
 */
void WxMaterialEditorBase::SetPreviewMaterial(UMaterialInstance* InMaterialInstance)
{
	...
	PreviewMeshComponent->Materials.AddItem( InMaterialInstance );
	PreviewSkeletalMeshComponent->Materials.Empty();
	PreviewSkeletalMeshComponent->Materials.AddItem( InMaterialInstance );
}
...
void WxMaterialEditorBase::OnSetPreviewMeshFromSelection(wxCommandEvent& In)
{
	...
	if ( SelectedStaticMesh )
	{
		...
		MaterialInstance->PreviewMesh = SelectedStaticMesh->GetPathName();
	...
	else
	{
		...
		if ( SelectedSkeletalMesh )
		{
			...
			MaterialInstance->PreviewMesh = SelectedSkeletalMesh->GetPathName();
			bFoundPreviewMesh = TRUE;
		}
	}
