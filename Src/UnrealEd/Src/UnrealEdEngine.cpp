...
{
	if( GApp && GApp->EditorFrame->ViewportConfigData )
	{
...
UBOOL UUnrealEdEngine::SaveDirtyPackages(UBOOL bShouldSaveMap)
{
	...
	do
	{
		...
		for ( TObjectIterator<UPackage> It ; It ; ++It )
		{
			...
			{
				const INT YesNoCancelReply = appMsgf( AMT_YesNoCancel, *LocalizeUnrealEd("SaveDirtyPackagesNotFullyLoadedQ"), *Package->GetName() );
		...
		for ( TObjectIterator<UPackage> It ; It ; ++It )
		{
			...
			if( !bShouldIgnorePackage )
			{
				...
				do
				{
					...
					// Find out the user wants to do with this dirty package.  If the user already
					// specified that the package should be fully loaded, just go ahead and save it.
					const INT YesNoCancelReply = RootPackagesThatWereFullyLoaded.ContainsItem( Package ) ?
						0 : // Yes
						appMsgf( AMT_YesNoCancel, *LocalizeUnrealEd("Prompt_17"), *Package->GetName(), *Filename, *Filename );
					// act on it
					switch (YesNoCancelReply)
					{
						case 0: // Yes
						...
						case 2: // Cancel
							// if the user hit cancel, then he wants to stop and return to the editor
							GFileManager->SetDefaultDirectory();
							return FALSE;
					}
