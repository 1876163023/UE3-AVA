 ...
 */
void UUnrealEdEngine::edactCopySelected(UBOOL bReselectPrefabActors)
{
 ...
 */
void UUnrealEdEngine::edactPasteSelected(UBOOL bDuplicate, UBOOL bOffsetLocations)
{
	const FScopedBusyCursor BusyCursor;
...
public:
	...
	void DuplicateActorsToLevel(TArray<AActor*>& OutNewActors, TArray<APrefabInstance*>& OutNewPrefabInstances, ULevel* DestLevel, UBOOL bOffsetLocations)
	{
		...
		GEditor->edactCopySelected( TRUE );

		// Paste to the dest level.
		GWorld->CurrentLevel = DestLevel;
		GEditor->edactPasteSelected( TRUE, bOffsetLocations );
 ...
 */
UBOOL UUnrealEdEngine::edactDeleteSelected()
{
 ...
 */
UBOOL UUnrealEdEngine::ShouldAbortActorDeletion() const
{
	UBOOL bResult = FALSE;
	...
	for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		{
			// Warn the user that the actors to be deleted are referenced by Kismet, and prompt for continue.
			const UBOOL bDoDelete = appMsgf( AMT_YesNo, *LocalizeUnrealEd("Prompt_16") );
			bResult = !bDoDelete;
			break;
		}
	}
...
void UUnrealEdEngine::edactReplaceSelectedBrush()
{
	...
	for ( INT BrushIndex = 0 ; BrushIndex < BrushesToReplace.Num() ; ++BrushIndex )
	{
		...
		ABrush* NewBrush = csgAddOperation( DefaultBrush, SrcBrush->PolyFlags, (ECsgOper)SrcBrush->CsgOper );
		if( NewBrush )
		{
