 ...
 * Refreshes the page.
 */
void WxRemoteControlActorsPage::RefreshPage()
{
	if (bBuildTreeFlag)
	{
		BuildActorTree();
	...
	while(actorsAdded < Actors.Num())
	{
		for(INT i = 0; i < Actors.Num(); ++i)
		{
			...
			if(NULL == ActorItemMap.Find(ad.ActorName))
			{
				...
				wxTreeItemId item = TreeCtrl->AppendItem(parentItemId, *ad.ActorName, TREE_ICON_PAWN, TREE_ICON_PAWN);
				actorsAdded++;
...
void WxRemoteControlActorsPage::OnEditActorTrace(wxCommandEvent &In)
{
	...
	if ( HitActor )
	{
		...
		GetGame()->ShowEditActor( *HitActor->GetName() );
	}
}
 ...
 */
INT WxRemoteControlActorsPage::BuildSubObjectTreeHelper(const wxTreeItemId& ParentID,
													  ...
													  UBOOL IsRoot)
{
	...
	if (IsRoot || (!PreviouslySeenID) || (PreviouslySeenID->ObjectID != invalidId))
	{
		...
		if (!IsRoot)
		{
			...
			{
				// Determine if the current occurance of this object is closer to an actor than the last occurance
...
void WxRemoteControlActorsPage::DeleteBranch(wxTreeItemId Node, StringSubObjectItemMap& ObjectMap)
{
	...
	while( ItemStack.Num() > 0 )
	{
		...
		if(!child.IsOk())
		{
			...
			check(NodeData);
			ObjectMap.Remove(NodeData->GetActorName());
	...
	{
		ObjectMap.Remove(CurrentData->GetActorName());
