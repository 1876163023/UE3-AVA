...
public:
	...
	UBOOL Show( UBrushBuilder* InBrushBuilder, UBOOL bInShow = TRUE )
	{
		...
		SetTitle( *FString::Printf( *LocalizeUnrealEd("BrushBuilderCaption"), *BrushBuilder->ToolTip ) );
...
void WxButtonGroup::OnModeCoverEdit( wxCommandEvent& In )		{	GUnrealEd->Exec(TEXT("MODE COVEREDIT"));	}
...
UBOOL WxButtonBar::Create( wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const FString& name )
{
	...
	{
		bg->AddButtonChecked( IDM_COVEREDIT, TEXT("Btn_CoverMode"), *LocalizeUnrealEd("CoverEditMode") );
	}
