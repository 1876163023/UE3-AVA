...
void WxInterpEd::SetActiveTrack(UInterpGroup* InGroup, INT InTrackIndex)
{
	...
	UUIObject* Object = GrInst->GroupObject;
	if (UIEditor && Object && !Object->IsSelected())
	{
		bIgnoreActorSelection = true;

		TArray<UUIObject*> Objects;
		UIEditor->SetSelectedWidgets( Objects );		
...
void WxInterpEd::SetSnapEnabled(UBOOL bInSnapEnabled)
{
	...
	if(bSnapToKeys)
	{
		CurveEd->SetInSnap(false, SnapAmount);
	}
	else
	{
...
void WxInterpEd::AddTrackToSelectedGroup(UClass* TrackClass, UInterpTrack* TrackToCopy)
{
	...
	// Warn when creating movement track on a static actor, warn and offer to bail out.
	if(TrackClass->IsChildOf(UInterpTrackMove::StaticClass()))
	{
...
void WxInterpEd::DuplicateGroup(UInterpGroup* GroupToDuplicate)
{
	...
	InterpEdTrans->BeginSpecial( TEXT("NewGroup") );
	{
		...
		NewGroupInst->InitGroupInst(NewGroup, NULL, NULL);
...
void WxInterpEd::AddTrackToCurveEd(class UInterpGroup* InGroup, INT InTrackIndex)
{
	...
	else
	{
		...
		IData->CurveEdSetup->AddCurveToCurrentTab(InterpTrack, CurveName, CurveColor);
	}
...
void WxInterpEd::ActorModified()
{
	...
	if(bAdjustingKeyframe)
	{
	    ...
	    if( !GrInst->GroupActor && !GrInst->GroupObject )
		    return;
		...
		else
		{
			...
			{
				if (SelectedWidgets(i) == GrInst->GroupObject)
				{
					bTrackActorModified = TRUE;
					break;
				}
...
void WxInterpEd::ActorSelectionChange()
{
	...
	if (UIEditor == NULL)
	{
		...
		UInterpGroupInst* GrInst = Interp->FindGroupInst(SingleActor,NULL);
		if(GrInst)
		{
			check(GrInst->Group);
	...
	else 
	{
		...
		if (UIEditor)
		{
			...
			{
				SingleObject = SelectedWidgets(0);
			}		
		}		
		...
		UInterpGroupInst* GrInst = Interp->FindGroupInst(NULL,SingleObject);
		if(GrInst)
		{
			check(GrInst->Group);
			SetActiveTrack(GrInst->Group, INDEX_NONE);
	...
	if(bDuplicatingTrack == FALSE)
	{
		...
		if ( Actor != NULL || Object != NULL )
		{
			...
			if (Actor)
				...
				Object->GetInterpFloatPropertyNames(PropNames);
			...
			if( dlg.ShowModal( TEXT("ChooseProperty"), TEXT("PropertyName"), PropStrings, 0, TRUE ) == wxID_OK )
			{
				...
				if ( TrackAddPropName != NAME_None )
				{
					...
					for(INT i=0; i<InterpEd->ActiveGroup->InterpTracks.Num(); i++)
					{
						...
						UInterpTrackRotatorProp* TestRotatorTrack = Cast<UInterpTrackRotatorProp>( InterpEd->ActiveGroup->InterpTracks(i) );
						if(TestRotatorTrack && TestRotatorTrack->PropertyName == TrackAddPropName)
						{
							appMsgf(AMT_OK, *LocalizeUnrealEd("Error_RotatorPropertyAlreadyControlled"));
							return FALSE;
						}
					}
	...
	if(bDuplicatingTrack == FALSE)
	{
		...
		if ( Actor != NULL || Object != NULL )
		{
			...
			if (Actor)
				...
				Object->GetInterpVectorPropertyNames(PropNames);
...
* @return	Returns true if this track can be created and false if some criteria is not met (i.e. A named property is already controlled for this group).
*/
UBOOL UInterpTrackRotatorPropHelper::PreCreateTrack( const UInterpTrack *TrackDef, UBOOL bDuplicatingTrack ) const
{
	if( bDuplicatingTrack == FALSE )
	{
		// For Property tracks - pop up a dialog to choose property name.
		...
		if ( Actor != NULL || Object != NULL )
		{
			TArray<FName> PropNames;
			if (Actor)
				GetInterpRotatorPropertyNames(Actor->GetClass(),PropNames);
			else if (Object)
				GetInterpRotatorPropertyNames(Object->GetClass(),PropNames);
...
* @param	TrackIndex	The index of the Track that as just added.  This is the index returned by InterpTracks.AddItem.
*/
void  UInterpTrackRotatorPropHelper::PostCreateTrack( UInterpTrack *Track, UBOOL bDuplicatingTrack, INT TrackIndex ) const
{
	if( bDuplicatingTrack == FALSE )
	{
		UInterpTrackRotatorProp	*PropTrack = CastChecked<UInterpTrackRotatorProp>(Track);
...
IMPLEMENT_CLASS(UInterpTrackRotatorPropHelper);
