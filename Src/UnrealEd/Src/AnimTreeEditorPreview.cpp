...
static const FLOAT	FloorDragSpeed = 0.25f;
static const FLOAT	FloorTurnSpeed = 0.005f;
...
UBOOL FAnimTreeEdPreviewVC::InputAxis(FViewport* Viewport, INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
	...
	if( bCtrlDown )
	{
		...
		if(AnimTreeEd->bShowFloor)
		{
			...
			if(Viewport->KeyState(KEY_LeftMouseButton))
			{
				...
				{
					// Translate up/down
					NewOrigin += FloorDragSpeed * FVector(0.f, -0.f, DragY);
				}
				else
				{
					...
					NewOrigin += FloorDragSpeed * WorldSpaceDrag;
				}
			...
			{
				const FQuat RotQuat( FVector(0,0,1), FloorTurnSpeed * DragX );
	...
	else if(bManipulating)
	{
		...
		{
			FQuat DeltaWorldQuat( WorldManDir, -DragMag * RotateSpeed );
			FVector DeltaWorldTranslate = WorldManDir * DragMag * TranslateSpeed;
		...
		{
			FVector DeltaLocalTranslate = LocalManDir * DragMag * TranslateSpeed;

			check(AnimTreeEd->SelectedControls.Num() == 1); // Shouldn't draw axes unless this is the case
			USkelControlBase* Control = AnimTreeEd->SelectedControls(0);
	...
	if( AnimTreeEd->bShowBoneNames )
	{
		...
		for(INT i=0; i<AnimTreeEd->PreviewSkelComp->SpaceBases.Num(); i++)
		{
			...
			if(proj.W > 0.f) // This avoids drawing bone names that are behind us.
			{
				...
				const FColor BoneNameColor = FColor(255,255,255);

				//if(bHitTesting) Canvas->SetHitProxy( new HPhATBoneNameProxy(i) );
				DrawString(Canvas,XPos, YPos, *BoneName.ToString(), GEngine->SmallFont, BoneNameColor);
...
void UAnimTreeEdSkelComponent::Render(const FSceneView* View,FPrimitiveDrawInterface* PDI)
{
	...
	if( AnimTreeEd->bShowSkeleton )
	{
		FStaticLODModel& LODModel = SkeletalMesh->LODModels(PredictedLODLevel);

		// Are we displaying bone only below a certain child?
		const INT StartDisplayBoneIndex = (SkeletalMesh->StartDisplayBoneName == NAME_None ? INDEX_NONE : SkeletalMesh->MatchRefBone(SkeletalMesh->StartDisplayBoneName));
		...
		for(INT i=0; i<LODModel.RequiredBones.Num(); i++)
		{
			...
			if( StartDisplayBoneIndex != INDEX_NONE && 
				BoneIndex != StartDisplayBoneIndex &&
				!SkeletalMesh->BoneIsChildOf(BoneIndex, StartDisplayBoneIndex) )
			{
				continue;
			}
			...
			if( BoneIndex == 0 )
			{
				PDI->DrawLine(BonePos, LocalToWorld.GetOrigin(), FColor(255, 0, 255), SDPG_Foreground);
			}
			else
			{
				// Link Parent to Bone
				...
				PDI->DrawLine(ParentPos, BonePos, FColor(255,255,255), SDPG_Foreground);
			}
	...
	for(INT i=0; i<AnimTreeEd->AnimNodeEditInfos.Num() && !AnimTreeEd->PreviewVC->bDrawingInfoWidget; i++)
	{
		...
		if( EditInfo->ShouldDrawWidget() )
		{
			...
			FUnrealEdUtils::DrawWidget(View, PDI, WidgetTM, 0, 0, HighlightAxis, bRotWidget);
	...
	if(	!AnimTreeEd->PreviewVC->bDrawingInfoWidget && 
		AnimTreeEd->SelectedControls.Num() == 1 && 
		AnimTreeEd->SelectedNodes.Num() == 0 )
	{
		// Because a SkelControl could be used by multiple chains, we have to draw the widgets for each use in a chain.

		USkelControlBase* SelectedControl = AnimTreeEd->SelectedControls(0);
				...
				while(Control)
				{
					if(Control == SelectedControl)
					{
						...
						for(INT WidgetNum = 0; WidgetNum < NumWidgets; WidgetNum++)
						{
							...
							const FMatrix WidgetTM = Control->GetWidgetTM(WidgetNum, AnimTreeEd->PreviewSkelComp, BoneIndex);
							...
							// Info2 is bone index
							FUnrealEdUtils::DrawWidget(View, PDI, WidgetTM, WidgetNum, BoneIndex, HighlightAxis, FALSE);
						}
					}
...
UBOOL UAnimTreeEdSkelComponent::LegLineCheck(const FVector& Start, const FVector& End, FVector& HitLocation, FVector& HitNormal)
{
	// Get the AnimTreeEditor itself.
	WxAnimTreeEditor* AnimTreeEd = (WxAnimTreeEditor*)AnimTreeEdPtr;
	check(AnimTreeEd);
	...
	if(AnimTreeEd->bShowFloor)
	{
		...
		const UBOOL bHit = !AnimTreeEd->FloorComp->LineCheck(Hit, End, Start, FVector(0), TRACE_AllBlocking);
		if(bHit)
		{
