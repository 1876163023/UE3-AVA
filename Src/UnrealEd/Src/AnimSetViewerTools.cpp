...
void WxAnimSetViewer::SetSelectedSkelMesh(USkeletalMesh* InSkelMesh, UBOOL bClearMorphTarget)
{
	...
	// Extra Mesh 3개 추가.
	SkelMeshAux4Combo->SetSelection(0);
	PreviewSkelCompAux4->SetSkeletalMesh(NULL);
	PreviewSkelCompAux4->UpdateParentBoneMap();

	SkelMeshAux5Combo->SetSelection(0);
	PreviewSkelCompAux5->SetSkeletalMesh(NULL);
	PreviewSkelCompAux5->UpdateParentBoneMap();

	SkelMeshAux6Combo->SetSelection(0);
	PreviewSkelCompAux6->SetSkeletalMesh(NULL);
	PreviewSkelCompAux6->UpdateParentBoneMap();
 ...
 */
void WxAnimSetViewer::ImportMorphTarget()
{
	if( !SelectedMorphSet )
	{
				...
				else
				{
					...
					else
					{
						...
						if ( ColladaMeshes.Num() > 0 )
						{
							...
							for( INT MeshIndex = 0; MeshIndex < 3 && MeshIndex < ColladaMeshes.Num(); ++MeshIndex )
							{
...
void WxAnimSetViewer::UpdateSkelComponents()
{
	...
	// Extra Mesh 3개 추가.
	FComponentReattachContext ReattachContext5(PreviewSkelCompAux4);
	FComponentReattachContext ReattachContext6(PreviewSkelCompAux5);
	FComponentReattachContext ReattachContext7(PreviewSkelCompAux6);
}
