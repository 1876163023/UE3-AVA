...
void WxInterpEd::OnContextNewGroup( wxCommandEvent& In )
{
	...
	if(!bDirGroup && !bDuplicateGroup)
	{
		...
		if (UIEditor)
		{
			...
			{
				GroupObject = SelectedWidgets(0);
			}

			if (GroupObject)
			{
				/*UObject* SequenceOutermost = Interp->GetOutermost();
				UObject* ActorOutermost = GroupObject->GetOutermost();
				if(ActorOutermost != SequenceOutermost)
				{
					appMsgf(AMT_OK, *LocalizeUnrealEd("Error_ActorNotInSequenceLevel"));
					return;
				...
				for(INT i=0; i<Interp->GroupInst.Num(); i++)
				{
					if( GroupObject == Interp->GroupInst(i)->GroupObject )
					{
						appMsgf(AMT_OK, *LocalizeUnrealEd("Error_GroupAlreadyAssocWithActor"));
						return;
					}
	...
	else
	{
		...
		NewGroupInst->InitGroupInst(NewGroup, GroupActor, GroupObject);
	}
	...
	else
	{
		...
		if(GroupActor)
		{
			...
			NewObjVar->ObjPosY = Interp->ObjPosY + Interp->DrawHeight;
		...
		else if (GroupObject)
		{
			...
			NewObjVar->ObjPosX = Interp->ObjPosX + 50 * Interp->VariableLinks.Num();
			NewObjVar->ObjPosY = Interp->ObjPosY + Interp->DrawHeight;
			NewObjVar->ObjValue = GroupObject;
			NewObjVar->OnCreated();
