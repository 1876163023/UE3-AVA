...
void WxCascade::OnNewEmitter(wxCommandEvent& In)
{
	...
	if (NewEmitClass == UParticleSpriteEmitter::StaticClass())
	{
		...
		if (NewEmitClass == UParticleSpriteEmitter::StaticClass())
		{
			...
			UParticleSpriteEmitter* NewSpriteEmitter = (UParticleSpriteEmitter*)NewEmitter;
			UMaterialInstance* CurrentMaterial = GEditor->GetSelectedObjects()->GetTop<UMaterialInstance>();
			if (CurrentMaterial)
			{
			...
			{
				NewSpriteEmitter->Material = LoadObject<UMaterialInstance>(NULL, TEXT("EngineMaterials.DefaultParticle"), NULL, LOAD_None, NULL);
			}
		}
	...
	if (SelectedModule || SelectedEmitter)
	{
		...
		if (SelectedModule)
		{
			if (SelectedModule->IsDisplayedInCurveEd(CurveEd->EdSetup))
			{
				...
				{
					CurveEd->EdSetup->ChangeCurveColor(Curves(i), SelectedModule->ModuleEditorColor);
				}
			}
		}
...
void WxCascade::InsertModule(UParticleModule* Module, UParticleEmitter* TargetEmitter, INT TargetIndex, UBOOL bSetSelected)
{
	...
	if (Module->IsA(UParticleModuleTypeDataBase::StaticClass()))
	{
		...
		//@todo. This is old code... remove it soon.
#if 0
		if (Module->IsA(UParticleModuleTypeDataSubUV::StaticClass()))
		{
			// See if there is already a SubImageIndex module...
			UBOOL bHasSubImageIndex = FALSE;
			INT i;

			for (i = 0; i < LODLevel->Modules.Num(); i++)
			{
				if (LODLevel->Modules(i)->IsA(UParticleModuleSubUV::StaticClass()))
				{
					bHasSubImageIndex = TRUE;
					break;
				}
			}

			if (!bHasSubImageIndex)
			{
				// Find the index in the list...
				for (i = 0; i < ParticleModuleClasses.Num(); i++)
				{
					if (ParticleModuleClasses(i) == UParticleModuleSubUV::StaticClass())
					{
						UParticleEmitter* pkTempEmitter = SelectedEmitter;
						SelectedEmitter = TargetEmitter;
						CreateNewModule(i);
						SelectedEmitter = pkTempEmitter;
						break;
					}
				}
			}
...
void WxCascade::SetSelectedInCurveEditor()
{
	...
	if (SelectedModule)
	{
		...
		for (TFieldIterator<UStructProperty> It(SelectedModule->GetClass()); It; ++It)
		{
