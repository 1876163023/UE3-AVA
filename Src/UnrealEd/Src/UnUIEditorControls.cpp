...
void WxSceneTreeCtrl::OnBeginDrag( wxTreeEvent& Event )
{
	...
	// 현재 WxSceneTreeCtrl의 Message Pumping은 FWindowsViewport로 가는 메세지를 가로막는다.
	// 원래는 메세지를 받을 수 있었으나 12월 간의 구현 변경으로 인해 변경된것으로 보임
	// @TODO : 근본적인 해결책은 아니기 때문에 가능하면 원인을 해결해서 엔진코드를 따라가는것이 좋을것으로 보임
...
void WxUIEditorMainToolBar::CreateControls()
{
	...
	TArray<wxString> ViewportSizeStrings;
	ViewportSizeStrings.AddItem(TEXT("Auto"));
	...
	for( ResMapType::TIterator it(ResMap) ; it ; ++it)
		ViewportSizeStrings.AddItem(*FString::Printf(TEXT("%dx%d"),it.Value().Width,it.Value().Height));

	cmb_ViewportSize = new wxComboBox( this, ID_UITOOL_VIEW_SIZE, TEXT("Auto"), wxDefaultPosition, wxDefaultSize, ViewportSizeStrings.Num(), &ViewportSizeStrings(0), wxCB_DROPDOWN );
