...
void WxDlgUIEvent_MetaObject::OnOK(wxCommandEvent &Event)
{
	...
	if(MetaObject && InputEventEnum)
	{
		...
		// Loop through all items in the list to create a new list of actions.  For each item in the list, we will try to see if a action already exists for that
		...
		if( State != NULL )
		{
			const INT NumActions = State->StateInputActions.Num();
			...
			for ( INT StateInputIndex = State->StateInputActions.Num() - 1; StateInputIndex >= 0; StateInputIndex-- )
			{
				FInputKeyAction& StateInputAction = State->StateInputActions(StateInputIndex);
				if ( !NewActions.ContainsItem(StateInputAction) )
				{
					State->RemoveInputAction(StateInputAction);
				}
			}
			...
			for ( INT NewActionIndex = 0; NewActionIndex < NewActions.Num(); NewActionIndex++ )
			{
				...
				if ( !State->StateInputActions.ContainsItem(NewAction) )
				{
					State->AddInputAction(NewAction);
				}
			}
