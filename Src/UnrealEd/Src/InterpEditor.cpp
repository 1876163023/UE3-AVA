...
UBOOL FInterpEdViewportClient::InputAxis(FViewport* Viewport, INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
	...
	,	UIEditor( NULL )
{
	...
	for(INT i=0; i<Interp->GroupInst.Num(); i++)
	{
		...
		if ( GroupActor )
		{
			...
			if( GrInst->Group->HasMoveTrack() )
			{
				// Warn if any groups are trying to act on bStatic actors!
				if ( GroupActor->bStatic )
				{
					appMsgf( AMT_OK, *LocalizeUnrealEd("GroupOnStaticActor"), *GrInst->Group->GroupName.ToString(), *GroupActor->GetName() );
				}
			}
		}
	}
