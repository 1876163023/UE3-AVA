 ...
 */
void WxPrimitiveStatsBrowser::InsertColumn( EPrimitiveStatsBrowserColumns ColumnId, const TCHAR* ColumnHeader )
{
	ListControl->InsertColumn( ColumnId, ColumnHeader );
...
void WxPrimitiveStatsBrowser::Create(INT DockID,const TCHAR* FriendlyName, wxWindow* Parent)
{
	...
	InsertColumn( PCSBC_Count,			*LocalizeUnrealEd("PrimitiveStatsBrowser_Count")			);
	InsertColumn( PCSBC_Triangles,		*LocalizeUnrealEd("PrimitiveStatsBrowser_Triangles")		);
	InsertColumn( PCSBC_ResourceSize,	*LocalizeUnrealEd("PrimitiveStatsBrowser_ResourceSize")		);
	InsertColumn( PCSBC_LightsLM,		*LocalizeUnrealEd("PrimitiveStatsBrowser_LightsLM")			);
	InsertColumn( PCSBC_LightsOther,	*LocalizeUnrealEd("PrimitiveStatsBrowser_LightsOther")		);
	InsertColumn( PCSBC_LightsTotal,	*LocalizeUnrealEd("PrimitiveStatsBrowser_LightsTotal")		);
	InsertColumn( PCSBC_ObjLightCost,	*LocalizeUnrealEd("PrimitiveStatsBrowser_ObjLightCost")		);
	InsertColumn( PCSBC_TriangleCost,	*LocalizeUnrealEd("PrimitiveStatsBrowser_TriangleCost")		);
	InsertColumn( PCSBC_LightMapData,	*LocalizeUnrealEd("PrimitiveStatsBrowser_LightMapData")		);
	InsertColumn( PCSBC_ShadowMapData,	*LocalizeUnrealEd("PrimitiveStatsBrowser_ShadowMapData")	);
	InsertColumn( PCSBC_LMSMResolution,	*LocalizeUnrealEd("PrimitiveStatsBrowser_LMSMResolution")	);
	InsertColumn( PCSBC_Sections,		*LocalizeUnrealEd("PrimitiveStatsBrowser_Sections")			);
	InsertColumn( PCSBC_RadiusMin,		*LocalizeUnrealEd("PrimitiveStatsBrowser_RadiusMin")		);
	InsertColumn( PCSBC_RadiusMax,		*LocalizeUnrealEd("PrimitiveStatsBrowser_RadiusMax")		);
	InsertColumn( PCSBC_RadiusAvg,		*LocalizeUnrealEd("PrimitiveStatsBrowser_RadiusAvg")		);
}
...
struct FPrimitiveStats
{
	...
	FLOAT GetColumnData( UINT Index ) const
	{
		...
		case PCSBC_ObjLightCost:
			...
			break;
		case PCSBC_TriangleCost:
			return LightsOther * Triangles;
			break;
	...
	if( IsShown() == TRUE && !GIsPlayInEditorWorld && GWorld && GWorld->CurrentLevel )
	{
		...
		ListControl->Freeze();
		{
			...
			for( TMap<UObject*,FPrimitiveStats>::TIterator It(ResourceToStatsMap); It; ++It )
			{
				...
				CombinedTriangleCost		+= StatsEntry.GetColumnData( PCSBC_TriangleCost		);
			...
			for( INT StatsIndex=0; StatsIndex<PrimitiveStats.Num(); StatsIndex++ )
			{
				...
				ListControl->SetItem( ItemIndex, PCSBC_Count,			*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_Count			)) ); // Count
				ListControl->SetItem( ItemIndex, PCSBC_Triangles,		*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_Triangles		)) ); // Triangles
				ListControl->SetItem( ItemIndex, PCSBC_ResourceSize,	*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_ResourceSize	)) ); // ResourceSize
				...
				ListControl->SetItem( ItemIndex, PCSBC_ObjLightCost,	*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_ObjLightCost	)) ); // ObjLightCost
				ListControl->SetItem( ItemIndex, PCSBC_TriangleCost,	*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_TriangleCost	)) ); // TriangleCost
				ListControl->SetItem( ItemIndex, PCSBC_LightMapData,	*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_LightMapData	)) ); // LightMapData
				ListControl->SetItem( ItemIndex, PCSBC_ShadowMapData,	*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_ShadowMapData	)) ); // ShadowMapData
				ListControl->SetItem( ItemIndex, PCSBC_LMSMResolution,	*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_LMSMResolution	)) ); // LMSMResolution
				ListControl->SetItem( ItemIndex, PCSBC_Sections,		*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_Sections		)) ); // Sections
				ListControl->SetItem( ItemIndex, PCSBC_RadiusMin,		*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_RadiusMin		)) ); // RadiusMin
				ListControl->SetItem( ItemIndex, PCSBC_RadiusMax,		*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_RadiusMax		)) ); // RadiusMax
				ListControl->SetItem( ItemIndex, PCSBC_RadiusAvg,		*FString::Printf(TEXT("%i")	, (INT)	StatsEntry.GetColumnData( PCSBC_RadiusAvg		)) ); // RadiusAvg
			}
			...
			if( TRUE )
			{
				...
				ListControl->SetItem( ItemIndex, PCSBC_Count,			*FString::Printf(TEXT("%i")	, (INT)	CombinedStats.Count			) ); // Count
				ListControl->SetItem( ItemIndex, PCSBC_Triangles,		*FString::Printf(TEXT("%i")	, (INT)	CombinedStats.Triangles		) ); // Triangles
				ListControl->SetItem( ItemIndex, PCSBC_ResourceSize,	*FString::Printf(TEXT("%i")	, (INT)	CombinedStats.ResourceSize	) ); // ResourceSize
				...
				ListControl->SetItem( ItemIndex, PCSBC_ObjLightCost,	*FString::Printf(TEXT("%i")	, (INT)	CombinedObjLightCost		) ); // ObjLightCost
				ListControl->SetItem( ItemIndex, PCSBC_TriangleCost,	*FString::Printf(TEXT("%i")	, (INT)	CombinedTriangleCost		) ); // TriangleCost
				ListControl->SetItem( ItemIndex, PCSBC_LightMapData,	*FString::Printf(TEXT("%i")	, (INT)	CombinedLightMapData		) ); // LightMapData
				ListControl->SetItem( ItemIndex, PCSBC_ShadowMapData,	*FString::Printf(TEXT("%i")	, (INT)	CombinedShadowMapData		) ); // ShadowMapData
				ListControl->SetItem( ItemIndex, PCSBC_LMSMResolution,	*FString::Printf(TEXT("%i")	, (INT)	CombinedLMSMResolution		) ); // LMSMResolution
				ListControl->SetItem( ItemIndex, PCSBC_Sections,		*FString::Printf(TEXT("%i")	, (INT)	CombinedStats.Sections		) ); // Sections
				ListControl->SetItem( ItemIndex, PCSBC_RadiusMin,		*FString::Printf(TEXT("%i")	, (INT)	CombinedStats.RadiusMin		) ); // RadiusMin
				ListControl->SetItem( ItemIndex, PCSBC_RadiusMax,		*FString::Printf(TEXT("%i")	, (INT)	CombinedStats.RadiusMax		) ); // RadiusMax
				ListControl->SetItem( ItemIndex, PCSBC_RadiusAvg,		*FString::Printf(TEXT("%i")	, (INT)	CombinedStats.RadiusAvg		) ); // RadiusAvg
			}
