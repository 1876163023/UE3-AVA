...
WxCurveEdPresetDlg::WxCurveEdPresetDlg(WxCurveEditor* parent) :
	...
	bIsSaveDlg(FALSE)
{
	FString	Title	= FString::Printf(*LocalizeUnrealEd("CurveEdPresetCaption"), TEXT(""));
...
void WxCurveEdPresetDlg::CreateControls()
{
	...
	for (INT CurveIndex = 0; CurveIndex < CEP_CURVE_MAX; CurveIndex++)
	{
		...
		CurveControls[CurveIndex].PropertyWindow = new WxPropertyWindow;
 ...
 */
bool WxCurveEdPresetDlg::Show(UBOOL bShow, const TCHAR* CurveName, INT ValidCurves, TArray<UClass*>& CurveEdPresets, UBOOL bInIsSaveDlg)
{
	bIsSaveDlg	= bInIsSaveDlg;
	
	FString	Title	= FString::Printf(*LocalizeUnrealEd("CurveEdPresetCaption"), bIsSaveDlg ? *LocalizeUnrealEd("Saving") : TEXT(""));
	this->SetTitle(*Title);
...
void WxCurveEdPresetDlg::OnOK(wxCommandEvent& In)
{
		...
		if (ValidityFlags & (1 << CurveIndex))
		{
			if (CurveControls[CurveIndex].CurrentPreset)
			{
				if (CurveControls[CurveIndex].CurrentPreset->eventCheckAreSettingsValid(bIsSaveDlg) == FALSE)
				{
					...
					appMsgf(AMT_OK, *LocalizeUnrealEd("CurveEdPreset_InvalidSetting"), *CurveLabel);
	...
	if (bSettingsAreOk)
	{
		...
		for (CurveIndex = 0; CurveIndex < CEP_CURVE_MAX; CurveIndex++)
		{
			...
			{
				CurveControls[CurveIndex].PropertyWindow->SetObject(NULL, 0, 0, 0);
			}
		}
...
void WxCurveEdPresetDlg::OnPresetCombo(wxCommandEvent& In)
{
	...
	if (CurveIndex != -1)
	{
		...
		if (Controls->CurrentPreset == NULL)
		{
			...
			Controls->PropertyWindow->SetObject(Base, 1, 0, 1);
		}
	}
}
		...
		for (INT CurveIndex = 0; CurveIndex < CurveEdPresets.Num(); CurveIndex++)
		{
			...
			if (BasePreset)
			{
				...
				if (CurveIndex == 0)
				{
					...
					{
						CurveControls[ComboIndex].PropertyWindow->SetObject(BasePreset, 1, 0, 1);
					}
				}
			}
		}
