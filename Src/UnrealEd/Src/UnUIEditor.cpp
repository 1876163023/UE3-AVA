 ...
 */
UBOOL UEditorUISceneClient::OpenScene( UUIScene* Scene, ULocalPlayer*, UUIScene** , BYTE)
{
...
void UEditorUISceneClient::RenderScenes( FCanvas* Canvas )
{
	...
	{
		AnimSubscribers(ObjIndex)->TickAnim(GDeltaTime);
	}
 ...
 */
UBOOL UEditorUISceneClient::InputAxis(INT ControllerId,FName Key,FLOAT Delta,FLOAT DeltaTime)
{
	UBOOL bResult = FALSE;
 ...
 */
UBOOL UEditorUISceneClient::InputChar(INT ControllerId, const FInputCompositionStringData& CompStrData)
{
	UBOOL bResult = FALSE;
 ...
 */
UBOOL FEditorUIObjectViewportClient::InputAxis(FViewport* Viewport,INT ControllerId,FName Key,FLOAT Delta,FLOAT DeltaTime)
{
...
void FEditorUIObjectViewportClient::DrawPostProcess( FViewport* Viewport, FCanvas* Canvas, UPostProcessChain* UIPostProcessChain )
{
	...
	FMatrix ProjMat = FCanvas::CalcProjectionMatrix(SizeX,SizeY,fFOV,NEAR_CLIPPING_PLANE);
...
{
	// Render the background and all objects.
	if ( UIEditor == NULL || !UIEditor->bWindowInitialized )
	{
		return;
	}

	// @ WARN : RenderTarget을 바꾸면 RHIEndDrawViewport 중에 D3D_DRIVER_INTERNAL_ERROR가 난다.
	...
	Canvas->PushAbsoluteTransform(FScaleMatrix(Zoom2D) * FTranslationMatrix(FVector(Origin2D.X,Origin2D.Y,0)));
	{
		...
		if( !Canvas->GetHitProxyConsumer() )
		{
			...
			// @TODO : PostProcessChain을 연결. (쓰레기 프레임버퍼가 화면에 뿌려짐)
