...
UBOOL FCurveEdViewportClient::InputKey(FViewport* Viewport, INT ControllerId, FName Key, EInputEvent Event,FLOAT /*AmountDepressed*/,UBOOL /*Gamepad*/)
{
	...
	else if( Key == KEY_RightMouseButton )
	{
		if(Event == IE_Released)
		{
			...
			if(HitResult)
			{
				if( CurveEd->OnRightMouseButtonReleased( HitResult ) )
				{
					// nothing to do
				}
				else
...
UBOOL FCurveEdViewportClient::InputAxis(FViewport* Viewport, INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
...
: wxWindow( InParent, InID )
{
	...
	ThumbPos_Vert	= 0;
	EntryCount		= 0;
    ScrollBar_Vert->SetScrollbar(ThumbPos_Vert, LabelEntryHeight, LabelEntryHeight * MaxLabels, LabelEntryHeight);
...
void WxCurveEditor::OnSetKeyColor(wxCommandEvent& In)
{
	...
	else
	{
		...
		EdInterface->SetKeyOut(0, SelKey.KeyIndex, Value);
		Value = (FLOAT)(clr.Green());
		if (Entry.bClamp)
		{
			Value = Clamp<FLOAT>(Value, Entry.ClampLow, Entry.ClampHigh);
		}
		EdInterface->SetKeyOut(1, SelKey.KeyIndex, Value);
		Value = (FLOAT)(clr.Blue());
		if (Entry.bClamp)
		{
			Value = Clamp<FLOAT>(Value, Entry.ClampLow, Entry.ClampHigh);
		}
...
void WxCurveEditor::OnTabCreate(wxCommandEvent& In)
{
	...
	if (dlg.ShowModal(TEXT("CreateTab"), TEXT("Name"), TEXT("")) == wxID_OK)
	{
		...
		for (INT ii = 0; ii < EdSetup->Tabs.Num(); ii++)
		{
			...
			{
				appMsgf(AMT_OK, *LocalizeUnrealEd("TabNameInUse"), *newName);
				bFound	= true;
				break;
			}
		}
...
void WxCurveEditor::OnPresetCurves(wxCommandEvent& In)
{
	...
	if (PresetDialog)
	{
		PresetDialog->Show(TRUE, *(Entry->CurveName), ValidCurves, CurveEdPresets);
	}
...
void WxCurveEditor::OnSavePresetCurves(wxCommandEvent& In)
{
	...
	if (PresetDialog)
	{
		PresetDialog->Show(TRUE, *(Entry->CurveName), ValidCurves, CurveEdPresets, TRUE);
	}
}
...
void WxCurveEditor::AdjustScrollBar(wxRect& rcClient)
{
	if (ScrollBar_Vert)
	{
		wxRect rcSBV = ScrollBar_Vert->GetClientRect();
		...
		if (ToolBar)
		{
			ScrollBar_Vert->SetSize(0, ToolbarHeight, rcSBV.GetWidth(), rcClient.GetHeight());
		}
		else
		{
	...
	for(INT i=0; i<EdSetup->Tabs(EdSetup->ActiveTab).Curves.Num(); i++)
	{
		...
		if (EdInterface)
		{
			...
			for (INT ii = 0; ii < NumSubs; ii++)
			{
				...
				case 0:
					// Red
					ButtonColor = CURVEEDENTRY_HIDESUBCURVE(Entry.bHideCurve, ii) ? FColor( 32,  0,  0) : FColor(255,  0,  0);
					break;
				case 1:
					if ((NumSubs == 4) || (NumSubs == 6))
					{
						// Dark red
						ButtonColor = CURVEEDENTRY_HIDESUBCURVE(Entry.bHideCurve, ii) ? FColor( 28,  0,  0) : FColor(196,  0,  0);
					}
					else
					{
						// Green
						ButtonColor = CURVEEDENTRY_HIDESUBCURVE(Entry.bHideCurve, ii) ? FColor(  0, 32,  0) : FColor(  0,255,  0);
					}
					break;
				case 2:
					if ((NumSubs == 4) || (NumSubs == 6))
					...
					{
						// Blue
						ButtonColor = CURVEEDENTRY_HIDESUBCURVE(Entry.bHideCurve, ii) ? FColor(  0,  0, 32) : FColor(  0,  0,255);
					}
					break;
				case 3:
					// Dark green
					ButtonColor = CURVEEDENTRY_HIDESUBCURVE(Entry.bHideCurve, ii) ? FColor(  0, 28,  0) : FColor(  0,196,  0);
					break;
				case 4:
					// Blue
					ButtonColor = CURVEEDENTRY_HIDESUBCURVE(Entry.bHideCurve, ii) ? FColor(  0,  0, 32) : FColor(  0,  0,255);
				...
				case 5:
					// Dark blue
					ButtonColor = CURVEEDENTRY_HIDESUBCURVE(Entry.bHideCurve, ii) ? FColor(  0,  0, 28) : FColor(  0,  0,196);
					break;
				}
...
void WxCurveEditor::DrawEntry( FViewport* Viewport, FCanvas* Canvas, const FCurveEdEntry& Entry, INT CurveIndex )
{
	...
	for(INT SubIdx=0; SubIdx<NumSubs; SubIdx++)
	{
		...
		for(INT KeyIdx=0; KeyIdx<NumKeys; KeyIdx++)
		{
			...
			DrawTile(Canvas, NewKeyPos.X-3, NewKeyPos.Y-3, 7, 7, 0.f, 0.f, 1.f, 1.f, BorderColor );
...
void WxCurveEditor::DrawGrid(FViewport* Viewport, FCanvas* Canvas)
{
	...
	while( InNum*InGridSpacing < EndIn )
	{
		...
		DrawLine2D(Canvas, FVector2D(GridPos.X, 0), FVector2D(GridPos.X, CurveViewY), GridColor );
		InNum++;
	}
	...
	while( InNum*InGridSpacing < EndIn )
	{
		...
		DrawString(Canvas, GridPos.X + 2, CurveViewY - YL - 2, *ScaleLabel, GEngine->SmallFont, GridTextColor );
		InNum++;
	}
...
: wxToolBar( InParent, InID, wxDefaultPosition, wxDefaultSize, wxTB_HORIZONTAL | wxTB_3DBUTTONS )
{
	...
	AddSeparator();
	AddTool(IDM_CURVE_TAB_CREATE, *LocalizeUnrealEd("CurveTabCreate"), TabCreateB);
	...
	AddControl(TabCombo);
	AddTool(IDM_CURVE_TAB_DELETE, *LocalizeUnrealEd("CurveTabDelete"), TabDeleteB);
