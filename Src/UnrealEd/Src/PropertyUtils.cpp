...
void UPropertyDrawNumeric::Draw( wxDC* InDC, wxRect InRect, BYTE* InReadAddress, UProperty* InProperty, UPropertyInputProxy* InInputProxy )
{
	...
	if( Str.Len() == 0 )
	{
		if(	InProperty->IsA(UFloatProperty::StaticClass()) == TRUE )
		{
			//			256 UU = 487.68 cm = 16 feet. ::	1 meter = 52.5 UU 

			//float* pFloatValue = (float*)((InReadAddress - InProperty->Offset) + InProperty->Offset + 0 * InProperty->ElementSize);
			float* pFloatValue = (float*)InReadAddress;
			Str = FString::Printf(TEXT("%.2f(%.2fm)"), *pFloatValue, *pFloatValue/52.5f);
		}
		else if( InProperty->IsA(UIntProperty::StaticClass()) == TRUE )
		{
			int* pIntValue = (int*)InReadAddress;
			Str = FString::Printf(TEXT("%d(%.2fm)"), *pIntValue, *pIntValue/52.5f);
		}
		else if( InProperty->IsA(UByteProperty::StaticClass()) == TRUE)
		{
			Byte* pBytValue = (Byte*)InReadAddress;
			Str = FString::Printf(TEXT("%d(%.2fm)"), *pBytValue, *pBytValue/52.5f);
		}
		else
			InProperty->ExportText( 0, Str, InReadAddress - InProperty->Offset, InReadAddress - InProperty->Offset, NULL, PPF_Localized );
	}
	else
	{
		if(	InProperty->IsA(UFloatProperty::StaticClass()) == TRUE )
		{
			float value = _tstof(&Str[0]);
			Str = FString::Printf(TEXT("%.2f(%.2fm)"), value, value/52.5f);
		}
		else if( InProperty->IsA(UIntProperty::StaticClass()) == TRUE )
		{
			int value= _ttoi(&Str[0]);
			Str = FString::Printf(TEXT("%d(%.2fm)"), value, value/52.5f);
		}
		else if( InProperty->IsA(UByteProperty::StaticClass()) == TRUE)
		{
			Byte value = (BYTE)Str[0];
			Str = FString::Printf(TEXT("%d(%.2fm)"), value, value/52.5f);
		}
	}
...
void UPropertyInputColor::ButtonClick( WxPropertyWindow_Base* InItem, ePropButton InButton )
{
	...
	switch( InButton )
	{
		case PB_Browse:
		{
			...
			// TJ: Get topmost window for modal dialog to prevent clicking away
			wxWindow* TopWindow = ObjectNode->GetPropertyWindow();
...
void UPropertyInputNumeric::SendToObjects( WxPropertyWindow_Base* InItem )
{
	...
	if( ObjectNode->GetNumObjects() == 1 || Equation.Len() || TextCtrl->GetValue().Len() )
	{
		...
		BOOL bMeterUnit = FALSE;
		// 가장 마지막 단어가 m이라면, 사용자가 미터로 입력한 것이므로, UU로 바꾸어 준다
		if( Equation.Len() > 0)
		{
			TCHAR tchLast = Equation[Equation.Len() - 1];
			if( tchLast == TEXT('M') || tchLast == TEXT('m') )
			{
				EqResult = float(int(EqResult*52.5f)); // 미터로 입력하였을 경우에, UU서 소숫점을 남기지 않는다
				bMeterUnit = TRUE;
			}
		}
...
void UPropertyInputCombo::CreateControls( WxPropertyWindow_Base* InItem, UClass* InBaseClass, const wxRect& InRC, const TCHAR* InValue )
{
	...
	else if( Cast<UClassProperty>(InItem->Property) )			
	{
		...
		{
			if( It->IsChildOf( ClassProp->MetaClass ) && !(It->ClassFlags&CLASS_Hidden) && !(It->ClassFlags&CLASS_Abstract) && !(It->ClassFlags&CLASS_HideDropDown)&& !(It->ClassFlags&CLASS_Deprecated) )
			{
...
void UPropertyInputEditInline::ButtonClick( WxPropertyWindow_Base* InItem, ePropButton InButton )
{
	switch( InButton )
	{
		case PB_NewObject:
		{
			...
			for( TObjectIterator<UClass> It ; It ; ++It )
			{
				if( It->IsChildOf(ObjProp->PropertyClass) &&
