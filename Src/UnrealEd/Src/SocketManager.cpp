	...
	:	wxToolBar( InParent, InID, wxDefaultPosition, wxDefaultSize, wxTB_HORIZONTAL | wxTB_3DBUTTONS )
{
	...
	AddSeparator();
	AddTool(IDM_ANIMSET_CLEARPREVIEWS, *LocalizeUnrealEd("ClearPreviews"), ClearPreviewB);
...
BEGIN_EVENT_TABLE( WxSocketManager, wxFrame )
...
WxSocketManager::WxSocketManager(WxAnimSetViewer* InASV, wxWindowID InID)
	:	wxFrame( InASV, InID, *LocalizeUnrealEd("SocketManager"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE | wxFRAME_FLOAT_ON_PARENT | wxFRAME_NO_TASKBAR )
{
	AnimSetViewer = InASV;
	...
	TopSizerH->Add( LeftSizerV, 0, wxGROW | wxALL, GControlBorder );

	wxStaticText* ListLabel = new wxStaticText( this, -1, *LocalizeUnrealEd("Sockets") );
	LeftSizerV->Add( ListLabel, 0, wxALIGN_LEFT|wxALL|wxADJUST_MINSIZE, GControlBorder );

	SocketList = new wxListBox( this, IDM_ANIMSET_SOCKETLIST, wxDefaultPosition, wxSize(200, -1), 0, NULL, wxLB_SINGLE );
	LeftSizerV->Add( SocketList, 1, wxGROW | wxALL, GControlBorder );

	wxBoxSizer* ButtonSizerH = new wxBoxSizer( wxHORIZONTAL );
	LeftSizerV->Add( ButtonSizerH, 0, wxGROW | wxALL, GControlBorder );

	NewSocketButton = new wxButton( this, IDM_ANIMSET_NEWSOCKET, *LocalizeUnrealEd("NewSocket"), wxDefaultPosition, wxSize(-1, 30) );
	ButtonSizerH->Add( NewSocketButton, 1, wxGROW | wxRIGHT, GControlBorder );

	DeleteSocketButton = new wxButton( this, IDM_ANIMSET_DELSOCKET, *LocalizeUnrealEd("DeleteSocket"), wxDefaultPosition, wxSize(-1, 30) );
	ButtonSizerH->Add( DeleteSocketButton, 1, wxGROW | wxLEFT, GControlBorder );
	...
	SocketProps = new WxPropertyWindow;
	SocketProps->Create( this, this );
	TopSizerH->Add( SocketProps, 1, wxGROW | wxALL, GControlBorder );
