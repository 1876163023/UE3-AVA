/*=============================================================================
	Copyright (c) 2005-2006 Epic Games, Inc. All Rights Reserved.
=============================================================================*/
...
#include "../../../External/FaceFX/FxStudio/Inc/FxRenderWidgetUE3.h"
#endif // WITH_FACEFX
...
public:
	...
	WxMBGenericBrowser()
	{
		...
		FileMenu->AppendSeparator();
		FileMenu->Append( IDMN_CheckTrashcan, *LocalizeUnrealEd("CheckTrashcan"), *LocalizeUnrealEd("ToolTip_CheckTrashcan") );
		FileMenu->AppendSeparator();
...
BEGIN_EVENT_TABLE( WxGenericBrowser, WxBrowser )
	...
	EVT_MENU( IDMN_CheckTrashcan, WxGenericBrowser::OnCheckTrashcan )
	...
	EVT_MENU( IDMN_AVA_SelectSurfaceMatchingSelectedMaterial, WxGenericBrowser::Ava_OnSelectSurfaceMatchingSelectedMaterial )
	//>@ ava
...
public:
	...
	{
		if ( Object != NULL && !Object->HasAnyFlags(RF_TagExp) )
		{
...
void WxGenericBrowser::ImportFiles(const wxArrayString& InFiles, const TArray<UFactory*> &InFactories)
{
	...
	for( UINT FileIndex = 0 ; FileIndex < InFiles.Count() ; FileIndex++ )
	{
		...
		// Only select factory for the first file
		if( !bSawSuccessfulImport )
		{
			...
			{
				WxDlgGenericComboEntry SelectFactory;

				// Get the factory names
				TArray<FString>	FactoryNames;
				
				FactoryNames.Empty( Factories.Num() );
				for( INT i = 0; i < Factories.Num(); i++ )
				{
					FactoryNames.AddItem( Factories( i )->GetClass()->GetName() );
				}

				const INT Result = SelectFactory.ShowModal( TEXT( "SelectImportFactory" ), TEXT( "Factory" ), FactoryNames, 0, TRUE );
				if( Result == wxID_OK )
				{
					Factory = Factories( SelectFactory.GetSelectedIndex() );
				}
			}
		}
...
//<@ ava specific ;  2007. 8. 24 changmin
void WxGenericBrowser::Ava_OnSelectSurfaceMatchingSelectedMaterial( wxCommandEvent& In )
{
	GUnrealEd->Exec( TEXT("POLY SELECT MATCHING SELECTEDMATERIAL") );
}
//>@ ava
...
void WxGenericBrowser::OnCheckTrashcan( wxCommandEvent& In )
{
	FString OutputString;
	TArray<UObject*> Objects;
	TArray<UObject*> TrashcanObjects;

	TArray<FString> PackageNameList =  GPackageFileCache->GetPackageFileList();
	for(INT i = PackageNameList.Num() - 1 ; i >= 0 ; i--)
	{
		if( PackageNameList(i).InStr(TRASHCAN_DIRECTORY_NAME) != INDEX_NONE || 
			PackageNameList(i).InStr(GEditor->AutoSaveDir) != INDEX_NONE )
		{
	...
	// Collect the referencing information from Package currently not loaded
	INT GCCount = 0;
	for(INT i = PackageNameList.Num() ; i >= 0 ; i--, GCCount++)
	{
		UPackage* Package = Cast<UPackage>(UObject::LoadPackage(NULL, *PackageNameList(i),NULL));
		if( Package )
			PackageList.AddItem(Package);
		else
			OutputString += FString::Printf( TEXT("can't open package %s. skipped%s"), *PackageNameList(i), LINE_TERMINATOR);

		if( GCCount % 5 == 0 || i == 0)
		{
			GEditor->CheckForTrashcanReferences(PackageList,Objects,TrashcanObjects);
...
void WxGenericBrowser::OnObjectDuplicate(wxCommandEvent& In)
{
	...
	for( INT ObjectIndex = 0 ; ObjectIndex < SelectedObjects.Num() ; ++ObjectIndex )
	{
		...
		if( Object )
		{
			...
			dlg.SetTitle( *LocalizeUnrealEd("Duplicate") );
			FString Reason;

			FString Package = Object->GetOutermost()->GetName();
			FString Group = Object->GetOuter()->GetOuter() ? Object->GetFullGroupName( 1 ) : TEXT("");
			FString Name = FString::Printf( TEXT("%s_Dup"), *Object->GetName() );
			...
			if( dlg.ShowModal( Package, Group, Name ) == wxID_OK )
			{
				...
				if( !HandleFullyLoadingPackages( TopLevelPackages, TEXT("Duplicate") ) )
				{
					// User aborted.
				}
				else if(!dlg.GetPackage().Len() || !dlg.GetObjectName().Len())
				{
				...
				else
				{
					...
					UObject* DupObject = GEditor->StaticDuplicateObject(Object,Object,UObject::CreatePackage(NULL,*Pkg),*dlg.GetObjectName());
					if(DupObject)
					{
...
// Based on WxGenericBrowser::OnObjectDuplicate()
void WxGenericBrowser::OnObjectDuplicateWithRefs(wxCommandEvent& In)
{
	...
	WxDlgMove MoveDialog;
	MoveDialog.SetTitle( *LocalizeUnrealEd("DuplicateWithReferences") );
	MoveDialog.PGNCtrl->NameLabel->Enable();
	MoveDialog.PGNCtrl->NameEdit->Enable();
	MoveDialog.PGNCtrl->NameLabel->SetLabel(*LocalizeUnrealEd("NameSuffix"));

	UObject* Object = SelectedObjects(0);
	FString Package = Object->GetOutermost()->GetName();
	FString Group = Object->GetOuter()->GetOuter() ? Object->GetFullGroupName( 1 ) : TEXT("");
	FString Name = FString::Printf( TEXT("_Dup") );

	const int MoveDialogResult = MoveDialog.ShowModal(Package, Group, Name);
	...
	// Get string for Texture package/Group
	FString TxtPkgGrp;
	{
		// If texture package is not specified then use main package
		FString TxtPkg = (MoveDialog.GetNewTxtPackage().Len() ) ? 
			*MoveDialog.GetNewTxtPackage() : *MoveDialog.GetNewPackage();
		FString TxtGrp = *MoveDialog.GetNewTxtGroup();
		if ( TxtGrp.Len() )
		{
			TxtPkgGrp = FString::Printf(TEXT("%s.%s"), *TxtPkg, *TxtGrp);
		}
		else
		{
			TxtPkgGrp = FString::Printf(TEXT("%s"), *TxtPkg);
		}
	}

	// Get string for Mesh Package/Group
	FString MshPkgGrp;
	{
		// If texture package is not specified then use main package
		FString MshPkg = *MoveDialog.GetNewPackage();
		FString MshGrp = *MoveDialog.GetNewGroup();
		if ( MshGrp.Len() )
		{
			MshPkgGrp = FString::Printf(TEXT("%s.%s"), *MshPkg, *MshGrp);
		}
		else
		{
			MshPkgGrp = FString::Printf(TEXT("%s"), *MshPkg);
		}
	}
	...
	for( INT ObjectIndex = 0 ; ObjectIndex < SelectedObjects.Num() ; ++ObjectIndex )
	{
		...
		if( Object )
		{
			...
			ReferencedObjects.AddItem(Object);
			...
			FString Reason;
			UBOOL	bUserAborted = FALSE;

			if( !MoveDialog.GetNewPackage().Len() )
			{
				ErrorMessage += TEXT("Invalid package name supplied");
			}
			else
			{
				for (INT RefObjectIndex = 0 ; RefObjectIndex < ReferencedObjects.Num() ; ++RefObjectIndex )
				{
					UObject *RefObject = ReferencedObjects(RefObjectIndex);

					const UBOOL bIsTextureOrMaterial = RefObject->IsA(UMaterial::StaticClass()) || RefObject->IsA(UTexture::StaticClass());

					UObject* ExistingPackage;
					if ( bIsTextureOrMaterial )
					{
						ExistingPackage = UObject::FindPackage(NULL, *TxtPkgGrp);
					}
					else
					{
						ExistingPackage = UObject::FindPackage(NULL, *MshPkgGrp);
					}

					const FString ObjName = FString::Printf(TEXT("%s%s"), *RefObject->GetName(), *MoveDialog.GetNewName());
			...
			// User aborted, no need to display message box.
			if( bUserAborted )
			{
				continue;
			}
			...
			for ( INT RefObjectIndex = 0 ; RefObjectIndex < ReferencedObjects.Num() ; ++RefObjectIndex )
			{
				...
				const FString Pkg( bIsTextureOrMaterial ? TxtPkgGrp : MshPkgGrp );
				UObject* ExistingPackage = UObject::FindPackage(NULL, *Pkg);
				const FString ObjName = FString::Printf( TEXT("%s%s"), *RefObject->GetName(), *MoveDialog.GetNewName() );
				...
				UObject* DupObject = GEditor->StaticDuplicateObject( RefObject, RefObject, UObject::CreatePackage(NULL,*Pkg), *ObjName );
				if( DupObject )
				{
...
void WxGenericBrowser::OnObjectRename(wxCommandEvent& In)
{
	...
	// Make sure packages objects are renamed from are fully loaded.
	if( !HandleFullyLoadingPackages( SelectedObjects, TEXT("Rename") ) )
	{
		return;
	}
	...
	for ( INT Index = 0 ; Index < SelectedObjects.Num() ; Index++ )
	{
		...
		if( !Cast<UPackage>(SelectedObjects(Index)->GetOuter()->GetOuter()) )
		{
			Group = TEXT("");
			Package = SelectedObjects(Index)->GetOuter()->GetName();
		}
		else
		{			
			Group = SelectedObjects(Index)->GetFullGroupName( 1 );
			Package = SelectedObjects(Index)->GetOutermost()->GetName();
		}

		if( RenameDialog.ShowModal(Package, Group, SelectedObjects(Index)->GetName()) == wxID_OK )
		{
			if( !RenameDialog.GetNewPackage().Len() || !RenameDialog.GetNewName().Len() )
			{
				appMsgf(AMT_OK,*LocalizeUnrealEd("Error_InvalidInput"));
			}
			else
			{
				...
				FString Pkg;
				if( RenameDialog.GetNewGroup().Len() > 0 )
				{
					Pkg = FString::Printf(TEXT("%s.%s.%s"),*RenameDialog.GetNewPackage(),*RenameDialog.GetNewGroup(),*RenameDialog.GetNewName());
				}
				else
				{
					Pkg = FString::Printf(TEXT("%s.%s"),*RenameDialog.GetNewPackage(),*RenameDialog.GetNewName());
				}

				// We can rename on top of an object redirection (basically destroy the redirection and put us in its place).
				UObjectRedirector* Redirector = Cast<UObjectRedirector>( UObject::StaticFindObject(UObjectRedirector::StaticClass(), NULL, *Pkg) );

				// If we found a redirector, check that the object it points to is of the same class.
				if ( Redirector && Redirector->DestinationObject->GetClass() == SelectedObjects(Index)->GetClass())
				{
				...
				UPackage* NewPackage = UObject::CreatePackage(NULL,*(RenameDialog.GetNewGroup().Len() ? RenameDialog.GetNewPackage() + TEXT(".") + RenameDialog.GetNewGroup() : RenameDialog.GetNewPackage()));

				// Test to see if the rename will succeed.
				if ( SelectedObjects(Index)->Rename(*RenameDialog.GetNewName(), NewPackage, REN_Test) )
				{
					bSawSuccessfulRename = TRUE;
					GEditor->RenameObject(SelectedObjects(Index), NewPackage, *RenameDialog.GetNewName(), REN_None);
				}
				else
				{
...
// Based on WxGenericBrowser::OnObjectRename()
void WxGenericBrowser::OnObjectRenameWithRefs(wxCommandEvent& In) 
{
	...
	for (INT Index = 0; Index < SelectedObjects.Num(); Index++)
	{
		...
		{
			bFirstTime = FALSE;
			const int MoveDialogResult = MoveDialog.ShowModal(Package, Group, SelectedObjects(Index)->GetName());
			if ( MoveDialogResult != wxID_OK && MoveDialogResult != ID_OK_ALL )
			{
				return;
			}
...

		{
			// Find references.
			TArray<UObject*> ReferencedObjects;
			ReferencedObjects.AddItem( SelectedObjects(Index) );
			if ( MoveDialog.GetIncludeRefs() )
			{
				{
					TArray<UClass*> IgnoreClasses;
			...
			for ( INT ObjectIndex = 0 ; ObjectIndex < ReferencedObjects.Num() ; ++ObjectIndex )
			{
				...
				FString	ThisGroup, ThisPackage;
				if(!Cast<UPackage>(Object->GetOuter()->GetOuter()))
				{
					ThisGroup = TEXT("");
					ThisPackage = Object->GetOuter()->GetName();
				}
				else
				{			
					ThisGroup = Object->GetFullGroupName( TRUE );
					ThisPackage = Object->GetOutermost()->GetName();
				}
				...
				PkgName = *MoveDialog.GetNewPackage();
				GrpName = ThisGroup;//*MoveDialog.GetNewGroup();

				if ( GrpName.Len() )
				{
					PkgGrp = FString::Printf(TEXT("%s.%s"), *PkgName, *GrpName);
					Pkg = FString::Printf(TEXT("%s.%s.%s"), *PkgName, *GrpName, *Object->GetName());
				}
				else
				{
					PkgGrp = FString::Printf(TEXT("%s"), *PkgName);
					Pkg = FString::Printf(TEXT("%s.%s"), *PkgName, *Object->GetName());
				}
				...
				// If we found a redirector, check that the object it points to is of the same class.
				if ( Redirector && Redirector->DestinationObject->GetClass() == Object->GetClass() )
				{
					// Remove public flag if set to ensure below rename doesn't create a redirect.
					Redirector->ClearFlags( RF_Public );
					// Instead of deleting we rename the redirector into a dummy package.
				...
				UPackage* NewPackage = UObject::CreatePackage(NULL, *PkgGrp);

				// Test to see if the rename will succeed.
				if ( Object->Rename(*Object->GetName(), NewPackage, REN_Test) )
				{
					bSawSuccessfulRename = TRUE;
					GEditor->RenameObject(Object, NewPackage, *Object->GetName(), REN_None);
				}
				else
				{
					ErrorMessage += FString::Printf( *LocalizeUnrealEd("Error_ObjectNameAlreadyExists"), *SelectedObjects(Index)->GetFullName() );
...
void WxGenericBrowser::OnObjectDelete(wxCommandEvent& In)
{
	...
	for (INT Index = 0; Index < SelectedObjects.Num(); Index++)
	{
		...
		if ( !bDeleteAborted )
		{
			...
			{
				// We cannot savely delete this object. Print out a list of objects referencing this one
...
// Based on WxGenericBrowser::OnObjectDelete()
...
void WxGenericBrowser::ExportObjects(const TArray<UObject*>& ObjectsToExport, UBOOL bPromptIndividualFilenames)
{
	...
	for (INT Index = 0; Index < ObjectsToExport.Num(); Index++)
	{
		...
		else
		{
			...
			{
				WxDlgGenericComboEntry SelectExporter;

				// Get the factory names
				TArray<FString>	ExporterNames;

				ExporterNames.Empty( ValidExporters.Num() );
				for( INT i = 0; i < ValidExporters.Num(); i++ )
				{
					ExporterNames.AddItem( ValidExporters( i )->GetClass()->GetName() );
				}

				const INT Result = SelectExporter.ShowModal( TEXT( "SelectExporter" ), TEXT( "Exporter" ), ExporterNames, 0, TRUE );
				if( Result == wxID_OK )
				{
					ExporterToUse = ValidExporters( SelectExporter.GetSelectedIndex() );
				}
			}
...
public:
	wxGBLeftContainerMenu()
	{
		...
		if( AudioDevice )
		{
			...
			// Add SoundGroup submenu to Sound submenu.
			wxMenu*	SoundMenu = new wxMenu();
			SoundMenu->Append(IDMN_PackageContext_BatchProcess_SoundGroups,*LocalizeUnrealEd("SoundGroups"),SoundGroupMenu);
			SoundMenu->Append( IDMN_PackageContext_ClusterSounds, *LocalizeUnrealEd("ClusterSounds"), TEXT("") );
			SoundMenu->Append( IDMN_PackageContext_ClusterSoundsWithAttenuation, *LocalizeUnrealEd("ClusterSoundsWithAttenuation"), TEXT("") );
			SoundMenu->Append( IDMN_PackageContext_SetAsCompressedSamples, *LocalizeUnrealEd("SetAsCompressedSamples"), TEXT("") );
			SoundMenu->Append( IDMN_PackageContext_SetAsUncompressedSamples, *LocalizeUnrealEd("SetAsUncompressedSamples"), TEXT("") );			


			// Add Sound submenu to BatchProcess menu
			Append( IDMN_PackageContext_BatchProcess, *LocalizeUnrealEd("BatchProcess"), SoundMenu );
		}
	}
};
...
BEGIN_EVENT_TABLE( WxGBLeftContainer, wxPanel )
	...
	EVT_MENU( IDMN_PackageContext_SetAsCompressedSamples, WxGBLeftContainer::OnSetAsCompressedSamples )
	EVT_MENU( IDMN_PackageContext_SetAsUncompressedSamples, WxGBLeftContainer::OnSetAsUncompressedSamples )
...
void WxGBLeftContainer::OnSetAsCompressedSamples(wxCommandEvent& InEvent)
{
	SetAsCompressedSamples( TRUE );
}

void WxGBLeftContainer::OnSetAsUncompressedSamples(wxCommandEvent& InEvent)
{
	SetAsCompressedSamples( FALSE );
}

void WxGBLeftContainer::SetAsCompressedSamples(UBOOL bCompressedSample)
{
	// Retrieve list of selected packages.
	TArray<UPackage*> SelectedPackages;
	GetSelectedPackages( &SelectedPackages );
	...
	for( INT PackageIndex = 0 ; PackageIndex < SelectedPackages.Num() ; ++PackageIndex )
	{
		...
		for ( INT WaveIndex = 0 ; WaveIndex < WaveList.Num() ; ++WaveIndex )
		{
			...
			if (Wave)
			{
				Wave->bCompressedSample = bCompressedSample;

				Wave->CompressedPCData.RemoveBulkData();
				Wave->CompressedXbox360Data.RemoveBulkData();
				Wave->CompressedPS3Data.RemoveBulkData();
	...
	if((Key == KEY_LeftMouseButton || Key == KEY_RightMouseButton) && (Event == IE_Pressed || Event == IE_DoubleClick))
	{
		...
		if( HitObject )
		{
			...
			else
			{
				...
				if( Key == KEY_LeftMouseButton && GetGBSelection()->IsSelected( HitObject ) && Cast<UMaterialInstance>( HitObject) )
				{
...
UBOOL WxGBRightContainer::InputAxis(FViewport* Viewport,INT ControllerId,FName Key,FLOAT Delta,FLOAT DeltaTime)
{
	if ( bIsScrolling )
	{
...
void WxGBRightContainer::GetObjectsInPackages(const TArray<UPackage*>& InPackages, TArray<UObject*>& OutObjects, UBOOL bIgnoreUsageFilter/*=FALSE*/ )
{
	...
	for( TObjectIterator<UObject> It ; It ; ++It )
	{
		...
		{
			// Filter by the current brower resource type.
...
void WxGBRightContainer::SyncToObject(UObject* InObject, UBOOL bDeselectAll)
{
	...
	if ( bDeselectAll )
	{
		GetGBSelection()->SelectNone( InObject->GetClass() );
	}
