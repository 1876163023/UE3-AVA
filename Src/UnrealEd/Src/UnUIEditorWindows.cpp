...
#include "InterpEditor.h" // For UI-Matinee expansion
/* ==========================================================================================================
	FUISelectionTool
========================================================================================================== */
/**
...
IMPLEMENT_DYNAMIC_CLASS(WxUIEditorBase, wxFrame)
	...
	if ( !Delta.IsNearlyZero() )
	{
		...
		if(NumWidgets == 1)
		{
			...
			if ( Widget->Rotation.AnchorType == RA_Absolute )
			{
				...
				Widget->SetAnchorPosition( NewAnchorLocation );
...
void WxUIEditorBase::OnContext_BindListElement( wxCommandEvent& Event )
{
	...
	if(SelectedWidgets.Num() == 1)
	{
		...
		if ( ListWidget->DataProvider )
		{
			...
			if ( SchemaProvider )
			{
				TMap<FName,FString> CellTagsMap;
				TArray<FName> CellTags;
				TArray<FString> CellHeaderTexts;
				SchemaProvider->GetElementCellTags(CellTagsMap);
				CellTagsMap.GenerateKeyArray(CellTags);
				CellTagsMap.GenerateValueArray(CellHeaderTexts);

				const INT AbsIdx = Event.GetId() - IDM_UI_LISTELEMENTTAG_START;
				INT ElementIdx = AbsIdx % (CellTags.Num() + 1 + UIORIENT_MAX * UIALIGN_MAX + CLIP_MAX );	// +1 for the "remove" item
				INT ColumnIdx = AbsIdx / (CellTags.Num() + 1 + UIORIENT_MAX * UIALIGN_MAX + CLIP_MAX );
				...
				if(ColumnIdx >= 0)
				{
					if ( ElementIdx < CellTags.Num() )
					{
						...
						FString CellHeaderText = CellHeaderTexts(ElementIdx);

						INT ListCellCount = ListWidget->CellDataComponent != NULL
							? ListWidget->CellDataComponent->ElementSchema.Cells.Num()
							: ListWidget->GetMaxNumVisibleColumns();
						...
						if ( ColumnIdx > ListCellCount )
						{
							...
							if ( ListWidget->InsertSchemaCell(ClickedColumn, CellTag, CellHeaderText) )
							{
								bSuccessful = TRUE;
							}
						}
						else
						{
							...
							if ( ListWidget->SetCellBinding(CellTag, CellHeaderText, ColumnIdx) )
							{
								bSuccessful = TRUE;
							}
						}
					...
					{
						INT ProcIndex = ElementIdx - CellTags.Num();
						...
						else if( 0 <= ( ProcIndex -= 1 ) && ProcIndex < (UIALIGN_MAX * UIORIENT_MAX) )
						{
							BYTE Align = ProcIndex % UIALIGN_MAX;
							BYTE Dimension = ProcIndex / UIALIGN_MAX;
							if ( ListWidget != NULL && ListWidget->CellDataComponent != NULL)
							{
								ListWidget->CellDataComponent->SetColumnAlignment(ColumnIdx, (EUIOrientation)Dimension, (EUIAlignment)Align);
							}
							bSuccessful = TRUE;
						}
						else if ( 0 <= (ProcIndex -= UIALIGN_MAX * UIORIENT_MAX) && ProcIndex < CLIP_MAX )
						{
							BYTE ClipMode = ProcIndex;
							if ( ListWidget != NULL && ListWidget->CellDataComponent != NULL)
							{
								ListWidget->CellDataComponent->SetColumnClipMode(ColumnIdx, (ETextClipMode)ClipMode);
							}
						}
					}
				}
...
void WxUIKismetEditor::OpenMatinee(USeqAct_Interp* Interp)
{
	Super::OpenMatinee( Interp );
	...
	if( GEditorModeTools().GetCurrentModeID() == EM_InterpEdit )
	{
		...
		/// tell editor that it is called by ui editor :)
		InterpEditMode->InterpEd->UIEditor = UIEditor;
	}
}
