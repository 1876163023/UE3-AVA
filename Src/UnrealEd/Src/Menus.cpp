...
WxMainContextMenu::WxMainContextMenu()
{
	...
	if( bHaveKActor || bHaveStaticMesh || bHaveMover )
	{
		...
		if(bHaveStaticMesh)
		{
			if ( bAllSelectedStaticMeshesHaveCollisionModels )
			{
				...
				ConvertMenu->Append(IDMENU_ActorPopupConvertStaticMeshToavaKActor, *LocalizeUnrealEd("ConvertStaticMeshToavaKActor"), TEXT("") );
			}
		...
		if(FirstActor != NULL)
		{
			...
			if(MeshComponents.Num())
			{
				...
				for(INT MeshIdx=0; MeshIdx<MeshComponents.Num(); MeshIdx++)
				{
					...
					{
						UMaterialInstance* MaterialInstance = MeshComponent->GetMaterial(MaterialIdx);

						wxMenu* EditCreateMenu = new wxMenu;
						{
							EditCreateMenu->Append(IDM_CREATEMATERIALINSTANCE_START+MenuIdx, *LocalizeUnrealEd("CreateMaterialInstance"), TEXT(""));
							EditCreateMenu->Append(IDM_EDITMATERIALINSTANCE_START+MenuIdx, *LocalizeUnrealEd("EditMaterialInstance"), TEXT(""));
							EditCreateMenu->Append(IDM_ASSIGNMATERIALINSTANCE_START+MenuIdx, *LocalizeUnrealEd("AssignMaterial"), TEXT(""));
						}
						MaterialMenu->Append(wxID_ANY, *MaterialInstance->GetName(), EditCreateMenu);
						MenuIdx++;
					}
	...
	Append(IDM_BackDropPopupPlayFromHereInEditor_Precompute, *LocalizeUnrealEd("ToolTip_137_Precompute"), *LocalizeUnrealEd("ToolTip_135_Precompute"));
}
...
WxMainContextSurfaceMenu::WxMainContextSurfaceMenu()
{
	...
	UMaterialInstance* mi = GEditor->GetSelectedObjects()->GetTop<UMaterialInstance>();
