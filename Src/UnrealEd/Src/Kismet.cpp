...
void WxKismet::DoubleClickedObject(UObject* Obj)
{
	...
	{
		USequenceFrame *Comment = (USequenceFrame*)(Obj);
		WxDlgGenericStringEntry dlg;
		INT Result = dlg.ShowModal( TEXT("EditComment"), TEXT("CommentText"), TEXT("Comment") );
		if (Result == wxID_OK)
		{
...
USequenceObject* WxKismet::NewShorcutObject()
{
	...
	// If we got a class (and its a subclass of USequenceObject), create a new object.
	if(NewSeqObjClass && NewSeqObjClass->IsChildOf(USequenceObject::StaticClass()))
	{
...
BEGIN_EVENT_TABLE( WxKismet, wxFrame )
	...
	EVT_MENU(IDM_KISMET_SWITCH_ADD, WxKismet::OnConextSwitchAdd)
...
void WxKismet::InitSeqObjClasses()
{
	...
	{
		if ( It->IsChildOf(USequenceObject::StaticClass()) && IsValidSequenceClass(*It) )
		{
...
void WxKismet::OnContextCreateLinkedVariable(wxCommandEvent &In)
{
	if(ConnSeqOp != NULL && ConnType == LOC_VARIABLE)
	{
		...
		if(varClass)
		{
			...
			//@fixme - use AddSequenceObject instead
...
void WxKismet::OnConextSwitchAdd( wxCommandEvent& In )
{
	check( SelectedSeqObjs.Num() == 1 );

	USeqCond_SwitchClass* Switch = Cast<USeqCond_SwitchClass>( SelectedSeqObjs(0) );
	check(Switch);
	...
	Switch->ClassArray.InsertZeroed( Num );
	Switch->ClassArray(Num).ClassName = NAME_None;
}

/** Handler for removing a switch connector */
void WxKismet::OnConextSwitchRemove( wxCommandEvent& In )
{
	USeqCond_SwitchClass* Switch = Cast<USeqCond_SwitchClass>(ConnSeqOp);
	check(Switch);
	...
	if( appStricmp( *Switch->OutputLinks(ConnIndex).LinkDesc, TEXT("Default") ) != 0 )
	{
		...
		Switch->ClassArray.Remove( ConnIndex );
	}
}
...
void WxKismet::OnContextBreakAllOpLinks( wxCommandEvent& In )
{
	...
	for(INT i=0; i<SelectedSeqObjs.Num(); i++)
	{
		// Mark object for undo
		if(!ModifiedObjects.ContainsItem(SelectedSeqObjs(i)))
		{
			SelectedSeqObjs(i)->Modify();
			ModifiedObjects.AddItem(SelectedSeqObjs(i));
		}
	...
	if (Result == wxID_OK)
	{
		...
		else
		{
			USequence *NewSeq = NULL;
			{
				...
				NewSeq = ConstructObject<USequence>(USequence::StaticClass(), Sequence, *NewSeqName, RF_Transactional);
