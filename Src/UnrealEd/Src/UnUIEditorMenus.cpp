...
: wxMenu(), NextStyleMenuID(IDM_UI_STYLECLASS_BEGIN), NextEditStyleMenuID(IDM_UI_STYLEREF_BEGIN)
{
	if ( InOwner != NULL )
	{
...
//  [11/1/2006 otterrrr, Insert] , Extended StyleMenu ( 2 Level Style Menu )
			const INT MaxContainItem = 50;
			const INT MaxContainCount = 50;
			INT ContainerCount = ValidStyles.Num()/ MaxContainItem + 1;
			checkf(ContainerCount <= MaxContainCount, TEXT("No more ContainMenus for StyleContainter - might need to increase the number of MaxContainCount"));

			wxMenu* FixedContainStyleMenu[MaxContainCount];
			for(INT i = 0 ; i < ContainerCount ; i++)
			{
				FString Msg = appItoa(i * MaxContainItem) + TEXT(" ~ ") + appItoa((i + 1) * (MaxContainItem) - 1);
				wxString wxStr = *Msg;
				FixedContainStyleMenu[i] = new wxMenu;
				AvailableStylesMenu->Append(0,wxStr,FixedContainStyleMenu[i]);
			}
//  [11/1/2006 otterrrr]
			INT SelectedIndex = INDEX_NONE;
			for ( INT i = 0; i < ValidStyles.Num(); i++ )
			{
				UUIStyle* Style = ValidStyles(i);
...
//  [11/1/2006 otterrrr, Modify] Uncomment if you want to recover the original code.
//				AvailableStylesMenu->AppendCheckItem( StyleMenuID, *Style->GetStyleName() );
				FixedContainStyleMenu[ i / MaxContainItem ]->AppendCheckItem( StyleMenuID, *Style->GetStyleName() );
...
void WxUIMenu::AppendDataStoreMenu()
{
	...
	if ( UIEditor->SceneManager->AreDataStoresInitialized() )
	{
		WxDlgUIDataStoreBrowser* DataStoreBowser = UIEditor->SceneManager->GetDataStoreBrowser();

		FName Tag;
		const UBOOL bTagSelected = DataStoreBowser->GetSelectedTag(Tag);

		if(bTagSelected)
		{
			const FString DataPath = DataStoreBowser->GetSelectedTagPath();
			FString MenuString = FString::Printf(*LocalizeUI(TEXT("UIEditor_BindDataStore_F")), *DataPath);
			wxMenuItem* Item = Append(IDM_UI_BINDWIDGETSTODATASTORE, *MenuString);

			if ( UIEditor->IsSelectedDataStoreTagValid(SelectedWidgets) == FALSE )
			{
				Item->Enable(false);
			}
		}
		...
		{
			wxMenuItem* Item = Append(IDM_UI_BINDWIDGETSTODATASTORE, *LocalizeUI(TEXT("UIEditor_BindDataStore")));
			Item->Enable(false);
		}

		Append(IDM_UI_CLEARWIDGETSDATASTORE, *LocalizeUI(TEXT("UIEditor_ClearDataStore")));
	}
	else
	{
...
void WxUIListBindMenu::Create( UUIObject* InActiveWidget, INT ColumnIdx )
{
	...
	if ( OwnerList->DataProvider )
	{
		...
		if ( CellProvider )
		{
			...
			CellProvider->GetElementCellTags(ElementMap);
			ElementMap.GenerateKeyArray(ElementNames);
			...
			if(ColumnIdx > -1)
			{
				...
				const INT CmdUnitSize = ElementNames.Num() + 1 + UIORIENT_MAX * UIALIGN_MAX + CLIP_MAX;
				...
				// Cell Alignment 메뉴
				static UEnum* DimEnum = FindObject<UEnum>(ANY_PACKAGE, TEXT("EUIOrientation"), TRUE);
				static UEnum* AlignEnum = FindObject<UEnum>(ANY_PACKAGE, TEXT("EUIAlignment"), TRUE);
				for( INT DimIndex = 0 ; DimIndex < UIORIENT_MAX ; DimIndex++ )
				{
					wxMenu* AlignMenu = new wxMenu;
					for( INT AlignIndex = 0 ; AlignIndex < UIALIGN_MAX ; AlignIndex++)
					{
						INT MenuIndex = IDM_UI_LISTELEMENTTAG_START + ColumnIdx * ( ElementNames.Num() + 1 + UIORIENT_MAX * UIALIGN_MAX + CLIP_MAX) + ElementNames.Num() + 1;
						wxMenuItem* AlignItem = AlignMenu->AppendCheckItem( MenuIndex + UIALIGN_MAX * DimIndex + AlignIndex, AlignEnum ? AlignEnum->GetEnum(AlignIndex).GetName() : *appItoa(AlignIndex) );
						if( OwnerList != NULL && OwnerList->CellDataComponent != NULL && OwnerList->CellDataComponent->ElementSchema.Cells.IsValidIndex(ColumnIdx) && OwnerList->CellDataComponent->ElementSchema.Cells(ColumnIdx).Alignment[DimIndex] == AlignIndex )
							AlignItem->Check();
					}
					FString AlignMenuName = LocalizeUI(TEXT("UIEditor_CellAlignment")) + TEXT("[") + (DimEnum ? DimEnum->GetEnum(DimIndex).GetName() : *appItoa(DimIndex)) + TEXT("]");
					Append( IDM_UI_LISTCELL_ALIGNMENT, *AlignMenuName, AlignMenu);
				}
				AppendSeparator();

				// Cell ClipMode 메뉴
				static UEnum* ClipEnum = FindObject<UEnum>(ANY_PACKAGE, TEXT("ETextClipMode"),TRUE);
				wxMenu* ClipMenu = new wxMenu;
				for( INT i = 0 ; i < CLIP_MAX ; i++ )
				{
					INT MenuIndex = IDM_UI_LISTELEMENTTAG_START + ColumnIdx * ( ElementNames.Num() + 1 + UIORIENT_MAX * UIALIGN_MAX + CLIP_MAX) + (ElementNames.Num() + 1 + UIORIENT_MAX * UIALIGN_MAX);
					wxMenuItem* ClipItem = ClipMenu->AppendCheckItem( MenuIndex + i, ClipEnum ? ClipEnum->GetEnum(i).GetName() : *appItoa(i) );
					if( OwnerList != NULL && OwnerList->CellDataComponent != NULL && OwnerList->CellDataComponent->ElementSchema.Cells.IsValidIndex(ColumnIdx) && OwnerList->CellDataComponent->ElementSchema.Cells(ColumnIdx).ClipMode == i )
						ClipItem->Check();
				}
				FString ClipMenuName = TEXT("ClipMode");
				Append( IDM_UI_LISTCELL_CLIPMODE, *ClipMenuName, ClipMenu );
				AppendSeparator();
...
wxMenu* WxUIListBindMenu::CreateColumnMenu(UUIList* InList, const TArray<FName> &ElementNames, wxMenu* ColumnMenu, INT ColumnIdx)
{
	...
	// | Element 개수 ( CellTags.Num ) || 'AppendCell' || 'InsertCell' || 'RemoveCell'  ||  UIORIENT_MAX * UIALING_MAX || CLIP_MAX ||
	INT MenuIdx = IDM_UI_LISTELEMENTTAG_START + ColumnIdx * (ElementNames.Num() + 1 + UIORIENT_MAX * UIALIGN_MAX + CLIP_MAX);
