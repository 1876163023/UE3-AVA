...
UObject* UUnrealEdEngine::Archetype_CreateFromObject( UObject* ArchetypeBase, FString& ArchetypeName, FString& PackageName, FString& GroupName )
{
	...
	if ( ArchetypeName.Len() > 0 && PackageName.Len() > 0 )
	{
		...
		// now update the actor class browser so that the new archetype UClass appears in the tree
		//@todo - should archetypes appear in the actor browser?
...
static UBOOL SequenceReferencesOnlyObjs(USequence* InSeq, TArray<UObject*>& InObjects)
{
	...
	if(BadObjs.Num() > 0)
	{
		FString BadObjMessage = FString::Printf( *LocalizeUnrealEd("Prefab_SequenceRefsOutsidePrefab"), *InSeq->ObjName );
...
static USequence* FindPrefabSequence(TArray<UObject*>& InObjects)
{
	...
	for(INT i=0; i<InObjects.Num(); i++)
	{
		...
		// @todo Would be nice to have FindSeqObjectsByObjectRef, to avoid potential ambiguity here (objects with same name but different outer).
		TArray<USequenceObject*> ReferringSeqObjs;
		RootSeq->FindSeqObjectsByObjectName(*Obj->GetName(), ReferringSeqObjs);

		for(INT j=0; j<ReferringSeqObjs.Num(); j++)
		{
			// Find the sequence that this SeqObj is in.
			USequence* ContainSeq = Cast<USequence>( ReferringSeqObjs(j)->GetOuter() );
			if(ContainSeq)
			{
...
static UBOOL ObjectIsValidForPrefab(UObject* InObj)
{
	...
	if( InObj &&								// Non-null
		...
		(!Actor || !Actor->IsABuilderBrush()) )	// Isn't a builder brush
	{
		return TRUE;
	}
	else
...
UPrefab* UUnrealEdEngine::Prefab_NewPrefab( TArray<UObject*> InObjects )
{
	...
	WxDlgNewArchetype dlg;
	int Result = dlg.ShowModal(PackageName, GroupName, PrefabName);
	if ( Result == wxID_CANCEL )
	{
		return NULL;
	}
	...
	if ( PrefabName.Len() > 0 && PackageName.Len() > 0 )
	{
		...
		if(PrefabSeq)
		{
			...
			const INT SeqAnswer = appMsgf( AMT_YesNoCancel, *LocalizeUnrealEd("Prefab_FoundSeq"), *PrefabSeq->GetName(), *PrefabSeq->GetName() );
		...
		for(INT i=0; i<AddObjects.Num(); i++)
		{
			...
			if(BrushInst)
			{
				...
				csgCopyBrush(BrushArchetype, BrushInst, BrushInst->PolyFlags, 0, FALSE, FALSE);
			}
		...
		if ( InstanceToArchetypeMap.Num() > 0 )
		{
			...
			UPrefab::ResolveInterDependencies(InstanceToArchetypeMap, true);
		}
...
{
#if GEMINI_TODO
	UTextureRenderTarget2D* RendTarget = GetScratchRenderTarget();

	// Create the scene capture probe
	FSceneCaptureProbe2D CaptureProbe;
	CaptureProbe.TextureTarget = RendTarget;
	CaptureProbe.bNeedsCaptureUpdate = true;
	...
	if(Scene && Viewport)
	{
		...
		CaptureProbe.View = View;

		// Re-render the scene with the capture probe in it, then remove probe.
		Scene->AddSceneCaptureProbe(&CaptureProbe);
		Viewport->Draw();
		Scene->RemoveSceneCaptureProbe(&CaptureProbe);
	...
	*/
	...
	for(INT i=0; i<AddObjects.Num(); i++)
	{
		...
		if(!FindArcRef)
		{
			...
			{
				ABrush* BrushArchetype = CastChecked<ABrush>(NewArchetype);
				csgCopyBrush(BrushArchetype, NewBrush, NewBrush->PolyFlags, 0, FALSE, FALSE);
			}
	...
	UPrefab::ResolveInterDependencies(SelectedInstToArchMap, true);
	...
	else 
	{
		...
		if(PrefabSeq)
		{
			UBOOL bUseSeq = appMsgf( AMT_YesNo, *LocalizeUnrealEd("Prefab_FoundSeqYesNo"), *PrefabSeq->GetName(), *PrefabSeq->GetName() );
			if(bUseSeq)
			{
		...
		if(ObjInst)
		{
			...
			for(INT i=0; i<ObjToRename.Num(); i++)
			{
				...
				*/
				...
				// Make a new name for this object (to remove the Prefab name from the object name).
				FName NewObjName = MakeUniqueObjectName( Obj->GetOuter(), Obj->GetClass() );

				// Rename the object.
				Obj->Rename( *NewObjName.ToString(), Obj->GetOuter(), REN_ForceNoResetLoaders );
			}
		}
	}
...
void UUnrealEdEngine::edactAddPrefab()
{
	...
	if( Prefab )
	{
		...
		APrefabInstance* PrefabInstance = Prefab_InstancePrefab( Prefab, ClickLocation, FRotator(0,0,0) );
		if ( PrefabInstance )
		{
...
void UUnrealEdEngine::edactUpdatePrefabFromInstance()
{
	...
	{
		if ( !WarnAboutHiddenLevels( *LocalizeUnrealEd("UpdatePrefabHiddenLevelsQ") ) )
		{
			return;
		}
		...
		const UBOOL bProceed = appMsgf( AMT_YesNo, *LocalizeUnrealEd("Prefab_UpdatePrefabAreYouSure"), *PrefabInstance->TemplatePrefab->GetName() );
		if(!bProceed)
		{
			return;
		}
