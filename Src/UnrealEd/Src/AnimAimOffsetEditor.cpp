...
static const FLOAT RotateSpeed = 10.f;
...
UBOOL UAnimNodeEditInfo_AimOffset::ShouldDrawWidget()
{
	if( EditWindow && EditNode && EditNode->CurrentProfileIndex < EditNode->Profiles.Num() )		
	{
		...
		if( EditWindow->EditBoneIndex < P.AimComponents.Num() )
		{
			return TRUE;
		}
	}
...
BEGIN_EVENT_TABLE( WxAnimAimOffsetEditor, wxFrame )
...
: wxFrame( InEditor, InID, TEXT("AimOffset Editor"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE | wxFRAME_FLOAT_ON_PARENT | wxFRAME_NO_TASKBAR )
{
...
void WxAnimAimOffsetEditor::OnRemoveBone( wxCommandEvent& In )
{
	...
	if( EditBoneIndex < P.AimComponents.Num() )
	{
		// Remove AimComponent
...
void WxAnimAimOffsetEditor::OnSelectBone( wxCommandEvent& In )
{
	...
	check(EditBoneIndex < P.AimComponents.Num());
...
{
	WxFileDialog dlg( GApp->EditorFrame, *LocalizeUnrealEd("Open"), *GApp->LastDir[LD_GENERIC_SAVE], TEXT(""), TEXT("AimOffsetProfile files (*.aop)|*.aop|All files (*.*)|*.*"), wxOPEN | wxFILE_MUST_EXIST | wxHIDE_READONLY );
...
void WxAnimAimOffsetEditor::OnSaveProfile( wxCommandEvent& In )
{
	...
	WxFileDialog dlg(GApp->EditorFrame, *LocalizeUnrealEd("Save"),  *GApp->LastDir[LD_GENERIC_SAVE], *DefaultFileName, TEXT("AimOffsetProfile files (*.aop)|*.aop|All files (*.*)|*.*"), wxSAVE | wxHIDE_READONLY | wxOVERWRITE_PROMPT );
	...
	GConfig->SetString(ProfileSectionName, TEXT("ProfileName"), P.ProfileName.GetName(), *FileName);
	...
	// Anim Names
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_LU"), P.AnimName_LU.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_LC"), P.AnimName_LC.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_LD"), P.AnimName_LD.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_CU"), P.AnimName_CU.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_CC"), P.AnimName_CC.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_CD"), P.AnimName_CD.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_RU"), P.AnimName_RU.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_RC"), P.AnimName_RC.GetName(), *FileName);
	GConfig->SetString(ProfileSectionName, TEXT("AnimName_RD"), P.AnimName_RD.GetName(), *FileName);
	...
	for(INT i=0; i<P.AimComponents.Num(); i++)
	{
		...
		const TCHAR* SectionName = AimComp.BoneName.GetName();
...
void WxAnimAimOffsetEditor::OnDelProfile( wxCommandEvent& In )
{
	...
	FAimOffsetProfile& P = EditInfo->EditNode->Profiles(EditInfo->EditNode->CurrentProfileIndex);
	UBOOL bDoIt = appMsgf( AMT_YesNo, *LocalizeUnrealEd("AOE_DelAreYouSure"), *P.ProfileName.ToString() );
	if(bDoIt)
	{
