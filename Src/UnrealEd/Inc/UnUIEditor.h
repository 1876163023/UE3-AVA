...
{
	DECLARE_DYNAMIC_CLASS( WxUIEditorBase )

	/** 
	 * 오직 DECLARE_DYNAMIC_CLASS를 위해 만든 default constructor.
	 * protected이기 때문에 default constructor로 WxUIEditorBase를 생성할 수 없다 
	 */
protected:
	WxUIEditorBase() :
	 wxFrame()
...
public:
	...
	virtual void OpenMatinee(USeqAct_Interp* Interp);
...
/**
 * This dialog allows the user to edit the docking set for a widget in the scene.  It contains combo boxes which allow the user to choose a docking
 * target and target face for each face of the widget from the list of widgets contained within the specified container.
 */
class WxDlgDockingEditor : public wxDialog, public FSerializableObject
{
...
public:
	...
	wxSpinCtrl*		spin_DockPadding[UIFACE_MAX];
