...
public:
	...
	/**
	 * The different ways that referenced assets can be preesented
	 */
	enum EReferencedAssetViewMode
	{
		/** display assets in a multi-column list view, along with additional data about the object */
		RAV_List,

		/** display assets as thumbnails, similar to the GB */
		RAV_Thumbnail,
	};
	...
	class FFindAssetsArchive : public FArchive
	{
		...
		*/
		 ...
		 */
		UBOOL ShouldSearchForAssets(UObject* Object,
	...
	public:
		 ...
		 */
		FFindAssetsArchive( UObject* InSearch, TArray<UClass*> IgnoreClasses, const TArray<UObject*>& IgnorePackages, TArray<UObject*>& OutAssetList,
			ObjectReferenceGraph* ReferenceGraph=NULL, INT MaxRecursion=0, UBOOL bIncludeClasses=TRUE, UBOOL bIncludeDefaults=FALSE );
...
protected:
	 ...
	 */
	EReferencedAssetViewMode ViewMode;
	/**
	...
	/** this splitter is never seen; it is used to easily switch between thumbnail and list view */
	wxSplitterWindow* ViewModeSplitter;
...
public:
	 ...
	 */
	void SetViewMode( EReferencedAssetViewMode InViewMode );
	 ...
	 * scrolling.
	 */
	UBOOL InputKey(FViewport* Viewport,INT ControllerId,FName Key,EInputEvent Event,FLOAT AmountDepressed = 1.f,UBOOL bGamepad = FALSE);

	/**
	 * Updates the scrolling location based on the mouse movement
	 */
	UBOOL InputAxis(FViewport* Viewport,INT ControllerId,FName Key,FLOAT Delta,FLOAT DeltaTime);
...
private:
	...
	void UI_ViewMode( wxUpdateUIEvent& Event );
