...
/**
* 이미지, 텍스트 효과들을 설정하기위한 패널
* 페이드 시간, 페이드 커브, 스케일링 시간, 스케일링 커브(수평, 수직)을 설정할 수 있도록 한다
* @TODO : Preview
*/
class WxEffectAdjustmentPanel: public wxPanel
{
	DECLARE_DYNAMIC_CLASS(WxEffectAdjustmentPanel)
	DECLARE_EVENT_TABLE()
...
protected:
	...
	*/
	...
	/** Owner style property group panel used for notification of effect changes */
	WxStylePropertiesGroup*		OwnerPropertyGroup;

//	wxStaticText*				lbl_FadeAdjust;		// CreateControl에서 생성
	WxUIChoice*					cmb_FadeType;			
	WxUITextCtrl*				txt_FadeDuration;
	WxUITextCtrl*				txt_FadeCurve;
	wxBitmapButton*				btn_FadeCurveAssign;
	wxBitmapButton*				btn_FadeCurveBrowse;
	wxBitmapButton*				btn_FadeCurveDelete;

//	wxStaticText*				lbl_ScaleAdjust;	// CreateControl에서 생성
	WxUITextCtrl*				txt_ScaleAxisX;
	WxUITextCtrl*				txt_ScaleAxisY;
	WxUITextCtrl*				txt_ScaleCurveHorz;
	WxUITextCtrl*				txt_ScaleCurveVert;
	WxUITextCtrl*				txt_ScaleDuration;
	wxBitmapButton*				btn_ScaleCurveHorzAssign;
	wxBitmapButton*				btn_ScaleCurveVertAssign;
	wxBitmapButton*				btn_ScaleCurveHorzBrowse;
	wxBitmapButton*				btn_ScaleCurveVertBrowse;
	wxBitmapButton*				btn_ScaleCurveHorzDelete;
	wxBitmapButton*				btn_ScaleCurveVertDelete;
...
private:
	void OnClick_UseSelected( wxCommandEvent& Event );
	void OnClick_DelSelected( wxCommandEvent& Event );
	void UpdatePresetCurveProperty( int WidgetID, UCurveEdPresetCurve* PresetCurve );

	void OnText_ScaleAxis( wxCommandEvent& Event );
	void OnText_ScaleDuration( wxCommandEvent& Event );
	void OnText_FadeDuration( wxCommandEvent& Event );
	void OnCombo_FadeType( wxCommandEvent& Event );

	void UpdateScaleAxis( BYTE Orient, FLOAT AxisX );
	void UpdateScaleDuration( FLOAT ScaleDuration );
	void UpdateFadeDuration( FLOAT FadeDuration );
	void UpdateFadeType( BYTE FadeType  );
};
...
public:
	...
	: cmb_Font(NULL), chklst_Attributes(NULL), pnl_Preview(NULL), pnl_Color(NULL), pnl_Effect(NULL)
	{
	}
...
private:
	...
	/** 이펙트를 설정하기 위한 패널 */
	WxEffectAdjustmentPanel*	pnl_Effect;
...
public:
	WxStyleImagePropertiesGroup(): pnl_Color(NULL), pnl_DefaultTexture(NULL),pnl_Preview(NULL),pnl_Effect(NULL){}
