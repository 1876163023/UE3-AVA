...
struct HWidgetUtilProxy : public HHitProxy
{
	...
	HWidgetUtilProxy(INT InInfo1, INT InInfo2, EAxis InAxis, const FMatrix& InWidgetMatrix, UBOOL bInRotWidget):
	HHitProxy(HPP_UI),
		Info1(InInfo1),
		Info2(InInfo2),
		Axis(InAxis),
		WidgetMatrix(InWidgetMatrix),
		bRotWidget(bInRotWidget)
	{}
...
{
public:
	static void DrawWidget(const FSceneView* View,FPrimitiveDrawInterface* PDI, const FMatrix& WidgetMatrix, INT InInfo1, INT InInfo2, EAxis HighlightAxis, UBOOL bRotWidget);
};
