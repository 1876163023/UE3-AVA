...
$developer_root_dir                      = 'D:\\depot_epicgames\\UnrealEngine3-Gemini\\';
$developer_buildLogDir                   = 'D:\\depot_epicgames\\UnrealEngine3-Gemini\\BuildLogs\\';
...
$doBuildPreCompiledShaders = 0;    # Compile precompiled shaders
...
$doBuildXenon	     = 0;    # Build Xenon?  Effectively 0 if doBuild is 0.
$doBuildXenonLTCG    = 0;    # build the xenon LTCG?  Effectively 0 if doBuild is 0.
$xenonXDKVersion     = 4314; # the version of the Xenon XDK

$doPreCompileShadersExample = 0;
$doPreCompileShadersUT = 0;
$doPreCompileShadersWarfare = 0;
$doPreCompileShadersGearGame = 0;
$doPreCompileShadersFullRebuild = 0;
...
$doUE3Docs           = 1;    # Generate UE3 documentation?  Effectively 0 if doDocs is 0.
$doXenonDocs         = 1;    # Generate Xenon-specific documentation?  Effectively 0 if doDocs is 0.
...
$root_dir					= 'E:\\Build_UE3\\UnrealEngine3-Gemini\\';

$buildLogDir				= 'E:\\Build_UE3\\BuildLogs\\';                           # Dir to dump log files into
$buildLogLink               = '\\\\Build-Server\\BuildLogs\\';                        # Link to the log file dir.
$buildSyncBatchLink         = '\\\\Build-Server\\Builds\\SyncToUE3Builds\\';          # Link to the sync batch file dir.
$avarisBuildFile            = '\\\\Build-Server\\BuildFlags\\makeUC2build.txt';       # Link to the file that triggers Avaris builds.
...
$buildclientspec  = 'UnrealEngine3Build';        # The clientspec who's changes are searched for the last build.  Never changes, even in dev.
#$cspecpath        = 'E:\\Build_UE3\\UnrealEngine3';							
$owner            = 'build_machine';
$depot            = '//depot/UnrealEngine3-Gemini/';
...
$addrWarfareInternal   = 'warfare-list@epicgames.com';
$addrWarfareExternal   = 'warfare-external-list@epicgames.com';

$addrEnvyInternal      = 'envy-list@epicgames.com';
$addrEnvyExternal      = 'envy-external-list@epicgames.com';

$addrTesters                    = 'scott.bigwood@epicgames.com';
...
$failSubject           = "UE3 Gemini BUILD FAILED";
...
@buildGames = ('War','UT','Example');	# List of games to build.  Consulted if buildAllGames = 0.
$buildAllGames = 1;								# Flag to indicate if all builds should be compiled.  Overrides buildGames list.
...
sub setVersion
{
	...
	$version = sprintf( "UnrealEngine3-Gemini_Build_[%04d-%02d-%02d_%02d.%02d]", $year + 1900, $mon + 1, $mday, $hour, $min );

	$announceSubject = "New UnrealEngine3-Gemini version in P4: $version";
...
sub setAnnounceMailHeaderText
{
  ...
  $announceMailHeaderText = 
    ...
    "\n$buildSyncBatchFile\n\n".
    "This build was labeled as: ".'@'."latestUnrealEngine3-Gemini\n\n".
...
sub getCheckoutListPreCompiledShaders
{
	# Declare temporaries.
	my @checkoutPotentials;
	my @checkoutExcludePotentials;
	...
	$checkoutfilelistfilePreCompiledShaders		= $root_dir.'Development\\Build\\BuildFileListPreCompiledShaders.txt';		# List of files to delete and build - included in P4.
	$checkoutfileexcludefilePreCompiledShaders	= $root_dir.'Development\\Build\\BuildFileExcludeListPreCompiledShaders.txt';	# List of files to exclude from build - included in P4.

	if ( open( CHECKOUTFILELIST, $checkoutfilelistfilePreCompiledShaders ) )
	{
		# Grab the list of include paths.
		chomp( @checkoutPotentials = <CHECKOUTFILELIST> );     # Strip trailing newlines.
		close( CHECKOUTFILELIST );
		...
		# Grab the list of exclude paths.
		if ( open( CHECKOUTFILEEXCLUDE, $checkoutfileexcludefilePreCompiledShaders ) )
		{
			chomp( @checkoutExcludePotentials = <CHECKOUTFILEEXCLUDE> );
			close( CHECKOUTFILEEXCLUDE );
		}
...
sub checkoutAll
{
	...
	else
	{
        ...
        #here we want to check to see if we should delete all of the RefShaderCache files or not

	if( $doPreCompileShadersFullRebuild  == 1 )
	{
	    foreach $file (@checkoutFiles)
	    {
	        system( "del $file" );
...
##############
# This will compile each of the various platforms' reference shadercache.  It will call the commandlet to do the
# precompiling and report any errors.
#
#precompileshaders platform=xenon -refcache
#precompileshaders platform=pc -refcache
#precompileshaders platform=ps3 -refcache -paths=runbaked
#
############
sub doPreCompiledShadersWork
{
  if( $doPreCompileShadersExample == 1 )
  {
      clock "PRECOMPILESHADERS_EXAMPLE";

      doPreCompilerShaderCommandlet( $root_dir."ExampleGame\\".$script_dir."Logs\\Launch.log", $root_dir."\\Development\\Build\\PreCompiledShaders.bat ExampleGame" );

      unclock "PRECOMPILESHADERS_EXAMPLE";
  }

  if( $doPreCompileShadersUT == 1 )
  {
      clock "PRECOMPILESHADERS_UT";

      doPreCompilerShaderCommandlet( $root_dir."UTGame\\".$script_dir."Logs\\Launch.log", $root_dir."\\Development\\Build\\PreCompiledShaders.bat UTGame" );

      unclock "PRECOMPILESHADERS_UT";
  }

  if( $doPreCompileShadersWarfare == 1 )
  {
      clock "PRECOMPILESHADERS_WARFARE";

      doPreCompilerShaderCommandlet( $root_dir."WarGame\\".$script_dir."Logs\\Launch.log", $root_dir."\\Development\\Build\\PreCompiledShaders.bat WarGame" );

      unclock "PRECOMPILESHADERS_WARFARE";
  }

  if( $doPreCompileShadersGearGame == 1 )
  {
      clock "PRECOMPILESHADERS_GEARGAME";

      doPreCompilerShaderCommandlet( $root_dir."GearGame\\".$script_dir."Logs\\Launch.log", $root_dir."\\Development\\Build\\PreCompiledShaders.bat GearGame" );

      unclock "PRECOMPILESHADERS_GEARGAME";
  }
...
sub createLabels
{
	# Unique version labels are no longer created.
	if( 0 )
	{
	...
	{
	    # Create the latestUnrealEngine3-PCAndXenon label.
	    #system( " -c \"$clientspec\" label -d latestUnrealEngine3-PCAndXenon" );
	    open( LABELCMD, "p4 -c \"$clientspec\" label -i" );
	    print LABELCMD "Label:\tlatestUnrealEngine3-Gemini-PCAndXenon\n";
	    print LABELCMD "Owner:\t$owner\n";
	    print LABELCMD "Description:\n\t$version\n\nBuilt from changelist:  $builtFromChangelistNum\n";		
	    print LABELCMD "Options:\tunlocked\n";
	    print LABELCMD "View:\n\t\"$depot\...\"\n";
	    close( LABELCMD );
	    system( "p4 -c \"$clientspec\" labelsync -l latestUnrealEngine3-Gemini-PCAndXenon" );
	}
	...
	open( LABELCMD, "p4 -c \"$clientspec\" label -i" );
	print LABELCMD "Label:\tlatestUnrealEngine3-Gemini\n";
	print LABELCMD "Owner:\t$owner\n";
	print LABELCMD "Description:\n\t$version\n\nBuilt from changelist:  $builtFromChangelistNum\n";		
	print LABELCMD "Options:\tunlocked\n";
	print LABELCMD "View:\n\t\"$depot\...\"\n";
	close( LABELCMD );
	system( "p4 -c \"$clientspec\" labelsync -l latestUnrealEngine3-Gemini" );
}
...
sub assembleChangesSinceLastBuild
{
	...
	foreach $line (<CHANGES>)
	{
		...
		if ( $line =~ /^Change ([0-9]+) on ([0-9]{4})\/([0-9]{2})\/([0-9]{2}).+UnrealEngine3-Gemini_Build_/ )
		{
			$lastbuildchange = $1;
			...
			{
				if ($line =~ m/UnrealEngine3-Gemini_Build_\[([0-9]{4})-([0-9]{2})-([0-9]{2})_([0-9]{2})\.([0-9]{2})\]/)
				{
...
sub announceWarfareChanges
{
	...
	while ($success == 0 && $attempts < 10)
	{
		open( SMTP, "|\"$blat\" - -t $addrWarfareExternal -f $addrFrom -s \"$announceSubjectWarfare\" -server $mailserver > output.txt" );
...
sub announceEnvyChanges
{
	...
	while ($success == 0 && $attempts < 10)
	{
		open( SMTP, "|\"$blat\" - -t $addrEnvyExternal -f $addrFrom -s \"$announceSubjectEnvy\" -server $mailserver > output.txt" );
...
sub announceBuildSuccessPreCompiledShaders
{
    local $gameName = $_[0];
    local $mailText = $gameName." Gemini Ref Shader Cache has been updated";
	...
	while ($success == 0 && $attempts < 10)
	{
	    ...
	    open( SMTP, "|\"$blat\" - -t $addrAnnounceChangeLogToCoders -f $addrFrom -s \"$mailText\" -server $mailserver > output.txt" );
	    print SMTP "$mailText";
	    close(SMTP);

	    open( SMTP, "|\"$blat\" - -t $addrEnvyExternal -f $addrFrom -s \"$mailText\" -server $mailserver > output.txt" );
	    print SMTP "$mailText";
	    close(SMTP);

	    open( SMTP, "|\"$blat\" - -t $addrWarfareExternal -f $addrFrom -s \"$mailText\" -server $mailserver > output.txt" );
	    print SMTP "$mailText";
	    close(SMTP);
...
sub doPreCompilerShaderCommandlet
{
    local $PCSLog = $_[0];
    local $commandToRun = $_[1];

    # Remove log file.
    system( "del ".$PCSLog." /f" );
    system( $commandToRun );

    # check the log from the PreCompiledShaders for errors
    #local $compilelog = compileScript( $game );
    #if( parsePreCompiledShadersLog( $PCSLog ) == 0 )
    if( 0 )
    {
	# Script compile failed.  Copy the log over the failed log directory.
	system( "copy $PCSLog $failedLogDir /y" );
	
	# Unlock if we did a checkout.
	revertAll();
	
	announceFailure( "PreCompiledShaders ($game) failed", $failedLogDirLink."Launch.log" );
	return;
    }
}
...
sub parsePreCompiledShadersLog
{
	local $log_file = $_[0];
	#print "parseScriptLog: parsing $log_file\n";
...
sub compileScript
{
	...
	# Compile.
	system( $systemdir."\\".$build_game."Game.com make -full -FINAL_RELEASE -unattended LOG=Launch.log");
...
sub setDeveloper
{
	...
	$qaBuildInfoFile				= '\\\\build-server\\BuildFlags\\QABuildInfoCandidateToBeSignedOffOn.ini';	# text .ini file containing the version QA version info
...
sub oldDocumentationNag
{
	...
	if ( 0 ) {
		...
		foreach $line (@sortedDoxytext)
		{
			...
			{
				@fields = split / /, $line;
				$swappedRoot = 'E:/Build_UE3/UnrealEngine3-Gemini/';
...
sub dobuild
{
	...
	if ($doCheckout == 1)
	{
		...
		if ( !getCheckoutList() )
		{
			...
			print "UE3 build failed: Couldn't read checkout list from $checkoutfilelistfile.\n\n";
			return;
		}
	...
	if ( !assembleChangesSinceLastBuild(0) )
	{
		...
		print "UE3 build failed: Couldn't find last build version.\n\n";
	...
	if( $doAQABuild == 1 )
	{
	    ...
	    open( QA_BEING_VETTED_FILE, ">\\\\build-server\\BuildFlags\\QABuildIsBeingVetted.txt" );
	...
	if ($uploadSymbols == 1)
	{
		...
		system("store_symbols $version $version");
...
sub doPreCompiledShaders
{
	...
	if( $doCheckout == 1 )
	{
	    ...
	    # Retrieve the checkout list from file.
	    if ( !getCheckoutListPreCompiledShaders() )
	    {
	        announceFailure("Couldn't get checkout list; opening $checkoutfilelistfilePreCompiledShaders  for read", "<no log file>");
		print "UE3 PreCompileShaders failed: Couldn't read checkout list from $checkoutfilelistfilePreCompiledShaders.\n\n";
		return;
	    }
	...
	# now do the actual precompiling work
	doPreCompiledShadersWork();
	...
	if( $doSubmit == 1 )
	{
	    ...
	    if( submitAll() == 0 )
	    {
	        print "UE3 PreCompileShaders failed: Perforce submit failed.  Reverting.\n\n";
		revertAll();
		return;
	      }
	...
	if( $doPreCompileShadersExample == 1 )
	{
	  announceBuildSuccessPreCompiledShaders( "Example" );
	}

	if( $doPreCompileShadersUT == 1 )
	{
	  announceBuildSuccessPreCompiledShaders( "UT" );
	}

	if( $doPreCompileShadersWarfare == 1 )
	{
	  announceBuildSuccessPreCompiledShaders( "Warfare" );
	}

	if( $doPreCompileShadersGearGame == 1 )
	{
	  announceBuildSuccessPreCompiledShaders( "GearGame" );
	}
...
sub parseCommandLineArgs
{
	...
	#	  setAnnounceMailHeaderText;
		...
		elsif( $curArg eq 'doBuildPreCompiledShaders_Full' )
		{
		  $doPreCompileShadersFullRebuild = 1;

		    $root_dir = 'C:\\Build_UE3\\UnrealEngine3\\';
		    $buildLogDir = 'C:\\Build_UE3\\BuildLogs\\';
		    $clientspec = 'REFSHADER-FARM_UE3';
		    $owner = 'build_machine';

		    $doBuildPreCompiledShaders = 1;
		    $doPreCompileShadersExample = 1;
		    $doPreCompileShadersUT = 1;
		    $doPreCompileShadersWarfare = 1;
		    $doPreCompileShadersGearGame = 1;
		    print "   Doing a  doBuildPreCompiledShaders.\n";

		}
		elsif( $curArg eq 'doBuildPreCompiledShaders_All' )
		{
		    $root_dir = 'C:\\Build_UE3\\UnrealEngine3\\';
		    $buildLogDir = 'C:\\Build_UE3\\BuildLogs\\';
		    $clientspec = 'REFSHADER-FARM_UE3';
		...
		elsif( $curArg eq 'doBuildPreCompiledShaders_Example' )
		{
		    $root_dir = 'C:\\Build_UE3\\UnrealEngine3\\';
		    $buildLogDir = 'C:\\Build_UE3\\BuildLogs\\';
		    $clientspec = 'REFSHADER-FARM_UE3';
		...
		elsif( $curArg eq 'doBuildPreCompiledShaders_UT' )
		{
		    $root_dir = 'C:\\Build_UE3\\UnrealEngine3\\';
		    $buildLogDir = 'C:\\Build_UE3\\BuildLogs\\';
		    $clientspec = 'REFSHADER-FARM_UE3';
		...
		elsif( $curArg eq 'doBuildPreCompiledShaders_Warfare' )
		{
		    $root_dir = 'C:\\Build_UE3\\UnrealEngine3\\';
		    $buildLogDir = 'C:\\Build_UE3\\BuildLogs\\';
		    $clientspec = 'REFSHADER-FARM_UE3';
...
# Don't allow any UE3-Gemini builds if Avaris is in the middle of a build
if( ( -e $avarisBuildFile )
  )
{
	print "Avaris or UE3 build file exists!\n";
	announceFailure( "Avaris or UE3 build currently underway; build", "<no log file>" );
	print "Avaris or UE3 build currently underway; build refused\n\n";
	exit( 0 );
}
else
{
	print "Can't see Avaris or UE3 build file\nProceeding with build\n";
	print $lineSpacer;
}
...
if( $doBuildPreCompiledShaders == 1 )
{
    print "rocking PCS\n";
    doPreCompiledShaders;
}
else
{
    dobuild;
