...
$doBuildPCLTCG	     = 1;    # Build PC LTCG?  Effectively 0 if doBuild is 0.
...
$xenonXDKVersion     = 4929; # the version of the Xenon XDK that we are using

$doBuildPS3          = 1;    # build PS3? effectively 0 if doBuild is 0
$PS3SDKVersion       = "110.006"; # the version of the PS3 SDK that we are using
...
$doDocs              = 1;    # Generate documentation?
...
$doSourceServer       = 1;	 # modify .pdb files with src revisions for source server usage
...
$addrWarfareInternal   = 'warfare-list@epicgames.com'; #'QA@epicgames.com'; # 'warfare-list@epicgames.com';
$addrWarfareExternal   = 'warfare-external-list@epicgames.com'; #'wrbldnts@microsoft.com';  # 'warfare-external-list@epicgames.com';

$addrEnvyInternal      = 'QA@epicgames.com'; # 'envy-list@epicgames.com'; # 'QA@epicgames.com';
$addrEnvyExternal      = 'QA@epicgames.com'; # 'envy-external-list@epicgames.com'; # 'QA@epicgames.com';
...
@buildGames = ('War','UT','Example','Gear');	# List of games to build.  Consulted if buildAllGames = 0.
...
sub setVersion
{
	...
	$version = sprintf( "UnrealEngine3_Build_[$versionTimeStamp]" );
...
sub setAnnounceMailHeaderText
{
  ...
  $announceMailHeaderText = 
    ...
    "\n$buildSyncBatchFile\n\n".
    "This build was labeled as: ".'@'."latestUnrealEngine3\n\n".
...
sub createLabels
{
	  ...
	  )
	{
	    ...
	    open( LABELCMD, "p4 -c \"$clientspec\" label -i" );
	    print LABELCMD "Label:\tlatestUnrealEngine3-PCAndXenon\n";
	    print LABELCMD "Owner:\t$owner\n";
	    print LABELCMD "Description:\n\t$version\n\nBuilt from changelist:  $builtFromChangelistNum\n";		
	    print LABELCMD "Options:\tunlocked\n";
	    print LABELCMD "View:\n\t\"$depot\...\"\n";
	    close( LABELCMD );
	    system( "p4 -c \"$clientspec\" labelsync -l latestUnrealEngine3-PCAndXenon" );
	}
	...
	foreach $change ( @depotChanges )
	{
		...
		if ($change =~ /Change ([0-9]+) on/)
		{
			...
			foreach $line (<DESCRIBE>)
			{
				...
				{
					#print "saw a line containing #GearGame\n";
					$reading_project = 5;
				}
...
sub announceToQAPeopleThatABuildIsBuilding
{
	...
	while ($success == 0 && $attempts < 10)
	{
		# Open a mail pipe.  The pipe is redirected to output.txt so that we can
		# check for success by testing for the presence of output.txt
		open( SMTP, "|\"$blat\" - -t $addrTesters -f $addrFrom -s \"New Build has been triggered\" -server $mailserver > output.txt" );

		print SMTP "New Build has been triggered\n\n";
...
sub compilePS3
{
    ...
    system( "$systemdir\\PS3\\buildPS3.bat $root_dir\\Development\\Src\\PS3 $buildlog ".$buildGame." release $systemdir\\PS3\\" );

    
#    system( "$systemdir\\PS3\\setPS3EnvVars.bat" );
#    system( "pushd $root_dir\\Development\\Src\\PS3" );
#    system( "make $root_dir\\Development\\Src\\PS3\\makefile GAMENAME=".$buildGame." BUILDTYPE=release " ); # \> $buildlog" );
#    system( "popd" );

#   my $runString = "make $root_dir\\Development\\Src\\PS3\\makefile GAMENAME=".$buildGame." BUILDTYPE=release > $buildlog";
#    print "- Building Example Game and UT Game for PS3".$runString."\n";
...
sub oldDocumentationNag
{
	...
	if ( 0 ) {
		...
		foreach $line (@sortedDoxytext)
		{
			...
			{
				@fields = split / /, $line;
				$swappedRoot = 'E:/Build_UE3/UnrealEngine3/';
...
sub dobuild
{
	...
	if ( $doBuild == 1 )
	{
		...
		if( $doBuildPS3 == 1 )
		{
		    clock "BUILD CPP FOR PS3 RELEASE";

		    $compilelog = compilePS3( EXAMPLEGAME );
		    print "PS3 compilelog is $compilelog\n";
		    if ( parseNativeLog( $compilelog ) == 0 )
		    {
			system( "copy $compilelog $failedLogDir /y" );
			...
			announceFailure( "PS3 ExampleGame compile", $failedLogLink );
			return;
		    }

		    $compilelog = compilePS3( UTGAME );
		    print "PS3 compilelog is $compilelog\n";
		    if ( parseNativeLog( $compilelog ) == 0 )
		    {
			system( "copy $compilelog $failedLogDir /y" );
			...
			announceFailure( "PS3 UTGame compile", $failedLogLink );
			return;
		    }

		    unclock "BUILD CPP FOR PS3 RELEASE";
		}
	}
	...
	if( $doSourceServer == 1 )
	{
	    clock "SOURCE SERVER UPDATING";
	    system( "P4Index.cmd" ); # this will do all of the source server file updating and modifying the .pdb.  This will miss the UnObjVer.cpp and other .xml files we have checked out.  The issue is that we need to have checked those in before we can get their revisions but the .pdb files be locked by that point.  All of those files just have "version" info in them so we will be off by one on them but that should not be what we are trying to debug / fix.
	    unclock "SOURCE SERVER UPDATING";
	}
	...
	if ($uploadSymbols == 1)
	{
		...
		system("store_symbols $builtFromChangelistNum $version");
...
sub parseCommandLineArgs
{
	...
	foreach $argnum (0 .. $ARGV )
	{
		...
		elsif( $curArg eq 'testPS3' )
		{
	#	    $doAQABuild = 1;
		  print "testing ps3 build\n";
		  setDeveloper;
		  setVersion;
		  compilePS3( "EXAMPLEGAME" );
