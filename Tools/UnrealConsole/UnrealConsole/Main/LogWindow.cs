...
namespace UnrealConsole
{
	...
	public class FLogWindow
	{
		...
		/// </summary>
		/// <param name="Text">The test to display.</param>
		private void InternalPrint(string Text)
		{
			LastPrint = Text;
		...
		{
			// Make Print() thread safe by marshalling the print into the UI thread
			TTYOutput.BeginInvoke(new PrintDelegate(this.InternalPrint), Text);
		}
        ...
        public void SetPlatformName(string InName)
        {
            PlatformName = InName;
        }
        ...
        {
            int ConsoleState = UnrealConsole.DLLInterface.GetConsoleState(UnrealConsole.ConnectedTargetIndex);
            if (ConsoleState == (int)EConsoleState.CS_Crashed || ConsoleState == (int)EConsoleState.CS_Asserted)
            {
                HandleCrash(LastPrint);
            }
            else
            {
                ReportGuard = false;
        ...
        public void HandleCrash(string AssertMessage)
        {
            Print("\n\n\nDetected a crash:\n");
            ...
            if (UnrealConsole.DLLInterface.GetCallstack(UnrealConsole.ConnectedTargetIndex, Callstack))
            {
                ...
                {
                    string TranslatedCallstack = "";
                    // go voer it, and convert the callstack to readable addresses
                    foreach (UInt32 Address in Callstack)
                    {
                        TranslatedCallstack += UnrealConsole.DLLInterface.ResolveAddressToString(Address) + "\n";
                    }

                    Print(TranslatedCallstack);

                    //don't submit two reports back-to-back, which avoids multiple reports sent for PS3 crashes
                    if (!ReportGuard)
                    {
                        CrashReporter Reporter = new CrashReporter();
                        //fire up the autoreporter
                        string CrashResult = Reporter.SendCrashReport(AssertMessage, TranslatedCallstack, TTYOutput.Text, PlatformName);
                        //setup the guard so that the crash won't be submitted twice
                        ReportGuard = true;
                        Print("\n" + CrashResult);
                    }
