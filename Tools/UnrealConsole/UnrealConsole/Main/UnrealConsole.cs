...
namespace UnrealConsole
{
    ...
    public class UnrealConsole : System.Windows.Forms.Form
    {
		...
		private MenuItem Menu_EditFind;
		...
		{
			if(e.TabPage == TTYPage)
			{
				Menu_EditFind.Enabled = true;
			}
			else
			{
				Menu_EditFind.Enabled = false;
        ...
        private void InitializeComponent()
        {
			...
			this.MemoryText.BackColor = System.Drawing.SystemColors.Window;
			...
			this.TTYPage.UseVisualStyleBackColor = true;
			// 
			// TTYInput
			// 
			...
			this.TTYText.BorderStyle = System.Windows.Forms.BorderStyle.None;
			...
			// ScreenPage
			// 
			this.ScreenPage.Controls.Add(this.ScreenCaptureBox);
			...
			this.ScreenPage.Size = new System.Drawing.Size(1276, 716);
		...
		/// </summary>
		/// <param name="Data">A pointer to the ASCII data to be displayed.</param>
		/// <param name="Length">The number of characters in the ASCII string pointed to by <see cref="Data"/></param>
		unsafe void OnTTYEventCallback(IntPtr Data, int Length)
		{
			if(Length > 0)
			{
				string Str = new string((sbyte*)Data.ToPointer(), 0, Length);
...
//			Application.Exit();
        ...
        private void SetTarget(string NewPlatformName, string NewTargetName)
        {
	        ...
	        if (ConnectedTargetIndex != -1)
	        {
				...
				DLLInterface.SetTTYCallback(PlatformIndex, null);
	        ...
	        else if (NewPlatformName == null)
	        {
		        ...
		        for (int PlatformIndex = 0; PlatformIndex < DLLInterface.GetNumPlatforms() && PlatformName == null; PlatformIndex++)
		        {
                    ...
                    if (DLLInterface.ActivatePlatform(DLLInterface.GetPlatformName(PlatformIndex)))
                    {
			...
			int PlatformIdentifier = -1;
	        if (DLLInterface.ActivatePlatform(NewPlatformName))
	        {
				// get the index for the platform
				PlatformIdentifier = DLLInterface.GetPlatformIndex(NewPlatformName);

				DLLInterface.SetTTYCallback(PlatformIdentifier, new TTYEventCallbackDelegate(this.OnTTYEventCallback));
	        ...
	        if (ConnectedTargetIndex == -1)
	        {
				if(PlatformIdentifier != -1)
				{
					DLLInterface.SetTTYCallback(PlatformIdentifier, null);
				}
	        ...
	        else
	        {
                ...
                LogWindow.SetPlatformName(PlatformName);
				...
				LastUsedSettings.LastTargets[PlatformIdentifier] = TargetName;
        ...
        private bool Connect()
        {
	        ...
	        if (Connection != null)
	        {
		        ...
		        if (bOk)
		        {
			        ...
			        // tell console to start tracking memory
...
//			bLoadedOk = false;
        ...
        private void IDM_SCREENCAPTURE_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Screenshot = Image.FromFile("TempScreenshot.bmp");
            }
            catch (Exception)
            {
                MessageBox.Show("Failed to open screenshot. Target may have failed to write out a file");
                return;
            }
		...
		private void Menu_EditFind_Click(object sender, EventArgs e)
		{
			if(!FindDlg.Visible)
			{
				if(FindDlg.IsDisposed)
				{
					FindDlg = new FindDialog();
				}

				FindDlg.TextBox = TTYText;
				FindDlg.Show(this);
			}
		}
    }
