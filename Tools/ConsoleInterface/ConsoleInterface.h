...
public __delegate void TTYEventCallbackDelegate(IntPtr Data, int Length);
public __delegate void SetTTYEventCallbackDelegate([MarshalAs(UnmanagedType::FunctionPtr)]TTYEventCallbackDelegate *Callback);
...
public:
	...
	*/
	 ...
	 */
	void ResolveAddressToSymboInfo(UInt32 Address, String*& OutFilename, String*& OutFunction, Int32& OutLine );
	 ...
	 */
	void SetTTYCallback(int PlatformIndex, TTYEventCallbackDelegate *Callback);
...
private:
	...
	/** 
	 * This delegate must be held onto because delegates can be garbage collected which removes the native __stdcall to managed  __clrcall thunk. This results in stack corruption and other pleasantries
	 * when the function is called from native code.
	 */
	TTYEventCallbackDelegate *TTYEventCallback;
};
