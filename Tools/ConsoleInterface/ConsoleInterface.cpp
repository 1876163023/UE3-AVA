...
FPlatformInfo KnownPlatforms[NUM_PLATFORMS] = 
{ 
	{ "PS3", "PlayStation3", "PS3\\PS3Tools.dll" } ,
	{ "PS3 Mod", "PlayStation3 Mod", "PS3\\PS3ModTools.dll" } ,
	{ "Xbox360", "Xenon", "Xenon\\XeTools.dll" } ,
	{ "PC", "Windows", "Windows\\WindowsTools.dll" } , // @note: PC is expected to be first
};
...
String* DLLInterface::GetFriendlyPlatformName(String* PlatformName)
{
	...
	if (PlatformIndex == -1)
	{
		// if not found, then return what was passed in
		return PlatformName;
	}
...
bool DLLInterface::ActivatePlatform(String* PlatformName)
{
	...
	else
	{
		...
		// did something fail during setup?
		if (CurrentConsoleSupport == NULL)
		{
			// unload the DLL
			FreeLibrary(CachedData.DLLHandle);
			CachedData.DLLHandle = NULL;
		}
	}
...
/**
 * This function sets the TTY output callback function for managed code.
 *
 * @param	Callback		The managed function pointer (delegate) to the callback.
 * @param	PlatformIndex	The index of the platform the callback is being set for.
 */
void DLLInterface::SetTTYCallback(int PlatformIndex, TTYEventCallbackDelegate *Callback)
{
	if(PlatformIndex >= 0 && PlatformIndex < NUM_PLATFORMS)
	{
		TTYEventCallback = Callback;
		// get the cached data object
		FCachedConsoleData& CachedData = CachedConsoleDLLData[PlatformIndex];
		void *SetCallbackPtr = GetProcAddress(CachedData.DLLHandle, "SetTTYCallBack");

		if(SetCallbackPtr)
		{
			SetTTYEventCallbackDelegate *SetCallbackDel = static_cast<SetTTYEventCallbackDelegate*>(Marshal::GetDelegateForFunctionPointer(IntPtr(SetCallbackPtr), __typeof(SetTTYEventCallbackDelegate)));
			SetCallbackDel(Callback);
		}
	}
}
 ...
 */
void DLLInterface::ResolveAddressToSymboInfo(UInt32 Address, String*& OutFilename, String*& OutFunction, Int32& OutLine )
{
	const int MaxSymbolLength = 512;

	wchar_t SymbolFilename[MaxSymbolLength+1];
	wchar_t SymbolFunction[MaxSymbolLength+1];
	int		SymbolLine;

	CurrentConsoleSupport->ResolveAddressToSymboInfo(Address, MaxSymbolLength, SymbolFilename, SymbolFunction, SymbolLine );

	OutFilename = SymbolFilename;
	OutFunction = SymbolFunction;
	OutLine		= SymbolLine;
}
