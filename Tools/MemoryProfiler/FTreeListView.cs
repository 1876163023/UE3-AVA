...
namespace MemoryProfiler
{
	...
	public class FTreeListView : Control
	{
		...
		private void DrawRows(Graphics Gfx)
		{
			...
			foreach(FTreeListViewItem Item in ItemCollection)
			{
				...
				if(Index >= StartIndex)
				{
					if(Item == SelectedRow)
					{
						DrawSelectedRow(Gfx, Item, XOff, YOff);
					}
					else
					{
						DrawRow(Gfx, Item, XOff, YOff);
					}
				...
				if(Item.Expanded)
				{
					DrawNodeRows(Gfx, Item, StartIndex, ClientHeight, XOff, ref YOff, ref Index);
				}
			}
		}
		 ...
		 */
		private void DrawNodeRows(Graphics Gfx, FTreeListViewItem Item, int StartIndex, int ClientHeight, float XOff, ref float YOff, ref int Index)
		{
			foreach(FTreeListViewItem Node in Item.Nodes)
			{
				...
				if(Index >= StartIndex)
				{
					if(Node == SelectedRow)
					{
						DrawSelectedRow(Gfx, Node, XOff, YOff);
					}
					else
					{
						DrawRow(Gfx, Node, XOff, YOff);
					}
				...
				if(Node.Expanded)
				{
					DrawNodeRows(Gfx, Node, StartIndex, ClientHeight, XOff, ref YOff, ref Index);
				}
			}
		}
		 ...
		 * @param	YOff	The Y offset of the row.
		 */
		private void DrawSelectedRow(Graphics Gfx, FTreeListViewItem Item, float XOff, float YOff)
		{
			RectangleF Rect = new RectangleF(XOff, YOff, ColumnCollection[0].Width, this.LineHeight);
			Rectangle GlyphRect = new Rectangle((int)Rect.X + ENTRY_PADLEFT + TREE_EXPANSION_GLYPH_PAD, (int)Rect.Y, TREE_EXPANSION_GLYPH_WIDTH, (int)Rect.Height);
			float XOffset = Rect.X + ENTRY_PADLEFT * 2 + TREE_EXPANSION_GLYPH_WIDTH;
			RectangleF GlyphOffsetRect = new RectangleF(XOffset, Rect.Y, Rect.Width - XOffset, Rect.Height);
			...
			int HalfLineHeight = (int)Rect.Height / 2;
			...
			DrawSelectedRowEntry(Gfx, Item.Text, ColumnCollection[0], ref GlyphOffsetRect);
			Rect.X += Rect.Width;
			...
			for(int i = 0; i < NumIterations; ++i)
			{
				...
				DrawSelectedRowEntry(Gfx, Item.SubItems[i].Text, Column, ref Rect);

				Rect.X += Rect.Width;
			}

			int LineX = (int)XOff + EXPANDED_LINE_X;
			if(Item.Parent != null)
			{
				Gfx.DrawLine(ExpandedItemLinePen, new Point(LineX, (int)YOff - HalfLineHeight), new Point(LineX, GlyphCenter.Y));
				...
				{
					GlyphCenter.X = (int)GlyphOffsetRect.X + (ENTRY_PADLEFT - EXPANDED_LINE_PAD_RIGHT);
					Gfx.DrawLine(ExpandedItemLinePen, GlyphCenter, new Point(LineX, GlyphCenter.Y));
				}
			}
		...
		/**
		 * Draws a column entry for a selected row.
		 * 
		 * @param	Gfx		The GDI+ graphics device.
		 ...
		 */
		private void DrawSelectedRowEntry(Graphics Gfx, string Text, FTreeListViewColumnHeader Column, ref RectangleF Rect)
		{
		 ...
		 * @param	YOff	The Y offset of the row.
		 */
		private void DrawRow(Graphics Gfx, FTreeListViewItem Item, float XOff, float YOff)
		{
			RectangleF Rect = new RectangleF(XOff, YOff, ColumnCollection[0].Width, this.LineHeight);
			Rectangle GlyphRect = new Rectangle((int)Rect.X + ENTRY_PADLEFT + TREE_EXPANSION_GLYPH_PAD, (int)Rect.Y, TREE_EXPANSION_GLYPH_WIDTH, (int)Rect.Height);
			float TextXOffset = Rect.X + ENTRY_PADLEFT * 2 + TREE_EXPANSION_GLYPH_WIDTH;
			RectangleF GlyphOffsetRect = new RectangleF(TextXOffset, Rect.Y, Rect.Width - TextXOffset, Rect.Height);
			int HalfLineHeight = (int)Rect.Height / 2;
			...
			DrawRowEntry(Gfx, Item.Text, ColumnCollection[0], ref GlyphOffsetRect, Item.BackColor, Item.ForeColor);
			...
			for(int i = 0; i < NumIterations; ++i)
			{
				...
				{
					DrawRowEntry(Gfx, Item.SubItems[i].Text, Column, ref Rect, Item.BackColor, Item.ForeColor);
				}
				else
				{
					DrawRowEntry(Gfx, Item.SubItems[i].Text, Column, ref Rect, Item.SubItems[i].BackColor, Item.SubItems[i].ForeColor);
				}
		...
		/**
		 * Draws a column entry for an unselected row.
		 * 
		 * @param	Gfx		The GDI+ graphics device.
		 * @param	Text	The text to draw.
		 * @param	Column	The column the entry belongs to.
		 * @param	Rect	The bounds of the row.
		 */
		private void DrawRowEntry(Graphics Gfx, string Text, FTreeListViewColumnHeader Column, ref RectangleF Rect, Color BackgroundColor, Color ForeGroundColor)
		{
			RectangleF StringRect = Rect;
			StringRect.X += ENTRY_PADLEFT;
			StringRect.Width -= ENTRY_PADLEFT;
		...
		protected override void OnMouseWheel(MouseEventArgs Args)
		{
			...
			{
				//120 is the resolution of current mice per tick
				int Ticks = -Args.Delta / 120;
				...
				if(Y >= HEADER_HEIGHT && Y <= ClientHeight)
				{
					...
					for(int RowOffset = 0; RowOffset < ViewableRows; ++RowOffset)
					{
						if(Rect.Contains(X, Y))
						{
							...
							ReturnItem = GetViewableRow(RowOffset);
							...
							if(ReturnItem != null)
							{
								...
								int LeftGlyphBound = (HorizontalScrollBar.Visible ? -HorizontalScrollBar.Value : 0) + ENTRY_PADLEFT + TREE_EXPANSION_GLYPH_PAD;
		 ...
		 * @param	Item			The parent item.
		 */
		void GetViewableSubNodes(ref int RowsToDraw, ref bool bFoundSubNode, ref FTreeListViewItem RowItem, int Index, FTreeListViewItem Item)
		{
			foreach(FTreeListViewItem Node in Item.Nodes)
			{
				if(bFoundSubNode)
				...
				{
					GetViewableSubNodes(ref RowsToDraw, ref bFoundSubNode, ref RowItem, Index, Node);
				}
			}
		}
		 ...
		 */
		FTreeListViewItem GetViewableRow(int Index)
		{
