...
namespace CookerTools
{
	...
	public class TOCInfo
	{
		...
		public TOCInfo(string InSourceFilename, string InDestinationFilename, string InConsoleFilename, int InSize, int InCompressedSize, int InStartSector, string InCRC, DateTime InLastWriteTime)
		{
	...
	public class CookerToolsClass
	{
		...
		/// </summary>
		private DLLInterface DLLInterface = null;

		/// <summary>
		/// Temporarily saving the platform name for Watson hack
		/// </summary>
		...
		/// Constructor
		/// </summary>
		/// <param name="InStartupPath">Path application was launched from</param>
		/// <param name="InOutputHandler">Interface to an object to output to</param>
		public CookerToolsClass(string InStartupPath, IOutputHandler InOutputHandler)
		{
		...
		{
			// make sure the given platform is allowed
			DLLInterface = new DLLInterface();
			if (DLLInterface.ActivatePlatform(Platform) == false)
			{
				return false;
			}
			...
			// some temp platform-specific code
			SavedPlatform = Platform;
			if (Platform == "Xenon")
			{
				TOCFilename = "Xbox360TOC.txt";
			}
			else
			{
				TOCFilename = Platform + "TOC.txt";
			}
		...
		/// </summary>
		/// <param name="GameName">Name of the game ("War")</param>
		...
		public void SetGameAndConsoleInfo(string InGameName, ArrayList InConsoleNames, string InBaseDirectory)
		{
		...
		public void SetGameAndConsoleInfo(string InGameName, ArrayList InConsoleNames, string InBaseDirectory, ArrayList InRequestedTargetPaths)
		{
			SetGameInfo(InGameName);
			SetConsoleInfo(InConsoleNames);
			SetBaseDirectory(InBaseDirectory);
		...
		/// <summary>
		/// Checks to see if the game has a set of specific splash screens
		/// </summary>
		/// <param name="SourceFolder">The folder to search</param>
		/// <param name="SourcePattern">the pattern to search for</param>
		/// <returns>true if there are files matching, false otherwise</returns>
		bool HasGameSpecificSplashScreens(string SourceFolder, string SourcePattern)
		{
		...
		private void TocAddFile( ArrayList TOC, FileInfo SrcFile, string SourceFolder, string DestinationFolder, string TargetSourceFolder )
		{
			...
			if( SrcFile.Exists )
			{
				...
				TOC.Add(new TOCInfo(SourceFile, DestinationFile, TargetFile, (int)SrcFile.Length, UncompressedSize, StartSector, "0", ExistingFile.LastWriteTimeUtc));
			}
			else
			{
				// In case the file is force added and doesn't exist - eg. the TOC file
				TOC.Add(new TOCInfo(SourceFile, DestinationFile, TargetFile, 0, UncompressedSize, StartSector, "0", System.DateTime.UtcNow));
			}
		}
		...
		private void TocIncludeFiles( ArrayList TOC, string SourceFolder, string SourcePattern, string DestinationFolder, bool bRecursive )
		{
			...
			try
			{
				...
				{
					TocAddFile( TOC, SrcFile, SourceFolder, DestinationFolder, TargetSourceFolder );
				}
				...
				if (bRecursive)
				{
					...
					foreach( DirectoryInfo Dir in Dirs )
					{
						...
						TocIncludeFiles(TOC, OriginalSourceFolder + Dir.Name + "\\", SourcePattern, OriginalDestinationFolder + Dir.Name + "\\" , bRecursive);
					}
				}
			}
			// gracefully handle copying from non existant folder.
		...
		private ArrayList TocBuild()
		{
			...
			// splash screens, including game specific ones.
			if (HasGameSpecificSplashScreens(CachedGameName + "Game\\Splash\\" + SavedPlatform + "\\","*.bmp"))
			{
				TocIncludeFiles( TOC, CachedGameName + "Game\\Splash\\" + SavedPlatform + "\\", "*.bmp", "Splash\\", false ); 
			}
			else
			{
				TocIncludeFiles( TOC, "Binaries\\" + SavedPlatform + "\\Splash\\", "*.bmp", "Splash\\", false );
			}
			
			// include the cooked inis
			TocIncludeFiles( TOC, CachedGameName + "Game\\Config\\" + SavedPlatform + "\\Cooked\\", "Coalesced.ini", null, false );
			...
			TocIncludeFiles( TOC, CachedGameName + "Game\\Localization\\", "Coalesced.*", null, false ); // recursive copy

			// Game specific files.
			TocIncludeFiles( TOC, CachedGameName + "Game\\Cooked" + SavedPlatform + "\\", "*.xxx", null, false );
			
			// @todo: Add a new .xml file for platform settings (or query DLL)
			TocIncludeFiles( TOC, "", CachedGameName + "Game-*.xex", null, false );
			TocIncludeFiles( TOC, "", CachedGameName + "Game-*.elf", null, false );

			if( CachedGameSettings != null && CachedGameSettings.SyncPaths != null )
			{
				foreach (PathInfo Info in CachedGameSettings.SyncPaths)
				{
					TocIncludeFiles( TOC, Info.Path, Info.Wildcard, null, Info.bIsRecursive );
				}
			}
			...
			TocAddFile( TOC, new FileInfo( TOCFilename ), SourceFolder, DestinationFolder, TargetSourceFolder );
		...
		private ArrayList TocRead( String TOCPath )
		{
			...
			{			
				...
				while( true )
				{
					...
					TOC.Add( new TOCInfo( "", "", ConsoleFilenname, Size, CompressedSize, StartSector, CRC, new DateTime(0) ) );
				}
		...
		private bool TargetSync( ArrayList TOC )
		{
			...
			OutputHandler.OutputText( "\r\n[SYNCING WITH CONSOLE STARTED]", Color.Green );
			DateTime StartTime = DateTime.UtcNow;
		...
		private bool WatsonAlreadyRunning( Process[] Processes, int ConsoleIndex )
		{
			string WindowTitle = "UnrealWatson " + ConnectedTargetNames[ConsoleIndex];
			foreach( Process aProcess in Processes )
			{
				if( aProcess.MainWindowTitle == WindowTitle )
				{
					return( true );
				}
			}
		...
		private void SpawnWatsonForConsole( string WorkingDirectory, int ConsoleIndex )
		{
			// Launch UnrealXbWatson only if it isn't already running
			ProcessStartInfo PSI = new ProcessStartInfo();

			PSI.WorkingDirectory = WorkingDirectory;
			PSI.FileName = WorkingDirectory + "\\Xenon\\UnrealWatson.exe";
			PSI.Arguments = ConnectedTargetNames[ConsoleIndex].ToString();
		...
		public bool TargetReboot( bool bShouldRunGame, string Configuration, string CommandLine )
		{
			...
			// Only spawn Watsons if we're running an xex
			// @hack: Only spawn watson for xbox
			if( bShouldRunGame && SavedPlatform == "Xenon")
			{
				string WorkingDirectory = System.Environment.CurrentDirectory;

				Process[] Processes = System.Diagnostics.Process.GetProcessesByName( "UnrealWatson" );

				// Spawn a Watson for each valid console
				foreach( int ConsoleIndex in ConnectedTargets )
				{
					if( !WatsonAlreadyRunning( Processes, ConsoleIndex ) )
					{
						SpawnWatsonForConsole( WorkingDirectory, ConsoleIndex );
					}
				}
			}
