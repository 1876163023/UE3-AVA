...
namespace Controller
{
    class P4
    {
        ...
        public void SyncToRevision( ScriptParser Builder, StreamWriter Log, string Revision )
        {
            Array Output;
            string Command;
            ...
            Command = "sync //depot/" + Builder.SourceControlBranch + "/..." + Revision;
                ...
                foreach( string Line in OpenedOutput )
                {
                    ...
                    if( Line.ToLower().StartsWith( "depotfile " ) )
                    {
                        ...
                        if( HaveRev != 0 && HeadRev != 0 )
                        {
                            if( HaveRev != HeadRev )
                            {
                                SubmitFileErrorList += "File not at head revision: '" + File + "'\r\n";
                                NeedResolve = true;
                            }
                        }
                        ...
                        if( OtherOpenCount > 0 )
                        {
                            if( OtherLocked )
                            {
                                SubmitFileErrorList += "File '" + File + "' is *LOCKED* by:\r\n";
                                ...
                                for( int i = 0; i < OtherOpenCount; i++ )
                                {
                                    ...
                                    foreach( string Stat in FStatOutput )
                                    {
                                        ...
                                        {
                                            SubmitFileErrorList += "\t" + Stat.Substring( Key.Length ) + "\r\n";
                                        }
                                    }
                                }
                            }
        ...
        public void TagFile( ScriptParser Builder, string FileName, StreamWriter Log )
        {
            ...
            // Get label depending on dependencies
            string Revision = Parent.GetLabelToSync().TrimStart( "@".ToCharArray() );
            ...
            {
                // Set the changelist description
                Builder.Write( Log, "Executing 'P4 tag -l " + Revision + " " + FileName + "'" );
                Output = Interface.run( "tag -l " + Revision + " " + FileName );
                Write( Builder, Log, Output );
            }
            catch( System.Runtime.InteropServices.COMException ex )
            {
