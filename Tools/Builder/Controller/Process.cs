...
namespace Controller
{
    class BuildProcess
    {
        ...
        private bool UpdatesActive = false;
        ...
        public BuildProcess( Main InParent, ScriptParser InBuilder, StreamWriter InLog, string InExecutable, string InCommandLine, string WorkingDirectory, bool StatusUpdates, bool InCaptureOutputDebugString )
        {
            Parent = InParent;
            Builder = InBuilder;
            Log = InLog;
            ...
            Builder.Write( Log, "Spawning: " + Executable + " " + CommandLine );
        ...
        public void Kill()
        {
            ...
            if( Name.IndexOf( ".com" ) >= 0 )
            {
                ...
                Process[] ChildProcesses = Process.GetProcessesByName( Name );
                foreach( Process ChildProcess in ChildProcesses )
                {
                    Parent.Log( " ... killing: '" + ChildProcess.ProcessName + "'", Color.Red );
                    ChildProcess.Kill();
                }

                if( Name.ToLower() == "devenv" )
                {
                    ChildProcesses = Process.GetProcessesByName( "cl_Incredibuild_Interop_0" );
                    foreach( Process ChildProcess in ChildProcesses )
                    {
                        Parent.Log( " ... killing: '" + ChildProcess.ProcessName + "'", Color.Red );
                        try
                    ...
                    ChildProcesses = Process.GetProcessesByName( "cl" );
                    foreach( Process ChildProcess in ChildProcesses )
                    {
                        Parent.Log( " ... killing: '" + ChildProcess.ProcessName + "'", Color.Red );
                        try
