...
#define FMOD_VERSION    0x00040427
...
[
    ...
    Some opening modes (ie FMOD_OPENUSER, FMOD_OPENMEMORY, FMOD_CREATECOMPRESSEDMEMSAMPLE, FMOD_OPENRAW) will need extra information.
    ...
    <b>Important!</b>  Specifying FMOD_CREATECOMPRESSEDMEMSAMPLE will POINT to your memory rather allocating 
    its own copy and duplicating it internally!
    <b><u>This means you cannot free the memory while FMOD is using it, until after Sound::release is called.</b></u>
    
    <b>Xbox 360 memory</b> On Xbox 360 Specifying FMOD_CREATECOMPRESSEDMEMSAMPLE to a virtual memory address will cause FMOD_ERR_INVALID_ADDRESS
...
#define FMOD_CREATECOMPRESSEDMEMSAMPLE 0x10000A00  /* Open MP2, MP3, IMAADPCM or XMA from memory and leave it compressed in memory, without FMOD duplicating the data.  "name_or_data" will be interpreted as a pointer to memory instead of filename for creating sounds.  Use FMOD_CREATESOUNDEXINFO to specify length.  During playback the FMOD software mixer will decode it in realtime as a 'compressed sample'.  Can only be used in combination with FMOD_SOFTWARE (or FMOD_HARDWARE with XADPCM on Xbox).  Overrides FMOD_CREATESAMPLE.  If the sound data is not ADPCM, MPEG or XMA it will behave as if it was created with FMOD_CREATESAMPLE and decode the sound into PCM. */
...
{                       
    ...
    int     maxADPCMcodecs;     /* For use with FMOD_CREATECOMPRESSEDSAMPLE only.  ADPCM codecs consume 26,040 bytes per instance (based on FSB encoded ADPCM block size) and this number will determine how many ADPCM channels can be played simultaneously.  Default = 32. */
    int     maxXMAcodecs;       /* For use with FMOD_CREATECOMPRESSEDSAMPLE only.  XMA   codecs consume 16,932 bytes per instance and this number will determine how many XMA channels can be played simultaneously.  Default = 32.  */
    int     ASIONumChannels;
