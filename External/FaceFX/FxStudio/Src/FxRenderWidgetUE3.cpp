...
UBOOL FFaceFXStudioViewportClient::InputAxis(FViewport* Viewport, INT ControllerId, FName Key, FLOAT Delta, FLOAT DeltaTime)
{
...
void FxRenderWidgetUE3::OnQueryRenderWidgetCaps( FxWidget* FxUnused(sender), FxRenderWidgetCaps& renderWidgetCaps )
{
	...
	renderWidgetCaps.renderWidgetVersion = 1000;
...
void FxRenderWidgetUE3::SetSkeletalMesh( USkeletalMesh* pSkeletalMesh )
{
	...
	if( PreviewSkelComp )
	{
		...
		//@todo Why is this needed so that animating material instance constant
		//      scalar parameters actually works?  The AddComponent() call above
		//      does a ConditionalAttach(), but apparently that isn't "good 
		//      enough."
...
void FxRenderWidgetUE3::Tick( FLOAT DeltaSeconds )
{
	if( PreviewSkelComp )
	{
		...
		{
			PreviewSkelComp->Animations->TickAnim(DeltaSeconds, 1.0f);
		}
